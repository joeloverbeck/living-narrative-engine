...
class PipelinePerformanceMetrics {
  ...
  initializeDefaultThresholds() {
    ...
    this.setStageThreshold('component_filtering', { maxDuration: 100, maxMemory: 50 * 1024 * 1024 });
    this.setStageThreshold('prerequisite_evaluation', { maxDuration: 200, maxMemory: 25 * 1024 * 1024 });
    this.setStageThreshold('multi_target_resolution', { maxDuration: 500, maxMemory: 100 * 1024 * 1024 });
    this.setStageThreshold('action_formatting', { maxDuration: 150, maxMemory: 30 * 1024 * 1024 });
    
    // Overall pipeline thresholds
    this.setStageThreshold('pipeline_total', { maxDuration: 1000, maxMemory: 200 * 1024 * 1024 });
  }
  ...
  startStageExecution(executionId, stageName, stageContext = {}) {
    ...
    if (!execution) {
      this.logger.warn(`Stage execution started for unknown pipeline: ${executionId}`);
      return null;
    }
  ...
  completeStageExecution(executionId, stageName, stageResult = {}) {
    ...
    if (!stageData) {
      this.logger.warn(`Stage completion for unknown stage: ${stageName} in ${executionId}`);
      return;
    }
    ...
    stageData.resourceUtilization = this.calculateResourceUtilization(stageData);
  ...
  completePipelineExecution(executionId, pipelineResult = {}) {
    const execution = this.activeExecutions.get(executionId);
    if (!execution) {
      this.logger.warn(`Pipeline completion for unknown execution: ${executionId}`);
      return;
    }
  ...
  analyzeStagePerformance(stageName, stageData) {
    ...
    // Check error rate
    const errorRate = stageData.result.errorCount / (stageData.result.itemsProcessed || 1);
  ...
  calculateThroughput(stageData) {
    ...
    const itemsPerSecond = (stageData.result.itemsProcessed / stageData.duration) * 1000;
    const bytesPerSecond = stageData.context.inputSize > 0 ? 
      (stageData.context.inputSize / stageData.duration) * 1000 : 0;
  ...
  calculateEfficiency(stageData) {
    ...
    const throughputScore = Math.min(stageData.throughput.itemsPerSecond / 100, 1.0); // Normalize
    const memoryEfficiency = stageData.memoryDelta > 0 ? 
      Math.min(1000000 / stageData.memoryDelta, 1.0) : 1.0; // Better with less memory
  ...
  calculateResourceUtilization(stageData) {
    return {
      ...
      ioEfficiency: stageData.context.inputSize > 0 ? 
        stageData.result.outputSize / stageData.context.inputSize : 0
    };
  }
  ...
  calculateStageBreakdown(execution) {
    ...
    breakdown.forEach(stage => {
      stage.percentage = totalStageTime > 0 ? 
        (stage.duration / totalStageTime) * 100 : 0;
    });
  ...
  generateOptimizationRecommendations(execution) {
    ...
    if (execution.totalDuration > 2000) { // >2 seconds
      recommendations.push({
    ...
    if (execution.totalMemoryDelta > 100 * 1024 * 1024) { // >100MB
      recommendations.push({
  ...
  analyzeTrends(execution) {
    const history = this.performanceHistory.get('pipeline') || [];
    if (history.length < 5) {
      return { trend: 'insufficient_data', message: 'Need more executions for trend analysis' };
    }

    const recent = history.slice(-5);
    const avgDuration = recent.reduce((sum, exec) => sum + exec.totalDuration, 0) / recent.length;
  ...
  calculatePerformanceRating(execution) {
    ...
    const memoryPenalty = Math.max(0, (execution.totalMemoryDelta - 50 * 1024 * 1024) / (10 * 1024 * 1024)); // Penalty after 50MB
    ...
    const throughputBonus = Math.min(execution.overallThroughput.itemsPerSecond / 100, 10);
  ...
  getPerformanceSummary() {
    ...
    if (pipelineHistory.length > 0) {
      ...
      summary.pipeline.averageDuration = recentHistory.reduce((sum, h) => sum + h.totalDuration, 0) / recentHistory.length;
      summary.pipeline.successRate = recentHistory.filter(h => h.success).length / recentHistory.length;
    }

    return summary;
  }

  getRecentAlerts(timeRange = 60 * 60 * 1000) { // Last hour
...
class MetricsAggregator {
  ...
  aggregateStageMetrics(metrics) {
    ...
    return {
      ...
      averageThroughput: this.calculateAverage(recent, 'throughput.itemsPerSecond'),
  ...
  calculateAverage(metrics, field) {
    ...
    const values = metrics.map(m => this.getNestedValue(m, field)).filter(v => v != null);
    return values.reduce((sum, v) => sum + v, 0) / values.length;
  }

  calculateMedian(metrics, field) {
    const values = metrics.map(m => this.getNestedValue(m, field)).filter(v => v != null).sort((a, b) => a - b);
  ...
  completePipelineExecution(pipelineResult = {}) {
    ...
    if (execution) {
      this.captureActionData('performance', 'pipeline_execution', {
        ...
        overallThroughput: execution.overallThroughput,
        stageBreakdown: execution.stageBreakdown,
        performanceRating: this.performanceMetrics.calculatePerformanceRating(execution),
        timestamp: new Date().toISOString()
      });
    }
...
describe('PipelinePerformanceMetrics', () => {
  ...
  describe('Pipeline Execution Tracking', () => {
    it('should track complete pipeline execution', async () => {
      ...
      // Start pipeline
      const pipelineExecution = metrics.startPipelineExecution(executionId, context);
      ...
      metrics.startStageExecution(executionId, 'component_filtering', { inputSize: 1000 });
      ...
      const completedStage = metrics.completeStageExecution(executionId, 'component_filtering', stageResult);
      ...
      const completedPipeline = metrics.completePipelineExecution(executionId, pipelineResult);
    ...
    it('should handle stage execution errors', async () => {
      ...
      const completedStage = metrics.completeStageExecution(executionId, 'test_stage', stageResult);
  ...
  describe('Performance Analysis', () => {
    it('should identify bottlenecks', () => {
      const execution = {
        ...
        totalDuration: 1000,
        stageBreakdown: [
          { stageName: 'slow_stage', duration: 600, percentage: 60, efficiency: { overall: 0.2 } },
          { stageName: 'fast_stage', duration: 400, percentage: 40, efficiency: { overall: 0.8 } }
        ]
      };
    ...
    it('should generate optimization recommendations', () => {
      ...
      const recommendations = metrics.generateOptimizationRecommendations(execution);

      expect(recommendations.length).toBeGreaterThan(0);
      expect(recommendations.some(r => r.type === 'general_optimization')).toBe(true);
      expect(recommendations.some(r => r.type === 'memory_optimization')).toBe(true);
    });
  ...
  describe('Metrics Aggregation', () => {
    it('should aggregate stage metrics', () => {
      ...
      const aggregated = metrics.metricsAggregator.aggregateStageMetrics(stageMetrics);
    ...
    it('should provide performance summary', () => {
      ...
      metrics.performanceHistory.set('pipeline', [
        { totalDuration: 500, success: true, timestamp: new Date().toISOString() }
      ]);
  ...
  describe('Threshold Management', () => {
    it('should detect threshold violations', () => {
      ...
      metrics.setStageThreshold(stageName, { maxDuration: 100, maxMemory: 1000 });
