...
## Dependencies: PHASE1-TICKET1, PHASE1-TICKET3
...
/** @typedef {import('../../../scopeDsl/scopeInterpreter.js').ScopeInterpreter} ScopeInterpreter */
...
/** @typedef {import('../../../scopeDsl/utils/targetContextBuilder.js').TargetContextBuilder} TargetContextBuilder */
...
export class MultiTargetResolutionStage extends PipelineStage {
  ...
  /**
   * @param {Object} deps
   * @param {ScopeInterpreter} deps.scopeInterpreter
  ...
  async executeInternal(context, trace) {
    ...
    trace?.step(`Resolving targets for action '${actionDef.id}'`, 'MultiTargetResolutionStage');
  ...
  #isLegacyAction(actionDef) {
    return typeof actionDef.targets === 'string' || 
  ...
  async #resolveLegacyTarget(context, trace) {
    ...
    trace?.step(`Resolving legacy scope '${scope}'`, 'MultiTargetResolutionStage');
    ...
    if (targetContexts.length === 0) {
      trace?.info('No targets found for legacy action', 'MultiTargetResolutionStage');
  ...
  async #resolveMultiTargets(context, trace) {
    ...
    trace?.info(`Target resolution order: ${resolutionOrder.join(', ')}`, 'MultiTargetResolutionStage');
    ...
    for (const targetKey of resolutionOrder) {
      ...
      trace?.step(`Resolving ${targetKey} target`, 'MultiTargetResolutionStage');
  ...
  #buildScopeContext(actor, actionContext, resolvedTargets, targetDef, trace) {
    ...
    const baseContext = this.#contextBuilder.buildBaseContext(
      actor.id,
      actionContext.location?.id || actor.getComponent('core:position')?.locationId
    );
  ...
  async #resolveScope(scope, context, trace) {
    try {
      ...
      trace?.info(`Scope resolved to ${entityIds.length} entities`, 'MultiTargetResolutionStage');
      
      return entityIds;
      
    } catch (error) {
      this.logger.error(`Error evaluating scope '${scope}':`, error);
      trace?.failure(`Scope evaluation failed: ${error.message}`, 'MultiTargetResolutionStage');
      return [];
    }
  }
  ...
  #getEntityDisplayName(entityId) {
    try {
      ...
      const name = entity.getComponent('core:description')?.name ||
...
### Step 2: Create Target Context Builder

Create file: `src/scopeDsl/utils/targetContextBuilder.js`

```javascript
/**
 * @file Builds context objects for scope DSL evaluation
 */

// Type imports
/** @typedef {import('../../entities/entityManager.js').default} EntityManager */
/** @typedef {import('../../game/gameStateManager.js').default} GameStateManager */
/** @typedef {import('../../logging/consoleLogger.js').default} ILogger */

import { validateDependency } from '../../utils/validationUtils.js';

/**
 * Builds context objects for scope DSL evaluation with multi-target support
 */
export class TargetContextBuilder {
  ...
  constructor({ entityManager, gameStateManager, logger }) {
    validateDependency(entityManager, 'IEntityManager');
    validateDependency(gameStateManager, 'IGameStateManager');
  ...
  buildBaseContext(actorId, locationId) {
    ...
    if (!actor) {
      throw new Error(`Actor entity not found: ${actorId}`);
    }
    
    // Location is optional
    let location = null;
    if (locationId) {
      location = this.#buildEntityContext(locationId);
      if (!location) {
        this.#logger.warn(`Location entity not found: ${locationId}`);
        // Create minimal location context
        location = { id: locationId, components: {} };
      }
    } else {
      // Try to get location from actor's position
      const position = this.#entityManager.getEntity(actorId)?.getComponent('core:position');
      if (position?.locationId) {
        location = this.#buildEntityContext(position.locationId) || 
                   { id: position.locationId, components: {} };
      } else {
        // Fallback to empty location
        location = { id: 'unknown', components: {} };
      }
    }
   ...
   * @param {Object} resolvedTargets - Already resolved targets keyed by name
   * @param {Object} targetDef - Current target definition
  ...
  buildDependentContext(baseContext, resolvedTargets, targetDef) {
    ...
    if (Object.keys(resolvedTargets).length > 0) {
      ...
      // Convert resolved targets to context format
      for (const [key, targets] of Object.entries(resolvedTargets)) {
        context.targets[key] = targets.map(target => 
          this.#buildEntityContext(target.id)
        ).filter(Boolean);
      }
    }
   ...
   * @returns {Object|null} Entity context or null if not found
   */
  #buildEntityContext(entityId) {
    try {
      ...
      this.#logger.error(`Error building entity context for ${entityId}:`, error);
      return null;
    }
  }
  ...
  #buildGameContext() {
    try {
      ...
      this.#logger.error('Error building game context:', error);
      // Return minimal game context
      return {
        turnNumber: 0
      };
    }
...
export function registerActionServices(container) {
  ...
  // Register target context builder
  container.register(ACTION_TOKENS.ITargetContextBuilder, TargetContextBuilder, {
    dependencies: [
      tokens.IEntityManager,
      tokens.IGameStateManager,
      tokens.ILogger
    ]
  });
  
  // Register multi-target resolution stage
  container.register(ACTION_TOKENS.IMultiTargetResolutionStage, MultiTargetResolutionStage, {
    dependencies: [
...
describe('MultiTargetResolutionStage', () => {
  ...
  describe('Multi-Target Resolution', () => {
    ...
    it('should resolve dependent targets with context', async () => {
      ...
      mockDeps.targetContextBuilder.buildBaseContext.mockReturnValue(baseContext);
      
      // First resolution (primary)
      mockDeps.scopeInterpreter.evaluate
        .mockResolvedValueOnce(['npc1']);
      
      mockDeps.entityManager.getEntity
        .mockReturnValue({ id: 'npc1', getComponent: jest.fn() });
      ...
      mockDeps.targetContextBuilder.buildDependentContext.mockReturnValue(dependentContext);
      ...
      expect(result.shouldContinue).toBe(true);
      expect(mockDeps.targetContextBuilder.buildDependentContext).toHaveBeenCalledWith(
        baseContext,
    ...
    it('should handle optional targets', async () => {
      ...
      mockDeps.entityManager.getEntity
        .mockReturnValue({ id: 'target1', getComponent: jest.fn() });
