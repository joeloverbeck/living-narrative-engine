...
describe('State Management Integration', () => {
  ...
  describe('Enhanced Error Handling', () => {
    it('should use executeWithErrorHandling for service operations', async () => {
      const executeWithErrorHandlingSpy = jest.spyOn(controller, '_executeWithErrorHandling');
    ...
    it('should handle service errors with retry logic', async () => {
      ...
      expect(mockCharacterBuilderService.getAllCharacterConcepts).toHaveBeenCalledTimes(3);
    });
    
    it('should display error message after retries exhausted', async () => {
      const error = new Error('Persistent service error');
      mockCharacterBuilderService.getAllCharacterConcepts.mockRejectedValue(error);
