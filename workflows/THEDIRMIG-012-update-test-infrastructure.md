...
describe('ThematicDirectionsManagerController', () => {
  ...
  describe('Constructor', () => {
    it('should initialize with base controller dependencies', () => {
      ...
      expect(controller.characterBuilderService).toBe(testBase.mocks.characterBuilderService);
  ...
  describe('Lifecycle Methods', () => {
    ...
    it('should load initial data during initialization', async () => {
      ...
      expect(testBase.mocks.characterBuilderService.getAllThematicDirectionsWithConcepts)
        .toHaveBeenCalled();
    });
  });
  ...
  describe('State Management', () => {
    it('should show empty state when no directions', async () => {
      ...
      expect(testBase.mocks.uiStateManager.setState)
    ...
    it('should show results state when directions exist', async () => {
      ...
      expect(testBase.mocks.uiStateManager.setState)
        .toHaveBeenCalledWith(UI_STATES.RESULTS);
    });
    ...
    it('should show loading state during data fetch', async () => {
      ...
      // Should show loading
      expect(testBase.mocks.uiStateManager.setState)
        .toHaveBeenCalledWith(UI_STATES.LOADING);
...
describe('ThematicDirectionsManagerController Integration', () => {
  ...
  describe('Cross-Component Integration', () => {
    it('should integrate with event bus for updates', async () => {
      ...
      // Verify UI updated
      const directionElement = document.querySelector('[data-direction-id="1"]');
...
describe('ThematicDirectionsManagerController - Base Controller Integration', () => {
  ...
  describe('Base Controller Features', () => {
    it('should inherit base controller lifecycle', async () => {
      ...
      // Verify order
      expect(lifecycleSpy.cache.mock.invocationCallOrder[0])
        .toBeLessThan(lifecycleSpy.events.mock.invocationCallOrder[0]);
    });
    ...
    it('should leverage base controller state management', async () => {
      ...
      expect(testBase.mocks.uiStateManager.setState).toHaveBeenCalledWith('loading');
      
      controller._showError('Test error');
      expect(testBase.mocks.uiStateManager.setState).toHaveBeenCalledWith('error');
      
      controller._showState('results');
      expect(testBase.mocks.uiStateManager.setState).toHaveBeenCalledWith('results');
    });
...
it('should call lifecycle hooks in correct order', async () => {
  ...
  expect(callOrder).toEqual([
    'preInit', 'cache', 'events', 'services', 'data', 'ui', 'post'
  ]);
});
```
