...
describe('Trait Generation Performance', () => {
  ...
  it('should generate traits for complex character within 5 seconds', async () => {
    ...
    performance.measure(
      'complex-trait-generation',
      'complex-generation-start',
      'complex-generation-end'
    );
...
describe('UI Responsiveness', () => {
  ...
  it('should handle input validation without blocking UI', async () => {
    ...
    const isValid = await controller._validateCharacterDefinition(
  ...
  it('should render results without UI freezing', async () => {
    ...
    const displayData = enhancer.enhanceForDisplay(
      largeTraitSet,
      'Test Character'
    );
...
describe('Memory Efficiency', () => {
  ...
  it('should efficiently manage large character definitions', async () => {
    ...
    await processor.processResponse(
      veryLargeResponse,
      testBed.getStandardCharacterData()
    );
...
describe('Concurrency Performance', () => {
  it('should handle 5 concurrent generations efficiently', async () => {
    ...
    const promises = Array(5)
      .fill(null)
      .map(() => generator.generateRewrittenTraits(character));
  ...
  it('should maintain response quality under concurrent load', async () => {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      expect(results[i].characterName).toBe(
        differentCharacters[i]['core:name'].text
      );

      // Results should be different from each other
      for (let j = i + 1; j < results.length; j++) {
        expect(results[i].rewrittenTraits).not.toEqual(
          results[j].rewrittenTraits
        );
      }
    }
  });
...
describe('Token Estimation Performance', () => {
  it('should estimate tokens quickly for various input sizes', async () => {
    ...
    for (const input of testInputs) {
      ...
      performance.measure(
        'token-estimation',
        'token-estimation-start',
        'token-estimation-end'
      );
