...
export class ClichesGeneratorController extends BaseCharacterBuilderController {
  ...
  _cacheElements() {
    ...
    this.#directionDisplay = document.getElementById('selected-direction-display');
    ...
    this._elements.directionContent = document.getElementById('direction-content');
    ...
    this.#loadingOverlay = document.getElementById('loading-overlay') || this.#createLoadingOverlay();
  ...
  async _loadInitialData() {
    try {
      ...
      const directions = await this._services.characterBuilderService.getAllThematicDirections();
      
      if (!directions || directions.length === 0) {
        this._showEmptyState('No thematic directions found. Please create some first.');
        return;
      }
      ...
      this._showInfoMessage('Select a thematic direction to view or generate clichés');
      
    } catch (error) {
      this._logger.error('Failed to load initial data:', error);
      this._showErrorState('Failed to load thematic directions. Please refresh the page.');
    }
  }
  ...
  async #organizeDirectionsByConcept(directions) {
    ...
    for (const direction of directions) {
      if (!conceptMap.has(direction.conceptId)) {
        ...
        const concept = await this._services.characterBuilderService.getCharacterConcept(
          direction.conceptId
        );
  ...
  #extractConceptTitle(conceptText) {
    ...
    const title = (firstLine.length < firstSentence.length ? firstLine : firstSentence).trim();
  ...
  #populateDirectionSelector(organizedData) {
    ...
    this.#directionSelector.innerHTML = '<option value="">-- Choose a thematic direction --</option>';
  ...
  async #handleDirectionSelection(directionId) {
    ...
    try {
      ...
      const hasCliches = await this._services.characterBuilderService.hasClichesForDirection(
        directionId
      );
      
      if (hasCliches) {
        // Load and display existing clichés
        const cliches = await this._services.characterBuilderService.getClichesByDirectionId(
          directionId
        );
  ...
  async #handleGenerateCliches() {
    ...
    try {
      ...
      this._showLoadingState('Generating clichés... This may take a few moments.');
      
      // Clear any existing error messages
      this._clearMessages();
      
      // Generate clichés
      const cliches = await this._services.characterBuilderService.generateClichesForDirection(
        this.#currentConcept,
      ...
      this._showSuccessMessage(`Generated ${cliches.getTotalCount()} clichés successfully!`);
  ...
  #displayCliches(cliches) {
    ...
    if (displayData.tropesAndStereotypes && displayData.tropesAndStereotypes.length > 0) {
      html += `
    ...
    return `
      <div class="cliche-category" data-category="${category.id}">
        ...
        <ul class="cliche-list">
          ${category.items
            .map(item => `<li class="cliche-item">${this._escapeHtml(item)}</li>`)
            .join('')}
        </ul>
      </div>
    `;
...
describe('ClichesGeneratorController', () => {
  ...
  describe('Initialization', () => {
    it('should load directions on init', async () => {
      ...
      expect(mockServices.characterBuilderService.getAllThematicDirections)
        .toHaveBeenCalled();
    });
  });
