...
describe('ThematicDirectionsManagerController Performance', () => {
  ...
  describe('Filtering Performance', () => {
    ...
    it('should debounce filter input efficiently', async () => {
      ...
      console.log(
        `Debounced filtering completed in: ${(endTime - startTime).toFixed(2)}ms`
      );
    });
  });
...
async function testController(ControllerClass, mocks, itemCount, label) {
  ...
  mocks.characterBuilderService.getAllThematicDirectionsWithConcepts.mockResolvedValue(
    generateDirections(itemCount)
  );
...
function compareResults(original, migrated) {
  ...
  for (const metric of metrics) {
    ...
    console.log(
      `  ${metric}: ${percent}% ${percent > 0 ? 'slower' : 'faster'} ${status}`
    );
  }
  ...
  console.log(
    `  Memory: ${memPercent}% ${memDiff > 0 ? 'more' : 'less'} ${memStatus}`
  );
}
...
function generateSummary(results) {
  ...
  for (const scenario of Object.keys(results.original)) {
    ...
    for (const metric of metrics) {
      const percent =
        ((migrated[metric] - original[metric]) / original[metric]) * 100;
...
describe('ThematicDirectionsManagerController Memory Profile', () => {
  ...
  it('should not leak memory during repeated operations', async () => {
    ...
    console.log(
      `  External growth trend: ${(externalGrowth.trend * 100).toFixed(1)}%`
    );
    console.log(
      `  Final heap size: ${(memorySnapshots[iterations - 1].heapUsed / 1024 / 1024).toFixed(2)}MB`
    );
  });
