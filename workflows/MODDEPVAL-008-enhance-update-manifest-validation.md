...
async function updateModManifest(modName, options = {}) {
  ...
  if (opts.validateReferences && !opts.skipValidationOnDryRun) {
    try {
      ...
      violationReporter = container.resolve(tokens.IViolationReporter);
    } catch (error) {
  ...
  try {
    ...
    if (error instanceof ValidationError) {
      result.validation.performed = true;
      result.validation.violations = error.validationResult?.violations || [];
      
      if (opts.validationFormat !== 'none' && violationReporter) {
...
async function performManifestUpdate(modName, modPath, manifestPath, opts) {
  ...
  const newManifest = await buildManifest(modName, existingManifest, scanResult, opts);
...
export { updateModManifest, performManifestUpdate, runValidation, ValidationError };
...
```json
{
  "scripts": {
    ...
    "fix-mod-violations": "npm run update-manifest --validate --strict --format=console",
    "validate-mod": "npm run update-manifest --validate --dry-run --format=console",
    "report-mod-violations": "npm run update-manifest --validate --format=json --output=validation-reports/{modName}-violations.json"
  }
}
```
...
// Add new validation-aware functions
export {
  ...existingFunctions,
  updateModManifest,
...
describe('UpdateManifest - Validation Integration', () => {
  ...
  describe('Validation Integration', () => {
    ...
    it('should fail on violations when strictMode enabled', async () => {
      ...
      await expect(updateModManifest('test-mod', {
        validateReferences: true,
    ...
    it('should generate validation reports in different formats', async () => {
      ...
      const reportExists = await fs.access('test-report.json').then(() => true).catch(() => false);
