...
class ActionDiscoveryService {
  ...
  constructor({ 
    ...
    if (actionAwareTraceFactory) {
      validateDependency(actionAwareTraceFactory, 'IActionAwareTraceFactory', null, {
  ...
  async getValidActions(actorEntity, baseContext, options = {}) {
    ...
    try {
      ...
      if (trace) {
        ...
        this.#logger.debug(
          `Created ${traceType} for actor ${actorEntity.id}`,
          { actorId: actorEntity.id, traceType }
        );
      }
      ...
      if (trace?.withSpanAsync) {
        return await trace.withSpanAsync(
          ...
          { actorId: actorEntity.id },
          async (span) => {
            span.info(`Discovering actions for actor: ${actorEntity.id}`, source);
            return await this.#executePipeline(actorEntity, baseContext, trace, options);
          }
        );
      }
  ...
  async #createTraceContext(actorId, baseContext, options) {
    try {
      ...
      this.#logger.debug(
        ...
        { actorId, actionTracingEnabled, hasActionAwareFactory: !!this.#actionAwareTraceFactory }
      );
  ...
  #isActionTracingEnabled() {
    try {
      ...
      this.#logger.warn('Error checking action tracing status, assuming disabled', error);
      return false;
    }
  }
...
export function registerActionsServices(container) {
  ...
  container.register(
    ...
    (deps) => {
      ...
      const actionTracingAvailable = !!(actionAwareTraceFactory && actionTraceFilter);
      logger.info(
...
class ActionDiscoveryServiceTracingValidator {
  ...
  constructor({ logger }) {
    this.#logger = ensureValidLogger(logger, 'ActionDiscoveryServiceTracingValidator');
  }
  ...
  validateConfiguration(discoveryService) {
    ...
    if (!status.available) {
      ...
      if (!status.hasFactory) {
        issues.push('ActionAwareTraceContextFactory is not registered or available');
      }
      
      warnings.push('Action tracing will not be available for ActionDiscoveryService');
    }

    // Check enabled state
    if (status.available && !status.enabled) {
      warnings.push('Action tracing is available but disabled in configuration');
    }
...
export class ActionDiscoveryServiceTracingTestHelper {
  ...
  constructor(discoveryService) {
    ...
    this.#originalGetValidActions = discoveryService.getValidActions.bind(discoveryService);
  }
  ...
  enableTracingForTest(testConfig = {}) {
    ...
    this.#discoveryService.getValidActions = async (actorEntity, baseContext, options = {}) => {
      const enhancedOptions = {
        ...options,
        trace: true // Force tracing for test
      };

      const result = await this.#originalGetValidActions(actorEntity, baseContext, enhancedOptions);
...
describe('ActionDiscoveryService - Action Tracing Enhancement', () => {
  ...
  describe('Trace Context Creation', () => {
    ...
    it('should fall back to standard trace when ActionAwareTrace creation fails', async () => {
      ...
      expect(result.success).toBe(true);
      expect(testBed.getCreatedTraceType()).toBe('StructuredTrace');
      expect(testBed.getWarningLogs()).toContain('Failed to create trace context');
    });
  ...
  describe('Backward Compatibility', () => {
    it('should work identically to original service when tracing is disabled', async () => {
      ...
      const standardResult = await standardService.getValidActions(actor, baseContext);
      const enhancedResult = await enhancedService.getValidActions(actor, baseContext);
...
export class ActionDiscoveryServiceTestBed {
  ...
  createDiscoveryService(options = {}) {
    ...
    const mockTraceContextFactory = this.createMockTraceContextFactory(traceContextFactoryFailure);
    ...
    return {
      create: jest.fn().mockImplementation(async (options) => {
        ...
        const mockTrace = {
          ...
          step: jest.fn(),
          info: jest.fn(),
          withSpanAsync: jest.fn().mockImplementation(async (name, context, callback) => {
