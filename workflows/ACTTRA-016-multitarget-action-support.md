...
class MultiTargetTraceCapture {
  ...
  captureMultiTargetAction(action, trace, context) {
    ...
    try {
      ...
      const resolutionResults = this.traceTargetResolution(action, context, trace);
  ...
  analyzeTargetSpecification(action) {
    ...
    if (action.targets) {
      ...
      specification.staticTargets = Array.isArray(action.targets) ? action.targets : [action.targets];
      specification.expectedTargetCount = specification.staticTargets.length;
      specification.complexity = specification.staticTargets.length > 10 ? 'medium' : 'low';
    }
    ...
    if (action.scope || action.targetScope) {
      ...
      specification.scopeExpressions = Array.isArray(scopeExpr) ? scopeExpr : [scopeExpr];
      specification.complexity = this.evaluateScopeComplexity(specification.scopeExpressions);
    }
  ...
  traceTargetResolution(action, context, trace) {
    ...
    if (action.targets) {
      const staticResults = this.traceStaticTargetResolution(action.targets, trace);
    ...
    if (action.targetQuery || action.dynamicTargets) {
      const dynamicResults = this.traceDynamicTargetResolution(action, context, trace);
    ...
    const uniqueTargets = this.deduplicateTargets(resolutionResults.resolvedTargets);
    const validationResults = this.validateResolvedTargets(uniqueTargets, action, trace);
  ...
  traceScopeTargetResolution(scopeExpression, context, trace) {
    ...
    const scopeExpressions = Array.isArray(scopeExpression) ? scopeExpression : [scopeExpression];
    ...
    for (let i = 0; i < scopeExpressions.length; i++) {
      ...
      try {
        ...
        const scopeTargets = Array.isArray(evaluationResult) ? evaluationResult : [evaluationResult];
  ...
  analyzeScopeExpression(expression) {
    ...
    if (typeof expression === 'string') {
      ...
      if (analysis.operators.includes('[') || analysis.operators.includes('{')) {
  ...
  assessQueryCacheability(query) {
    ...
    if (typeof query === 'string' && !query.includes('random') && !query.includes('time')) {
      return 'high';
    }
    return 'low';
  }
...
class ActionAwareStructuredTrace extends StructuredTrace {
  ...
  isMultiTargetAction(action) {
    ...
    const hasMultipleStaticTargets = Array.isArray(action.targets) && action.targets.length > 1;
    const hasScopeExpression = !!(action.scope || action.targetScope);
    const hasDynamicTargets = !!(action.targetQuery || action.dynamicTargets);
    const hasTargetTypes = Array.isArray(action.targetTypes) && action.targetTypes.length > 1;
    
    return hasMultipleStaticTargets || hasScopeExpression || hasDynamicTargets || hasTargetTypes;
  }
  ...
  captureMultiTargetProcessing(action, context) {
    ...
    try {
      ...
      const results = this.multiTargetCapture.captureMultiTargetAction(action, this, context);
  ...
  getTargetResolutionSummary() {
    ...
    summary.averageTargetsPerAction = summary.totalActionsProcessed > 0 
      ? totalTargets / summary.totalActionsProcessed 
      : 0;
    summary.averageResolutionTime = summary.totalActionsProcessed > 0 
    ...
    targetSpecifications.forEach(spec => {
      ...
      summary.resolutionMethods[method] = (summary.resolutionMethods[method] || 0) + 1;
    });
  ...
  captureActionData(category, type, data) {
    ...
    if (category === 'multi_target' && !this.shouldCaptureMultiTargetData(type)) {
      return;
    }
...
describe('MultiTargetTraceCapture', () => {
  ...
  describe('Target Resolution Tracing', () => {
    it('should trace static target resolution', () => {
      ...
      mockEntityManager.getEntity.mockImplementation(id => ({ id, type: 'character' }));
    ...
    it('should trace scope target resolution', () => {
      ...
      const result = capture.traceScopeTargetResolution(scopeExpression, context, mockTrace);
  ...
  describe('Target Relationship Analysis', () => {
    it('should analyze parent-child relationships', () => {
      ...
      const relationships = capture.analyzeTargetRelationships(targets, mockTrace);

      expect(relationships.relationships).toHaveLength(2); // parent-child + sibling
      expect(relationships.relationships[0].relationship.type).toBe('parent_child');
  ...
  describe('Scope Expression Analysis', () => {
    ...
    it('should analyze complex scope expressions', () => {
      const expression = 'actor.followers[].items[{"==": [{"var": "type"}, "weapon"]}]';
  ...
  describe('Target Validation', () => {
    it('should validate resolved targets', () => {
      ...
      const result = capture.validateResolvedTargets(targets, action, mockTrace);
  ...
  describe('Performance Optimization', () => {
    it('should handle large target sets efficiently', () => {
      const largeTargetList = Array.from({ length: 200 }, (_, i) => `target${i}`);
      const action = { id: 'large-action', targets: largeTargetList };
      const context = {};
      
      mockEntityManager.getEntity.mockImplementation(id => ({ id, type: 'entity' }));

      const startTime = performance.now();
      const result = capture.captureMultiTargetAction(action, mockTrace, context);
