...
export function updateMovementLock(entityManager, entityId, locked) {
  ...
  if (bodyComponent && bodyComponent.body && bodyComponent.body.root) {
    ...
    if (bodyComponent.body.parts) {
      for (const [partType, partId] of Object.entries(bodyComponent.body.parts)) {
        const movementComponent = entityManager.getComponentData(partId, 'core:movement');
        if (movementComponent) {
          // Clone and update the movement component
          const updatedMovement = typeof structuredClone === 'function'
  ...
  if (existing) {
    const move = typeof structuredClone === 'function'
