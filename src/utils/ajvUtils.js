  ...
  if (errors.length > 50 && data) {
    ...
    if (operationType) {
      ...
      const relevantErrors = errors.filter(error => {
        ...
        return !error.schemaPath.includes('/anyOf/') || 
               error.schemaPath.includes(`/${operationType}`);
      });
      
      if (relevantErrors.length > 0 && relevantErrors.length < errors.length / 2) {
    ...
    if (typeErrors.length > 0) {
      ...
      const validTypes = typeErrors.map(e => e.params?.allowedValue).filter(Boolean);
      ...
      if (actualType) {
        ...
        const typeIndex = typeErrors.findIndex(e => e.params?.allowedValue === actualType);
        if (typeIndex >= 0 && typeIndex < typeErrors.length - 1) {
          ...
          const relevantSlice = nextTypeIndex > 0 
    ...
    return `Warning: ${errors.length} validation errors detected (likely cascading from anyOf validation).\\n` +
