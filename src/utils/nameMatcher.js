...
export function matchNames(candidates, phrase, logger) {
    logger.debug(`NameMatcher.matchNames called with phrase: "${phrase}", ${candidates?.length || 0} candidates.`);
    ...
    if (!Array.isArray(candidates) || candidates.length === 0) {
        logger.debug(`NameMatcher.matchNames: No candidates provided to match against phrase "${phrase}".`);
        return {
    ...
    for (const candidate of candidates) {
        if (!candidate || typeof candidate.id !== 'string' || typeof candidate.name !== 'string' || candidate.name.trim() === "") {
            logger.warn(`NameMatcher.matchNames: Skipping invalid candidate: ${JSON.stringify(candidate)}`);
            continue;
        }
    ...
    logger.debug(`NameMatcher.matchNames - Phrase: "${normalizedPhrase}" - Exact: ${exactMatches.length}, StartsWith: ${startsWithMatches.length}, Substring: ${substringMatches.length}`);

    /** @type {MatcherResult} */
    let result;

    const formatAmbiguousErrorMessage = (matchedItems, term, type) => {
        const ambiguousNames = matchedItems.map(c => `"${c.name}"`).slice(0, 3).join(', ');
    ...
    if (exactMatches.length === 1) {
        logger.debug(`NameMatcher.matchNames: Unique exact match found: ID "${exactMatches[0].id}", Name "${exactMatches[0].name}".`);
        result = {
            status: ResolutionStatus.FOUND_UNIQUE,
            target: exactMatches[0]
        };
    } else if (exactMatches.length > 1) {
        logger.debug(`NameMatcher.matchNames: Ambiguous exact matches found for "${normalizedPhrase}". Count: ${exactMatches.length}.`);
        result = {
        ...
        logger.debug(`NameMatcher.matchNames: Unique startsWith match found: ID "${startsWithMatches[0].id}", Name "${startsWithMatches[0].name}".`);
        result = {
            status: ResolutionStatus.FOUND_UNIQUE,
            target: startsWithMatches[0]
        };
    } else if (startsWithMatches.length > 1) {
        logger.debug(`NameMatcher.matchNames: Ambiguous startsWith matches found for "${normalizedPhrase}". Count: ${startsWithMatches.length}.`);
        result = {
            status: ResolutionStatus.AMBIGUOUS,
            target: null,
            candidates: startsWithMatches,
            error: formatAmbiguousErrorMessage(startsWithMatches, phrase, 'item starting with')
        };
    } else if (substringMatches.length === 1) {
        logger.debug(`NameMatcher.matchNames: Unique substring match found: ID "${substringMatches[0].id}", Name "${substringMatches[0].name}".`);
        result = {
            status: ResolutionStatus.FOUND_UNIQUE,
            target: substringMatches[0]
        };
    } else if (substringMatches.length > 1) {
        logger.debug(`NameMatcher.matchNames: Ambiguous substring matches found for "${normalizedPhrase}". Count: ${substringMatches.length}.`);
        result = {
            status: ResolutionStatus.AMBIGUOUS,
            target: null,
            candidates: substringMatches,
            error: formatAmbiguousErrorMessage(substringMatches, phrase, 'item containing')
        };
    } else {
        logger.debug(`NameMatcher.matchNames: No matches found for phrase "${normalizedPhrase}".`);
        result = {
            status: ResolutionStatus.NOT_FOUND,
            target: null,
            error: `Nothing found to match "${phrase}".`
