...
export function getEntityDisplayName(entity, logger) {
    if (!entity) {
        logger?.debug("EntityUtils.getEntityDisplayName: Received null or undefined entity.");
        return undefined;
    }
    ...
    if (typeof entity.name === 'string' && entity.name.trim() !== '') {
        logger?.debug(`EntityUtils.getEntityDisplayName: Entity '${entity.id}' using fallback entity.name property ('${entity.name}') as '${NAME_COMPONENT_ID}' was not found or invalid.`);
        return entity.name;
    }

    logger?.warn(`EntityUtils.getEntityDisplayName: Entity '${entity.id}' has no usable name from '${NAME_COMPONENT_ID}' or entity.name.`);
    return undefined;
}