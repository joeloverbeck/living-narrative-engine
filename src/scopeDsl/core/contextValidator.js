...
class ContextValidator {
   ...
   */
  constructor(criticalProperties = ['actorEntity', 'runtimeCtx', 'dispatcher', 'cycleDetector', 'depthGuard']) {
  ...
  validate(context) {
    ...
    if (missingProperties.length > 0) {
      throw new Error(`[CRITICAL] Context is missing required properties: ${missingProperties.join(', ')}`);
    }
  ...
  _validatePropertyTypes(context) {
    ...
    if (context.dispatcher && typeof context.dispatcher.resolve !== 'function') {
      throw new Error('[CRITICAL] dispatcher must have a resolve method');
    }

    // depth should be a non-negative number
    if (context.depth !== undefined && (typeof context.depth !== 'number' || context.depth < 0)) {
    ...
    if (context.cycleDetector) {
      if (typeof context.cycleDetector.enter !== 'function' || typeof context.cycleDetector.leave !== 'function') {
        throw new Error('[CRITICAL] cycleDetector must have enter and leave methods');
      }
    }
