...
export class CoverageTracingEnhancer {
  ...
  constructor({
    ...
    validateDependency(
      traceFormatter,
      'ICoverageResolutionTraceFormatter',
      logger,
      {
  ...
  enhanceSlotAccessResolver(slotAccessResolver, config) {
    ...
    enhancedResolver.canResolve =
      slotAccessResolver.canResolve.bind(slotAccessResolver);

    // Wrap the resolve method with structured tracing
    const originalResolve = slotAccessResolver.resolve.bind(slotAccessResolver);
    enhancedResolver.resolve = this.#createTracingWrapper(
      originalResolve,
      config
    );
  ...
  #createTracingWrapper(originalResolve, config) {
    return (node, ctx) => {
      ...
      if (
        config.scopeDslTracing.coverageResolution.enabled &&
        this.#shouldTrace(node, ctx)
      ) {
        ...
        if (this.#isValidStructuredTrace(ctx.structuredTrace)) {
          return this.#resolveWithStructuredTracing(
            originalResolve,
            node,
            ctx,
            config
          ...
          this.#logger.warn(
            'Existing structured trace is invalid, creating new one'
          );
  ...
  #resolveWithStructuredTracing(originalResolve, node, ctx, config) {
    ...
    this.#logger.debug(
      `Started coverage resolution span for slot: ${node.field}`
    );

    let performanceMarker = null;
    if (performanceMonitor) {
      performanceMarker = performanceMonitor.startMeasurement(
        'coverage_resolution'
      );
    }
    ...
    try {
      ...
      const result = this.#interceptResolutionPhases(
        originalResolve,
        node,
        ctx,
        span,
      ...
      const allSpans = structuredTrace.getSpans
        ? structuredTrace.getSpans()
        : [];
      const finalSelectionSpan = allSpans.find(
        (s) =>
      ...
      if (finalSelectionSpan) {
        ...
        if (
          finalSelectionAttrs.selectedItem &&
          finalSelectionAttrs.selectedItem !== 'none'
        ) {
      ...
      this.#logger.debug(
        `Coverage resolution completed successfully for slot: ${node.field}, results: ${result.size}`
      );

      return result;
    } catch (error) {
      this.#logger.error(
        `Coverage resolution error for slot ${node.field}:`,
        error
      );
      ...
      this.#logger.debug(
        `Ended coverage resolution span for slot: ${node.field}`
      );
    }
  }
