...
export class AITurnHandler extends BaseTurnHandler {
    ...
    constructor({
        ...
        if (!gameWorldAccess) throw new Error("AITurnHandler: Missing gameWorldAccess");
        if (!illmAdapter) throw new Error("AITurnHandler: Invalid ILLMAdapter");
        if (!commandProcessor) throw new Error("AITurnHandler: Invalid ICommandProcessor");
        if (!commandOutcomeInterpreter) throw new Error("AITurnHandler: Invalid ICommandOutcomeInterpreter");
        if (!safeEventDispatcher) throw new Error("AITurnHandler: Invalid ISafeEventDispatcher");
        if (!subscriptionManager) throw new Error("AITurnHandler: Invalid SubscriptionLifecycleManager");
        if (!entityManager) throw new Error("AITurnHandler: Invalid IEntityManager");
        if (!actionDiscoverySystem) throw new Error("AITurnHandler: Invalid IActionDiscoverySystem");
        if (!promptBuilder) throw new Error("AITurnHandler: Invalid IPromptBuilder");
        if (!aiPlayerStrategyFactory) throw new Error("AITurnHandler: Invalid IAIPlayerStrategyFactory");
        if (!turnContextFactory) throw new Error("AITurnHandler: Invalid ITurnContextFactory");
        if (!gameStateProvider) throw new Error("AITurnHandler: Invalid IAIGameStateProvider");
        if (!promptContentProvider) throw new Error("AITurnHandler: Invalid IAIPromptContentProvider");
        if (!llmResponseProcessor) throw new Error("AITurnHandler: Invalid ILLMResponseProcessor");
        ...
        this._logger.debug(`${this.constructor.name} initialized successfully with injected dependencies.`);
    }
    ...
    _getAIIsAwaitingExternalEventFlag() {
        ...
        if (this.#aiIsAwaitingExternalEvent && this.#aiAwaitingExternalEventForActorId !== currentActorInContext?.id) {
            this._logger.warn(`${this.constructor.name}._getAIIsAwaitingExternalEventFlag: Flag true for ${this.#aiAwaitingExternalEventForActorId}, context actor ${currentActorInContext?.id}.`);
        }
        return false;
    }
    ...
    async startTurn(actor) {
        ...
        if (!this._currentState) {
            ...
            if (fallbackInitialState) {
                this._logger.warn(`${this.constructor.name}.startTurn: Attempting to set initial state again.`);
                this._setInitialState(fallbackInitialState);
                if (!this._currentState) throw new Error("AITurnHandler: _currentState is null, and recovery failed.");
            } else {
                throw new Error("AITurnHandler: _currentState is null, and turnStateFactory failed to provide a state.");
            }
        }
        await this._currentState.startTurn(this, actor);
        this._logger.info(`${this.constructor.name}.startTurn: Turn for AI actor ${actor.id} delegated.`);
    }
    ...
    _resetTurnStateAndResources(logContext = 'N/A') {
        ...
        this.#aiIsAwaitingExternalEvent = false;
        this.#aiAwaitingExternalEventForActorId = null;
        this._logger.debug(`${this.constructor.name}: AI 'isAwaitingExternalEvent' flags reset. Context: '${logContext}'.`);
    }
    ...
    async destroy() {
        ...
        if (this.#illmAdapter && typeof this.#illmAdapter.cancelOngoingOperations === 'function') {
            try {
                ...
                this._logger.warn(`${this.constructor.name}: Error cancelling ILLMAdapter ops: ${e.message}`, e);
            }
        }
        this.#aiIsAwaitingExternalEvent = false;
        this.#aiAwaitingExternalEventForActorId = null;
