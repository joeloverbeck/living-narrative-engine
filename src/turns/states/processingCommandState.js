    ...
    if (shouldEndTurn) {
      if (turnCtx && typeof turnCtx.endTurn === 'function') {
        ...
        if (actorToEndTurnFor && actorToEndTurnFor.id) {
          // Ensure we have a valid actor ID from context
          logger.info(
            `${this.getStateName()}: Ending turn for actor ${actorToEndTurnFor.id} due to processing exception.`
          );
          try {
            await turnCtx.endTurn(error); // Pass the original error
          } catch (endTurnError) {
            logger.error(
              `${this.getStateName()}: Error calling turnCtx.endTurn() for ${actorToEndTurnFor.id}: ${endTurnError.message}`,
              endTurnError
            );
            ...
            if (
              ...
              logger.warn(
                `${this.getStateName()}: Resetting handler for actor ${actorToEndTurnFor.id} due to error in turnCtx.endTurn().`
              );
              await this._handler._resetTurnStateAndResources(
          ...
          logger.warn(
            `${this.getStateName()}: Cannot end turn via ITurnContext: endTurn method is available but no valid actor found in context (current actor ID: ${currentActorIdForLog}). Attempting handler reset.`
          );
          if (
            this._handler?._resetTurnStateAndResources &&
            this._handler?._transitionToState
            ...
            await this._handler._resetTurnStateAndResources(
              `exception-no-actor-to-end-${this.getStateName()}`
            );
            await this._handler._transitionToState(
              new TurnIdleState(this._handler)
            );
        ...
        logger.warn(
          `${this.getStateName()}: Cannot end turn for actor ${currentActorIdForLog}: ITurnContext or its endTurn method is unavailable. Attempting handler reset.`
        );
        if (
          this._handler?._resetTurnStateAndResources &&
          this._handler?._transitionToState
          ...
          logger.error(
            `${this.getStateName()}: CRITICAL - Cannot end turn OR reset handler for ${currentActorIdForLog}. System may be unstable.`
          );
        }
      }
    } else {
