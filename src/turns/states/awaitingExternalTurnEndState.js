...
export class AwaitingExternalTurnEndState extends AbstractTurnState {
    ...
    async exitState(handler, nextState) {
        ...
        await this.#performUnsubscribe(logger, `exitState-for-${actorIdForLog}-to-${nextState?.getStateName() ?? 'None'}`);
        ...
        if (turnCtx && typeof turnCtx.isAwaitingExternalEvent === 'function' && typeof turnCtx.setAwaitingExternalEvent === 'function') {
            try {
                if (turnCtx.isAwaitingExternalEvent()) {
                    ...
                    logger.debug(`${this.getStateName()}: Clearing awaiting external event flag for actor ${actorIdToClear} on exit via ITurnContext.`);
                    turnCtx.setAwaitingExternalEvent(false, actorIdToClear);
                } else {
                    const currentActorInCtx = turnCtx.getActor();
                    const actorIdToCheck = currentActorInCtx?.id ?? actorIdForLog;
                    logger.debug(`${this.getStateName()}: Awaiting external event flag was already false for actor ${actorIdToCheck} on exit.`);
                }
            } catch (flagErr) {
                const actorIdInError = turnCtx.getActor()?.id ?? actorIdForLog;
                logger.warn(`${this.getStateName()}: Failed to clear awaiting external event flag for ${actorIdInError} on exit: ${flagErr.message}`, flagErr);
            }
            ...
            logger.debug(`${this.getStateName()}: ITurnContext not available on exit for actor ${actorIdForLog} during handler destruction. Flag clearing might have been handled by state.destroy or is no longer possible.`);
        } else if (!turnCtx) {
    ...
    async handleTurnEndedEvent(handler, payload) {
        ...
        if (!turnCtx) {
            ...
            fallbackLogger.warn(`${this.getStateName()}: ${TURN_ENDED_ID} received, but no active ITurnContext. Payload for ${payload?.entityId}. Instance ${this._instanceId || 'N/A'} ignoring and cleaning up.`);
            await this.#performUnsubscribe(fallbackLogger, `no-context-on-event-for-${payload?.entityId}`);
            return;
        }
        ...
        if (!currentActorInContext) {
            logger.warn(`${this.getStateName()}: ${TURN_ENDED_ID} received for payload actor ${payload?.entityId}, but no actor in current ITurnContext. Instance ${this._instanceId || 'N/A'} cleaning up subscription and ignoring.`);
        ...
        if (!turnCtx.isAwaitingExternalEvent()) {
            logger.warn(`${this.getStateName()}: ${TURN_ENDED_ID} for ${payloadActorId} (context actor ${waitingActorId}) received, but ITurnContext is no longer awaiting an external event. May have been handled, timed out, or state exited/destroyed. Cleaning up subscription.`);
        ...
        try {
            ...
            logger.error(`${this.getStateName()}: Error calling turnCtx.endTurn for actor ${payloadActorId}: ${e.message}`, e);
            // If endTurn itself fails, we're in a bad spot.
            // Attempt to clean up this state's subscription at least.
            await this.#performUnsubscribe(logger, `endTurn-failed-for-${payloadActorId}`);
    ...
    async handleSubmittedCommand(handler, commandString, actorEntityFromSubscriptionCallback) {
        ...
        if (!turnCtx) {
            ...
            handler._handleTurnEnd(actorEntityFromSubscriptionCallback?.id || `unknown-actor-no-ctx-cmd-${this.getStateName()}`, new Error(msg));
            return;
        }
    ...
    async destroy(handler) {
        ...
        } else if (typeof handler.getCurrentActor === 'function') { // Fallback to handler's actor
        ...
        logger.info(`${this.getStateName()}: State instance destroy() called for actor ${actorIdForLog}. Performing state-specific cleanup. Handler is being destroyed: ${handler._isDestroyed}.`);

        // 1. Ensure unsubscription happens. This is the primary responsibility of this state's destroy.
        await this.#performUnsubscribe(logger, `state-destroy-for-${actorIdForLog}`);
        ...
        if (turnCtx && typeof turnCtx.isAwaitingExternalEvent === 'function' && typeof turnCtx.setAwaitingExternalEvent === 'function') {
            if (turnCtx.isAwaitingExternalEvent()) {
                try {
                    ...
                    logger.debug(`${this.getStateName()} (destroy): Clearing awaiting external event flag for ${actorIdInCtx} via ITurnContext during state destruction.`);
                    turnCtx.setAwaitingExternalEvent(false, actorIdInCtx);
                } catch (flagErr) {
                    logger.warn(`${this.getStateName()} (destroy): Failed to clear awaiting flag for ${turnCtx.getActor()?.id ?? actorIdForLog} during state destruction: ${flagErr.message}`);
                }
            }
        } else if (handler._isDestroyed) { // If no context but handler is being destroyed
            logger.debug(`${this.getStateName()} (destroy): ITurnContext not available for actor ${actorIdForLog} during state destruction (handler is being destroyed). Flag clearing likely handled or not possible.`);
        } else { // No context, and not clear if handler is being destroyed (should be rare if called from handler.destroy)
            logger.warn(`${this.getStateName()} (destroy): ITurnContext not available for actor ${actorIdForLog} during state destruction. Cannot clear awaiting flag via context.`);
        }
        ...
        logger.debug(`${this.getStateName()}: State-specific destroy handling for actor ${actorIdForLog} complete.`);
    }
