...
export class AIGameStateProvider extends IAIGameStateProvider {
  ...
  async _getAvailableActions(actor, turnContext, locationSummary, logger) {
    ...
    try {
      if (typeof turnContext.getActionDiscoverySystem === 'function') {
        ...
        if (turnContext && typeof turnContext.getEntityManager === 'function') {
          try {
            ...
            logger.warn(
              `AIGameStateProvider (_getAvailableActions): Error accessing EntityManager for action context: ${e.message}. Actions requiring EM may not be discovered correctly.`
            );
            entityManagerForActions = null;
          }
        } else {
          logger.warn(
            `AIGameStateProvider (_getAvailableActions): turnContext.getEntityManager is not a function. Actions requiring EM may not be discovered correctly.`
          );
          entityManagerForActions = null;
        }
        ...
        const actionCtx = {
          currentLocation: positionComponent?.locationId
            ? { id: positionComponent.locationId }
            : null,
      ...
      logger.error(
        `AIGameStateProvider: Error while discovering actions for actor ${actor.id}: ${adsErr.message}`,
        {
          error: adsErr,
          stack: adsErr.stack,
        }
