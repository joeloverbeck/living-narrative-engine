...
export default class RoundManager {
  ...
  async startRound(strategy = 'round-robin') {
    ...
    this.#logger.debug(`Found ${actors.length} actors to start the round: ${actorIds.join(', ')}`);

    // Start the new round in the service
    await this.#turnOrderService.startNewRound(actors, strategy);
    this.#inProgress = true;
    
    this.#logger.debug(`Successfully started a new round with ${actors.length} actors using the '${strategy}' strategy.`);
  }
