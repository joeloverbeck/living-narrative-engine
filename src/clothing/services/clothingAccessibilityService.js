...
export class ClothingAccessibilityService {
    ...
    return {
      getComponentData: (entityId, componentId) => {
        try {
          ...
          this.#logger.debug(
            'Entity instance lookup failed, trying definition fallback',
            {
              entityId,
              componentId,
              error: error.message,
  ...
  #getEquipmentState(entityId) {
    try {
      ...
      this.#logger.warn('Failed to get equipment state', {
        entityId,
        error: error.message,
      });
      return {};
  ...
  #applyCoverageBlocking(items, entityId, equipment, mode) {
    ...
    const shouldApplyBlocking =
      mode === 'topmost' || mode === 'topmost_no_accessories';
        ...
        if (blocking.blockedSlots) {
          ...
          if (targetSlot && targetLayer) {
            for (const rule of blocking.blockedSlots) {
              if (
                rule.slot === targetSlot &&
                rule.layers.includes(targetLayer)
              ) {
                this.#logger.debug(
  ...
  #calculateItemPriority(item, context = 'removal') {
    ...
    const layerValue =
      LAYER_PRIORITY_WITHIN_COVERAGE[item.layer] ||
  ...
  #sortByPriority(items, context) {
    ...
    return itemsWithPriority.map(
      ({ priority: _priority, originalIndex: _originalIndex, ...item }) => item
    );
  }
  ...
  getAccessibleItems(entityId, options = {}) {
    ...
    if (equippedItems.length === 0) {
      this.#logger.debug(
        'ClothingAccessibilityService: No equipped items found'
      );
    ...
    this.#logger.debug(
      'ClothingAccessibilityService: After coverage blocking',
      {
        accessibleItems,
      }
    );
