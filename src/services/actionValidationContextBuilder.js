...
import {EXITS_COMPONENT_ID, POSITION_COMPONENT_ID} from '../constants/componentIds.js'; // Adjust path if necessary
// --- END FIX ---
...
export class ActionValidationContextBuilder {
    ...
    constructor({entityManager, logger}) {
        if (!entityManager?.getEntityInstance || typeof entityManager.getComponentData !== 'function') { // Added check for getComponentData
            throw new Error('ActionValidationContextBuilder requires a valid EntityManager with getEntityInstance and getComponentData methods.');
        }
    ...
    buildContext(actionDefinition, actor, targetContext) {
        ...
        if (targetContext.type === 'entity' && targetContext.entityId) {
            ...
            if (targetEntityInstance) {
                ...
                this.#logger.warn(`ActionValidationContextBuilder: Target entity '${targetContext.entityId}' not found for action '${actionDefinition.id}'. Context will have null target entity data.`);
            }
        }
        ...
        if (targetContext.type === 'direction' && targetContext.direction) {
            ...
            const actorPositionData = this.#entityManager.getComponentData(actor.id, POSITION_COMPONENT_ID);
            const actorLocationId = actorPositionData?.locationId;

            if (actorLocationId && typeof actorLocationId === 'string') {
                this.#logger.debug(`ActionValidationContextBuilder: Actor '${actor.id}' is at location '${actorLocationId}'. Fetching exits for direction '${targetContext.direction}'.`);
                const exitsData = this.#entityManager.getComponentData(actorLocationId, EXITS_COMPONENT_ID);

                if (Array.isArray(exitsData)) {
                    const matchedExit = exitsData.find(exit => exit && exit.direction === targetContext.direction);
                    if (matchedExit) {
                        this.#logger.debug(`ActionValidationContextBuilder: Found matching exit for direction '${targetContext.direction}':`, matchedExit);
                        ...
                        this.#logger.warn(`ActionValidationContextBuilder: Direction '${targetContext.direction}' not found in ${EXITS_COMPONENT_ID} data for location '${actorLocationId}'.`);
                    }
                } else {
                    this.#logger.warn(`ActionValidationContextBuilder: No valid ${EXITS_COMPONENT_ID} data (or not an array) found for location '${actorLocationId}'.`);
                }
            } else {
                this.#logger.warn(`ActionValidationContextBuilder: Could not determine actor's current location (actor ID: '${actor.id}') to fetch exits for direction '${targetContext.direction}'. Missing or invalid ${POSITION_COMPONENT_ID} component data.`);
            }
        }
