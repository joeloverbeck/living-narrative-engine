...
export default class ShortTermMemoryService {
     ...
     * @param {number}               [options.defaultMaxEntries=50]  Fallback when mem.maxEntries is missing/invalid.
     */
    ...
    addThought(mem, newText, now = new Date()) {
        if (!mem || typeof mem !== 'object') {
            throw new TypeError('mem must be an object conforming to core:short_term_memory schema');
        }
        ...
        // 4. Trim to capacity ---------------------------------------------------
        const max = Number.isInteger(mem.maxEntries) && mem.maxEntries > 0
            ? mem.maxEntries
