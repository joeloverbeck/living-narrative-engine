...
export class HttpConfigurationProvider extends IConfigurationProvider {
     ...
     * @param {ILogger} [options.logger=console] - An ILogger instance for logging. Defaults to the global console.
     */
    constructor(options = {}) {
    ...
    async fetchData(sourceUrl) {
        if (!sourceUrl || typeof sourceUrl !== 'string' || sourceUrl.trim() === '') {
            const errorMessage = 'HttpConfigurationProvider: sourceUrl must be a non-empty string.';
            this.#logger.error(errorMessage);
            throw new Error(errorMessage);
        }

        this.#logger.info(`HttpConfigurationProvider: Attempting to load configurations from ${sourceUrl}`);

        try {
            const response = await fetch(sourceUrl);

            if (!response.ok) {
                const errorStatusText = response.statusText || `HTTP status ${response.status}`;
                this.#logger.error(`HttpConfigurationProvider: Failed to fetch configuration from ${sourceUrl}. Status: ${response.status} ${errorStatusText}`);
                throw new Error(`Failed to fetch configuration file from ${sourceUrl}: ${errorStatusText}`);
            }
            ...
            try {
                ...
                this.#logger.error(`HttpConfigurationProvider: Failed to parse JSON response from ${sourceUrl}.`, {error: parseError instanceof Error ? parseError.message : String(parseError)});
                // @ts-ignore
                throw new Error(`Failed to parse configuration data from ${sourceUrl} as JSON: ${parseError.message}`);
            }

            this.#logger.info(`HttpConfigurationProvider: Successfully fetched and parsed configuration from ${sourceUrl}.`);
            return jsonData;
            ...
            if (error instanceof Error && (error.message.startsWith('Failed to fetch configuration file') || error.message.startsWith('Failed to parse configuration data'))) {
                // These specific errors are already logged with good detail, so just rethrow.
                throw error;
            }

            const errorMessage = error instanceof Error ? error.message : String(error);
            this.#logger.error(`HttpConfigurationProvider: Error loading or parsing configuration from ${sourceUrl}. Detail: ${errorMessage}`, {error});
            throw new Error(`Could not load configuration from ${sourceUrl}: ${errorMessage}`);
        }
    }
}
