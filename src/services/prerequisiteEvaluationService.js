...
export class PrerequisiteEvaluationService {
    ...
    // --- START: Refactor-AVS-3.3.1 ---
    // No change needed here for 3.3.2
    /** @private @type {ActionValidationContextBuilder} */
     ...
     */
    constructor({logger, jsonLogicEvaluationService, actionValidationContextBuilder}) { // Added actionValidationContextBuilder in 3.3.1
        // --- Constructor Guards ---
        // No changes needed here for 3.3.2
        if (!logger?.debug || !logger?.error || !logger.warn) {
            throw new Error('PrerequisiteEvaluationService requires a valid ILogger instance.');
        }
        if (!jsonLogicEvaluationService?.evaluate) {
            throw new Error('PrerequisiteEvaluationService requires a valid JsonLogicEvaluationService instance.');
        }
        // --- START: Refactor-AVS-3.3.1 ---
        if (!actionValidationContextBuilder?.buildContext) { // Added Guard in 3.3.1
            throw new Error('PrerequisiteEvaluationService requires a valid ActionValidationContextBuilder instance.');
        }
        // --- END: Refactor-AVS-3.3.1 ---
        ...
        this.#logger.info('PrerequisiteEvaluationService initialised (with ActionValidationContextBuilder).');
    }
     ...
     */
    evaluate(prerequisites, actionDefinition, actor, targetContext) { // Signature updated in 3.3.1
        ...
        if (!prerequisites || prerequisites.length === 0) {
            ...
            this.#logger.debug(`PrereqEval[${actionId}]: → PASSED (No prerequisites to evaluate).`); // AC6
        ...
        try {
            ...
            this.#logger.debug(`PrereqEval[${actionId}]: Evaluation Context Built Successfully.`); // AC6

        } catch (buildError) { // Task: Wrap in try...catch, Handle errors in catch(err) // AC3
            // Task: Log error detailing failure (include required context) and return false // AC4
            this.#logger.error(`PrereqEval[${actionId}]: ← FAILED (Internal Error: Failed to build evaluation context via ActionValidationContextBuilder). Error: ${buildError.message}`, {
                actorId: actorId,
        ...
        // --- START: Refactor-AVS-3.3.2 ---
        // Task: Update Logging (Use local actionId)
        this.#logger.debug(`PrereqEval[${actionId}]: Evaluating ${prerequisites.length} prerequisite rule(s)...`); // AC6
        // --- END: Refactor-AVS-3.3.2 ---
        for (const [index, prereqObject] of prerequisites.entries()) {
            const ruleNumber = index + 1;

            // Basic structural check of the prerequisite object itself
            if (!prereqObject || typeof prereqObject !== 'object' || Array.isArray(prereqObject)) {
                // --- START: Refactor-AVS-3.3.2 ---
                // Task: Update Logging (Use local actionId)
                this.#logger.error(
            ...
            if (!ruleLogic || typeof ruleLogic !== 'object') { // JsonLogic rules are objects
                // --- START: Refactor-AVS-3.3.2 ---
                // Task: Update Logging (Use local actionId)
                this.#logger.error(
            ...
            try {
                ...
                // --- START: Refactor-AVS-3.3.2 ---
                // Task: Update Logging (Use local actionId)
                this.#logger.error(`PrereqEval[${actionId}]: ← FAILED (Rule ${ruleNumber}/${prerequisites.length}): Error during JsonLogic evaluation. Rule: ${JSON.stringify(prereqObject)}`, {
            ...
            if (!pass) {
                // --- START: Refactor-AVS-3.3.2 ---
                // Task: Update Logging (Use local actionId)
                this.#logger.debug(`PrereqEval[${actionId}]: ← FAILED (Rule ${ruleNumber}/${prerequisites.length}): Prerequisite check FAILED. Rule: ${JSON.stringify(prereqObject)}`); // AC6
                // --- END: Refactor-AVS-3.3.2 ---
                ...
                // --- START: Refactor-AVS-3.3.2 ---
                // Task: Update Logging (Use local actionId)
                this.#logger.debug(`PrereqEval[${actionId}]:   - Prerequisite Rule ${ruleNumber}/${prerequisites.length} PASSED: ${JSON.stringify(prereqObject)}`); // AC6
                // --- END: Refactor-AVS-3.3.2 ---
            }
        }
        ...
        // --- START: Refactor-AVS-3.3.2 ---
        // Task: Update Logging (Use local actionId)
        this.#logger.debug(`PrereqEval[${actionId}]: → PASSED (All ${prerequisites.length} prerequisite rules evaluated successfully).`); // AC6
        // --- END: Refactor-AVS-3.3.2 ---
