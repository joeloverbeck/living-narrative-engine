...
class AjvSchemaValidator {
    ...
    constructor(logger) {
        ...
        try {
            ...
            addFormats(this.#ajv);
            this.#logger.debug('AjvSchemaValidator: Ajv instance created and formats added.');
    ...
    async addSchema(schemaData, schemaId) {
        if (!this.#ajv) {
            ...
            return Promise.reject(new Error('AjvSchemaValidator: Ajv instance not available.'));
        }
        ...
        try {
            ...
            const rejectionError =
                error instanceof Error
                    ? error
                    : new Error(`Failed to add schema '${keyToRegister}': ${String(error)}`);
