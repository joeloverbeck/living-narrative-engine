...
class DefaultPathResolver extends IPathResolver {
    ...
    constructor(configuration) {
        ...
        for (const m of requiredMethods) {
            if (typeof configuration[m] !== 'function') {
                throw new Error(`${serviceName} requires a valid ${configInterface} with method ${m}().`);
            }
        }
     ...
     */
    #join(...segments) {
        const relevant = segments.filter(s => s !== null && s !== undefined && s !== '');
        let path = relevant
            .map((seg, i) => (i < relevant.length - 1 ? String(seg).replace(/\/+$/, '') : String(seg)))
            .join('/');
    ...
    resolveSchemaPath(filename) {
        ...
        return this.#join(this.#config.getBaseDataPath(),
    ...
    resolveRulePath(filename) {
        ...
        if (typeof this.#config.getRuleBasePath !== 'function') {
            throw new Error('Configuration service does not provide getRuleBasePath().');
        }
        return this.#join(this.#config.getBaseDataPath(),
