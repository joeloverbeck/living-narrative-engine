...
class EffectExecutionService {
    ...
    async executeEffects(effects, context) {
        ...
        for (const effectData of effects) {
            const handler = this.#handlers.get(effectData.effect_type);

            if (handler) {
                aggregatedMessages.push({text: `Executing effect: ${effectData.effect_type}`, type: 'internal'});
                try {
                    // Await in case handlers become async in the future
                    const result = await handler(effectData.effect_params, context);
                    ...
                    console.error(`EffectExecutionService: Error executing effect handler '${effectData.effect_type}' for ${context.itemName}:`, error);
                    aggregatedMessages.push({
                ...
                console.warn(`EffectExecutionService: No registered effect handler found for type: ${effectData.effect_type} in item ${context.itemName}. Skipping effect.`);
                aggregatedMessages.push({
