...
function findPotentialExitMatches(context, rawPhrase, logger) {
    ...
    if (!currentLocation || !entityManager) {
        logger.error('exitResolver: context missing currentLocation or entityManager');
        return results;
    }
    ...
    for (const exit of exitsData) {
        ...
        const data = {direction: dir, targetLocationEntity: targetLoc, blockerEntity: blockerEnt};
...
export async function resolveTargetExit(
    ...
    if (!bus || !logger) {
        console.error('exitResolver: context missing validatedEventDispatcher or logger');
        return null;
    }
    ...
    if (directionMatches.length > 1) {
        ...
        await bus.dispatchValidated('textUI:display_message', {
            text: TARGET_MESSAGES.AMBIGUOUS_DIRECTION
                ? TARGET_MESSAGES.AMBIGUOUS_DIRECTION(phrase, directionMatches.map(d => d.direction))
    ...
    if (nameMatches.length > 1) {
        const disp = nameMatches.map(m => getDisplayName(m.targetLocationEntity) || m.direction).join(', ');
        await bus.dispatchValidated('textUI:display_message', {
            text: TARGET_MESSAGES.TARGET_AMBIGUOUS_CONTEXT
                ? TARGET_MESSAGES.TARGET_AMBIGUOUS_CONTEXT(actionVerb, phrase, nameMatches.map(m => m.targetLocationEntity))
