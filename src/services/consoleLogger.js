...
class ConsoleLogger {
     ...
     * @param {string | number} [logLevelInput=LogLevel.INFO] - The desired initial log level.
    ...
    constructor(logLevelInput = LogLevel.INFO) {
        ...
        if (this.#currentLogLevel <= LogLevel.INFO) {
            ...
            console.info(`[ConsoleLogger] Initialized. Log level set to ${this.getLogLevelName(this.#currentLogLevel)} (${this.#currentLogLevel}).`);
        }
    }
    ...
    setLogLevel(logLevelInput) {
        ...
        if (typeof logLevelInput === 'string') {
            ...
            if (upperCaseLevel in LOG_LEVEL_MAP) {
                ...
                console.warn(`[ConsoleLogger] Invalid log level string: '${logLevelInput}'. Using previous or default: ${this.getLogLevelName(this.#currentLogLevel)}.`);
                newLevel = this.#currentLogLevel; // Keep current level if new one is invalid
            }
        } else if (typeof logLevelInput === 'number' && Object.values(LogLevel).includes(logLevelInput)) {
            newLevel = logLevelInput;
            changed = true;
        } else {
            console.warn(`[ConsoleLogger] Invalid log level input type: '${typeof logLevelInput}', value: '${logLevelInput}'. Using previous or default: ${this.getLogLevelName(this.#currentLogLevel)}.`);
        ...
        if (this.#currentLogLevel !== newLevel && changed) {
            ...
            console.info(`[ConsoleLogger] Log level changing from ${this.getLogLevelName(this.#currentLogLevel)} to ${this.getLogLevelName(newLevel)}.`);
