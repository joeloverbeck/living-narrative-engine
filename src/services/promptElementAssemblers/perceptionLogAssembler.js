...
export class PerceptionLogAssembler extends IPromptElementAssembler {
    ...
    assemble(elementConfig, promptData, placeholderResolver, allPromptElementsMap) {
        if (!elementConfig || !promptData || !placeholderResolver || !allPromptElementsMap) {
            this.#logger.error('PerceptionLogAssembler.assemble: Missing required parameters.', {
        ...
        const wrapperPrefix = placeholderResolver.resolve(elementConfig.prefix || "", promptData);
        const wrapperSuffix = placeholderResolver.resolve(elementConfig.suffix || "", promptData);
        ...
        if (!perceptionLogArray || !Array.isArray(perceptionLogArray) || perceptionLogArray.length === 0) {
            this.#logger.debug(`PerceptionLogAssembler: Perception log array for '${elementConfig.key}' missing or empty in PromptData. Skipping wrapper content assembly.`);
            // Return wrapper prefix/suffix only if they are non-empty, otherwise empty string.
            return (wrapperPrefix || wrapperSuffix) ? `${wrapperPrefix}${wrapperSuffix}` : "";
        }
        ...
        if (!pLogEntryConfig) {
            this.#logger.warn(`PerceptionLogAssembler: Missing '${PERCEPTION_LOG_ENTRY_KEY}' config for perception log (wrapper key: '${elementConfig.key}'). Entries will not be formatted with specific prefix/suffix.`);
        ...
        for (const entry of perceptionLogArray) {
            if (typeof entry !== 'object' || entry === null) {
                this.#logger.warn(`PerceptionLogAssembler: Invalid perception log entry encountered in array for '${elementConfig.key}'. Skipping this entry.`, {entry});
                continue;
            }
            ...
            if (pLogEntryConfig) {
                ...
                const entryConfigPrefixCleaned = PerceptionLogAssembler.#cleanTimestampAttributes(pLogEntryConfig.prefix || "");
                const entryConfigSuffixCleaned = PerceptionLogAssembler.#cleanTimestampAttributes(pLogEntryConfig.suffix || "");
                ...
                const resolvedEntryPrefix = placeholderResolver.resolve(entryConfigPrefixCleaned, entryForResolution, promptData);
                const resolvedEntrySuffix = placeholderResolver.resolve(entryConfigSuffixCleaned, entryForResolution, promptData);
        ...
        if (assembledLogEntries === "") {
            if (!pLogEntryConfig) {
                this.#logger.debug(`PerceptionLogAssembler: Perception log wrapper for '${elementConfig.key}' processed. Entries were not formatted or added to output due to missing '${PERCEPTION_LOG_ENTRY_KEY}' config.`);
            } else {
                this.#logger.debug(`PerceptionLogAssembler: Perception log wrapper for '${elementConfig.key}' processed, but all log entries resulted in empty strings (e.g., empty content and/or empty resolved prefixes/suffixes).`);
            }
        } else { // assembledLogEntries !== ""
            this.#logger.debug(`PerceptionLogAssembler: Perception log wrapper for '${elementConfig.key}' processed with formatted entries.`);
        }
