// src/core/services/loggerConfigLoader.js
// --- FILE START ---
...
export class LoggerConfigLoader {
     ...
     * @param {string} [dependencies.configPath] - Optional override for the default config file path.
     */
    constructor(dependencies = {}) {
        ...
        if (dependencies.configPath && typeof dependencies.configPath === 'string') {
    ...
    async loadConfig(filePath) {
        ...
        const logInfo = (msg, ...args) => (this.#logger.info ? this.#logger.info(msg, ...args) : console.info(msg, ...args));
        const logError = (msg, ...args) => (this.#logger.error ? this.#logger.error(msg, ...args) : console.error(msg, ...args));
        const logWarn = (msg, ...args) => (this.#logger.warn ? this.#logger.warn(msg, ...args) : console.warn(msg, ...args));

        logInfo(`[LoggerConfigLoader] Attempting to load logger configuration from: ${path}`);
        ...
        try {
            ...
            logInfo(`[LoggerConfigLoader] Successfully fetched and parsed logger configuration from ${path}.`);

            if (typeof parsedResponse !== 'object' || parsedResponse === null) {
                logWarn(`[LoggerConfigLoader] Configuration file from ${path} is malformed (not an object). Content:`, parsedResponse);
                return {
                    error: true,
            ...
            if (Object.keys(parsedResponse).length === 0) {
                logInfo(`[LoggerConfigLoader] Logger configuration file at ${path} is empty or contains an empty JSON object. No log level specified.`);
                // Return empty object, indicates no specific config found but file was parsable
                return {};
            }

            if (parsedResponse.logLevel !== undefined) {
                if (typeof parsedResponse.logLevel !== 'string') {
                    logWarn(`[LoggerConfigLoader] 'logLevel' in ${path} must be a string. Found: ${typeof parsedResponse.logLevel}. Value: ${parsedResponse.logLevel}`);
                    return {
                        error: true,
                ...
                logInfo(`[LoggerConfigLoader] 'logLevel' property not found in ${path}. No log level specified.`);
            ...
            logError(`[LoggerConfigLoader] Failed to load or parse logger configuration from ${path}. Error: ${error.message}`, {
                path,
                originalError: { message: error.message, name: error.name, stack: error.stack } // Include more error details
            });

            let stage = 'fetch_or_parse';
            if (error.message) {
                const lowerMsg = error.message.toLowerCase();
                if (lowerMsg.includes('json') || lowerMsg.includes('parse') || lowerMsg.includes('token')) {
                    stage = 'parse';
                } else if (lowerMsg.includes('failed to fetch') || lowerMsg.includes('network') || lowerMsg.includes('not found') || lowerMsg.includes('status')) {
                    stage = 'fetch';
                }
            }
