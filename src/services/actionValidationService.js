...
export class ActionValidationService {
  ...
  constructor({
    ...
    if (!prerequisiteEvaluationService?.evaluate || prerequisiteEvaluationService.evaluate.length !== 4) // Check method and arity (remains unchanged)
      throw new Error('ActionValidationService requires a valid PrerequisiteEvaluationService with the correct evaluate signature.');
    // --- Refactor-AVS-3.4: Remove dependency ---
    ...
    this.#logger.info('ActionValidationService initialised (dependencies: EM, Logger, DCCC, PES).');
  }
  ...
  _checkDomainAndContext(actionDefinition, actorEntity, targetContext) {
    ...
    const isCompatible = this.#domainContextCompatibilityChecker.check(actionDefinition, targetContext);

    if (!isCompatible) {
      if (contextType === 'none' && expectedDomain !== 'none') {
        this.#logger.debug(`Validation[${actionId}]: Domain/Context check PASSED (deferred): Initial check context is 'none' but domain '${expectedDomain}' requires a target. Checker result (${isCompatible}) ignored for this step.`);
      } else {
        this.#logger.debug(`Validation[${actionId}]: ← STEP 1 FAILED (domain/context compatibility checker returned false). Expected Domain: '${expectedDomain}', Actual Context Type: '${contextType}'.`);
        return false;
      }
    }
    ...
    if (
      ...
      this.#logger.debug(`Validation[${actionId}]: ← STEP 1 FAILED ('self' target mismatch). Target entity '${targetContext.entityId}' !== Actor '${actorId}'.`);
      return false;
    }

    this.#logger.debug(`Validation[${actionId}]: → STEP 1 PASSED (Domain/Context Compatible or Deferred).`);
    return true;
  }
  ...
  _collectAndValidatePrerequisites(actionDefinition) {
    ...
    } else if ('prerequisites' in actionDefinition && actionDefinition.prerequisites != null) {
      this.#logger.warn(
  ...
  isValid(actionDefinition, actorEntity, targetContext) {
    ...
    try {
      ...
      this.#logger.debug('END Validation: FAILED (Structural Sanity - Throwing)');
      throw err;
    }
    ...
    try {
      ...
      if (!this._checkDomainAndContext(actionDefinition, actorEntity, targetContext)) {
        this.#logger.debug(`END Validation: FAILED (Domain/Context) for action '${actionId}'.`);
        return false;
      }

      // Step 2: Verify Target Entity Existence
      this._verifyTargetEntityExistence(targetContext, actionId);
      this.#logger.debug(`Validation[${actionId}]: → STEP 2 PASSED (Entity Existence Checked).`);

      // Step 3: Collect and Validate Prerequisites Format
      const prerequisites = this._collectAndValidatePrerequisites(actionDefinition);
      this.#logger.debug(`Validation[${actionId}]: → STEP 3 PASSED (Prerequisites Collected: ${prerequisites.length}).`);

      // Step 4: Evaluate Prerequisites (Context building now happens inside PES.evaluate)
      let allPrerequisitesPassed = true;

      if (prerequisites.length > 0) { // Check gates the call
        this.#logger.debug(`Validation[${actionId}]: Delegating prerequisite evaluation (including context building) to PrerequisiteEvaluationService...`);
        ...
        this.#logger.debug(`Validation[${actionId}]: → STEP 4 SKIPPED (No prerequisites to evaluate).`);
      }

      // Check the outcome of the prerequisite evaluation step
      if (!allPrerequisitesPassed) {
        // Failure message is already logged by PES
        this.#logger.debug(`END Validation: FAILED (Prerequisite Not Met or Error during evaluation) for action '${actionId}'.`);
        return false;
      }
      // If we got here, prerequisites either passed or were skipped
      this.#logger.debug(`Validation[${actionId}]: → STEP 4 PASSED (Prerequisite evaluation successful or skipped).`);
      ...
      this.#logger.debug(`END Validation: FAILED (Unexpected Error) for action '${actionId}'.`);
