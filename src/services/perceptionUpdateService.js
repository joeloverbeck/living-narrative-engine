...
class PerceptionUpdateService {
  ...
  async handleQuery(queryDetails) {
    this.#logger.debug('PerceptionUpdateService.handleQuery: Received query', {
      queryDetails,
    });
    if (
      !queryDetails ||
      typeof queryDetails !== 'object' ||
      ...
      this.#logger.error(`PerceptionUpdateService.handleQuery: ${errorMsg}`, {
        queryDetails,
      });
    ...
    switch (action) {
      ...
      default:
        ...
        this.#logger.warn(
          `PerceptionUpdateService.handleQuery: ${unknownActionMsg}`,
          { queryDetails }
        );
        return Promise.resolve({
          success: false,
          error: unknownActionMsg,
          logsUpdated: 0,
        });
    }
  }
  ...
  async addEntryToLogsInLocation(params) {
    this.#logger.debug(
      'PerceptionUpdateService.addEntryToLogsInLocation: Received request',
      params
    );
    const warnings = [];
    ...
    for (const field of requiredEntryFields) {
      if (
        ...
        // Allow empty targetId
        const errorMsg = `Missing or invalid required field in entry: ${field}.`;
        this.#logger.error(
          `PerceptionUpdateService.addEntryToLogsInLocation: ${errorMsg}`,
          { entry }
        );
        return { success: false, error: errorMsg, logsUpdated: 0 };
      }
    ...
    try {
      ...
      this.#logger.debug(
        `PerceptionUpdateService: Found ${entityIdsInLocation ? entityIdsInLocation.size : 0} entities in location '${locationId}'. Filtered to ${perceiverEntityIds.length} entities with '${PERCEPTION_LOG_COMPONENT_ID}'.`
      );
      ...
      for (const entityId of perceiverEntityIds) {
        let perceptionLogComponentData = this.#entityManager.getComponentData(
          entityId,
          PERCEPTION_LOG_COMPONENT_ID
        );
        ...
        if (!perceptionLogComponentData) {
          ...
          mutablePerceptionLog = JSON.parse(
            JSON.stringify(perceptionLogComponentData)
          );
          ...
          if (
            typeof mutablePerceptionLog.maxEntries !== 'number' ||
            mutablePerceptionLog.maxEntries < 1
          ) {
        ...
        if (
          mutablePerceptionLog.logEntries.length >
          mutablePerceptionLog.maxEntries
        ) {
          mutablePerceptionLog.logEntries =
            mutablePerceptionLog.logEntries.slice(
              -mutablePerceptionLog.maxEntries
            );
        }

        try {
          const updateSuccess = this.#entityManager.addComponent(
            entityId,
            PERCEPTION_LOG_COMPONENT_ID,
            mutablePerceptionLog
          );
          if (updateSuccess) {
            updatedCount++;
            this.#logger.debug(
              `PerceptionUpdateService: Successfully updated perception log for entity ${entityId}.`
            );
          } else {
      ...
      this.#logger.error(
        'PerceptionUpdateService.addEntryToLogsInLocation: Unhandled error during perception update logic.',
        {
