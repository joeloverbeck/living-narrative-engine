...
class GenericTriggerSystem {
    ...
    initialize() {
        ...
        // NO LONGER subscribes to hardcoded events like 'event:room_entered' or 'event:entity_moved' here.
    }
    ...
    _checkFilters(listenCondition, eventName, eventData) {
        ...
        for (const filterKey in listenCondition.filters) {
            ...
            if (eventName === 'event:entity_died') {
            ...
            } else if (eventName === 'event:entity_moved') { // Entity changed location reference
                ...
                // else if (eventName === 'event:item_used') {
    ...
    _executeUpdateEntityComponent(target, parameters) {
        ...
        try {
            ...
            if (component_name === 'Lockable' && component_data.hasOwnProperty('isLocked')) {
                ...
                if (targetLockedState === false && typeof componentInstance.unlock === 'function') {
                    componentInstance.unlock();
                    console.log(`[DEBUG] Trigger Action (Trigger: ${triggerId}): Called unlock() on ${component_name} for entity '${entity_id}'.`);
                    updatePerformed = true;
                } else if (targetLockedState === true && typeof componentInstance.lock === 'function') {
                    componentInstance.lock();
                    console.log(`[DEBUG] Trigger Action (Trigger: ${triggerId}): Called lock() on ${component_name} for entity '${entity_id}'.`);
                    updatePerformed = true;
                }
            }

            // If no specific method handled it, or for other components/properties, perform shallow merge
            if (!updatePerformed) {
                for (const key in component_data) {
                    // Only update properties that exist on the component instance
                    // Avoids accidentally adding new properties via triggers.
                    if (componentInstance.hasOwnProperty(key)) {
                        // TODO: Consider adding type checking or validation here?
                        componentInstance[key] = component_data[key];
                        updatePerformed = true; // Mark that at least one property was potentially updated
                    } else {
            ...
            // If any update was potentially made (either via method or merge), consider it a success.
            success = true;

            // Dispatch an event indicating the component was updated by a trigger
