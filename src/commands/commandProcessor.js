...
class CommandProcessor {
    ...
    constructor(options) {
        ...
        if (!logger || typeof logger.info !== 'function' || typeof logger.error !== 'function' || typeof logger.debug !== 'function' || typeof logger.warn !== 'function') {
            const errorMsg = 'CommandProcessor Constructor: CRITICAL - Invalid or missing ILogger instance.';
            console.error(errorMsg);
            throw new Error(errorMsg);
        }
        this.#logger = logger;
        ...
        try {
            ...
            this.#_validateDependency(validatedEventDispatcher, 'validatedEventDispatcher', ['dispatchValidated']); // Used by ActionContext shim
    ...
    #_validateInput(actor, command) {
        if (!actor || typeof actor.id !== 'string') {
            this.#logger.error('CommandProcessor.#_validateInput: Invalid or missing actor.');
            return {
                isValid: false, actorId: null, trimmedCommand: null,
                errorResult: this.#_createFailureResult('Internal error: Invalid actor.', 'Invalid or missing actor provided.')
            };
        }
        ...
        if (!trimmedCommand) {
            this.#logger.warn(`CommandProcessor.#_validateInput: Empty command by ${actorId}.`);
            const failureResult = this.#_createFailureResult(undefined, 'Empty command string.');
            failureResult.originalInput = "";
            return {
                isValid: false, actorId: actorId, trimmedCommand: null,
    ...
    async #_parseCommand(actorId, commandString) {
        this.#logger.debug(`CommandProcessor.#_parseCommand: Parsing: "${commandString}" for ${actorId}`);
        const parsedCommand = this.#commandParser.parse(commandString);
        this.#logger.debug(`CommandProcessor.#_parseCommand: Parsed: ${JSON.stringify(parsedCommand)}`);

        if (parsedCommand.error) {
            const parsingError = parsedCommand.error;
            this.#logger.warn(`CommandProcessor.#_parseCommand: Parse fail for "${commandString}" by ${actorId}. Error: ${parsingError}`);
            ...
            const failureResult = this.#_createFailureResult(parsingError, `Parsing Error: ${parsingError}`);
        ...
        this.#logger.debug(`CommandProcessor.#_parseCommand: Parse OK for "${commandString}", actionId: ${parsedCommand.actionId}.`);
        return {parsedCommand: parsedCommand, errorResult: null};
    }

    async #_fetchActionDefinition(actionId, actorId, originalInput) {
        this.#logger.debug(`CommandProcessor.#_fetchActionDefinition: Fetching ActionDef for '${actionId}'.`);
        ...
        this.#logger.debug(`CommandProcessor.#_fetchActionDefinition: Found ActionDef for '${actionId}'.`);
    ...
    async #_fetchLocationContext(actorId, actionDefinition, originalInput) {
        ...
        try {
            ...
            if (!currentLocation && actionDefinition.target_domain !== 'none' && actionDefinition.target_domain !== 'self') {
            ...
            this.#logger.debug(`CommandProcessor.#_fetchLocationContext: Location for ${actorId}: ${currentLocation?.id ?? 'None'}. Action '${actionDefinition.id}' domain: '${actionDefinition.target_domain}'.`);
    ...
    #_buildActionContext(actorEntity, currentLocation, parsedCommand) {
        this.#logger.debug(`CommandProcessor.#_buildActionContext: Actor: ${actorEntity.id}, Location: ${currentLocation?.id ?? 'null'}, Command: ${parsedCommand.actionId}`);
        const actionContext = {
            ...
            eventBus: {
                dispatch: async (eventName, payload) => {
                    if (eventName === 'textUI:display_message') {
                        this.#logger.debug(`CommandProcessor (Shim): Dispatching '${eventName}' via VED.`);
                        ...
                        return this.#validatedEventDispatcher.dispatchValidated(eventName, payload);
                    } else {
                        this.#logger.warn(`CommandProcessor (Shim): Unsupported event '${eventName}'. Ignoring.`);
                        return false;
                    }
                }
            },
        ...
        this.#logger.debug(`CommandProcessor.#_buildActionContext: ActionContext built.`);
        return actionContext;
    }

    async #_resolveTarget(actionDefinition, actionContext, originalInput) {
        this.#logger.debug(`CommandProcessor.#_resolveTarget: Resolving target for action '${actionDefinition.id}'...`);
        const resolutionResult = await this.#targetResolutionService.resolveActionTarget(actionDefinition, actionContext);
        const {status, targetType, targetId, error: resolverErrorMsg} = resolutionResult;
        this.#logger.debug(`CommandProcessor.#_resolveTarget: Resolution done. Status: ${status}, Type: ${targetType}, TargetID: ${targetId}`);

        const isSuccessfulResolution = (status === ResolutionStatus.FOUND_UNIQUE || status === ResolutionStatus.SELF) ||
            (status === ResolutionStatus.NONE && actionDefinition.target_domain === 'none');
        ...
        if (!isSuccessfulResolution) {
            ...
            const userError = resolverErrorMsg ? `Target error: ${resolverErrorMsg}` : `Target unclear or invalid (Status: ${status}).`;
        ...
        this.#logger.debug(`CommandProcessor.#_resolveTarget: Target resolution successful.`);
    ...
    async #_dispatchActionAttempt(actorId, actionIdFromDefinition, resolutionResult, originalCommandString) {
        ...
        const payload = {
            ...
            direction: resolutionResult.targetType === 'direction' ? resolutionResult.targetId : null,
            originalInput: originalCommandString,
        };

        this.#logger.info(`CommandProcessor.#_dispatchActionAttempt: Dispatching core:attempt_action for "${originalCommandString}".`);
        ...
        const dispatchSuccess = await this.#dispatchWithErrorHandling('core:attempt_action', payload, 'core:attempt_action dispatch');

        if (dispatchSuccess) {
            this.#logger.info(`CommandProcessor.#_dispatchActionAttempt: Dispatched core:attempt_action successfully for "${originalCommandString}" by ${actorId}.`);
            ...
            await this.#dispatchSystemError(userMsg, `Failed to dispatch core:attempt_action (VED validation likely). Payload: ${JSON.stringify(payload)}`);
            const failureResult = this.#_createFailureResult(userMsg, internalMsg);
    ...
    async processCommand(actor, command) {
        ...
        if (!validation.isValid) return /** @type {CommandResult} */ (validation.errorResult);
        ...
        try {
            ...
            const definitionOutcome = await this.#_fetchActionDefinition(parsedCommand.actionId, actorId, trimmedCommand);
            ...
            const locationOutcome = await this.#_fetchLocationContext(actorId, actionDefinition, trimmedCommand);
            ...
            const targetOutcome = await this.#_resolveTarget(actionDefinition, actionContext, trimmedCommand);
            ...
            if (dispatchOutcome.success) {
                this.#logger.info(`CommandProcessor: Successfully processed and dispatched action for "${trimmedCommand}" by ${actorId}.`);
                return {
                    success: true,
                    turnEnded: false,
            ...
            const failureResult = this.#_createFailureResult(criticalUserErrorMsg, internalMsg, true);
            failureResult.originalInput = trimmedCommand;
            failureResult.actionResult = {actionId: "core:unknown_action_due_to_critical_error"};
            return failureResult;
        }
    }

    async #dispatchWithErrorHandling(eventName, payload, loggingContextName) {
        this.#logger.debug(`CommandProcessor.#dispatchWithErrorHandling: Attempting dispatch: ${loggingContextName} ('${eventName}')`);
        try {
            ...
            if (success) {
                this.#logger.debug(`CommandProcessor.#dispatchWithErrorHandling: Dispatch successful for ${loggingContextName}.`);
            } else {
                // This 'else' means dispatchSafely returned false, likely because VED returned false.
                this.#logger.warn(`CommandProcessor.#dispatchWithErrorHandling: SafeEventDispatcher reported failure for ${loggingContextName} (likely VED validation failure). Payload: ${JSON.stringify(payload)}`);
            }
            return success;
        } catch (dispatchError) {
            this.#logger.error(`CommandProcessor.#dispatchWithErrorHandling: CRITICAL - Error during dispatchSafely for ${loggingContextName}. Error: ${dispatchError.message}`, dispatchError);
            // If dispatchSafely itself throws, it's a more fundamental issue.
            await this.#dispatchSystemError('System error during event dispatch.', `Exception in dispatchSafely for ${eventName}`, dispatchError);
            return false;
        }
    }
    ...
    async #dispatchSystemError(userMessage, internalDetails, originalError = null) {
        ...
        if (originalError) {
            this.#logger.error(`CommandProcessor System Error: ${internalDetails}. Original Error: ${originalError.message}`, originalError);
        } else {
            this.#logger.error(`CommandProcessor System Error: ${internalDetails}`);
        }

        const dispatchSuccess = await this.#safeEventDispatcher.dispatchSafely('core:system_error_occurred', payload);

        if (!dispatchSuccess) {
            this.#logger.error(`CommandProcessor: CRITICAL FAILURE - Failed to dispatch 'core:system_error_occurred' event itself. Context: UserMessage='${userMessage}', InternalDetails='${internalDetails}'.`);
        }
    }
}
