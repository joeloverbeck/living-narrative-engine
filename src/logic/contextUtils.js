 ...
 */
export function resolvePlaceholders(input, executionContext, logger, currentPath = '') {
    if (typeof input === 'string') {
            ...
            if (executionContext && typeof executionContext === 'object') {
                ...
                try {
                    ...
                    if (placeholderPath.startsWith('context.')) {
                        if (executionContext.evaluationContext && typeof executionContext.evaluationContext.context === 'object' && executionContext.evaluationContext.context !== null) {
                            effectiveResolutionRoot = executionContext.evaluationContext.context;
                            pathForResolvePath = placeholderPath.substring('context.'.length);
                        } else {
                            logger?.warn(`Placeholder "${placeholderPath}" uses "context." prefix, but executionContext.evaluationContext.context is missing or invalid. Path: ${fullLogPath}`);
                            // Fallthrough: attempt to resolve from root, pathForResolvePath remains placeholderPath
                        }
                    }
                    // --- END MODIFIED LOGIC FOR CONTEXT PATHS ---
                    resolvedValue = resolvePath(effectiveResolutionRoot, pathForResolvePath);

                } catch (e) {
                    // pathForResolvePath is accessible here due to moved declaration
                    logger?.error(`Error resolving path "${placeholderPath}" (interpreted as "${pathForResolvePath}") from ${placeholderSyntax}. Path: ${fullLogPath}`, e);
                    resolvedValue = undefined;
                }

                if (resolvedValue === undefined) {
                    // pathForResolvePath is accessible here
                    logger?.warn(`Placeholder path "${placeholderPath}" (interpreted as "${pathForResolvePath}") from ${placeholderSyntax} could not be resolved. Path: ${fullLogPath}`);
                    return input;
                }
                ...
                logger?.warn(`Cannot resolve placeholder path "${placeholderPath}" from ${placeholderSyntax}: executionContext is not a valid object. Path: ${fullLogPath}`);
                return input;
            }
        } else {
            let replaced = false;
                    ...
                    try {
                        ...
                        if (placeholderPath.startsWith('context.')) {
                            if (executionContext.evaluationContext && typeof executionContext.evaluationContext.context === 'object' && executionContext.evaluationContext.context !== null) {
                                ...
                                logger?.warn(`Embedded placeholder "${placeholderPath}" uses "context." prefix, but executionContext.evaluationContext.context is missing or invalid. Path: ${fullLogPath}`);
                                // Fallthrough
                            }
                        }
                        ...
                        // pathForResolvePath is accessible here
                        logger?.error(`Error resolving path "${placeholderPath}" (interpreted as "${pathForResolvePath}") from embedded ${placeholderSyntax}. Path: ${fullLogPath}`, e);
                        resolvedValue = undefined;
                    }

                    if (resolvedValue === undefined) {
                        // pathForResolvePath is accessible here
                        logger?.warn(`Embedded placeholder path "${placeholderPath}" (interpreted as "${pathForResolvePath}") from ${placeholderSyntax} could not be resolved. Path: ${fullLogPath}`);
                        return match;
                    }
                    replaced = true;
                    const stringValue = resolvedValue === null ? 'null' : String(resolvedValue);
                    logger?.debug(`Replaced embedded placeholder ${placeholderSyntax} with string: "${stringValue}"`);
                    return stringValue;
                } else {
                    logger?.warn(`Cannot resolve embedded placeholder path "${placeholderPath}" from ${placeholderSyntax}: executionContext is not a valid object. Path: ${fullLogPath}`);
                    return match;
                }
            });
        ...
        const resolvedArray = input.map((item, index) => {
            const resolvedItem = resolvePlaceholders(item, executionContext, logger, `${currentPath}[${index}]`);
        ...
        for (const key in input) {
            if (Object.prototype.hasOwnProperty.call(input, key)) {
                ...
                const resolvedValue = resolvePlaceholders(originalValue, executionContext, logger, `${currentPath}.${key}`);
