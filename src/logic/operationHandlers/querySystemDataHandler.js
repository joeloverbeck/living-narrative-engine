...
class QuerySystemDataHandler {
    ...
    execute(params, nestedExecutionContext) { // Parameter renamed for clarity
        ...
        if (!nestedExecutionContext?.evaluationContext?.context ||
        ...
        try {
            ...
            nestedExecutionContext.evaluationContext.context[trimmedResultVariable] = result;
            // --- END CORRECTION ---
        } catch (contextError) {
        ...
        if (result !== undefined) {
            ...
            logger.warn(`QUERY_SYSTEM_DATA: Query to source "${trimmedSourceId}" returned undefined or an error occurred during query. Stored 'undefined' in "${trimmedResultVariable}".`);
        }
