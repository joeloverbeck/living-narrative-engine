...
class SystemLogicInterpreter {
  ...
  constructor({
    ...
    if (!eventBus || typeof eventBus.subscribe !== 'function') {
      throw new Error(
        "SystemLogicInterpreter requires a valid EventBus instance with a 'subscribe' method."
      );
    }
    if (!dataRegistry || typeof dataRegistry.getAllSystemRules !== 'function') {
      throw new Error(
        "SystemLogicInterpreter requires a valid IDataRegistry instance with an 'getAllSystemRules' method."
      );
    }
    if (
      !jsonLogicEvaluationService ||
      typeof jsonLogicEvaluationService.evaluate !== 'function'
    ) {
      throw new Error(
        "SystemLogicInterpreter requires a valid JsonLogicEvaluationService instance with an 'evaluate' method."
      );
    }
    if (
      !entityManager ||
      ...
      throw new Error(
        "SystemLogicInterpreter requires a valid EntityManager instance with a 'getEntityInstance' method."
      );
    }
    if (
      !operationInterpreter ||
      ...
      throw new Error(
        "SystemLogicInterpreter requires a valid OperationInterpreter instance with an 'execute' method."
      );
    }
  ...
  #loadAndCacheRules() {
    ...
    if (!allRules || !Array.isArray(allRules)) {
      this.#logger.error(
        'Failed to load system rules from data registry. Result was not an array.'
      );
      return;
    }
    ...
    for (const rule of allRules) {
      if (!rule || typeof rule.event_type !== 'string') {
        this.#logger.warn(
          'Skipping invalid rule definition (missing or invalid event_type):',
          rule
        );
        continue;
      }
  ...
  #subscribeToEvents() {
    ...
    if (eventTypesToListen.length > 0) {
      if (!this.#boundEventHandler) {
        this.#logger.error(
          'SystemLogicInterpreter: Bound event handler is null during subscription attempt.'
        );
        return;
      }
  ...
  #handleEvent(event) {
    ...
    this.#logger.debug(
      `Received event: ${eventType}. Found ${matchingRules.length} potential rule(s).`,
    ...
    try {
      ...
      this.#logger.debug(
        `[Event: ${eventType}] Assembling JsonLogic context via createJsonLogicContext... (ActorID: ${actorId}, TargetID: ${targetId})`
      );

      // Step 1: Get the JsonLogicEvaluationContext (expected to be flat: {event, actor, target, context, logger})
      jsonLogicDataForEvaluation = createJsonLogicContext(
        event,
        actorId,
        targetId,
        this.#entityManager,
      ...
      if (
        ...
        typeof jsonLogicDataForEvaluation.context !== 'object' ||
        jsonLogicDataForEvaluation.context === null
      ) {
        this.#logger.warn(
          `[Event: ${eventType}] createJsonLogicContext did not return a valid JsonLogicEvaluationContext with a '.context' object. Creating a default one.`
        );
        jsonLogicDataForEvaluation = {
          event: event,
          actor: null,
        ...
        if (
          ...
          try {
            jsonLogicDataForEvaluation.actor =
              this.#entityManager.getEntityInstance(actorId) || null;
            if (jsonLogicDataForEvaluation.actor)
              this.#logger.debug(`[Default Context] Resolved actor ${actorId}`);
            else
              this.#logger.debug(
                `[Default Context] Actor ${actorId} not found`
              );
          } catch (e) {
            this.#logger.error(
              `[Default Context] Error getting actor instance ${actorId}`,
              e
            );
          }
        }
        if (
          ...
          try {
            jsonLogicDataForEvaluation.target =
              this.#entityManager.getEntityInstance(targetId) || null;
            if (jsonLogicDataForEvaluation.target)
              this.#logger.debug(
                `[Default Context] Resolved target ${targetId}`
              );
            else
              this.#logger.debug(
                `[Default Context] Target ${targetId} not found`
              );
          } catch (e) {
            this.#logger.error(
              `[Default Context] Error getting target instance ${targetId}`,
              e
            );
          }
        }
      ...
      this.#logger.error(
        `[Event: ${eventType}] Critical error during ExecutionContext assembly. Cannot proceed with rule processing for this event.`,
        contextError
      );
      return;
    }
    ...
    this.#logger.debug(
      `Processing ${matchingRules.length} rule(s) for event type: ${eventType} with final execution context...`
    );
    ...
    matchingRules.forEach((rule) => {
      try {
        ...
        this.#logger.error(
          `[CRITICAL] Uncaught error during #processRule execution for rule '${rule.rule_id || 'NO_ID'}', event '${eventType}':`,
          error
        );
      }
    });
    ...
    try {
      if (finalNestedExecutionContext?.evaluationContext?.context) {
        this.#logger.debug(
          'Final state of executionContext.evaluationContext.context keys: ' +
            Object.keys(
              finalNestedExecutionContext.evaluationContext.context
            ).join(', ')
        );
      } else {
        this.#logger.warn(
          'Could not log final evaluationContext.context keys, path invalid.'
        );
      }
    } catch (logError) {
  ...
  #evaluateRuleCondition(rule, jsonLogicDataForEval) {
    // Receives the FLAT JsonLogicContext
    ...
    if (
      !jsonLogicDataForEval ||
      typeof jsonLogicDataForEval.context !== 'object' ||
      jsonLogicDataForEval.context === null
    ) {
      this.#logger.error(
        `[Rule ${ruleId} - ConditionEval] Invalid jsonLogicDataForEval or its .context property. Condition evaluation cannot proceed safely.`
      );
    ...
    if (
      ...
      try {
        conditionResult = this.#jsonLogicEvaluationService.evaluate(
          rule.condition,
          jsonLogicDataForEval
        );
        this.#logger.debug(
          `[Rule ${ruleId}] Condition evaluation raw result: ${conditionResult}`
        );
      ...
      this.#logger.debug(
        `[Rule ${ruleId}] Condition evaluation final boolean result: ${conditionPassed}`
      );
    } else {
      this.#logger.debug(
  ...
  #processRule(rule, event, finalNestedExecutionContext) {
    // Receives the FULL NESTED context
    const ruleId = rule.rule_id || 'NO_ID';
    this.#logger.debug(
      `Processing rule '${ruleId}' for event '${event.type}'. Full context received.`
    );
    ...
    if (
      ...
      this.#logger.error(
        `[Rule ${ruleId}] The 'evaluationContext' or 'evaluationContext.context' within the provided execution context is invalid. Cannot evaluate rule condition.`,
        { receivedFullContext: finalNestedExecutionContext }
      );
      return; // Halt processing for this rule
    }

    const evaluationResult = this.#evaluateRuleCondition(
      rule,
      jsonLogicDataForEval
    );
    ...
    if (evaluationResult.conditionPassed) {
      if (Array.isArray(rule.actions) && rule.actions.length > 0) {
        this.#logger.debug(
          `[Rule ${ruleId}] Executing ${rule.actions.length} actions.`
        );
        ...
        this.#logger.debug(
          `[Rule ${ruleId}] No valid actions defined or action list is empty.`
        );
      }
    } else {
  ...
  _executeActions(actions, finalNestedExecutionContext, scopeDescription) {
    // Receives FULL NESTED context
    if (!Array.isArray(actions) || actions.length === 0) {
      this.#logger.debug(
        `No actions to execute for scope: ${scopeDescription}.`
      );
      return;
    }
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (!operation || typeof operation !== 'object') {
        this.#logger.error(
          `---> [${scopeDescription} - Action ${operationIndex}] CRITICAL: Operation at index ${i} is not a valid object. Halting sequence. Operation:`,
          operation
        );
        break;
      }

      const operationDesc = operation.comment
        ? `(Comment: ${operation.comment})`
        : '';
      const opTypeString = operation?.type ?? 'MISSING_TYPE';
      this.#logger.debug(
        `---> [${scopeDescription} - Action ${operationIndex}/${actions.length}] Processing Operation: ${opTypeString} ${operationDesc}`
      );

      try {
        if (!this.#operationInterpreter) {
          this.#logger.error(
            `---> [${scopeDescription} - Action ${operationIndex}] CRITICAL: OperationInterpreter not available! Halting sequence for this rule.`
          );
          break;
        }

        if (opTypeString === 'IF') {
          this.#handleIfOperation(
            operation,
            finalNestedExecutionContext,
            scopeDescription,
            operationIndex
          ...
          this.#operationInterpreter.execute(
            operation,
            finalNestedExecutionContext
          );
          this.#logger.debug(
            `---> [${scopeDescription} - Action ${operationIndex}] OperationInterpreter.execute call completed (no error thrown) for type: ${opTypeString}`
          );
        }
  ...
  #handleIfOperation(
    ...
    if (
      ...
      !Array.isArray(params.then_actions)
    ) {
      this.#logger.error(
        `---> [${parentIfDesc}] Invalid IF operation structure: Missing or invalid 'condition' or 'then_actions'. Skipping IF block execution.`
      );
      return;
    }
    ...
    try {
      ...
      const jsonLogicDataForIfCondition =
        finalNestedExecutionContext.evaluationContext;

      if (
        !jsonLogicDataForIfCondition ||
        typeof jsonLogicDataForIfCondition.context !== 'object' ||
        jsonLogicDataForIfCondition.context === null
      ) {
        this.#logger.error(
          `---> [${parentIfDesc}] Invalid jsonLogicDataForIfCondition or its .context property for IF condition. Treating as FALSE.`
        );
      ...
      this.#logger.error(
        `---> [${parentIfDesc}] Error evaluating IF condition. Error:`,
        evalError
      );
    ...
    try {
      if (conditionResult) {
        this.#logger.debug(
          `---> [${parentIfDesc}] Condition TRUE. Executing THEN branch.`
        );
        this._executeActions(
          then_actions,
          finalNestedExecutionContext,
        ...
        if (
          else_actions &&
          Array.isArray(else_actions) &&
          else_actions.length > 0
        ) {
          this.#logger.debug(
            `---> [${parentIfDesc}] Condition FALSE. Executing ELSE branch.`
          );
          this._executeActions(
            else_actions,
            finalNestedExecutionContext,
          ...
          this.#logger.debug(
            `---> [${parentIfDesc}] Condition FALSE and no ELSE branch present or actions empty. Continuing after IF block.`
          );
        }
      }
    } catch (nestedExecutionError) {
      this.#logger.error(
        `---> [${parentIfDesc}] Error during nested action execution (THEN/ELSE). Rethrowing. Error:`,
  ...
  shutdown() {
    ...
    if (
      ...
      try {
        ...
        this.#logger.error(
          'SystemLogicInterpreter: Error during event bus unsubscription:',
          error
        );
      }
    } else {
      if (!this.#boundEventHandler) {
        this.#logger.warn(
          'SystemLogicInterpreter: Shutdown called, but no event handler was stored (possibly never initialized or subscribed).'
        );
      } else {
        this.#logger.warn(
          'SystemLogicInterpreter: Shutdown called, but EventBus or unsubscribe method is unavailable.'
        );
      }
    }
    this.#ruleCache.clear();
