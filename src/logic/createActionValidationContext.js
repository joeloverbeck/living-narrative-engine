...
export function createActionValidationContext(
  ...
  if (
    !actorEntity?.id?.trim() ||
    typeof actorEntity.hasComponent !== 'function'
  ) {
  ...
  debug(
    `Creating ActionValidationContext (action='${actionDefinition.id}' actor='${actorEntity.id}', ctxType='${targetContext.type}').`
  );
  ...
  if (
    ...
    if (targetEntity) {
      ...
      debug(
        `Target entity [${targetId}] not found or failed to load. ctx.target set accordingly.`
      );
    }
