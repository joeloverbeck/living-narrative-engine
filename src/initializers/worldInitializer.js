...
class WorldInitializer {
    ...
    constructor({
        ...
        if (!worldContext) throw new Error('WorldInitializer requires a WorldContext.'); // Dependency kept for now as per ticket instructions
        ...
        if (!referenceResolver) throw new Error('WorldInitializer requires a ReferenceResolver.');
        ...
        this.#logger.info('WorldInitializer: Instance created (with ReferenceResolver).');
    }
    ...
    async #_dispatchWorldInitEvent(eventName, payload, identifierForLog) {
        try {
            ...
            this.#logger.debug(`WorldInitializer (EventDispatch): Successfully dispatched '${eventName}' for ${identifierForLog}.`);
        } catch (e) {
            this.#logger.error(`WorldInitializer (EventDispatch): Failed dispatching '${eventName}' event for ${identifierForLog}. Error:`, e);
    ...
    async #_instantiateAllEntitiesFromDefinitions() {
        ...
        for (const entityDef of allEntityDefinitions) {
            ...
            if (instance) {
                ...
                this.#logger.debug(`WorldInitializer (Pass 1): Instantiated entity ${instance.id} (from definition: ${instance.definitionId})`);
    ...
    async #_resolveReferencesForEntityComponents(entity) {
        ...
        this.#logger.debug(`WorldInitializer (Pass 2 RefResolution): Processing components for entity ${entity.id}.`);

        if (entriesIterable && typeof entriesIterable[Symbol.iterator] === 'function') {
            const iterator = entriesIterable[Symbol.iterator]();
            if (!(iterator && typeof iterator.next === 'function')) {
                this.#logger.warn(`WorldInitializer (Pass 2 RefResolution): Entity ${entity.id} componentEntries[Symbol.iterator]() did not return a valid iterator. Skipping component resolution for this entity.`);
                return;
            }
        } else {
            this.#logger.warn(`WorldInitializer (Pass 2 RefResolution): Entity ${entity.id} componentEntries IS NOT ITERABLE or is problematic. Value: ${String(entriesIterable)}. Skipping component resolution for this entity.`);
            return;
        }
        ...
        try {
                ...
                if (componentDefinition?.resolveFields && Array.isArray(componentDefinition.resolveFields)) {
                    for (const spec of componentDefinition.resolveFields) {
                        if (!spec || !spec.resolutionStrategy) {
                            ...
                            this.#logger.warn(`WorldInitializer (Pass 2 RefResolution): Invalid resolveFields spec for component ${componentTypeId} on entity ${entity.id}`, spec);
                            continue;
                        }
                        ...
                        } = this.#referenceResolver.resolve(componentDataInstance, spec, entity.id, componentTypeId);

                        if (valueChanged && resolvedValue !== undefined) {
                            if (specDataPathIsSelf) {
                                entity.addComponent(componentTypeId, resolvedValue);
                                // Standardized log prefix
                                this.#logger.debug(`WorldInitializer (Pass 2 RefResolution): Updated component [${componentTypeId}] data directly for entity ${entity.id} via addComponent.`);
                            } else if (specDataPath) {
                                _set(componentDataInstance, specDataPath, resolvedValue);
                                // Standardized log prefix
                                this.#logger.debug(`WorldInitializer (Pass 2 RefResolution): Modified path '${specDataPath}' in component [${componentTypeId}] for entity ${entity.id}.`);
                            }
                        }
                    }
                }
            ...
            this.#logger.error(`WorldInitializer (Pass 2 RefResolution): CRITICAL error during component iteration for entity ${entity.id}. Error:`, loopError);
     ...
     * @private
     */
    #_addEntityToSpatialIndex(entity) { // Removed async
        ...
        if (positionComponentData && typeof positionComponentData.locationId === 'string' && positionComponentData.locationId.trim() !== '') {
            ...
            if (locationIdForSpatialIndex.includes(':')) {
                this.#logger.warn(`WorldInitializer (Spatial Index): Entity ${entity.id}'s position component locationId '${locationIdForSpatialIndex}' still appears to be an unresolved definitionId. Spatial index might be incorrect if this is not intended.`);
            }
            ...
            this.#logger.debug(`WorldInitializer (Spatial Index): Added entity ${entity.id} to spatial index at location ${locationIdForSpatialIndex}.`);
            return true;

        } else if (positionComponentData) {
            this.#logger.debug(`WorldInitializer (Spatial Index): Entity ${entity.id} has a position component but missing, malformed, or empty locationId after resolution. Not added to spatial index.`);
        } else {
            this.#logger.debug(`WorldInitializer (Spatial Index): Entity ${entity.id} has no position component. Not added to spatial index.`);
        }
        return false;
    }
    ...
    async #_processSingleEntityForPass2(entity) {
        if (!entity ||
            ...
            this.#logger.error(`WorldInitializer (Pass 2 Processing): Entity ${entity?.id || 'Unknown ID'} is invalid or missing required component access methods. Skipping processing for this entity.`);
            return false;
        }
        ...
        try {
            ...
            this.#logger.warn(`WorldInitializer (Pass 2 Processing): Entity ${entity.id} failed component reference resolution. Skipping spatial index addition. Error: ${resolutionError.message}`);
    ...
    async #_resolveReferencesAndPopulateSpatialIndex(instantiatedEntities) {
        this.#logger.info('WorldInitializer (Pass 2): Resolving component references and populating spatial index for entities...');
        let entitiesAddedToSpatialIndex = 0;

        for (const entity of instantiatedEntities) {
            const wasSuccessfullyProcessedAndAdded = await this.#_processSingleEntityForPass2(entity);
            if (wasSuccessfullyProcessedAndAdded) {
                entitiesAddedToSpatialIndex++;
            }
        }

        this.#logger.info(`WorldInitializer (Pass 2): Completed entity processing. Processed ${instantiatedEntities.length} entities. Added ${entitiesAddedToSpatialIndex} entities to spatial index.`);
    }
    ...
    async initializeWorldEntities() {
        ...
        try {
            const {entities: instantiatedEntities} = await this.#_instantiateAllEntitiesFromDefinitions();

            if (instantiatedEntities && instantiatedEntities.length > 0) {
                await this.#_resolveReferencesAndPopulateSpatialIndex(instantiatedEntities);
            } else {
                this.#logger.info('WorldInitializer (Pass 2): Skipped. No entities were instantiated in Pass 1.');
            }
            ...
            if (!String(error?.message).includes("CRITICAL error during component iteration")) {
