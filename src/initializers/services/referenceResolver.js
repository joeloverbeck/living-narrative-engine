...
class ReferenceResolver {
  ...
  _resolveDirect(currentValue, entityId, componentTypeId, dataPathDisplay) {
    ...
    if (typeof currentValue === 'string' && currentValue.includes(':')) {
      const targetInstance =
        this.#entityManager.getPrimaryInstanceByDefinitionId(currentValue);
      if (targetInstance) {
        if (targetInstance.id !== currentValue) {
          resolvedValue = targetInstance.id;
          valueChanged = true;
          this.#logger.debug(
            `ReferenceResolver: Resolved ${dataPathDisplay} for entity ${entityId}: '${currentValue}' -> '${resolvedValue}'.`
          );
        } else {
        ...
        this.#logger.warn(
          `ReferenceResolver: Could not resolve ${dataPathDisplay} definitionId '${currentValue}' for entity ${entityId}.`
        );
      }
    }
    ...
    if (Array.isArray(currentValue)) {
      ...
      const resolvedArrayValues = currentValue.map((defId, index) => {
        ...
        if (typeof defId === 'string' && defId.includes(':')) {
          const targetInstance =
            this.#entityManager.getPrimaryInstanceByDefinitionId(defId);
          if (targetInstance) {
            if (targetInstance.id !== defId) {
              // Log only if it actually changed
              this.#logger.debug(
                `ReferenceResolver: Resolved ${itemPathDisplay} for entity ${entityId}: '${defId}' -> '${targetInstance.id}'.`
              );
            }
            return targetInstance.id;
          } else {
            this.#logger.warn(
              `ReferenceResolver: Could not resolve ${itemPathDisplay} definitionId '${defId}' for entity ${entityId}.`
            );
  ...
  _resolveArrayOfObjects(
    ...
    if (
      ...
      spec.resolutionStrategy &&
      typeof spec.resolutionStrategy.idField === 'string'
    ) {
          ...
          if (typeof definitionId === 'string' && definitionId.includes(':')) {
            ...
            if (targetInstance) {
              ...
              if (targetInstance.id !== definitionId) {
                ...
                this.#logger.debug(
                  `ReferenceResolver: Resolved ${itemPathDisplay} for entity ${entityId}: '${definitionId}' -> '${targetInstance.id}'.`
                );
              } else {
              ...
              this.#logger.warn(
                `ReferenceResolver: Could not resolve ${itemPathDisplay} definitionId '${definitionId}' for entity ${entityId}.`
              );
            }
          }
        }
