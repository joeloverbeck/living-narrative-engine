...
class SystemInitializer {
    ...
    constructor({resolver, logger, validatedEventDispatcher, initializationTag}) {
        ...
        if (!validatedEventDispatcher || typeof validatedEventDispatcher.dispatchValidated !== 'function') throw new Error("SystemInitializer requires a valid ValidatedEventDispatcher.");
        if (!initializationTag || typeof initializationTag !== 'string' || initializationTag.trim() === '') throw new Error('SystemInitializer requires a non-empty string initializationTag.');
        ...
        this.#logger.debug(`SystemInitializer instance created. Tag: '${this.#initializationTag}'.`);
    }
    ...
    async _resolveSystems() {
        ...
        try {
            ...
            if (!Array.isArray(resolvedSystems)) {
                this.#logger.warn(`SystemInitializer: resolveByTag for tag '${this.#initializationTag}' did not return an array. Treating as empty.`);
                resolvedSystems = [];
            }
            ...
            this.#logger.error(`SystemInitializer: Failed to resolve systems by tag '${this.#initializationTag}'. Error: ${resolveTagError.message}`, resolveTagError);
    ...
    async #_initializeSingleSystem(system) {
        ...
        if (system && typeof system.initialize === 'function') {
            ...
            if (system) {
                this.#logger.debug(`SystemInitializer: System '${systemName}' has no initialize() method, skipping.`);
            } else {
                this.#logger.warn(`SystemInitializer: Encountered null/undefined entry for tag '${this.#initializationTag}', skipping.`);
            }
        }
    }
