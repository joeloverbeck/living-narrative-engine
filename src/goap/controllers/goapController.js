...
import {
  getNumericConstraintDiagnostics as getNumericConstraintDiagnosticsSnapshot,
...
class GoapController {
  ...
  constructor({
    ...
    if (goapEventDispatcher) {
      validateDependency(
        goapEventDispatcher,
        'IGoapEventDispatcher',
        this.#logger,
        {
          requiredMethods: [
            'dispatch',
            'getComplianceSnapshot',
            'getComplianceForActor',
          ],
        }
      );
    } else {
    ...
    this.#eventDispatcher =
      goapEventDispatcher ??
      createGoapEventDispatcher(validatedEventBus, this.#logger);
  ...
  #deleteActorPlan(actorId) {
    assertNonBlankString(
      actorId,
      'actorId',
      'GoapController.#deleteActorPlan',
      this.#logger
    );
  ...
  #captureTaskLibraryDiagnostics(actorId) {
    ...
    for (const entry of normalizationEntries) {
      ...
      this.#dispatchEvent(GOAP_EVENTS.TASK_PRECONDITIONS_NORMALIZED, payload, {
  ...
  #recordDependencyDiagnostics(snapshot) {
    ...
    const enriched = {
      ...
      timestamp: Date.now(),
      status:
        Array.isArray(snapshot.missingMethods) &&
        snapshot.missingMethods.length > 0
          ? 'warn'
          : 'ok',
    };
  ...
  async decideTurn(actor, world) {
    ...
    const isTopLevelInvocation = this.#recursionDepth === 0;
    ...
    try {
      ...
      this.#dispatchEvent(
        GOAP_EVENTS.TASK_REFINED,
        {
          ...
          actionRefs:
            refinementResult.stepResults?.map((s) => s.actionRef) || [],
        },
        {
          actorId: actor.id,
          goalId: activePlan?.goal?.id,
  ...
  #selectGoal(actor, world) {
    ...
    if (!allGoals || allGoals.length === 0) {
      this.#logger.debug('No goals registered in system', {
        actorId: actor.id,
      });
      return null;
    }
    ...
    const context = this.#contextAssemblyService.assemblePlanningContext(
      actor.id
    );
  ...
  #isGoalRelevant(goal, context) {
    if (!goal.relevance) {
      ...
      this.#logger.debug(
        'Goal has no relevance condition, treating as always relevant',
        {
          goalId: goal.id,
        }
      );
  ...
  #cleanupActorDiagnostics(actorId) {
    ...
    this.#taskLibraryDiagnostics.delete(actorId);
    this.#goalPathDiagnostics.delete(actorId);
    this.#effectFailureTelemetry.delete(actorId);
    clearPlanningStateDiagnostics(actorId);
    clearNumericConstraintDiagnostics(actorId);
  }
    ...
    switch (fallbackBehavior) {
      ...
      case 'continue': {
        ...
        if (!planContinues) {
          ...
          this.#clearPlan(
            actorId,
            'Goal achieved (remaining tasks failed but skippable)'
          );
          this.#recursionDepth = 0;
          return null;
        }
  ...
  getActivePlan(actorId) {
    assertNonBlankString(
      actorId,
      'actorId',
      'GoapController.getActivePlan',
      this.#logger
    );

    const plan = this.#activePlans.get(actorId);
    if (!plan) {
      return null;
    }
  ...
  getMemoryPressureSnapshot(options = {}) {
    ...
    trackLevel(
      'failedGoals',
      failureTracking.failedGoals,
      resolveThreshold('failedGoals')
    );
    trackLevel(
      'failedTasks',
      failureTracking.failedTasks,
      resolveThreshold('failedTasks')
    );
  ...
  getTaskLibraryDiagnostics(actorId) {
    ...
    try {
      ...
      this.#logger.warn(
        'Task library diagnostics contain non-serializable data',
        {
          actorId,
          error: error.message,
        }
  ...
  getNumericConstraintDiagnostics(actorId) {
    assertNonBlankString(
      actorId,
      'actorId',
      'GoapController.getNumericConstraintDiagnostics',
      this.#logger
    );

    return getNumericConstraintDiagnosticsSnapshot(actorId);
  }
  ...
  getEventComplianceDiagnostics(actorId) {
    ...
    const actorDiagnostics =
      this.#eventDispatcher.getComplianceForActor(actorId);
    const globalDiagnostics =
      this.#eventDispatcher.getComplianceForActor('global');
    ...
    if (
      ...
      planningDiagnostics =
        this.#eventDispatcher.getPlanningComplianceSnapshot();
    }
