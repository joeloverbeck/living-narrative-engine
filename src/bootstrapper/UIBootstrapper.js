...
export class UIBootstrapper {
    ...
    gatherEssentialElements(doc) {
        if (!doc) {
            throw new Error('Fatal Error: Document object was not provided to UIBootstrapper.');
        }

        const docContext = new DocumentContext(doc);

        const elementsToQuery = [
            {key: 'outputDiv', selector: '#outputDiv', name: 'outputDiv (ID: outputDiv)'},
            {key: 'errorDiv', selector: '#error-output', name: 'errorDiv (ID: error-output)'},
            {key: 'inputElement', selector: '#speech-input', name: 'inputElement (ID: speech-input)'},
            {key: 'titleElement', selector: 'h1', name: 'titleElement (Selector: h1)'}
        ];
        ...
        elementsToQuery.forEach(item => {
            try {
                ...
                console.error(`[UIBootstrapper] Error querying for ${item.name} using selector "${item.selector}":`, e);
        ...
        console.log('[UIBootstrapper] All essential UI elements found successfully.');
