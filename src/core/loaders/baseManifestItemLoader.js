...
export class BaseManifestItemLoader {
     ...
     */
    constructor(contentType, config, pathResolver, dataFetcher, schemaValidator, dataRegistry, logger) {
        ...
        if (!logger || typeof logger !== 'object') { throw new TypeError("BaseManifestItemLoader requires a valid ILogger instance."); }
        const requiredLoggerMethods = ['info', 'warn', 'error', 'debug'];
        for (const method of requiredLoggerMethods) { if (typeof logger[method] !== 'function') { throw new TypeError(`BaseManifestItemLoader: ILogger instance must have a '${method}' method.`); } }
        this._logger = logger;
        if (!config || typeof config !== 'object') { this._logger.error("BaseManifestItemLoader: Invalid IConfiguration dependency provided."); throw new TypeError("BaseManifestItemLoader requires a valid IConfiguration instance."); }
        if (typeof config.getModsBasePath !== 'function') { this._logger.error("BaseManifestItemLoader: IConfiguration missing 'getModsBasePath' method."); throw new TypeError("BaseManifestItemLoader: IConfiguration instance must have a 'getModsBasePath' method."); }
        if (typeof config.getContentTypeSchemaId !== 'function') { this._logger.error("BaseManifestItemLoader: IConfiguration missing 'getContentTypeSchemaId' method."); throw new TypeError("BaseManifestItemLoader: IConfiguration instance must have a 'getContentTypeSchemaId' method."); }
        if (!pathResolver || typeof pathResolver !== 'object') { this._logger.error("BaseManifestItemLoader: Invalid IPathResolver dependency provided."); throw new TypeError("BaseManifestItemLoader requires a valid IPathResolver instance."); }
        if (typeof pathResolver.resolveModContentPath !== 'function') { this._logger.error("BaseManifestItemLoader: IPathResolver missing 'resolveModContentPath' method."); throw new TypeError("BaseManifestItemLoader: IPathResolver instance must have a 'resolveModContentPath' method."); }
        if (!dataFetcher || typeof dataFetcher !== 'object') { this._logger.error("BaseManifestItemLoader: Invalid IDataFetcher dependency provided."); throw new TypeError("BaseManifestItemLoader requires a valid IDataFetcher instance."); }
        if (typeof dataFetcher.fetch !== 'function') { this._logger.error("BaseManifestItemLoader: IDataFetcher missing 'fetch' method."); throw new TypeError("BaseManifestItemLoader: IDataFetcher instance must have a 'fetch' method."); }
        if (!schemaValidator || typeof schemaValidator !== 'object') { this._logger.error("BaseManifestItemLoader: Invalid ISchemaValidator dependency provided."); throw new TypeError("BaseManifestItemLoader requires a valid ISchemaValidator instance."); }
        if (typeof schemaValidator.validate !== 'function') { this._logger.error("BaseManifestItemLoader: ISchemaValidator missing 'validate' method."); throw new TypeError("BaseManifestItemLoader: ISchemaValidator instance must have a 'validate' method."); }
        if (typeof schemaValidator.getValidator !== 'function') { this._logger.error("BaseManifestItemLoader: ISchemaValidator missing 'getValidator' method."); throw new TypeError("BaseManifestItemLoader: ISchemaValidator instance must have a 'getValidator' method."); }
        if (typeof schemaValidator.isSchemaLoaded !== 'function') { this._logger.error("BaseManifestItemLoader: ISchemaValidator missing 'isSchemaLoaded' method."); throw new TypeError("BaseManifestItemLoader: ISchemaValidator instance must have a 'isSchemaLoaded' method."); }
        if (!dataRegistry || typeof dataRegistry !== 'object') { this._logger.error("BaseManifestItemLoader: Invalid IDataRegistry dependency provided."); throw new TypeError("BaseManifestItemLoader requires a valid IDataRegistry instance."); }
        if (typeof dataRegistry.store !== 'function') { this._logger.error("BaseManifestItemLoader: IDataRegistry missing 'store' method."); throw new TypeError("BaseManifestItemLoader: IDataRegistry instance must have a 'store' method."); }
        if (typeof dataRegistry.get !== 'function') { this._logger.error("BaseManifestItemLoader: IDataRegistry missing 'get' method."); throw new TypeError("BaseManifestItemLoader: IDataRegistry instance must have a 'get' method."); }
        ...
        this._primarySchemaId = this._config.getContentTypeSchemaId(trimmedContentType);
        if (this._primarySchemaId) {
            this._logger.debug(`${this.constructor.name}: Primary schema ID for content type '${trimmedContentType}' found: '${this._primarySchemaId}'`);
        } else {
            this._logger.warn(`${this.constructor.name}: Primary schema ID for content type '${trimmedContentType}' not found in configuration. Primary validation might be skipped.`);
            this._primarySchemaId = null;
        }
        this._logger.debug(`${this.constructor.name}: Initialized successfully for content type '${trimmedContentType}'.`);
    }
    ...
    _validatePrimarySchema(data, filename, modId, resolvedPath) {
        ...
        if (!schemaId) {
            this._logger.debug(`${loaderName} [${modId}]: Skipping primary schema validation for '${filename}' as no primary schema ID is configured for this loader.`);
        ...
        this._logger.debug(`${loaderName} [${modId}]: Validating '${filename}' against primary schema '${schemaId}'.`);
        ...
        if (!validationResult.isValid) {
            ...
            this._logger.error(errorMsg, { modId, filename, resolvedPath, schemaId, validationErrors: validationResult.errors });
            const errorDetails = validationResult.errors?.map(e => `  - Path: ${e.instancePath || '/'} | Message: ${e.message} | Params: ${JSON.stringify(e.params)}`).join('\n');
    ...
    _storeItemInRegistry(category, modId, baseItemId, dataToStore, sourceFilename) { // <<< MODIFIED RETURN TYPE
        ...
        try {
            ...
            this._logger.error(
                ...
                { category, modId, baseItemId, finalRegistryKey, sourceFilename, error: error?.message || String(error) },
                error
            );
            throw error;
        }
    ...
    async loadItemsForMod(modId, modManifest, contentKey, contentTypeDir, typeName) { // <<< MODIFIED RETURN TYPE
        ...
        if (typeof contentTypeDir !== 'string' || contentTypeDir.trim() === '') {
            const errorMsg = `${this.constructor.name}: Programming Error - Invalid 'contentTypeDir' provided for loading ${typeName} for mod '${trimmedModId}'. Must be a non-empty string. Received: ${contentTypeDir}`; this._logger.error(errorMsg); throw new TypeError(errorMsg);
        }
        const trimmedContentTypeDir = contentTypeDir.trim();
        if (typeof typeName !== 'string' || typeName.trim() === '') {
            const errorMsg = `${this.constructor.name}: Programming Error - Invalid 'typeName' provided for loading content for mod '${trimmedModId}'. Must be a non-empty string. Received: ${typeName}`; this._logger.error(errorMsg); throw new TypeError(errorMsg);
        }
