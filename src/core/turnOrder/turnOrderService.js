...
/**
 * @fileoverview Implements the Turn Order Service responsible for managing
...
export class TurnOrderService extends ITurnOrderService {
    ...
    constructor({logger}) {
        ...
        if (!logger || typeof logger.info !== 'function' || typeof logger.error !== 'function' || typeof logger.warn !== 'function') {
            ...
            throw new Error('TurnOrderService requires a valid ILogger instance (info, error, warn methods).');
        }
        this.#logger = logger;
     ...
     * @protected // Or public, depending on intended usage
     * @returns {void}
     */
    clearCurrentRound() {
    ...
    startNewRound(entities, strategy, initiativeData) {
        this.#logger.info(`TurnOrderService: Starting new round with strategy "${strategy}".`);
        this.clearCurrentRound(); // Clear previous state first

        // Validate entities array
        if (!Array.isArray(entities) || entities.length === 0) {
            this.#logger.error('TurnOrderService.startNewRound: Failed - entities array must be a non-empty array.');
        ...
        if (!entities.every(e => e && typeof e.id === 'string' && e.id !== '')) {
            this.#logger.error('TurnOrderService.startNewRound: Failed - entities array contains invalid entities (missing or invalid id).');
            ...
            switch (strategy) {
                case 'initiative':
                    ...
                    if (!(initiativeData instanceof Map) || initiativeData.size === 0) {
                        this.#logger.error('TurnOrderService.startNewRound (initiative): Failed - initiativeData Map is required and must not be empty.');
                        throw new Error('Valid initiativeData Map is required for the "initiative" strategy.');
                    }

                    this.#currentQueue = new InitiativePriorityQueue();
                    this.#logger.debug('TurnOrderService: Initialized InitiativePriorityQueue.');

                    for (const entity of entities) {
                        const initiativeScore = initiativeData.get(entity.id);

                        if (initiativeScore === undefined || typeof initiativeScore !== 'number' || !Number.isFinite(initiativeScore)) {
                            this.#logger.warn(`TurnOrderService.startNewRound (initiative): Entity "${entity.id}" missing valid initiative score. Defaulting to 0.`);
                    ...
                    this.#logger.info(`TurnOrderService: Populated InitiativePriorityQueue with ${entities.length} entities.`);
                    break;

                case 'round-robin':
                    this.#currentQueue = new SimpleRoundRobinQueue();
                    this.#logger.debug('TurnOrderService: Initialized SimpleRoundRobinQueue.');
                    for (const entity of entities) {
                        this.#currentQueue.add(entity); // Priority is ignored by SimpleRoundRobinQueue
                    }
                    this.#logger.info(`TurnOrderService: Populated SimpleRoundRobinQueue with ${entities.length} entities.`);
                    break;

                default:
                    this.#logger.error(`TurnOrderService.startNewRound: Failed - Unsupported turn order strategy "${strategy}".`);
                    this.#currentStrategy = null; // Reset strategy if invalid
                    throw new Error(`Unsupported turn order strategy: ${strategy}`);
            }
        } catch (error) {
            this.#logger.error(`TurnOrderService.startNewRound: Error during queue population for strategy "${strategy}": ${error.message}`, error);
            // Clean up partially initialized state
            this.clearCurrentRound();
            throw error; // Re-throw the error after logging and cleanup
        }

        this.#logger.info(`TurnOrderService: New round successfully started with ${this.#currentQueue.size()} active entities.`);
    }
    ...
    getNextEntity() {
        if (!this.#currentQueue) {
            this.#logger.warn('TurnOrderService.getNextEntity: Called when no round is active.');
            return null;
        }
        const nextEntity = this.#currentQueue.getNext();
        if (nextEntity) {
            this.#logger.debug(`TurnOrderService: Advancing turn to entity "${nextEntity.id}".`);
        } else {
            this.#logger.info('TurnOrderService: getNextEntity returned null (queue is likely empty).');
        }
        return nextEntity;
    }
     ...
     * @param {number} [initiativeValue=0] - The initiative score for the entity. Required and used only if the current strategy is 'initiative'. Defaults to 0 if not provided or invalid in initiative mode. Ignored for 'round-robin'.
     * @returns {void}
    ...
    addEntity(entity, initiativeValue = 0) {
        if (!this.#currentQueue || !this.#currentStrategy) {
            this.#logger.error('TurnOrderService.addEntity: Cannot add entity, no round is currently active.');
            throw new Error('Cannot add entity: No round is active.');
        }
        if (!entity || typeof entity.id !== 'string' || entity.id === '') {
            this.#logger.error('TurnOrderService.addEntity: Failed - Cannot add invalid entity (missing or invalid id).');
        ...
        try {
            if (this.#currentStrategy === 'initiative') {
                ...
                if (typeof score !== 'number' || !Number.isFinite(score)) {
                    this.#logger.warn(`TurnOrderService.addEntity (initiative): Invalid initiative value "${initiativeValue}" provided for entity "${entity.id}". Defaulting to 0.`);
                    score = 0;
                }
                this.#logger.debug(`TurnOrderService: Adding entity "${entity.id}" with initiative ${score} to the current round.`);
                this.#currentQueue.add(entity, score);
            } else if (this.#currentStrategy === 'round-robin') {
                this.#logger.debug(`TurnOrderService: Adding entity "${entity.id}" to the end of the round-robin queue.`);
                ...
                this.#logger.error(`TurnOrderService.addEntity: Internal error - current strategy "${this.#currentStrategy}" is unknown.`);
                throw new Error(`Internal error: Unknown current strategy "${this.#currentStrategy}"`);
            }
            this.#logger.info(`TurnOrderService: Entity "${entity.id}" successfully added to the turn order.`);
        } catch (error) {
            this.#logger.error(`TurnOrderService.addEntity: Failed to add entity "${entity.id}": ${error.message}`, error);
    ...
    removeEntity(entityId) {
        if (!this.#currentQueue) {
            this.#logger.warn(`TurnOrderService.removeEntity: Called for entity "${entityId}" when no round is active. No action taken.`);
            // Don't throw an error here, just log and return, as removing from a non-existent round is a no-op.
            return;
        }
        if (typeof entityId !== 'string' || entityId === '') {
            this.#logger.error('TurnOrderService.removeEntity: Failed - Invalid entityId provided.');
