...
export class EventBusCommandInputGateway extends ICommandInputPort {
    ...
    constructor({ validatedEventDispatcher }) {
        ...
        if (!validatedEventDispatcher || typeof validatedEventDispatcher.subscribe !== 'function' || typeof validatedEventDispatcher.unsubscribe !== 'function') {
            throw new Error('EventBusCommandInputGateway: Invalid or missing validatedEventDispatcher dependency (requires subscribe, unsubscribe).');
        }
            ...
            try {
                ...
                if (result instanceof Promise) {
                    result.catch(error => {
                        ...
                        console.error('EventBusCommandInputGateway: Error in async listener:', error);
                    });
                }
            } catch (error) {
                // Optional: Log synchronous listener errors
                console.error('EventBusCommandInputGateway: Error in sync listener:', error);
            }
        }
    ...
    onCommand(listener) {
        if (typeof listener !== 'function') {
            throw new Error('EventBusCommandInputGateway.onCommand: listener must be a function.');
        }
        ...
        try {
            ...
            console.error(`EventBusCommandInputGateway: Failed to subscribe listener to core:submit_command: ${subError.message}`, subError);
            throw new Error(`EventBusCommandInputGateway: Failed to subscribe to VED event 'core:submit_command'.`);
        }
        ...
        listenersToUnsubscribe.forEach(listener => {
            ...
            if (handlerToRemove) {
                try {
                    ...
                    console.error(`EventBusCommandInputGateway: Error during destroy unsubscribe for core:submit_command: ${unsubError.message}`, unsubError);
                }
            }
        });
