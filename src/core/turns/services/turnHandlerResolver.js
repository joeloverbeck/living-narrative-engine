...
class TurnHandlerResolver extends ITurnHandlerResolver {
    ...
    constructor({logger, createPlayerTurnHandler, createAiTurnHandler}) {
        ...
        if (typeof createPlayerTurnHandler !== 'function') {
            this.#logger.error('TurnHandlerResolver: Invalid or missing createPlayerTurnHandler factory function.');
            throw new Error('TurnHandlerResolver: Invalid or missing createPlayerTurnHandler factory function.');
        }
        this.#createPlayerTurnHandler = createPlayerTurnHandler;

        if (typeof createAiTurnHandler !== 'function') {
            this.#logger.error('TurnHandlerResolver: Invalid or missing createAiTurnHandler factory function.');
            throw new Error('TurnHandlerResolver: Invalid or missing createAiTurnHandler factory function.');
        }
        this.#createAiTurnHandler = createAiTurnHandler;

        this.#logger.debug('TurnHandlerResolver initialized with handler factories.');
    }
    ...
    async resolveHandler(actor) {
        ...
        if (actor.hasComponent(PLAYER_COMPONENT_ID)) {
            this.#logger.info(`TurnHandlerResolver: Creating new PlayerTurnHandler for actor ${actor.id}.`);
            try {
                ...
                if (!handler || typeof handler.startTurn !== 'function') {
                    this.#logger.error(`TurnHandlerResolver: createPlayerTurnHandler factory did not return a valid handler for actor ${actor.id}.`);
                    return null;
                }
                return handler;
            } catch (error) {
                this.#logger.error(`TurnHandlerResolver: Error creating PlayerTurnHandler for actor ${actor.id}: ${error.message}`, error);
                return null;
            }
        } else if (actor.hasComponent(ACTOR_COMPONENT_ID) && !actor.hasComponent(PLAYER_COMPONENT_ID)) {
            this.#logger.info(`TurnHandlerResolver: Creating new AITurnHandler for actor ${actor.id}.`);
            try {
                const handler = this.#createAiTurnHandler();
                // Optional: Basic check
                if (!handler || typeof handler.startTurn !== 'function') {
                    this.#logger.error(`TurnHandlerResolver: createAiTurnHandler factory did not return a valid handler for actor ${actor.id}.`);
                    return null;
                }
                return handler;
            } catch (error) {
                this.#logger.error(`TurnHandlerResolver: Error creating AITurnHandler for actor ${actor.id}: ${error.message}`, error);
                return null;
            }
        } else {
            this.#logger.info(`TurnHandlerResolver: No specific turn handler factory found for actor ${actor.id}. Returning null.`);
            return null;
        }
    }
}
