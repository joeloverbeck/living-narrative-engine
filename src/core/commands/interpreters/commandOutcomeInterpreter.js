...
class CommandOutcomeInterpreter extends ICommandOutcomeInterpreter {
    ...
    constructor({dispatcher, logger}) {
        ...
        if (!dispatcher || typeof dispatcher.dispatchSafely !== 'function') {
            this.#logger.error('CommandOutcomeInterpreter Constructor: Invalid ISafeEventDispatcher.');
            throw new Error('CommandOutcomeInterpreter: Invalid ISafeEventDispatcher dependency.');
        }
    ...
    async interpret(result, turnContext) {
        ...
        if (!result || typeof result.success !== 'boolean') {
            ...
            await this.#dispatcher.dispatchSafely(SYSTEM_ERROR_OCCURRED_ID, {
                message: baseErrorMsg, type: 'error',
        ...
        const cpFailureEndsTurn = typeof result.turnEnded === 'boolean' ? result.turnEnded : true; // Default to true for safety on failure

        this.#logger.debug(`CommandOutcomeInterpreter: Interpreting for ${actorId}. CP_Success=${result.success}, CP_TurnEndedOnFail=${cpFailureEndsTurn}, Input="${originalInput}"`);
        ...
        if (typeof processedActionId !== 'string' || !processedActionId.trim()) {
            ...
            processedActionId = chosenAction?.actionDefinitionId || "core:unknown_action";
            this.#logger.debug(`CommandOutcomeInterpreter: actor ${actorId}: result.actionResult.actionId ('${result.actionResult?.actionId}') invalid/missing. Using chosenActionId: '${processedActionId}'.`);
        }

        if (result.success) {
            eventName = 'core:action_executed';
            const resultFieldMessages = [];
            if (result.actionResult?.messages && Array.isArray(result.actionResult.messages)) {
            ...
            if (resultFieldMessages.length === 0 && result.message) {
                resultFieldMessages.push({text: String(result.message), type: 'info'});
            }
            ...
            this.#logger.info(`Actor ${actorId}: CommandProcessor success for action '${processedActionId}'. Directive: ${directive}.`);

        } else { // result.success is false (failure within CommandProcessor pipeline)
            eventName = 'core:action_failed';
            const userFacingError = result.error || 'The action could not be completed.';
            ...
            if (cpFailureEndsTurn) {
                ...
                this.#logger.warn(`Actor ${actorId}: CommandProcessor failure for action '${processedActionId}' but CP_TurnEndedOnFail was false. Forcing END_TURN_FAILURE.`);
                directive = TurnDirective.END_TURN_FAILURE;
            }
            this.#logger.info(`Actor ${actorId}: CommandProcessor failure for action '${processedActionId}'. Directive: ${directive}.`);
        }
