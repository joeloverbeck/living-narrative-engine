...
class ModDependencyValidator {
    ...
    static validate(manifests, logger) {
        ...
        if (!(manifests instanceof Map)) {
            throw new Error('ModDependencyValidator.validate: Input `manifests` must be a Map.');
        }
        if (!logger || typeof logger.warn !== 'function') {
            throw new Error('ModDependencyValidator.validate: Input `logger` must be a valid ILogger instance.');
        }
                ...
                for (const dep of manifest.dependencies) {
                    ...
                    if (!targetManifest) {
                        ...
                        if (required) {
                            ...
                            fatals.push(`Mod '${manifest.id}' requires missing dependency '${dep.id}'.`);
                        } else {
                            // AC: Non-fatal: Optional dependency absent
                            logger.warn(`Mod '${manifest.id}' optional dependency '${dep.id}' is not loaded.`);
                        }
                    } else {
            ...
            if (manifest.conflicts && Array.isArray(manifest.conflicts)) {
                for (const conflictId of manifest.conflicts) {
                    ...
                    if (manifests.has(conflictIdLower)) {
                        ...
                        fatals.push(`Mod '${manifest.id}' conflicts with loaded mod '${conflictId}'.`);
                    }
                }
            }
        ...
        logger.info('ModDependencyValidator: All dependency and conflict checks passed.');
    }
}
