...
function buildDependencyGraph(requestedIds, manifestsMap) {
    ...
    const requestedLower = new Set(requestedIds.map(id => String(id).toLowerCase()));
...
function resolveOrder(requestedIds, manifestsMap, logger) {
    /* Input checks */
    if (requestedIds == null) throw new Error('modLoadOrderResolver.resolveOrder: `requestedIds` null/undefined.');
    if (!Array.isArray(requestedIds)) throw new Error('modLoadOrderResolver.resolveOrder: `requestedIds` must be an array.');
    if (!(manifestsMap instanceof Map)) throw new Error('modLoadOrderResolver.resolveOrder: `manifestsMap` must be a Map.');
    if (!logger || typeof logger.info !== 'function') {
        throw new Error('modLoadOrderResolver.resolveOrder: `logger` does not implement ILogger.');
    }
    ...
    const priorityOf = (id) => (id.toLowerCase() === 'core' ? -1 : (reqIndex.get(id.toLowerCase()) ?? Number.MAX_SAFE_INTEGER));

    /* 3 – compute in-degrees */
    const inDeg = new Map();
    nodes.forEach(n => inDeg.set(n, 0));
    for (const tos of edges.values()) tos.forEach(to => inDeg.set(to, inDeg.get(to) + 1));
    ...
    if (sorted.length < nodes.size) {
        ...
        throw new ModDependencyError(`DEPENDENCY_CYCLE: Cyclic dependency detected among mods: ${cycle.join(', ')}`);
        // --- END TICKET CHANGE ---
    }
    ...
    const differs =
        ...
        originalOrder.some((id, idx) => id.toLowerCase() !== (sorted[idx] ?? '').toLowerCase());
    ...
    logger.info(`modLoadOrderResolver: Resolved load order (${sorted.length} mods): ${sorted.join(' → ')}`);
