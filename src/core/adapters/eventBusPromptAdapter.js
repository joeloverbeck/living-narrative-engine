...
export class EventBusPromptAdapter extends IPromptOutputPort {
    ...
    constructor({ safeEventDispatcher, validatedEventDispatcher }) {
        ...
        if (safeEventDispatcher && typeof safeEventDispatcher.dispatchSafely === 'function') {
            ...
            console.warn("EventBusPromptAdapter: ISafeEventDispatcher not provided or invalid, falling back to IValidatedEventDispatcher. Dispatch errors may not be caught gracefully by the adapter.");
        } else {
            throw new Error('EventBusPromptAdapter: Requires a valid ISafeEventDispatcher (preferred) or IValidatedEventDispatcher.');
        }
    }
    ...
    async prompt(entityId, availableActions, error) {
        ...
        if (typeof entityId !== 'string' || !entityId) {
            ...
            throw new Error("EventBusPromptAdapter.prompt: entityId must be a non-empty string.");
        }
        if (!Array.isArray(availableActions)) {
            console.error("EventBusPromptAdapter.prompt: availableActions must be an array.");
        ...
        if (this.#isSafeDispatcher) {
            ...
            try {
                ...
                console.error(`EventBusPromptAdapter: Error dispatching 'core:player_turn_prompt' via VED: ${dispatchError.message}`, dispatchError);
