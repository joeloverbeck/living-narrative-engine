...
describe('LogHandler', () => {
  ...
  test('constructor should throw if logger is missing', () => {
    expect(() => new LogHandler({})).toThrow('LogHandler requires a valid ILogger instance.');
  });

  test('constructor should throw if logger is invalid (missing methods)', () => {
    expect(() => new LogHandler({ logger: { info: jest.fn() } })).toThrow('LogHandler requires a valid ILogger instance.'); // Missing other methods
    expect(() => new LogHandler({ logger: 'not a logger' })).toThrow('LogHandler requires a valid ILogger instance.');
  });
  ...
  test('execute should default to "info" level if level is invalid', () => {
  ...
  test('execute should log error and not call log methods if params is null', () => {
    ...
    expect(mockLogger.warn).not.toHaveBeenCalled();
    expect(mockLogger.debug).not.toHaveBeenCalled();
    // Note: error was called ONLY for the parameter validation, not for logging the (non-existent) message
  });
  ...
  test('execute should use fallback value for missing top-level variables', () => {
    ...
    logHandler.execute(params, baseMockContext);
    expect(mockLogger.info).toHaveBeenCalledWith(`Missing: ${INTERPOLATION_FALLBACK}`);
  });
  ...
  test('execute should handle null values correctly (use fallback)', () => {
    const params = { message: 'Null value: {nullVar}' };
    logHandler.execute(params, baseMockContext);
    expect(mockLogger.info).toHaveBeenCalledWith(`Null value: ${INTERPOLATION_FALLBACK}`);
  });
  ...
  test('execute should handle empty context for interpolation', () => {
    ...
    const emptyContext = { logger: mockLogger, evaluationContext: {} };
  ...
  test('execute should log error to console if logger method throws', () => {
    ...
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation(() => {}); // Also spy on fallback log
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ message: 'Test logger failure', originalError: error })
    );
