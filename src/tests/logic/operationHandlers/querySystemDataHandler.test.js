 ...
 * @param {ILogger} [specificLoggerForEvalContext=null] - Logger for evaluationContext.logger.
 * @param {ILogger} [topLevelLogger=null] - Logger for the top-level executionContext.logger.
 * @param {any} [actorData=null] - Data for actor properties.
 * @param {any} [targetData=null] - Data for target properties.
...
function buildNestedMockExecutionContext(
    ...
    return {
        ...
        evaluationContext: {    // Nested evaluation context
            event: eventData,
...
describe('QuerySystemDataHandler', () => {
    ...
    describe('Constructor', () => {
        test('initializes successfully with valid dependencies', () => {
            ...
            handler = new QuerySystemDataHandler({logger: logger, systemDataRegistry: mockSystemDataRegistry});
            expect(handler).toBeInstanceOf(QuerySystemDataHandler);
            expect(logger.debug).toHaveBeenCalledWith('QuerySystemDataHandler: Instance created successfully.');
        });

        test('throws TypeError if logger dependency is missing or invalid', () => {
            const invalidLoggers = [null, undefined, {}, {info: jest.fn(), warn: jest.fn(), error: jest.fn()}];
        ...
        test('throws TypeError if systemDataRegistry dependency is missing or invalid', () => {
            ...
            const invalidRegistries = [null, undefined, {}, {query: 'not a function'}];
            invalidRegistries.forEach(invalidRegistry => {
                expect(() => new QuerySystemDataHandler({logger: tempLogger, systemDataRegistry: invalidRegistry}))
    ...
    describe('execute', () => {
        ...
        describe('Parameter Validation', () => {
            ...
            ])('logs error and returns if params are invalid (%s)', (desc, invalidParams) => {
            ...
            ])('logs error and returns if "query_details" is invalid (missing)', (desc, params) => {
                const ctx = buildNestedMockExecutionContext();
                handler.execute(params, ctx);
                expect(mockConstructorLogger.error).toHaveBeenCalledWith(
            ...
            test('logs error and returns if "query_details" is invalid (undefined)', () => {
                const params = {source_id: 's', query_details: undefined, result_variable: 'r'};
                const ctx = buildNestedMockExecutionContext();
                handler.execute(params, ctx);
                expect(mockConstructorLogger.error).toHaveBeenCalledWith(
                    'QUERY_SYSTEM_DATA: Missing required "query_details" parameter.',
            ...
            test('allows null as query_details', () => {
                ...
                expect(mockConstructorLogger.error).not.toHaveBeenCalledWith(expect.stringContaining('query_details'));
        ...
        describe('Execution Context Validation', () => {
            ...
            test.each([
                ...
                ['evaluationContext without context', {evaluationContext: {logger: createMockLogger()}}],
        ...
        describe('Core Execution Scenarios', () => {
            test('executes successfully, calls registry.query, stores result in context', () => {
                ...
                expect(mockConstructorLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Successfully queried source "${defaultSourceId}"`));
            });
            ...
            test('handles successful query returning complex object', () => {
                ...
                expect(mockConstructorLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Stored result in "${defaultResultVariable}": ${JSON.stringify(complexResult)}`));
            });
            ...
            test('handles successful query returning null', () => {
                ...
                expect(mockConstructorLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Stored result in "${defaultResultVariable}": null`));
            });
            ...
            test('handles query error (registry throws), stores undefined, logs error', () => {
                ...
                expect(mockConstructorLogger.error).toHaveBeenCalledWith(
                    ...
                    expect.objectContaining({error: queryError.message, stack: queryError.stack})
                );
                expect(mockConstructorLogger.warn).toHaveBeenCalledWith(
                    `QUERY_SYSTEM_DATA: Query to source "${defaultSourceId}" returned undefined or an error occurred during query. Stored 'undefined' in "${defaultResultVariable}".`
                );
            ...
            test('uses ONLY the constructor logger, ignoring any logger in execution context', () => {
                ...
                const ctx = buildNestedMockExecutionContext({}, specificEvalContextLogger, specificTopLevelLogger);

                mockSystemDataRegistry.query.mockReturnValue(successResult);
                handler.execute(defaultParams, ctx);

                // Verify the CONSTRUCTOR logger (mockConstructorLogger) was used
                expect(mockConstructorLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Attempting to query source "${defaultSourceId}"`));
            ...
            test('handles non-stringifiable result during logging gracefully', () => {
                ...
                expect(mockConstructorLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Stored result in "${defaultResultVariable}": [Could not stringify result]`));
            });
        });
    });
});