...
import OperationInterpreter from '../../logic/operationInterpreter.js'; // Corrected path
import OperationRegistry from '../../logic/operationRegistry.js';   // Corrected path
...
describe('SystemLogicInterpreter Integration Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockLogger = { // Define mockLogger here so it's accessible to helpers
    ...
    it('executes actions when rule has no condition', async () => {
        ...
        expect(mockLogger.error).not.toHaveBeenCalledWith(expect.stringContaining('Failed to assemble'));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Cached rule '${RULE_NO_COND_BASIC.rule_id}'`));
        expect(mockLogger.debug).toHaveBeenCalledWith(
            expect.stringMatching(new RegExp(`^Received event: ${eventType}\\. Found \\d+ potential rule\\(s\\)\\.$`)),
            {payload: eventPayload}
        );
        // These logs should now be present and match
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Assembling JsonLogic context via createJsonLogicContext... (ActorID: ${eventPayload.actorId}, TargetID: ${eventPayload.targetId})`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] createJsonLogicContext returned a valid JsonLogicEvaluationContext.`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Final ExecutionContext (nested structure) assembled successfully.`));
        ...
        expect(executeSpy).toHaveBeenCalledWith(RULE_NO_COND_BASIC.actions[0], expectedNestedCtx);

        expect(mockLogHandler).toHaveBeenCalledTimes(1);
        expect(mockLogHandler).toHaveBeenCalledWith(TEST_ACTION_LOG.parameters, expectedNestedCtx);
    ...
    it('executes actions when rule condition evaluates to true', async () => {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Assembling JsonLogic context via createJsonLogicContext...`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Final ExecutionContext (nested structure) assembled successfully.`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Rule ${RULE_COND_TRUE_BASIC.rule_id}] Condition found. Evaluating using jsonLogicDataForEval...`));
        ...
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(RULE_COND_TRUE_BASIC.condition, expectedFlatCtx);
        ...
        expect(executeSpy).toHaveBeenCalledWith(RULE_COND_TRUE_BASIC.actions[0], expectedNestedCtx);
    ...
    it('does NOT execute actions when rule condition evaluates to false', async () => {
        ...
        const {eventType} = await arrangeAndAct(RULE_COND_FALSE_BASIC, eventPayload, false); // condition expects false, evaluate returns false
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Assembling JsonLogic context via createJsonLogicContext...`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Event: ${eventType}] Final ExecutionContext (nested structure) assembled successfully.`));
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`[Rule ${RULE_COND_FALSE_BASIC.rule_id}] Condition found. Evaluating using jsonLogicDataForEval...`));
        ...
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(RULE_COND_FALSE_BASIC.condition, expectedFlatCtx);
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledTimes(1);
