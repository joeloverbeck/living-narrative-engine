...
describe('JsonLogicEvaluationService - Target Component Access Tests ([PARENT_ID].7)', () => {
  ...
  beforeEach(() => {
    ...
    mockEntityManager.getComponentData.mockImplementation((entityId, compId) => undefined);
  });
  ...
  describe('Target Component Access (target.components.*)', () => {
    ...
    describe('Component Existence Checks', () => {
      const componentExistsRule = {'!!': {'var': `target.components.${componentId}`}};
      const componentIsNullRule = {'==': [{'var': `target.components.${componentId}`}, null]};
      const componentIsNotNullRule = {'!=': [{'var': `target.components.${componentId}`}, null]};
      ...
      test('{"!!": "target.components.armor"} should return true when component exists', () => {
        ...
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, componentId);
    ...
    describe('Property Access (Component Exists)', () => {
      const propertyValueRule = (value) => ({'==': [{'var': `target.components.${componentId}.${propertyId}`}, value]});
      const missingPropertyIsNullRule = {'==': [{'var': `target.components.${componentId}.${missingPropertyId}`}, null]};
      const nestedPropertyValueRule = (value) => ({'==': [{'var': `target.components.${statsComponentId}.${nestedPropertyPath}`}, value]});
      const nestedMissingPropertyIsNullRule = {'==': [{'var': `target.components.${statsComponentId}.material.missingNested`}, null]};
      const nestedMissingMiddlePropertyIsNullRule = {'==': [{'var': `target.components.${statsComponentId}.missingMiddle.hardness`}, null]};
      ...
      test('{"==": ["target.components.attributes.material.hardness", 7]} should return true for nested property access', () => {
        ...
        expect(result).toBe(true);
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, statsComponentId);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      test('{"==": ["target.components.attributes.material.missingNested", null]} should return true for missing nested property', () => {
        ...
        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(nestedMissingPropertyIsNullRule, context);
      ...
      test('{"==": ["target.components.attributes.missingMiddle.hardness", null]} should return true for missing intermediate nested property', () => {
        ...
        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(nestedMissingMiddlePropertyIsNullRule, context);
    ...
    describe('Property Access (Component Missing)', () => {
      const missingComponentPropertyIsNullRule = {'==': [{'var': `target.components.${missingComponentId}.someProp`}, null]};
      ...
      test('{"==": ["target.components.missingComp.someProp", null]} should return true when component is missing', () => {
        ...
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === targetId && compId === missingComponentId ? undefined : undefined // Explicit undefined for this component
        );

        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(missingComponentPropertyIsNullRule, context);

        expect(result).toBe(true);
        expect(mockEntityManager.getEntityInstance).toHaveBeenCalledWith(targetId);
        // Check getComponentData was called with the missing ID for the target
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, missingComponentId);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
    }); // End describe Property Access (Component Missing)

    describe('Namespaced Component Access', () => {
      const namespacedPropValueRule = (value) => ({'==': [{'var': `target.components.${namespacedComponentId}.status`}, value]});
      const namespacedMissingPropIsNullRule = {'==': [{'var': `target.components.${namespacedComponentId}.missingStatus`}, null]};
      const namespacedCompMissingIsNullRule = {'==': [{'var': `target.components.${namespacedComponentId}`}, null]};
      ...
      test('{"==": ["target.components.game:state.status", "active"]} should return true when namespaced component and property exist', () => {
        ...
        mockEntityManager.getEntityInstance.mockReturnValue(mockTarget);
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === targetId && compId === namespacedComponentId ? nsComponentData : undefined
        );

        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(namespacedPropValueRule('active'), context);

        expect(result).toBe(true);
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, namespacedComponentId);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      test('{"==": ["target.components.game:state.status", "active"]} should return false when namespaced component exists but property value differs', () => {
        ...
        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(namespacedPropValueRule('active'), context); // Rule checks for active
      ...
      test('{"==": ["target.components.game:state.missingStatus", null]} should return true when namespaced component exists but property is missing', () => {
        ...
        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(namespacedMissingPropIsNullRule, context);
      ...
      test('{"==": ["target.components.game:state", null]} should return true when namespaced component itself is missing', () => {
        ...
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === targetId && compId === namespacedComponentId ? undefined : undefined
        );

        const context = createJsonLogicContext(baseEvent, actorId, targetId, mockEntityManager, mockLogger);
        const result = service.evaluate(namespacedCompMissingIsNullRule, context);
    ...
    describe('Target Missing', () => {
      const anyComponentPathRule = {'==': [{'var': `target.components.${componentId}.${propertyId}`}, null]};
      const anyComponentExistenceRule = {'!!': {'var': `target.components.${componentId}`}};
      ...
      test('accessing any component path should resolve to null when target in context is null', () => {
        ...
        expect(mockEntityManager.getEntityInstance).not.toHaveBeenCalledWith(null); // Shouldn't be called with null
      ...
      test('accessing any component path should resolve to null when target entity is not found', () => {
        ...
        const context = createJsonLogicContext(baseEvent, actorId, missingTargetId, mockEntityManager, mockLogger);
        const result = service.evaluate(anyComponentPathRule, context);
