...
/** @typedef {import('../../logic/defs.js').JsonLogicEvaluationContext} JsonLogicEvaluationContextFlat */ // Renamed for clarity
/** @typedef {import('../../logic/defs.js').ExecutionContext} ExecutionContextNested */ // Assuming this is the nested one
...
import OperationRegistry from '../../logic/operationRegistry.js'; // Corrected import path
...
const MOCK_ENEMY_HP_5 = {
    id: 'enemy-1',
    components: {'health': {current: 5, max: 10}, 'attributes': {vulnerable: true}},
    getComponentData: function (type) {
        return this.components[type];
    }, hasComponent: function (type) {
        return type in this.components;
...
const MOCK_ENEMY_HP_0 = {
    id: 'enemy-2',
    components: {'health': {current: 0, max: 10}, 'attributes': {vulnerable: false}},
    getComponentData: function (type) {
        return this.components[type];
    }, hasComponent: function (type) {
        return type in this.components;
...
const MOCK_EVENT_ENEMY_SPOTTED = {type: 'ENEMY_SPOTTED', payload: {actorId: 'player-2', targetId: 'enemy-1'}};
const MOCK_EVENT_ENEMY_SPOTTED_NO_BUFF = {type: 'ENEMY_SPOTTED', payload: {actorId: 'player-1', targetId: 'enemy-1'}};
const MOCK_EVENT_ITEM_PICKED = {type: 'ITEM_PICKED_UP', payload: {actorId: 'player-1', itemId: 'potion-123'}};
const MOCK_EVENT_PLAYER_HEALED = {type: 'PLAYER_HEALED', payload: {targetId: 'player-1', amount: 10}};
const MOCK_EVENT_DOOR_OPENED = {type: 'DOOR_OPENED', payload: {actorId: 'player-1', doorId: 'door-main-hall'}};
...
const MOCK_EVENT_CUSTOM = {type: 'CUSTOM_EVENT', payload: {entityId: 'player-1', value: 42, source: 'magic_crystal'}};
...
describe('SystemLogicInterpreter - Integration Tests - Conditional Execution Setup', () => {
    ...
    const createExpectedFlatJsonLogicContext = (event, actorEntity, targetEntity) => {
        return {
            event: expect.objectContaining({
                type: event.type,
                // Match payload precisely or allow for null if event.payload could be undefined
                payload: event.payload === undefined || event.payload === null ? null : expect.objectContaining(event.payload),
            }),
            actor: actorEntity
                ? expect.objectContaining({id: actorEntity.id, components: expect.any(Object)})
                : null,
            target: targetEntity
                ? expect.objectContaining({id: targetEntity.id, components: expect.any(Object)})
                : null,
    ...
    const createExpectedNestedContext = (event, actorEntity, targetEntity, topLevelLoggerInstance) => {
        // The actor/target at the top level of finalNestedExecutionContext come from jsonLogicDataForEvaluation.actor/target
        const flatLogicContextForNesting = createExpectedFlatJsonLogicContext(event, actorEntity, targetEntity);
        return {
            event: expect.objectContaining({
                type: event.type,
                payload: event.payload === undefined || event.payload === null ? null : expect.objectContaining(event.payload)
            }),
            actor: actorEntity ? expect.objectContaining({id: actorEntity.id, components: expect.any(Object)}) : null,
            target: targetEntity ? expect.objectContaining({
                id: targetEntity.id,
    ...
    beforeEach(() => {
        ...
        mockDataRegistry = {getAllSystemRules: jest.fn().mockReturnValue([]), getEntityDefinition: jest.fn(),};
        mockJsonLogicEvaluationService = {evaluate: jest.fn().mockReturnValue(true),};
        mockEntityManager = {
            ...
            getComponentData: jest.fn(), hasComponent: jest.fn(),
        };
        ...
        interpreter = new SystemLogicInterpreter({
            ...
            entityManager: mockEntityManager, operationInterpreter: operationInterpreterRealInstance // Pass the real instance
        });
    ...
    it('should subscribe to the EventBus during initialization if rules are loaded', () => {
        mockDataRegistry.getAllSystemRules.mockReturnValue([MOCK_RULE_NO_CONDITION]);
        interpreter.initialize();
    ...
    it('should load and cache rules from the DataRegistry during initialization', () => {
        ...
        expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Finished caching rules. 2 event types have associated rules.'));
    });
    ...
    it('should call the mock JsonLogicEvaluationService.evaluate when processing a rule with a condition', () => {
        ...
        const expectedFlatCtx = createExpectedFlatJsonLogicContext(event, MOCK_PLAYER_WITH_BUFF, MOCK_ENEMY_HP_5, mockLogger);
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(rule.condition, expectedFlatCtx);

        expect(executeSpy).toHaveBeenCalledTimes(1);
        // OperationInterpreter.execute receives the NESTED context
        const expectedNestedCtx = createExpectedNestedContext(event, MOCK_PLAYER_WITH_BUFF, MOCK_ENEMY_HP_5, mockLogger);
    ...
    it('should use the mock EntityManager to assemble context data', () => {
        ...
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledTimes(1);
        // JsonLogicEvaluationService.evaluate receives the FLAT context
        const expectedFlatCtx = createExpectedFlatJsonLogicContext(event, MOCK_PLAYER_NO_BUFF, null, mockLogger);
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(rule.condition, expectedFlatCtx);

        expect(executeSpy).toHaveBeenCalledTimes(1);
        // OperationInterpreter.execute receives the NESTED context
        const expectedNestedCtx = createExpectedNestedContext(event, MOCK_PLAYER_NO_BUFF, null, mockLogger);
        expect(executeSpy).toHaveBeenCalledWith(rule.actions[0], expectedNestedCtx);
    });
    ...
    it('should call OperationInterpreter.execute when a rule condition passes or is absent', () => {
        ...
        const expectedNestedForItem = createExpectedNestedContext(eventItem, MOCK_PLAYER_NO_BUFF, null, mockLogger);
        expect(executeSpy).toHaveBeenCalledTimes(1);
        expect(executeSpy).toHaveBeenCalledWith(ruleNoCond.actions[0], expectedNestedForItem);
        ...
        const expectedFlatForSpottedCond = createExpectedFlatJsonLogicContext(eventSpotted, MOCK_PLAYER_WITH_BUFF, MOCK_ENEMY_HP_5, mockLogger);
        const expectedNestedForSpottedOp = createExpectedNestedContext(eventSpotted, MOCK_PLAYER_WITH_BUFF, MOCK_ENEMY_HP_5, mockLogger);

        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledTimes(1);
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(ruleCondTrue.condition, expectedFlatForSpottedCond);
        expect(executeSpy).toHaveBeenCalledTimes(1);
        expect(executeSpy).toHaveBeenCalledWith(ruleCondTrue.actions[0], expectedNestedForSpottedOp);
    });
    ...
    it('should NOT call OperationInterpreter.execute when a rule condition fails', () => {
        ...
        const expectedFlatCtx = createExpectedFlatJsonLogicContext(event, MOCK_PLAYER_NO_BUFF, MOCK_ENEMY_HP_5, mockLogger);
        expect(mockJsonLogicEvaluationService.evaluate).toHaveBeenCalledWith(rule.condition, expectedFlatCtx);
    ...
    it('should NOT call OperationInterpreter.execute when condition evaluation throws an error', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(
            expect.stringContaining("Error during condition evaluation. Treating condition as FALSE."),
            evaluationError
        );
        expect(mockLogger.info).toHaveBeenCalledWith(
    ...
    it('should have all required mock rules, entities, and events defined', () => {
        expect(MOCK_RULE_INVISIBILITY_BUFF).toBeDefined(); /* ... and so on for all mocks ... */
