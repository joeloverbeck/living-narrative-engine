...
import Entity from '../../entities/entity.js'; // Adjust path if needed for mock creation
...
describe('TEST-109: Validate JSON-LOGIC-PATTERNS.MD - Compound Logic (Patterns 12-14)', () => {
  ...
  beforeEach(() => {
    ...
    mockEntityManager.getComponentData.mockImplementation((entityId, componentTypeId) => null);
  });
  ...
  describe('AC1: Pattern 12 - AND Lockable', () => {
    ...
    test('should evaluate TRUE when target exists, component exists, AND state is "locked"', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === targetId ? mockTarget : undefined);
      ...
      expect(mockEntityManager.getComponentData.mock.calls.length).toBeGreaterThanOrEqual(1);
    });
    ...
    test('should evaluate FALSE when target exists, component exists, BUT state is NOT "locked"', () => {
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, componentId);
      expect(mockEntityManager.getComponentData.mock.calls.length).toBeGreaterThanOrEqual(1); // Called for component access
    });
  ...
  describe('AC2: Pattern 12 - AND Key/Door', () => {
    ...
    test('should evaluate TRUE when all conditions are met', () => {
      ...
      // No need to mock hasComponent for this rule path
      mockEntityManager.getComponentData.mockImplementation((id, compId) => {
        if (id === actorId && compId === actorKeyComp) return {desc: 'A shiny key'}; // For !! var check
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, actorKeyComp);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, targetLockComp);
      // hasComponent should NOT have been called by this rule
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
    ...
    test('should evaluate FALSE when actor is missing the key component', () => {
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, actorKeyComp); // getComponentData was called for the check
    ...
    test('should evaluate FALSE when target ID does not match', () => {
      ...
      mockEntityManager.getComponentData.mockImplementation((id, compId) => {
        if (id === actorId && compId === actorKeyComp) return {desc: 'A shiny key'};
        // State of other door doesn't matter due to ID check failing first
        if (id === otherTargetId && compId === targetLockComp) return lockedState;
        return null;
      });
      const context = createJsonLogicContext(baseEvent, actorId, otherTargetId, mockEntityManager, mockLogger);
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, actorKeyComp); // First part called
    ...
    test('should evaluate FALSE when target exists, ID matches, but state is not "locked"', () => {
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, actorKeyComp);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, targetLockComp); // Both checks performed
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
  ...
  describe('AC3: Pattern 13 - OR Status', () => {
    ...
    test('should evaluate TRUE if actor has poison component ONLY', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === actorId ? mockActor : undefined);
      mockEntityManager.getComponentData.mockImplementation((id, compId) => {
      ...
      // Assert
      expect(result).toBe(true);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, poisonComp); // Called for poison check
      expect(mockEntityManager.getComponentData).toHaveBeenCalledTimes(1); // Short-circuited
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
    ...
    test('should evaluate TRUE if actor has disease component ONLY', () => {
      ...
      // Assert
      expect(result).toBe(true);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, poisonComp); // Called for poison check (returns null)
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, diseaseComp); // Called for disease check
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
    ...
    test('should evaluate FALSE if actor has NEITHER poison nor disease component', () => {
      ...
      // Assert
      expect(result).toBe(false);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, poisonComp);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, diseaseComp);
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
  ...
  describe('AC4: Pattern 14 - NOT Lockable', () => {
    ...
    test('should evaluate TRUE when target state is NOT "locked" (e.g., "unlocked")', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === targetId ? mockTarget : undefined);
      mockEntityManager.getComponentData.mockImplementation((id, compId) => (id === targetId && compId === componentId) ? unlockedState : null);
      const context = createJsonLogicContext(baseEvent, null, targetId, mockEntityManager, mockLogger);
      ...
      // Assert
      expect(result).toBe(true);
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(targetId, componentId); // Called by var
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
    ...
    test('should evaluate TRUE when target state is NOT "locked" (e.g., "jammed")', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === targetId ? mockTarget : undefined);
      mockEntityManager.getComponentData.mockImplementation((id, compId) => (id === targetId && compId === componentId) ? jammedState : null);
      const context = createJsonLogicContext(baseEvent, null, targetId, mockEntityManager, mockLogger);
    ...
    test('should evaluate FALSE when target state IS "locked"', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === targetId ? mockTarget : undefined);
      mockEntityManager.getComponentData.mockImplementation((id, compId) => (id === targetId && compId === componentId) ? lockedState : null);
      const context = createJsonLogicContext(baseEvent, null, targetId, mockEntityManager, mockLogger);
  ...
  describe('AC6: Pattern 14 - NOT Status', () => {
    ...
    test('should evaluate TRUE when component is missing', () => {
      ...
      expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, componentId); // Called by var
      expect(mockEntityManager.hasComponent).not.toHaveBeenCalled();
    });
    ...
    test('should evaluate FALSE when component exists', () => {
      // Arrange
      mockEntityManager.getEntityInstance.mockImplementation(id => id === actorId ? mockActor : undefined);
      mockEntityManager.getComponentData.mockImplementation((id, compId) => (id === actorId && compId === componentId) ? componentData : null);
      const context = createJsonLogicContext(baseEvent, actorId, null, mockEntityManager, mockLogger);
