...
import {resolvePlaceholders} from '../../logic/contextUtils.js'; // Adjust path to your contextUtils.js
import resolvePath from '../../utils/resolvePath.js'; // Assuming resolvePath is used internally and correctly located
...
const createMockExecutionContext = (customContextVars = {}, eventData = {}, actorData = null, targetData = null) => ({
    event: {type: 'mockEvent', payload: eventData},
    actor: actorData || {id: 'mockActor', name: 'ActorName', components: {stats: {health: 100}}},
    ...
    evaluationContext: {
        ...
        actor: actorData || {id: 'mockActor', name: 'ActorName', components: {stats: {health: 100}}}, // Also often duplicated
        target: targetData,
        context: { // This is the actual variable store for {context.varName}
            varA: 'valueA',
            numVar: 123,
            boolVar: true,
            nullVar: null,
...
describe('resolvePlaceholders (contextUtils.js)', () => {
    ...
    describe('1. Full String Placeholder Resolution ("{...}" entire string)', () => {
        test('1.1 should resolve `context.` path for a string variable', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Resolved full string placeholder {context.varA} to: valueA'));
        });
        ...
        test('1.5 should resolve `context.` path for an object variable (returns raw object)', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toEqual({nestedKey: 'nestedValue', otherNum: 456});
        });

        test('1.6 should resolve a nested `context.` path (e.g., {context.objVar.nestedKey})', () => {
            const context = createMockExecutionContext();
            const input = "{context.objVar.nestedKey}";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("nestedValue");
        });
        ...
        test('1.10 should return original string if `context.` path is not found and log warning', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("{context.nonExistentVar}");
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Placeholder path "context.nonExistentVar" (interpreted as "nonExistentVar") from {context.nonExistentVar} could not be resolved.'));
        });

        test('1.11 should return original string if `event.` path is not found and log warning', () => {
            const context = createMockExecutionContext();
            const input = "{event.nonExistentKey}";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("{event.nonExistentKey}");
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Placeholder path "event.nonExistentKey" (interpreted as "event.nonExistentKey") from {event.nonExistentKey} could not be resolved.'));
        });
        ...
        test('1.12 should return original string if `context.` path used but `evaluationContext.context` is missing, and log warning', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("{context.varA}");

            expect(mockLogger.warn).toHaveBeenCalledTimes(2); // Ensure exactly two warnings
            expect(mockLogger.warn.mock.calls[0][0]).toBe(`Placeholder "context.varA" uses "context." prefix, but executionContext.evaluationContext.context is missing or invalid. Path: ${placeholderSyntax}`);
            expect(mockLogger.warn.mock.calls[1][0]).toBe(`Placeholder path "context.varA" (interpreted as "context.varA") from ${placeholderSyntax} could not be resolved. Path: ${placeholderSyntax}`);
        });
        ...
        test('1.14 should return original string if executionContext itself is not an object, and log warning', () => {
            const input = "{context.varA}";
            expect(resolvePlaceholders(input, null, mockLogger)).toBe("{context.varA}");
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Cannot resolve placeholder path "context.varA" from {context.varA}: executionContext is not a valid object.'));
        });
    ...
    describe('2. Embedded Placeholder Resolution ("String with {...} in it")', () => {
        test('2.1 should resolve a single embedded `context.` placeholder', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Value is valueA.");
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Replaced embedded placeholder {context.varA} with string: "valueA"'));
        });

        test('2.2 should resolve multiple embedded placeholders (context, event, actor)', () => {
            const context = createMockExecutionContext({score: 9000}, {detail: 'urgent'});
            const input = "Actor: {actor.name}, Event: {event.payload.detail}, Score: {context.score}, Num: {context.numVar}.";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Actor: ActorName, Event: urgent, Score: 9000, Num: 123.");
        });

        test('2.3 should resolve embedded `context.` placeholder that is null', () => {
            const context = createMockExecutionContext();
            const input = "This is {context.nullVar}.";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("This is null.");
        });
        ...
        test('2.4 should resolve embedded `context.` placeholder that is an object (stringifies to [object Object])', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Object: [object Object].");
        });
        ...
        test('2.5 should resolve embedded `context.` placeholder that is an array (stringifies with commas)', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Array: item1,[object Object].");
        });


        test('2.6 should handle mixed resolved and unresolved embedded placeholders', () => {
            const context = createMockExecutionContext();
            const input = "Found: {context.varA}, Missing: {context.nonExistent}, Event: {event.type}.";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Found: valueA, Missing: {context.nonExistent}, Event: mockEvent.");
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Embedded placeholder path "context.nonExistent" (interpreted as "nonExistent") from {context.nonExistent} could not be resolved.'));
        });
        ...
        test('2.7 should handle embedded `context.` path when `evaluationContext.context` is missing, and log warning', () => {
            ...
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Value: {context.varA}.");

            expect(mockLogger.warn).toHaveBeenCalledTimes(2); // Ensure exactly two warnings
            expect(mockLogger.warn.mock.calls[0][0]).toBe(`Embedded placeholder "context.varA" uses "context." prefix, but executionContext.evaluationContext.context is missing or invalid. Path: ${fullLogPathForEmbedded}`);
            expect(mockLogger.warn.mock.calls[1][0]).toBe(`Embedded placeholder path "context.varA" (interpreted as "context.varA") from ${placeholderSyntaxInLog} could not be resolved. Path: ${fullLogPathForEmbedded}`);
        });

        test('2.8 should not replace anything if no placeholders are present', () => {
            const context = createMockExecutionContext();
            const input = "Just a plain string.";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("Just a plain string.");
            expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Replaced embedded placeholder'));
        });
    });

    describe('3. Recursive Resolution (Objects and Arrays as Input)', () => {
        test('3.1 should resolve placeholders in object values', () => {
            const context = createMockExecutionContext({custom: "customVal"}, {data: "eventData"});
            const input = {
                key1: "{context.varA}",
        ...
        test('3.4 should return original object/array if no changes were made', () => {
            ...
            expect(resolvePlaceholders(inputObject, context, mockLogger)).toBe(inputObject); // Should be same instance

            const inputArray = ["plain", "strings", "only"];
            expect(resolvePlaceholders(inputArray, context, mockLogger)).toBe(inputArray); // Should be same instance
        });
    });
    ...
    describe('4. Non-String/Object/Array Inputs', () => {
        test.each([
            ...
            [new Date()],
            [() => "function"],
        ])('4.1 should return input %p as is if it is not a string, array, or plain object', (inputValue) => {
            const context = createMockExecutionContext();
            expect(resolvePlaceholders(inputValue, context, mockLogger)).toBe(inputValue);
        });
    });
    ...
    describe('5. Specific Scenarios from Original Bug Report', () => {
        test('5.1 should correctly resolve current_location_id from {context.actorPos.locId}', () => {
            ...
            expect(resolvePlaceholders(inputParams, specificContext, mockLogger)).toEqual(expectedParams);
        });
        ...
        test('5.2 should correctly resolve message string with {context.actorName.text} and {event.payload.direction}', () => {
            ...
            const inputMessage = "{context.actorName.text} tries to go {event.payload.direction}, but cannot find a path.";
            const expectedMessage = "Hero tries to go south, but cannot find a path.";
            expect(resolvePlaceholders(inputMessage, specificContext, mockLogger)).toBe(expectedMessage);
        });
    });
    ...
    describe('6. Edge case with placeholder path "context" (no dot)', () => {
        test('6.1 should resolve placeholder "{context}" from top-level if it exists', () => {
            ...
            const input = "{context}";
            expect(resolvePlaceholders(input, context, mockLogger)).toBe("This is a top-level context string");
        });
