...
const createMockFetcher = (idToResponse = {}, errorIds = []) => ({
    fetch: jest.fn(async (path) => { // Make mock async to match real fetcher
        ...
        if (path.includes('/schemas/action-definition.schema.json')) {
            return {$id: 'http://example.com/schemas/action-definition.schema.json', type: 'object'};
        }
...
const createMockRegistry = () => {
    ...
    const getAll = jest.fn((type) => {
        const typeMap = data.get(type);
        // Return deep copies
        return typeMap ? Array.from(typeMap.values()).map(obj => JSON.parse(JSON.stringify(obj))) : [];
    });
...
describe('WorldLoader → ModDependencyValidator integration (missing dependency)', () => {
    ...
    const manifestSchema = {
        ...
        properties: {
            ...
            conflicts: {type: 'array', default: [], items: {type: 'string', minLength: 1}},
            gameVersion: {type: 'string', minLength: 1},
            content: {type: 'object', default: {}, additionalProperties: {type: 'array', items: {type: 'string'}}}
        },
    ...
    beforeEach(async () => {
        ...
        /* -------------------- Fixture manifests ------------------------------ */
        const baseManifest = {id: 'basegame', name: 'Base Game', version: '1.0.0', content: {}};
        const badManifest = {
            id: 'badmod',
            name: 'Bad Mod',
            version: '0.1.0',
    ...
    it('rejects with ModDependencyError and fetches only mod manifests (no content)', async () => {
        ...
        expect(registry.store).toHaveBeenCalledWith('mod_manifests', 'basegame', expect.objectContaining({id: 'basegame'}));
        expect(registry.store).toHaveBeenCalledWith('mod_manifests', 'badmod', expect.objectContaining({id: 'badmod'}));
        // Final mod order shouldn't be stored
        expect(registry.store).not.toHaveBeenCalledWith('meta', 'final_mod_order', expect.any(Array));
