...
describe('WorldLoader Integration Test Suite (TEST-LOADER-7.1)', () => {
    ...
    beforeEach(() => {
        ...
        mockCoreManifest = {
            ...
            content: {
                ...
                characters: ['core/entity_player_base.json'], // <<< Using characters
            },
        ...
        mockConfiguration.getContentTypeSchemaId.mockImplementation((typeName) => {
            switch (typeName) {
                ...
                // *** NOTE: Still returning 'schema:entities' here based on your original test code.
                // If you have ALSO renamed the schema ID itself, you need to update this mock too.
                // If only the manifest content key changed, this is fine.
                case 'entities':
                    return 'schema:entities';
                case 'characters': // <<< Potentially add this if schema ID also changed
                    return 'schema:characters'; // <<< Example if schema ID changed
                default:
                    return `schema:${typeName}`; // Return something for others if needed
            }
        });

        // Configure ISchemaValidator for essential schemas
        // *** NOTE: Adjust 'schema:entities' to 'schema:characters' if the schema ID itself changed.
        mockValidator.isSchemaLoaded.mockImplementation((schemaId) => {
            return ['schema:game', 'schema:components', 'schema:mod-manifest', 'schema:entities' /* or 'schema:characters' */].includes(schemaId);
        });
        ...
        const setupContentLoaderMock = (loaderMock, typeName, count) => {
            loaderMock.loadItemsForMod.mockImplementation(async (modIdArg, manifestArg, contentKeyArg, contentTypeDirArg, typeNameArg) => {
                ...
                // Check against the typeName used in the setup call
                if (modIdArg.toLowerCase() === coreModId.toLowerCase() && typeNameArg === typeName) {
                    ...
                    for (let i = 0; i < count; i++) {
                        // *** Use the correct typeName passed to setupContentLoaderMock for registry storage ***
                        const itemId = `${modIdArg}:${typeName}_item_${i}`; // e.g., core:characters_item_0
                        const itemData = {id: itemId, data: `mock ${typeName} data ${i}`};
                        mockRegistry.store(typeName, itemId, itemData); // Simulate storage with correct typeName
                    }
        ...
        // *** FIX: Use 'characters' as the typeName for the EntityLoader simulation ***
        setupContentLoaderMock(mockEntityLoader, 'characters', 1);   // Use 'characters' here
    ...
    it('should successfully load world with only the "core" mod', async () => {
        ...
        // *** NOTE: Adjust 'schema:entities' if the schema ID also changed ***
        ...
        expect(mockValidator.isSchemaLoaded).toHaveBeenCalledWith('schema:entities'); // or 'schema:characters'
        ...
        // Check EntityLoader was called for 'characters'
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenCalledWith(
            coreModId,
            mockCoreManifest,
            'characters', // contentKey (from manifest)
            'characters', // contentTypeDir (from WorldLoader config)
            'characters'  // typeName (from WorldLoader config)
        );
        ...
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(
            expect.anything(), expect.anything(), 'items', expect.anything(), expect.anything()
        );
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(
            expect.anything(), expect.anything(), 'locations', expect.anything(), expect.anything()
        );
        // etc for items, locations if EntityLoader handles them via different keys
        ...
        // *** FIX: Expect 'characters' as the type and the corresponding item ID ***
        expect(mockRegistry.store).toHaveBeenCalledWith('characters', 'core:characters_item_0', expect.any(Object));
        ...
        expect(summaryLines).toEqual(expect.arrayContaining([
            ...
            // *** FIX: Expect 'characters' in the summary ***
            expect.stringMatching(/characters\s+: 1 loaded/),   // From mockEntityLoader returning 1 (using 'characters')
        ...
        // Ensure old 'entities' type isn't shown
