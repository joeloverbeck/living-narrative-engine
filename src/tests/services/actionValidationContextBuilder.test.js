...
describe('ActionValidationContextBuilder', () => {
    ...
    it('should throw an error if EntityManager dependency is invalid', () => {
        ...
        expect(() => new ActionValidationContextBuilder({entityManager: null, logger: mockLogger}))
            .toThrow('ActionValidationContextBuilder requires a valid EntityManager with getEntityInstance and getComponentData methods.');
        const incompleteEntityManager = {getEntityInstance: jest.fn()}; // Missing getComponentData
        expect(() => new ActionValidationContextBuilder({entityManager: incompleteEntityManager, logger: mockLogger}))
            .toThrow('ActionValidationContextBuilder requires a valid EntityManager with getEntityInstance and getComponentData methods.');
        ...
        describe("Scenario 2: Target Type 'entity', Entity Not Found", () => {
            ...
            it('should build the context with null target entity and log a warning', () => {
                ...
                expect(mockLogger.warn).toHaveBeenCalledWith(
                    ...
                    expect.stringContaining(`Target entity '${missingTargetId}' not found for action '${sampleActionDefinition.id}'. Context will have null target entity data.`)
                );
                // --- END FIX ---
                expect(mockLogger.error).not.toHaveBeenCalled();
        ...
        describe("Scenario 3: Target Type 'direction'", () => {
            ...
            it('should build the context correctly for a direction target (when no exit data is found)', () => {
                ...
                mockEntityManager.getComponentData.mockImplementation((entityId, componentId) => {
                    ...
                    if (entityId === 'loc:current' && componentId === EXITS_COMPONENT_ID) {
                ...
                expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, POSITION_COMPONENT_ID);
                expect(mockEntityManager.getComponentData).toHaveBeenCalledWith('loc:current', EXITS_COMPONENT_ID);
                expect(mockLogger.warn).toHaveBeenCalledWith(
                    expect.stringContaining(`No valid ${EXITS_COMPONENT_ID} data (or not an array) found for location 'loc:current'`)
                );
                expect(mockLogger.error).not.toHaveBeenCalled();
                expect(mockLogger.debug).toHaveBeenCalledWith(
                ...
                expect(mockLogger.debug).toHaveBeenCalledWith(
                    expect.stringContaining(`Actor '${actorId}' is at location 'loc:current'. Fetching exits for direction '${direction}'.`)
                );
            });
            ...
            it('should build the context with resolved exitDetails and blocker for a found direction target', () => {
                ...
                const specificTargetContext = ActionTargetContext.forDirection(specificDirection);
                ...
                mockEntityManager.getComponentData.mockImplementationOnce((entityId, componentId) => {
                    if (entityId === actorId && componentId === POSITION_COMPONENT_ID) {
                    ...
                    .mockImplementationOnce((entityId, componentId) => {
                        if (entityId === actorCurrentLocationId && componentId === EXITS_COMPONENT_ID) {
                            return [
                ...
                expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Found matching exit for direction '${specificDirection}'`), mockExitObject);
            });
