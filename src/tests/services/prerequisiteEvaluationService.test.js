...
describe('PrerequisiteEvaluationService', () => {
    ...
    beforeEach(() => {
        ...
        mockJsonLogicServiceInstance = new JsonLogicEvaluationService({logger: mockLogger});
        mockBuilderInstance = new ActionValidationContextBuilder({logger: mockLogger});
    ...
    test('should return true and log debug message when prerequisites array is empty', () => {
        const prerequisites = [];
        const result = service.evaluate(prerequisites, mockActionDefinition, mockActor, mockTargetContext);
    ...
    test('should return true and log appropriately when all prerequisite rules pass', () => {
        ...
        expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(mockActionDefinition, mockActor, mockTargetContext);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenCalledTimes(prerequisites.length);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenNthCalledWith(1, prerequisites[0].logic, mockBuiltContext);
    ...
    test('should return false and log failure when the first prerequisite rule fails', () => {
        ...
        const result = service.evaluate(prerequisites, specificActionDef, mockActor, mockTargetContext);

        expect(result).toBe(false);
        expect(mockBuilderInstance.buildContext).toHaveBeenCalledTimes(1);
        expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(specificActionDef, mockActor, mockTargetContext);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenCalledTimes(1);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenCalledWith(prerequisites[0].logic, mockBuiltContext);
        // ... log checks ...
        expect(mockLogger.error).not.toHaveBeenCalled();
    });
    ...
    describe('Context Builder Error Handling', () => {
        test('should return false, log error, and not call JsonLogic when buildContext throws', () => {
            ...
            const result = service.evaluate(prerequisites, errorActionDef, mockActor, mockTargetContext);

            expect(result).toBe(false);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledTimes(1);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(errorActionDef, mockActor, mockTargetContext);
            expect(mockLogger.error).toHaveBeenCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith(
                expect.stringContaining(`PrereqEval[${errorActionDef.id}]: ← FAILED (Internal Error: Failed to build evaluation context`),
    ...
    describe('Invalid Prerequisite Item Types (Non-Object)', () => {
        ...
        ])('should return false and log error if an item in prerequisites is a $description', ({value}) => {
            const prerequisites = [value];
            const result = service.evaluate(prerequisites, invalidItemActionDef, mockActor, mockTargetContext);

            expect(result).toBe(false);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledTimes(1);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(invalidItemActionDef, mockActor, mockTargetContext);
            expect(mockLogger.error).toHaveBeenCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith(
                expect.stringContaining(`PrereqEval[${invalidItemActionDef.id}]: ← FAILED (Rule 1/1): Prerequisite item is not a valid object: ${JSON.stringify(value)}`)
            );
            expect(mockJsonLogicServiceInstance.evaluate).not.toHaveBeenCalled();
        });
    });
    ...
    describe("Invalid Prerequisite 'logic' Property", () => {
        ...
        test.each([
            {prereq: {failure_message: "No logic here"}, description: "missing 'logic' property"},
            ...
            const result = service.evaluate(prerequisites, invalidLogicActionDef, mockActor, mockTargetContext);

            expect(result).toBe(false);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledTimes(1);
            expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(invalidLogicActionDef, mockActor, mockTargetContext);
            expect(mockLogger.error).toHaveBeenCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith(
                expect.stringContaining(`PrereqEval[${invalidLogicActionDef.id}]: ← FAILED (Rule 1/1): Prerequisite object is missing the required 'logic' property or it's not an object: ${JSON.stringify(prereq)}`)
            );
            expect(mockJsonLogicServiceInstance.evaluate).not.toHaveBeenCalled();
        });
    });
    ...
    test('should return false and log error if jsonLogicEvaluationService.evaluate throws an error', () => {
        ...
        const result = service.evaluate(prerequisites, jsonLogicErrorActionDef, mockActor, mockTargetContext);

        expect(result).toBe(false);
        expect(mockBuilderInstance.buildContext).toHaveBeenCalledTimes(1);
        expect(mockBuilderInstance.buildContext).toHaveBeenCalledWith(jsonLogicErrorActionDef, mockActor, mockTargetContext);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenCalledTimes(1);
        expect(mockJsonLogicServiceInstance.evaluate).toHaveBeenCalledWith(prerequisites[0].logic, mockBuiltContext);
        expect(mockLogger.error).toHaveBeenCalledTimes(1);
        expect(mockLogger.error).toHaveBeenCalledWith(
            expect.stringContaining(`PrereqEval[${jsonLogicErrorActionDef.id}]: ← FAILED (Rule 1/1): Error during JsonLogic evaluation. Rule: ${JSON.stringify(prerequisites[0])}`),
            expect.objectContaining({error: mockError.message, stack: mockError.stack})
        );
