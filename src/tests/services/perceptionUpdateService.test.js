...
import {PERCEPTION_LOG_COMPONENT_ID} from "../../constants/componentIds.js"; // Adjust path as needed
...
/** @typedef {import('../../services/perceptionUpdateService.js').LogEntry} LogEntry */
/** @typedef {import('../../services/perceptionUpdateService.js').AddEntryParams} AddEntryParams */
/** @typedef {import('../../services/perceptionUpdateService.js').PerceptionUpdateResult} PerceptionUpdateResult */
/** @typedef {import('../../services/perceptionUpdateService.js').QueryDetailsForPerceptionUpdate} QueryDetailsForPerceptionUpdate */
...
describe('PerceptionUpdateService', () => {
    describe('Constructor', () => {
        test('should create an instance with valid dependencies', () => {
            const service = new PerceptionUpdateService({logger: mockLogger, entityManager: mockEntityManager});
            expect(service).toBeInstanceOf(PerceptionUpdateService);
            expect(mockLogger.info).toHaveBeenCalledWith('PerceptionUpdateService: Instance created.');
        });

        test('should throw error if logger is missing or invalid', () => {
            expect(() => new PerceptionUpdateService({entityManager: mockEntityManager}))
                .toThrow('PerceptionUpdateService: Valid ILogger instance is required.');
            expect(() => new PerceptionUpdateService({logger: {}, entityManager: mockEntityManager}))
                .toThrow('PerceptionUpdateService: Valid ILogger instance is required.');
            expect(() => new PerceptionUpdateService({
                logger: {info: jest.fn()},
        ...
        test('should throw error if entityManager is missing or invalid', () => {
            expect(() => new PerceptionUpdateService({logger: mockLogger}))
                .toThrow('PerceptionUpdateService: Valid IEntityManager instance with required methods (getEntitiesInLocation, hasComponent, getComponentData, addComponent) is required.');
            ...
            expect(() => new PerceptionUpdateService({
                ...
                .toThrow('PerceptionUpdateService: Valid IEntityManager instance with required methods');
        });
    });
    ...
    describe('handleQuery Method', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(service, 'addEntryToLogsInLocation').mockResolvedValue({success: true, logsUpdated: 0});
        });
        ...
        test('should dispatch to addEntryToLogsInLocation for "addEntryToLogsInLocation" action', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('PerceptionUpdateService.handleQuery: Received query', {queryDetails});
        });
        ...
        test('should return error for an unknown action', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith('PerceptionUpdateService.handleQuery: Unknown action: unknownAction', {queryDetails});
        ...
        test('should return error for invalid queryDetails (null, undefined, no action)', async () => {
            ...
            for (const invalidQuery of testCases) {
                ...
                const result = await service.handleQuery(/** @type {any} */(invalidQuery));
                expect(result).toEqual({
                    success: false,
                    error: "Invalid queryDetails: must be an object with a non-empty 'action' string property.",
                    logsUpdated: 0
                });
    ...
    describe('addEntryToLogsInLocation Method', () => {
        ...
        beforeEach(() => {
            ...
            mockEntityManager.hasComponent.mockImplementation((entityId, componentId) =>
                entityId === perceiver1Id && componentId === PERCEPTION_LOG_COMPONENT_ID
            );
        ...
        describe('Parameter Validation', () => {
            test('should return error if params object itself is invalid', async () => {
                ...
                for (const invalidParams of invalidParamsList) {
                    ...
                    const result = await service.addEntryToLogsInLocation(/** @type {any} */(invalidParams));
                    expect(result).toEqual({
                        success: false,
            ...
            test('should return error if entry object is invalid or missing', async () => {
                ...
                for (const invalidEntry of invalidEntries) {
                    mockLogger.error.mockClear();
                    const params = {locationId, entry: /** @type {any} */(invalidEntry)};
                    const result = await service.addEntryToLogsInLocation(params);
                    expect(result).toEqual({
                        success: false,
            ...
            requiredEntryFields.forEach(field => {
                test(`should return error if required entry field "${field}" is missing or invalid`, async () => {
                    ...
                    let result = await service.addEntryToLogsInLocation({locationId, entry});
                    expect(result).toEqual({
                        success: false,
            ...
            test('should allow empty string for optional entry.targetId', async () => {
                ...
                const result = await service.addEntryToLogsInLocation({locationId, entry});
        ...
        describe('Entity Querying & Basic Log Updates', () => {
            ...
            test('should return success with logsUpdated: 0 if location has entities but none have perception_log component', async () => {
                ...
                mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([nonPerceiverId, 'item:another_rock']));
                ...
                expect(result).toEqual({success: true, logsUpdated: 0, warnings: []});
                expect(mockEntityManager.getEntitiesInLocation).toHaveBeenCalledWith(locationId);
                expect(mockEntityManager.hasComponent).toHaveBeenCalledWith(nonPerceiverId, PERCEPTION_LOG_COMPONENT_ID);
                expect(mockEntityManager.hasComponent).toHaveBeenCalledWith('item:another_rock', PERCEPTION_LOG_COMPONENT_ID);
                expect(mockEntityManager.getComponentData).not.toHaveBeenCalled();
                expect(mockEntityManager.addComponent).not.toHaveBeenCalled();
                expect(mockLogger.info).toHaveBeenCalledWith(
                    `PerceptionUpdateService: No entities with '${PERCEPTION_LOG_COMPONENT_ID}' found in location '${locationId}'. No logs will be updated.`
            ...
            test('should initialize log for a single perceiver if it has no existing core:perception_log', async () => {
                ...
                expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(perceiver1Id, PERCEPTION_LOG_COMPONENT_ID);
                expect(mockEntityManager.addComponent).toHaveBeenCalledTimes(1);
                expect(mockEntityManager.addComponent).toHaveBeenCalledWith(perceiver1Id, PERCEPTION_LOG_COMPONENT_ID, {
                    maxEntries: DEFAULT_MAX_LOG_ENTRIES,
                    logEntries: [entry], // Entry directly used as eventId was provided
                });
                expect(mockLogger.error).toHaveBeenCalledWith( // Service logs this as an error due to unexpected state
            ...
            test('should append to existing log for a single perceiver', async () => {
                ...
                mockEntityManager.getComponentData.mockReturnValue(JSON.parse(JSON.stringify(initialLogData))); // Deep clone like service
            ...
            test('should update logs for multiple perceivers in a location', async () => {
                ...
                mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([perceiver1Id, nonPerceiverId, perceiver2Id]));
                mockEntityManager.hasComponent.mockImplementation((entityId, componentId) => {
                    if (componentId !== PERCEPTION_LOG_COMPONENT_ID) return false;
                    return entityId === perceiver1Id || entityId === perceiver2Id;
                });

                const logData1 = {maxEntries: DEFAULT_MAX_LOG_ENTRIES, logEntries: [existingLogEntry1]};
                ...
                mockEntityManager.getComponentData.mockImplementation((entityId, componentId) => {
                    if (componentId === PERCEPTION_LOG_COMPONENT_ID) {
                        if (entityId === perceiver1Id) return JSON.parse(JSON.stringify(logData1));
                ...
                expect(mockEntityManager.addComponent).toHaveBeenCalledWith(perceiver2Id, PERCEPTION_LOG_COMPONENT_ID, {
                    maxEntries: DEFAULT_MAX_LOG_ENTRIES,
                    logEntries: [entry],
                });
                expect(mockLogger.info).toHaveBeenCalledWith(
            ...
            test('should generate eventId if not provided in the entry', async () => {
                ...
                expect(updatedLogData.logEntries[0].eventId).toEqual(expect.stringMatching(/^ple_\d{13}_[a-z0-9]{9}$/));
                expect(updatedLogData.logEntries[0].descriptionText).toBe(entry.descriptionText); // Check other fields still match
            });
        });
        ...
        describe('Log Appending, Trimming, and Data Correction', () => {
            test('should append log entry when count is less than maxEntries', async () => {
                ...
                const existingEntries = [createSampleLogEntry('old1'), createSampleLogEntry('old2')];
            ...
            test('should append log entry when count reaches maxEntries (no trimming yet)', async () => {
                ...
                expect(result.success).toBe(true);
                expect(result.logsUpdated).toBe(1);
                const updatedLog = mockEntityManager.addComponent.mock.calls[0][2].logEntries;
            ...
            test('should append and trim oldest entries when logEntries count exceeds maxEntries', async () => {
                ...
                const result = await service.addEntryToLogsInLocation({locationId, entry: newEntry});
            ...
            test('should reset logEntries to empty array if existing logEntries is not an array, then append', async () => {
                ...
                expect(mockLogger.warn).toHaveBeenCalledWith(`PerceptionUpdateService: ${warningMsg}`);
            ...
            test('should use default maxEntries if existing maxEntries is invalid (e.g., not a number, < 1), then append/trim', async () => {
                ...
                const existingEntries = Array.from({length: DEFAULT_MAX_LOG_ENTRIES}, (_, i) => {
                ...
                const invalidMaxEntriesValues = [0, -5, "not-a-number", null, undefined];
                for (const invalidMax of invalidMaxEntriesValues) {
                    ...
                    expect(updatedLogData.logEntries.length).toBe(DEFAULT_MAX_LOG_ENTRIES); // Trimming happened based on default
                    expect(updatedLogData.logEntries[DEFAULT_MAX_LOG_ENTRIES - 1]).toEqual(entry); // Newest entry is last
                }
            });
        });
    });
