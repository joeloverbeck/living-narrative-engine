...
import Entity from '../../entities/entity.js'; // Import Entity if creating instances
...
describe('TargetResolutionService - Domain \'equipment\'', () => {
    ...
    describe('5.8: Ambiguous Match', () => {
        test('should return AMBIGUOUS if noun phrase matches multiple items', async () => {
            ...
            expect(result.targetType).toBe('entity');
            expect(result.targetId).toBeNull();
            expect(result.candidates).toEqual(expect.arrayContaining(['goldRing', 'silverRing']));
            expect(result.candidates.length).toBe(2);
            expect(result.error).toMatch(/Which item containing "ring" did you mean\? For example: "Gold Ring", "Silver Ring"/);
        });
    });
