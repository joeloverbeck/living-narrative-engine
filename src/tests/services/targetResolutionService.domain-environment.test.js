...
import Entity from '../../entities/entity.js'; // Available if complex mocks are needed
...
describe('TargetResolutionService - Domain \'environment\'', () => {
    ...
    const createActionContext = (nounPhraseValue, currentActingEntity = mockActorEntity, currentMockLocation = mockLocationEntity) => {
    ...
    describe('6.1: Actor Location Not Found', () => {
        test('should return ERROR if actor location cannot be determined', async () => {
            ...
            expect(result.error).toBe("Internal error: Cannot determine your current location.");
    ...
    describe('6.2: No Entities in Location (Besides actor)', () => {
        describe('6.2.1: nounPhrase empty', () => {
            ...
            test('should return NOT_FOUND with "There is nothing here." if location only contains the actor', async () => {
                mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([mockActorEntity.id]));
                const actionContext = createActionContext(''); // Empty nounPhrase
    ...
    describe('6.3: Entity in Location Not Found by EntityManager', () => {
        test('should skip entity, log, and return NOT_FOUND if entity is not found by EntityManager', async () => {
            ...
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([ghostEntityId, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
    ...
    describe('6.4: Entity in Location Has No Name', () => {
        test('should skip entity, log, and return NOT_FOUND if entity has no name', async () => {
            ...
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([namelessEntity.id, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
                if (id === mockActorEntity.id) return mockActorEntity;
    ...
    describe('6.6: No Noun Phrase Provided (entities exist)', () => {
        test('should return NONE with "You need to specify which item here."', async () => {
            ...
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([goblinEntity.id, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
    ...
    describe('6.7: Unique Match', () => {
        test('should find a unique entity by exact name ("Goblin Guard")', async () => {
            const goblinGuardEntity = createMockEnvEntity('goblinGuard1', 'Goblin Guard');
            const orcEntity = createMockEnvEntity('orc1', 'Orc Peon');
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([goblinGuardEntity.id, orcEntity.id, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
    ...
    describe('6.8: Ambiguous Match', () => {
        test('should return AMBIGUOUS for "chest" with "Wooden Chest" and "Iron Chest"', async () => {
            const woodenChestEntity = createMockEnvEntity('woodChest', 'Wooden Chest');
            const ironChestEntity = createMockEnvEntity('ironChest', 'Iron Chest');
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([woodenChestEntity.id, ironChestEntity.id, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
            ...
            expect(result.targetType).toBe('entity');
            expect(result.targetId).toBeNull();
            expect(result.candidates).toEqual(expect.arrayContaining([woodenChestEntity.id, ironChestEntity.id]));
            expect(result.candidates.length).toBe(2);
            expect(result.error).toMatch(/Which item containing "chest" did you mean\? For example: "Wooden Chest", "Iron Chest"./);
        });
    });
    ...
    describe('Entity name fallback in environment', () => {
        test('should use entity.name if NAME_COMPONENT_ID is missing for an environment entity', async () => {
            ...
            mockEntityManager.getEntitiesInLocation.mockReturnValue(new Set([itemWithFallbackName.id, mockActorEntity.id]));
            mockEntityManager.getEntityInstance.mockImplementation(id => {
            ...
            const actionContext = createActionContext('Fallback Item from Environment'); // Corrected context
            const result = await service.resolveActionTarget(actionDefinition, actionContext);
