...
import ValidatedEventDispatcher from '../../services/validatedEventDispatcher.js'; // Adjust path as needed
...
jest.mock('../../core/eventBus.js'); // Mock EventBus module
jest.mock('../../core/services/gameDataRepository.js'); // Mock GameDataRepository module
...
describe('ValidatedEventDispatcher', () => {
    ...
    describe('Constructor', () => {
        ...
        test('should throw error if eventBus dependency is missing', () => {
            expect(() => new ValidatedEventDispatcher({
                gameDataRepository: mockGameDataRepository,
                schemaValidator: mockSchemaValidator,
                logger: mockLogger,
            })).toThrow("ValidatedEventDispatcher: Missing required dependency 'eventBus'.");
        });
        ...
        test('should throw error if gameDataRepository dependency is missing', () => {
            expect(() => new ValidatedEventDispatcher({
                ...
                schemaValidator: mockSchemaValidator,
                logger: mockLogger,
            })).toThrow("ValidatedEventDispatcher: Missing required dependency 'gameDataRepository'.");
        });
        ...
        test('should throw error if schemaValidator dependency is missing', () => {
            expect(() => new ValidatedEventDispatcher({
                ...
                gameDataRepository: mockGameDataRepository,
                logger: mockLogger,
            })).toThrow("ValidatedEventDispatcher: Missing required dependency 'schemaValidator'.");
        });
        ...
        test('should throw error if logger dependency is missing', () => {
            expect(() => new ValidatedEventDispatcher({
                ...
                gameDataRepository: mockGameDataRepository,
                schemaValidator: mockSchemaValidator,
            })).toThrow("ValidatedEventDispatcher: Missing required dependency 'logger'.");
        });
        ...
        test('should log info message on successful instantiation', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith("ValidatedEventDispatcher: Instance created.");
    ...
    describe('dispatchValidated', () => {
        ...
        const eventDefinitionWithSchema = {
            ...
            payloadSchema: {type: 'object', properties: {data: {type: 'string'}}},
        };
        ...
        test('should dispatch event successfully when validation passes', async () => {
            ...
            expect(mockGameDataRepository.getEventDefinition).toHaveBeenCalledWith(eventName);
            expect(mockSchemaValidator.isSchemaLoaded).toHaveBeenCalledWith(schemaId);
            expect(mockSchemaValidator.validate).toHaveBeenCalledWith(schemaId, payload);
            expect(mockEventBus.dispatch).toHaveBeenCalledWith(eventName, payload);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Validating payload'));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('validation SUCCEEDED'));
            // FIX: Check for payload in dispatch log message
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Dispatching event'), payload);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('dispatch successful'));
        ...
        test('should NOT dispatch event and return false when validation fails', async () => {
            // Arrange
            const validationErrors = [{instancePath: '/data', message: 'should be number'}];
            mockGameDataRepository.getEventDefinition.mockReturnValue(eventDefinitionWithSchema);
            mockSchemaValidator.isSchemaLoaded.mockReturnValue(true);
            mockSchemaValidator.validate.mockReturnValue({valid: false, errors: validationErrors});
        ...
        test('should handle validation failure when errors array is null/undefined', async () => {
            ...
            mockGameDataRepository.getEventDefinition.mockReturnValue(eventDefinitionWithSchema);
            mockSchemaValidator.isSchemaLoaded.mockReturnValue(true);
            mockSchemaValidator.validate.mockReturnValue({valid: false, errors: null}); // No error details
        ...
        test('should dispatch event but log warning if schema is not loaded', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(
                expect.stringContaining(`Payload schema '${schemaId}' not found/loaded for event '${eventName}'. Skipping validation`)
            );
            expect(mockLogger.error).not.toHaveBeenCalled();
            // FIX: Check for payload in dispatch log message
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Dispatching event'), payload);
        ...
        test('should dispatch event without validation if definition has no payloadSchema', async () => {
            // Arrange
            mockGameDataRepository.getEventDefinition.mockReturnValue(basicEventDefinition); // No payloadSchema
            mockEventBus.dispatch.mockResolvedValue(undefined);
            ...
            expect(mockEventBus.dispatch).toHaveBeenCalledWith(eventName, payload);
            expect(mockLogger.debug).toHaveBeenCalledWith(
                expect.stringContaining(`Event definition '${eventName}' found, but no 'payloadSchema' defined. Skipping validation`)
            );
            // FIX: Check for payload in dispatch log message
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Dispatching event'), payload);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('dispatch successful'));
        ...
        test('should dispatch event but log warning if event definition is not found', async () => {
            ...
            expect(mockEventBus.dispatch).toHaveBeenCalledWith(eventName, payload);
            expect(mockLogger.warn).toHaveBeenCalledWith(
                expect.stringContaining(`EventDefinition not found for '${eventName}'. Cannot validate payload. Proceeding with dispatch.`)
            );
            // FIX: Check for payload in dispatch log message
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Dispatching event'), payload);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('dispatch successful'));
        ...
        test('should return false and log error if eventBus.dispatch throws an error', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                expect.stringContaining(`Error occurred during EventBus.dispatch for event '${eventName}'`),
                dispatchError
            );
            // Ensure success/skip messages are not logged in this failure case
            expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('dispatch successful'));
            expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Dispatch explicitly skipped'));
        });
    });
});