...
import {getEntityIdsForScopes} from "../../services/entityScopeService"; // Import the constant
...
describe('TargetResolutionService - Domain \'direction\'', () => {
    ...
    const createActionContext = (nounPhraseValue, currentActingEntity = mockActorEntity) => {
        return {
            ...
            parsedCommand: {
                ...
                originalInput: typeof nounPhraseValue === 'string' ? nounPhraseValue : '',
                error: null,
                preposition: null,
    ...
    beforeEach(() => {
        ...
        mockWorldContext.getLocationOfEntity.mockReturnValue({ id: 'defaultMockLoc' });
    ...
    describe('7.1: Current Location Not Found', () => {
        test('should return ERROR if current location is unknown (actorEntity null)', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith("TargetResolutionService.#_resolveDirection: Could not determine current location. actorEntity ID: undefined");
        });
        ...
        test('should return ERROR if current location is unknown (actorEntity non-null)', async () => {
            ...
            expect(result.targetId).toBeNull();
            expect(result.error).toBe("Internal error: Your location is unknown.");
            expect(mockWorldContext.getCurrentLocation).toHaveBeenCalledWith(mockActorEntity);
            expect(mockLogger.warn).toHaveBeenCalledWith("TargetResolutionService.#_resolveDirection: Could not determine current location. actorEntity ID: actor1");
        });

        test('should return ERROR if current location entity has no ID', async () => {
            mockWorldContext.getCurrentLocation.mockReturnValue({ name: "A nameless void" }); // No .id property
            const actionContext = createActionContext('north');
    ...
    describe('7.2: Location Has No EXITS_COMPONENT_ID / Component is Empty/Invalid Array', () => {
        ...
        test('should return NOT_FOUND if location has no EXITS_COMPONENT_ID', async () => {
            ...
            expect(mockLocationNoExits.getComponentData).toHaveBeenCalledWith(EXITS_COMPONENT_ID);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`has no '${EXITS_COMPONENT_ID}' component`));
        });
    ...
    describe('7.3: exitsComponentData Contains Invalid Exit Objects (skipped, logged)', () => {
        ...
        beforeEach(() => {
            ...
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithMixedExits);
        });
        ...
        test('should skip invalid exit objects and still find valid ones', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid exit object or missing/empty direction string: null"));
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid exit object or missing/empty direction string: {\"direction\":123}"));
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid exit object or missing/empty direction string: {\"direction\":\"\"}"));
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid exit object or missing/empty direction string: {\"someOtherProp\":\"East\"}"));
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid exit object or missing/empty direction string: {\"direction\":\"  \"}"));
        });
    });
    ...
    describe('7.4: All Exits in Component are Invalid', () => {
        ...
        beforeEach(() => {
            ...
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithOnlyInvalidExits);
        });
        ...
        test('should return NOT_FOUND with "no clearly marked exits" if all exits are invalid', async () => {
            ...
            mockLocationWithOnlyInvalidExits.getComponentData.mockReturnValue(exitsData);
            const actionContext = createActionContext('north'); // Corrected actionContext
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("no valid direction strings found"));
        });
    });
    ...
    describe('7.5: No Noun Phrase Provided (valid exits exist)', () => {
        ...
        beforeEach(() => {
            ...
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithValidExits);
            mockLocationWithValidExits.getComponentData.mockReturnValue([{ direction: "North" }, { direction: "South" }]);
        });
        ...
        test.each([
        ...
        ])('should return NONE "Which direction...?" for %s', async (nounPhraseValue, _description) => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith("TargetResolutionService.#_resolveDirection: No nounPhrase (direction) provided.");
        });
    });
    ...
    describe('7.6: Unique Direction Match (Case-Insensitive)', () => {
        ...
        beforeEach(() => {
            ...
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithExits);
        ...
        test('should find a unique direction match case-insensitively (e.g. "north" for "North")', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith("TargetResolutionService.#_resolveDirection: Found unique direction: 'North'");
        });
    ...
    describe('7.7: No Matching Direction', () => {
        ...
        beforeEach(() => {
            mockLocationWithExits.getComponentData.mockReset();
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithExits);
            mockLocationWithExits.getComponentData.mockReturnValue([{ direction: "West" }, { direction: "Up" }]);
        });
    ...
    describe('7.8: Ambiguous Direction Match', () => {
        ...
        beforeEach(() => {
            ...
            mockWorldContext.getCurrentLocation.mockReturnValue(mockLocationWithAmbiguousExits);
        ...
        test('should return AMBIGUOUS if nounPhrase matches multiple exits due to casing', async () => {
            ...
            expect(result.targetType).toBe('direction');
            expect(result.targetId).toBeNull();
            expect(result.error).toBe(`The direction "${nounPhraseValue}" is ambiguously defined here.`);
