...
// Class under test
import MoveCoordinatorSystem from '../../systems/moveCoordinatorSystem.js'; // Adjust path if needed
...
describe('MoveCoordinatorSystem', () => {
    ...
    describe('#handleMoveAttempted', () => {
        ...
        it('should dispatch action:move_failed with TARGET_LOCATION_NOT_FOUND if target location is missing', async () => {
            const payload = { ...basePayload };
            ...
            expect(mockEventBus.dispatch).toHaveBeenCalledWith('action:move_failed', {
        ...
        it('should dispatch action:move_failed with blocker details if checkMovementBlock returns blocked', async () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining(`Movement blocked for actor ${payload.entityId}. Blocker: ${blockerResult.blockerDisplayName}`));
        });

        // AC 6: Failure: Movement Execution Returns `false` (TRG-8.5)
        it('should dispatch action:move_failed with MOVEMENT_EXECUTION_FAILED if executeMove returns false', async () => {
            const payload = { ...basePayload };
        ...
        it('should dispatch action:move_failed with MOVE_EXECUTION_ERROR if executeMove throws an error', async () => {
            const payload = { ...basePayload };
        ...
        it('should dispatch action:move_failed with COORDINATOR_INTERNAL_ERROR if a dependency throws unexpectedly (top-level catch)', async () => {
            const payload = { ...basePayload };
