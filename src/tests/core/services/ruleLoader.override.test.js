...
describe('ComponentDefinitionLoader (Sub-Ticket 6.3: Override Behavior)', () => {
    ...
    it('should override component definition and schema from a later mod', async () => {
        ...
        //    - Warning for schema overwrite uses the full ID from the file.
        ...
        //    - REMOVED: Expectation for definition overwrite warning, as it's not logged in this scenario.
        // expect(mockLogger.warn).toHaveBeenCalledWith(
        //     expect.stringContaining(`Overwriting existing ${registryCategory} definition with key '${fooQualifiedId}'.`)
        // );

        // Ensure exactly ONE warning (schema overwrite) fired during Phase 2
        expect(mockLogger.warn).toHaveBeenCalledTimes(1); // <<< CORRECTED EXPECTED COUNT
        ...
        //    Note: The actual log comes from BaseManifestItemLoader
        expect(mockLogger.debug).toHaveBeenCalledWith(
