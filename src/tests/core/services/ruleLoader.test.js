// src/tests/core/services/ruleLoader.test.js
...
const createMockDataFetcher = (pathToResponse = {}, errorPaths = []) => ({
    ...
    mockSuccess: function (path, responseData) {
        ...
        this.fetch.mockImplementation(async (p) => {
            if (errorPaths.includes(p)) return Promise.reject(new Error(`Mock Fetch Error: Failed to fetch ${p}`));
            if (Object.prototype.hasOwnProperty.call(pathToResponse, p)) return Promise.resolve(JSON.parse(JSON.stringify(pathToResponse[p])));
            return Promise.reject(new Error(`Mock Fetch Error: 404 Not Found for path ${p}`));
        });
    },
    ...
    mockFailure: function (path, errorMessage = `Mock Fetch Error: Failed to fetch ${path}`) {
        ...
        // Re-assign the mock function to capture the updated closures
        this.fetch.mockImplementation(async (p) => {
            if (errorPaths.includes(p)) return Promise.reject(new Error(errorMessage)); // Use provided message
            if (Object.prototype.hasOwnProperty.call(pathToResponse, p)) return Promise.resolve(JSON.parse(JSON.stringify(pathToResponse[p])));
            return Promise.reject(new Error(`Mock Fetch Error: 404 Not Found for path ${p}`));
        });
    }
 ...
 * @param {object} [overrides={}] - Optional overrides for mock methods.
 * @returns {import('../../../core/interfaces/coreServices.js').ISchemaValidator & {_setSchemaLoaded: Function, mockValidatorFunction: Function}} Mocked schema validator service with test helpers.
 */
const createMockSchemaValidator = (overrides = {}) => {
    ...
    const mockValidator = {
        ...
        validate: jest.fn((schemaId, data) => {
            ...
            if (!loadedSchemas.has(schemaId)) { // Check if schema is even supposed to be loaded
        ...
        resetValidatorFunction: (schemaId) => {
            if (schemaValidators.has(schemaId)) {
                schemaValidators.get(schemaId).mockImplementation(() => ({isValid: true, errors: null}));
            } else {
    ...
    return {
        ...
        store: jest.fn((type, id, data) => {
            ...
            try {
                ...
                console.error(`MockDataRegistry Error: Could not clone data for ${type}/${id}. Is it valid JSON?`, data);
        ...
        setManifest: jest.fn((data) => { /* Store manifest if needed */
        }),
...
describe('RuleLoader (Sub-Ticket 4.1: Test Setup & Mocking)', () => {
    ...
    const setupMockFetcherFailure = (path, error = new Error(`Mock Fetch Error: Failed to fetch ${path}`)) => {
    ...
    const setupMockValidatorFailure = (errors, schemaId = defaultRuleSchemaId) => {
    ...
    beforeEach(() => {
        ...
        mockValidator._setSchemaLoaded(defaultRuleSchemaId, { /* mock schema object if needed */});
    ...
    it('mocks should allow configuration per test using helpers', () => {
        const testPath = `./data/mods/${modId}/system-rules/rule1.json`;
        const testData = {rule_id: 'test-rule', event_type: 'test-event', actions: []};
        ...
        expect(mockFetcher.fetch(testPath)).resolves.toEqual(testData); // Check fetcher setup
        expect(mockValidator.validate(defaultRuleSchemaId, {})).toEqual({isValid: false, errors: validationErrors}); // Check validator setup
    });
