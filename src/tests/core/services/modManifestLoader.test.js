...
const createMockFetcher = (idToResponse = {}, errorIds = []) => ({
    fetch: jest.fn((path) => {
        ...
        if (errorIds.includes(modId)) return Promise.reject(new Error(`Fail ${modId}`));
...
describe('ModManifestLoader — branch edges', () => {
    ...
    it('throws on empty id', async () => {
        await expect(buildLoader(deps).loadRequestedManifests([''])).rejects.toThrow(TypeError);
        expect(deps.logger.error).toHaveBeenCalledWith(
...
describe('ModManifestLoader — integration (AjvSchemaValidator)', () => {
    ...
    it('loads two valid mods, skips failing fetch, and stores exactly two', async () => {
        ...
        expect(deps.registry.store).toHaveBeenNthCalledWith(1, 'mod_manifests', 'good1', m1);
        expect(deps.registry.store).toHaveBeenNthCalledWith(2, 'mod_manifests', 'good2', m2);
