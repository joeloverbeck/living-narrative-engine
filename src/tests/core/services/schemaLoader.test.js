...
describe('SchemaLoader', () => {
  ...
  it('should not try to add a schema if it is already loaded', async () => {
    ...
    // Assert
    expect(mockConfiguration.getSchemaFiles).toHaveBeenCalledTimes(1);
    expect(mockPathResolver.resolveSchemaPath).toHaveBeenCalledTimes(filesToLoad.length); // Called for both
    ...
    expect(mockSchemaValidator.isSchemaLoaded).toHaveBeenCalledTimes(filesToLoad.length);
    expect(mockSchemaValidator.isSchemaLoaded).toHaveBeenCalledWith(commonSchemaId);
    expect(mockSchemaValidator.isSchemaLoaded).toHaveBeenCalledWith(entitySchemaId);
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(`SchemaLoader: Processing ${filesToLoad.length} schemas listed in configuration...`);

    // Check the debug log for skipping the loaded schema
    expect(mockLogger.debug).toHaveBeenCalledWith(`SchemaLoader: Schema '${commonSchemaId}' from ${commonSchemaFile} already loaded. Skipping addition.`);
    expect(mockLogger.debug).toHaveBeenCalledTimes(1); // Only one was skipped

    // Check final success log message (only 1 *new* schema was added)
    expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Schema processing complete. Added 1 new schemas'));
  ...
  it('should throw and log error if a schema file is missing $id', async () => {
    ...
    mockConfiguration.getSchemaFiles.mockReturnValue([commonSchemaFile, badSchemaFile]);
    mockPathResolver.resolveSchemaPath.mockImplementation(filename => `./test/schemas/${filename}`);
    mockDataFetcher.fetch.mockImplementation(async (path) => {
      if (path === commonSchemaPath) return commonSchemaData;
    ...
    // Act & Assert
    await expect(schemaLoader.loadAndCompileAllSchemas())
      .rejects
      .toThrow(`Schema file ${badSchemaFile} (at ${badSchemaPath}) is missing required '$id' property.`);
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('SchemaLoader: One or more configured schemas failed to load or process. Aborting.'),
  ...
  describe('Constructor Dependency Validation', () => {
    ...
    it('should throw error if configuration is missing or invalid', () => {
      ...
      expect(() => new SchemaLoader(invalidConfig, mockPathResolver, mockDataFetcher, mockSchemaValidator, mockLogger)).toThrow(/configuration/);
    });
