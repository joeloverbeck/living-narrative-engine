...
describe('WorldLoader Integration Test Suite - Mod Overrides and Load Order (Sub-Ticket 8)', () => {
    ...
    beforeEach(() => {
        ...
        mockFooManifest = {
            id: fooModId, version: '1.0.0', name: 'Foo Mod', gameVersion: '^1.0.0',
            content: { items: [itemFilename] },
        };
        mockBarManifest = {
            id: barModId, version: '1.0.0', name: 'Bar Mod', gameVersion: '^1.0.0',
            content: { items: [itemFilename] },
        };
        ...
        mockGameConfigLoader.loadConfig.mockResolvedValue([coreModId, fooModId, barModId]);
        ...
        mockEntityLoader.loadItemsForMod.mockImplementation(async (modIdArg, manifestArg, contentKeyArg, contentTypeDirArg, typeNameArg) => {
            if (typeNameArg === 'items' && contentKeyArg === 'items') {
                // Determine which data to "load" and store based on the modId
                let itemData;
                if (modIdArg === coreModId) {
                    itemData = { id: `${coreModId}:${baseItemId}`, description: 'Core potion effect', value: 10 };
                } else if (modIdArg === fooModId) {
                    itemData = { id: `${fooModId}:${baseItemId}`, description: 'Foo potion effect (override)', value: 20 };
                } else if (modIdArg === barModId) {
                    itemData = { id: `${barModId}:${baseItemId}`, description: 'Bar potion effect (final override)', value: 30 };
                } else {
    ...
    it('should load content respecting finalOrder and handle overrides correctly', async () => {
        ...
        expect(mockRegistry.store).toHaveBeenCalledWith('meta', 'final_mod_order', finalOrder);
        ...
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenNthCalledWith(1, coreModId, mockCoreManifest, 'items', 'items', 'items');
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenNthCalledWith(2, fooModId, mockFooManifest, 'items', 'items', 'items');
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenNthCalledWith(3, barModId, mockBarManifest, 'items', 'items', 'items');
        ...
        expect(mockRegistry.store).toHaveBeenCalledWith('entities', `${coreModId}:${baseItemId}`, corePotionStoredData);
        ...
        expect(mockRegistry.store).toHaveBeenCalledWith('entities', `${fooModId}:${baseItemId}`, fooPotionStoredData);
        ...
        expect(mockRegistry.store).toHaveBeenCalledWith('entities', `${barModId}:${baseItemId}`, barPotionStoredData);

        // 4. Verify Final Registry State - check that get returns the correct data for each prefixed ID
        // Core Potion
        const corePotionFinal = mockRegistry.get('entities', `${coreModId}:${baseItemId}`);
        ...
        // Foo Potion
        const fooPotionFinal = mockRegistry.get('entities', `${fooModId}:${baseItemId}`);
        expect(fooPotionFinal).toBeDefined();
        expect(fooPotionFinal).toEqual(fooPotionStoredData);

        // Bar Potion
        const barPotionFinal = mockRegistry.get('entities', `${barModId}:${baseItemId}`);
        ...
        expect(mockValidatedEventDispatcher.dispatchValidated).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                worldName,
                modsLoaded: finalOrder,
                counts: {
                    items: { count: 3, overrides: 0, errors: 0 } // Reflects 1 item loaded per mod by the mock
                }
            }),
            expect.anything()
        );
        expect(mockValidatedEventDispatcher.dispatchValidated).not.toHaveBeenCalledWith(
            expect.stringContaining('failed'), expect.anything(), expect.anything()
        );
    });
});