...
import * as ModLoadOrderResolver from "../../../core/modding/modLoadOrderResolver";
...
describe('WorldLoader Integration Test Suite (TEST-LOADER-7.1)', () => {
    ...
    it('should successfully load world with only the "core" mod', async () => {
        ...
        expect(mockedValidateModEngineVersions).toHaveBeenCalledWith(expectedValidationMap, mockLogger);

        // 9. Verify resolveOrder was called.
        expect(mockedResolveOrder).toHaveBeenCalledTimes(1);
        expect(mockedResolveOrder).toHaveBeenCalledWith([coreModId], expectedValidationMap, mockLogger);
        ...
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenCalledWith(coreModId, mockCoreManifest, 'characters', 'characters', 'characters');
        ...
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(expect.anything(), expect.anything(), 'blockers', expect.anything(), expect.anything());
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(expect.anything(), expect.anything(), 'connections', expect.anything(), expect.anything());
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(expect.anything(), expect.anything(), 'items', expect.anything(), expect.anything());
        expect(mockEntityLoader.loadItemsForMod).not.toHaveBeenCalledWith(expect.anything(), expect.anything(), 'locations', expect.anything(), expect.anything());
        ...
        expect(mockValidatedEventDispatcher.dispatchValidated).not.toHaveBeenCalledWith(
            'initialization:world_loader:mod_load_failed', expect.anything(), expect.anything()
        );
        expect(mockValidatedEventDispatcher.dispatchValidated).not.toHaveBeenCalledWith(
            'initialization:world_loader:content_load_failed', expect.anything(), expect.anything()
        );
    });
});