// src/tests/core/loaders/baseManifestItemLoader.validatePrimarySchema.test.js
...
describe('BaseManifestItemLoader _validatePrimarySchema', () => {
    ...
    beforeEach(() => {
        ...
        loader._validatePrimarySchema = BaseManifestItemLoader.prototype._validatePrimarySchema;
    ...
    it('Success Case: should call validator, log debug, and return success result', () => {
        ...
        // Act
        const result = loader._validatePrimarySchema(data, filename, modId, resolvedPath);
    ...
    it('Unloaded Schema Case: should log warning, skip validation, and return success', () => {
        ...
        expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Validating'));
        expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('successful'));
        expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Skipping')); // The "Skipping" message for *missing* schemaId should not appear here
    });
    ...
    it('Validation Failure Case: should log error with details and throw', () => {
        ...
        const validationErrors = [
            { instancePath: '/key', message: 'should be integer', params: { type: 'integer' } },
            { instancePath: '/other', message: 'is required', params: { missingProperty: 'other' } }
        ];
        ...
        // Act & Assert
        expect(() => loader._validatePrimarySchema(data, filename, modId, resolvedPath))
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ // Check context object passed to logger
                modId,
                filename,
                resolvedPath,
                schemaId,
        ...
        expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('successful'));
        expect(mockLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Skipping'));
    });
