...
const mockEntityManager = () => {
    const instance = {
        ...
        getEntitiesWithComponents: jest.fn((componentId) => {
            return Array.from(instance.activeEntities.values())
                .filter(entity => entity.hasComponent(componentId));
        }),
...
const mockValidatedEventDispatcher = () => {
    ...
    const dispatcher = {
        ...
        subscribe: jest.fn((eventName, callback) => {
            dispatcher._subscriptions[eventName] = dispatcher._subscriptions[eventName] || [];
...
class MockTurnHandler {
    constructor(actor) {
        ...
        this.startTurn = jest.fn(async (currentActor) => {
            ...
            throw new Error(`Simulated startTurn failure for ${currentActor?.id || 'unknown actor'}`);
        });
...
describe('TurnManager - Error Handling', () => {
    ...
    afterEach(async () => { // Make afterEach async if TurnManager.stop() is consistently async
    ...
    test('should stop advancing if handlerResolver fails', async () => {
        ...
        turnOrderService.getNextEntity.mockReset().mockResolvedValueOnce(mockActor1);
        const resolveError = new Error('Simulated Handler Resolution Failure');
        turnHandlerResolver.resolveHandler.mockReset().mockRejectedValue(resolveError);
    ...
    test('should stop advancing and log error if getNextEntity returns null unexpectedly', async () => {
        ...
        expect(dispatcher.dispatchValidated).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                ...
                details: 'Turn order inconsistency: getNextEntity() returned null/undefined when queue was not empty.'
            })
        );
        // stop() calls clearCurrentRound
        expect(turnOrderService.clearCurrentRound).toHaveBeenCalledTimes(1);
