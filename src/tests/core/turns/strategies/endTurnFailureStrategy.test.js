...
import EndTurnFailureStrategy from '../../../../core/turns/strategies/endTurnFailureStrategy.js'; // Adjust path as needed
...
describe('EndTurnFailureStrategy', () => {
    ...
    describe('execute', () => {
        it('should throw an error if the directive is not END_TURN_FAILURE', async () => {
            ...
            await expect(strategy.execute(mockTurnContext, directive, cmdProcResult))
                .rejects
                .toThrow('EndTurnFailureStrategy: Wrong directive (END_TURN_SUCCESS). Expected END_TURN_FAILURE.');
            expect(mockLogger.error).toHaveBeenCalledWith('EndTurnFailureStrategy: Wrong directive (END_TURN_SUCCESS). Expected END_TURN_FAILURE.');
        ...
        it('should call turnContext.endTurn with an error if contextActor is null', async () => {
            ...
            const expectedErrorMsg = 'EndTurnFailureStrategy: No actor found in ITurnContext for END_TURN_FAILURE. Critical issue.';

            await strategy.execute(mockTurnContext, directive, cmdProcResult);

            expect(mockLogger.error).toHaveBeenCalledWith(expectedErrorMsg + " Ending turn with a generic error indicating missing actor.");
            ...
            expect(mockTurnContext.endTurn.mock.calls[0][0].message).toBe(expectedErrorMsg);
        });

        it('should use cmdProcResult.error if it is an Error instance', async () => {
            const directive = TurnDirective.END_TURN_FAILURE;
            const specificError = new Error("Command processing failed specifically!");
            const cmdProcResult = {success: false, error: specificError};

            await strategy.execute(mockTurnContext, directive, cmdProcResult);

            expect(mockLogger.info).toHaveBeenCalledWith(`EndTurnFailureStrategy: Executing END_TURN_FAILURE for actor ${mockActor.id}. Error: ${specificError.message}`);
        ...
        it('should wrap cmdProcResult.error in a new Error if it is a string', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(`EndTurnFailureStrategy: Executing END_TURN_FAILURE for actor ${mockActor.id}. Error: ${errorString}`);
            expect(mockTurnContext.endTurn).toHaveBeenCalledTimes(1);
            expect(mockTurnContext.endTurn).toHaveBeenCalledWith(expect.any(Error));
            expect(mockTurnContext.endTurn.mock.calls[0][0].message).toBe(errorString);
        });
        ...
        it('should wrap cmdProcResult.error in a new Error if it is a non-Error, non-string, non-null value', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(`EndTurnFailureStrategy: Executing END_TURN_FAILURE for actor ${mockActor.id}. Error: ${expectedWrappedErrorMessage}`);
            expect(mockTurnContext.endTurn).toHaveBeenCalledTimes(1);
            expect(mockTurnContext.endTurn).toHaveBeenCalledWith(expect.any(Error));
            expect(mockTurnContext.endTurn.mock.calls[0][0].message).toBe(expectedWrappedErrorMessage);
        });
        ...
        it('should create a default error if cmdProcResult.error is null', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(`EndTurnFailureStrategy: Executing END_TURN_FAILURE for actor ${mockActor.id}. Error: ${expectedDefaultErrorMessage}`);
            expect(mockTurnContext.endTurn).toHaveBeenCalledTimes(1);
            expect(mockTurnContext.endTurn).toHaveBeenCalledWith(expect.any(Error));
            expect(mockTurnContext.endTurn.mock.calls[0][0].message).toBe(expectedDefaultErrorMessage);
        });
