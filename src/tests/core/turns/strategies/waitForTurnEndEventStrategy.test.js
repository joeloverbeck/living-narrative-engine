...
import WaitForTurnEndEventStrategy from '../../../../core/turns/strategies/waitForTurnEndEventStrategy.js';
...
import {TURN_ENDED_ID} from '../../../../core/constants/eventIds.js'; // Mocked or actual, depending on test needs
...
class MockTurnContext {
    constructor(actor, logger) {
        ...
        this.requestTransition = jest.fn(async (StateClass, constructorArgs = []) => {
    ...
    resetAllMocks() {
        ...
        if (this._logger && typeof this._logger.info === 'function' && this._logger.info.mockClear) {
...
describe('WaitForTurnEndEventStrategy', () => {
    ...
    test('should call turnContext.endTurn with an error if turnContext.getActor() returns null', async () => {
        ...
        const directive = TurnDirective.WAIT_FOR_EVENT;
        const cmdProcResult = {};
        const expectedErrorMsg = 'WaitForTurnEndEventStrategy: No actor found in ITurnContext. Cannot transition to AwaitingExternalTurnEndState without an actor.';
    ...
    test('should call turnContext.endTurn if requestTransition fails', async () => {
        ...
        mockTurnContext.requestTransition.mockRejectedValueOnce(new Error(transitionErrorMessage));
        ...
        expect(mockTurnContext.requestTransition).toHaveBeenCalledWith(AwaitingExternalTurnEndState);
        expect(mockLogger.error).toHaveBeenCalledWith(expectedOverallErrorMsg, expect.any(Error));
        expect(mockTurnContext.endTurn).toHaveBeenCalledTimes(1);
        expect(mockTurnContext.endTurn).toHaveBeenCalledWith(expect.any(Error));
        const errorArg = mockTurnContext.endTurn.mock.calls[0][0];
