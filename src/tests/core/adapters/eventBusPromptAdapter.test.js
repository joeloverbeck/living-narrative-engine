...
import { EventBusPromptAdapter } from '../../../core/turns/adapters/eventBusPromptAdapter.js';
...
describe('EventBusPromptAdapter', () => {
    ...
    it('should throw an error if no valid dispatcher is provided', () => {
        expect(() => new EventBusPromptAdapter({})).toThrow(/Requires a valid ISafeEventDispatcher \(preferred\) or IValidatedEventDispatcher/);
        expect(() => new EventBusPromptAdapter({ safeEventDispatcher: {} })).toThrow();
        expect(() => new EventBusPromptAdapter({ validatedEventDispatcher: {} })).toThrow();
        expect(() => new EventBusPromptAdapter({ safeEventDispatcher: {}, validatedEventDispatcher: {} })).toThrow();
    });

    it('should prefer ISafeEventDispatcher if provided and valid', () => {
        const adapter = new EventBusPromptAdapter({ safeEventDispatcher: mockSafeDispatcher, validatedEventDispatcher: mockVed });
    ...
    it('should use IValidatedEventDispatcher if ISafeEventDispatcher is not provided or invalid', () => {
        ...
        const adapter = new EventBusPromptAdapter({ validatedEventDispatcher: mockVed });
        expect(adapter).toBeDefined();
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('falling back to IValidatedEventDispatcher'));

        const adapter2 = new EventBusPromptAdapter({ safeEventDispatcher: {}, validatedEventDispatcher: mockVed });
    ...
    it('should throw an error during prompt if entityId is invalid', async () => {
        const adapter = new EventBusPromptAdapter({ safeEventDispatcher: mockSafeDispatcher });
        await expect(adapter.prompt(null, [])).rejects.toThrow('entityId must be a non-empty string');
        await expect(adapter.prompt('', [])).rejects.toThrow('entityId must be a non-empty string');
        await expect(adapter.prompt(123, [])).rejects.toThrow('entityId must be a non-empty string');
    });

    it('should throw an error during prompt if availableActions is not an array', async () => {
        const adapter = new EventBusPromptAdapter({ safeEventDispatcher: mockSafeDispatcher });
        await expect(adapter.prompt('player1', null)).rejects.toThrow('availableActions must be an array');
        await expect(adapter.prompt('player1', {})).rejects.toThrow('availableActions must be an array');
        await expect(adapter.prompt('player1', 'not-an-array')).rejects.toThrow('availableActions must be an array');
    });
    ...
    it('should call dispatchSafely with correct arguments when using ISafeEventDispatcher', async () => {
        ...
        const actions = [{ id: 'act1', command: 'do act1' }, { id: 'act2', command: 'do act2' }];
    ...
    it('should reject if dispatchValidated throws an error', async () => {
        ...
        await expect(adapter.prompt(entityId, actions)).rejects.toThrow(dispatchError);
        expect(mockVed.dispatchValidated).toHaveBeenCalledTimes(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Error dispatching \'core:player_turn_prompt\' via VED'), expect.any(Error));
