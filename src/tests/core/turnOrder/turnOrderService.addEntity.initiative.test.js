...
describe('TurnOrderService', () => {
    ...
    beforeEach(() => { // No longer async
        ...
        initialInitiativeData = new Map([['a', 10], ['b', 5]]);
    ...
    describe("addEntity ('initiative' strategy active)", () => {
        ...
        it("Test Case 11.10.1: should add an entity with a valid priority", () => {
            ...
            expect(mockInitiativeQueueInstance.add).toHaveBeenCalledWith(entityToAdd, priority);

            // 2. Logging
            expect(mockLogger.debug).toHaveBeenCalledTimes(1);
            expect(mockLogger.debug).toHaveBeenCalledWith(`TurnOrderService: Adding entity "${entityToAdd.id}" with initiative ${priority} to the current round.`);
        ...
        it("Test Case 11.10.2: should add an entity with default priority 0 if priority is missing", () => {
            ...
            // 1. Queue Interaction
            expect(mockInitiativeQueueInstance.add).toHaveBeenCalledTimes(1); // Should pass now
            expect(mockInitiativeQueueInstance.add).toHaveBeenCalledWith(entityToAdd, expectedPriority);

            // 2. Logging
            expect(mockLogger.debug).toHaveBeenCalledTimes(1);
            expect(mockLogger.debug).toHaveBeenCalledWith(`TurnOrderService: Adding entity "${entityToAdd.id}" with initiative ${expectedPriority} to the current round.`);
            expect(mockLogger.info).toHaveBeenCalledTimes(1);
            expect(mockLogger.info).toHaveBeenCalledWith(`TurnOrderService: Entity "${entityToAdd.id}" successfully added to the turn order.`);
        ...
        it("Test Case 11.10.3: should add an entity with default priority 0 and log warning if priority is null", () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(`TurnOrderService.addEntity (initiative): Invalid initiative value "${invalidPriority}" provided for entity "${entityToAdd.id}". Defaulting to ${expectedPriority}.`);
            expect(mockLogger.debug).toHaveBeenCalledTimes(1);
            expect(mockLogger.debug).toHaveBeenCalledWith(`TurnOrderService: Adding entity "${entityToAdd.id}" with initiative ${expectedPriority} to the current round.`);
            expect(mockLogger.info).toHaveBeenCalledTimes(1);
            expect(mockLogger.info).toHaveBeenCalledWith(`TurnOrderService: Entity "${entityToAdd.id}" successfully added to the turn order.`);
