    ...
    describe('executeUse', () => {
        ...
        it('should handle successful item use on a connection target (DO+P+IO)', () => {
            ...
            const mockConnectionObject = {
                id: targetConnectionId, // Add the id property
                connectionId: targetConnectionId, // Keep connectionId if other code uses it, or remove if redundant
            ...
            expect(mockEventBus.dispatch).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    explicitTargetEntityId: null,
                    explicitTargetConnectionEntityId: targetConnectionId, // <-- Change this line
                    // userEntityId: playerId, // You might want to add these back for completeness
                    // itemInstanceId: itemId, // if they were removed, though objectContaining
                    // itemDefinitionId: itemDefId, // only requires the specified keys to match.
                })
            );
        ...
        it('should handle successful item use with no explicit target', () => {
            ...
            expect(mockEventBus.dispatch).toHaveBeenCalledWith('event:item_use_attempted', expect.objectContaining({
                ...
                explicitTargetConnectionEntityId: null, // Correct
            }));
            expect(result.success).toBe(true);
            expect(mockDispatch).not.toHaveBeenCalledWith('ui:message_display', expect.objectContaining({type: 'error'}));
        });
