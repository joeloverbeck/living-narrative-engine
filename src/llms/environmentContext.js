...
export class EnvironmentContext {
     ...
     */
    constructor({logger, executionEnvironment, projectRootPath = null, proxyServerUrl = null}) {
        // Validate logger
        if (!logger || typeof logger.info !== 'function' || typeof logger.warn !== 'function' || typeof logger.error !== 'function' || typeof logger.debug !== 'function') {
            const errorMsg = 'EnvironmentContext: Constructor requires a valid logger instance with info, warn, error, and debug methods.';
        ...
        if (typeof executionEnvironment === 'string' && VALID_EXECUTION_ENVIRONMENTS.includes(executionEnvironment.toLowerCase())) {
        ...
        if (this.#executionEnvironment === 'server') {
            if (!projectRootPath || typeof projectRootPath !== 'string' || projectRootPath.trim() === '') {
                const errorMsg = "EnvironmentContext: Constructor requires 'projectRootPath' (non-empty string) when executionEnvironment is 'server'.";
                this.#logger.error(errorMsg);
                throw new Error(errorMsg);
            }
            this.#projectRootPath = projectRootPath.trim();
            this.#logger.debug(`EnvironmentContext: Server-side projectRootPath set to: '${this.#projectRootPath}'`);
        } else if (projectRootPath) {
            this.#logger.warn(`EnvironmentContext: 'projectRootPath' ("${projectRootPath}") was provided, but executionEnvironment is '${this.#executionEnvironment}', not 'server'. It will be ignored.`);
        ...
        if (this.#executionEnvironment === 'client') {
            if (proxyServerUrl && typeof proxyServerUrl === 'string' && proxyServerUrl.trim() !== '') {
                try {
                    ...
                    this.#logger.info(`EnvironmentContext: Client-side proxy URL configured to: '${this.#proxyServerUrl}'.`);
                } catch (e) {
                    this.#logger.warn(`EnvironmentContext: Provided proxyServerUrl '${proxyServerUrl}' for client environment is not a valid URL. Falling back to default: '${DEFAULT_PROXY_SERVER_URL}'. Error: ${e.message}`);
                    this.#proxyServerUrl = DEFAULT_PROXY_SERVER_URL;
                }
            } else {
                this.#proxyServerUrl = DEFAULT_PROXY_SERVER_URL;
                if (proxyServerUrl === null || proxyServerUrl === undefined) {
                    this.#logger.info(`EnvironmentContext: Client-side proxyServerUrl not provided. Using default: '${this.#proxyServerUrl}'.`);
                } else {
                    this.#logger.warn(`EnvironmentContext: Client-side proxyServerUrl provided but was empty or invalid ('${proxyServerUrl}'). Using default: '${this.#proxyServerUrl}'.`);
                }
            }
        } else {
            ...
            if (proxyServerUrl && typeof proxyServerUrl === 'string' && proxyServerUrl.trim() !== '') {
                try {
                    ...
                    this.#proxyServerUrl = proxyServerUrl.trim();
                    this.#logger.debug(`EnvironmentContext: proxyServerUrl ('${this.#proxyServerUrl}') was provided for non-client environment ('${this.#executionEnvironment}'). It might not be used.`);
                } catch (e) {
                    this.#logger.debug(`EnvironmentContext: Provided proxyServerUrl '${proxyServerUrl}' for non-client environment is not a valid URL. Setting to default ('${DEFAULT_PROXY_SERVER_URL}'), but it might not be used. Error: ${e.message}`);
                    this.#proxyServerUrl = DEFAULT_PROXY_SERVER_URL;
                }
            } else {
                this.#proxyServerUrl = DEFAULT_PROXY_SERVER_URL; // Set a default even if not client, though it might not be used.
                if (proxyServerUrl) { // if it was provided but empty/invalid type
                    this.#logger.debug(`EnvironmentContext: proxyServerUrl was provided but invalid for non-client environment ('${this.#executionEnvironment}'). Defaulting to '${this.#proxyServerUrl}', though it might not be used.`);
                } else {
                    this.#logger.debug(`EnvironmentContext: proxyServerUrl not provided for non-client environment ('${this.#executionEnvironment}'). Defaulting to '${this.#proxyServerUrl}', though it might not be used.`);
                }
            }
        }
        this.#logger.info(`EnvironmentContext: Instance created. Execution environment: ${this.#executionEnvironment}.`);
    }
