...
export class RetryHttpClient extends IHttpClient {
   ...
   * @param {number} [params.defaultBaseDelayMs] - Optional. Initial delay for retries in ms.
   * @param {number} [params.defaultMaxDelayMs] - Optional. Max delay between retries in ms.
   */
  constructor({
    logger,
    ...
    if (typeof defaultMaxRetries !== 'number' || defaultMaxRetries < 0) {
      this.#logger.warn(
        `RetryHttpClient: Invalid defaultMaxRetries (${defaultMaxRetries}). Using default: ${DEFAULT_MAX_RETRIES}.`
      );
    ...
    if (typeof defaultBaseDelayMs !== 'number' || defaultBaseDelayMs < 0) {
      this.#logger.warn(
        `RetryHttpClient: Invalid defaultBaseDelayMs (${defaultBaseDelayMs}). Using default: ${DEFAULT_BASE_DELAY_MS}.`
      );
    ...
    if (
      ...
      const newMaxDelay = Math.max(
        this.#defaultBaseDelayMs,
        DEFAULT_MAX_DELAY_MS
      );
      this.#logger.warn(
        `RetryHttpClient: Invalid defaultMaxDelayMs (${defaultMaxDelayMs}). Adjusted to ${newMaxDelay} to be >= baseDelayMs and system default.`
      );
  ...
  async #tryReadResponseBody(response) {
    ...
    try {
      ...
      this.#logger.debug(
        `RetryHttpClient.#tryReadResponseBody: Failed to parse response as JSON. Attempting to read as text. Error: ${jsonError.message}`
      );
      try {
        ...
        this.#logger.warn(
          `RetryHttpClient.#tryReadResponseBody: Failed to read response body as text after JSON parse failure. Error: ${textError.message}`
        );
  ...
  async request(url, options) {
    ...
    while (currentAttempt <= this.#defaultMaxRetries) {
      ...
      try {
        this.#logger.debug(
          `RetryHttpClient.request: Attempt ${currentAttempt} of ${this.#defaultMaxRetries + 1} to ${url}.`
        );
        const response = await fetch(url, options);

        if (response.ok) {
          this.#logger.info(
            `RetryHttpClient.request: Successful response (status ${response.status}) from ${url} on attempt ${currentAttempt}.`
          );
          try {
            const responseBody = await response.json();
            this.#logger.debug(
              `RetryHttpClient.request: Successfully parsed JSON response from ${url}.`
            );
            return responseBody;
          } catch (jsonError) {
            this.#logger.error(
              `RetryHttpClient.request: Malformed JSON response from ${url} despite successful status ${response.status}.`,
              {
        ...
        if (
          ...
          currentAttempt <= this.#defaultMaxRetries
        ) {
          const delayMs = this.#calculateDelay(currentAttempt); // Pass current retry number (1 for first retry)
          this.#logger.warn(
            `RetryHttpClient.request: Attempt ${currentAttempt} for ${url} failed with retryable status ${status}. Retrying in ${delayMs}ms...`,
            {
              responseBodyPreview:
          ...
          const reason = RETRYABLE_STATUS_CODES.includes(status)
            ? 'max retries reached for retryable status'
            : 'non-retryable status';
          this.#logger.error(
            `RetryHttpClient.request: Failed for ${url} on attempt ${currentAttempt} with status ${status} (${reason}). No more retries.`,
            { responseBody }
          );
        ...
        this.#logger.warn(
          `RetryHttpClient.request: Network/fetch error during attempt ${currentAttempt} for ${url}. Error: ${error.message}`,
          { originalError: error }
        );
        lastError = new HttpClientError(
          ...
          } // Network errors are generally considered retryable
        );

        if (currentAttempt <= this.#defaultMaxRetries) {
          const delayMs = this.#calculateDelay(currentAttempt);
          this.#logger.warn(
            `RetryHttpClient.request: Attempt ${currentAttempt} for ${url} encountered network error. Retrying in ${delayMs}ms...`
          );
          await new Promise((resolve) => setTimeout(resolve, delayMs));
          continue; // Next attempt
        } else {
          this.#logger.error(
            `RetryHttpClient.request: Failed for ${url} due to persistent network error after ${currentAttempt} attempts. Error: ${error.message}`
          );
    ...
    this.#logger.error(
      `RetryHttpClient.request: Exited retry loop for ${url} without success after ${currentAttempt} attempts. Throwing last known error or a generic one.`
    );
    if (lastError) {
      ...
      if (
        ...
        lastError.isRetryableFailure === undefined
      ) {
