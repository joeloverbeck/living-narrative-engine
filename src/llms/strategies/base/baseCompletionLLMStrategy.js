...
export class BaseCompletionLLMStrategy extends BaseLLMStrategy {
  ...
  _constructPromptPayload(gameSummary, promptFrame, llmConfig) {
    this.logger.debug(
      `BaseCompletionLLMStrategy._constructPromptPayload for apiType '${llmConfig.apiType}'. gameSummary (finalPromptString) length: ${gameSummary.length}. llmConfig.promptFrame will be ignored.`
    );
