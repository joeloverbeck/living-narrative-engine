...
export class BaseOpenRouterStrategy extends BaseChatLLMStrategy {
    ...
    async execute(params) {
        ...
        const baseMessagesPayload = this._constructPromptPayload(gameSummary, llmConfig);
        ...
        this.logger.debug(`${this.constructor.name} (${llmId}): Constructed base prompt payload:`, {
            ...
            messagesPreview: baseMessagesPayload.messages.map(m => ({
                ...
                contentPreview: typeof m.content === 'string'
                    ? m.content.substring(0, 70) + (m.content.length > 70 ? '...' : '')
        ...
        try {
            ...
            const isHttpClientError = error.name === 'HttpClientError' || (error.hasOwnProperty('status') && error.hasOwnProperty('response') && error.hasOwnProperty('url'));
