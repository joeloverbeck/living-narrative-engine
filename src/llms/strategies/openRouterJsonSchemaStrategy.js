 ...
 * conforming to a predefined schema (OPENROUTER_GAME_AI_ACTION_SPEECH_SCHEMA) and
 * extracts the JSON content from the response, primarily from `message.content`.
 */
export class OpenRouterJsonSchemaStrategy extends BaseOpenRouterStrategy {
    ...
    _buildProviderRequestPayloadAdditions(baseMessagesPayload, llmConfig) {
        ...
        const schemaReference = OPENROUTER_GAME_AI_ACTION_SPEECH_SCHEMA.schema || OPENROUTER_GAME_AI_ACTION_SPEECH_SCHEMA;

        const responseFormat = {
            type: "json_schema",
            json_schema: schemaReference
        };
        // this.logger is guaranteed to be valid here.
        this.logger.debug(`${this.constructor.name} (${llmId}): Using response_format with schema: ${OPENROUTER_GAME_AI_ACTION_SPEECH_SCHEMA.name || 'UnnamedSchema'}`, {llmId});
    ...
    async _extractJsonOutput(responseData, llmConfig, providerRequestPayload) {
        ...
        if (extractedJsonString === null && message.tool_calls && Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
            ...
            if (toolCall?.type === "function" &&
                ...
                this.logger.warn(`${this.constructor.name} (${llmId}): tool_calls structure for fallback did not match expected schema or arguments were empty. Expected function name '${expectedToolName}'.`, {
                    llmId,
                    expectedToolName,
                    toolCallDetails: {
                        type: toolCall?.type,
