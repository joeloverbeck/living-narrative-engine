...
export class OpenRouterJsonSchemaStrategy extends BaseOpenRouterStrategy {
    ...
    _buildProviderRequestPayloadAdditions(baseMessagesPayload, llmConfig) {
        ...
        if (!configuredJsonSchema || typeof configuredJsonSchema !== 'object' || Object.keys(configuredJsonSchema).length === 0) {
            ...
            this.logger.warn(`${this.constructor.name} (${llmId}): 'jsonSchema' in llmConfig.jsonOutputStrategy is missing, not an object, or is an empty object. Ensure the configuration provides a valid JSON schema.`, {
                llmId,
        ...
        this.logger.debug(`${this.constructor.name} (${llmId}): Using response_format with JSON schema from llmConfig.jsonOutputStrategy.jsonSchema.`, {
            llmId,
            schemaKeys: configuredJsonSchema ? Object.keys(configuredJsonSchema) : 'N/A'
        });
    ...
    async _extractJsonOutput(responseData, llmConfig, providerRequestPayload) {
        ...
        if (message.content && typeof message.content === 'string' && message.content.trim() !== '') {
            ...
            this.logger.info(`${this.constructor.name} (${llmId}): Extracted JSON string from message.content.`, {llmId});
            ...
            this.logger.info(`${this.constructor.name} (${llmId}): Extracted JSON object from message.content and stringified it.`, {llmId});
        } else {
            if (message.content === '' || (typeof message.content === 'string' && message.content.trim() === '')) {
                this.logger.warn(`${this.constructor.name} (${llmId}): message.content was an empty string. Will check tool_calls fallback.`, {llmId});
            } else if (message.hasOwnProperty('content')) {
                this.logger.warn(`${this.constructor.name} (${llmId}): message.content was present but not a non-empty string or object (type: ${typeof message.content}, value: ${message.content}). Will check tool_calls fallback.`, {
                    llmId,
                    contentType: typeof message.content,
                    contentValue: message.content
                });
            } else {
                this.logger.info(`${this.constructor.name} (${llmId}): message.content is missing. Will check tool_calls fallback.`, {llmId});
            }
        }
        ...
        if (extractedJsonString === null && message.tool_calls && Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
            this.logger.info(`${this.constructor.name} (${llmId}): message.content not usable, attempting tool_calls fallback.`, {llmId});
            ...
            const expectedToolNameForFallback = OPENROUTER_GAME_AI_ACTION_SPEECH_SCHEMA.name;
            ...
            if (toolCall?.type === "function" &&
                ...
                this.logger.info(`${this.constructor.name} (${llmId}): Extracted JSON string from tool_calls fallback (function: ${toolCall.function.name}).`, {
                    llmId,
                    functionName: toolCall.function.name
                });
            } else {
                this.logger.warn(`${this.constructor.name} (${llmId}): tool_calls structure for fallback did not match expected schema or arguments were empty. Expected function name '${expectedToolNameForFallback}'.`, {
                    llmId,
        ...
        if (extractedJsonString === null || extractedJsonString.trim() === '') {
            ...
            this.logger.error(errorMsg, {llmId, responseDataPreview: JSON.stringify(responseData)?.substring(0, 500)});
            throw new LLMStrategyError(errorMsg, llmId, null, {responsePreview: JSON.stringify(responseData)?.substring(0, 500)});
        }
