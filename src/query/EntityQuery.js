...
export default class EntityQuery {
  ...
  matches(entity) {
    ...
    if (this.without.length > 0 && this.without.some(componentTypeId => entity.hasComponent(componentTypeId))) {
      return false;
    }

    // 2. 'withAll' check: If the entity fails to have even one component from the 'withAll' list, skip it.
    if (this.withAll.length > 0 && !this.withAll.every(componentTypeId => entity.hasComponent(componentTypeId))) {
      return false;
    }

    // 3. 'withAny' check: If a 'withAny' list is provided, the entity must have at least one component from it.
    if (this.withAny.length > 0 && !this.withAny.some(componentTypeId => entity.hasComponent(componentTypeId))) {
      return false;
    }
