...
export function configureContainer(
    ...
    const logger = /** @type {ConsoleLogger} */ (container.resolve(tokens.ILogger));

    // Log that we're attempting to load the remote configuration.
    // This message will use the `initialLogLevel`.
    logger.debug(`[ContainerConfig] Initial logger registered with level: ${initialLogLevel}. Attempting to load remote logger configuration...`);
    ...
    (async () => {
        try {
            ...
            logger.debug('[ContainerConfig] Starting asynchronous load of logger configuration...');

            const loggerConfigResult = await loggerConfigLoader.loadConfig(); // loadConfig() is defined in LoggerConfigLoader

            if (loggerConfigResult && !loggerConfigResult.error && loggerConfigResult.logLevel !== undefined) {
                if (typeof loggerConfigResult.logLevel === 'string') {
                    // Log before changing level (will use current level)
                    logger.debug(`[ContainerConfig] Logger configuration loaded successfully. Requested level: '${loggerConfigResult.logLevel}'. Applying...`);
                    ...
                    logger.warn(`[ContainerConfig] Logger configuration loaded, but 'logLevel' is not a string: ${loggerConfigResult.logLevel}. Retaining current log level.`);
                }
            } else if (loggerConfigResult && loggerConfigResult.error) {
                logger.warn(`[ContainerConfig] Failed to load logger configuration from '${loggerConfigResult.path || 'default path'}'. Error: ${loggerConfigResult.message}. Stage: ${loggerConfigResult.stage || 'N/A'}. Retaining current log level.`);
            } else {
                // This case means the file was loaded but either empty, or logLevel property was missing.
                logger.info('[ContainerConfig] Logger configuration file loaded but no specific logLevel found or file was empty. Retaining current log level.');
            }
        } catch (error) {
            // Catch any unexpected errors during the async loading/processing.
            logger.error('[ContainerConfig] CRITICAL ERROR during asynchronous logger configuration loading:', {
    ...
    logger.info('[ContainerConfig] Starting synchronous bundle registration while logger config continues loading in background (if not already done).');
    ...
    try {
        ...
        logger.info(`[ContainerConfig] Data source 'GameDataRepository' registered in SystemDataRegistry.`);

        const worldContextInstance = /** @type {WorldContext} */ (
            container.resolve(tokens.IWorldContext)
        );
        const worldContextKey = 'WorldContext';
        logger.debug(`[ContainerConfig] Registering data source '${worldContextKey}' in SystemDataRegistry...`);
        systemDataRegistry.registerSource(worldContextKey, worldContextInstance);
        logger.info(`[ContainerConfig] Data source '${worldContextKey}' successfully registered in SystemDataRegistry.`);
        ...
        logger.debug(`[ContainerConfig] Registering data source '${perceptionUpdateServiceKey}' in SystemDataRegistry...`);
        systemDataRegistry.registerSource(perceptionUpdateServiceKey, perceptionUpdateServiceInstance);
        logger.info(`[ContainerConfig] Data source '${perceptionUpdateServiceKey}' successfully registered in SystemDataRegistry.`);

    } catch (error) {
        logger.error('[ContainerConfig] CRITICAL ERROR during SystemDataRegistry population:', {
            message: error.message,
            stack: error.stack,
            errorObj: error
        });
    ...
    logger.info('[ContainerConfig] Configuration and registry population complete.');
}
