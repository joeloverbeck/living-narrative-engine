...
class ViolationReporter {
  ...
  _generateConsoleReport(data, options = {}) {
    ...
    lines.push(`❌ ${data.violations.length} cross-reference violations detected`);
    lines.push('');
    ...
    if (violationsBySeverity.size > 0) {
      ...
      ['critical', 'high', 'medium', 'low'].forEach(severity => {
        ...
        const colorFunc = colors ? this._getSeverityColor(severity) : (text) => text;
        
        lines.push(colorFunc(`${severityIcon} ${severity.toUpperCase()} (${severityViolations.length}):`));
        
        severityViolations.forEach(violation => {
          lines.push(`  ❌ ${violation.referencedMod}:${violation.referencedComponent}`);
          ...
          if (verbose && violation.impact) {
            lines.push(`     📊 Impact: loading=${violation.impact.loadingFailure}, runtime=${violation.impact.runtimeFailure}`);
          }
      ...
      for (const [referencedMod, violations] of violationsByMod) {
        ...
        violations.forEach(violation => {
          ...
          if (violation.file && violation.file !== 'multiple') {
            lines.push(`      📁 ${violation.file}${violation.line ? ':' + violation.line : ''}`);
          }
        });
  ...
  _generateEcosystemConsoleReport(results, options = {}) {
    ...
    if (hasSeverityData) {
      ...
      const severityTotals = this._calculateEcosystemSeverityTotals(modsWithViolations);
      ...
      ['critical', 'high', 'medium', 'low'].forEach(severity => {
        ...
        if (count > 0) {
          ...
          const colorFunc = colors ? this._getSeverityColor(severity) : (text) => text;
          lines.push(colorFunc(`  ${icon} ${severity.toUpperCase()}: ${count} violations`));
        }
      });
      lines.push('');
    }
      ...
      for (const [referencedMod, violations] of violationsByMod) {
        ...
        if (showSuggestions) {
          ...
          if (firstViolation.suggestedFixes?.length > 0) {
            const primaryFix = firstViolation.suggestedFixes.find(f => f.priority === 'primary');
            if (primaryFix) {
              lines.push(`    💡 ${primaryFix.description}`);
            }
  ...
  _generateEcosystemSummary(results) {
    ...
    const modsWithViolations = Array.from(results.values()).filter(r => r.hasViolations).length;
  ...
  _generateEcosystemHtmlContent(results) {
    const modsWithViolations = Array.from(results.entries()).filter(([, report]) => report.hasViolations);
    const html = [];
    ...
    const totalViolations = modsWithViolations.reduce((sum, [, report]) => sum + report.violations.length, 0);
    
    html.push('  <div class="error">');
    html.push(`    <h2>❌ ${totalViolations} violations found across ${modsWithViolations.length} mods</h2>`);
    html.push('  </div>');
    ...
    modsWithViolations.forEach(([modId, report]) => {
      ...
      report.violations.forEach(violation => {
        const severityClass = violation.severity ? `severity-${violation.severity}` : '';
        html.push(`    <div class="violation ${severityClass}">`);
        html.push(`      <strong>${violation.referencedMod}:${violation.referencedComponent}</strong>`);
        
        if (violation.file && violation.file !== 'multiple') {
          html.push(`      <br>📁 ${violation.file}${violation.line ? ':' + violation.line : ''}`);
        }
        ...
        html.push(`      <br>💡 ${violation.suggestedFix || 'Add dependency to manifest'}`);
        html.push('    </div>');
      });
  ...
  _generateEcosystemMarkdownContent(results) {
    ...
    lines.push(`## ❌ ${totalViolations} violations found across ${modsWithViolations.length} mods`);
    lines.push('');
    ...
    modsWithViolations.forEach(([modId, report]) => {
      ...
      report.violations.forEach(violation => {
        ...
        if (violation.severity) {
          lines.push(`  - ${this._getSeverityIcon(violation.severity)} Severity: ${violation.severity}`);
        }
        
        lines.push(`  - 💡 ${violation.suggestedFix || 'Add dependency to manifest'}`);
        lines.push('');
      });
    });
