...
class InputSanitizer {
  ...
  constructor({ config, logger = console }) {
    ...
    this.allowedExtensions = this.#config.allowedExtensions || ['.json', '.scope'];
    this.blockedPaths = this.#config.blockedPaths || ['node_modules', '.git', '.env', 'secrets'];
  ...
  sanitizeScopeDslContent(content, filePath = 'unknown') {
    ...
    const referenceCount = (content.match(/[a-zA-Z][a-zA-Z0-9_]*:[a-zA-Z][a-zA-Z0-9_-]*/g) || []).length;
  ...
  #checkForDangerousKeys(obj, filePath) {
    const dangerousKeys = this.#config.dangerousKeys || ['__proto__', 'constructor', 'prototype'];
  ...
  #getModBasePath(filePath) {
    ...
    if (modsIndex !== -1) {
      ...
      if (modName) {
        return filePath.substring(0, modsIndex + 'data/mods/'.length + modName.length);
      }
    }
