...
/**
 * @fileoverview Defines BaseModalRenderer, an abstract class for managing common modal dialog behaviors.
 */
...
export class BaseModalRenderer extends BoundDomRendererBase {
     ...
     */
    constructor({logger, documentContext, validatedEventDispatcher, elementsConfig, ...otherDeps}) {
        super({logger, documentContext, validatedEventDispatcher, elementsConfig, ...otherDeps});

        if (!this.elements.modalElement) {
            this.logger.error(`${this._logPrefix} 'modalElement' is not defined in 'elementsConfig' or not found in the DOM. Modal functionality will be severely impaired.`);
            // Consider throwing an error if critical elements like modalElement are missing.
        }
        if (!this.elements.closeButton && Object.keys(elementsConfig).includes('closeButton')) {
            this.logger.error(`${this._logPrefix} 'closeButton' was configured in 'elementsConfig' but not found in the DOM.`);
        } else if (!Object.keys(elementsConfig).includes('closeButton')) {
            this.logger.warn(`${this._logPrefix} 'closeButton' was not specified in 'elementsConfig'. Modal may not be closable via a dedicated button.`);
        }

        // Check for statusMessageElement (optional, but log if configured and not found)
        if (!this.elements.statusMessageElement && Object.keys(elementsConfig).includes('statusMessageElement')) {
            this.logger.warn(`${this._logPrefix} 'statusMessageElement' was configured in 'elementsConfig' but not found in the DOM. Local status messages will not be displayed.`);
        } else if (this.elements.statusMessageElement) {
            this.logger.debug(`${this._logPrefix} 'statusMessageElement' bound successfully. Local status messages are available.`);
        }
    ...
    _bindCoreModalEvents() {
        if (this.elements.closeButton) {
            this._addDomListener(this.elements.closeButton, 'click', this.hide.bind(this));
        } else {
            this.logger.debug(`${this._logPrefix} Close button element not available for binding events.`);
        }
        ...
        if (this.elements.modalElement) {
            ...
            this._addDomListener(this.elements.modalElement, 'click', (event) => {
                if (event.target === this.elements.modalElement) {
                    this.logger.debug(`${this._logPrefix} Backdrop clicked, hiding modal.`);
                    this.hide();
                }
            });
        }
        ...
        this._addDomListener(this.documentContext.document, 'keydown', (event) => {
            if (event.key === 'Escape' && this.isVisible) {
                this.logger.debug(`${this._logPrefix} Escape key pressed, hiding modal.`);
                this.hide();
            }
        });
    }
    ...
    show() {
        if (this.isVisible) {
            this.logger.warn(`${this._logPrefix} show() called when modal is already visible.`);
            return;
        }
        if (!this.elements.modalElement) {
            this.logger.error(`${this._logPrefix} Cannot show modal, 'modalElement' is not available.`);
            return;
        }

        this._previouslyFocusedElement = /** @type {HTMLElement} */ (this.documentContext.document.activeElement);
        ...
        requestAnimationFrame(() => {
            if (this.elements.modalElement) { // Check if still valid in async callback
        ...
        try {
            ...
            this.logger.error(`${this._logPrefix} Error in _onShow lifecycle hook.`, error);
        }
        ...
        if (elementToFocus && typeof elementToFocus.focus === 'function') {
            ...
            this.logger.debug(`${this._logPrefix} No specific initial focus element. Trying fallbacks.`);
            if (this.elements.modalElement.hasAttribute('tabindex') && typeof this.elements.modalElement.focus === 'function') {
                this.elements.modalElement.focus();
            } else if (this.elements.closeButton && typeof this.elements.closeButton.focus === 'function') {
                this.elements.closeButton.focus();
            } else {
                this.logger.warn(`${this._logPrefix} Could not find any element to focus in the modal.`);
            }
        }
    ...
    hide() {
        ...
        if (!this.elements.modalElement) {
            this.logger.error(`${this._logPrefix} Cannot hide modal, 'modalElement' is not available.`);
            this.isVisible = false;
            return;
        }
        ...
        try {
            ...
            this.logger.error(`${this._logPrefix} Error in _onHide lifecycle hook.`, error);
        }

        const returnFocusElement = this._getReturnFocusElement();
        if (returnFocusElement && typeof returnFocusElement.focus === 'function') {
            returnFocusElement.focus();
        } else if (this._previouslyFocusedElement && typeof this._previouslyFocusedElement.focus === 'function') {
            if (this.documentContext.document.body.contains(this._previouslyFocusedElement) && this._previouslyFocusedElement.offsetParent !== null) {
                this._previouslyFocusedElement.focus();
            } else {
                this.logger.debug(`${this._logPrefix} Previously focused element is no longer focusable.`);
            }
        } else {
            this.logger.debug(`${this._logPrefix} No specific element to return focus to.`);
        }
    ...
    _getInitialFocusElement() {
        if (this.elements.closeButton && typeof this.elements.closeButton.focus === 'function') {
            return /** @type {HTMLElement} */ (this.elements.closeButton);
        }
        if (this.elements.modalElement && this.elements.modalElement.hasAttribute('tabindex') && typeof this.elements.modalElement.focus === 'function') {
            return /** @type {HTMLElement} */ (this.elements.modalElement);
        }
        this.logger.debug(`${this._logPrefix} No default initial focus element identified. Override in subclass or ensure modal/closeButton is focusable.`);
        return null;
    }
     ...
     * @param {string} [type='info'] - The type of message (e.g., 'info', 'error', 'success', 'warning').
    ...
    _displayStatusMessage(message, type = 'info') {
        const statusEl = /** @type {HTMLElement} */ (this.elements.statusMessageElement);
        if (!statusEl) {
            this.logger.warn(`${this._logPrefix} '_displayStatusMessage' called, but 'statusMessageElement' is not available. Message: "${message}" (type: ${type})`);
            return;
        }
        ...
        this.logger.debug(`${this._logPrefix} Displayed status message: "${message}" (type: ${type})`);
    }
    ...
    _clearStatusMessage() {
        ...
        if (!statusEl) {
            ...
            this.logger.debug(`${this._logPrefix} '_clearStatusMessage' called, but 'statusMessageElement' is not available. No message to clear.`);
            return;
        }
    ...
    _setOperationInProgress(inProgress) {
        if (!this._operationInProgressAffectedElements || this._operationInProgressAffectedElements.length === 0) {
            this.logger.debug(`${this._logPrefix} No elements configured for '_setOperationInProgress'.`);
            return;
        }

        this._operationInProgressAffectedElements.forEach(elementKey => {
            const element = /** @type {HTMLButtonElement | HTMLInputElement} */ (this.elements[elementKey]);
            if (element && typeof element.disabled !== 'undefined') {
                element.disabled = inProgress;
            } else if (this.elements[elementKey]) {
                this.logger.warn(`${this._logPrefix} Element "${elementKey}" found for '_setOperationInProgress' but does not support 'disabled' property.`);
            } else {
                this.logger.debug(`${this._logPrefix} Element "${elementKey}" not found for _setOperationInProgress (may be optional for this modal).`);
            }
        });
        this.logger.debug(`${this._logPrefix} Operation in progress state set to ${inProgress}. Affected elements: ${this._operationInProgressAffectedElements.join(', ')}.`);
    }
