 ...
 * @extends {BaseListDisplayComponent<AvailableAction>}
 */
export class ActionButtonsRenderer extends BaseListDisplayComponent {
     ...
     * @param {string} [params.sendButtonSelector='#player-confirm-turn-button'] - CSS selector for the send button.
    ...
    constructor({
        ...
        const factoryErrorMessage = "[ActionButtonsRenderer] 'domElementFactory' dependency is missing or invalid (must have create and button methods).";
        if (!domElementFactory ||
            typeof domElementFactory.create !== 'function' ||
            typeof domElementFactory.button !== 'function') {
            (logger || console).error(factoryErrorMessage, { receivedFactory: domElementFactory });
            throw new Error(factoryErrorMessage);
        }

        if (!actionButtonsContainerSelector || typeof actionButtonsContainerSelector !== 'string' || actionButtonsContainerSelector.trim() === '') {
        ...
        const elementsConfig = {
            listContainerElement: { selector: actionButtonsContainerSelector, required: true },
            sendButtonElement: { selector: sendButtonSelector, required: false, expectedType: HTMLButtonElement },
            speechInputElement: { selector: speechInputSelector, required: false, expectedType: HTMLInputElement }
        };
        ...
        if (this.elements.sendButtonElement) {
            ...
            this._addDomListener(this.elements.sendButtonElement, 'click', this.#handleSendAction.bind(this));
            this.logger.debug(`${this._logPrefix} 'Confirm Action' button listener added via _addDomListener.`);
        } else {
            this.logger.warn(`${this._logPrefix} 'Confirm Action' button (selector: '${sendButtonSelector}') not found or not a button. Send functionality will be unavailable.`);
        }

        if (this.elements.speechInputElement) {
            this.logger.debug(`${this._logPrefix} Speech input element (selector: '${speechInputSelector}') cached.`);
        } else {
            this.logger.warn(`${this._logPrefix} Speech input element (selector: '${speechInputSelector}') not found or not an input. Speech input will be unavailable.`);
        }
        ...
        this.logger.debug(`${this._logPrefix} Subscribed to VED event '${this._EVENT_TYPE_SUBSCRIBED}' via _addSubscription.`);

        this.refreshList().catch(error => {
            this.logger.error(`${this._logPrefix} Error during initial list refresh:`, error);
        });
    }
    ...
    _renderListItem(actionObject, itemIndex) {
        ...
        if (!actionObject || typeof actionObject.id !== 'string' || !actionObject.id.trim()) {
            this.logger.warn(`${this._logPrefix} Skipping invalid action object in _renderListItem (missing or empty id): `, {actionObject});
            return null;
        }
        if (typeof actionObject.command !== 'string' || !actionObject.command.trim()) {
            this.logger.warn(`${this._logPrefix} Skipping invalid action object (missing command):`, {actionObject});
            return null;
        }
        if (typeof actionObject.name !== 'string' || !actionObject.name.trim()) {
            this.logger.warn(`${this._logPrefix} Skipping invalid action object (missing name):`, {actionObject});
            return null;
        }
        if (typeof actionObject.description !== 'string' || !actionObject.description.trim()) {
            this.logger.warn(`${this._logPrefix} Skipping invalid action object (missing description):`, {actionObject});
            return null;
        }
        ...
        if (!button) {
            this.logger.error(`${this._logPrefix} Failed to create button element for action: "${buttonText}" (ID: ${actionId}) using domElementFactory.`);
            return null;
        }
        ...
        button.addEventListener('click', () => {
            ...
            const clickedAction = this.availableActions.find(a => a.id === actionId);
            if (!clickedAction) {
                this.logger.error(`${this._logPrefix} Critical: Clicked action button with ID '${actionId}' but could not find corresponding action.`, { clickedActionId: actionId });
                return;
            }
            ...
            if (this.selectedAction && this.selectedAction.id === clickedAction.id) {
                ...
                this.selectedAction = null;
                this.logger.info(`${this._logPrefix} Action deselected: '${clickedAction.name}' (ID: ${clickedAction.id})`);
            } else {
                if (this.selectedAction && this.elements.listContainerElement) {
                    const previousButton = this.elements.listContainerElement.querySelector(`button.action-button.selected[data-action-id="${this.selectedAction.id}"]`);
                    if (previousButton) {
    ...
    _onListRendered(actionsData, container) {
        ...
        const actualActionButtons = container.querySelectorAll('button.action-button');
        ...
        if (this.selectedAction && !actionsData?.find(a => a.id === this.selectedAction?.id)) {
            this.selectedAction = null;
            this.logger.debug(`${this._logPrefix} Previously selected action is no longer available. Selection cleared.`);
        }

        if (this.selectedAction) {
            const selectedButton = container.querySelector(`button.action-button[data-action-id="${this.selectedAction.id}"]`);
    ...
    async #handleUpdateActions(eventObject) {
        ...
        if (eventObject?.payload &&
            ...
            const validActions = innerPayload.actions.filter(action => {
                ...
                if (!isValid) {
                    this.logger.warn(`${this._logPrefix} Invalid action object found in payload:`, {action});
                }
                return isValid;
            });

            if (validActions.length !== innerPayload.actions.length) {
                this.logger.warn(`${this._logPrefix} Received '${eventTypeForLog}' with some invalid items. Only valid actions will be rendered.`);
            }
            ...
            this.logger.warn(`${this._logPrefix} Received invalid or incomplete event for '${eventTypeForLog}'. Clearing actions.`, {receivedObject: eventObject});
            this.#currentActorId = null;
            this.selectedAction = null;
            this.availableActions = [];
        }
        ...
        try {
            ...
            this.logger.error(`${this._logPrefix} Error refreshing list in #handleUpdateActions:`, error);
        }
    }
    ...
    async #handleSendAction() {
        ...
        if (!this.#currentActorId) {
            this.logger.error(`${this._logPrefix} #handleSendAction: Cannot send action, currentActorId is not set.`);
            return;
        }
        ...
        this.logger.info(`${this._logPrefix} Attempting to send action: '${actionName}' (ID: ${actionId}) for actor ${this.#currentActorId}, Speech: "${speechText}"`);
        ...
        try {
            ...
            if (dispatchResult) {
                this.logger.debug(`${this._logPrefix} Event '${PLAYER_TURN_SUBMITTED_ID}' dispatched for action '${actionId}' by actor '${this.#currentActorId}'.`);
                if (this.elements.speechInputElement) this.elements.speechInputElement.value = '';
                ...
                if (this.elements.listContainerElement) {
                    const selectedButton = this.elements.listContainerElement.querySelector(`button.action-button.selected[data-action-id="${actionId}"]`);
                    if (selectedButton) {
                        selectedButton.classList.remove('selected');
                    }
                }
            } else {
                this.logger.error(`${this._logPrefix} Failed to dispatch '${PLAYER_TURN_SUBMITTED_ID}' for action '${actionId}'.`, {payload: eventPayload});
            }
        } catch (error) {
            this.logger.error(`${this._logPrefix} Exception during dispatchValidated for '${PLAYER_TURN_SUBMITTED_ID}'.`, { error, payload: eventPayload });
        }
    }
    ...
    dispose() {
        ...
        if (this.elements && this.elements.listContainerElement) {
            while (this.elements.listContainerElement.firstChild) {
                this.elements.listContainerElement.removeChild(this.elements.listContainerElement.firstChild);
            }
            this.logger.debug(`${this._logPrefix} Cleared listContainerElement content during dispose.`);
        }
