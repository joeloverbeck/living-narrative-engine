 ...
 * @extends {BaseListDisplayComponent<LogEntryObject>}
 */
export class PerceptionLogRenderer extends BaseListDisplayComponent {
    ...
    constructor({
        ...
        const elementsConfig = {
            listContainerElement: { selector: '#perception-log-list', required: true, expectedType: HTMLUListElement }
        };
        ...
        this.logger.debug(`${this._logPrefix} Subscribed to VED event '${TURN_STARTED_ID}'.`);

        this.refreshList().catch(error => {
            this.logger.error(`${this._logPrefix} Error during initial refreshList in constructor:`, error);
        });
     ...
     */
    _getListItemsData() {
        this.logger.debug(`${this._logPrefix} _getListItemsData called. Current actor ID: ${this.#currentActorId}`);
        if (!this.#currentActorId) {
            return [];
        }

        try {
            const actorEntity = this.#entityManager.getEntityInstance(this.#currentActorId);
            if (!actorEntity) {
                this.logger.warn(`${this._logPrefix} Actor entity '${this.#currentActorId}' not found in _getListItemsData.`);
                return [];
            }

            if (!actorEntity.hasComponent(PERCEPTION_LOG_COMPONENT_ID)) {
                this.logger.info(`${this._logPrefix} Actor '${this.#currentActorId}' does not have a '${PERCEPTION_LOG_COMPONENT_ID}' component.`);
                return [];
            }
            ...
            if (!perceptionData || !Array.isArray(perceptionData.logEntries) || perceptionData.logEntries.length === 0) {
                this.logger.info(`${this._logPrefix} Actor '${this.#currentActorId}' has '${PERCEPTION_LOG_COMPONENT_ID}' component, but 'logEntries' are empty or malformed.`);
                return [];
            }

            this.logger.debug(`${this._logPrefix} Successfully fetched ${perceptionData.logEntries.length} log entries for actor '${this.#currentActorId}'.`);
            return perceptionData.logEntries;

        } catch (error) {
            this.logger.error(`${this._logPrefix} Error fetching perception log data for actor '${this.#currentActorId}':`, error);
            return [];
        }
    }
     ...
     */
    _renderListItem(logEntry, itemIndex, listData) { // <-- Corrected signature
        // listData parameter is available if needed, though not used in this specific implementation
        if (!logEntry || typeof logEntry !== 'object' || typeof logEntry.descriptionText !== 'string') {
            this.logger.warn(`${this._logPrefix} Malformed log entry at index ${itemIndex}. Skipping.`, {logEntry});
            return null;
        }

        if (!this.domElementFactory) {
            this.logger.error(`${this._logPrefix} DomElementFactory not available in _renderListItem. Cannot create <li>.`);
            return null;
        }
        ...
        if (li) {
            ...
            this.logger.warn(`${this._logPrefix} Failed to create <li> element for log entry via domElementFactory.`);
        }
        return li;
    }
    ...
    _getEmptyListMessage() {
        ...
        try {
            ...
            this.logger.error(`${this._logPrefix} Error in _getEmptyListMessage:`, error);
        }
    ...
    _onListRendered(listData, container) {
        this.logger.debug(`${this._logPrefix} _onListRendered called. Scrolling to bottom.`);
        this.#scrollToBottom();
    }
    ...
    '#handleTurnStarted'(event) {
        this.logger.debug(`${this._logPrefix} #handleTurnStarted received '${event.type}' event. Payload:`, event.payload);

        if (!event.payload || typeof event.payload.entityId !== 'string' || !event.payload.entityId) {
            this.logger.warn(`${this._logPrefix} '${event.type}' event is missing entityId or entityId is invalid. Clearing current actor and refreshing list.`);
            this.#currentActorId = null;
        } else {
            this.#currentActorId = event.payload.entityId;
            this.logger.info(`${this._logPrefix} Current actor ID set to: ${this.#currentActorId}`);
        }

        this.refreshList().catch(error => {
            this.logger.error(`${this._logPrefix} Error during refreshList in #handleTurnStarted:`, error);
            if (this.elements.listContainerElement && this.domElementFactory) {
                const pError = this.domElementFactory.p('error-message', 'Error updating perception log.');
                if (pError) {
    ...
    '#scrollToBottom'() {
        if (this.elements.listContainerElement) {
            if (this.elements.listContainerElement.scrollHeight > this.elements.listContainerElement.clientHeight) {
                this.elements.listContainerElement.scrollTop = this.elements.listContainerElement.scrollHeight;
                this.logger.debug(`${this._logPrefix} Scrolled perception log to bottom.`);
            } else {
                this.logger.debug(`${this._logPrefix} Perception log does not require scrolling or is not scrollable.`);
            }
        } else {
            this.logger.warn(`${this._logPrefix} Cannot scroll to bottom: listContainerElement not found in this.elements.`);
        }
    }
