...
export class BaseListDisplayComponent extends BoundDomRendererBase {
    ...
    constructor({
        ...
        if (!this.elements.listContainerElement) {
            ...
            this.logger.error(errorMsg, {elementsConfigReceived: elementsConfig, resolvedElements: this.elements});
            throw new Error(errorMsg);
        }
        this.logger.debug(`${this._logPrefix} List container element successfully bound:`, this.elements.listContainerElement);
    }
     ...
     */
    _getListItemsData() {
        this.logger.error(`${this._logPrefix} _getListItemsData() is an abstract method and must be implemented by the subclass.`);
    ...
    _renderListItem(itemData, itemIndex, listData) {
        this.logger.error(`${this._logPrefix} _renderListItem() is an abstract method and must be implemented by the subclass.`);
    ...
    _getEmptyListMessage() {
        this.logger.error(`${this._logPrefix} _getEmptyListMessage() is an abstract method and must be implemented by the subclass.`);
    ...
    _onListRendered(listData, container) {
        ...
        this.logger.debug(`${this._logPrefix} _onListRendered hook called. List data count: ${listData ? listData.length : 'null'}.`);
    }
    ...
    async renderList() {
        ...
        if (!this.elements.listContainerElement) {
            this.logger.error(`${this._logPrefix} Cannot render list: 'listContainerElement' is not available.`);
            return;
        }
        ...
        try {
            ...
            this.logger.debug(`${this._logPrefix} Fetched list items data. Count: ${itemsData ? itemsData.length : 'null/undefined'}`);
        } catch (error) {
            this.logger.error(`${this._logPrefix} Error fetching list items data in _getListItemsData():`, error);
            DomUtils.clearElement(this.elements.listContainerElement);
            const errorMsg = this.domElementFactory?.p('error-message', 'Error loading list data.') || this.documentContext.document.createTextNode('Error loading list data.');
        ...
        if (!itemsData || !Array.isArray(itemsData) || itemsData.length === 0) {
            this.logger.info(`${this._logPrefix} List data is empty or not an array. Displaying empty message.`);
            const emptyMessage = this._getEmptyListMessage();
            if (typeof emptyMessage === 'string') {
                if (this.domElementFactory) {
                    const pEmpty = this.domElementFactory.p('empty-list-message', emptyMessage);
                    if (pEmpty) {
                        this.elements.listContainerElement.appendChild(pEmpty);
                    } else { // Fallback if p creation fails
                ...
                this.logger.warn(`${this._logPrefix} _getEmptyListMessage() returned an invalid type. Expected string or HTMLElement.`, {type: typeof emptyMessage});
                const fallbackEmptyMsg = this.domElementFactory?.p('empty-list-message', 'List is empty.') || this.documentContext.document.createTextNode('List is empty.');
            ...
            this.logger.debug(`${this._logPrefix} Populating list with ${itemsData.length} items.`);
            let renderedCount = 0;
            itemsData.forEach((itemData, index) => {
                try {
                    const listItemElement = this._renderListItem(itemData, index, itemsData);
                    if (listItemElement instanceof HTMLElement) {
                        ...
                        this.logger.warn(`${this._logPrefix} _renderListItem for item at index ${index} did not return an HTMLElement or null. Skipping.`, {
                            itemData,
                            returnedValue: listItemElement
                        });
                    }
                } catch (error) {
                    this.logger.error(`${this._logPrefix} Error in _renderListItem for item at index ${index}:`, error, {itemData});
                    // Optionally render an error placeholder for this specific item
                }
            });
            this.logger.info(`${this._logPrefix} Rendered ${renderedCount} out of ${itemsData.length} items into the list.`);
        }

        try {
            this._onListRendered(itemsData, this.elements.listContainerElement);
        } catch (error) {
            this.logger.error(`${this._logPrefix} Error in _onListRendered hook:`, error);
        }
    }
     ...
     */
    async refreshList() {
        this.logger.debug(`${this._logPrefix} refreshList() called, invoking renderList().`);
