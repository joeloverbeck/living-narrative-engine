...
export class ProcessingIndicatorController extends BoundDomRendererBase {
     ...
     * @param {string} [params.speechInputSelector='#speech-input'] - CSS selector for the speech input for player composing indicator.
     */
    constructor({
                    logger,
                    documentContext,
        ...
        if (this.elements.speechInputForPlayerIndicator) {
            ...
            this.logger.debug(`${this._logPrefix} Speech input element for player indicator cached.`);
        } else {
            this.logger.info(`${this._logPrefix} Speech input element (selector: '${speechInputSelector}') not found. Player input indicator will not be active via direct input listening.`);
        }
    ...
    #initializeIndicatorElement() {
        this.#indicatorElement = this.documentContext.query('#processing-indicator');

        if (!this.#indicatorElement) {
            if (this.#domElementFactory && this.elements.outputDiv) {
                this.logger.debug(`${this._logPrefix} #processing-indicator not found in DOM. Creating dynamically.`);
                ...
                if (this.#indicatorElement) {
                    for (let i = 0; i < 3; i++) {
                        ...
                        if (dot) {
                            ...
                            this.logger.warn(`${this._logPrefix} Failed to create dot span for indicator.`);
                        }
                    }
                    this.elements.outputDiv.appendChild(this.#indicatorElement);
                    this.logger.debug(`${this._logPrefix} Processing indicator created and appended to #outputDiv.`);
                } else {
                    this.logger.error(`${this._logPrefix} Failed to create #processing-indicator element using DomElementFactory.`);
                }
            } else {
                this.logger.error(`${this._logPrefix} Cannot create #processing-indicator: DomElementFactory or #outputDiv element missing.`);
            }
        } else {
            this.logger.debug(`${this._logPrefix} #processing-indicator found in DOM.`);
            // Ensure it's a child of outputDiv if it exists but isn't already.
            if (this.elements.outputDiv && this.#indicatorElement.parentElement !== this.elements.outputDiv) {
                this.logger.warn(`${this._logPrefix} #processing-indicator exists but is not a child of #outputDiv. Moving it.`);
                this.elements.outputDiv.appendChild(this.#indicatorElement);
            }
        }
    }
    ...
    #subscribeToEvents() {
        ...
        this.logger.debug(`${this._logPrefix} Subscribed to ${AI_TURN_PROCESSING_STARTED}.`);
        ...
        this.logger.debug(`${this._logPrefix} Subscribed to ${AI_TURN_PROCESSING_ENDED}.`);
        ...
        if (this.#speechInputElement) {
            ...
            this.logger.debug(`${this._logPrefix} Added 'input' listener to speech input element for player indicator.`);
        }
        ...
        this.logger.debug(`${this._logPrefix} Subscribed to ${PLAYER_TURN_SUBMITTED_ID} for hiding player indicator.`);
    }
    ...
    #showIndicator(type) {
        if (this.#indicatorElement) {
            ...
            this.logger.warn(`${this._logPrefix} Attempted to show indicator, but #indicatorElement is null.`);
        }
    }
    ...
    #hideIndicator(type) {
        ...
        if (this.#indicatorElement) {
            this.logger.debug(`${this._logPrefix} Hiding processing indicator for type: ${type}.`);
            this.#indicatorElement.classList.remove('visible');
        } else {
            this.logger.warn(`${this._logPrefix} Attempted to hide indicator, but #indicatorElement is null.`);
        }
    }
     ...
     */
    dispose() {
        this.logger.debug(`${this._logPrefix} Disposing ProcessingIndicatorController.`);
        ...
        if (this.#indicatorElement && this.#indicatorElement.parentElement) {
            ...
            this.logger.debug(`${this._logPrefix} Removed processing indicator element from DOM.`);
        }
        this.#indicatorElement = null;
        this.#speechInputElement = null; // Clear reference
        this.#domElementFactory = null; // Clear reference

        this.logger.info(`${this._logPrefix} ProcessingIndicatorController disposed.`);
    }
}