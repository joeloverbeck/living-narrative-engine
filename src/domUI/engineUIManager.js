...
export class EngineUIManager {
    ...
    constructor({eventDispatcher, domUiFacade, logger}) {
        if (!eventDispatcher) {
            this.#logger?.error('EngineUIManager: ISafeEventDispatcher dependency is required at construction.'); // Logger might not be set yet
            throw new Error('EngineUIManager: ISafeEventDispatcher dependency is required.');
        }
        if (!domUiFacade) {
            this.#logger?.error('EngineUIManager: DomUiFacade dependency is required at construction.');
            throw new Error('EngineUIManager: DomUiFacade dependency is required.');
        }
        if (!logger) {
            // No logger to log with if logger itself is missing.
            console.error('EngineUIManager: ILogger dependency is required at construction.');
    ...
    #handleEngineInitializingUI(event) {
        ...
        this.#logger.debug(`EngineUIManager: Received ${ENGINE_INITIALIZING_UI}`, payload);
        if (!payload || typeof payload.worldName !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_INITIALIZING_UI}.`, payload);
            return;
        }
        ...
        this.#logger.info(`EngineUIManager: Handled ${ENGINE_INITIALIZING_UI}. UI updated for world: ${payload.worldName}.`);
    }
    ...
    #handleEngineReadyUI(event) {
        ...
        if (!payload || typeof payload.message !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_READY_UI}.`, payload);
            return;
        }

        this.#domUiFacade.title.set(payload.activeWorld || "Game Ready");
        this.#domUiFacade.input.setEnabled(true, payload.message);
        this.#logger.info(`EngineUIManager: Handled ${ENGINE_READY_UI}. UI set to ready state. Active world: ${payload.activeWorld || "N/A"}.`);
    }
    ...
    #handleEngineOperationInProgressUI(event) {
        const payload = event.payload;
        this.#logger.debug(`EngineUIManager: Received ${ENGINE_OPERATION_IN_PROGRESS_UI}`, payload);
        if (!payload || typeof payload.titleMessage !== 'string' || typeof payload.inputDisabledMessage !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_OPERATION_IN_PROGRESS_UI}.`, payload);
            return;
        }

        this.#domUiFacade.title.set(payload.titleMessage);
        this.#domUiFacade.input.setEnabled(false, payload.inputDisabledMessage);
        this.#logger.info(`EngineUIManager: Handled ${ENGINE_OPERATION_IN_PROGRESS_UI}. Title: "${payload.titleMessage}". Input disabled.`);
    }
    ...
    #handleEngineOperationFailedUI(event) {
        const payload = event.payload;
        this.#logger.debug(`EngineUIManager: Received ${ENGINE_OPERATION_FAILED_UI}`, payload);
        if (!payload || typeof payload.errorMessage !== 'string' || typeof payload.errorTitle !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_OPERATION_FAILED_UI}.`, payload);
            return;
        }
        ...
        this.#logger.error(`EngineUIManager: Handled ${ENGINE_OPERATION_FAILED_UI}. Error Title: "${payload.errorTitle}", Message: "${payload.errorMessage}".`);
    }
    ...
    #handleEngineStoppedUI(event) {
        const payload = event.payload;
        this.#logger.debug(`EngineUIManager: Received ${ENGINE_STOPPED_UI}`, payload);
        if (!payload || typeof payload.inputDisabledMessage !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_STOPPED_UI}.`, payload);
            return;
        }

        this.#domUiFacade.input.setEnabled(false, payload.inputDisabledMessage);
        this.#domUiFacade.title.set("Game Stopped"); // Optional update as per ticket
        this.#logger.info(`EngineUIManager: Handled ${ENGINE_STOPPED_UI}. Input disabled. Title set to "Game Stopped".`);
    }
    ...
    #handleEngineMessageDisplayRequested(event) {
        const payload = event.payload;
        this.#logger.debug(`EngineUIManager: Received ${ENGINE_MESSAGE_DISPLAY_REQUESTED}`, payload);
        if (!payload || typeof payload.message !== 'string' || typeof payload.type !== 'string') {
            this.#logger.warn(`EngineUIManager: Invalid or missing payload for ${ENGINE_MESSAGE_DISPLAY_REQUESTED}.`, payload);
            return;
        }
        ...
        if (!validTypes.includes(payload.type)) {
            this.#logger.warn(`EngineUIManager: Invalid message type "${payload.type}" for ${ENGINE_MESSAGE_DISPLAY_REQUESTED}. Defaulting to 'info'.`, payload);
            payload.type = 'info';
        }


        this.#domUiFacade.messages.render(payload.message, payload.type);
        this.#logger.info(`EngineUIManager: Handled ${ENGINE_MESSAGE_DISPLAY_REQUESTED}. Displayed ${payload.type} message: "${payload.message.substring(0, 50)}..."`);
    }
    ...
    #handleRequestShowSaveGameUI(event) {
        ...
        if (this.#domUiFacade.saveGame && typeof this.#domUiFacade.saveGame.show === 'function') {
            this.#domUiFacade.saveGame.show();
            this.#logger.info(`EngineUIManager: Handled ${REQUEST_SHOW_SAVE_GAME_UI}. Save Game UI requested to show.`);
        } else {
            this.#logger.warn(`EngineUIManager: SaveGameUI component not available or 'show' method missing on DomUiFacade. Cannot show Save Game UI.`);
        }
    }
    ...
    #handleRequestShowLoadGameUI(event) {
        ...
        if (this.#domUiFacade.loadGame && typeof this.#domUiFacade.loadGame.show === 'function') {
            this.#domUiFacade.loadGame.show();
            this.#logger.info(`EngineUIManager: Handled ${REQUEST_SHOW_LOAD_GAME_UI}. Load Game UI requested to show.`);
        } else {
            this.#logger.warn(`EngineUIManager: LoadGameUI component not available or 'show' method missing on DomUiFacade. Cannot show Load Game UI.`);
        }
    }
    ...
    #handleCannotSaveGameInfo(event) {
        ...
        const message = "Cannot save at this moment (e.g. game not fully initialized or in a critical state).";
        this.#domUiFacade.messages.render(message, 'info');
        this.#logger.info(`EngineUIManager: Handled ${CANNOT_SAVE_GAME_INFO}. Displayed info message: "${message}"`);
    }
