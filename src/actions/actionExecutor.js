...
class ActionExecutor {
     ...
     * @returns {Promise<ActionResult>} A Promise resolving to the result from the handler,
     * or a default error result if no handler is found or an error occurs.
     */
    async executeAction(actionId, context) { // <<< Make async
        const handler = this.handlers.get(actionId);

        if (handler) {
            try {
                const result = await handler(context); // result from executeOpen/handleAction...

                // <<< --- ADD LOGGING BEFORE CHECK --- >>>
                console.log(`ActionExecutor: Received result from handler for "${actionId}":`, JSON.stringify(result));
                // <<< --- END LOGGING --- >>>


                // Basic validation of the returned result
                if (typeof result !== 'object' || result === null || typeof result.success !== 'boolean') {
                    console.error(`ActionExecutor: Handler for "${actionId}" returned an invalid ActionResult structure.`, result); // Log the exact object again on error
                    // Return a valid ActionResult structure for the error
                    return { // <<< No need to await here, just returning an object
                        success: false,
            ...
            return { // <<< No need to await here
                success: false,
                messages: [{ text: `Internal Error: The action '${actionId.split(':').pop()}' is not configured.`, type: 'error' }],
                newState: undefined
            };
        }
    }
