 ...
 * 7. Dispatch EVENT_MOVE_ATTEMPTED with detailed payload if validation passes.
 * 8. Dispatch EVENT_MOVE_FAILED if any validation step fails.
 *
 * Does NOT modify player position directly. Relies on a MovementSystem listening
 * to EVENT_MOVE_ATTEMPTED.
 *
...
export function executeMove(context) {
    ...
    try {
        ...
        const details = `Error dispatching ${EVENT_MOVE_ATTEMPTED}: ${error.message}`;
        dispatch(EVENT_MOVE_FAILED, {
            actorId,
            locationId: currentLocationId,
            direction,
        ...
        console.error(`executeMove: Failed to dispatch ${EVENT_MOVE_ATTEMPTED} for ${actorId} moving towards ${targetLocationId} via ${connectionEntityId}`, error);
        success = false;
    }
