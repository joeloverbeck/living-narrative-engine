 ...
 * Does NOT apply damage or handle death directly; dispatches 'event:attack_intended'.
...
export async function executeAttack(context) {
    ...
    const onFoundUnique = (innerContext, targetEntity, messages) => {
        ...
        const dispatchResult = dispatchEventWithCatch(
            innerContext,
            'event:attack_intended',
            eventPayload,
            messages, // Pass messages array for internal logging
            {
                success: `Dispatched event:attack_intended vs ${targetDisplayName} (${targetEntity.id})`,
                errorUser: TARGET_MESSAGES.INTERNAL_ERROR, // User message on dispatch failure
                errorInternal: `Failed to dispatch event:attack_intended vs ${targetDisplayName} (${targetEntity.id}).` // Internal log on failure
            }
        );
