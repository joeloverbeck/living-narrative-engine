...
export async function executeTake(context) {
    ...
    const onFoundUnique = (innerContext, targetItemEntity, messages) => {
        ...
        // AC 4: Use dispatchEventWithCatch inside callback for event:item_picked_up
        ...
        const dispatchResult = dispatchEventWithCatch(
            innerContext,
            'event:item_picked_up', // The core game event
            eventPayload,
            messages, // Pass messages array for internal logging from the utility
            {
                success: `Dispatched event:item_picked_up for ${itemName} (${targetItemId}) from location ${locationId}`,
                errorUser: TARGET_MESSAGES.INTERNAL_ERROR, // User message on dispatch failure
                errorInternal: `Failed to dispatch event:item_picked_up for ${itemName} (${targetItemId}) from location ${locationId}.` // Internal log on failure
            }
        );
        ...
        if (dispatchResult.success) {
            ...
            try {
                innerContext.dispatch('action:take_failed', {
                    ...
                    locationId: locationId,
                    details: `Failed to dispatch event:item_picked_up for ${itemName}` // Pass limited details
                });
                messages.push({
