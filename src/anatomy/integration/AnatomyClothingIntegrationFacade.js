/**
 * @file Migration facade for AnatomyClothingIntegrationService
 * Provides backward compatibility while using new decomposed services internally
 * This facade maintains the original API to allow gradual migration of dependent services
...
/**
 * Migration facade that provides the original AnatomyClothingIntegrationService API
 * while delegating to the new decomposed services internally.
 * 
 * This facade allows dependent services to continue working without immediate updates
 * while we migrate them one by one to use the decomposed services directly.
 * 
 * @deprecated Use the decomposed services directly instead of this facade
 */
class AnatomyClothingIntegrationFacade extends BaseService {
  #logger;
  #entityManager;
  #bodyGraphService;
  ...
  constructor({
    ...
    this.#logger = this._init('AnatomyClothingIntegrationFacade', logger, {
      entityManager: {
        value: entityManager,
    ...
    this.#logger.warn(
      'AnatomyClothingIntegrationFacade is deprecated. ' +
      'Please migrate to use the decomposed services directly.'
    );
  }
  ...
  setSlotEntityMappings(mappings) {
    ...
    this.#logger.debug(
      `AnatomyClothingIntegrationFacade: Updated slot-entity mappings with ${this.#slotEntityMappings.size} entries`
    );
  }
  ...
  async #getEntityAnatomyStructure(entityId) {
    ...
    if (allParts.length === 0) {
      this.#logger.debug(
        'AnatomyClothingIntegrationFacade: Body graph empty, using fallback joint traversal'
      );
    ...
    this.#logger.debug(
      `AnatomyClothingIntegrationFacade: Collected ${sockets.size} unique sockets from ${allParts.length + 1} entities`
    );
      ...
      for (const entity of entitiesWithJoints) {
        ...
        try {
          ...
          if (joint?.parentEntityId === currentEntityId) {
            ...
            visited.add(partId);
            this.#logger.debug(
              `AnatomyClothingIntegrationFacade: Found connected part ${partId} with parent ${currentEntityId}`
            );
          }
        } catch (err) {
...
export default AnatomyClothingIntegrationFacade;