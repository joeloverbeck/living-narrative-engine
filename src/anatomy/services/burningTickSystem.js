...
class BurningTickSystem extends BaseService {
   ...
   */
  async processTick() {
    const burningParts = this.#entityManager.getEntitiesWithComponent(BURNING_COMPONENT_ID);
  ...
  async #processBurningPart(partId) {
    ...
    const burningData = this.#entityManager.getComponentData(partId, BURNING_COMPONENT_ID);
    if (!burningData) {
      return;
    }
