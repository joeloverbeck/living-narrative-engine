...
class RuleLoader extends BaseManifestItemLoader {
    ...
    async _processFetchedItem(modId, filename, resolvedPath, data, typeName) { // <<< MODIFIED Return Type in JSDoc
        ...
        if (typeof ruleIdInData === 'string' && ruleIdInData.trim()) {
            ...
            if (baseRuleId.startsWith(`${modId}:`)) {
                this._logger.warn(`RuleLoader [${modId}]: rule_id '${baseRuleId}' in ${filename} already prefixed. Stripping prefix.`);
            ...
            baseRuleId = namePart;
            this._logger.debug(`RuleLoader [${modId}]: No valid 'rule_id' found. Derived baseRuleId '${baseRuleId}' from filename.`);
        }
        ...
        try {
            ...
            didOverride = this._storeItemInRegistry('rules', modId, baseRuleId, data, filename); // <<< CAPTURE result
        } catch (storageError) {
        ...
        this._logger.debug(`RuleLoader [${modId}]: Successfully processed rule from ${filename}. Returning final registry key: ${finalRegistryKey}, Overwrite: ${didOverride}`);
    ...
    async loadAllRules(modsToLoad) {
        this._logger.warn(`RuleLoader: loadAllRules is potentially deprecated. Aggregation should use results from loadItemsForMod.`);
        ...
        for (const {modId, manifest} of modsToLoad) {
            ...
            try {
                ...
                this._logger.error(`RuleLoader: Unexpected error during rule loading orchestration for mod '${modId}'. Error: ${error.message}`, { error, modId });
                // Continue to next mod
            }
        }

        this._logger.info(`RuleLoader: Finished loading rules for all mods. Total rules count: ${totalRulesLoaded}.`);
