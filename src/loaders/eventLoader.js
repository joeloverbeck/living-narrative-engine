...
class EventLoader extends BaseManifestItemLoader {
    ...
    async _processFetchedItem(modId, filename, resolvedPath, data, typeName) { // <<< MODIFIED Return Type in JSDoc
        ...
        if (colonIndex === -1) {
            ...
            if (namespacePart && baseIdPart) { baseEventId = baseIdPart; }
        }
        ...
        if (payloadSchema && typeof payloadSchema === 'object' && Object.keys(payloadSchema).length > 0) {
            ...
            if (!this._schemaValidator.isSchemaLoaded(payloadSchemaId)) {
                ...
                try {
                    ...
                    this._logger.debug(`EventLoader [${modId}]: Successfully registered payload schema '${payloadSchemaId}'.`);
                    ...
                    this._logger.error(errorMsg, { error: addSchemaError?.message || addSchemaError }, addSchemaError);
                    throw new Error(`CRITICAL: Failed to register payload schema '${payloadSchemaId}'.`);
                }
            } else {
                this._logger.warn(`EventLoader [${modId}]: Payload schema ID '${payloadSchemaId}' for event '${trimmedFullEventId}' was already loaded. Overwriting/duplicate.`);
            }
        } else {
            this._logger.debug(`EventLoader [${modId}]: No valid payloadSchema found for event '${trimmedFullEventId}'. Skipping registration.`);
        }
        ...
        try {
            ...
            didOverride = this._storeItemInRegistry(typeName, modId, baseEventId, data, filename); // <<< CAPTURE result
        } catch (storageError) {
        ...
        this._logger.debug(`EventLoader [${modId}]: Successfully processed event definition from ${filename}. Returning final registry key: ${finalRegistryKey}, Overwrite: ${didOverride}`);
