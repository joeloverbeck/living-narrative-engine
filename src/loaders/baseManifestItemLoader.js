...
export class BaseManifestItemLoader extends AbstractLoader {
  ...
  async loadItemsForMod(
    ...
    // <<< MODIFIED RETURN TYPE
    this._logger.info(
      `${this.constructor.name}: Loading ${registryKey} definitions for mod '${modId}'.`
    );
    if (typeof modId !== 'string' || modId.trim() === '') {
      this._logger.error(
        `${this.constructor.name}: Invalid 'modId' provided for loading ${registryKey}. Must be a non-empty string. Received: ${modId}`
      );
      return { count: 0, overrides: 0, errors: 0 }; // <<< MODIFIED RETURN VALUE
    }
    ...
    if (!modManifest || typeof modManifest !== 'object') {
      this._logger.error(
        `${this.constructor.name}: Invalid 'modManifest' provided for loading ${registryKey} for mod '${trimmedModId}'. Must be a non-null object. Received: ${typeof modManifest}`
      );
      return { count: 0, overrides: 0, errors: 0 }; // <<< MODIFIED RETURN VALUE
    }
    ...
    const trimmedContentTypeDir = diskFolder.trim();
    ...
    const trimmedTypeName = registryKey.trim();
