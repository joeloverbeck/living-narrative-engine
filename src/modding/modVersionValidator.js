...
export default function validateModEngineVersions(manifests, logger) {
    ...
    if (!(manifests instanceof Map)) {
        ...
        throw new Error('validateModEngineVersions: Input `manifests` must be a Map.');
    }
    if (!logger || typeof logger.info !== 'function' || typeof logger.error !== 'function') {
        // Basic type check for the logger
        throw new Error('validateModEngineVersions: Input `logger` must be a valid ILogger instance.');
    }
