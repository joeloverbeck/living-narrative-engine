...
property('socket consistency', () => {
  ...
  const bySubType = groupBy(allEntities, e => e.components['anatomy:part']?.subType);

  for (const [subType, entities] of Object.entries(bySubType)) {
    const socketSets = entities.map(e =>
      new Set(e.components['anatomy:sockets']?.sockets?.map(s => s.id) || [])
    );
