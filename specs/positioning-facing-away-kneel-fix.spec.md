...
describe('positioning:actors_in_location_facing scope', () => {
  it('should exclude actors that the current actor is facing away from', () => {
    ...
    const results = resolveScope('positioning:actors_in_location_facing', { actor: actor1 });
...
```json
{
  ...
  "parameters": {
    ...
    "then_actions": [
      { "type": "FAIL_ACTION", "parameters": { "reason": "Cannot kneel to someone you're facing away from" }}
    ]
  }
}
```
