...
console.log(
  `Found ${knownOperations.length} operations in KNOWN_OPERATION_TYPES\n`
);
...
for (const operationType of knownOperations) {
  ...
  if (!operationSchemaContent.includes(`./operations/${schemaFileName}`)) {
    console.error(
      `❌ ${operationType}: Not referenced in operation.schema.json`
    );
    console.error(
      `   Add: { "$ref": "./operations/${schemaFileName}" } to anyOf array\n`
    );
    hasErrors = true;
  } else {
