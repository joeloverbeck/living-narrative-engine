...
async function debugSchemaLoading() {
  ...
  try {
    ...
    console.log(
      `Rule schema loaded initially: ${validator.isSchemaLoaded(ruleSchemaId)}`
    );
    ...
    console.log(
      `Rule schema loaded after loadAndCompileAllSchemas: ${validator.isSchemaLoaded(ruleSchemaId)}`
    );
    ...
    const testRuleData = {
      ...
      actions: [
        {
          type: 'LOG',
          parameters: { message: 'Actor is getting up from furniture' },
        },
    ...
    const isValid = await validator.validateAgainstSchema(
      testRuleData,
      ruleSchemaId
    );
