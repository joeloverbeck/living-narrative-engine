...
Operations in the rule execution system can produce result values that are stored in `result_variable` fields and used by subsequent operations. Understanding these result structures is essential for:
...
### Best Practices for Result Variables
...
### Troubleshooting Result Variables
