...
## Summary
Extract `#generateRandomState()`, `#sampleValue()`, and `#sampleGaussianDelta()` from `MonteCarloSimulator` into a dedicated `RandomStateGenerator` service. This separates state sampling concerns from simulation orchestration while preserving the current sampling behavior and axis definitions.
...
3. New test: `generate('uniform', 'static')` samples previous/current independently
4. New test: `generate('gaussian', 'static')` uses Box-Muller sampling and clamps to range
5. New test: `generate('gaussian', 'dynamic')` uses delta-based sampling for current vs previous
6. New test: Generated mood axes are in [-100, 100] integer range
7. New test: Generated sexual axes respect their specific integer ranges
8. New test: Generated affectTraits are in [0, 100] integer range

### Invariants That Must Remain True
1. Mood axes range: [-100, 100] with integer rounding
2. Sexual ranges: sex_excitation/inhibition [0, 100], baseline_libido [-50, 50] with integer rounding
3. Sigma values unchanged: MOOD_DELTA_SIGMA=15, SEXUAL_DELTA_SIGMA=12, LIBIDO_DELTA_SIGMA=8
4. Box-Muller transform for Gaussian sampling and Gaussian deltas
5. State structure: `{current: {mood, sexual}, previous: {mood, sexual}, affectTraits}`
6. Mood axes list is unchanged (8 axes): valence, arousal, agency_control, threat, engagement, future_expectancy, self_evaluation, affiliation
7. Affect traits list is unchanged (3 traits): affective_empathy, cognitive_empathy, harm_aversion

## Implementation Notes

### RandomStateGenerator Interface (aligned to current MonteCarloSimulator behavior)
```javascript
/**
...
class RandomStateGenerator {
  ...
  generate(distribution = 'uniform', samplingMode = 'static') {
    // Previous state sampled first (always random)
    const previous = {
    ...
    const current =
      samplingMode === 'dynamic'
        ? {
            mood: this.#generateMoodDelta(previous.mood),
            sexual: this.#generateSexualDelta(previous.sexual),
          }
        : {
            mood: this.#generateMood(distribution),
            sexual: this.#generateSexual(distribution),
    ...
    const affectTraits = this.#generateAffectTraits(distribution);
  ...
  #generateAffectTraits(distribution) {
    const traits = {};
...
## Outcome
- Extracted random state generation into `RandomStateGenerator` with existing mood axes, trait axes, rounding, and sampling semantics preserved.
- Wired the new service into DI and `MonteCarloSimulator` while keeping public APIs stable.
- Added focused unit tests for random state generation and updated simulator tests to inject the new service.
