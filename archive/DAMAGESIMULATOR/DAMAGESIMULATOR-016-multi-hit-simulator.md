...
### Modify
- `src/domUI/damage-simulator/DamageSimulatorUI.js` - Integrate simulator controls (add 'multiHitSimulator' to valid child component names)
- `src/dependencyInjection/registrations/damageSimulatorRegistrations.js` - Register simulator
- `src/dependencyInjection/tokens/tokens-ui.js` - Add MultiHitSimulator token
- `css/damage-simulator.css` - Add simulator control styles (use existing `.ds-button--*` naming convention)
...
<div class="ds-multi-hit-simulator">
  ...
  <div class="ds-sim-controls">
    <button id="sim-run-btn" class="ds-button ds-button--primary">▶ Run Simulation</button>
    <button id="sim-stop-btn" class="ds-button ds-button--danger" disabled>⏹ Stop</button>
  </div>
...
## Definition of Done
- [x] MultiHitSimulator created with full JSDoc
- [x] Unit tests with ≥90% coverage
- [x] Simulator registered in DI container
- [x] Integrated with DamageSimulatorUI
- [x] Hit count configuration working
- [x] Delay configuration working
- [x] All target modes implemented
- [x] Run/Stop controls functional
- [x] Progress display updates correctly
- [x] Summary statistics generated
- [x] Immediate stop capability works
- [x] CSS styles complete
- [x] ESLint passes: `npx eslint src/domUI/damage-simulator/MultiHitSimulator.js`
...
Created the MultiHitSimulator component for batch damage simulation with configurable hit counts (1-100), delays (0-1000ms), and three targeting modes (random, round-robin, focus).

### Files Created
- `src/domUI/damage-simulator/MultiHitSimulator.js` (835 lines) - Full implementation with:
  - Inner `TargetSelector` class for target mode handling
  - Cancellable delay mechanism for immediate stop capability
  - Event emission for progress, completion, stopped, and error states
  - UI rendering with run/stop controls, progress bar, and results display

- `tests/unit/domUI/damage-simulator/MultiHitSimulator.test.js` (815 lines) - 53 unit tests covering:
  - Constructor validation and dependency injection
  - Hit count and delay range validation
  - All three targeting modes (random, round-robin, focus)
  - Run/stop lifecycle with immediate stop capability
...
### Files Modified
- `src/dependencyInjection/tokens/tokens-ui.js` - Added `MultiHitSimulator` token
- `src/dependencyInjection/registrations/damageSimulatorRegistrations.js` - Added factory registration following existing pattern
- `src/domUI/damage-simulator/DamageSimulatorUI.js` - Added 'multiHitSimulator' to valid child component names
- `css/damage-simulator.css` - Added comprehensive styles for simulator UI (216 lines)

### Key Design Decisions
1. **Factory Pattern for DI**: Used singletonFactory lifecycle with containerElement provided at runtime (consistent with other damage simulator components)
2. **Cancellable Delays**: Implemented `#delayResolve` pattern to allow immediate stop of pending delays
3. **Event-Driven Updates**: All state changes communicated through eventBus for loose coupling with other components
4. **Simplified Dependencies**: Removed `damageHistoryTracker` from constructor - history tracking happens via event subscription

### Test Results
- 53 unit tests passing
- All 13 acceptance criteria tests implemented and passing
- DI registration tests passing (4 tests)

### Deviations from Ticket
1. **CSS Class Names**: Used existing `.ds-button--*` convention as noted in ticket corrections
2. **Constructor Dependencies**: Simplified from ticket - removed damageHistoryTracker (uses events instead)
