...
> **API Corrections (2024-12-31)**: The sample code below has been updated to reflect actual API method signatures:
> - `context.ui.loadEntity()` → `handleEntitySelection()` (DamageSimulatorUI.js)
> - `context.services.composer.setConfiguration()` → `updateDamageConfig()` (DamageCapabilityComposer.js)
> - Other service methods (`applyDamage`, `getAnalytics`, `configure`, `run`, `stop`, `calculateProbabilities`) are correct as documented.
...
export async function setupDamageSimulator() {
  // Set up real services with mocked boundaries (logger, eventDispatcher)
  // Following pattern from tests/integration/anatomy/damage-application.integration.test.js
  // Return initialized services and cleanup function
}
...
describe('Damage Simulator Workflow', () => {
  ...
  describe('Entity Loading', () => {
    it('should load entity and display anatomy correctly', async () => {
      ...
      // Act - Use actual API: handleEntitySelection (not loadEntity)
      await context.ui.handleEntitySelection(entityDefId);
  ...
  describe('Damage Application Flow', () => {
    it('should configure damage and apply to entity', async () => {
      // Arrange - Use actual API: handleEntitySelection
      await context.ui.handleEntitySelection('test_humanoid');
      // Use actual API: updateDamageConfig (not setConfiguration)
      context.services.composer.updateDamageConfig(SAMPLE_DAMAGE_ENTRY);
...
describe('Analytics Accuracy', () => {
  it('should calculate hits-to-destroy within 10% accuracy', async () => {
    // Arrange - Use actual API: handleEntitySelection
    await context.ui.handleEntitySelection('test_humanoid');
    const damageEntry = { base_damage: 10, damageType: 'slashing', penetration: 0 };
    // Use actual API: updateDamageConfig
    context.services.composer.updateDamageConfig(damageEntry);
...
describe('Multi-Hit Simulation', () => {
  it('should execute correct number of hits', async () => {
    // Arrange - Use actual API: handleEntitySelection
    await context.ui.handleEntitySelection('test_humanoid');
    // configure() API is correct as documented
...
- [x] Test fixtures documented and reusable
- [x] Workflow tests cover full entity → damage → display flow
- [x] Analytics accuracy tests validate calculations
- [x] Multi-hit simulation tests cover all modes
- [x] All tests pass consistently (no flaky tests)
- [x] Tests clean up resources properly
- [x] `npm run test:integration` includes new tests
- [x] Tests complete in reasonable time (<60s total)
...
### Files Created
1. `tests/common/damage-simulator/damageSimulatorTestFixtures.js` - Shared test fixtures
2. `tests/integration/damage-simulator/damageSimulatorWorkflow.integration.test.js` - Workflow integration tests
3. `tests/integration/damage-simulator/analyticsAccuracy.integration.test.js` - Analytics accuracy tests
4. `tests/integration/damage-simulator/multiHitSimulation.integration.test.js` - Multi-hit simulation tests
...
1. **Test Fixtures**: Created comprehensive fixtures with:
   - `seedTestAnatomy()` - Seeds entity manager with body parts and health components
   - `createMockAnatomyData()` - Mock anatomy data for testing
   - `createSimulationConfig()` - Valid simulation configurations
   - Helper functions for mock logger, dispatcher, and hit probability utilities

2. **Workflow Tests**: 17 tests covering:
   - Entity loading and anatomy component validation
   - Damage application flow with health reduction verification
   - History tracking with statistics
   - Error handling with graceful recovery
   - State consistency across multiple damage applications

3. **Analytics Tests**: 21 tests covering:
   - Hits-to-destroy calculations (10% accuracy threshold)
   - Hit probability distributions (sum to 100%)
   - Death condition monitoring
   - Real-time analytics updates
   - Edge cases (0 damage, max health)
   - Aggregate statistics accuracy

4. **Multi-Hit Simulation Tests**: 24 tests covering:
   - Correct hit count execution
   - Target mode behavior (random, round-robin, focused)
   - Stop functionality with minimal delay
   - Concurrent simulation prevention
   - Progress tracking and event dispatching
   - Summary statistics accuracy

### Issues Resolved During Implementation
1. **API Corrections**: Updated ticket sample code to use actual API signatures (`handleEntitySelection`, `updateDamageConfig`)
2. **Property Naming**: Fixed `current`/`max` → `currentHealth`/`maxHealth` to match DamageResolutionService expectations
3. **Race Condition**: Fixed stop functionality test by using minimal delay (1ms) instead of zero delay
