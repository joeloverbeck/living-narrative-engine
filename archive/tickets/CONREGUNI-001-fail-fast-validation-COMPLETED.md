...
Add immediate validation after condition registration to detect infrastructure misconfigurations early. If a registered condition cannot be looked up via `dataRegistry.getConditionDefinition()`, throw an informative error immediately. `registerCondition()` already writes to both `fixture._loadedConditions` and `testEnv._loadedConditions`; this ticket only adds the fail-fast validation step.
...
- `registerCondition()` already writes to both `fixture._loadedConditions` and `testEnv._loadedConditions`; no additional synchronization changes are needed here.
- `dataRegistry.getConditionDefinition()` exists on the test environment and is the authoritative lookup path for conditions in tests.
...
- [x] `registerCondition()` validates findability after storage
- [x] Throws informative error with debug state if not findable
- [x] New test case for fail-fast behavior added
- [x] All existing condition registration tests pass
- [x] No changes to public API signature

## Outcome

- Added fail-fast validation after registration and captured diagnostics when the lookup chain is misconfigured.
- Updated condition registration unit tests to install the dataRegistry override before using `registerCondition()`.
