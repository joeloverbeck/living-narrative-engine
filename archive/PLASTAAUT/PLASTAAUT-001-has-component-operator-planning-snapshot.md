...
- The unresolved gap is that `PlanningStateView` raises `GOAP_STATE_MISS` errors when `GOAP_STATE_ASSERT=1`, but `HasComponentOperator.evaluate` catches _all_ errors and returns `false`, so those hard failures never propagate to JSON Logic evaluations.
