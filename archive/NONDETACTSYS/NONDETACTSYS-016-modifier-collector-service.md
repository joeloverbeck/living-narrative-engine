...
- [x] Service has no side effects on entity state
- [x] Empty input returns valid ModifierCollection
- [x] Stacking rules follow spec (highest absolute value wins)
- [x] All dependencies properly validated
...
Successfully implemented ModifierCollectorService as a Phase 5 stub with full infrastructure for future modifier-based gameplay.
...
- **All combat tests**: 152 tests, all passing
- **Lint**: 0 errors (22 warnings for JSDoc formatting - pre-existing style)

### Implementation Notes

1. **Constructor validation order**: Logger validated FIRST (before entityManager) to match existing service patterns (SkillResolverService) - this ensures logger is available for subsequent validation messages.

2. **Phase 5 stub**: `#collectActionModifiers` returns empty array as JSON Logic condition evaluation is out of scope. Infrastructure ready for future implementation.

3. **entityManager reserved**: Private field `#entityManager` is declared but unused in Phase 5 stub. Will be used for buff/equipment/environment modifier collection in future phases.

### Verification Commands

```bash
# All tests pass
NODE_ENV=test npx jest tests/unit/combat/ --no-coverage --silent
# Result: 4 passed, 4 total (152 tests)

# Lint clean (no errors)
npx eslint src/combat/services/ModifierCollectorService.js
# Result: 0 errors, 22 warnings (JSDoc style)
```
