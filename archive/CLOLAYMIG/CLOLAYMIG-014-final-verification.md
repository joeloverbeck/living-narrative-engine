...
1. **Fixed outdated integration test** in `tests/integration/mods/base-clothing/baseClothingEntityMigration.integration.test.js`:
   ...
   - Updated describe block to "Post-Migration Cleanup Verification"
   - Changed tests to verify:
     - Entities are NO LONGER in clothing mod (expected 0)
     - Entities exist ONLY in base-clothing mod with correct `base-clothing:` prefix
