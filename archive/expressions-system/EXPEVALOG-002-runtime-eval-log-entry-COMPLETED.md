...
- `ExpressionEvaluatorService` already exposes `evaluateAll`, and `ExpressionDispatcher` already has `dispatchWithResult`; no new evaluator or dispatcher APIs are required.
...
Logging now posts evaluation entries with ordered matches, selected summary, dispatch result, and event type; the runtime uses the existing evaluator/dispatcher APIs plus a new logger utility and endpoint config hook as planned.
