...
- Add `dispatchWithResult(actorId, expression, turnNumber)` that returns an object like:
  - `attempted` (boolean; true only when the event bus dispatch was invoked)
  - `success` (boolean)
  - `rateLimited` (boolean)
...
- Update unit tests to cover the new return shape for `dispatchWithResult()` (rate-limited, missing location, dispatch error, success) while keeping existing `dispatch()` tests intact.
...
- Added `dispatchWithResult()` returning attempted/success/rateLimited/reason and kept `dispatch()` as a boolean wrapper.
- Expanded unit coverage to assert structured outcomes for success, rate limiting, missing location, and dispatch errors.
