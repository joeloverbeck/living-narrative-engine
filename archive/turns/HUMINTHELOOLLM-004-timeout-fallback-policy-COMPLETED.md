...
**Summary:** The LLM gate already dispatches `llm:suggested_action`, sets `awaitingExternalEvent`, and waits on the prompt. There is currently *no* timeout or fallback policy. Add an opt-in timeout for pending LLM suggestions with policies `autoAccept`, `autoWait`, or `noop`, keeping logging and single-processing guarantees.
**Status:** Completed
