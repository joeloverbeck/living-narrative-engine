...
- `items:liquid_container` already supports `tags`, so tagging is available; we just lack canonical content with a `disinfectant` tag and non-zero volume.
...
## Deliverables
- New entity definition at `data/mods/items/entities/definitions/antiseptic_bottle.entity.json` (portable item with `items:liquid_container.tags` containing `"disinfectant"` and `currentVolumeMilliliters > 0`).
...
## Acceptance criteria
- Content test/validation confirms the antiseptic bottle is loadable, included in the Items manifest, and has `items:liquid_container.tags` containing `"disinfectant"` with positive volume.
