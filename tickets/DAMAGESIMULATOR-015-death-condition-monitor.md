...
### Reference (Read Only)
- `data/mods/anatomy/components/death_on_destruction.component.json` - Death trigger component
- `src/anatomy/services/damageTypeEffectsService.js` - Death condition handling
...
1. Identify parts with `anatomy:death_on_destruction` component
2. Track current health vs destruction threshold
...
function findDeathTriggeringParts(anatomyData) {
  ...
  for (const part of anatomyData.parts) {
    const deathComponent = part.components['anatomy:death_on_destruction'];
    if (deathComponent) {
      criticalParts.push({
        partId: part.id,
        partName: part.name,
        deathThreshold: deathComponent.threshold || 0,
        currentHealth: part.components['anatomy:health']?.current || 0,
        maxHealth: part.components['anatomy:health']?.max || 0
      });
    }
  }
...
## Definition of Done
- [ ] DeathConditionMonitor created with full JSDoc
- [ ] Unit tests with ≥90% coverage
- [ ] Monitor registered in DI container
- [ ] Integrated with DamageSimulatorUI
- [ ] Death-triggering parts identified correctly
- [ ] Health status displays accurately
- [ ] Hits-until-death calculated correctly
- [ ] Warning indicators functional
- [ ] Death alert triggers appropriately
- [ ] Real-time updates from damage events
- [ ] CSS styles complete with animations
- [ ] ESLint passes: `npx eslint src/domUI/damage-simulator/DeathConditionMonitor.js`
