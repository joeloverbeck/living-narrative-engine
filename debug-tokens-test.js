...
class SafeEventDispatcher {
  ...
  async dispatch(eventName, payload, options = {}) {
    ...
    if (isErrorEvent || hasErrorKeywords || this.#isHandlingError) {
      console.log(`BLOCKED: Event ${eventName} - isErrorEvent: ${isErrorEvent}, hasErrorKeywords: ${hasErrorKeywords}, isHandlingError: ${this.#isHandlingError}`);
      return false;
    }
...
async function test() {
  ...
  for (let i = 1; i <= 3; i++) {
    await dispatcher.dispatch('core:system_error_occurred', { message: `Error ${i}` });
  }
}
