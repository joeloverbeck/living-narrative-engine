...
import {afterEach, beforeEach, describe, expect, jest, test, fail} from '@jest/globals'; // Added fail
...
describe('proxyApiUtils', () => {
    ...
    beforeEach(() => {
        ...
        ensureValidLogger.mockImplementation((loggerParam, fallbackMessagePrefix) => {
            if (loggerParam &&
    ...
    describe('Workspace_retry', () => {
        ...
        const mockErrorJsonResponseData = {error: {message: 'error details', code: 'SOME_CODE'}};
        const mockErrorTextResponseData = 'A plain text error occurred on the server.';
        ...
        const mockFetchResponse = (status, body, isJsonBody = true, ok = true) => {
            ...
            if (isJsonBody) {
                ...
                response.text.mockResolvedValueOnce(typeof body === 'string' ? body : JSON.stringify(body));
            } else {
                response.text.mockResolvedValue(body);
                response.json.mockRejectedValueOnce(new Error('Response is not valid JSON'));
            }
            return response;
        };

        test('should return parsed JSON on successful fetch (first attempt)', async () => {
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));

            const result = await Workspace_retry(mockUrl, mockDefaultOptions, mockMaxRetries, mockBaseDelayMs, mockMaxDelayMs, mockLogger);

            expect(result).toEqual(mockSuccessResponseData);
            expect(fetch).toHaveBeenCalledTimes(1);
            expect(fetch).toHaveBeenCalledWith(mockUrl, mockDefaultOptions);
            expect(ensureValidLogger).toHaveBeenCalledWith(mockLogger, 'Workspace_retry');
            expect(mockLogger.info).toHaveBeenCalledWith(`Workspace_retry: Initiating request sequence for ${mockUrl} with maxRetries=${mockMaxRetries}, baseDelayMs=${mockBaseDelayMs}, maxDelayMs=${mockMaxDelayMs}.`);
            expect(mockLogger.debug).toHaveBeenCalledWith(`Attempt 1/${mockMaxRetries} - Fetching ${mockDefaultOptions.method} ${mockUrl}`);
            expect(mockLogger.debug).toHaveBeenCalledWith(`Workspace_retry: Attempt 1/${mockMaxRetries} for ${mockUrl} - Request successful (status 200). Parsing JSON response.`);
            expect(mockLogger.info).toHaveBeenCalledWith(`Workspace_retry: Successfully fetched and parsed JSON from ${mockUrl} after 1 attempt(s).`);
        ...
        RETRYABLE_HTTP_STATUS_CODES.forEach(status => {
            test(`should retry on retryable HTTP status ${status} and succeed on second attempt`, async () => {
                fetch
                    .mockResolvedValueOnce(mockFetchResponse(status, mockErrorJsonResponseData, true, false))
                    .mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));

                mathRandomSpy.mockReturnValue(0.5); // No jitter for predictable delay

                const promise = Workspace_retry(mockUrl, mockDefaultOptions, mockMaxRetries, mockBaseDelayMs, mockMaxDelayMs, mockLogger);
                ...
                const expectedDelay = _calculateRetryDelay_forTest(1, mockBaseDelayMs, mockMaxDelayMs);
                expect(setTimeout).toHaveBeenCalledTimes(1);
                expect(setTimeout).toHaveBeenCalledWith(expect.any(Function), expectedDelay);
                expect(mockLogger.warn).toHaveBeenCalledWith(
                    expect.stringContaining(`Attempt 1/${mockMaxRetries} for ${mockUrl} failed with status ${status}. Retrying in ${expectedDelay}ms... Error body preview: ${JSON.stringify(mockErrorJsonResponseData).substring(0, 100)}`)
                );
                expect(mockLogger.info).toHaveBeenCalledWith(`Workspace_retry: Successfully fetched and parsed JSON from ${mockUrl} after 2 attempt(s).`);
            });
        });

        test('should fail immediately for non-retryable HTTP status (e.g., 400)', async () => {
            const nonRetryableStatus = 400;
            fetch.mockResolvedValueOnce(mockFetchResponse(nonRetryableStatus, mockErrorJsonResponseData, true, false));

            await expect(
                Workspace_retry(mockUrl, mockDefaultOptions, mockMaxRetries, mockBaseDelayMs, mockMaxDelayMs, mockLogger)
            ).rejects.toThrow(
        ...
        const networkErrorMessages = [
            'failed to fetch', 'network request failed', 'dns lookup failed',
            'socket hang up', 'econnrefused', 'econreset', 'enotfound', 'etimedout',
        ];
        ...
        test('should correctly parse JSON error response body on failure', async () => {
            fetch.mockResolvedValueOnce(mockFetchResponse(500, mockErrorJsonResponseData, true, false));
            mathRandomSpy.mockReturnValue(0.5);

            await expect(
                Workspace_retry(mockUrl, mockDefaultOptions, 1, mockBaseDelayMs, mockMaxDelayMs, mockLogger)
            ).rejects.toThrow(
                `API request to ${mockUrl} failed after 1 attempt(s) with status 500: ${JSON.stringify(mockErrorJsonResponseData)}`
            );
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Attempt 1 for ${mockUrl} - Error response body (JSON): ${JSON.stringify(mockErrorJsonResponseData).substring(0, 500)}`));
        });

        test('should correctly parse text error response body if JSON parsing fails', async () => {
            const response = {
                ok: false, status: 500, statusText: 'Internal Server Error',
                json: jest.fn().mockRejectedValueOnce(new Error('Invalid JSON syntax for error response')),
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Attempt 1 for ${mockUrl} - Error response body (Text): ${mockErrorTextResponseData.substring(0, 500)}`));
        });
        ...
        test('should handle failure to parse error response body as JSON or text', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(`Attempt 1 for ${mockUrl} - Failed to read error response body as JSON or text. Error: ${textParseError.message}`);
        });
        ...
        test('should use fallback logger if no logger is provided (via ensureValidLogger mock)', async () => {
            ...
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));
            await Workspace_retry(mockUrl, mockDefaultOptions, 1, mockBaseDelayMs, mockMaxDelayMs, null);

            expect(ensureValidLogger).toHaveBeenCalledWith(null, 'Workspace_retry');
            expect(fallbackLoggerInstance.info).toHaveBeenCalledWith(`Workspace_retry: Initiating request sequence for ${mockUrl} with maxRetries=1, baseDelayMs=${mockBaseDelayMs}, maxDelayMs=${mockMaxDelayMs}.`);
            expect(fallbackLoggerInstance.debug).toHaveBeenCalledWith(`Attempt 1/1 - Fetching GET ${mockUrl}`);
        });
        ...
        test('should use fallback logger if an invalid logger is provided (via ensureValidLogger mock)', async () => {
            ...
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));
            await Workspace_retry(mockUrl, mockDefaultOptions, 1, mockBaseDelayMs, mockMaxDelayMs, invalidLogger);

            expect(ensureValidLogger).toHaveBeenCalledWith(invalidLogger, 'Workspace_retry');
            expect(fallbackLoggerInstance.info).toHaveBeenCalledWith(`Workspace_retry: Initiating request sequence for ${mockUrl} with maxRetries=1, baseDelayMs=${mockBaseDelayMs}, maxDelayMs=${mockMaxDelayMs}.`);
        });

        test('should use actual fallback (console) logger when ensureValidLogger is set to use actual implementation', async () => {
            const actualEnsureValidLogger = jest.requireActual('../../src/utils/loggerUtils.js').ensureValidLogger;
            ensureValidLogger.mockImplementationOnce((logger, prefix) => actualEnsureValidLogger(logger, prefix));
            ...
            expect(consoleSpies.info).toHaveBeenCalledWith('Workspace_retry: ', expectedInitialLogMessage);

            const expectedAttemptLogMessage = `Attempt 1/1 - Fetching GET ${mockUrl}`;
            expect(consoleSpies.debug).toHaveBeenCalledWith('Workspace_retry: ', expectedAttemptLogMessage);
            ...
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));
            await Workspace_retry(mockUrl, mockDefaultOptions, 1, mockBaseDelayMs, mockMaxDelayMs, invalidLogger);
            expect(consoleSpies.warn).toHaveBeenCalledWith('Workspace_retry: ', 'An invalid logger instance was provided. Falling back to console logging with prefix "Workspace_retry".');
        });
        ...
        test('should correctly use provided method in options and log it', async () => {
            ...
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));
            await Workspace_retry(mockUrl, postOptions, 1, mockBaseDelayMs, mockMaxDelayMs, mockLogger);

            expect(fetch).toHaveBeenCalledWith(mockUrl, postOptions);
            expect(mockLogger.debug).toHaveBeenCalledWith(`Attempt 1/1 - Fetching POST ${mockUrl}`);
        });
        ...
        test('should default to GET method for logging if options.method is undefined', async () => {
            ...
            fetch.mockResolvedValueOnce(mockFetchResponse(200, mockSuccessResponseData));
            await Workspace_retry(mockUrl, optionsWithoutMethod, 1, mockBaseDelayMs, mockMaxDelayMs, mockLogger);

            expect(fetch).toHaveBeenCalledWith(mockUrl, optionsWithoutMethod);
            expect(mockLogger.debug).toHaveBeenCalledWith(`Attempt 1/1 - Fetching GET ${mockUrl}`);
        });
        ...
        test('should handle maxRetries = 0: attempts once, fails on error, no retry', async () => {
            ...
            await expect(
                Workspace_retry(mockUrl, mockDefaultOptions, 0, mockBaseDelayMs, mockMaxDelayMs, mockLogger)
            ).rejects.toThrow(
                `API request to ${mockUrl} failed after 1 attempt(s) with status ${nonRetryableStatus}: ${JSON.stringify(mockErrorJsonResponseData)}`
            );
            expect(fetch).toHaveBeenCalledTimes(1);
        ...
        test('should handle maxRetries = 0 with network error: attempts once, fails, no retry', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                `Workspace_retry: Failed for ${mockUrl} after 1 attempt(s) due to persistent network error: ${networkError.message}`,
                {originalErrorName: networkError.name, originalErrorMessage: networkError.message}
            );
        });
    });
});
