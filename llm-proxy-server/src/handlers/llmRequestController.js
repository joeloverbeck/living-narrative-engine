...
import {
    ...
    LOG_LLM_ID_NOT_APPLICABLE, HTTP_HEADER_CONTENT_TYPE
...
export class LlmRequestController {
    ...
    constructor(logger, llmConfigService, apiKeyService, llmRequestService) {
        ...
        if (!llmConfigService) throw new Error("LlmRequestController: llmConfigService is required.");
        if (!apiKeyService) throw new Error("LlmRequestController: apiKeyService is required.");
        if (!llmRequestService) throw new Error("LlmRequestController: llmRequestService is required.");
    ...
    _validateRequest(llmId, targetPayload) {
        if (!llmId || typeof llmId !== 'string') {
            return {
                message: 'Client request validation failed: llmId is required and must be a string.',
                stage: 'request_validation_llmid_missing',
                details: { receivedLlmId: llmId, problem: "llmId is missing or not a string" }
            };
        }
        if (!targetPayload || typeof targetPayload !== 'object') {
            return {
                ...
                details: { llmId, receivedTargetPayloadType: typeof targetPayload, problem: "targetPayload is missing or not an object" }
            };
        }
        return null;
    }
    ...
    async handleLlmRequest(req, res) {
        ...
        this.#logger.info(`LlmRequestController: Received POST request on /api/llm-request from ${req.ip}.`, { payloadSummary: clientPayloadSummary });
        ...
        if (!this.#llmConfigService.isOperational()) {
            ...
            const message = initError?.message || 'Proxy server is not operational due to unknown configuration issues.';
            const stage = initError?.stage || 'initialization_failure_unknown';
            const details = initError?.details || {};
            this.#logger.warn(`LlmRequestController: Proxy not operational. Stage: ${stage}, Message: ${message}`, { details });
            // MODIFIED: Use imported constant
            sendProxyError(res, 503, stage, message, details, LOG_LLM_ID_PROXY_NOT_OPERATIONAL, this.#logger);
            return;
        }
        ...
        if (validationError) {
            this.#logger.warn(`LlmRequestController: Request validation failed. Stage: ${validationError.stage}, Message: ${validationError.message}`, { details: validationError.details, llmId: llmId || LOG_LLM_ID_NOT_APPLICABLE });
            // MODIFIED: Use imported constant
            sendProxyError(res, 400, validationError.stage, validationError.message, validationError.details, llmId || LOG_LLM_ID_REQUEST_VALIDATION_FAILED, this.#logger);
            return;
        }
        ...
        if (!llmModelConfig) {
            ...
            const details = { requestedLlmId: llmId, reason: "No configuration found for this llmId in the proxy's settings." };
            this.#logger.warn(`LlmRequestController: ${message} llmId: '${llmId}'.`, details);
            sendProxyError(res, 400, stage, message, details, llmId, this.#logger);
            return;
        }
        this.#logger.info(`LlmRequestController: Retrieved LLMModelConfig for llmId '${llmId}': DisplayName: ${llmModelConfig.displayName}.`, { llmId });
        ...
        if (requiresKey) {
            this.#logger.info(`LlmRequestController: API key is required for llmId '${llmId}'. Invoking ApiKeyService.`, { llmId });
            const apiKeyResult = await this.#apiKeyService.getApiKey(llmModelConfig, llmId);

            if (apiKeyResult.errorDetails) {
                /** @type {ApiKeyErrorObject} */
                const keyError = apiKeyResult.errorDetails;
                let statusCode = 500;
                if (keyError.stage?.includes('config') || keyError.stage?.includes('file_root_path_missing') || keyError.stage?.includes('sources_missing')) {
                    statusCode = 400;
                } else if (keyError.stage?.includes('not_set_or_empty') || keyError.stage?.includes('file_empty') || keyError.stage?.includes('not_found_or_unreadable')) {
                    statusCode = 500;
                }

                this.#logger.error(`LlmRequestController: ApiKeyService reported an error for llmId '${llmId}'. Status Code for client: ${statusCode}, Stage: ${keyError.stage}, Message: ${keyError.message}`, { details: keyError.details, llmId });
                sendProxyError(
                    res,
                    statusCode,
                    keyError.stage,
            ...
            if (!actualApiKey) {
                ...
                const details = { llmId, reason: "ApiKeyService returned no key and no error for a required key." };
                this.#logger.error(`LlmRequestController: ${message}`, { details, llmId });
                sendProxyError(res, 500, stage, message, details, llmId, this.#logger);
                return;
            }
            this.#logger.info(`LlmRequestController: API key successfully obtained for llmId '${llmId}' from source: ${apiKeySourceForLog}.`, { llmId });
        } else {
            this.#logger.info(`LlmRequestController: LLM '${llmId}' (apiType: ${llmModelConfig.apiType}) does not require a proxy-managed API key.`, { llmId });
            apiKeySourceForLog = "Not applicable (local LLM or no key needed)";
        }
        this.#logger.debug(`LlmRequestController: API Key Status after retrieval for '${llmId}': Source: ${apiKeySourceForLog}, Required: ${requiresKey}, KeyPresent: ${!!actualApiKey}`, { llmId });

        // 5. Forward Request to LLM Provider
        try {
            this.#logger.info(`LlmRequestController: Handing off request for llmId '${llmId}' to LlmRequestService.`, { llmId });
            ...
            if (result.success) {
                this.#logger.info(`LlmRequestController: LlmRequestService returned success for llmId '${llmId}'. Relaying to client with status ${result.statusCode}.`, { llmId });
                res.status(result.statusCode)
                    // MODIFIED: Use imported constant for fallback Content-Type
                    .set(HTTP_HEADER_CONTENT_TYPE, result.contentTypeIfSuccess || CONTENT_TYPE_JSON)
                    .json(result.data);
            } else {
                this.#logger.warn(`LlmRequestController: LlmRequestService returned failure for llmId '${llmId}'. Status: ${result.statusCode}, Stage: ${result.errorStage}, Message: ${result.errorMessage}`, { errorDetails: result.errorDetailsForClient, llmId });

                sendProxyError(
                    res,
                    result.statusCode,
                    result.errorStage || 'llm_service_unknown_error_stage',
                    result.errorMessage || 'An unspecified error occurred in the LLM request service.',
                    result.errorDetailsForClient || { llmId, reason: "LlmRequestService did not provide error details." },
                    llmId,
                    this.#logger
                );
            }
            ...
            this.#logger.error(`LlmRequestController: CRITICAL - LlmRequestService threw an unexpected exception for llmId '${llmId}'. Error: ${serviceException.message}`, {
                details,
