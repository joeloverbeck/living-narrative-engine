 ...
 * }
 */
export async function Workspace_retry(url, options, maxRetries, baseDelayMs, maxDelayMs, logger) {
    ...
    async function attemptFetchRecursive(currentAttempt) {
        ...
        try {
            currentLogger.debug(`Attempt ${currentAttempt}/${maxRetries} - Fetching ${options.method || 'GET'} ${url}`);
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    currentLogger.debug(`Attempt ${currentAttempt} for ${url} - Error response body (JSON): ${errorBodyText.substring(0, 500)}`);
                } catch (e_json) {
                    try {
                        errorBodyText = await response.text();
                        currentLogger.debug(`Attempt ${currentAttempt} for ${url} - Error response body (Text): ${errorBodyText.substring(0, 500)}`);
                    } catch (e_text) {
                        currentLogger.warn(`Attempt ${currentAttempt} for ${url} - Failed to read error response body as JSON or text. Error: ${e_text.message}`);
                    }
                }

                const isRetryableStatusCode = RETRYABLE_HTTP_STATUS_CODES.includes(response.status);

                if (isRetryableStatusCode && currentAttempt < maxRetries) {
                    // MODIFIED: Use helper function _calculateRetryDelay
                    const waitTimeMs = _calculateRetryDelay(currentAttempt, baseDelayMs, maxDelayMs);
                    currentLogger.warn(`Attempt ${currentAttempt}/${maxRetries} for ${url} failed with status ${response.status}. Retrying in ${waitTimeMs}ms... Error body preview: ${errorBodyText.substring(0,100)}`);
                    ...
                    currentLogger.error(`Workspace_retry: ${errorMessage} (Attempt ${currentAttempt}/${maxRetries}, Non-retryable or max retries reached)`);
                    throw new Error(errorMessage);
                }
            }
            currentLogger.debug(`Workspace_retry: Attempt ${currentAttempt}/${maxRetries} for ${url} - Request successful (status ${response.status}). Parsing JSON response.`);
            const responseData = await response.json();
            currentLogger.info(`Workspace_retry: Successfully fetched and parsed JSON from ${url} after ${currentAttempt} attempt(s).`);
            return responseData;
            ...
            if (isNetworkError && currentAttempt < maxRetries) {
                // MODIFIED: Use helper function _calculateRetryDelay
                const waitTimeMs = _calculateRetryDelay(currentAttempt, baseDelayMs, maxDelayMs);
                currentLogger.warn(`Workspace_retry: Attempt ${currentAttempt}/${maxRetries} for ${url} failed with network error: ${error.message}. Retrying in ${waitTimeMs}ms...`);
                await new Promise(resolve => setTimeout(resolve, waitTimeMs));
                return attemptFetchRecursive(currentAttempt + 1);
            } else {
                ...
                if (isNetworkError) {
                    ...
                    currentLogger.error(finalErrorMessage, { originalErrorName: error.name, originalErrorMessage: error.message });
                } else {
                    finalErrorMessage = `Workspace_retry: Failed for ${url} after ${currentAttempt} attempt(s). Unexpected error: ${error.message}`;
                    currentLogger.error(finalErrorMessage, { originalErrorName: error.name, originalErrorMessage: error.message, stack: error.stack });
                }
    ...
    currentLogger.info(`Workspace_retry: Initiating request sequence for ${url} with maxRetries=${maxRetries}, baseDelayMs=${baseDelayMs}, maxDelayMs=${maxDelayMs}.`);
