 ...
 * @param {string} [fallbackMessagePrefix='FallbackLogger'] - A prefix for messages logged by the fallback console logger.
 * @returns {ILogger} A valid logger instance (either the one provided or a console-based fallback).
 */
export function ensureValidLogger(logger, fallbackMessagePrefix = 'FallbackLogger') {
    if (logger &&
    ...
    if (logger) { // Logger was provided but invalid
        fallbackLogger.warn(`An invalid logger instance was provided. Falling back to console logging with prefix "${fallbackMessagePrefix}".`);
    } else { // Logger was not provided at all
