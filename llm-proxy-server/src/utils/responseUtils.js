 ...
 * @param {string} [llmIdForLog=LOG_LLM_ID_NOT_APPLICABLE] - The LLM ID for server-side log.
 * @param {ILogger} logger - An ILogger instance for logging the error.
 */
export function sendProxyError(res, httpStatusCode, stage, errorMessage, details = {}, llmIdForLog = LOG_LLM_ID_NOT_APPLICABLE, logger) {
    ...
    try {
        ...
        if (!res.headersSent) {
            try {
                res.status(500).send('Internal Server Error: Failed to format and send detailed error response.');
                ...
                effectiveLogger.error(
                    ...
                    { lastDitchSendErrorDetails: { name: lastDitchSendError.name, stack: lastDitchSendError.stack }}
                );
            }
        }
    }
