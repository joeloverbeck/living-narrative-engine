...
const llmConfigService = new LlmConfigService(fileSystemReader, proxyLogger, appConfigService);

// Initialize ApiKeyService
const apiKeyService = new ApiKeyService(proxyLogger, fileSystemReader, appConfigService);
...
app.post('/api/llm-request', (req, res) => llmRequestController.handleLlmRequest(req, res));
...
(async () => {
    ...
    app.listen(PORT, () => {
        ...
        if (appConfigService.isProxyPortDefaulted()) {
            proxyLogger.info(`(Note: PROXY_PORT environment variable was not set or invalid, using default.)`);
        }
        ...
        if (resolvedLlmConfigPath) {
            proxyLogger.info(`LLM configurations loaded from: ${resolvedLlmConfigPath}`);
        } else {
        ...
        if (llmConfigService.isOperational()) {
            ...
            proxyLogger.info(`LLM Proxy Server: Successfully loaded ${numLlmConfigs} LLM configurations. Proxy is OPERATIONAL.`);
            ...
            proxyLogger.error(`LLM Proxy Server: CRITICAL - Failed to initialize LLM configurations. Proxy is NOT OPERATIONAL.`);
        ...
        if (proxyAllowedOrigin && proxyAllowedOrigin.trim() !== '') {
            proxyLogger.info(`LLM Proxy Server: CORS enabled for origin(s): ${proxyAllowedOrigin}`);
        } else {
            proxyLogger.info(`LLM Proxy Server: PROXY_ALLOWED_ORIGIN not set, CORS is not specifically configured (default browser policies apply).`);
        }

        // AC 4.e: PROXY_PROJECT_ROOT_PATH_FOR_API_KEY_FILES Status
        const apiKeyFileRootPath = appConfigService.getProxyProjectRootPathForApiKeyFiles();
        if (apiKeyFileRootPath && apiKeyFileRootPath.trim() !== '') {
            proxyLogger.info(`LLM Proxy Server: API Key file root path set to: '${apiKeyFileRootPath}'.`);
        } else {
            if (llmConfigService.isOperational() && llmConfigService.hasFileBasedApiKeys()) {
                proxyLogger.warn(`LLM Proxy Server: WARNING - PROXY_PROJECT_ROOT_PATH_FOR_API_KEY_FILES is NOT SET. File-based API key retrieval WILL FAIL for configured LLMs that use apiKeyFileName.`);
            } else {
                proxyLogger.info(`LLM Proxy Server: PROXY_PROJECT_ROOT_PATH_FOR_API_KEY_FILES is not set (this may be fine if no LLMs use file-based API keys or if config failed to load).`);
            }
        }
    ...
    proxyLogger.error('LLM Proxy Server: A critical error occurred during asynchronous server startup sequence PRIOR to app.listen.', error);
    ...
    proxyLogger.error('LLM Proxy Server: CRITICAL - Proxy will NOT be operational due to a severe error during startup initialization steps.');
...
app.use((err, req, res, next) => {
    proxyLogger.error(
        "Global Error Handler: Unhandled error caught!",
        {
