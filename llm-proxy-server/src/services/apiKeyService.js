...
export class ApiKeyService {
    ...
    _createErrorDetails(message, stage, detailsContext, originalError = null) {
        ...
        if (originalError && originalError.message && !detailsContext.originalErrorMessage) {
        ...
        this.#logger.warn(
            ...
            { details: detailsContext, originalError: originalError ? { message: originalError.message, name: originalError.name, stack: originalError.stack } : undefined }
        );
    ...
    isApiKeyRequired(llmModelConfig) {
        if (!llmModelConfig || !llmModelConfig.apiType) {
            this.#logger.debug('ApiKeyService.isApiKeyRequired: llmModelConfig or apiType is missing, assuming key not required.');
            return false;
        }
        const apiTypeLower = llmModelConfig.apiType.toLowerCase();
        // MODIFIED: Use imported constant
        const isLocalType = LOCAL_API_TYPES_REQUIRING_NO_PROXY_KEY.includes(apiTypeLower);

        this.#logger.debug(`ApiKeyService.isApiKeyRequired: For apiType='${llmModelConfig.apiType}', isLocalType=${isLocalType}. Key required: ${!isLocalType}`);
        return !isLocalType;
    }
    ...
    async _readApiKeyFromFile(fileName, projectRootPath, llmId) {
        ...
        if (safeFileName !== fileName) {
            this.#logger.warn(`ApiKeyService._readApiKeyFromFile: Original fileName '${fileName}' for llmId '${llmId}' was normalized to '${safeFileName}'. Ensure the provided fileName is just the file's name.`);
        }

        const fullPath = path.join(projectRootPath, safeFileName);
        this.#logger.debug(`ApiKeyService._readApiKeyFromFile: Attempting to read API key from '${fullPath}' for llmId '${llmId}'.`);

        try {
            // MODIFIED: Use imported constant for encoding
            const apiKey = await this.#fileSystemReader.readFile(fullPath, DEFAULT_ENCODING_UTF8);
            const trimmedKey = apiKey.trim();

            if (trimmedKey === '') {
                this.#logger.warn(`ApiKeyService._readApiKeyFromFile: API key file '${fullPath}' for llmId '${llmId}' is empty or contains only whitespace.`);
                return { key: null, error: this._createErrorDetails(
                        `API key file is empty or contains only whitespace.`,
                        'api_key_file_empty',
                        { llmId, attemptedFile: fullPath, reason: "File is empty or whitespace only." }
                    )};
            }

            this.#logger.info(`ApiKeyService._readApiKeyFromFile: Successfully retrieved API key for llmId '${llmId}' from '${fullPath}'.`);
            return { key: trimmedKey, error: null };

        } catch (error) {
            if (error.code === 'ENOENT' || error.code === 'EACCES' || error.code === 'EPERM') {
                const message = `API key file not found or not readable.`;
                this.#logger.warn(`ApiKeyService._readApiKeyFromFile: ${message} Path: '${fullPath}', Code: ${error.code}, for llmId '${llmId}'.`);
                return { key: null, error: this._createErrorDetails(
                        message,
                        'api_key_file_not_found_or_unreadable',
                        { llmId, attemptedFile: fullPath, reason: `File system error code: ${error.code}` },
                        error
                    )};
            } else {
                const message = `Unexpected error reading API key file.`;
                this.#logger.error(`ApiKeyService._readApiKeyFromFile: ${message} Path: '${fullPath}', for llmId '${llmId}'. Error: ${error.message}`, {
                ...
                return { key: null, error: this._createErrorDetails(
                        ...
                        { llmId, attemptedFile: fullPath, reason: "Unexpected file system/read error." },
                        error
                    )};
            }
        }
    ...
    async getApiKey(llmModelConfig, llmId) {
        if (!this.isApiKeyRequired(llmModelConfig)) {
            this.#logger.info(`ApiKeyService.getApiKey: API key not required for llmId '${llmId}' (apiType: ${llmModelConfig?.apiType}).`);
            return { apiKey: null, errorDetails: null, source: "Not applicable (local LLM or no key needed)" };
        }

        this.#logger.info(`ApiKeyService.getApiKey: Attempting API key retrieval for cloud service llmId '${llmId}'.`);
        ...
        if (envVarName) {
            this.#logger.debug(`ApiKeyService.getApiKey: Checking environment variable '${envVarName}' for llmId '${llmId}'.`);
            const envValue = process.env[envVarName];
            if (envValue && envValue.trim() !== '') {
                actualApiKey = envValue.trim();
                apiKeySource = `environment variable '${envVarName}'`;
                this.#logger.info(`ApiKeyService.getApiKey: Successfully retrieved API key for llmId '${llmId}' from ${apiKeySource}.`);
            } else {
                this.#logger.warn(`ApiKeyService.getApiKey: Environment variable '${envVarName}' for llmId '${llmId}' is not set or is empty.`);
        ...
        if (!actualApiKey && fileName) {
            this.#logger.debug(`ApiKeyService.getApiKey: Attempting to retrieve key from file '${fileName}' for llmId '${llmId}' (env var '${envVarName || 'N/A'}' failed or not configured).`);
            const projectRootPath = this.#appConfigService.getProxyProjectRootPathForApiKeyFiles();
            ...
            if (!projectRootPath || projectRootPath.trim() === '') {
                ...
                this.#logger.error(`ApiKeyService.getApiKey: ${message} For llmId '${llmId}', attempted file '${fileName}'.`);
                finalErrorDetails = this._createErrorDetails(
                    message,
                    'api_key_retrieval_file_root_path_missing',
                    { llmId, attemptedFile: fileName, reason: "PROXY_PROJECT_ROOT_PATH_FOR_API_KEY_FILES environment variable not set." }
                );
                // This is a terminal error for file-based retrieval path.
            } else {
                const fileReadResult = await this._readApiKeyFromFile(fileName, projectRootPath, llmId);
        ...
        if (noEnvVarConfigured && noFileConfigured) {
            ...
            finalErrorDetails = this._createErrorDetails(
                ...
                { llmId, reason: "LLM configuration for this cloud service does not specify how to obtain an API key." }
            );
            ...
            if (!finalErrorDetails) {
                finalErrorDetails = this._createErrorDetails(
                    ...
                    { llmId, attemptedFile: fileName, reason: "File retrieval failed (specific reason might be in earlier logs from _readApiKeyFromFile)." }
                );
            }
            ...
            finalErrorDetails = this._createErrorDetails(
                ...
                { llmId, attemptedEnvVar: envVarName, attemptedFile: fileName, reason: combinedReason, originalErrorMessage: finalErrorDetails?.details?.originalErrorMessage }
            );
            ...
            this.#logger.error(`ApiKeyService.getApiKey: Reached unexpected state for llmId '${llmId}'. API key is null, but no specific error was set. This should not happen.`);
            finalErrorDetails = this._createErrorDetails(
                `API key for LLM could not be retrieved due to an unknown internal error.`,
                'api_key_retrieval_unknown_error',
                { llmId, attemptedEnvVar: envVarName || 'N/A', attemptedFile: fileName || 'N/A' }
            );
        }
