...
* **API Key Variables (e.g., `OPENROUTER_API_KEY_ENV_VAR_PLACEHOLDER`, `ANTHROPIC_API_KEY_ENV_VAR_PLACEHOLDER`)**: The
  proxy server needs access to the *actual* API keys for the LLM providers it will forward requests to.
    ...
    * **Important**: The names in `.env.example` like `OPENROUTER_API_KEY_ENV_VAR_PLACEHOLDER` are placeholders for the
      *actual API key values* you need to set in your `.env` file. The proxy will use the string value of `apiKeyEnvVar`
      from its `llm-configs.json` as the *name* of the environment variable to look up.
...
2. It primarily uses the following fields from the matched LLM configuration:
    ...
    * `apiKeyEnvVar`: If present, this string is used as the *name* of an environment variable that the proxy server
    ...
    * `apiKeyFileName`: If `apiKeyEnvVar` is not used or fails, and `apiKeyFileName` is present, this string is used as
      the *name* of a file (e.g., `my_api_key.txt`). The proxy will attempt to read this file from the directory
