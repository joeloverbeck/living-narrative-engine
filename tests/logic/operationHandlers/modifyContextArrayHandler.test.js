...
describe('ModifyContextArrayHandler', () => {
  ...
  describe('constructor', () => {
    it('should throw an error if logger is not provided', () => {
      expect(
        () => new ModifyContextArrayHandler({ safeEventDispatcher: mockSafeEventDispatcher })
      ).toThrow("Dependency 'ILogger' with a 'warn' method is required.");
    });

    it('should throw an error if logger does not have a warn method', () => {
      expect(
        () => new ModifyContextArrayHandler({ logger: {}, safeEventDispatcher: mockSafeEventDispatcher })
      ).toThrow("Dependency 'ILogger' with a 'warn' method is required.");
    });

    it('should throw an error if safeEventDispatcher is not provided', () => {
      expect(() => new ModifyContextArrayHandler({ logger: mockLogger })).toThrow(
    ...
    it('should throw an error if safeEventDispatcher does not have a dispatch method', () => {
      expect(
        () => new ModifyContextArrayHandler({ logger: mockLogger, safeEventDispatcher: {} })
  ...
  describe('execute', () => {
    it('should log a warning if params is not an object', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'MODIFY_CONTEXT_ARRAY: params missing or invalid.', { params: null }
      );
    });
    ...
    it('should log a warning if executionContext or evaluationContext.context is missing', () => {
      ...
      handler.execute({ variable_path: 'myArray', mode: 'push', value: 4 }, { evaluationContext: {} });
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'MODIFY_CONTEXT_ARRAY: Cannot execute because the execution context is missing.'
      );
    });
    ...
    describe('when variable_path resolution is problematic', () => {
      it('should log a warning if path exists but is NOT an array', () => {
        ...
        handler.execute({ variable_path: 'notAnArray', mode: 'push', value: 4 }, executionContext);
        expect(mockLogger.warn).toHaveBeenCalledWith(
      ...
      it('should log a warning if path does not exist AND mode is not push/push_unique (e.g., pop)', () => {
        handler.execute({ variable_path: 'nonExistentPath', mode: 'pop' }, executionContext);
        expect(mockLogger.warn).toHaveBeenCalledWith(
      ...
      it('initializes and pushes if path is new and mode is push', () => {
        handler.execute({ variable_path: 'newArrayForPush', mode: 'push', value: 1 }, executionContext);
        expect(mockLogger.warn).not.toHaveBeenCalled();
        expect(executionContext.evaluationContext.context.newArrayForPush).toEqual([1]);
      });

      it('initializes and push_uniques if path is new and mode is push_unique', () => {
        handler.execute({ variable_path: 'newArrayForPushUnique', mode: 'push_unique', value: 1 }, executionContext);
        expect(mockLogger.warn).not.toHaveBeenCalled();
        expect(executionContext.evaluationContext.context.newArrayForPushUnique).toEqual([1]);
      });
    });

    it('should log a warning for unknown mode', () => {
      handler.execute({ variable_path: 'myArray', mode: 'unknown_mode' }, executionContext);
      expect(mockLogger.warn).toHaveBeenCalledWith(
    ...
    describe('mode: push', () => {
      it('should log a warning if value is missing for push mode', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push' }, executionContext);
        expect(mockLogger.warn).toHaveBeenCalledWith(
      ...
      it('should push a value to the array', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push', value: 4 }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3, 4]);
      });

      it('should push a value to a nested array', () => {
        handler.execute({ variable_path: 'nested.array', mode: 'push', value: 'c' }, executionContext);
        expect(executionContext.evaluationContext.context.nested.array).toEqual(['a', 'b', 'c']);
      });

      it('should store the modified array in result_variable if provided', () => {
        handler.execute(
          { variable_path: 'myArray', mode: 'push', value: 4, result_variable: 'result' },
          executionContext
        );
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3, 4]);
        expect(executionContext.evaluationContext.context.result).toEqual([1, 2, 3, 4]);
      });
    });

    describe('mode: push_unique', () => {
      it('should log a warning if value is missing for push_unique mode', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push_unique' }, executionContext);
        expect(mockLogger.warn).toHaveBeenCalledWith(
      ...
      it('should push a primitive value if it does not exist', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push_unique', value: 4 }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3, 4]);
      });

      it('should not push a primitive value if it already exists', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push_unique', value: 2 }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3]);
      });

      it('should push an object value if it does not exist (by deep comparison)', () => {
        const obj = { id: 1 };
        const newObj = { id: 2 };
        executionContext.evaluationContext.context.objectArray = [cloneDeep(obj)];
        handler.execute(
          { variable_path: 'objectArray', mode: 'push_unique', value: newObj },
          executionContext
        );
        expect(executionContext.evaluationContext.context.objectArray).toEqual([obj, newObj]);
      });
      ...
      it('should not push an object value if it already exists (by deep comparison)', () => {
        ...
        executionContext.evaluationContext.context.objectArray = [cloneDeep(obj1)];
        handler.execute(
          { variable_path: 'objectArray', mode: 'push_unique', value: obj2 },
          executionContext
        );
        expect(executionContext.evaluationContext.context.objectArray).toEqual([obj1]);
      });
       it('should push a null value if it does not exist', () => {
        handler.execute({ variable_path: 'myArray', mode: 'push_unique', value: null }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3, null]);
      });
      ...
      it('should not push a null value if it already exists', () => {
        ...
        expect(executionContext.evaluationContext.context.myArray.filter(v => v === null).length).toBe(1);
        expect(executionContext.evaluationContext.context.myArray).toContain(null);
      });


      it('should store the modified array in result_variable if provided', () => {
        handler.execute(
          { variable_path: 'myArray', mode: 'push_unique', value: 4, result_variable: 'result' },
          executionContext
        );
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3, 4]);
        expect(executionContext.evaluationContext.context.result).toEqual([1, 2, 3, 4]);
    ...
    describe('mode: pop', () => {
      it('should pop a value from the array', () => {
        handler.execute({ variable_path: 'myArray', mode: 'pop' }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2]);
      });

      it('should do nothing if array is empty', () => {
        handler.execute({ variable_path: 'emptyArray', mode: 'pop' }, executionContext);
      ...
      it('should store the popped value in result_variable if provided', () => {
        handler.execute(
          { variable_path: 'myArray', mode: 'pop', result_variable: 'poppedItem' },
          executionContext
        );
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2]);
       ...
       it('should store undefined in result_variable if array is empty and popped', () => {
        handler.execute(
          { variable_path: 'emptyArray', mode: 'pop', result_variable: 'poppedItem' },
          executionContext
        );
        expect(executionContext.evaluationContext.context.emptyArray).toEqual([]);
        expect(executionContext.evaluationContext.context.poppedItem).toBeUndefined();
      });
    });

    describe('mode: remove_by_value', () => {
      it('should log a warning if value is missing for remove_by_value mode', () => {
        handler.execute({ variable_path: 'myArray', mode: 'remove_by_value' }, executionContext);
        expect(mockLogger.warn).toHaveBeenCalledWith(
      ...
      it('should remove a primitive value from the array', () => {
        handler.execute({ variable_path: 'myArray', mode: 'remove_by_value', value: 2 }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 3]);
      });
      ...
      it('should remove an object value from the array (by deep comparison)', () => {
        ...
        executionContext.evaluationContext.context.objectArray = [cloneDeep(obj1), cloneDeep(obj2)];
        handler.execute(
          { variable_path: 'objectArray', mode: 'remove_by_value', value: objToRemove },
          executionContext
        );
        expect(executionContext.evaluationContext.context.objectArray).toEqual([obj2]);
      });

      it('should do nothing if primitive value is not found', () => {
        handler.execute({ variable_path: 'myArray', mode: 'remove_by_value', value: 5 }, executionContext);
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 2, 3]);
      });
      ...
      it('should do nothing if object value is not found', () => {
        ...
        handler.execute(
          { variable_path: 'objectArray', mode: 'remove_by_value', value: objToMiss },
          executionContext
        );
        expect(executionContext.evaluationContext.context.objectArray).toEqual([obj1, obj2]);
      });

      it('should store the modified array in result_variable if provided', () => {
        handler.execute(
          { variable_path: 'myArray', mode: 'remove_by_value', value: 2, result_variable: 'result' },
          executionContext
        );
        expect(executionContext.evaluationContext.context.myArray).toEqual([1, 3]);
        expect(executionContext.evaluationContext.context.result).toEqual([1, 3]);
      });
    });

    it('should not modify the original array in the rule definition (cloning check)', () => {
      const originalContext = cloneDeep(executionContext.evaluationContext.context);
      handler.execute({ variable_path: 'myArray', mode: 'push', value: 4 }, executionContext);
      ...
      const arrayBeforeModification = [...executionContext.evaluationContext.context.myArray];

      handler.execute({ variable_path: 'myArray', mode: 'push', value: 30 }, executionContext);
      ...
      expect(executionContext.evaluationContext.context.myArray).toEqual([10, 20, 30]);
    });

    it('should throw error for unsafe property __proto__ in setPath', () => {
        expect(() => {
            handler.execute({ variable_path: 'myArray.__proto__.polluted', mode: 'push', value: 'test'}, executionContext)
        }).toThrow('Unsafe property name detected: __proto__');
    });

    it('should throw error for unsafe property constructor in setPath', () => {
        expect(() => {
            handler.execute({ variable_path: 'myArray.constructor.polluted', mode: 'push', value: 'test'}, executionContext)
    ...
    it('should correctly create nested objects if they do not exist during setPath', () => {
      handler.execute({ variable_path: 'newly.nested.array', mode: 'push', value: 'hello' }, executionContext);
      expect(executionContext.evaluationContext.context.newly.nested.array).toEqual(['hello']);
    });
  });
}); 