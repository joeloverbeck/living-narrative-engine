...
const getMockContext = (overrides = {}) => {
  ...
  const merged = {
    ...
    evaluationContext: {
      ...
      context: {
        // Ensure context object is also copied/merged
  ...
  if (overrides.evaluationContext) {
    merged.evaluationContext = {
      ...
      context: {
        // Ensure context within overridden evaluationContext is merged correctly
        ...(base.evaluationContext?.context || {}),
...
describe('QueryComponentHandler', () => {
  ...
  test('constructor should throw if EntityManager is missing or invalid', () => {
    expect(() => new QueryComponentHandler({ logger: mockLogger })).toThrow(
      /EntityManager/
    );
    expect(
      () => new QueryComponentHandler({ entityManager: {}, logger: mockLogger })
    ).toThrow(/getComponentData/);
    expect(
      () =>
        new QueryComponentHandler({
          entityManager: { getComponentData: 'not-a-func' },
          logger: mockLogger,
        })
    ).toThrow(/getComponentData/);
  });
  ...
  test('execute should correctly resolve entity_ref: { entityId: "..." }', () => {
    const params = {
      entity_ref: { entityId: mockSpecificId },
      component_type: 'game:inventory',
      result_variable: 'invData',
    };
    const context = getMockContext();
  ...
  test('execute should correctly resolve entity_ref: { entityId: " with_spaces " }', () => {
    ...
    const params = {
      entity_ref: { entityId: idWithSpaces },
      component_type: 'game:inventory',
      result_variable: 'invData',
    };
    const context = getMockContext();
  ...
  test('execute should interpret non-keyword string entity_ref as direct ID', () => {
    const params = {
      entity_ref: mockSpecificId,
      component_type: 'core:tag',
      result_variable: 'tags',
    };
    const context = getMockContext();
  ...
  test('execute should log error and return if entity_ref: "target" cannot be resolved', () => {
    ...
    const context = getMockContext({
      evaluationContext: { target: { id: null } },
    }); // Target ID missing
    handler.execute(params, context);
    expect(mockLogger.error).toHaveBeenCalledWith(
  ...
  test('execute should log error and return if component_type is missing or invalid', () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('"component_type" parameter'),
      expect.anything()
    );
    expect(mockEntityManager.getComponentData).not.toHaveBeenCalled();
    mockLogger.error.mockClear();
    ...
    handler.execute(
      { entity_ref: 'actor', component_type: '  ', result_variable: 'res' },
      context
    );
    expect(mockLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('"component_type" parameter'),
  ...
  test('execute should log error and return if result_variable is missing or invalid', () => {
    const context = getMockContext();
    handler.execute(
      { entity_ref: 'actor', component_type: 'core:test' },
      context
    );
    expect(mockLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('"result_variable" parameter'),
      expect.anything()
    );
    expect(mockEntityManager.getComponentData).not.toHaveBeenCalled();
    mockLogger.error.mockClear();
  ...
  test('execute should log error and return if evaluationContext.context is missing', () => {
    ...
    handler.execute(params, context);
    expect(mockLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('evaluationContext.context is missing'),
      expect.anything()
    );
  ...
  test('execute should call entityManager.getComponentData and store object result', () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining('Attempting to query component')
    );
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `Successfully queried component "ns:comp" from entity "${mockActorId}". Result stored in "compData": ${JSON.stringify(mockResult)}`
      )
    );
  });
  ...
  test('execute should call entityManager.getComponentData and store null result', () => {
    const params = {
      entity_ref: 'actor',
      component_type: 'ns:optionalComp',
      result_variable: 'maybeComp',
    };
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `Successfully queried component "ns:optionalComp" from entity "${mockActorId}". Result stored in "maybeComp": null`
      )
    );
  });
  ...
  test('execute should call entityManager.getComponentData and store undefined result (component not found)', () => {
    const params = {
      entity_ref: 'target',
      component_type: 'ns:nonexistent',
      result_variable: 'notFoundComp',
    };
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `Component "ns:nonexistent" not found on entity "${mockTargetId}". Stored 'undefined' in "notFoundComp".`
      )
    );
  });
  ...
  test('execute should trim component_type and result_variable', () => {
    const params = {
      entity_ref: 'actor',
      component_type: '  padded:comp  ',
      result_variable: '  paddedVar  ',
    };
    ...
    expect(context.logger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `Storing result in context variable "${trimmedResultVar}"`
      )
    );
    ...
    expect(context.logger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `Result stored in "${trimmedResultVar}": ${JSON.stringify(mockResult)}`
      )
    );
  ...
  test('execute should catch and log errors from entityManager.getComponentData', () => {
    ...
    const params = {
      entity_ref: 'actor',
      component_type: 'ns:problematic',
      result_variable: 'problem',
    };
    const context = getMockContext();
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        'Stored \'undefined\' in "problem" due to EntityManager error.'
      )
    );
  });
  ...
  test('execute uses logger from execution context if available', () => {
    ...
    handler.execute(
      { entity_ref: 'actor' /* missing other params */ },
      context
    );
    ...
    expect(specificLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining('Successfully queried component')
    );
