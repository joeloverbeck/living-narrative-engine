  ...
  describe('Comparison Operators (==, !=, <, >, <=, >=)', () => {
    ...
    describe('Operator: ==', () => {
      ...
      it('should compare booleans correctly', () => {
        expect(service.evaluate({'==': [true, true]}, mockContext)).toBe(true);
        expect(service.evaluate({'==': [false, false]}, mockContext)).toBe(true);
        expect(service.evaluate({'==': [true, false]}, mockContext)).toBe(false);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });

      it('should compare with null (only null == null)', () => {
        expect(service.evaluate({'==': [null, null]}, mockContext)).toBe(true);
        expect(service.evaluate({'==': [0, null]}, mockContext)).toBe(false);
        expect(service.evaluate({'==': ['', null]}, mockContext)).toBe(false);
        expect(service.evaluate({'==': [false, null]}, mockContext)).toBe(false); // json-logic specific
        expect(service.evaluate({'==': [true, null]}, mockContext)).toBe(false);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      it('should handle type coercion like json-logic-js (loose equality)', () => {
        ...
        expect(service.evaluate({'==': [null, false]}, mockContext)).toBe(false);
    ...
    describe('Operator: !=', () => {
      ...
      it('should compare booleans correctly', () => {
        expect(service.evaluate({'!=': [true, false]}, mockContext)).toBe(true);
        expect(service.evaluate({'!=': [true, true]}, mockContext)).toBe(false);
        expect(service.evaluate({'!=': [false, false]}, mockContext)).toBe(false);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      it('should compare with null (anything non-null != null)', () => {
        ...
        expect(service.evaluate({'!=': [false, null]}, mockContext)).toBe(true); // json-logic specific
        expect(service.evaluate({'!=': ['a', null]}, mockContext)).toBe(true);
        expect(service.evaluate({'!=': [null, null]}, mockContext)).toBe(false);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      it('should handle type coercion like json-logic-js (loose inequality)', () => {
        ...
        expect(service.evaluate({'!=': [null, false]}, mockContext)).toBe(true);
    ...
    describe('Operator: <', () => {
      ...
      it('should handle type coercion (numeric comparison)', () => {
        ...
        expect(service.evaluate({'<': [false, true]}, mockContext)).toBe(true);   // 0 < 1
        expect(service.evaluate({'<': [true, false]}, mockContext)).toBe(false);  // 1 < 0
        ...
        expect(service.evaluate({'<': [null, null]}, mockContext)).toBe(false);   // 0 < 0
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
    ...
    describe('Operator: >', () => {
      ...
      it('should handle type coercion (numeric comparison)', () => {
        ...
        expect(service.evaluate({'>': [0, 'a']}, mockContext)).toBe(false); // 0 > NaN is false
        // Boolean coercion (true=1, false=0)
        expect(service.evaluate({'>': [true, false]}, mockContext)).toBe(true);   // 1 > 0
        expect(service.evaluate({'>': [false, true]}, mockContext)).toBe(false);  // 0 > 1
    ...
    describe('Operator: <=', () => {
      ...
      it('should handle type coercion (numeric comparison)', () => {
        ...
        // Boolean coercion (true=1, false=0)
        expect(service.evaluate({'<=': [false, true]}, mockContext)).toBe(true);   // 0 <= 1
        expect(service.evaluate({'<=': [true, true]}, mockContext)).toBe(true);   // 1 <= 1
        expect(service.evaluate({'<=': [false, false]}, mockContext)).toBe(true);  // 0 <= 0
        expect(service.evaluate({'<=': [true, false]}, mockContext)).toBe(false);  // 1 <= 0
        // Null coercion (null=0)
        expect(service.evaluate({'<=': [null, 1]}, mockContext)).toBe(true);     // 0 <= 1
        expect(service.evaluate({'<=': [null, 0]}, mockContext)).toBe(true);     // 0 <= 0
        expect(service.evaluate({'<=': [-1, null]}, mockContext)).toBe(true);    // -1 <= 0
        expect(service.evaluate({'<=': [null, null]}, mockContext)).toBe(true);    // 0 <= 0
