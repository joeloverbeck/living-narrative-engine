...
describe('SystemLogicInterpreter - Integration Test - Placeholder Resolution', () => {
  ...
  beforeEach(() => {
    ...
    mockEntityManager = {
      getEntityInstance: jest.fn((entityId) => {
        if (entityId === MOCK_PLAYER_ENTITY_FULL.id)
          return MOCK_PLAYER_ENTITY_FULL;
        return undefined;
      }),
  ...
  it('should resolve a placeholder pointing to an array in the event payload and pass the array to the operation handler', () => {
    mockDataRegistry.getAllSystemRules.mockReturnValue([
      MOCK_RULE_PLACEHOLDER_RESOLUTION,
    ]);
    ...
    const unresolvedWarning = mockLogger.loggedMessages.find(
      (log) =>
        log.level === 'warn' &&
        log.message.includes(
          'Placeholder path "event.payload.sourceData" from {event.payload.sourceData} could not be resolved'
        )
    );
    expect(unresolvedWarning).toBeUndefined();

    const validationErrorLog = mockLogger.loggedMessages.find(
      (log) =>
        log.level === 'error' &&
