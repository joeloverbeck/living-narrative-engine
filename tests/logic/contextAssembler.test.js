...
describe('createJsonLogicContext (contextAssembler.js)', () => {
  ...
  describe('AC2: Event Population', () => {
    ...
    test('should populate event.payload as null if input payload is null/undefined', () => {
      ...
      const eventWithUndefined = {
        type: 'TEST_UNDEF_PAYLOAD',
        payload: undefined,
      };
      const contextUndef = createJsonLogicContext(
        eventWithUndefined,
        null,
        null,
        mockEntityManager,
  ...
  describe('AC6: Actor Population (Invalid ID Type)', () => {
    ...
    test.each([
      ...
      'should set actor to null and log warning for invalid actorId type: %p',
      (invalidId) => {
        // Arrange
        mockEntityManager.getEntityInstance.mockImplementation((id) => {
          if (id === targetId) return mockTargetEntity;
        ...
        if (invalidId) {
          // Only truthy invalid types should log warning
          expect(mockLogger.warn).toHaveBeenCalledWith(
            expect.stringContaining(
              `Invalid actorId type provided: [${typeof invalidId}]`
            )
          );
          expect(mockLogger.warn).toHaveBeenCalledTimes(1);
        } else {
          // Falsy invalid types (like false) should not log
  ...
  describe('AC7: Target Population (Invalid ID Type)', () => {
    ...
    test.each([
      ...
      'should set target to null and log warning for invalid targetId type: %p',
      (invalidId) => {
        ...
        if (invalidId) {
          // typeof invalidId === 'boolean' && invalidId === false will be falsy
          expect(mockLogger.warn).toHaveBeenCalledWith(
            expect.stringContaining(
              `Invalid targetId type provided: [${typeof invalidId}]`
            )
          );
