...
describe('JsonLogicEvaluationService - Actor Component Access Tests ([PARENT_ID].6)', () => {
  ...
  describe('Actor Component Access (actor.components.*)', () => {
    ...
    describe('Component Existence Checks', () => {
      const componentExistsRule = {'!!': {'var': `actor.components.${componentId}`}};
      const componentIsNullRule = {'==': [{'var': `actor.components.${componentId}`}, null]};
      const componentIsNotNullRule = {'!=': [{'var': `actor.components.${componentId}`}, null]};
      ...
      test('{"!!": "actor.components.health"} should return true when component exists', () => {
        ...
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, componentId);
    ...
    describe('Property Access (Component Exists)', () => {
      const propertyValueRule = (value) => ({'==': [{'var': `actor.components.${componentId}.${propertyId}`}, value]});
      const missingPropertyIsNullRule = {'==': [{'var': `actor.components.${componentId}.${missingPropertyId}`}, null]};
      const nestedPropertyValueRule = (value) => ({'==': [{'var': `actor.components.${statsComponentId}.${nestedPropertyPath}`}, value]});
      const nestedMissingPropertyIsNullRule = {'==': [{'var': `actor.components.${statsComponentId}.attributes.missingNested`}, null]};
      const nestedMissingMiddlePropertyIsNullRule = {'==': [{'var': `actor.components.${statsComponentId}.missingMiddle.strength`}, null]};
    ...
    describe('Property Access (Component Missing)', () => {
      const missingComponentPropertyIsNullRule = {'==': [{'var': `actor.components.${missingComponentId}.someProp`}, null]};
      ...
      test('{"==": ["actor.components.missingComp.someProp", null]} should return true when component is missing', () => {
        ...
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === actorId && compId === missingComponentId ? undefined : undefined // Explicit undefined
        );
    ...
    describe('Namespaced Component Access', () => {
      const namespacedPropValueRule = (value) => ({'==': [{'var': `actor.components.${namespacedComponentId}.level`}, value]});
      const namespacedMissingPropIsNullRule = {'==': [{'var': `actor.components.${namespacedComponentId}.missingLevel`}, null]};
      const namespacedCompMissingIsNullRule = {'==': [{'var': `actor.components.${namespacedComponentId}`}, null]};
      ...
      test('{"==": ["actor.components.ns:stats.level", 5]} should return true when namespaced component and property exist', () => {
        ...
        mockEntityManager.getEntityInstance.mockReturnValue(mockActor);
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === actorId && compId === namespacedComponentId ? nsComponentData : undefined
        );
        ...
        expect(result).toBe(true);
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(actorId, namespacedComponentId);
        expect(mockLogger.error).not.toHaveBeenCalled();
      });
      ...
      test('{"==": ["actor.components.ns:stats", null]} should return true when namespaced component itself is missing', () => {
        ...
        mockEntityManager.getComponentData.mockImplementation((id, compId) =>
          id === actorId && compId === namespacedComponentId ? undefined : undefined
        );
    ...
    describe('Actor Missing', () => {
      const anyComponentPathRule = {'==': [{'var': `actor.components.${componentId}.${propertyId}`}, null]};
      const anyComponentExistenceRule = {'!!': {'var': `actor.components.${componentId}`}};
      ...
      test('accessing any component path should resolve to null when actor entity is not found', () => {
        ...
        mockEntityManager.getEntityInstance.mockImplementation((id) =>
          id === missingActorId ? undefined : mockActor // Ensure it only fails for 'ghost'
        );

        const context = createJsonLogicContext(baseEvent, missingActorId, null, mockEntityManager, mockLogger);
        const result = service.evaluate(anyComponentPathRule, context);

        expect(result).toBe(true); // Because context assembler sets actor to null, path resolves to null
        expect(mockEntityManager.getEntityInstance).toHaveBeenCalledWith(missingActorId);
