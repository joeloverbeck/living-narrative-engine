...
const MOCK_RULE_A_ENEMY_DEATH = {
  ...
  condition: {
    // Condition: Target HP must be 0 or less
    '<=': [{ var: 'target.components.health.current' }, 0],
  },
  actions: [
    {
      type: 'TEST_DISPATCH_EVENT',
      parameters: { eventName: 'dispatch_enemy_death' },
    },
  ], // Mock action
};
...
const MOCK_RULE_B_HIT_SFX = {
  ...
  event_type: 'EnemyDamaged', // Triggered by the same event
  condition: {
    // Condition: Damage dealt must be greater than 0
    '>': [{ var: 'event.payload.damage' }, 0],
  },
  actions: [
    { type: 'TEST_PLAY_SOUND', parameters: { soundName: 'play_hit_sfx' } },
  ], // Mock action
};
...
const MOCK_ATTACKER = {
  // Needed as context for the event
...
const MOCK_EVENT_ENEMY_DAMAGED_HP5 = {
  type: 'EnemyDamaged',
  payload: {
    actorId: MOCK_ATTACKER.id,
    targetId: MOCK_TARGET_ENEMY_HP_5.id,
    damage: 10,
  }, // Damage > 0
};
...
const MOCK_EVENT_ENEMY_DAMAGED_HP0 = {
  ...
  payload: {
    actorId: MOCK_ATTACKER.id,
    targetId: MOCK_TARGET_ENEMY_HP_0.id,
    damage: 15,
  }, // Damage > 0
};
  ...
  beforeEach(() => {
    ...
    mockEntityManager = {
      getEntityInstance: jest.fn().mockImplementation((entityId) => {
        ...
        if (entityId === MOCK_TARGET_ENEMY_HP_5.id)
          return MOCK_TARGET_ENEMY_HP_5;
        if (entityId === MOCK_TARGET_ENEMY_HP_0.id)
          return MOCK_TARGET_ENEMY_HP_0;
        return undefined;
      }),
  ...
  it('TC1: should execute only Rule B (play_hit_sfx) when target HP is 5', () => {
    ...
    mockJsonLogicEvaluationService.evaluate.mockImplementation(
      (condition, context) => {
        ...
        if (
          condition?.['<=']?.[0]?.var === 'target.components.health.current'
        ) {
    ...
    const skipLogForRuleA = mockLogger.loggedMessages.find(
      (log) =>
        ...
        log.message.includes('evaluating to false')
    );
