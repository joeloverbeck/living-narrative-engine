  ...
  beforeEach(() => {
    ...
    mockWorldContext = {
      getLocationOfEntity: jest
        .fn()
        .mockReturnValue({ id: 'mockRoomForInvContext' }), // Needed for _buildMinimalContextForScopes
  ...
  describe('Sub-Ticket/Test Case 4.3: Inventory - Item ID is Invalid', () => {
    test('Tests skipping of invalid item IDs in the inventory list.', async () => {
      ...
      mockActorEntity.addComponent(INVENTORY_COMPONENT_ID, {
        items: [null, 'itemValid', ''],
      }); // Invalid IDs mixed with valid
  ...
  describe('Sub-Ticket/Test Case 4.8: Inventory - Ambiguous Match (StartsWith)', () => {
    test('Tests ambiguous match based on startsWith.', async () => {
      ...
      expect(result.candidates).toEqual(
        expect.arrayContaining(['long_sword', 'long_bow'])
      );
      expect(result.candidates.length).toBe(2);
      expect(result.error).toBe(
        'Which item starting with "Long" did you mean? For example: "Long Sword", "Long Bow".'
      );
