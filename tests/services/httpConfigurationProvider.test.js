...
describe('HttpConfigurationProvider', () => {
    ...
    describe('fetchData', () => {
        it('should fetch and parse valid JSON data successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(`HttpConfigurationProvider: Attempting to load configurations from ${url}`);
            expect(mockLogger.info).toHaveBeenCalledWith(`HttpConfigurationProvider: Successfully fetched and parsed configuration from ${url}.`);
        ...
        it('should throw an error and log if sourceUrl is invalid (empty string)', async () => {
            ...
            const provider = new HttpConfigurationProvider({logger: mockLogger});
            const url = '';
            await expect(provider.fetchData(url)).rejects.toThrow('HttpConfigurationProvider: sourceUrl must be a non-empty string.');
            expect(mockLogger.error).toHaveBeenCalledWith('HttpConfigurationProvider: sourceUrl must be a non-empty string.');
        ...
        it('should throw an error and log if fetch response is not ok (e.g., 404)', async () => {
            ...
            await expect(provider.fetchData(url)).rejects.toThrow('Failed to fetch configuration file from http://example.com/nonexistent.json: Not Found');
            expect(mockLogger.error).toHaveBeenCalledWith(`HttpConfigurationProvider: Failed to fetch configuration from ${url}. Status: 404 Not Found`);
        ...
        it('should throw an error and log if fetch response is not ok (e.g., 500)', async () => {
            ...
            await expect(provider.fetchData(url)).rejects.toThrow(`Failed to fetch configuration file from ${url}: Internal Server Error`);
            expect(mockLogger.error).toHaveBeenCalledWith(`HttpConfigurationProvider: Failed to fetch configuration from ${url}. Status: 500 Internal Server Error`);
            expect(global.fetch).toHaveBeenCalledTimes(1);
        });
        ...
        it('should throw an error and log if response.json() fails (invalid JSON)', async () => {
            ...
            await expect(provider.fetchData(url)).rejects.toThrow(`Failed to parse configuration data from ${url} as JSON: ${parseError.message}`);
            expect(mockLogger.error).toHaveBeenCalledWith(`HttpConfigurationProvider: Failed to parse JSON response from ${url}.`, {error: parseError.message});
            expect(global.fetch).toHaveBeenCalledTimes(1);
        });
        ...
        it('should throw an error and log if fetch itself fails (network error)', async () => {
            ...
            await expect(provider.fetchData(url)).rejects.toThrow(`Could not load configuration from ${url}: ${networkError.message}`);
