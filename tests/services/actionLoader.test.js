...
describe('ActionLoader', () => {
    ...
    describe('loadItemsForMod (Actions)', () => {
        ...
        beforeEach(() => {
            ...
            if (actionLoader && loadItemsInternalSpy) { loadItemsInternalSpy.mockResolvedValue(expectedLoadResult); }
                ...
                loadItemsInternalSpy = jest.spyOn(actionLoader, '_loadItemsInternal').mockResolvedValue(expectedLoadResult);
                validatePrimarySchemaSpy = jest.spyOn(actionLoader, '_validatePrimarySchema').mockReturnValue({ isValid: true, errors: null });
            }
        });
        ...
        it('should return 0 count if manifest has no actions content key', async () => { // Renamed slightly for clarity
            expect(actionLoader).toBeDefined();
            expect(loadItemsInternalSpy).toBeDefined();
    ...
    describe('_processFetchedItem', () => {
        ...
        const expectedOverrideResult = { qualifiedId: finalRegistryKey, didOverride: true };


        beforeEach(() => { // Keep as is
            if (!actionLoader) {
                actionLoader = new ActionLoader(mockConfig, mockResolver, mockFetcher, mockValidator, mockRegistry, mockLogger);
                loadItemsInternalSpy = jest.spyOn(actionLoader, '_loadItemsInternal').mockResolvedValue({ count: 0, overrides: 0, errors: 0 }); // Correct return type
                validatePrimarySchemaSpy = jest.spyOn(actionLoader, '_validatePrimarySchema').mockReturnValue({ isValid: true, errors: null });
            }
            ...
            validatePrimarySchemaSpy.mockClear().mockReturnValue({ isValid: true, errors: null }); // Still clear spy, ensure it returns valid for precondition
        ...
        it('Success Path: should check registry, store, log, and return ID with override=false', async () => {
            ...
            const storeItemSpy = jest.spyOn(actionLoader, '_storeItemInRegistry').mockReturnValue(false);

            const result = await actionLoader._processFetchedItem(TEST_MOD_ID, filename, resolvedPath, fetchedData, ACTION_TYPE_NAME);
            ...
            expect(storeItemSpy).toHaveBeenCalledWith(ACTION_TYPE_NAME, TEST_MOD_ID, baseActionIdExtracted, fetchedData, filename);
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Successfully processed action from ${filename}. Returning final registry key: ${finalRegistryKey}, Overwrite: false`));
        ...
        it('Override Path: should check registry, log warning, store, log, and return ID with override=true', async () => {
            expect(actionLoader?._processFetchedItem).toBeDefined();
            const fetchedData = JSON.parse(JSON.stringify(baseActionData));
            const existingActionData = { id: finalRegistryKey, description: 'Old', modId: 'other', _sourceFile: 'old.json' };
            // Configure mocks specific to this test
            ...
            const storeItemSpy = jest.spyOn(actionLoader, '_storeItemInRegistry').mockReturnValue(true);
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Extracted full ID '${namespacedActionIdFromFile}' and base ID '${baseActionIdExtracted}'`));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Delegating storage for action (base ID: '${baseActionIdExtracted}')`));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Successfully processed action from ${filename}. Returning final registry key: ${finalRegistryKey}, Overwrite: true`));
        ...
        it('Primary Schema Validation Failure: should NOT throw error from _processFetchedItem (validation is external)', async () => {
            ...
            expect(actionLoader?._processFetchedItem).toBeDefined();
            const fetchedData = JSON.parse(JSON.stringify(baseActionData));
            const validationError = new Error(`Schema validation failed for '${filename}'`);
            ...
            await expect(actionLoader._processFetchedItem(TEST_MOD_ID, filename, resolvedPath, fetchedData, ACTION_TYPE_NAME)).resolves.toBeDefined();
            ...
            expect(mockLogger.error).not.toHaveBeenCalledWith(`ActionLoader [${TEST_MOD_ID}]: Halting processing for '${filename}' due to primary schema validation error.`);
        ...
        it('Registry Store Failure: should attempt storage (call helper), log error from helper, and re-throw', async () => {
            ...
            const storeItemSpy = jest.spyOn(actionLoader, '_storeItemInRegistry').mockImplementation(() => {
                throw storeError;
            });
