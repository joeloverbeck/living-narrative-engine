    ...
    describe('Handling of Optional and Conditional Parts', () => {
        const conditionalTestConfig = {
            ...
            promptElements: [
                ...
                {key: "bad_cond_flag", prefix: "Bad:", condition: {promptDataFlag: 123}} // Invalid promptDataFlag type
            ],
        ...
        test('should include optional part if content exists, omit if not', async () => {
            ...
            const resultWithContent = await promptBuilder.build("cond/test", {optPartContent: "Opt"});
            ...
            const resultWithoutContent = await promptBuilder.build("cond/test", {optPartContent: ""});
            expect(resultWithoutContent).toBe(""); // Since other parts are conditional and not triggered by this promptData

            const resultWithNullContent = await promptBuilder.build("cond/test", {optPartContent: null});
            expect(resultWithNullContent).toBe("");

            const resultWithUndefinedContent = await promptBuilder.build("cond/test", {}); // optPartContent is undefined
        ...
        test('should include/omit conditional part based on truthiness of flag', async () => {
            ...
            expect(mockStandardAssembler.assemble).toHaveBeenCalledWith(
                ...
                expect.objectContaining({enableIt: true, truthyCondContent: "Enabled"}),
                mockPlaceholderResolver,
                expect.any(Map)
            );
        ...
        test('should include/omit conditional part based on flag matching expectedValue', async () => {
            ...
            const resValueMissing = await promptBuilder.build("cond/test", {valCondContent: "ShouldNotAppear"}); // mode flag is missing
        ...
        test('should omit conditional part and warn if condition.promptDataFlag is invalid type', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(
                ...
                expect.objectContaining({condition: conditionalTestConfig.promptElements[3].condition})
            );
