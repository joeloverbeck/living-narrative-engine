...
const mockValidRootLLMConfig = {
    ...
    configs: {
        "claude_sonnet_adventure_wrappers_v2": {
            ...
            promptAssemblyOrder: ["system_prompt", "perception_log_wrapper", "user_input", "assistant_response_prefix"]
        },
        "generic_xml_v1": {
...
describe('LlmConfigLoader', () => {
    ...
    describe('constructor', () => {
        test('should use provided logger', () => {
            Workspace_retry.mockResolvedValueOnce(JSON.parse(JSON.stringify(minimalValidRootConfigForPathTests)));
        ...
        test('should use console if no logger is provided in constructor (and log errors for missing deps)', () => {
            ...
            expect(() => new LlmConfigLoader({
                schemaValidator: schemaValidatorMock,
                configuration: configurationMock,
            })).toThrow("LlmConfigLoader: Constructor requires a valid ILogger instance."); // Now expects "valid"
            ...
            expect(consoleInfoSpy).toHaveBeenCalledWith(expect.stringContaining(`Attempting to load LLM Prompt configurations from: ${defaultLlmConfigPath}`));
        ...
        test('should use default config path if none provided (verified by loadConfigs call)', async () => {
            ...
            expect(loggerMock.info).toHaveBeenCalledWith(`LlmConfigLoader: Attempting to load LLM Prompt configurations from: ${defaultLlmConfigPath}`);
        });
        ...
        test('should throw error if schemaValidator is missing', () => {
            expect(() => new LlmConfigLoader({
                logger: loggerMock,
                configuration: configurationMock
            })).toThrow('LlmConfigLoader: Constructor requires a valid ISchemaValidator instance.'); // Now expects "valid"
        });
        ...
        test('should throw error if configuration is missing', () => {
            expect(() => new LlmConfigLoader({
                logger: loggerMock,
                schemaValidator: schemaValidatorMock
            })).toThrow('LlmConfigLoader: Constructor requires a valid IConfiguration instance.'); // Now expects "valid"
        });
    });
    ...
    describe('loadConfigs', () => {
        test('AC1 & AC5 (adapted for prompt configs): should load, parse, schema validate, semantic validate, and return a valid llm-configs.json file successfully', async () => {
            const expectedRootConfig = JSON.parse(JSON.stringify(mockValidRootLLMConfig));
            ...
            expect(schemaValidatorMock.validate).toHaveBeenCalledWith('http://example.com/schemas/llm-configs.schema.json', expectedRootConfig);

            expect(loggerMock.info).toHaveBeenCalledWith(`LlmConfigLoader: Successfully fetched and parsed LLM Prompt configurations from ${defaultLlmConfigPath}.`);
            expect(loggerMock.info).toHaveBeenCalledWith(`LlmConfigLoader: Validating against schema ID: http://example.com/schemas/llm-configs.schema.json`);
            expect(loggerMock.info).toHaveBeenCalledWith(`LlmConfigLoader: LLM Prompt configuration file from ${defaultLlmConfigPath} passed schema validation.`);
            expect(loggerMock.info).toHaveBeenCalledWith(`LlmConfigLoader: Performing semantic validation for ${defaultLlmConfigPath}.`);
        ...
        test('AC2: should handle Workspace network errors (simulated by Workspace_retry rejecting)', async () => {
            ...
            expect(loggerMock.error).toHaveBeenCalledWith(
                ...
                expect.objectContaining({path: defaultLlmConfigPath, originalErrorDetails: networkError})
            );
        ...
        test('AC3: should handle HTTP errors from Workspace (e.g., 404 Not Found)', async () => {
            const httpError = new Error(`API request to ${defaultLlmConfigPath} failed after 1 attempt(s) with status 404: {"error":"Not Found"}`);
        ...
        test('AC3: should handle HTTP errors from Workspace (e.g., 500 Server Error)', async () => {
            const httpError = new Error(`API request to ${defaultLlmConfigPath} failed after 3 attempt(s) with status 500: {"error":"Server Issue"}`);
            Workspace_retry.mockRejectedValueOnce(httpError);
            const result = await loader.loadConfigs(defaultLlmConfigPath);
        ...
        test('should return error if schema ID cannot be retrieved from configuration', async () => {
            ...
            const result = await loader.loadConfigs(defaultLlmConfigPath);
            expect(result.message).toBe("LlmConfigLoader: Schema ID for 'llm-configs' is undefined. Cannot validate.");
        ...
        test('should return error if schema validation fails for root object', async () => {
            ...
            expect(result.validationErrors[0]).toEqual(expect.objectContaining({
            ...
            expect(loggerMock.error).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    ...
                    validationErrors: expect.arrayContaining([
                        expect.objectContaining({message: "must have required property 'defaultConfigId'"})
                    ])
                })
            );
        });
        ...
        test('should return error if semantic validation fails (e.g. promptAssemblyOrder key mismatch)', async () => {
            ...
            Workspace_retry.mockResolvedValueOnce(JSON.parse(JSON.stringify(semanticallyInvalidRoot)));
            schemaValidatorMock.validate.mockReturnValueOnce({isValid: true, errors: null});
        ...
        test('should return error if "configs" property is not a valid object map (semantic check)', async () => {
            ...
            expect(result.stage).toBe('semantic_validation');
            expect(result.semanticErrors.length).toBeGreaterThan(0);
            expect(result.semanticErrors[0]).toEqual(expect.objectContaining({
