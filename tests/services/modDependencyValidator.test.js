...
const createManifestMap = (manifestsArray) => {
  ...
  for (const manifest of manifestsArray) {
    if (!manifest || !manifest.id) {
      throw new Error(
        'Test setup error: Invalid manifest provided to createManifestMap'
      );
    }
...
describe('ModDependencyValidator', () => {
  ...
  it('should throw if manifests is not a Map', () => {
    expect(() => ModDependencyValidator.validate([], mockLogger)).toThrow(
  ...
  it('should throw if logger is invalid (missing warn)', () => {
    ...
    expect(() =>
      ModDependencyValidator.validate(manifests, invalidLogger)
    ).toThrow(
  ...
  it('should throw if logger is null/undefined', () => {
    const manifests = createManifestMap([]);
    // @ts-expect-error - Testing invalid logger type
    expect(() => ModDependencyValidator.validate(manifests, null)).toThrow(
      'ModDependencyValidator.validate: Input `logger` must be a valid ILogger instance.'
    );
    // @ts-expect-error - Testing invalid logger type
    expect(() => ModDependencyValidator.validate(manifests, undefined)).toThrow(
      'ModDependencyValidator.validate: Input `logger` must be a valid ILogger instance.'
    );
  });
  ...
  it('1. Happy-path: 3 mods, valid chain', () => {
    ...
    const modB = {
      id: 'ModB',
      version: '1.1.0',
      dependencies: [{ id: 'ModC', version: '^2.0.0' }],
    };
    const modA = {
      id: 'ModA',
      version: '1.0.0',
  ...
  it('2. Missing required dependency', () => {
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'MissingB', version: '1.0.0', required: true }],
    };
  ...
  it('3. Version mismatch (use range >=2.0.0)', () => {
    ...
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'ModB', version: '>=2.0.0', required: true }],
    };
    ...
    expect(() =>
      ...
      /Mod 'ModA' requires dependency 'ModB' version '>=2.0.0', but found version '1.5.0'/
    );
    expect(mockLogger.warn).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });
  ...
  it('5. Optional dependency bad version (warn only)', () => {
    ...
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'ModB', version: '>=2.0.0', required: false }],
    };
    const manifests = createManifestMap([modA, modB]);
  ...
  it('5a. Optional dependency invalid version (warn only)', () => {
    ...
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'ModB', version: '>=1.0.0', required: false }],
    };
    const manifests = createManifestMap([modA, modB]);
  ...
  it('5b. Required dependency invalid version (throws)', () => {
    ...
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'ModB', version: '>=1.0.0', required: true }],
    };
    const manifests = createManifestMap([modA, modB]);
    ...
    expect(() =>
      ...
      /Mod 'ModA' dependency 'ModB' has an invalid version format: 'totally-invalid'/
    );
    expect(mockLogger.warn).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });
  ...
  it('7. Conflict declared by both (double-entry)', () => {
    ...
    expect(capturedError.message).toContain(
      "Mod 'ModA' conflicts with loaded mod 'ModB'"
    );
    expect(capturedError.message).toContain(
      "Mod 'ModB' conflicts with loaded mod 'ModA'"
    );
  ...
  it('9. Multiple simultaneous failures aggregated into single throw', () => {
    ...
    const modC = {
      id: 'ModC',
      version: '1.0.0',
      dependencies: [{ id: 'MissingE', version: '1.0.0' }],
    }; // Missing required MissingE
    ...
    expect(errorMessages).toContain(
      "Mod 'ModA' requires dependency 'ModB' version '>=2.0.0', but found version '1.5.0'."
    ); // From ModA -> ModB check
    expect(errorMessages).toContain(
      "Mod 'ModA' conflicts with loaded mod 'ModD'."
    ); // From ModA -> ModD check
    expect(errorMessages).toContain(
      "Mod 'ModC' requires missing dependency 'MissingE'."
    ); // From ModC -> MissingE check
  ...
  it('should handle dependency object missing required field (defaults to required=true)', () => {
    ...
    const modA = {
      id: 'ModA',
      version: '1.0.0',
      dependencies: [{ id: 'MissingB', version: '1.0.0' }],
    };
    const manifests = createManifestMap([modA]);
