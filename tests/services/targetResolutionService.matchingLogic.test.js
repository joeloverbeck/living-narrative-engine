...
describe('TargetResolutionService - Advanced Name Matching Logic (via Inventory Domain)', () => {
  ...
  const createActionContext = (
    ...
    return {
      ...
      parsedCommand: {
        ...
        originalInput:
          typeof nounPhraseValue === 'string' ? nounPhraseValue : '', // for completeness
        error: null,
        preposition: null,
  ...
  beforeEach(() => {
    ...
    mockWorldContext = {
      getLocationOfEntity: jest
        .fn()
        .mockReturnValue({ id: 'mockRoomForContext' }), // For _buildMinimalContextForScopes
  ...
  const setupInventoryAndEntityManager = (inventoryItems) => {
    ...
    mockActorEntity.addComponent(INVENTORY_COMPONENT_ID, {
      items: itemEntityIds,
    });
    ...
    test('should select the exact match even if startsWith and substring matches exist', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames: Unique exact match found: ID "item2"'
        )
      );
    });
  });
  ...
  describe('8.2: StartsWith Match Priority', () => {
    test('should select the startsWith match if no exact match exists, even with substring matches', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames: Unique startsWith match found: ID "item1"'
        )
      );
    });
  });
  ...
  describe('8.3: Substring Match', () => {
    test('should select the substring match if no exact or startsWith matches exist', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames: Unique substring match found: ID "item1"'
        )
      );
    });
  });
  ...
  describe('8.10: No Candidates Provided to #_matchByName', () => {
    ...
    test('error from NameMatcher "Nothing found to match..." is overridden by inventory domain', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames called with phrase: "apple"'
        )
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames - Phrase: "apple" - Exact: 0, StartsWith: 0, Substring: 0'
        )
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames: No matches found for phrase "apple"'
        )
      );
    });
  });
