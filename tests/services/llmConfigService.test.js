...
describe('LLMConfigService', () => {
    ...
    describe('Constructor', () => {
        it('should throw an error if configurationProvider is missing', () => {
            expect(() => new LLMConfigService({logger: mockLogger, configurationProvider: undefined}))
                .toThrow('LLMConfigService: configurationProvider and logger are required options.');
        });

        it('should throw an error if logger is missing', () => {
            expect(() => new LLMConfigService({configurationProvider: mockConfigurationProvider, logger: undefined}))
                .toThrow('LLMConfigService: configurationProvider and logger are required options.');
        });
        ...
        it('should initialize with a configSourceIdentifier', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Configuration source identifier set to: ${sourceId}`));
        });
        ...
        it('should initialize with valid initialConfigs and mark configsLoadedOrAttempted as true', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().get(sampleConfig1.configId)).toEqual(sampleConfig1);
            expect(service.getConfigsLoadedOrAttemptedFlagForTest()).toBe(true);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Successfully loaded 1 initial configurations into cache.'));
        });
        ...
        it('should handle empty initialConfigs array', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('LLMConfigService: Empty array provided for initialConfigs. No initial configurations loaded.');
        });
        ...
        it('should warn if initialConfigs is not an array', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.warn).toHaveBeenCalledWith('LLMConfigService: initialConfigs was provided but is not an array. Ignoring.');
        });
        ...
        it('should filter out invalid initialConfigs and log warnings', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().size).toBe(1);
            expect(service.getLlmConfigsCacheForTest().has(sampleConfig1.configId)).toBe(true);
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Skipping invalid configuration object.'), {configAttempted: invalidConfig});
        ...
        it('should log if no source identifier and no initial configs loaded', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('No configuration source identifier provided and no initial configs loaded.'));
        });
    });

    describe('#isValidConfig (indirectly tested via addOrUpdateConfigs and loading)', () => {
        beforeEach(() => {
            service = new LLMConfigService({logger: mockLogger, configurationProvider: mockConfigurationProvider});
        });

        const baseValidConfig = {
            configId: "valid", modelIdentifier: "valid/model",
            promptElements: [{key: "test"}], promptAssemblyOrder: ["test"]
        };
        ...
        it('should consider a config invalid if configId is missing or empty', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: Missing or empty configId.', expect.any(Object));
        });
        ...
        it('should consider a config invalid if modelIdentifier is missing or empty', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: Missing or empty modelIdentifier.', expect.any(Object));
        });

        it('should consider a config invalid if promptElements is not an array', () => {
            // @ts-ignore
            service.addOrUpdateConfigs([{...baseValidConfig, promptElements: "not-an-array"}]);
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: promptElements is not an array.', expect.any(Object));
        });
        it('should consider a config invalid if promptElements contains invalid items', () => {
            service.addOrUpdateConfigs([{...baseValidConfig, promptElements: [{noKey: "bad"}]}]);
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: One or more promptElements are invalid (not an object or missing key).', expect.any(Object));
        });


        it('should consider a config invalid if promptAssemblyOrder is not an array', () => {
            // @ts-ignore
            service.addOrUpdateConfigs([{...baseValidConfig, promptAssemblyOrder: "not-an-array"}]);
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: promptAssemblyOrder is not an array.', expect.any(Object));
        });
        it('should consider a config invalid if promptAssemblyOrder contains non-string keys', () => {
            // @ts-ignore
            service.addOrUpdateConfigs([{...baseValidConfig, promptAssemblyOrder: [123]}]);
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.debug).toHaveBeenCalledWith('LLMConfigService.#isValidConfig: One or more keys in promptAssemblyOrder are not strings.', expect.any(Object));
        });
    });
    ...
    describe('addOrUpdateConfigs', () => {
        ...
        it('should add valid configurations to the cache', () => {
            ...
            expect(service.getLlmConfigsCacheForTest().get(sampleConfig1.configId)).toEqual(sampleConfig1);
            expect(service.getLlmConfigsCacheForTest().get(sampleConfig2.configId)).toEqual(sampleConfig2);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Processed 2 configs: 2 added, 0 updated, 0 skipped.'));
        });

        it('should update existing configurations in the cache', () => {
            service.addOrUpdateConfigs([{...sampleConfig1}]);
            const updatedConfig1 = {...sampleConfig1, displayName: "Model A v1 Updated"};
            service.addOrUpdateConfigs([updatedConfig1]);
            expect(service.getLlmConfigsCacheForTest().size).toBe(1);
            expect(service.getLlmConfigsCacheForTest().get(sampleConfig1.configId)).toEqual(updatedConfig1);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Processed 1 configs: 0 added, 1 updated, 0 skipped.'));
        });
        ...
        it('should log an error if input is not an array', () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('LLMConfigService.addOrUpdateConfigs: Input must be an array of LLMConfig objects.');
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
        });
        ...
        it('should set configsLoadedOrAttempted to true if configs are added and it was false', () => {
            ...
            service.addOrUpdateConfigs([{...sampleConfig1}]);
            expect(service.getConfigsLoadedOrAttemptedFlagForTest()).toBe(true);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Configurations added programmatically. Marking cache as "loaded/attempted"'));
        });
    });
    ...
    describe('#loadAndCacheConfigurationsFromSource (via getConfig and resetCache)', () => {
        ...
        it('should load and cache configurations from provider if sourceId is set and cache is empty', async () => {
            ...
            const fetchedData = {
                ...
                configs: {[sampleConfig1.configId]: {...sampleConfig1}, [sampleConfig2.configId]: {...sampleConfig2}}
            };
            ...
            expect(mockConfigurationProvider.fetchData).toHaveBeenCalledWith(sourceId);
            expect(service.getLlmConfigsCacheForTest().size).toBe(2);
            expect(service.getLlmConfigsCacheForTest().get(sampleConfig1.configId)).toEqual(sampleConfig1);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Successfully loaded and cached 2 configurations from ${sourceId}`));
            expect(service.getConfigsLoadedOrAttemptedFlagForTest()).toBe(true);
        });
        ...
        it('should not load from source if configSourceIdentifier is not set', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('No configSourceIdentifier set and no initial configurations were loaded.'));
        });
        ...
        it('should handle errors from configurationProvider.fetchData', async () => {
            ...
            expect(mockConfigurationProvider.fetchData).toHaveBeenCalledWith(sourceId);
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(`Error loading or parsing configurations from ${sourceId}. Detail: Fetch failed`), {error: fetchError});
            expect(service.getConfigsLoadedOrAttemptedFlagForTest()).toBe(true);
        });
        ...
        it('should handle invalid data structure from provider (e.g. missing "configs" map)', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                ...
                expect.objectContaining({source: sourceId, receivedData: invalidFetchedData})
            );
        });
        ...
        it('should skip invalid configs within fetched data and log them', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(
                ...
                expect.objectContaining({source: sourceId, configKey: "invalidKey", configData: invalidConfigInSource})
            );
        });
        ...
        it('should warn if configId in fetched object does not match its key in the map', async () => {
            ...
            const mismatchedConfig = {...sampleConfig1, configId: "actual_id_is_different"};
            const fetchedData = {
            ...
            expect(service.getLlmConfigsCacheForTest().has("actual_id_is_different")).toBe(true);
            expect(mockLogger.warn).toHaveBeenCalledWith(
                expect.stringContaining(`Config object's internal configId ("actual_id_is_different") does not match its key in the 'configs' map ("key_in_map")`),
                expect.any(Object)
            );
        });
        ...
        it('should not reload from source if load was attempted from source and failed (cache empty)', async () => {
            ...
            mockConfigurationProvider.fetchData.mockRejectedValueOnce(new Error("First fail"));
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Load previously attempted from source, but cache is empty'));
        });
    });
    ...
    describe('getConfig', () => {
        it('should return undefined and log error if llmId is not a non-empty string', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('LLMConfigService.getConfig: llmId must be a non-empty string.', {llmIdAttempted: ''});
            // @ts-ignore
            expect(await service.getConfig(null)).toBeUndefined();
            // @ts-ignore
            expect(mockLogger.error).toHaveBeenCalledWith('LLMConfigService.getConfig: llmId must be a non-empty string.', {llmIdAttempted: null});
        });
        ...
        it('should return undefined if cache is empty and no source can provide configs', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Cache is empty. Cannot find configuration for "non_existent_id".'));
        });
        ...
        it('should return config by direct configId match', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Found configuration by direct configId match for "${sampleConfig1.configId}"`));
        });
        ...
        it('should return a copy of the config, not the cached instance', async () => {
            ...
            const originalCachedConfig = service.getLlmConfigsCacheForTest().get(sampleConfig1.configId);
        ...
        it('should return config by exact modelIdentifier match if configId fails', async () => {
            ...
            expect(config).toEqual(sampleConfig1);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Selected configuration by exact modelIdentifier match for "${sampleConfig1.modelIdentifier}"`));
        });

        it('should return config by wildcard modelIdentifier match (longest prefix wins)', async () => {
            const initialConfigs = [{...sampleWildcardConfig}, {...sampleLongerWildcardConfig}, {...sampleConfig1}];
            service = new LLMConfigService({
                logger: mockLogger,
                configurationProvider: mockConfigurationProvider,
                initialConfigs
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Selected configuration by wildcard modelIdentifier match for "${targetModelId1}". Pattern: "${sampleLongerWildcardConfig.modelIdentifier}"`));
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Selected configuration by wildcard modelIdentifier match for "${targetModelId2}". Pattern: "${sampleWildcardConfig.modelIdentifier}"`));
        });
        ...
        it('should prioritize direct configId over modelIdentifier matches', async () => {
            ...
            const conflictingConfig = {
                ...
                promptElements: [{key: "k"}], promptAssemblyOrder: ["k"]
            };
            service = new LLMConfigService({
                logger: mockLogger,
                configurationProvider: mockConfigurationProvider,
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Found configuration by direct configId match for "provider/model-a-v1"`));
        });
        ...
        it('should prioritize exact modelIdentifier over wildcard modelIdentifier', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Selected configuration by exact modelIdentifier match for "provider/model-x"`));
        });
        ...
        it('should return undefined if no match is found after all checks', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('No configuration found for identifier "non_existent_identifier_123"'));
        });
    });
    ...
    describe('resetCache', () => {
        ...
        it('should clear the cache and reset configsLoadedOrAttempted flag', async () => {
            ...
            expect(service.getLlmConfigsCacheForTest().size).toBe(0);
            expect(service.getConfigsLoadedOrAttemptedFlagForTest()).toBe(false);
            expect(mockLogger.info).toHaveBeenCalledWith('LLMConfigService: Cache cleared and loaded state reset. Configurations will be reloaded from source on next request if source is configured.');
        });
        ...
        it('should force a reload from source on next getConfig call if source is configured', async () => {
            ...
            const fetchedData = {configs: {[sampleConfig2.configId]: {...sampleConfig2}}};
            mockConfigurationProvider.fetchData.mockResolvedValue(fetchedData);
