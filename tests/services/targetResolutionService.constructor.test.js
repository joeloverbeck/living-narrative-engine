...
describe('TargetResolutionService', () => {
  ...
  describe('Constructor - Missing Non-Logger Dependency', () => {
    const createOptions = (missingDependency) => ({
      entityManager:
        missingDependency === 'entityManager' ? null : mockEntityManager,
      worldContext:
        missingDependency === 'worldContext' ? null : mockWorldContext,
      gameDataRepository:
        missingDependency === 'gameDataRepository'
          ? null
          : mockGameDataRepository,
      logger: mockLogger,
      getEntityIdsForScopes:
        missingDependency === 'getEntityIdsForScopes'
          ? null
          : getEntityIdsForScopes, // Include in testing missing deps
    });
  ...
  describe('Constructor - Non-Logger Dependency Missing Method', () => {
    ...
    const testCases = [
      ...
      {
        depName: 'worldContext',
        method: 'getLocationOfEntity',
        getOptions: () => {
          const m = getBaseValidMocks();
      ...
      {
        depName: 'gameDataRepository',
        method: 'getAllActionDefinitions',
        getOptions: () => {
          const m = getBaseValidMocks();
  ...
  describe('resolveActionTarget - Basic Guard Clauses', () => {
    ...
    test('should return ERROR if actionDefinition is null', async () => {
      ...
      const expectedLoggerMsg =
        'TargetResolutionService.resolveActionTarget: Missing actionDefinition or actionContext. Action ID: undefined_action_definition.';
    ...
    test('should return ERROR if actionContext is null', async () => {
      ...
      const expectedErrorMsg = 'Internal error: Invalid action setup.';
      const expectedLoggerMsg =
        'TargetResolutionService.resolveActionTarget: Missing actionDefinition or actionContext. Action ID: test:action.';
    ...
    describe('resolveActionTarget - Missing actingEntity for actor-required domains', () => {
      const domainsRequiringActor = [
        'self',
        'inventory',
        'equipment',
        'environment',
      ];
      test.each(domainsRequiringActor)(
        'should return ERROR if actingEntity is null for domain "%s"',
        async (domain) => {
          const actionDefinition = {
            id: `test:${domain}-action`,
    ...
    test('should return NOT_FOUND for an unknown target_domain', async () => {
      const actionDefinition = {
        id: 'test:unknown',
        target_domain: 'fictional_domain',
      };
      ...
      const expectedErrorMsg =
        "Action 'test:unknown' has an unsupported target domain: fictional_domain.";
      const expectedLoggerMsg =
        "TargetResolutionService.resolveActionTarget: Unknown target domain 'fictional_domain' for action 'test:unknown'.";
    ...
    test('should catch unexpected errors from internal resolvers and return ERROR', async () => {
      ...
      const expectedPlayerError =
        "An unexpected internal error occurred while trying to resolve the target for action 'test:error-action'. Please contact support.";
      ...
      const loggerCall = mockLogger.error.mock.calls.find(
        (call) => call[0] === expectedLoggerMsg
      );
