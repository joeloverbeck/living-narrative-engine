...
describe('AIPromptContentProvider.getPromptData → notesArray', () => {
    ...
    const stubPerceptionLogFormatter = {
        format: (logArray) => logArray.map((entry) => ({content: entry.descriptionText || ''})),
    };
    ...
    test('when core:notes is absent → notesArray is []', async () => {
        const gameStateDto = {
            ...
            currentUserInput: '',
            perceptionLog: [],
            currentLocation: {name: 'Home', description: 'A humble abode', exits: [], characters: []},
            availableActions: [],
        };
    ...
    test('when core:notes.notes is not an array → notesArray is []', async () => {
        const gameStateDto = {
            ...
            currentUserInput: '',
            perceptionLog: [],
            currentLocation: {name: 'Town', description: 'A small village', exits: [], characters: []},
            availableActions: [],
        };
    ...
    test('when core:notes.notes is an empty array → notesArray is []', async () => {
        const gameStateDto = {
            ...
            currentUserInput: '',
            perceptionLog: [],
            currentLocation: {name: 'Forest', description: 'Leafy and green', exits: [], characters: []},
            availableActions: [],
        };
    ...
    test('when core:notes.notes has valid entries → notesArray is populated correctly', async () => {
        ...
        const gameStateDto = {
            ...
            currentUserInput: '',
            perceptionLog: [],
            currentLocation: {name: 'Castle', description: 'Cold stone walls', exits: [], characters: []},
            availableActions: [],
        };
    ...
    test('when core:notes.notes has malformed entries → filter them out', async () => {
        ...
        const gameStateDto = {
            ...
            currentUserInput: '',
            perceptionLog: [],
            currentLocation: {name: 'Tower', description: 'High up', exits: [], characters: []},
            availableActions: [],
        };

        const result = await provider.getPromptData(gameStateDto, logger);
        expect(result.notesArray).toEqual([{text: 'Valid note', timestamp: validTime}]);
    });
});