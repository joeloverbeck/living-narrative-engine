...
describe('PromptBuilder', () => {
  ...
  describe('Special Handling for Perception Log', () => {
    ...
    const noEntryConfig = {
      // Config without perception_log_entry
    ...
    beforeEach(() => {
      ...
      mockPerceptionLogAssembler.assemble.mockImplementation(
        (
          ...
          const resolvedWrapperPrefix = placeholderResolverInstance.resolve(
            wrapperConfig.prefix || '',
            promptData
          );
          const resolvedWrapperSuffix = placeholderResolverInstance.resolve(
          ...
          if (
            ...
            return resolvedWrapperPrefix || resolvedWrapperSuffix
              ? `${resolvedWrapperPrefix}${resolvedWrapperSuffix}`
              : '';
          }
          ...
          for (const entry of perceptionLogArray) {
            ...
            currentEntryPrefix = placeholderResolverInstance.resolve(
              currentEntryPrefix,
              entryForResolution,
              promptData
            );
            currentEntrySuffix = placeholderResolverInstance.resolve(
              currentEntrySuffix,
              entryForResolution,
              promptData
            );
    ...
    test('should gracefully omit perception log if array is empty, null, or undefined', async () => {
      ...
      expect(
        await promptBuilder.build('p_log_test', {
          ...baseData,
          perceptionLogArray: [],
        })
      ...
      logger.debug.mockClear();
      expect(await promptBuilder.build('p_log_test', { ...baseData })).toBe(
        expectedWhenEmpty
      ); // perceptionLogArray is undefined
      expect(logger.debug).toHaveBeenCalledWith(
        "PromptBuilder.build: Perception log array for 'perception_log_wrapper' missing or empty in PromptData. Skipping wrapper element."
      );
    });
    ...
    test('should process wrapper even if perception_log_entry dependencyInjection is missing (entries will be empty)', async () => {
      ...
      const promptData = {
        perceptionLogArray: [{ role: 'user', content: 'Message 1' }],
      };
    ...
    test('should skip invalid entries (null, non-object) in perceptionLogArray and log warning', async () => {
      ...
      mockPlaceholderResolver.resolve.mockImplementation((text, ds1, ds2) => {
        ...
        if (currentText.includes('{session_id}')) {
          ...
          currentText = currentText.replace(
            /{session_id}/g,
            globalDataSource.session_id || ''
          );
        }
        // For valid entry
        if (
          entryDataSource &&
          entryDataSource.role === 'user' &&
          entryDataSource.content === 'Valid'
        ) {
          ...
          currentText = currentText.replace(
            /{source_system}/g,
            entryDataSource.source_system ||
              globalDataSource.source_system ||
              ''
          );
        }
        return currentText;
    ...
    test('should handle entries with missing content (null/undefined) as empty string content', async () => {
      ...
      const promptData = {
        perceptionLogArray: [
          { role: 'user', timestamp: 'T_missing_content', content: null },
        ],
