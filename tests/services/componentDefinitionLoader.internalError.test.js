...
describe('ComponentLoader (Internal Definition Errors)', () => {
  ...
  it('should handle definitions with invalid "id" (null or empty string)', async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      expect.stringContaining(
        `Loading components definitions for mod '${modId}'.`
      )
    );
    expect(mockLogger.info).toHaveBeenCalledWith(
      expect.stringContaining(
        `Mod [${modId}] - Processed 0/2 components items. (2 failed)`
      ) // Correct summary for two failures
    );
  ...
  it('should handle definitions with invalid "dataSchema" (null or not an object)', async () => {
    ...
    expect(mockValidator.validate).toHaveBeenCalledWith(
      componentDefSchemaId,
      invalidDataNullSchema
    );
    expect(mockValidator.validate).toHaveBeenCalledWith(
      componentDefSchemaId,
      invalidDataStringSchema
    );
    ...
    const schemaTypeError1 = expect.objectContaining({
      message: `Invalid dataSchema type in ${filenameNullSchema} for component ${validId}`,
    });
    const expectedSpecificErrorDetails1 = expect.objectContaining({
      componentId: validId,
    ...
    const schemaTypeError2 = expect.objectContaining({
      message: `Invalid dataSchema type in ${filenameStringSchema} for component ${validId}`,
    });
    const expectedSpecificErrorDetails2 = expect.objectContaining({
      componentId: validId,
