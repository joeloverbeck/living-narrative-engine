...
describe('PerceptionLogAssembler', () => {
    ...
    const baseElementConfig = {key: 'perception_log_wrapper', prefix: '<log>', suffix: '</log>'};
    ...
    beforeEach(() => {
        ...
        sampleAllPromptElementsMap.set(PERCEPTION_LOG_ENTRY_KEY, {...basePLogEntryConfig});

        activeMockPlaceholderResolver.resolve.mockImplementation((str, _source1, _source2) => {
    ...
    describe('assemble - Parameter Validation', () => {
        it('should return empty string and log error if elementConfig is missing', () => {
            const result = assembler.assemble(null, basePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith('PerceptionLogAssembler.assemble: Missing required parameters.', expect.objectContaining({elementConfigProvided: false}));
        });
        it('should return empty string and log error if promptData is missing', () => {
            const result = assembler.assemble(baseElementConfig, null, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith('PerceptionLogAssembler.assemble: Missing required parameters.', expect.objectContaining({promptDataProvider: false}));
        });
        it('should return empty string and log error if placeholderResolver is missing', () => {
            const result = assembler.assemble(baseElementConfig, basePromptData, null, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith('PerceptionLogAssembler.assemble: Missing required parameters.', expect.objectContaining({placeholderResolverProvided: false}));
        });
        it('should return empty string and log error if allPromptElementsMap is missing', () => {
            const result = assembler.assemble(baseElementConfig, basePromptData, activeMockPlaceholderResolver, null);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith('PerceptionLogAssembler.assemble: Missing required parameters.', expect.objectContaining({allPromptElementsMapProvided: false}));
        });
    });
    ...
    describe('assemble - Wrapper Prefix/Suffix and Empty Log Array', () => {
        it('should resolve wrapper prefix and suffix using promptData', () => {
            activeMockPlaceholderResolver.resolve
                .mockImplementationOnce((str, data) => `resolved_prefix(${str}, ${data.global})`)
                .mockImplementationOnce((str, data) => `resolved_suffix(${str}, ${data.global})`);

            assembler.assemble(baseElementConfig, {
                ...basePromptData,
                perceptionLogArray: []
            }, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith(baseElementConfig.prefix || "", expect.objectContaining({global: 'globalValue'}));
            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith(baseElementConfig.suffix || "", expect.objectContaining({global: 'globalValue'}));
        });

        it('should return only resolved wrapper prefix/suffix if perceptionLogArray is null', () => {
            activeMockPlaceholderResolver.resolve.mockImplementation(str => str === baseElementConfig.prefix ? "PREFIX" : (str === baseElementConfig.suffix ? "SUFFIX" : ""));
            const result = assembler.assemble(baseElementConfig, {
                ...basePromptData,
                perceptionLogArray: null
            }, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("PREFIXSUFFIX");
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Perception log array for 'perception_log_wrapper' missing or empty"));
        });
        ...
        it('should return empty string if perceptionLogArray is empty and wrapper prefix/suffix are also empty/unresolved to empty', () => {
            ...
            const emptyWrapperConfig = {...baseElementConfig, prefix: "", suffix: ""};
            const result = assembler.assemble(emptyWrapperConfig, {
                ...basePromptData,
                perceptionLogArray: []
            }, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
    ...
    describe('assemble - Handling perception_log_entry Config', () => {
        it('should warn and produce no entry content if perception_log_entry config is missing', () => {
            ...
            const result = assembler.assemble(baseElementConfig, basePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("<log></log>");
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`Missing '${PERCEPTION_LOG_ENTRY_KEY}' config`));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Entries were not formatted or added to output due to missing '${PERCEPTION_LOG_ENTRY_KEY}' config.`));
    ...
    describe('assemble - Iterating PerceptionLogArray', () => {
        it('should skip and warn for invalid (non-object/null) entries in perceptionLogArray', () => {
            ...
            activeMockPlaceholderResolver.resolve.mockImplementation((str, entryData, _pData) => {
                if (str === basePLogEntryConfig.prefix && entryData) return `<entry role="${entryData.role}">`;
            ...
            const result = assembler.assemble(baseElementConfig, promptDataWithInvalidEntry, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid perception log entry encountered"), {entry: "invalid_string_entry"});
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid perception log entry encountered"), {entry: null});
            expect(result).toBe("<log><entry role=\"user\">Valid1</entry><entry role=\"assistant\">Valid2</entry></log>");
        });
        ...
        it('should use empty string for entry.content if it is null or undefined', () => {
            ...
            const result = assembler.assemble(baseElementConfig, promptDataWithNullContent, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("<log><entry role=\"user\"></entry><entry role=\"assistant\"></entry></log>");
        });
        ...
        it('should convert non-string entry.content to string', () => {
            ...
            const result = assembler.assemble(baseElementConfig, promptDataWithNonStringContent, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
    ...
    describe('assemble - Placeholder Resolution and Timestamp Cleaning for Entries', () => {
        const pLogEntryConfigWithTimestamp = {
            key: PERCEPTION_LOG_ENTRY_KEY,
            prefix: '<entry role="{role}" timestamp="dont_resolve_this" global="{global}">',
        ...
        beforeEach(() => {
            sampleAllPromptElementsMap.set(PERCEPTION_LOG_ENTRY_KEY, {...pLogEntryConfigWithTimestamp});
        });

        it('should clean timestamp attributes from entry prefix/suffix before resolving placeholders', () => {
            activeMockPlaceholderResolver.resolve.mockImplementation((str, _data1, _data2) => str);
            ...
            assembler.assemble(baseElementConfig, currentPromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);

            const expectedCleanedPrefix = '<entry role="{role}" global="{global}">';
            const expectedCleanedSuffix = '</entry>';
            const expectedEntryForResolution = {role: 'user', content: 'Test content', otherEntryKey: 'entrySpecific'};
        ...
        it('should resolve placeholders in entry prefix/suffix using entryForResolution and promptData', () => {
            activeMockPlaceholderResolver.resolve.mockImplementation((str, data1, data2) => {
            ...
            const result = assembler.assemble(baseElementConfig, currentPromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe(`<log><entry role="user" global="globalValue">Test content</entry></log>`);
        });

        it('should correctly clean various timestamp attribute formats', () => {
            const complexPLogEntryConfig = {
                key: PERCEPTION_LOG_ENTRY_KEY,
                prefix: '<obs role="{role}" timestamp="123" other="val" timestamp=\'456\' >Message: ',
                suffix: ' [End obs timestamp = 789]'
            };
            sampleAllPromptElementsMap.set(PERCEPTION_LOG_ENTRY_KEY, complexPLogEntryConfig);
            const currentEntryData = {role: 'system', content: 'System update.'};

            activeMockPlaceholderResolver.resolve.mockImplementation((str, data1, _data2) => {
                // These are the strings implied by the previous "Received" error log's FALLBACK_RESOLVE output
                const actualCleanedPrefixFromRuntime = '<obs role="{role}" other="val">Message:';
            ...
            expect(result).toBe('<log><obs role="system" other="val">Message:System update.[End obs</log>');
        });
    });
    ...
    describe('assemble - Final Output and Logging', () => {
        it('should correctly assemble the final string from all parts', () => {
            activeMockPlaceholderResolver.resolve.mockImplementation((str, data1, _data2) => {
                ...
                if (str === basePLogEntryConfig.prefix) return `[[ENTRY_PRE role=${data1.role}]]`;
                if (str === basePLogEntryConfig.suffix) return `[[ENTRY_SUFF role=${data1.role}]]`;
                return "";
            });
            const logArray = [
            ...
            const expected = "[[WRAP_PRE]]" +
                "[[ENTRY_PRE role=user]]Hi[[ENTRY_SUFF role=user]]" +
                "[[ENTRY_PRE role=ai]]Hello[[ENTRY_SUFF role=ai]]" +
                "[[WRAP_SUFF]]";
            expect(result).toBe(expected);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Perception log wrapper for 'perception_log_wrapper' processed with formatted entries."));
        });

        it('should log if all entries resulted in empty strings but pLogEntryConfig was present', () => {
            activeMockPlaceholderResolver.resolve.mockImplementation((str, _data1, _data2) => {
                if (str === baseElementConfig.prefix) return "WRAP_PRE";
                if (str === baseElementConfig.suffix) return "WRAP_SUFF";
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("all log entries resulted in empty strings"));
        });
    });
});
// --- FILE END ---