...
describe('StandardElementAssembler', () => {
    ...
    describe('assemble', () => {
        ...
        it('should return an empty string and log error if elementConfig is missing', () => {
            const result = assembler.assemble(null, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
        ...
        it('should return an empty string and log error if promptData is missing', () => {
            ...
            const result = assembler.assemble(elementConfig, null, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith(
                'StandardElementAssembler.assemble: Missing required parameters (elementConfig, promptData, or placeholderResolver).',
        ...
        it('should return an empty string and log error if placeholderResolver is missing', () => {
            const elementConfig = {key: "test_key"};
            const result = assembler.assemble(elementConfig, samplePromptData, null, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.error).toHaveBeenCalledWith(
                'StandardElementAssembler.assemble: Missing required parameters (elementConfig, promptData, or placeholderResolver).',
        ...
        it('should return an empty string and log warning if elementConfig.key is missing or invalid', () => {
            ...
            let result = assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            ...
            result = assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.warn).toHaveBeenCalledWith(
                `StandardElementAssembler.assemble: Invalid or missing 'key' in elementConfig. Cannot process element.`,
                {elementConfig}
        ...
        it('should correctly assemble with prefix, content, and suffix', () => {
            ...
            const result = assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            const expectedContentKey = snakeToCamel(elementConfig.key) + "Content"; // userQueryContent

            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith(elementConfig.prefix, samplePromptData);
            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith(elementConfig.suffix, samplePromptData);
            expect(result).toBe(`User: testValue ${samplePromptData[expectedContentKey]} EndQuery`);
        });
        ...
        it('should handle missing prefix (default to empty string)', () => {
            ...
            const result = assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith("", samplePromptData); // Prefix defaults to ""
        ...
        it('should handle missing suffix (default to empty string)', () => {
            ...
            const result = assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(activeMockPlaceholderResolver.resolve).toHaveBeenCalledWith("", samplePromptData); // Suffix defaults to ""
        ...
        it('should derive content key correctly using snakeToCamel', () => {
            ...
            assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            ...
            expect(assembler.assemble(elementConfig, samplePromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap))
        ...
        it('should use empty string for content if rawContent in promptData is null', () => {
            ...
            const localPromptData = {...samplePromptData, nullContentKeyContent: null};
            activeMockPlaceholderResolver.resolve.mockReturnValue("");

            const result = assembler.assemble(elementConfig, localPromptData, activeMockPlaceholderResolver, sampleAllPromptElementsMap);
            expect(result).toBe("");
            expect(mockLogger.debug).toHaveBeenCalledWith(
        ...
        it('should use empty string and log warning if rawContent is not a string, null, or undefined', () => {
            ...
            const localPromptData = {...samplePromptData, numericContentKeyContent: 123};
            activeMockPlaceholderResolver.resolve.mockReturnValue("");
        ...
        it('should return only prefix and suffix if content is empty/missing', () => {
            const elementConfig = {key: "missing_key", prefix: "Start:", suffix: ":End"};
            activeMockPlaceholderResolver.resolve.mockImplementation((str, _data) => {
