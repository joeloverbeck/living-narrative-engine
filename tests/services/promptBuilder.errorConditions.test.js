// tests/services/promptBuilder.errorConditions.test.js
// --- FILE START ---
...
describe('PromptBuilder', () => {
    ...
    beforeEach(() => {
        ...
        mockStandardAssembler.assemble.mockImplementation(
            (elementConfig, promptData, placeholderResolverInstance) => {
                ...
                let centralContentString = (typeof rawContent === 'string') ? rawContent : "";
    ...
    describe('Error Conditions and Edge Cases', () => {
        ...
        test('should warn and skip key if key in assembly_order not in promptElements', async () => {
            ...
            const result = await promptBuilder.build("vendor/err_key_model", {aContent: "Data for A"});
        ...
        test('should produce empty string if promptAssemblyOrder is empty', async () => {
            ...
            expect(await promptBuilder.build("vendor/empty_order_model", {aContent: "Data for A"})).toBe("");
            expect(logger.info).toHaveBeenCalledWith(expect.stringContaining(`Successfully assembled prompt for llmId: vendor/empty_order_model using config ${cfg.configId}. Length: 0`));
        });
        ...
        test('placeholders for deeply nested structures in promptData are not resolved by default by PlaceholderResolver', async () => {
            const cfg = {
                ...
                promptElements: [{key: "user", prefix: "Name: {user.name} {flat_val}"}], // Suffix removed, content assumed empty for simplicity
            ...
            mockPlaceholderResolver.resolve.mockImplementation((str, data) => {
                if (str === "Name: {user.name} {flat_val}") {
                    ...
                    if (!Object.prototype.hasOwnProperty.call(data, 'user.name')) {
                        logger.warn(`PlaceholderResolver: Placeholder "{user.name}" not found in provided data sources. Replacing with empty string.`);
                    }
                    const flatValResolved = Object.prototype.hasOwnProperty.call(data, 'flat_val') ? data.flat_val : "";
            ...
            const result = await promptBuilder.build("vendor/deep_placeholder_model", promptData);

            expect(mockPlaceholderResolver.resolve).toHaveBeenCalledWith("Name: {user.name} {flat_val}", promptData);
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('PlaceholderResolver: Placeholder "{user.name}" not found'));
