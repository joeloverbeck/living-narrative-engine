...
describe('EntityDisplayDataProvider', () => {
    ...
    describe('Constructor', () => {
        it('should throw an error if entityManager is missing or invalid', () => {
            expect(() => new EntityDisplayDataProvider({logger: mockLogger}))
                .toThrow("'entityManager' dependency is missing or invalid.");
            expect(() => new EntityDisplayDataProvider({entityManager: {}, logger: mockLogger}))
                .toThrow("'entityManager' dependency is missing or invalid.");
        });

        it('should throw an error if logger is missing or invalid', () => {
            expect(() => new EntityDisplayDataProvider({entityManager: mockEntityManager}))
                .toThrow("'logger' dependency is missing or invalid.");
            expect(() => new EntityDisplayDataProvider({entityManager: mockEntityManager, logger: {}}))
                .toThrow("'logger' dependency is missing or invalid.");
        });

        it('should instantiate successfully with valid dependencies', () => {
            expect(service).toBeInstanceOf(EntityDisplayDataProvider);
            expect(mockLogger.debug).toHaveBeenCalledWith('[EntityDisplayDataProvider] Service instantiated.');
        });
    });
    ...
    describe('getEntityName', () => {
        it('should return entity name from NAME_COMPONENT', () => {
            ...
            expect(mockEntityManager.getEntityInstance).toHaveBeenCalledWith('player1');
        ...
        it('should return entity.id if NAME_COMPONENT is missing or text is invalid', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity 'npcNoNameComp' found, but no valid NAME_COMPONENT_ID data. Returning entity ID as name."));
        ...
        it('should return defaultName if entity not found', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity with ID 'nonExistent' not found. Returning default name."));
        });

        it('should return defaultName if entityId is null or empty', () => {
            expect(service.getEntityName(null, 'Default')).toBe('Default');
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("called with null or empty entityId"));
    ...
    describe('getEntityPortraitPath', () => {
        it('should return correct portrait path', () => {
            const mockEntity = {
                ...
                getComponentData: jest.fn().mockReturnValue({imagePath: 'portraits/hero.png'}),
            };
            mockEntityManager.getEntityInstance.mockReturnValue(mockEntity);
            const path = service.getEntityPortraitPath('player1');
            expect(path).toBe('/data/mods/core/portraits/hero.png');
            expect(mockEntity.getComponentData).toHaveBeenCalledWith(PORTRAIT_COMPONENT_ID);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Constructed portrait path for 'player1': /data/mods/core/portraits/hero.png"));
        });

        it('should return null if entity not found', () => {
            mockEntityManager.getEntityInstance.mockReturnValue(null);
            expect(service.getEntityPortraitPath('nonExistent')).toBeNull();
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity with ID 'nonExistent' not found."));
        });
        ...
        it('should return null if PORTRAIT_COMPONENT is missing or imagePath is invalid', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity 'player1' has no valid PORTRAIT_COMPONENT_ID data or imagePath."));
        ...
        it('should return null if modId cannot be extracted from definitionId', () => {
            ...
            mockEntityManager.getEntityInstance.mockReturnValue(mockEntity);
            expect(service.getEntityPortraitPath('player1')).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Could not extract modId from definitionId 'invalidDefinitionId'"));
        });
    ...
    describe('getEntityDescription', () => {
        it('should return entity description from DESCRIPTION_COMPONENT', () => {
            ...
            expect(mockEntity.getComponentData).toHaveBeenCalledWith(DESCRIPTION_COMPONENT_ID);
        });

        it('should return defaultDescription if entity not found', () => {
            mockEntityManager.getEntityInstance.mockReturnValue(null);
            const desc = service.getEntityDescription('nonExistent', 'No description.');
        ...
        it('should return defaultDescription if DESCRIPTION_COMPONENT is missing', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity 'item1' found, but no valid DESCRIPTION_COMPONENT_ID data."));
        });

        it('should return defaultDescription if entityId is null or empty', () => {
            expect(service.getEntityDescription(null, 'Default Desc')).toBe('Default Desc');
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("called with null or empty entityId"));
        });
    });
    ...
    describe('getEntityLocationId', () => {
        it('should return locationId from POSITION_COMPONENT', () => {
            ...
            expect(mockEntity.getComponentData).toHaveBeenCalledWith(POSITION_COMPONENT_ID);
        });
        ...
        it('should return null if POSITION_COMPONENT or locationId is missing/invalid', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Entity 'player1' found, but no valid POSITION_COMPONENT_ID data or locationId."));
        ...
        it('should return compiled character info', () => {
            const mockEntity = {
                ...
                getComponentData: jest.fn((componentId) => {
                    if (componentId === NAME_COMPONENT_ID) return {text: 'Character Name'};
                    if (componentId === DESCRIPTION_COMPONENT_ID) return {text: 'Character Description'};
                    if (componentId === PORTRAIT_COMPONENT_ID) return {imagePath: 'path/to/image.png'};
                    return null;
                }),
            };
            mockEntityManager.getEntityInstance.mockReturnValue(mockEntity);
    ...
    describe('getLocationDetails', () => {
        it('should return compiled location details with exits', () => {
            const mockLocationEntity = {
                ...
                getComponentData: jest.fn((componentId) => {
                    ...
                    if (componentId === DESCRIPTION_COMPONENT_ID) return {text: 'A vast hall.'};
                    if (componentId === EXITS_COMPONENT_ID) return [
            ...
            expect(details).toEqual({
                ...
                exits: [
                    ...
                    {description: 'a secret passage', target: 'loc_secret', id: 'loc_secret'},
                    {description: 'Unspecified Exit', target: 'loc_no_dir', id: 'loc_no_dir'},
                    {description: 'Unspecified Exit', target: 'loc_empty_dir', id: 'loc_empty_dir'},
                ],
            });
            expect(mockEntityManager.getEntityInstance).toHaveBeenCalledWith('loc1');
            // Now these internal calls should happen:
            expect(mockLocationEntity.getComponentData).toHaveBeenCalledWith(NAME_COMPONENT_ID);
            expect(mockLocationEntity.getComponentData).toHaveBeenCalledWith(DESCRIPTION_COMPONENT_ID);
            expect(mockLocationEntity.getComponentData).toHaveBeenCalledWith(EXITS_COMPONENT_ID);
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid exit item in exits component for location 'loc1'"), expect.any(Object));
        });

        it('should return null if location entity not found', () => {
            mockEntityManager.getEntityInstance.mockReturnValue(null);
            expect(service.getLocationDetails('nonExistentLoc')).toBeNull();
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Location entity with ID 'nonExistentLoc' not found."));
        });
        ...
        it('should handle EXITS_COMPONENT_ID data being present but not an array', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(
                expect.stringContaining("Exits component data for location 'loc_bad_exits' is present but not an array."),
                expect.any(Object)
            );
        });
        ...
        it('should return null if locationEntityId is null or empty', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("called with null or empty locationEntityId"));
        });
    });
    ...
    describe('_getModIdFromDefinitionId', () => {
        it('should extract modId correctly', () => {
            ...
            expect(service._getModIdFromDefinitionId('myMod:someItem:variant')).toBe('myMod');
        });

        // ** FIX: Refactored assertions for this test case **
        it('should return null for invalid definitionId formats and log appropriately', () => {
            // Test case 1: String, but no colon (logs "Could not parse...")
            expect(service._getModIdFromDefinitionId('nodIdOnly')).toBeNull();
            expect(mockLogger.warn).toHaveBeenLastCalledWith(
                "[EntityDisplayDataProvider] _getModIdFromDefinitionId: Could not parse modId from definitionId 'nodIdOnly'. Expected format 'modId:entityName'."
            );

            // Test case 2: String, colon present, but empty modId part (logs "Could not parse...")
            expect(service._getModIdFromDefinitionId(':itemNoModId')).toBeNull();
            expect(mockLogger.warn).toHaveBeenLastCalledWith(
                "[EntityDisplayDataProvider] _getModIdFromDefinitionId: Could not parse modId from definitionId ':itemNoModId'. Expected format 'modId:entityName'."
            );
