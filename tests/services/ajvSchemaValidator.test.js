...
const commonSchema = {
  ...
  definitions: {
    namespacedId: {
      description:
        "A unique identifier string, typically namespaced using a colon (e.g., 'core:player', 'mod_combat:action_attack'). Allows alphanumeric characters, underscore, hyphen, and colon.",
      type: 'string',
      pattern: '^[a-zA-Z0-9_\\-:]+$',
    },
    DefinitionRefComponent: {
      description:
        'A generic component holding a single reference ID string, typically to a definition (e.g., an item definition ID on an instance, a character class definition).',
      type: 'object',
      required: ['id'],
      properties: {
        id: {
    ...
    nullableNamespacedId: {
      description:
        'A unique identifier string, typically namespaced (like namespacedId), or null.',
      oneOf: [
    ...
    NameComponent: {
      ...
      properties: {
        value: {
          type: 'string',
          description:
            "The primary display name or title of the entity (e.g., 'Player', 'Goblin Sentry', 'Iron Key').",
        },
      },
    },
    ...
    MetaDescriptionComponent: {
      ...
      properties: {
        keywords: {
          type: 'array',
          description:
            "A list of concise keywords or short phrases describing the location's key features, atmosphere, or contents (e.g., ['damp stone', 'flickering torchlight', 'smell of decay', 'distant dripping']). Used as input for LLM description generation.",
          items: { type: 'string' },
          minItems: 1,
        },
        style_hint: {
          type: 'string',
          description:
            "(Optional) Hint for the LLM about the desired writing style (e.g., 'brief', 'ominous', 'poetic').",
        },
      },
    },
    PositionComponent: {
      ...
      properties: {
        locationId: {
          $ref: '#/definitions/namespacedId',
          description:
            'The ID of the location entity where the entity resides.',
        },
        x: {
          description:
            'Optional x-coordinate within the location (if grid-based).',
          type: 'number',
          default: 0,
        },
        y: {
          description:
            'Optional y-coordinate within the location (if grid-based).',
          type: 'number',
          default: 0,
        },
      },
    ...
    InventoryComponent: {
      ...
      properties: {
        items: {
          type: 'array',
          description:
            "Array of item entity identifier strings currently in the entity's inventory.",
    ...
    AttackComponent: {
      ...
      properties: {
        ...
        attack_verb: {
          type: 'string',
          description:
            "Optional verb used in combat messages (e.g., 'attacks', 'claws', 'bites'). Defaults based on context if omitted.",
          default: 'attacks',
        },
      },
      required: ['damage'],
    ...
    SkillComponent: {
      ...
      properties: {
        skills: {
          type: 'object',
          description:
            "A map of skill IDs (e.g., 'core:skill_lockpicking') to their current integer value.",
    ...
    StatsComponent: {
      ...
      properties: {
        attributes: {
          type: 'object',
          description:
            "A map of attribute IDs (e.g., 'core:attr_strength') to their current integer value.",
          additionalProperties: { type: 'integer', minimum: 0 },
          propertyNames: { $ref: '#/definitions/namespacedId' },
    ...
    EntitiesPresentComponent: {
      type: 'object',
      description:
        'Lists the IDs of entities currently present within this entity (typically used for Locations or Containers).',
      properties: {
        entityIds: {
          type: 'array',
    ...
    ConnectionsComponent: {
      $comment:
        'This is a basic version. location.schema.json defines a more complex version specific to locations.',
      type: 'object',
      description:
        'Defines simple directional exits from one location to another.',
      properties: {
        exits: {
          type: 'object',
          description:
            'Map where keys are directions (or exit names) and values are target location IDs.',
    ...
    EquipmentComponent: {
      ...
      properties: {
        slots: {
          type: 'object',
          description:
            "A map where keys are slot IDs (e.g., 'core:slot_main_hand') and values are the entity ID string of the equipped item, or null if empty.",
    ...
    QuestLogComponent: {
      type: 'object',
      description:
        'Component for entities (typically the player) that track quest progress.',
      properties: {
        active_quests: {
          type: 'array',
    ...
    TypedParameterBase: {
      type: 'object',
      description:
        "Base structure for objects where a 'type' string dictates the structure of a 'parameters' object.",
      required: ['type', 'parameters'],
      properties: {
        type: {
          type: 'string',
          description:
            "Identifier determining the required structure of the 'parameters' object.",
        },
        parameters: {
          type: 'object',
          description:
            "Container for parameters specific to the 'type'. Specific properties defined in consuming schemas using oneOf/allOf.",
        },
      },
      additionalProperties: false,
    },
    eventDefinition: {
      type: 'object',
      description:
        'Defines an event structure containing a name/ID and optional data payload.',
      required: ['eventName'],
      properties: {
        eventName: {
          $ref: '#/definitions/namespacedId',
        ...
        eventData: {
          type: 'object',
          description:
            'Optional payload data object for the event. Structure depends on the specific event being fired.',
          additionalProperties: true,
        },
      },
      additionalProperties: false,
    ...
    PassageDetailsComponent: {
      /* ... definition using internal refs only ... */
    },
  },
};
// --- End complex schema ---
...
const invalidSimpleObjectData_ExtraProp = {
  name: 'Test',
  count: 1,
  extra: true,
}; // Fails additionalProperties
...
const invalidRequiredPropsData_WrongType = {
  id: 'req-789',
  value: 'not a number',
...
describe('AjvSchemaValidator', () => {
  ...
  describe('addSchema', () => {
    it('should resolve successfully when adding a valid schema with a unique ID', async () => {
      await expect(
        validator.addSchema(simpleObjectSchema, simpleObjectSchema.$id)
    ...
    it('should resolve successfully when adding the complex commonSchema', async () => {
      ...
      await expect(
        validator.addSchema(commonSchema, commonSchema.$id)
    ...
    it('should reject when attempting to add the same schema ID again', async () => {
      ...
      await expect(
        validator.addSchema(requiredPropsSchema, simpleObjectSchema.$id)
      ) // Using different schema but same ID
    ...
    it('should reject when attempting to add a structurally invalid schema', async () => {
      await expect(
        validator.addSchema(
          structurallyInvalidSchema,
          structurallyInvalidSchema.$id
        )
      ).rejects.toThrow(); // Ajv throws detailed errors for invalid schemas
      // Check it wasn't loaded
      expect(validator.isSchemaLoaded(structurallyInvalidSchema.$id)).toBe(
        false
      );
    });
    ...
    it('should reject when adding with non-string schemaId', async () => {
      await expect(
        validator.addSchema(simpleObjectSchema, 123)
      ).rejects.toThrow(/Invalid or empty schemaId/);
    });
  });
  ...
  describe('isSchemaLoaded', () => {
    ...
    it('should return false for a schema ID that has not been added', () => {
      expect(validator.isSchemaLoaded('test://schemas/non-existent')).toBe(
        false
      );
    });
  ...
  describe('getValidator', () => {
    ...
    it('should return undefined for a schema ID that has not been added', () => {
      expect(
        validator.getValidator('test://schemas/non-existent')
      ).toBeUndefined();
    });
    ...
    it('should return undefined if called after adding a schema failed', async () => {
      ...
      expect(
        validator.getValidator(structurallyInvalidSchema.$id)
      ).toBeUndefined();
    });
  ...
  describe('Returned Validator Function', () => {
    ...
    it('should return { isValid: false, errors: Array } for data missing required prop violating requiredPropsSchema', () => {
      const result = validateRequiredProps(
        invalidRequiredPropsData_MissingRequired
      );
      expect(result.isValid).toBe(false);
      expect(result.errors).toEqual(expect.any(Array));
      expect(result.errors.length).toBeGreaterThan(0);
