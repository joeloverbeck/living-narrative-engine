...
const createMockGameStateDto = ({hasActorState = true, hasActorPromptData = true} = {}) => {
    const dto = {};
    if (hasActorState) {
...
describe('GameStateValidationServiceForPrompting', () => {
    ...
    beforeEach(() => {
        ...
        service = new GameStateValidationServiceForPrompting({logger: mockLogger});
    });
    ...
    describe('Constructor', () => {
        it('should initialize with a logger', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith("GameStateValidationServiceForPrompting initialized.");
        });

        it('should throw an error if logger is not provided', () => {
            expect(() => new GameStateValidationServiceForPrompting({}))
                .toThrow("GameStateValidationServiceForPrompting: Logger dependency is required.");
            expect(() => new GameStateValidationServiceForPrompting({logger: null}))
                .toThrow("GameStateValidationServiceForPrompting: Logger dependency is required.");
        });
    });
    ...
    describe('validate', () => {
        it('should return invalid if gameStateDto is null', () => {
            ...
            expect(result.errorContent).toBe(ERROR_FALLBACK_CRITICAL_GAME_STATE_MISSING);
            expect(mockLogger.error).toHaveBeenCalledWith("GameStateValidationServiceForPrompting.validate: AIGameStateDTO is null or undefined.");
        });
        ...
        it('should return valid and warn if actorState is missing', () => {
            const dto = createMockGameStateDto({hasActorState: false, hasActorPromptData: true});
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith("GameStateValidationServiceForPrompting.validate: AIGameStateDTO is missing 'actorState'. This might affect prompt data completeness indirectly.");
        ...
        it('should return valid and warn if actorPromptData is missing', () => {
            const dto = createMockGameStateDto({hasActorState: true, hasActorPromptData: false});
            const result = service.validate(dto);
            expect(result.isValid).toBe(true); // Service logic considers this valid but warns
            expect(result.errorContent).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith("GameStateValidationServiceForPrompting.validate: AIGameStateDTO is missing 'actorPromptData'. Character info will be limited or use fallbacks.");
            expect(mockLogger.warn).toHaveBeenCalledTimes(1); // Ensure only this warning
            expect(mockLogger.error).not.toHaveBeenCalled();
        });

        it('should return valid and warn if both actorState and actorPromptData are missing', () => {
            const dto = createMockGameStateDto({hasActorState: false, hasActorPromptData: false});
            const result = service.validate(dto);
            expect(result.isValid).toBe(true); // Service logic considers this valid but warns
            expect(result.errorContent).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith("GameStateValidationServiceForPrompting.validate: AIGameStateDTO is missing 'actorState'. This might affect prompt data completeness indirectly.");
        ...
        it('should return valid if gameStateDto is valid and complete (has actorState and actorPromptData)', () => {
            const dto = createMockGameStateDto({hasActorState: true, hasActorPromptData: true});
            const result = service.validate(dto);
