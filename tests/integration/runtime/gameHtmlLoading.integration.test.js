...
describe('Game HTML Runtime Loading', () => {
  ...
  it('should be able to import endpointConfig without process.env errors', async () => {
    ...
    try {
      ...
      const { getEndpointConfig } = await import('../../../src/config/endpointConfig.js');
      ...
      expect(config.getLlmRequestEndpoint()).toBe('http://localhost:3001/api/llm-request');
  ...
  it('should simulate game.html bootstrap flow without errors', async () => {
    ...
    expect(consoleErrors.filter(msg => msg.includes('process is not defined'))).toHaveLength(0);
    ...
    if (bootstrapError && !bootstrapError.message.includes('process is not defined')) {
  ...
  it('should not throw process.env related errors during logging config initialization', async () => {
    ...
    try {
      ...
      const { DEFAULT_CONFIG } = await import('../../../src/logging/config/defaultConfig.js');
