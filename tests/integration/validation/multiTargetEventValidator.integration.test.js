...
describe('MultiTargetEventValidator - Integration Tests', () => {
  ...
  describe('Performance and Metrics Integration', () => {
    it('should track performance metrics across multiple validations', () => {
      const baseEvent = {
        eventName: 'core:attempt_action',
      ...
      expect(metrics.averageTime).toBeLessThan(10); // Should be fast
    });

    it('should handle performance degradation warnings', () => {
    ...
    it('should integrate with logging system for performance monitoring', () => {
      const mockLogger = {
        debug: jest.fn(),
        info: jest.fn(),
        warn: jest.fn(),
      ...
      testValidator.validateEvent(testEvent);

      // Logger should be called for debugging or warnings if performance is slow
  ...
  describe('Integration Stress Testing', () => {
    it('should handle high-volume validation requests', () => {
      const baseEvent = {
        eventName: 'core:attempt_action',
      ...
      expect(metrics.averageTime).toBeLessThan(1); // Less than 1ms per validation
    });
