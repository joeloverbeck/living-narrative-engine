...
describe('Operation Schema Registration Validation', () => {
  ...
  describe('Rule File Validation', () => {
    it('should validate entity_thought.rule.json without cascade errors', async () => {
      // Arrange
      const ruleDataContent = await readFile('data/mods/core/rules/entity_thought.rule.json', 'utf8');
    ...
    it('should validate handle_sit_down.rule.json without cascade errors', async () => {
      // Arrange
      const ruleDataContent = await readFile('data/mods/positioning/rules/handle_sit_down.rule.json', 'utf8');
      const ruleData = JSON.parse(ruleDataContent);
  ...
  describe('Missing Operation Schema Detection', () => {
    it('should detect all operation schemas are registered in operation.schema.json', async () => {
      ...
      const operationSchemaContent = await fs.readFile('data/schemas/operation.schema.json', 'utf8');
      const registeredSchemas = [];
      const refMatches = operationSchemaContent.match(/"\.\/operations\/[^"]+\.schema\.json"/g);
      if (refMatches) {
        registeredSchemas.push(...refMatches.map(match => match.slice(3, -1))); // Remove "./ prefix and quotes // Remove quotes
      }

      // Assert - Find missing registrations
      const missingSchemas = operationSchemas.filter(schema => !registeredSchemas.includes(schema));
