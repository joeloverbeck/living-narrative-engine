...
describe('ModCrossReferenceValidator - Integration', () => {
  ...
  describe('Real Component Integration', () => {
    ...
    it('should integrate with real reference extractor for file processing', async () => {
      ...
      const report = await validator.validateModReferences(modPath, manifestsMap);
      
      expect(report.hasViolations).toBe(true);
      expect(report.violations.length).toBeGreaterThan(0);
      
      // Verify that positioning references were detected
      const positioningViolations = report.violations.filter(v => v.referencedMod === 'positioning');
      expect(positioningViolations.length).toBeGreaterThan(0);
      
      // Check specific components were found
      const componentNames = positioningViolations.map(v => v.referencedComponent);
  ...
  describe('Real-world Scenarios', () => {
    it('should handle real intimacy mod referencing positioning components', async () => {
      ...
      const report = await validator.validateModReferences(intimacyPath, manifestsMap);
      ...
      positioningViolations.forEach(violation => {
        ...
        expect(violation.suggestedFix).toContain('Add "positioning" to dependencies');
      });
    });
    ...
    it('should handle complex multi-mod ecosystem validation', async () => {
      ...
      const modConfigs = {
        ...
        anatomy: { files: [], dependencies: [{ id: 'core', version: '^1.0.0' }] },
        positioning: { 
          files: [], 
          dependencies: [{ id: 'core', version: '^1.0.0' }] 
        },
      ...
      expect(results.get('intimacy').missingDependencies).toContain('positioning');
    ...
    it('should generate comprehensive ecosystem report', async () => {
      ...
      expect(report).toContain('Living Narrative Engine - Cross-Reference Validation Report');
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle missing mod directories gracefully', async () => {
      ...
      const validationFailureCalls = errorCalls.filter(call => 
        call[0] === 'Failed to validate mod missing-mod'
      );
  ...
  describe('Integration with File System', () => {
    it('should handle different file types in mod directory', async () => {
      ...
      expect(report.hasViolations).toBe(true);
      expect(report.referencedMods).toEqual(expect.arrayContaining(['positioning', 'intimacy']));
      expect(report.violations.length).toBeGreaterThan(0);
