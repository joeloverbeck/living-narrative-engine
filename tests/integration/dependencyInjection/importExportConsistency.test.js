  ...
  describe('Dependency Injection Services', () => {
    it('should have consistent import/export patterns for anatomy services', () => {
      ...
      for (const service of anatomyServices) {
        ...
        try {
          ...
          const hasDefaultExport = content.includes(`export default ${service.className}`);
          
          if (!hasDefaultExport) {
            // Check if it's a named export instead
            const hasNamedExport = content.includes(`export { ${service.className} }`) ||
                                   content.includes(`export class ${service.className}`);
            
            if (hasNamedExport) {
              errors.push(`${service.file} exports ${service.className} as named export but should use default export`);
            } else {
              errors.push(`${service.file} does not export ${service.className}`);
            }
          }
        } catch (err) {
      ...
      const registrationFile = path.join(projectRoot, 'src/dependencyInjection/registrations/worldAndEntityRegistrations.js');
      try {
        ...
        for (const service of anatomyServices) {
          ...
          if (content.includes(incorrectImport) && content.includes(service.file.replace('src/', '../../'))) {
            errors.push(`worldAndEntityRegistrations.js uses named import for ${service.className} but should use default import`);
          }
        }
      } catch (err) {
        errors.push(`Could not read worldAndEntityRegistrations.js: ${err.message}`);
      }
    ...
    it('should verify critical anatomy services are imported correctly', () => {
      ...
      for (const serviceName of defaultImports) {
        const hasDefaultImport = new RegExp(`import\\s+${serviceName}\\s+from`).test(content);
