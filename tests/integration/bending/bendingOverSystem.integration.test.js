...
describe('Bending Over System Integration', () => {
  ...
  beforeEach(() => {
    ...
    // Extend the scope resolver to handle bending:available_surfaces
    ...
    testEnv.unifiedScopeResolver.resolveSync = (scopeName, context) => {
      if (scopeName === 'bending:available_surfaces') {
        // Find all entities with bending:allows_bending_over in same location as actor
  ...
  describe('Closeness Management', () => {
    ...
    it('should handle straightening up from shared surface', async () => {
      ...
      testEnv.entityManager.addComponent(actor2, 'bending-states:bending_over', {
        surface_id: counter,
      });
