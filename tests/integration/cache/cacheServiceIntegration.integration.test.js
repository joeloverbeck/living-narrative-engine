...
describe('Cache Service Integration with DI Container', () => {
  ...
  beforeEach(() => {
    ...
    const mockClothingManagementService = testBed.createMock('ClothingManagementService', [
      'equipClothing', 'unequipClothing', 'getEquippedItems', 'validateCompatibility'
    ]);
    const mockEquipmentOrchestrator = testBed.createMock('EquipmentOrchestrator', [
      'orchestrateEquipment', 'validateEquipment'
    ]);
    const mockLayerCompatibilityService = testBed.createMock('LayerCompatibilityService', [
      'checkCompatibility', 'getConflicts'
    ]);
    const mockClothingSlotValidator = testBed.createMock('ClothingSlotValidator', [
      'validate', 'getAvailableSlots'
    ]);

    container.register(tokens.ClothingManagementService, () => mockClothingManagementService, { lifecycle: 'singleton' });
    container.register(tokens.EquipmentOrchestrator, () => mockEquipmentOrchestrator, { lifecycle: 'singleton' });
    container.register(tokens.LayerCompatibilityService, () => mockLayerCompatibilityService, { lifecycle: 'singleton' });
    container.register(tokens.ClothingSlotValidator, () => mockClothingSlotValidator, { lifecycle: 'singleton' });

    // These are required by IAnatomySystemFacade
    const mockBodyGraphService = testBed.createMock('BodyGraphService', [
      'createGraph', 'updateGraph', 'queryGraph'
    ]);
    const mockAnatomyDescriptionService = testBed.createMock('AnatomyDescriptionService', [
      'generateDescription', 'formatDescription'
    ]);
    const mockGraphIntegrityValidator = testBed.createMock('GraphIntegrityValidator', [
      'validate', 'checkIntegrity'
    ]);
    const mockAnatomyGenerationService = testBed.createMock('AnatomyGenerationService', [
      'generateAnatomy', 'createBodyPart'
    ]);
    const mockBodyBlueprintFactory = testBed.createMock('BodyBlueprintFactory', [
      'createBlueprint', 'validateBlueprint'
    ]);

    container.register(tokens.BodyGraphService, () => mockBodyGraphService, { lifecycle: 'singleton' });
    container.register(tokens.AnatomyDescriptionService, () => mockAnatomyDescriptionService, { lifecycle: 'singleton' });
    container.register(tokens.GraphIntegrityValidator, () => mockGraphIntegrityValidator, { lifecycle: 'singleton' });
    container.register(tokens.AnatomyGenerationService, () => mockAnatomyGenerationService, { lifecycle: 'singleton' });
    container.register(tokens.BodyBlueprintFactory, () => mockBodyBlueprintFactory, { lifecycle: 'singleton' });
  ...
  describe('Service Registration and Resolution', () => {
    it('should register all cache services as singletons', () => {
      ...
      expect(container.isRegistered(tokens.ICacheInvalidationManager)).toBe(true);
      expect(container.isRegistered(tokens.CacheInvalidationManager)).toBe(true);
    ...
    it('should resolve cache services with proper dependencies', () => {
      ...
      const cacheInvalidationManager = container.resolve(tokens.ICacheInvalidationManager);
      ...
      expect(container.resolve(tokens.ICacheInvalidationManager)).toBe(cacheInvalidationManager);
    ...
    it('should resolve concrete implementations through interfaces', () => {
      ...
      const interfaceManager = container.resolve(tokens.ICacheInvalidationManager);
      const concreteManager = container.resolve(tokens.CacheInvalidationManager);
  ...
  describe('Service Integration Workflow', () => {
    it('should support complete cache workflow through DI', () => {
      ...
      const invalidationManager = container.resolve(tokens.ICacheInvalidationManager);
      const metricsService = container.resolve(tokens.ICacheMetrics);
      const eventDispatcher = container.resolve(tokens.IValidatedEventDispatcher);
