...
describe('Cache Infrastructure Integration', () => {
  ...
  beforeEach(async () => {
    ...
    unifiedCache = new UnifiedCache({ logger: mockLogger }, {
      maxSize: 50,
      ttl: 5000,
      evictionPolicy: 'lru',
      enableMetrics: true,
  ...
  describe('End-to-End Cache Workflow', () => {
    it('should handle complete cache lifecycle with events', async () => {
      ...
      expect(unifiedCache.get('actor:player')).toEqual({ name: 'Hero', level: 1 });
      expect(unifiedCache.get('item:sword')).toEqual({ name: 'Iron Sword', damage: 10 });
      ...
      expect(unifiedCache.get('item:sword')).toEqual({ name: 'Iron Sword', damage: 10 }); // Still cached
    ...
    it('should handle pattern-based invalidation across multiple caches', () => {
      ...
      const itemCache = new UnifiedCache({ logger: mockLogger }, {
        maxSize: 30,
        evictionPolicy: 'lfu'
      });
      ...
      const totalInvalidated = Object.values(result).reduce((total, r) => total + (r.invalidated || 0), 0);
  ...
  describe('Event System Integration', () => {
    it('should handle manual invalidation (event listeners not connected)', async () => {
      ...
      expect(unifiedCache.get('actor:npc2')).toEqual({ name: 'Merchant', faction: 'neutral' });
    ...
    it('should handle pattern-based invalidation for components', async () => {
      ...
      unifiedCache.set('actor:player:inventory', { items: ['sword', 'potion'] });
      ...
      expect(unifiedCache.get('actor:player:inventory')).toEqual({ items: ['sword', 'potion'] });
    });

    it('should handle event validation errors gracefully', async () => {
      mockSchemaValidator.validate.mockReturnValue({ valid: false, errors: ['Invalid payload'] });
  ...
  describe('Cache Strategy Integration', () => {
    it('should work with different eviction strategies', () => {
      ...
      const lruCache = new UnifiedCache({ logger: mockLogger }, {
        maxSize: 3,
        evictionPolicy: 'lru'
      });
      
      const lfuCache = new UnifiedCache({ logger: mockLogger }, {
        maxSize: 3,
        evictionPolicy: 'lfu'
      });
  ...
  describe('Performance and Memory Integration', () => {
    it('should monitor memory usage across cache strategies', () => {
      ...
      const memoryCache1 = new UnifiedCache({ logger: mockLogger }, {
        maxSize: 100,
        maxMemoryUsage: 1024 * 1024, // 1MB
        evictionPolicy: 'lru'
      });
      
      const memoryCache2 = new UnifiedCache({ logger: mockLogger }, {
        maxSize: 50,
        maxMemoryUsage: 512 * 1024, // 512KB
        evictionPolicy: 'lfu'
      });
      
      cacheMetrics.registerCache('memory-cache-1', memoryCache1, { category: 'large' });
      cacheMetrics.registerCache('memory-cache-2', memoryCache2, { category: 'small' });
      
      // Add data to caches
      for (let i = 0; i < 10; i++) {
        memoryCache1.set(`key${i}`, { data: `large-object-${i}`, size: Array(100).fill('x').join('') });
  ...
  describe('Error Recovery and Resilience', () => {
    ...
    it('should recover from event dispatcher failures', async () => {
      ...
      validatedEventDispatcher.dispatch = jest.fn().mockRejectedValue(new Error('Dispatcher error'));
