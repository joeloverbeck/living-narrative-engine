...
describe('Debug multiActor', () => {
  ...
  it('should verify basic setup works', async () => {
    ...
    setup.gameDataRepository.get = jest.fn((key) => {
      getCallCount++;
      console.log(`[DEBUG] gameDataRepository.get call #${getCallCount} with key:`, key);
      if (key === 'tasks') {
        const result = {
          test: {
        ...
        console.log('[DEBUG] First task object:', result.test['test:simple_task']);
        return result;
      }
    ...
    setup.gameDataRepository.getTask = jest.fn((taskId) => {
      console.log('[DEBUG] gameDataRepository.getTask called with taskId:', taskId);
    ...
    const plannerLogger = setup.planner['#logger'] || setup.planner._logger || setup.controller['#planner']?.['#logger'];
    if (!plannerLogger) {
      // Try to access through private field reflection
      const plannerInstance = setup.controller['#planner'] || setup.planner;
      const loggerSpy = jest.fn((msg, ...args) => console.log('[PLANNER LOG]', msg, ...args));
    ...
    console.log('[DEBUG] hasEntity check:', setup.entityManager.hasEntity(actor.id));
    console.log('[DEBUG] getEntityInstance:', setup.entityManager.getEntityInstance(actor.id));
    ...
    const directPlanResult = setup.planner.plan(actor.id, registeredGoal, {}, {});
    ...
    console.log('[DEBUG] Repository.get call count:', setup.gameDataRepository.get.mock.calls.length);
    console.log('[DEBUG] Repository.get calls:', setup.gameDataRepository.get.mock.calls);
