...
describe('Anatomy Visualizer Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockDocument = {
      getElementById: jest.fn((id) => {
        switch (id) {
          case 'entity-selector': return mockEntitySelector;
          case 'anatomy-graph-container': return mockGraphContainer;
          case 'entity-description-content': return mockDescriptionContent;
          default: return null;
        }
      }),
  ...
  describe('initialization', () => {
    ...
    it('should handle errors when populating entity selector', async () => {
      ...
      expect(mockEntitySelector.innerHTML).toBe('<option value="">Error loading entities</option>');
    });
  });
  ...
  describe('entity loading', () => {
    ...
    it('should clean up previous entities before loading new one', async () => {
      // Arrange
      visualizerUI._createdEntities = ['old-entity-1', 'old-part-1', 'old-part-2'];
      ...
      mockRegistry.getEntityDefinition.mockReturnValue(mockDefinition);
      mockEntityManager.createEntityInstance.mockResolvedValue({ id: 'new-instance' });
      ...
      expect(mockEntityManager.removeEntityInstance).toHaveBeenNthCalledWith(1, 'old-part-2');
      expect(mockEntityManager.removeEntityInstance).toHaveBeenNthCalledWith(2, 'old-part-1');
      expect(mockEntityManager.removeEntityInstance).toHaveBeenNthCalledWith(3, 'old-entity-1');
    });
  });
  ...
  describe('event handling', () => {
    it('should load entity when selector changes', async () => {
      // Arrange
      let changeHandler;
      mockEntitySelector.addEventListener.mockImplementation((event, handler) => {
