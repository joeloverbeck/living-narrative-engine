...
describe('intimacy:handle_kiss_cheek rule integration', () => {
  beforeEach(() => {
    logger = {
      debug: jest.fn(),
      info: jest.fn(),
    ...
    const expandedRule = {
      ...
      actions: expandMacros(
        JSON.parse(JSON.stringify(kissCheekRule.actions)),
        macroRegistry,
        logger
      ),
    };
  ...
  it('validates the rule against the schema', () => {
    ...
    const valid = ajv.validate(ruleSchema, kissCheekRule);
    if (!valid) {
  ...
  it('should dispatch correct third-person events for actor and observers', () => {
    ...
    const expectedMessage = "Hero leans in to kiss Friend's cheek softly.";
  ...
  it('should function gracefully if name or position components are missing', () => {
    ...
    const expectedMessage =
      "Unnamed Character leans in to kiss Friend's cheek softly.";
