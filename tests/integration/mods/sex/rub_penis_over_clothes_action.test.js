...
import rubPenisOverClothesRule from '../../../../data/mods/sex/rules/handle_rub_penis_over_clothes.rule.json';
import eventIsActionRubPenisOverClothes from '../../../../data/mods/sex/conditions/event-is-action-rub-penis-over-clothes.condition.json';
...
/**
 * Creates handlers needed for the rub_penis_over_clothes rule.
 *
...
describe('sex:rub_penis_over_clothes action integration', () => {
  ...
  beforeEach(() => {
    ...
    const expanded = expandMacros(rubPenisOverClothesRule.actions, {
    ...
    const dataRegistry = {
      getAllSystemRules: jest
        .fn()
        .mockReturnValue([{ ...rubPenisOverClothesRule, actions: expanded }]),
      getConditionDefinition: jest.fn((id) =>
        id === 'sex:event-is-action-rub-penis-over-clothes'
          ? eventIsActionRubPenisOverClothes
          : undefined
      ),
    };
    ...
    testEnv = createRuleTestEnvironment({
      createHandlers,
      entities: [],
      rules: [{ ...rubPenisOverClothesRule, actions: expanded }],
      dataRegistry,
    });
  });
  ...
  it('performs rub penis over clothes action successfully', async () => {
    ...
    await testEnv.eventBus.dispatch(ATTEMPT_ACTION_ID, {
      ...
      targetId: 'bob',
      originalInput: 'rub_penis_over_clothes bob',
    });
  ...
  it('rule structure matches expected pattern', async () => {
    ...
    expect(rubPenisOverClothesRule.rule_id).toBe(
      'handle_rub_penis_over_clothes'
    );
    expect(rubPenisOverClothesRule.event_type).toBe('core:attempt_action');
    expect(rubPenisOverClothesRule.condition.condition_ref).toBe(
    ...
    const lastAction =
      rubPenisOverClothesRule.actions[
        rubPenisOverClothesRule.actions.length - 1
      ];
