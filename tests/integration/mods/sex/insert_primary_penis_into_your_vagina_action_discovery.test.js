/**
 * @file Integration tests for sex:insert_primary_penis_into_your_vagina action discovery.
...
import insertPrimaryPenisAction from '../../../../data/mods/sex/actions/insert_primary_penis_into_your_vagina.action.json';

describe('sex:insert_primary_penis_into_your_vagina action discovery', () => {
  let testFixture;
   ...
   * @param {boolean} [options.coverPenis=false] - Whether clothing covers the target's penis.
   * @param {boolean} [options.coverVagina=false] - Whether clothing covers the actor's vagina.
   * @param {boolean} [options.includePenis=true] - Whether to include a penis anatomy part for the target.
   * @param {boolean} [options.includeVagina=true] - Whether to include a vagina anatomy part for the actor.
   * @param {boolean} [options.actorSitting=false] - Whether the actor is sitting on furniture.
   * @param {boolean} [options.actorBeingFucked=false] - Whether the actor starts with the penetration state component.
  ...
  function buildScenario(options = {}) {
    ...
    const actorPelvisId = includeVagina ? 'actorPelvisWithVagina1' : 'actorPelvisNoVagina1';
    const actorVaginaId = `${actorPelvisId}_vagina`;
    const targetGroinId = includePenis ? 'targetGroinWithPenis1' : 'targetGroinNoPenis1';
    ...
    if (actorBeingFucked) {
      actorBuilder.withComponent('sex:being_fucked_vaginally', { actorId: 'dorian' });
    }
    ...
    const targetGroin = new ModEntityBuilder(targetGroinId)
      .asBodyPart({ parent: null, children: targetGroinChildren, subType: 'groin' })
      .build();
  ...
  describe('Action discovery scenarios', () => {
    it('appears when penis is uncovered and partners are close', async () => {
      ...
      const foundAction = actions.find(
        (action) => action.id === 'sex:insert_primary_penis_into_your_vagina'
      );
