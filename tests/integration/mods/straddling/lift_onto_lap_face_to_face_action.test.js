...
describe('lift_onto_lap_face_to_face - Action Execution', () => {
  ...
  describe('Component Manipulation', () => {
    ...
    it('should add straddling_waist component to PRIMARY target with facing_away=false', async () => {
      ...
      expect(
        updatedTarget.components['straddling-states:straddling_waist'].target_id
      ).toBe('actor1');
      expect(
        updatedTarget.components['straddling-states:straddling_waist'].facing_away
      ).toBe(false);
    });
    ...
    it('should keep ACTOR sitting_on component unchanged', async () => {
      ...
      expect(updatedActor.components['sitting-states:sitting_on'].spot_index).toBe(
        0
      );
    });
