...
describe('Mouth Engagement Operations - System Integration', () => {
  ...
  describe('Event System Integration', () => {
    it('should complete operations successfully with event bus integration', async () => {
      const actorId = await createTestActorWithMouth('event_actor', 'EventActor');
      const eventsSeen = [];
    ...
    it('should handle event dispatch failures gracefully', async () => {
      const actorId = await createTestActorWithMouth('fail_event_actor', 'FailEventActor');
    ...
    it('should handle sequential lock and unlock operations correctly', async () => {
      const actorId = await createTestActorWithMouth('multi_event_actor', 'MultiEventActor');
      const allEvents = [];
    ...
    it('should handle multiple event subscribers correctly', async () => {
      const actorId = await createTestActorWithMouth('multi_sub_actor', 'MultiSubActor');
      const subscriber1Events = [];
      const subscriber2Events = [];
      const subscriber3Events = [];
      
      // Multiple subscribers
      const unsub1 = eventBus.subscribe('*', (event) => subscriber1Events.push(event));
      const unsub2 = eventBus.subscribe('*', (event) => subscriber2Events.push(event));
      const unsub3 = eventBus.subscribe('*', (event) => subscriber3Events.push(event));
  ...
  describe('Error Handling Integration', () => {
    ...
    it('should handle corrupted entity data gracefully', async () => {
      const actorId = await createTestActorWithMouth('corrupt_actor', 'CorruptActor');
    ...
    it('should recover from partial operation failures', async () => {
      const actorId = await createTestActorWithMouth('partial_fail_actor', 'PartialFailActor');
    ...
    it('should handle operation registry errors', async () => {
      ...
      const actorId = await createTestActorWithMouth('faulty_actor', 'FaultyActor');
  ...
  describe('System State Consistency', () => {
    it('should maintain consistent state across operation failures', async () => {
      ...
      if (mouthParts.length > 0) {
        ...
        entityManager.removeComponent(mouthParts[0].partId, 'core:mouth_engagement');
      }
    ...
    it('should maintain data integrity during concurrent modifications', async () => {
      const actorId = await createTestActorWithMouth('concurrent_mod_actor', 'ConcurrentModActor');
    ...
    it('should handle system-wide cleanup correctly', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const actorId = await createTestActorWithMouth(`cleanup_${i}`, `CleanupActor${i}`);
        actors.push(actorId);
  ...
  describe('Integration with Utility Functions', () => {
    it('should integrate correctly with getMouthParts utility', async () => {
      const actorId = await createTestActorWithMouth('utility_actor', 'UtilityActor');
    ...
    it('should integrate correctly with isMouthLocked utility', async () => {
      const actorId = await createTestActorWithMouth('locked_check_actor', 'LockedCheckActor');
