...
/**
 * Creates handlers needed for the suck_on_tongue rule.
 *
...
describe('intimacy:suck_on_tongue action integration', () => {
  ...
  beforeEach(() => {
    ...
    const expanded = expandMacros(suckOnTongueRule.actions, {
    ...
    const dataRegistry = {
      getAllSystemRules: jest
        .fn()
        .mockReturnValue([{ ...suckOnTongueRule, actions: expanded }]),
      getConditionDefinition: jest.fn((id) => {
        switch (id) {
          case 'intimacy:event-is-action-suck-on-tongue':
            return eventIsActionSuckOnTongue;
          default:
            return undefined;
        }
      }),
    ...
    testEnv = createRuleTestEnvironment({
      createHandlers,
      entities: [],
      rules: [{ ...suckOnTongueRule, actions: expanded }],
      dataRegistry,
    });
  });
  ...
  it('perception log shows correct message for suck on tongue', async () => {
    ...
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Emma has sucked on David's tongue."
    );
