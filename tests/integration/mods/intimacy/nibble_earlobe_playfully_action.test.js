...
/**
 * Creates handlers needed for the nibble_earlobe_playfully rule.
 *
...
describe('intimacy:nibble_earlobe_playfully action integration', () => {
  ...
  beforeEach(() => {
    ...
    const expanded = expandMacros(nibbleEarlobePlayfullyRule.actions, {
    ...
    const dataRegistry = {
      getAllSystemRules: jest
        .fn()
        .mockReturnValue([
          { ...nibbleEarlobePlayfullyRule, actions: expanded },
        ]),
      getConditionDefinition: jest.fn((id) =>
        id === 'intimacy:event-is-action-nibble-earlobe-playfully'
    ...
    testEnv = createRuleTestEnvironment({
      createHandlers,
      entities: [],
      rules: [{ ...nibbleEarlobePlayfullyRule, actions: expanded }],
      dataRegistry,
    });
  });
  ...
  it('successfully executes nibble earlobe playfully action between close actors', async () => {
    ...
    await testEnv.eventBus.dispatch(ATTEMPT_ACTION_ID, {
      ...
      targetId: 'target1',
      originalInput: 'nibble_earlobe_playfully target1',
    });
    ...
    expect(successEvent.payload.message).toBe(
      "Alice nibbles on Bob's earlobe playfully."
    );
  ...
  it('perception log shows correct message for nibble earlobe playfully action', async () => {
    ...
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Sarah nibbles on James's earlobe playfully."
    );
  ...
  it('handles multiple close partners correctly', async () => {
    ...
    await testEnv.eventBus.dispatch(ATTEMPT_ACTION_ID, {
      ...
      actionId: 'intimacy:nibble_earlobe_playfully',
      targetId: 'target2',
      originalInput: 'nibble_earlobe_playfully target2',
    });
    ...
    expect(perceptibleEvent).toBeDefined();
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Alice nibbles on Charlie's earlobe playfully."
    );
  });
  ...
  it('generates proper perceptible event for observers', async () => {
    ...
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Elena nibbles on Marcus's earlobe playfully."
    );
  ...
  it('validates perceptible event message matches action success message', async () => {
    ...
    expect(successEvent.payload.message).toBe(
      "Diana nibbles on Victor's earlobe playfully."
    );
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Diana nibbles on Victor's earlobe playfully."
    );
  ...
  it('works correctly when actor is behind target', async () => {
    ...
    expect(successEvent).toBeDefined();
    expect(successEvent.payload.message).toBe(
      "Emma nibbles on Liam's earlobe playfully."
    );
  });
});
