...
describe('intimacy:nibble_lower_lip action integration', () => {
  ...
  it('works with different entity names and locations', async () => {
    ...
    // Check success event
    const successEvent2 = testFixture.events.find(
      (e) => e.eventType === 'core:display_successful_action_result'
    );
    expect(successEvent2).toBeDefined();
    expect(successEvent2.payload.message).toBe(
  ...
  it('works regardless of initiator status (no prerequisites)', async () => {
    ...
    for (const testCase of testCases) {
      ...
      const actorId =
        testCase.actorName === 'Alice' ? scenario.actor.id : scenario.target.id;
      const targetId =
        testCase.targetName === 'Alice'
          ? scenario.actor.id
          : scenario.target.id;
