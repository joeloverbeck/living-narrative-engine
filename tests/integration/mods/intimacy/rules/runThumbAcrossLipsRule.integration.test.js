/**
 * @file Integration tests for the intimacy:run_thumb_across_lips rule.
 */
...
describe('handle_run_thumb_across_lips rule integration', () => {
  let testFixture;
  ...
  it('condition evaluates correctly', () => {
    ...
    const condition = eventIsActionRunThumbAcrossLips.logic;
    ...
    const matchingData = {
      event: {
        payload: {
          actionId: 'intimacy:run_thumb_across_lips',
        },
      },
    };
  ...
  it('performs run thumb across lips action successfully', async () => {
  ...
  it('perceptible event contains correct message', async () => {
    ...
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "Alice runs a thumb across Beth's lips."
    );
  });
  ...
  it('works with different actor and target names', async () => {
    ...
    expect(perceptibleEvent).toBeDefined();
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "John runs a thumb across Mary's lips."
    );
  });

  it('works with names containing apostrophes', async () => {
    const scenario = testFixture.createCloseActors(["O'Brien", 'Sarah']);
    ...
    expect(perceptibleEvent).toBeDefined();
    expect(perceptibleEvent.payload.descriptionText).toBe(
      "O'Brien runs a thumb across Sarah's lips."
    );
  });
});
