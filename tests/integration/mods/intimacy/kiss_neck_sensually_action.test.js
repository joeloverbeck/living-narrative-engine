...
describe('intimacy:kiss_neck_sensually action integration', () => {
  ...
  it('handles multiple close partners correctly', async () => {
    ...
    expect(perceptibleEvent1.payload.descriptionText).toBe("Alice kisses Bob's neck sensually.");
    ...
    expect(perceptibleEvent2.payload.descriptionText).toBe("Alice kisses Charlie's neck sensually.");
  });
  ...
  it('generates proper perceptible event for observers', async () => {
    ...
    expect(perceptibleEvent.payload.descriptionText).toBe("Elena kisses Marcus's neck sensually.");
