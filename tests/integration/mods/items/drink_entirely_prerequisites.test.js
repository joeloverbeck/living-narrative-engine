 ...
 * @see data/mods/items/actions/drink_entirely.action.json
...
import drinkEntirelyAction from '../../../../data/mods/items/actions/drink_entirely.action.json';
...
describe('items:drink_entirely prerequisites', () => {
  ...
  describe('action definition structure', () => {
    ...
    test('should reference anatomy:actor-has-free-grabbing-appendage condition', () => {
      expect(drinkEntirelyAction.prerequisites.length).toBeGreaterThan(0);
      const prerequisite = drinkEntirelyAction.prerequisites[0];
  ...
  describe('prerequisite evaluation - free grabbing appendage available', () => {
    test('should pass when actor has exactly one free grabbing appendage', () => {
      const actor = { id: 'thirsty123', components: {} };
      ...
      const result = prerequisiteService.evaluate(
        drinkEntirelyAction.prerequisites,
        drinkEntirelyAction,
        actor
      );
