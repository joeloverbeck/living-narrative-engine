/**
 * @file Integration tests verifying affection-embracing:link_arms is forbidden when target has giving_blowjob component.
...
describe('affection-embracing:link_arms - giving_blowjob forbidden component', () => {
  let testFixture;
  ...
  beforeEach(async () => {
    ...
    configureActionDiscovery = async () => {
      ...
      scopeResolver.resolveSync = (scopeName, context) => {
        if (
          scopeName ===
          'affection-embracing:actors_with_arm_subtypes_facing_each_other_or_behind_target'
        ) {
  ...
  describe('Three-actor scenario: Actor with receiving_blowjob can target others', () => {
    it('should be available to actor with receiving_blowjob when targeting a third actor without giving_blowjob', async () => {
      ...
      actorData.entity.components['sex-states:receiving_blowjob'] = {
        giving_entity_id: targetGivingData.entity.id,
        consented: true,
      };

      // Blair is giving a blowjob to Alex
      targetGivingData.entity.components['sex-states:giving_blowjob'] = {
