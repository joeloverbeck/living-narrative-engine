/**
 * @file Integration tests for the hugging:release_self_from_hug_forceful action and rule.
 * @description Verifies forceful self-release state management, messaging, and discovery recovery for hugged actors.
 */
...
import releaseSelfForcefulRule from '../../../../data/mods/hugging/rules/handle_release_self_from_hug_forceful.rule.json';
import releaseSelfForcefulCondition from '../../../../data/mods/hugging/conditions/event-is-action-release-self-from-hug-forceful.condition.json';
import releaseSelfForcefulAction from '../../../../data/mods/hugging/actions/release_self_from_hug_forceful.action.json';
...
const FORCEFUL_ACTION_ID = 'hugging:release_self_from_hug_forceful';
const HUG_TIGHT_ACTION_ID = 'hugging:hug_tight';

describe('hugging:release_self_from_hug_forceful action integration', () => {
  let testFixture;
  ...
  beforeEach(async () => {
    testFixture = await ModTestFixture.forAction(
      'hugging',
      FORCEFUL_ACTION_ID,
      releaseSelfForcefulRule,
      releaseSelfForcefulCondition
    );
  ...
  it('clears hugging state and emits matching forceful release messaging', async () => {
    ...
    const expectedMessage =
      "Aster frees themselves forcefully from Briar's hug.";
  ...
  it('restores hug_tight availability and hides the forceful self-release after clearing the embrace', async () => {
    ...
    expect(actionIds).toContain(FORCEFUL_ACTION_ID);
    ...
    expect(actionIds).not.toContain(FORCEFUL_ACTION_ID);
  });
  ...
  it('leaves unrelated embrace data untouched when the target references a different partner', async () => {
    ...
    expect(actorInstance).toHaveComponentData('hugging-states:being_hugged', {
      hugging_entity_id: 'hugging:different_actor',
      consented: true,
    });
    expect(targetInstance).toHaveComponentData('hugging-states:hugging', {
      embraced_entity_id: 'hugging:someone_else',
      initiated: true,
    });
  });
