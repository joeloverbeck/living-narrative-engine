/**
 * @file Integration tests for the bending:straighten_up action and rule.
...
function setupStraighteningScenario(
  ...
  const actor = new ModEntityBuilder('test:actor1')
    ...
    .withComponent('bending-states:bending_over', { surface_id: 'test:surface1' })
    .asActor()
    .build();
...
describe('bending:straighten_up action integration', () => {
  let testFixture;
  ...
  describe('basic straighten up action execution', () => {
    it('should remove bending_over component from actor when action is performed', async () => {
      ...
      expect(actor.components['bending-states:bending_over']).toBeDefined();
      ...
      expect(
        updatedActor.components['bending-states:bending_over']
      ).toBeUndefined();
    });
  ...
  describe('prerequisite constraints', () => {
    ...
    it('should only allow straightening from the surface being bent over', async () => {
      ...
      const actorData =
        testFixture.entityManager.getEntityInstance('test:actor1');
      expect(actorData.components['bending-states:bending_over'].surface_id).toBe(
        'test:surface1'
      );
