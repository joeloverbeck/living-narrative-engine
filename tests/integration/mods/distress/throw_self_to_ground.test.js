...
describe('distress:throw_self_to_ground', () => {
  ...
  beforeEach(async () => {
    ...
    if (testFixture.testEnv.dataRegistry.getComponentDefinition && testFixture.testEnv.dataRegistry.getComponentDefinition.mockImplementation) {
        testFixture.testEnv.dataRegistry.getComponentDefinition.mockImplementation((id) => {
  ...
  describe('Action Discoverability', () => {
    it('should be executable when actor is standing', async () => {
      ...
      testFixture.assertActionSuccess('Ava throws themselves to the ground in grief.');
    });
    ...
    it('should be forbidden when actor has positioning:fallen', async () => {
     ...
     it('should be forbidden when actor has positioning:lying_down', async () => {
      const scenario = testFixture.createStandardActorTarget(['Ava', 'Marcus']);
      scenario.actor.components['positioning:lying_down'] = { furniture_id: 'some_bed' };
  ...
  describe('Rule Execution', () => {
    it('should add positioning:fallen component and log message', async () => {
      ...
      // Verify message
      testFixture.assertActionSuccess('Bob throws themselves to the ground in grief.');
