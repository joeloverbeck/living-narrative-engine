...
describe('Mouth Engagement - Stress Tests', () => {
  ...
  describe('High Load Scenarios', () => {
    it('should handle 1000+ entities with mouth engagement', async () => {
      ...
      console.log(`Created ${actorCount} actors in ${(creationEnd - creationStart).toFixed(2)}ms`);
      ...
      console.log(`Average lock time: ${(lockDuration / actorCount).toFixed(2)}ms per actor`);
      ...
      console.log(`Average unlock time: ${(unlockDuration / actorCount).toFixed(2)}ms per actor`);
    ...
    it('should handle sustained operation load', async () => {
      ...
      while (performance.now() - startTime < testDuration) {
        ...
        if (currentTime - lastReportTime >= reportInterval) {
          ...
          console.log(`  ${elapsed.toFixed(0)}s: ${operations} operations, ${opsPerSecond.toFixed(0)} ops/sec`);
      ...
      console.log(`Total: ${operations} operations in ${(totalTime/1000).toFixed(1)}s`);
    ...
    it('should handle burst load patterns', async () => {
      ...
      for (let i = 0; i < actorCount; i++) {
        actors.push(await createTestActorWithMouth(`burst_${i}`, `BurstActor${i}`));
      }
      ...
      for (let burst = 0; burst < burstCount; burst++) {
        console.log(`Burst ${burst + 1}/${burstCount}: ${operationsPerBurst} operations`);
        ...
        for (let i = 0; i < operationsPerBurst; i++) {
          ...
          const randomOp = Math.random() > 0.5 ? 'LOCK_MOUTH_ENGAGEMENT' : 'UNLOCK_MOUTH_ENGAGEMENT';
        ...
        console.log(`  Burst ${burst + 1} completed in ${burstDuration.toFixed(2)}ms (${opsPerSecond.toFixed(0)} ops/sec)`);
  ...
  describe('Memory Pressure Tests', () => {
    it('should maintain performance under memory pressure', async () => {
      ...
      console.log(`200 operations under memory pressure: ${duration.toFixed(2)}ms`);
    ...
    it('should handle entity churn (creation and destruction)', async () => {
      ...
      for (let cycle = 0; cycle < churnCycles; cycle++) {
        ...
        for (let i = 0; i < entitiesPerCycle; i++) {
          actors.push(await createTestActorWithMouth(`churn_${cycle}_${i}`, `ChurnActor${i}`));
        }
      ...
      const remainingEntities = entityManager.getEntityIds ? 
        entityManager.getEntityIds().length : 0;
  ...
  describe('Concurrent Load Tests', () => {
    it('should handle high concurrency without race conditions', async () => {
      ...
      for (let i = 0; i < actorCount; i++) {
        actors.push(await createTestActorWithMouth(`concurrent_${i}`, `ConcurrentActor${i}`));
      }
      ...
      for (let i = 0; i < concurrentOps; i++) {
        const randomActor = actors[Math.floor(Math.random() * actors.length)];
        const opType = Math.random() > 0.5 ? 'LOCK_MOUTH_ENGAGEMENT' : 'UNLOCK_MOUTH_ENGAGEMENT';
        ...
        operations.push(
          operationInterpreter.execute({
            ...
            console.error(`Operation failed for ${randomActor.id}: ${err.message}`);
            return null;
          })
        );
      }
      ...
      console.log(`${concurrentOps} concurrent operations in ${duration.toFixed(2)}ms`);
    ...
    it('should handle mixed operation patterns efficiently', async () => {
      ...
      for (let i = 0; i < actorCount; i++) {
        actors.push(await createTestActorWithMouth(`mixed_${i}`, `MixedActor${i}`));
      }
      ...
      console.log(`Mixed patterns: ${operations} operations in ${(totalTime/1000).toFixed(1)}s`);
      console.log(`Throughput: ${opsPerSecond.toFixed(0)} ops/sec`);
  ...
  describe('Recovery and Resilience', () => {
    it('should recover from rapid state toggles', async () => {
      ...
      for (let i = 0; i < toggleCount; i++) {
        const operation = i % 2 === 0 ? 'LOCK_MOUTH_ENGAGEMENT' : 'UNLOCK_MOUTH_ENGAGEMENT';
        await operationInterpreter.execute({
          type: operation,
          parameters: { actor_id: actor.id }
        }, context);
    ...
    it('should maintain data integrity under stress', async () => {
      ...
      for (let i = 0; i < actorCount; i++) {
        actors.push(await createTestActorWithMouth(`integrity_${i}`, `IntegrityActor${i}`));
      }
      ...
      for (const actor of actors) {
        ...
        if (!engagement) {
          ...
          integrityErrors++;
          console.error(`Invalid engagement state for ${actor.id}: ${engagement.locked}`);
        } else if (engagement.forcedOverride !== false && engagement.forcedOverride !== true) {
          integrityErrors++;
          console.error(`Invalid forcedOverride for ${actor.id}: ${engagement.forcedOverride}`);
        }
      }
      
      console.log(`Integrity check complete. Errors: ${integrityErrors}/${actors.length}`);
