/**
 * @file Integration tests for breaching:saw_through_barred_blocker action discovery
 */
...
import sawThroughBarredBlockerAction from '../../../../data/mods/breaching/actions/saw_through_barred_blocker.action.json';

const ACTION_ID = 'breaching:saw_through_barred_blocker';

describe('breaching:saw_through_barred_blocker Action Discovery', () => {
  let fixture;

  beforeEach(async () => {
    fixture = await ModTestFixture.forAction(
      'breaching',
      'saw_through_barred_blocker',
      { rule_id: 'breaching:handle_saw_through_barred_blocker', actions: [] }, // Dummy rule
      { id: 'breaching:event-is-action-saw-through-barred-blocker' }, // Dummy condition
      { skipValidation: true }
    );
  ...
  describe('Action structure', () => {
    it('has the expected template with chance placeholder', () => {
      expect(sawThroughBarredBlockerAction.template).toBe(
    ...
    it('uses opposed contest with craft skill and structural resistance', () => {
      expect(sawThroughBarredBlockerAction.chanceBased.enabled).toBe(true);
      expect(sawThroughBarredBlockerAction.chanceBased.contestType).toBe(
        'opposed'
      );
      expect(sawThroughBarredBlockerAction.chanceBased.actorSkill.component).toBe(
        'skills:craft_skill'
      );
      expect(
        sawThroughBarredBlockerAction.chanceBased.targetSkill.component
      ).toBe('blockers:structural_resistance');
      expect(
        sawThroughBarredBlockerAction.chanceBased.targetSkill.targetRole
      ).toBe('primary');
    });

    it('includes the corroded modifier in chance metadata', () => {
      const corrodedModifier =
        sawThroughBarredBlockerAction.chanceBased.modifiers.find(
    ...
    it('requires barred blockers with structural resistance', () => {
      expect(sawThroughBarredBlockerAction.required_components.primary).toEqual(
  ...
  it('should NOT discover if blocker is already being worked on (progress > 0)', async () => {
    // Location setup
    const locationId = fixture.createEntity({
      id: 'test-location-in-progress',
      components: [{ componentId: 'core:location', data: {} }],
    });
    const targetId = fixture.createEntity({ components: [{ componentId: 'core:location', data: {} }] });
  ...
  it('should discover if blocker has progress == 0', async () => {
    // Location setup
    const locationId = fixture.createEntity({
