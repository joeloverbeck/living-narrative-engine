/**
 * @file Integration tests for breaching:saw_through_barred_blocker_stage_three action discovery
 */
...
import sawThroughBarredBlockerStageThreeAction from '../../../../data/mods/breaching/actions/saw_through_barred_blocker_stage_three.action.json';

const ACTION_ID = 'breaching:saw_through_barred_blocker_stage_three';

describe('breaching:saw_through_barred_blocker_stage_three Action Discovery', () => {
  let fixture;

  beforeEach(async () => {
    fixture = await ModTestFixture.forAction(
      'breaching',
      'saw_through_barred_blocker_stage_three',
      { rule_id: 'breaching:handle_saw_through_barred_blocker_stage_three', actions: [] }, // Dummy rule
      { id: 'breaching:event-is-action-saw-through-barred-blocker-stage-three' }, // Dummy condition
      { skipValidation: true }
    );
    ...
    const customResolver = function(context) {
      ...
      exitsList.forEach((exit) => {
        ...
        if (isBarred && hasResistance && progressValue === 2) {
    ...
    ScopeResolverHelpers._registerResolvers(
      ...
      { 'blockers:sawable_barred_blockers_stage_three': customResolver }
    );
  });
  ...
  describe('Action structure', () => {
    it('has the expected template with chance placeholder', () => {
      expect(sawThroughBarredBlockerStageThreeAction.template).toBe(
    ...
    it('uses opposed contest with craft skill and structural resistance', () => {
      expect(sawThroughBarredBlockerStageThreeAction.chanceBased.enabled).toBe(true);
      expect(sawThroughBarredBlockerStageThreeAction.chanceBased.contestType).toBe(
        'opposed'
      );
      expect(
        sawThroughBarredBlockerStageThreeAction.chanceBased.actorSkill.component
      ).toBe('skills:craft_skill');
      expect(
        sawThroughBarredBlockerStageThreeAction.chanceBased.targetSkill.component
      ).toBe('blockers:structural_resistance');
      expect(
        sawThroughBarredBlockerStageThreeAction.chanceBased.targetSkill.targetRole
      ).toBe('primary');
    });

    it('includes the corroded modifier in chance metadata', () => {
      const corrodedModifier =
        sawThroughBarredBlockerStageThreeAction.chanceBased.modifiers.find(
    ...
    it('requires barred blockers with structural resistance', () => {
      expect(
        sawThroughBarredBlockerStageThreeAction.required_components.primary
      ).toEqual(
  ...
  it('should NOT discover if blocker has progress < 2', async () => {
    // Location setup
    const locationId = fixture.createEntity({
  ...
  it('should NOT discover if blocker has progress > 2', async () => {
    ...
    // Blocker with progress > 2
    const blockerId = fixture.createEntity({
      components: [
        { componentId: 'blockers:is_barred', data: {} },
        { componentId: 'blockers:structural_resistance', data: { value: 50 } },
        { componentId: 'core:progress_tracker', data: { value: 3 } },
      ],
    });
    await fixture.modifyComponent(locationId, 'locations:exits', [
      { direction: 'north', target: targetId, blocker: blockerId },
  ...
  it('should discover if blocker has progress == 2', async () => {
    // Location setup
    const locationId = fixture.createEntity({
      id: 'test-location-stage-three',
      components: [{ componentId: 'core:location', data: {} }],
    });
    const targetId = fixture.createEntity({ components: [{ componentId: 'core:location', data: {} }] });
