/**
 * @file Integration tests for saw_through_barred_blocker_stage_three rule structure and outcomes
 */

import { describe, it, expect } from '@jest/globals';

import sawThroughRule from '../../../../data/mods/breaching/rules/handle_saw_through_barred_blocker_stage_three.rule.json' assert { type: 'json' };
import sawThroughAction from '../../../../data/mods/breaching/actions/saw_through_barred_blocker_stage_three.action.json' assert { type: 'json' };
import sawThroughCondition from '../../../../data/mods/breaching/conditions/event-is-action-saw-through-barred-blocker-stage-three.condition.json' assert { type: 'json' };
...
describe('breaching:saw_through_barred_blocker_stage_three rule execution', () => {
  ...
  describe('Rule wiring', () => {
    it('should target core:attempt_action with the correct condition', () => {
      expect(sawThroughRule.rule_id).toBe(
        'handle_saw_through_barred_blocker_stage_three'
      );
      expect(sawThroughRule.event_type).toBe('core:attempt_action');
      expect(sawThroughRule.condition.condition_ref).toBe(
        'breaching:event-is-action-saw-through-barred-blocker-stage-three'
      );
    });
  ...
  describe('Outcome handling', () => {
    it('should upsert progress, mark the blocker breached, and emit a perceptible event on CRITICAL_SUCCESS', () => {
      ...
      const breachedOp = findBreachedOp(criticalIf);
    ...
    it('should upsert progress, mark the blocker breached, and emit a perceptible event on SUCCESS', () => {
      ...
      const breachedOp = findBreachedOp(successIf);
      expect(breachedOp).toBeDefined();
      expect(breachedOp.parameters.value).toEqual({});
  ...
  describe('Action/condition alignment', () => {
    it('should reference the action id in the condition', () => {
      expect(sawThroughCondition.logic['=='][1]).toBe(
        'breaching:saw_through_barred_blocker_stage_three'
      );
    });
