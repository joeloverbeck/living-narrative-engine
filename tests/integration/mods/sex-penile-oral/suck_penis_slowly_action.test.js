/**
 * @file Integration tests for sex-penile-oral:suck_penis_slowly action and rule.
 * @description Verifies slow blowjob continuation narration, perceptible event wiring, and component preservation (stateless action).
 */
...
const EXPECTED_MESSAGE =
  "Ava sucks Nolan's cock slowly, giving careful attention with the lips and tongue to every inch.";

/**
 * Builds the action index with the suck penis slowly action definition.
 *
 * @param {ModTestFixture} fixture - Active test fixture instance.
 */
function configureActionDiscovery(fixture) {
  fixture.testEnv.actionIndex.buildIndex([suckPenisSlowlyAction]);
}
 ...
 */
function buildSuckPenisSlowlyScenario() {
  const ACTOR_ID = 'ava';
...
describe('sex-penile-oral:suck_penis_slowly action integration', () => {
  let testFixture;
  ...
  it('produces narrative message matching specification format', async () => {
    const { entities, actorId, primaryId } = buildSuckPenisSlowlyScenario();
    testFixture.reset(entities);
    configureActionDiscovery(testFixture);
    ...
    expect(message).toContain('cock slowly');
    expect(message).toContain('careful attention with the lips and tongue');
    expect(message).toContain('every inch');
  });
