/**
 * @file Integration tests for sex-penile-oral:suck_penis_slowly action discovery.
 ...
 */
function buildSuckPenisSlowlyScenario(options = {}) {
  const {
...
function installReceivingBlowjobFromActorScopeOverride(fixture) {
  ...
  resolver.resolveSync = (scopeName, context) => {
    if (scopeName === 'sex-penile-oral:receiving_blowjob_from_actor') {
      ...
      // IMPORTANT: No kneeling filter here (unlike personal-space-states:close_actors)
...
/**
 * Registers the suck penis slowly action for discovery.
 *
 * @param {ModTestFixture} fixture - Active test fixture instance.
 */
...
describe('sex-penile-oral:suck_penis_slowly action discovery', () => {
  let testFixture;
  ...
  it('appears when actor is giving blowjob to partner', async () => {
    const { entities, actorId } = buildSuckPenisSlowlyScenario();
    ...
    expect(discovered.template).toBe("suck {primary}'s cock slowly");
  });
