...
describe('sex-penile-oral:ejaculate_in_mouth action integration', () => {
  ...
  it('dispatches correct narrative message and perceptible event', async () => {
    const { entities, actorId, primaryId, roomId } = buildEjaculateInMouthScenario();
  ...
  it('maintains blowjob components (does NOT remove them)', async () => {
    ...
    expect(actorAfter.components['positioning:receiving_blowjob'].giving_entity_id).toBe(
      primaryId
    );
    expect(primaryAfter.components['positioning:giving_blowjob'].receiving_entity_id).toBe(
      actorId
    );
  });

  it('does not affect other entities when ejaculating', async () => {
    const { entities: scenario1Entities, actorId: actor1Id, primaryId: primary1Id } =
    ...
    const primary2After = testFixture.entityManager.getEntityInstance(PRIMARY_ID_2);

    expect(actor2After.components['positioning:receiving_blowjob']).toBeDefined();
    expect(primary2After.components['positioning:giving_blowjob']).toBeDefined();
    expect(actor2After.components['positioning:receiving_blowjob'].giving_entity_id).toBe(
      PRIMARY_ID_2
    );
    expect(primary2After.components['positioning:giving_blowjob'].receiving_entity_id).toBe(
      ACTOR_ID_2
    );
  });
  ...
  it('maintains ongoing blowjob state through multiple ejaculation actions', async () => {
    ...
    const actorAfterFirst = testFixture.entityManager.getEntityInstance(actorId);
    const primaryAfterFirst = testFixture.entityManager.getEntityInstance(primaryId);
    expect(actorAfterFirst.components['positioning:receiving_blowjob']).toBeDefined();
    expect(primaryAfterFirst.components['positioning:giving_blowjob']).toBeDefined();
    ...
    const actorAfterSecond = testFixture.entityManager.getEntityInstance(actorId);
    const primaryAfterSecond = testFixture.entityManager.getEntityInstance(primaryId);
    expect(actorAfterSecond.components['positioning:receiving_blowjob']).toBeDefined();
    expect(primaryAfterSecond.components['positioning:giving_blowjob']).toBeDefined();
  });
});
