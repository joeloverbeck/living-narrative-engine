...
describe('Striking Attack Target State Modifiers', () => {
  ...
  describe('Chance Impact Examples', () => {
    /**
     * These tests demonstrate concrete examples of how modifiers
     * affect the chance calculation in combat scenarios.
     */

    it('should demonstrate +15 bonus impact on base 50% chance (fallen target)', () => {
      // Scenario: Equal skills (50% base chance) attacking a fallen target
      // Base chance: 50% → With +15 bonus: 65%
      setupServices({
        attacker: {
          'core:actor': { name: 'Equal Fighter' },
          'skills:melee_skill': { value: 50 },
        },
        fallen_target: {
          'core:actor': { name: 'Fallen Enemy' },
          'recovery-states:fallen': {},
          'skills:defense_skill': { value: 50 },
        },
      });
    ...
    it('should demonstrate +10 bonus impact on base 50% chance (restrained target)', () => {
      // Scenario: Equal skills (50% base chance) attacking a restrained target
      // Base chance: 50% → With +10 bonus: 60%
      setupServices({
        attacker: {
          'core:actor': { name: 'Equal Fighter' },
          'skills:melee_skill': { value: 50 },
        ...
        restrained_target: {
          'core:actor': { name: 'Restrained Enemy' },
          'physical-control-states:being_restrained': {
            restraining_entity_id: 'ally',
          },
          'skills:defense_skill': { value: 50 },
        },
      });
    ...
    it('should demonstrate +25 combined bonus on base 50% chance (fallen + restrained)', () => {
      // Scenario: Equal skills (50% base chance) attacking helpless target
      // Base chance: 50% → With +25 bonus: 75%
      setupServices({
        attacker: {
          'core:actor': { name: 'Equal Fighter' },
          'skills:melee_skill': { value: 50 },
        ...
        helpless_target: {
          'core:actor': { name: 'Helpless Enemy' },
          'recovery-states:fallen': {},
          'physical-control-states:being_restrained': {
            restraining_entity_id: 'ally',
          },
    ...
    it('should show no bonus when target is standing and free', () => {
      // Scenario: Equal skills (50% base chance), target is in normal state
      // Base chance: 50% → No modifier: still 50%
      setupServices({
        attacker: {
          'core:actor': { name: 'Equal Fighter' },
          'skills:melee_skill': { value: 50 },
