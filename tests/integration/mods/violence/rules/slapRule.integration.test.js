/**
 * @file Integration tests for the violence:slap rule.
 */
...
describe('handle_slap rule integration', () => {
  let testEnv;
  ...
  beforeEach(() => {
    ...
    const dataRegistry = {
      ...
      getConditionDefinition: jest.fn((id) => {
        if (id === 'violence:event-is-action-slap') {
          return eventIsActionSlap;
        }
        return undefined;
      }),
    };
  ...
  it('condition evaluates correctly', () => {
    ...
    const condition = eventIsActionSlap.logic;
  ...
  it('verifies perceptible log message matches requirement', async () => {
    testEnv.reset([
      {
        id: 'actor1',
        components: {
          [NAME_COMPONENT_ID]: { text: 'Sam' },
          [POSITION_COMPONENT_ID]: { locationId: 'room1' },
        },
      },
      {
    ...
    // The rule sets logMessage as the success action message
    const perceptibleEvent = testEnv.events.find(
      (e) => e.eventType === 'core:perceptible_event'
    );
    expect(perceptibleEvent).toBeDefined();

    // This should match the user requirement: "{actor} slaps {target} across the face"
    expect(perceptibleEvent.payload.descriptionText).toBe(
      'Sam slaps Alex across the face.'
    );

    // Verify the perception message is indeed "{actor} has slapped {target} across the face"
    // Note: The current implementation uses the same message for both.
    // The perceptible log message is the one seen by observers in the game world.
  });
});
