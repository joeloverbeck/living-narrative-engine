/**
 * @file Integration tests for the hand_holding:let_go_of_hand action and rule.
...
import letGoRule from '../../../../data/mods/hand_holding/rules/handle_let_go_of_hand.rule.json';
import letGoCondition from '../../../../data/mods/hand_holding/conditions/event-is-action-let-go-of-hand.condition.json';
import letGoAction from '../../../../data/mods/hand_holding/actions/let_go_of_hand.action.json';
import holdHandAction from '../../../../data/mods/hand_holding/actions/hold_hand.action.json';

describe('hand_holding:let_go_of_hand action integration', () => {
  let testFixture;

  beforeEach(async () => {
    testFixture = await ModTestFixture.forAction(
      'hand_holding',
      'hand_holding:let_go_of_hand',
      letGoRule,
      letGoCondition
    );
  });
  ...
  it('successfully releases a held hand and emits matching feedback', async () => {
    ...
    expect(
      actorInstance.components['hand_holding:holding_hand']
    ).toBeUndefined();
  ...
  it('does not disturb other hand-holding relationships', async () => {
    ...
    expect(otherActorInstance.components['hand_holding:holding_hand']).toEqual({
      held_entity_id: otherPair.target.id,
      initiated: true,
    });
    expect(otherTargetInstance.components['hand_holding:hand_held']).toEqual({
      holding_entity_id: otherPair.actor.id,
      consented: true,
    });
  });
  ...
  it('restores hold hand availability and hides let go after release', async () => {
    ...
    expect(actionIds).toContain('hand_holding:let_go_of_hand');
    ...
    expect(actionIds).not.toContain('hand_holding:let_go_of_hand');
  });

  it('ignores attempt_action events for other hand_holding actions', async () => {
    ...
    await testFixture.eventBus.dispatch('core:attempt_action', {
      ...
      actorId: scenario.actor.id,
      actionId: 'hand_holding:hold_hand',
      targetId: scenario.target.id,
  ...
  it('does not clear components when entity references do not match', async () => {
    ...
    scenario.actor.components['hand_holding:holding_hand'] = {
      held_entity_id: 'hand_holding:someone_else',
      initiated: true,
    };
    scenario.target.components['hand_holding:hand_held'] = {
      holding_entity_id: 'hand_holding:different_actor',
      consented: true,
    };
