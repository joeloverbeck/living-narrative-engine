 ...
 * @file Integration tests for ranged:throw_item_at_target action lighting prerequisite
 * @description Tests that the throw_item_at_target action correctly requires the actor's location to be lit
 *
 * Tests the prerequisite using the isActorLocationLit custom JSON Logic operator.
 * @see data/mods/ranged/actions/throw_item_at_target.action.json
...
import throwItemAtTargetAction from '../../../../data/mods/ranged/actions/throw_item_at_target.action.json';

describe('ranged:throw_item_at_target lighting prerequisite', () => {
  ...
  beforeEach(() => {
    ...
    // No other condition_refs for this action
  ...
  describe('action definition structure', () => {
    test('should have prerequisites array defined', () => {
      expect(throwItemAtTargetAction.prerequisites).toBeDefined();
      expect(Array.isArray(throwItemAtTargetAction.prerequisites)).toBe(true);
    });

    test('should have lighting prerequisite using isActorLocationLit operator', () => {
      const lightingPrereq = throwItemAtTargetAction.prerequisites.find(
    ...
    test('should have failure_message for lighting prerequisite', () => {
      ...
      expect(lightingPrereq.failure_message).toBe(
        'It is too dark to throw at anyone.'
      );
    });

    test('should preserve other action properties', () => {
      expect(throwItemAtTargetAction.id).toBe('ranged:throw_item_at_target');
      expect(throwItemAtTargetAction.template).toBe(
  ...
  describe('prerequisite evaluation - pass cases', () => {
    test('should pass when location is naturally lit', () => {
      ...
      const lightingPrereq = throwItemAtTargetAction.prerequisites.filter(
        (prereq) => prereq.logic?.isActorLocationLit
      );

      const result = prerequisiteService.evaluate(
        lightingPrereq,
        throwItemAtTargetAction,
        actor
      );
