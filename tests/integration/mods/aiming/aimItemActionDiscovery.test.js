/**
 * @file Integration tests for the aiming:aim_item action definition.
 * @description Tests that the aim_item action is properly defined and structured.
 */

import { describe, it, beforeEach, afterEach, expect } from '@jest/globals';
import { ModTestFixture } from '../../../common/mods/ModTestFixture.js';
import aimItemAction from '../../../../data/mods/aiming/actions/aim_item.action.json' assert { type: 'json' };

describe('aiming:aim_item action definition', () => {
  let testFixture;

  beforeEach(async () => {
    testFixture = await ModTestFixture.forAction('aiming', 'aiming:aim_item');
  });
  ...
  it('should have correct action structure', () => {
    ...
    expect(aimItemAction.id).toBe('aiming:aim_item');
    expect(aimItemAction.name).toBe('Aim Item');
    expect(aimItemAction.description).toBe(
      "Aim an aimable item at a target entity. The item must have aiming-states:aimable component and be in actor's inventory."
    );
  ...
  it('should use correct scope for primary targets', () => {
    ...
    expect(aimItemAction.targets.primary.scope).toBe('aiming:aimable_targets');
