...
import examineItemInLocationAction from '../../../../data/mods/observation/actions/examine_item_in_location.action.json' assert { type: 'json' };

describe('observation:examine_item_in_location action definition', () => {
  let testFixture;
  ...
  describe('Action discovery behavior', () => {
    it('should appear when portable items with description exist at actor location', () => {
      ...
      const examineActions = discoveredActions.filter(
        (action) => action.id === 'observation:examine_item_in_location'
      );
