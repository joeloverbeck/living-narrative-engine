...
describe('Sit on Lap from Sitting - Workflow Integration', () => {
  ...
  beforeEach(async () => {
    testFixture = await ModTestFixture.forAction('positioning', 'positioning:sit_on_lap_from_sitting_facing_away', sitOnLapFacingAwayRule, eventIsActionSitOnLapFacingAway);
  });
  ...
  describe('Complete Workflows', () => {
    it('should allow transition: both sitting -> sit on lap facing away', async () => {
      ...
      expect(updatedActor.components['positioning:straddling_waist'].facing_away).toBe(true);
      ...
      expect(updatedTarget.components['positioning:sitting_on'].furniture_id).toBe('chair2');
    });
    ...
    it('should handle both actors sitting on same furniture', async () => {
      ...
      expect(updatedTarget.components['positioning:sitting_on']).toBeDefined();
      expect(updatedTarget.components['positioning:sitting_on'].furniture_id).toBe('couch1');
  ...
  describe('State Validation', () => {
    ...
    it('should maintain target sitting_on throughout transition', async () => {
      ...
      expect(updatedTarget.components['positioning:sitting_on']).toEqual(targetOriginalSittingOn);
    });
  });
});
