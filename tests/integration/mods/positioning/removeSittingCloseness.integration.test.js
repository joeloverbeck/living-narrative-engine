...
describe('REMOVE_SITTING_CLOSENESS Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    entityManager.addComponent('furniture:test_couch', 'positioning:allows_sitting', {
    ...
    entityManager.addComponent('game:charlie', 'core:movement', { locked: true });
  });
  ...
  describe('Complete Workflow Integration', () => {
    it('should remove closeness and update movement locks when actor stands', async () => {
      ...
      const aliceCloseness = entityManager.getComponentData('game:alice', 'positioning:closeness');
      expect(aliceCloseness).toBeNull();

      // Verify Bob's closeness to Alice is removed
      const bobCloseness = entityManager.getComponentData('game:bob', 'positioning:closeness');
      expect(bobCloseness?.partners).toEqual(['game:charlie']);

      // Verify Charlie is unaffected (wasn't adjacent to Alice)
      const charlieCloseness = entityManager.getComponentData('game:charlie', 'positioning:closeness');
      expect(charlieCloseness?.partners).toEqual(['game:bob']);

      // Verify Alice's movement is unlocked
      const aliceMovement = entityManager.getComponentData('game:alice', 'core:movement');
      expect(aliceMovement?.locked).toBe(false);

      // Verify Bob and Charlie still locked (still have closeness)
      const bobMovement = entityManager.getComponentData('game:bob', 'core:movement');
      const charlieMovement = entityManager.getComponentData('game:charlie', 'core:movement');
  ...
  describe('Mixed Relationship Handling', () => {
    it('should preserve manual relationships while removing sitting-based ones', async () => {
      ...
      const davidCloseness = entityManager.getComponentData('game:david', 'positioning:closeness');
  ...
  describe('Complex Furniture Scenarios', () => {
    it('should handle all actors standing simultaneously', async () => {
      ...
      expect(entityManager.getComponentData('game:alice', 'positioning:closeness')).toBeNull();
      expect(entityManager.getComponentData('game:charlie', 'positioning:closeness')).toBeNull();
    ...
    it('should handle concurrent operations with race conditions', async () => {
      ...
      const operations = [
        { furniture_id: 'furniture:test_couch', actor_id: 'game:alice', spot_index: 0 },
        { furniture_id: 'furniture:test_couch', actor_id: 'game:bob', spot_index: 1 },
        { furniture_id: 'furniture:test_couch', actor_id: 'game:charlie', spot_index: 2 },
      ];
      ...
      expect(entityManager.getComponentData('game:alice', 'positioning:closeness')).toBeNull();
      expect(entityManager.getComponentData('game:bob', 'positioning:closeness')).toBeNull();
      expect(entityManager.getComponentData('game:charlie', 'positioning:closeness')).toBeNull();
    });
    ...
    it('should handle large furniture with multiple separation events', async () => {
      ...
      entityManager.addComponent('furniture:test_couch', 'positioning:allows_sitting', {
        spots: ['game:alice', 'game:bob', 'game:charlie', 'game:david', 'game:eve'],
      });
      ...
      const aliceCloseness = entityManager.getComponentData('game:alice', 'positioning:closeness');
      const eveCloseness = entityManager.getComponentData('game:eve', 'positioning:closeness');
      expect(aliceCloseness?.partners).toEqual(['game:bob']);
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle invalid furniture ID gracefully', async () => {
      ...
      // Should complete without error since furniture doesn't exist but actor has no closeness
      // (The handler will find no adjacent actors and log info message)
      expect(logger.info).toHaveBeenCalledWith(
        'RemoveSittingClosenessHandler: No formerly adjacent actors found',
        expect.objectContaining({
          actorId: 'game:alice',
    ...
    it('should handle malformed component data gracefully', async () => {
      ...
      // Should handle gracefully and log appropriately
      expect(logger.info).toHaveBeenCalledWith(
        'RemoveSittingClosenessHandler: No closeness relationships to remove',
        expect.objectContaining({
          actorId: 'game:alice',
  ...
  describe('Performance and Memory', () => {
    it('should handle operations on large furniture efficiently', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        if (i > 0) {
          entityManager.addComponent(`game:actor${i}`, 'positioning:closeness', {
            partners: i > 1 ? [`game:actor${i-1}`, `game:actor${i+1}`] : [`game:actor${i+1}`],
          });
        }
      }
