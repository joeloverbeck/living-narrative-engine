/**
 * @file Integration tests verifying affection:rest_head_on_shoulder is forbidden when target has giving_blowjob component.
...
describe('affection:rest_head_on_shoulder - giving_blowjob forbidden component', () => {
  let testFixture;
  ...
  beforeEach(async () => {
    ...
    configureActionDiscovery = () => {
      ...
      const originalResolve =
        scopeResolver.__restHeadShoulderOriginalResolve ||
        scopeResolver.resolveSync.bind(scopeResolver);

      scopeResolver.__restHeadShoulderOriginalResolve = originalResolve;
  ...
  describe('Baseline: Action available without giving_blowjob', () => {
    it('should be available when target does NOT have giving_blowjob component', () => {
      const scenario = testFixture.createCloseActors(['Zara', 'Adam']);
      configureActionDiscovery();
  ...
  describe('Forbidden: Action not available when target giving blowjob', () => {
    it('should NOT be available when target has giving_blowjob component', async () => {
      const scenario = testFixture.createCloseActors(['Beth', 'Carl']);
  ...
  describe('Three-actor scenario: Actor with receiving_blowjob can target others', () => {
    it('should be available to actor with receiving_blowjob when targeting a third actor without giving_blowjob', async () => {
      ...
      // Test 1: Action should succeed when targeting Fiona (no forbidden component)
      await expect(
        testFixture.executeAction(actor.id, targetThird.id)
      ).resolves.not.toThrow();

      // Test 2: Action should fail when targeting Evan (has giving_blowjob)
      testFixture.reset([room, actor, targetGiving, targetThird]);
      configureActionDiscovery();
