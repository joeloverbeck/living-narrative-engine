/**
 * @file Integration tests for the personal-space step_back rule.
 */

import { describe, it, beforeEach, expect, jest } from '@jest/globals';
import ruleSchema from '../../../../../data/schemas/rule.schema.json';
import createTestAjv from '../../../../common/validation/createTestAjv.js';
import eventIsActionStepBack from '../../../../../data/mods/personal-space/conditions/event-is-action-step-back.condition.json';
import stepBackRule from '../../../../../data/mods/personal-space/rules/step_back.rule.json';
...
describe('personal_space_handle_step_back rule integration', () => {
  let testEnv;
  ...
  beforeEach(() => {
    ...
    const dataRegistry = {
      ...
      getConditionDefinition: jest.fn((id) =>
        id === 'personal-space:event-is-action-step-back'
  ...
  it('actor leaves a triad leaving remaining pair intact', async () => {
    testEnv.reset([
      ...
      },
      {
        ...
        components: {
          ...
          'personal-space-states:closeness': { partners: ['a1', 'c1'] },
          'anatomy:body': {
            recipeId: 'anatomy:human',
            body: {
              root: 'body-b1',
      ...
      },
      {
        ...
        components: {
          ...
          'personal-space-states:closeness': { partners: ['a1', 'b1'] },
          'anatomy:body': {
            recipeId: 'anatomy:human',
            body: {
              root: 'body-c1',
    ...
    await testEnv.eventBus.dispatch(ATTEMPT_ACTION_ID, {
      ...
      actorId: 'a1',
      actionId: 'personal-space:step_back',
    ...
    expect(
      testEnv.entityManager.getComponentData('b1', 'personal-space-states:closeness')
    ).toEqual({
      partners: ['c1'],
    });
    expect(
    ...
    expect(
      testEnv.entityManager.getComponentData('c1', 'personal-space-states:closeness')
    ).toEqual({
      partners: ['b1'],
    });
    expect(
  ...
  it('actor stepping back from a pair frees the partner', async () => {
    testEnv.reset([
      ...
      },
      {
        ...
        components: {
          [NAME_COMPONENT_ID]: { text: 'B' },
          'personal-space-states:closeness': { partners: ['a1'] },
          'anatomy:body': {
            recipeId: 'anatomy:human',
            body: {
              root: 'body-b1',
  ...
  it('actor leaves a poly circle leaving remaining triad intact', async () => {
    ...
    for (const id of ['a1', 'b1', 'c1', 'd1']) {
      ...
      ent.components['personal-space-states:closeness'] = {
    ...
    for (const id of ['b1', 'c1', 'd1']) {
      const partners = testEnv.entityManager
        .getComponentData(id, 'personal-space-states:closeness')
        .partners.sort();
  ...
  it('complex state scenario: multiple actors with mixed facing states', async () => {
    testEnv.reset([
      ...
      },
      {
        ...
        components: {
          [NAME_COMPONENT_ID]: { text: 'C' },
          'personal-space-states:closeness': { partners: ['a1', 'b1', 'd1'] },
          // No facing_away component
          'anatomy:body': {
            recipeId: 'anatomy:human',
            body: {
    ...
    expect(
      testEnv.entityManager.getComponentData('d1', 'personal-space-states:closeness')
    ).toEqual({
      partners: ['b1', 'c1'],
    });
