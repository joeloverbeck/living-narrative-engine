/**
 * @file Integration tests for handle_swell_music_on_instrument rule execution.
...
import swellMusicRule from '../../../../data/mods/music/rules/handle_swell_music_on_instrument.rule.json' assert { type: 'json' };
import eventIsActionSwellMusic from '../../../../data/mods/music/conditions/event-is-action-swell-music-on-instrument.condition.json' assert { type: 'json' };

describe('music:swell_music_on_instrument - Rule Execution', () => {
  let testFixture;
  ...
  describe('Successfully executes swell_music_on_instrument action', () => {
    it('should dispatch perceptible event with mood-adjective-flavored message', async () => {
      ...
      await testFixture.executeAction('musician1', 'horn1');
      ...
      // For 'triumphant', the adjective is 'bold'
    ...
    it('should work with all 5 compatible moods from mood_lexicon', async () => {
      const moods = [
    ...
    it('should use fallback adjective when mood not found in lexicon', async () => {
      ...
      expect(perceptibleEvent.payload.descriptionText).toContain('dynamic');
    });
  });

  describe('Edge cases and validation', () => {
    it('should handle multiple sequential swell performances correctly', async () => {
      const room = ModEntityScenarios.createRoom('hall', 'Hall');
    ...
    it('should include targetId in perceptible event', async () => {
      ...
      await testFixture.executeAction('musician1', 'chimes1');
      ...
      const perceptibleEvent = perceptibleEvents[0];
      expect(perceptibleEvent.payload.targetId).toBe('chimes1');
    });
  });
});
