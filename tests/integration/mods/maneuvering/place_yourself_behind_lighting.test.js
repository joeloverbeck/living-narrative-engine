 ...
 * @file Integration tests for maneuvering:place_yourself_behind action lighting prerequisite
 * @description Tests that the place_yourself_behind action correctly requires the actor's location to be lit
 *
 * Tests the prerequisite using the isActorLocationLit custom JSON Logic operator.
 * @see data/mods/maneuvering/actions/place_yourself_behind.action.json
...
describe('maneuvering:place_yourself_behind lighting prerequisite', () => {
  ...
  describe('action definition structure', () => {
    test('should have prerequisites array defined', () => {
      expect(placeYourselfBehindAction.prerequisites).toBeDefined();
      expect(Array.isArray(placeYourselfBehindAction.prerequisites)).toBe(true);
    });

    test('should have lighting prerequisite using isActorLocationLit operator', () => {
      const lightingPrereq = placeYourselfBehindAction.prerequisites.find(
    ...
    test('should have failure_message for lighting prerequisite', () => {
      ...
      expect(lightingPrereq.failure_message).toBe(
        'It is too dark to position yourself.'
      );
    });

    test('should preserve other action properties', () => {
      expect(placeYourselfBehindAction.id).toBe(
        'maneuvering:place_yourself_behind'
      );
      expect(placeYourselfBehindAction.template).toBe(
        'place yourself behind {target}'
      );
    });
  });
  ...
  describe('prerequisite evaluation - pass cases', () => {
    test('should pass when location is naturally lit', () => {
      ...
      const lightingPrereq = placeYourselfBehindAction.prerequisites.filter(
        (prereq) => prereq.logic?.isActorLocationLit
      );
