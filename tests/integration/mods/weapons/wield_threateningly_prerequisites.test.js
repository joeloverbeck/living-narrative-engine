...
import wieldThreateninglyAction from '../../../../data/mods/weapons/actions/wield_threateningly.action.json';
...
describe('weapons:wield_threateningly prerequisites', () => {
  ...
  describe('action definition structure', () => {
    test('should have prerequisites array defined', () => {
      expect(wieldThreateninglyAction.prerequisites).toBeDefined();
      expect(Array.isArray(wieldThreateninglyAction.prerequisites)).toBe(true);
    });

    test('should reference anatomy:actor-has-free-grabbing-appendage condition', () => {
      expect(wieldThreateninglyAction.prerequisites.length).toBeGreaterThan(0);
      const prerequisite = wieldThreateninglyAction.prerequisites[0];
    ...
    test('should preserve other action properties', () => {
      expect(wieldThreateninglyAction.id).toBe('weapons:wield_threateningly');
      expect(wieldThreateninglyAction.template).toBe(
  ...
  describe('prerequisite evaluation - free grabbing appendage available', () => {
    test('should pass when actor has exactly one free grabbing appendage', () => {
      ...
      const result = prerequisiteService.evaluate(
        wieldThreateninglyAction.prerequisites,
        wieldThreateninglyAction,
        actor
      );
