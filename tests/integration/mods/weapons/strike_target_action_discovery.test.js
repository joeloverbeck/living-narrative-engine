/**
 * @file Integration tests for strike_target action discovery
 * Tests action structure, scope configuration, and chanceBased configuration
 */

import { describe, it, expect } from '@jest/globals';
import { readFile } from 'fs/promises';
import actionJson from '../../../../data/mods/weapons/actions/strike_target.action.json' assert { type: 'json' };
import conditionJson from '../../../../data/mods/weapons/conditions/event-is-action-strike-target.condition.json' assert { type: 'json' };

describe('strike_target action definition', () => {
  describe('Action Structure', () => {
    it('should have correct action ID', () => {
      expect(actionJson.id).toBe('weapons:strike_target');
    });

    it('should have correct name', () => {
      expect(actionJson.name).toBe('Strike at Target');
    });
    ...
    it('should have template with chance placeholder', () => {
      expect(actionJson.template).toBe('strike {target} with {weapon} ({chance}% chance)');
    });
  ...
  describe('Target Configuration', () => {
    ...
    it('should use wielded_blunt_weapons scope for primary target', () => {
      expect(actionJson.targets.primary.scope).toBe('weapons:wielded_blunt_weapons');
    });
...
describe('wielded_blunt_weapons scope definition', () => {
  it('should filter wielded items to blunt-capable weapons', async () => {
    const scopeContent = await readFile(
      new URL(
        '../../../../data/mods/weapons/scopes/wielded_blunt_weapons.scope',
        import.meta.url
      ),
      'utf-8'
    );
    ...
    expect(scopeContent).toContain('"blunt"');
  });
});

describe('event-is-action-strike-target condition', () => {
  describe('Condition Structure', () => {
    it('should have correct condition ID', () => {
      expect(conditionJson.id).toBe('weapons:event-is-action-strike-target');
    });
    ...
    it('should check for strike_target action ID', () => {
      expect(conditionJson.logic['==']).toBeDefined();
      expect(conditionJson.logic['=='][1]).toBe('weapons:strike_target');
    });
