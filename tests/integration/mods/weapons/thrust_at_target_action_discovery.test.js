/**
 * @file Integration tests for thrust_at_target action discovery
 * Tests action structure, scope configuration, and chanceBased configuration
 */

import { describe, it, expect } from '@jest/globals';
import { readFile } from 'fs/promises';
import actionJson from '../../../../data/mods/weapons/actions/thrust_at_target.action.json' assert { type: 'json' };
import conditionJson from '../../../../data/mods/weapons/conditions/event-is-action-thrust-at-target.condition.json' assert { type: 'json' };

describe('thrust_at_target action definition', () => {
  describe('Action Structure', () => {
    it('should have correct action ID', () => {
      expect(actionJson.id).toBe('weapons:thrust_at_target');
    });

    it('should have correct name', () => {
      expect(actionJson.name).toBe('Thrust at Target');
    });
...
describe('wielded_piercing_weapons scope definition', () => {
  it('should filter wielded items to piercing-capable weapons', async () => {
    ...
    expect(scopeContent).toContain('"piercing"');
  });
});

describe('event-is-action-thrust-at-target condition', () => {
  describe('Condition Structure', () => {
    it('should have correct condition ID', () => {
      expect(conditionJson.id).toBe('weapons:event-is-action-thrust-at-target');
    });
    ...
    it('should check for thrust_at_target action ID', () => {
      expect(conditionJson.logic['==']).toBeDefined();
      expect(conditionJson.logic['=='][1]).toBe('weapons:thrust_at_target');
    });
