 ...
 * @file Integration tests for deference:kneel_before action lighting prerequisite
 * @description Tests that the kneel_before action correctly requires the actor's location to be lit
 *
 * Tests the prerequisite using the isActorLocationLit custom JSON Logic operator.
 * @see data/mods/deference/actions/kneel_before.action.json
...
describe('deference:kneel_before lighting prerequisite', () => {
  ...
  describe('action definition structure', () => {
    test('should have prerequisites array defined', () => {
      expect(kneelBeforeAction.prerequisites).toBeDefined();
      expect(Array.isArray(kneelBeforeAction.prerequisites)).toBe(true);
    });

    test('should have lighting prerequisite using isActorLocationLit operator', () => {
      const lightingPrereq = kneelBeforeAction.prerequisites.find(
    ...
    test('should have failure_message for lighting prerequisite', () => {
      ...
      expect(lightingPrereq.failure_message).toBe(
        'It is too dark to kneel before anyone.'
      );
    });

    test('should preserve other action properties', () => {
      expect(kneelBeforeAction.id).toBe('deference:kneel_before');
      expect(kneelBeforeAction.template).toBe('kneel before {actor}');
    });
  });
  ...
  describe('prerequisite evaluation - pass cases', () => {
    test('should pass when location is naturally lit', () => {
      ...
      const lightingPrereq = kneelBeforeAction.prerequisites.filter(
        (prereq) => prereq.logic?.isActorLocationLit
      );
