/**
 * @file Integration tests for the cosmetics:apply_lipstick action definition.
...
import applyLipstickAction from '../../../../data/mods/cosmetics/actions/apply_lipstick.action.json' assert { type: 'json' };

const ACTION_ID = 'cosmetics:apply_lipstick';

describe('cosmetics:apply_lipstick action definition', () => {
  let testFixture;
  let configureActionDiscovery;

  beforeEach(async () => {
    testFixture = await ModTestFixture.forAction('cosmetics', ACTION_ID);

    // Register inventory scopes needed for apply_lipstick action discovery
  ...
  it('should scope targets to actor inventory lipstick items', () => {
    ...
    expect(applyLipstickAction.targets.primary).toEqual(
      expect.objectContaining({
        scope: 'inventory:actor_inventory_items',
        placeholder: 'item',
  ...
  describe('Action discovery behavior', () => {
    it('is available when the actor carries a lipstick item', () => {
      ...
      const lipstick = new ModEntityBuilder('lipstick1')
        ...
        .withComponent('cosmetics:can_apply_lipstick', {})
        .build();
