/**
 * @file Integration tests for lighting:extinguish_light_source action discovery.
 * @description Validates discovery rules for combustible, lit light sources.
 */
...
import extinguishLightAction from '../../../../data/mods/lighting/actions/extinguish_light_source.action.json' assert { type: 'json' };
import {
...
const ACTION_ID = 'lighting:extinguish_light_source';

describe('lighting:extinguish_light_source action discovery', () => {
  let testFixture;
  ...
  beforeEach(async () => {
    ...
    await ScopeResolverHelpers.registerCustomScope(
      testFixture.testEnv,
      'lighting',
      'lit_combustible_light_sources_in_inventory'
    );
    ...
    configureActionDiscovery = () => {
      ...
      testEnv.actionIndex.buildIndex([extinguishLightAction]);
    };
  });
  ...
  it('should discover action when actor has a lit oil lamp in inventory', () => {
  ...
  it('should discover action when actor has a lit candle in inventory', () => {
    const room = ModEntityScenarios.createRoom('candle_room', 'Candle Room');
    const lightSource = createCandle('lighting:lit_candle', { isLit: true });
  ...
  it('should not discover action when all light sources are unlit', () => {
    const room = ModEntityScenarios.createRoom('unlit_room', 'Unlit Room');
  ...
  it('should not discover action when light source is not in inventory', () => {
    ...
    const lightSource = createLitOilLamp('lighting:floor_lamp');
