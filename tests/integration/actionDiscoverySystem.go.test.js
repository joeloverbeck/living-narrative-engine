...
describe('ActionDiscoverySystem - Go Action (Fixed State)', () => {
    ...
    beforeEach(() => {
        ...
        Object.entries(heroEntityDefinitionData.components || {}).forEach(([typeId, data]) => {
            mockHeroEntity.addComponent(typeId, JSON.parse(JSON.stringify(data)));
        });

        mockAdventurersGuildLocation = new Entity(GUILD_INSTANCE_ID, GUILD_DEFINITION_ID);
        Object.entries(adventurersGuildEntityDefinitionData.components || {}).forEach(([typeId, data]) => {
            mockAdventurersGuildLocation.addComponent(typeId, JSON.parse(JSON.stringify(data)));
        });
        ...
        mockEntityManager.getComponentData.mockImplementation((instanceId, componentTypeId) => {
            let targetEntity;
            if (instanceId === HERO_INSTANCE_ID) targetEntity = mockHeroEntity;
            else if (instanceId === GUILD_INSTANCE_ID) targetEntity = mockAdventurersGuildLocation;

            return targetEntity ? targetEntity.getComponentData(componentTypeId) : undefined;
        });

        mockEntityManager.hasComponent.mockImplementation((instanceId, componentTypeId) => {
            let targetEntity;
            if (instanceId === HERO_INSTANCE_ID) targetEntity = mockHeroEntity;
            else if (instanceId === GUILD_INSTANCE_ID) targetEntity = mockAdventurersGuildLocation;

            return targetEntity ? targetEntity.hasComponent(componentTypeId) : false;
        });
        ...
        mockValidationService.isValid.mockImplementation((actionDef, actor, targetContext) => {
            ...
            if (actionDef.id === 'core:go') {
                ...
                if (targetContext.type === 'direction' && targetContext.direction === 'out to town') {
                    ...
                    const guildExits = mockAdventurersGuildLocation.getComponentData(EXITS_COMPONENT_ID);
                    const relevantExit = guildExits?.find(e => e.direction === 'out to town');
    ...
    it('should discover "go out to town" action when player is in adventurers guild and exit is available', async () => {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(
            expect.stringContaining(`Found ${adventurersGuildEntityDefinitionData.components[EXITS_COMPONENT_ID].length} potential exits from ${EXITS_COMPONENT_ID}. Checking validation...`)
        );
        expect(mockLogger.debug).toHaveBeenCalledWith(
        ...
        expect(mockEntityManager.getComponentData).toHaveBeenCalledWith(GUILD_INSTANCE_ID, EXITS_COMPONENT_ID);
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Finished action discovery for actor ${HERO_INSTANCE_ID}. Found 2 valid commands/actions.`));
    });
});
// --- FILE END ---