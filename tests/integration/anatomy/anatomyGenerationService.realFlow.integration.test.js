...
describe('AnatomyGenerationService real module integration', () => {
  ...
  it('generates anatomy and aggregates batch results with real orchestrator collaborators', async () => {
    ...
    const missingResult = await service.generateAnatomyIfNeeded('missing-entity');
    ...
    const actorNoRecipe = await testBed.entityManager.createEntityInstance(
      'core:actor',
    );
    await testBed.entityManager.addComponent(actorNoRecipe.id, ANATOMY_BODY_COMPONENT_ID, {});

    testBed.logger.warn.mockClear();
    const noRecipeResult = await service.generateAnatomyIfNeeded(actorNoRecipe.id);
    ...
    await testBed.entityManager.addComponent(failingActor.id, ANATOMY_BODY_COMPONENT_ID, {
    ...
    const aggregateFailureLog = testBed.logger.error.mock.calls.find(([message]) =>
