...
describe('Tortoise Beak Entity Validation', () => {
  const beakEntity = JSON.parse(
    readFileSync(
      join(process.cwd(), 'data/mods/anatomy/entities/definitions/tortoise_beak.entity.json'),
      'utf-8'
    )
  );
  ...
  describe('tortoise_beak entity', () => {
    ...
    it('should have correct description', () => {
      expect(beakEntity.description).toBe('Horny beak structure for herbivorous diet');
    });

    it('should have anatomy:part component with tortoise_beak subType', () => {
      expect(beakEntity.components['anatomy:part']).toBeDefined();
      expect(beakEntity.components['anatomy:part'].subType).toBe('tortoise_beak');
    });
    ...
    it('should have descriptors:texture component with "ridged" texture', () => {
      ...
      expect(beakEntity.components['descriptors:texture'].texture).toBe('ridged');
    });
    ...
    it('should have descriptors:color_extended component with "charcoal-gray" color', () => {
      ...
      expect(beakEntity.components['descriptors:color_extended'].color).toBe('charcoal-gray');
    });
    ...
    it('should have descriptors:shape_general component with "hooked" shape', () => {
      ...
      expect(beakEntity.components['descriptors:shape_general'].shape).toBe('hooked');
    });
  });
  ...
  describe('Descriptor validation', () => {
    it('should use valid texture value from descriptors:texture component schema', () => {
      const validTextures = [
        ...
        'silky', 'slick', 'slimy', 'smooth', 'smooth-muscular', 'smooth-segmented',
        'soft', 'suckered', 'terry', 'translucent', 'translucent-slick',
        'translucent-veined', 'velvety', 'webbed-clawed'
      ];
      
      expect(validTextures).toContain(beakEntity.components['descriptors:texture'].texture);
    });

    it('should use valid shape value from descriptors:shape_general component schema', () => {
      const validShapes = [
        'flat', 'angular', 'bulbous', 'conical', 'curved', 'cylindrical',
        'domed', 'elongated', 'hooked', 'oval', 'round', 'square', 'tapered'
      ];
      
      expect(validShapes).toContain(beakEntity.components['descriptors:shape_general'].shape);
    });
    ...
    it('should use valid color value from descriptors:color_extended component schema', () => {
      ...
      expect(validColors).toContain(beakEntity.components['descriptors:color_extended'].color);
    });
  });
});
