...
describe('Activity Description System Activation', () => {
  const activatedComponents = [
    { mod: 'companionship', name: 'following', expectedTemplate: 'is following', priority: 60, targetRole: 'leaderId' },
    { mod: 'companionship', name: 'leading', expectedTemplate: 'is leading others', priority: 58, targetRole: 'followers' },
    { mod: 'hand-holding', name: 'hand_held', expectedTemplate: 'hand is being held', priority: 65, targetRole: 'holding_entity_id' },
    { mod: 'hand-holding', name: 'holding_hand', expectedTemplate: 'is holding', priority: 67, targetRole: 'held_entity_id' },
    { mod: 'kissing', name: 'kissing', expectedTemplate: 'is kissing', priority: 72, targetRole: 'partner' },
    { mod: 'positioning', name: 'being_bitten_in_neck', expectedTemplate: 'neck is being bitten', priority: 70, targetRole: 'biting_entity_id' },
    { mod: 'positioning', name: 'being_hugged', expectedTemplate: 'is being hugged', priority: 63, targetRole: 'hugging_entity_id' },
    { mod: 'positioning', name: 'bending_over', expectedTemplate: 'is bending over', priority: 68, targetRole: 'surface_id' },
    { mod: 'positioning', name: 'biting_neck', expectedTemplate: 'is biting', priority: 73, targetRole: 'bitten_entity_id' },
    { mod: 'positioning', name: 'giving_blowjob', expectedTemplate: 'performing oral sex', priority: 80, targetRole: 'receiving_entity_id' },
    { mod: 'positioning', name: 'hugging', expectedTemplate: 'is hugging', priority: 66, targetRole: 'embraced_entity_id' },
    { mod: 'positioning', name: 'lying_down', expectedTemplate: 'is lying on', priority: 64, targetRole: 'furniture_id' },
    { mod: 'positioning', name: 'receiving_blowjob', expectedTemplate: 'receiving oral sex', priority: 78, targetRole: 'giving_entity_id' },
    { mod: 'positioning', name: 'sitting_on', expectedTemplate: 'is sitting on', priority: 62, targetRole: 'furniture_id' },
    { mod: 'positioning', name: 'straddling_waist', expectedTemplate: 'is straddling', priority: 82, targetRole: 'target_id' },
  ];

  describe('Component Activation Verification', () => {
    activatedComponents.forEach(({ mod, name, expectedTemplate, priority, targetRole }) => {
      ...
      describe(`${componentId}`, () => {
        ...
        it('should have activityMetadata property in dataSchema', () => {
          expect(componentDef.dataSchema.properties).toHaveProperty('activityMetadata');
        });

        it('should have valid activityMetadata structure', () => {
          const metadata = componentDef.dataSchema.properties.activityMetadata;
          expect(metadata.type).toBe('object');
          expect(metadata.additionalProperties).toBe(false);
          expect(metadata.description).toContain('Inline metadata for activity description generation');
        });
        ...
        it('should have template with expected content', () => {
          const field = componentDef.dataSchema.properties.activityMetadata.properties.template;
        ...
        it('should have correct targetRole mapping', () => {
          const field = componentDef.dataSchema.properties.activityMetadata.properties.targetRole;
          expect(field.type).toBe('string');
          expect(field.default).toBe(targetRole);
        });

        it('should have correct priority value', () => {
          const field = componentDef.dataSchema.properties.activityMetadata.properties.priority;
  ...
  describe('Reference Implementation Consistency', () => {
    it('should match kneeling_before reference implementation structure', () => {
      ...
      expect(testMetadata.additionalProperties).toBe(referenceMetadata.additionalProperties);
