...
describe('LLM Proxy Server Rate Limiting Issue Reproduction', () => {
  ...
  it('should reproduce rate limiting blocking health checks', async () => {
    ...
    const connectionRefusedResponses = statusCodes.filter(
      (status) => status === 503
    );
    ...
    console.log(
      ...
      connectionRefusedResponses.length
    );
    ...
    if (connectionRefusedResponses.length === responses.length) {
      console.log(
        'LLM proxy server is not running - cannot test rate limiting'
      );
  ...
  it('should reproduce rate limiting blocking game startup requests', async () => {
    ...
    if (rateLimitedResponses.length > 0) {
      console.warn(
        'ISSUE REPRODUCED: Game startup requests are being rate limited!'
      );
    }
