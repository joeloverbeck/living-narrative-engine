...
describe('Action Schema Backward Compatibility Integration', () => {
    ...
    test('should validate all existing action files with updated schema', async () => {
      ...
      if (invalidFiles.length > 0) {
        invalidFiles.forEach(result => {
          console.error(`INVALID: ${result.file} - ${result.errorMessage || result.error}`);
          if (result.errors) {
            result.errors.forEach(error => {
              console.error(`  - ${error.message} at ${error.instancePath || 'root'}`);
            });
          }
        });
        
        // Throw error with detailed information
        const errorDetails = invalidFiles.map(f => `${f.file}: ${f.errorMessage || f.error}`).join('\n');
        throw new Error(`Schema validation failed for ${invalidFiles.length} files:\n${errorDetails}`);
      }
  ...
  describe('Legacy Properties Detection', () => {
    test('should identify actions still using legacy scope property', async () => {
      ...
      console.log(`  New 'targets' property: ${newFormatActions.length} actions`);
      
      const multiTargetActions = newFormatActions.filter(a => a.isMultiTarget);
      console.log(`  Multi-target format: ${multiTargetActions.length} actions`);
      
      // Log some examples for reference
      if (legacyActions.length > 0) {
        console.log(`\nLegacy actions (first 5):`);
        legacyActions.slice(0, 5).forEach(action => {
          console.log(`  - ${action.id} (${action.file}): scope="${action.scope}"`);
        });
      }
      
      // This test just reports information, all existing actions should validate
      expect(legacyActions.length + newFormatActions.length).toBe(actionFiles.length);
    });
  });
    ...
    test('should verify no actions have both scope and targets properties', async () => {
      ...
      if (conflictingActions.length > 0) {
        ...
        conflictingActions.forEach(action => {
          console.log(`  - ${action.id} (${action.file}): has both scope and targets`);
        });
      }
    ...
    test('should verify all actions have required targeting property', async () => {
      ...
      if (missingTargetingActions.length > 0) {
        ...
        missingTargetingActions.forEach(action => {
          console.log(`  - ${action.id} (${action.file}): missing both scope and targets`);
        });
      }
  ...
  describe('Performance Verification', () => {
    test('should validate actions within performance threshold', async () => {
      ...
      for (const filePath of actionFiles.slice(0, 10)) { // Test first 10 for performance
        try {
          const fileContent = await readFile(filePath, 'utf-8');
          const actionData = JSON.parse(fileContent);
