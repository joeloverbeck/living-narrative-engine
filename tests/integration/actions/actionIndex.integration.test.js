...
describe('ActionIndex Integration Tests', () => {
  ...
  describe('Performance & Scale Testing', () => {
    it('should handle large action catalogs efficiently', () => {
      ...
      expect(buildEnd - buildStart).toBeLessThan(100); // Should complete in <100ms
      ...
      expect(queryEnd - queryStart).toBeLessThan(10); // Should complete in <10ms
    ...
    it('should handle frequent index rebuilds efficiently', () => {
      ...
      for (let rebuild = 0; rebuild < 10; rebuild++) {
        ...
        expect(end - start).toBeLessThan(50); // Each rebuild <50ms
      }
    });

    it('should maintain memory efficiency with large entity sets', () => {
      // Build index
