      ...
      'elara': {
        ...
        getComponentData: jest.fn((componentId) => {
          const components = {
            ...
            'clothing:equipment': { equipped: { torso_upper: { base: 'blazer123' } } }
          };
      ...
      'joel': {
        ...
        getComponentData: jest.fn((componentId) => {
          const components = {
            ...
            'clothing:equipment': { equipped: { torso_upper: { base: 'trenchcoat456' } } }
          };
          return components[componentId];
        }),
        hasComponent: jest.fn(() => true),
  ...
  describe('Multiple actors with different clothing', () => {
    it('should resolve different secondary targets for each primary target', async () => {
      ...
      unifiedScopeResolver.resolve.mockImplementation((scope, context) => {
        if (scope === 'intimacy:close_actors_facing_each_other_with_torso_clothing') {
          return {
            success: true,
      ...
      if (result.data.actionsWithTargets.length === 0) {
        console.log('BUG CONFIRMED: No actions resolved because secondary scope gets undefined context.target');
        console.log('The secondary scope was called with context.target: undefined');
        console.log('This happens because buildDependentContext only uses the first primary target');
        
        // Check how many times unifiedScopeResolver was called
        const secondaryScopeCalls = unifiedScopeResolver.resolve.mock.calls.filter(
          call => call[0] === 'intimacy:target_topmost_torso_upper_clothing'
        );
        console.log('Secondary scope was called', secondaryScopeCalls.length, 'time(s)');
    ...
    it('should correctly build context for each primary target when resolving secondary', async () => {
      ...
      expect(result.data.resolvedTargets.secondary[0].contextFromId).toBe('elara');
      expect(result.data.resolvedTargets.secondary[1].contextFromId).toBe('joel');
    });
  });
});