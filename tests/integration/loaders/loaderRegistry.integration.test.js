...
const createMockConfiguration = (overrides = {}) => ({
  ...
  getContentTypeSchemaId: jest.fn((typeName) => {
    ...
    if (typeName === 'components') return 'http://example.com/schemas/component.schema.json';
...
const createMockSchemaValidator = (overrides = {}) => {
  ...
  const mockValidator = {
    ...
    getValidator: jest.fn((schemaId) => schemaValidators.get(schemaId) || (() => ({ isValid: true, errors: null }))),
...
describe('Integration: Loaders, Registry State, and Overrides (REFACTOR-8.6)', () => {
  let mockConfig, mockResolver, mockFetcher, mockValidator, mockLogger, dataRegistry, actionLoader, componentLoader;
  ...
  const modAActionData = { id: coolActionBaseId, commandVerb: 'do_a', description: 'From Mod A' };
  const modAComponentData = { id: coolComponentBaseId, dataSchema: { type: 'object', properties: { propA: {} } }, description: 'From Mod A' };
  const modAManifest = { id: modAId, name: 'Mod A', version: '1.0', content: { actions: [actionFilename], components: [componentFilename] } };
  const modBId = 'modB';
  ...
  const modBActionData = { id: coolActionBaseId, commandVerb: 'do_b', description: 'From Mod B' };
  const modBComponentData = { id: coolComponentBaseId, dataSchema: { type: 'object', properties: { propB: {} } }, description: 'From Mod B' };
  const modBManifest = { id: modBId, name: 'Mod B', version: '1.0', content: { actions: [actionFilename], components: [componentFilename] } };
  ...
  beforeEach(() => {
    ...
    mockValidator._setSchemaLoaded('http://example.com/schemas/component.schema.json', {});
  });

  describe('Scenario 1: Non-Conflicting Base IDs', () => {
    beforeEach(() => {
      const fetcherConfig = { [modAActionPath]: modAActionData, [modAComponentPath]: modAComponentData, [modBActionPath]: modBActionData, [modBComponentPath]: modBComponentData };
    ...
    it('should store items from different mods with the same base ID under unique keys without warnings', async () => {
      await actionLoader.loadItemsForMod(modAId, modAManifest, ACTION_CONTENT_KEY, ACTION_CONTENT_DIR, ACTION_TYPE_NAME);
      await componentLoader.loadItemsForMod(modAId, modAManifest, 'components', 'components', 'components');
      await actionLoader.loadItemsForMod(modBId, modBManifest, ACTION_CONTENT_KEY, ACTION_CONTENT_DIR, ACTION_TYPE_NAME);
      await componentLoader.loadItemsForMod(modBId, modBManifest, 'components', 'components', 'components');

      expect(dataRegistry.store).toHaveBeenCalledTimes(4);
      const actionA = dataRegistry.get('actions', `${modAId}:${coolActionBaseId}`);
      const componentA = dataRegistry.get('components', `${modAId}:${coolComponentBaseId}`);
      const actionB = dataRegistry.get('actions', `${modBId}:${coolActionBaseId}`);
      const componentB = dataRegistry.get('components', `${modBId}:${coolComponentBaseId}`);
  ...
  describe('Scenario 2: True Key Override (Warning Check)', () => {
    ...
    const overrideActionDataV1 = { id: overrideActionIdInFile, description: 'Version 1' };
    const overrideActionDataV2 = { id: overrideActionIdInFile, description: 'Version 2' };
    ...
    beforeEach(() => {
      const fetcherConfig = { [overrideActionPathV1]: overrideActionDataV1, [overrideActionPathV2]: overrideActionDataV2 };
      mockFetcher = createMockDataFetcher(fetcherConfig);
      actionLoader._dataFetcher = mockFetcher;
    });

    it('should log a warning when an item is stored with the same final key as an existing item', async () => {
      await actionLoader._processFetchedItem(overrideModId, overrideActionFileV1, overrideActionPathV1, overrideActionDataV1, ACTION_TYPE_NAME);
      expect(mockLogger.warn).not.toHaveBeenCalled();
      expect(dataRegistry.get('actions', overrideFinalKey).description).toBe('Version 1');
      mockLogger.warn.mockClear();

      await actionLoader._processFetchedItem(overrideModId, overrideActionFileV2, overrideActionPathV2, overrideActionDataV2, ACTION_TYPE_NAME);
  ...
  describe('Scenario 3: Registry State Verification', () => {
    ...
    const modCComponentData = { id: 'unique_comp', dataSchema: {}, description: 'From Mod C' };
    // <<< FIX END >>>
    const modCManifest = { id: modCId, content: { components: [diverseComponentFilename] } };
    const modDId = 'modD';
    ...
    const modDManifest = { id: modDId, content: { actions: [diverseActionFilename] } };
    ...
    it('should store items with correct keys and augmented metadata', async () => {
      await actionLoader.loadItemsForMod(modAId, modAManifest, 'actions', 'actions', 'actions');
      await componentLoader.loadItemsForMod(modAId, modAManifest, 'components', 'components', 'components');
      await actionLoader.loadItemsForMod(modBId, modBManifest, 'actions', 'actions', 'actions');
      await componentLoader.loadItemsForMod(modBId, modBManifest, 'components', 'components', 'components');
      await componentLoader.loadItemsForMod(modCId, modCManifest, 'components', 'components', 'components');
      await actionLoader.loadItemsForMod(modDId, modDManifest, 'actions', 'actions', 'actions');

      const expectedItems = [
        { type: 'actions', key: 'modA:cool_action', sourceFile: actionFilename, modId: modAId, baseId: coolActionBaseId, originalData: modAActionData },
        { type: 'components', key: 'modA:cool_component', sourceFile: componentFilename, modId: modAId, baseId: coolComponentBaseId, originalData: modAComponentData },
        { type: 'actions', key: 'modB:cool_action', sourceFile: actionFilename, modId: modBId, baseId: coolActionBaseId, originalData: modBActionData },
        { type: 'components', key: 'modB:cool_component', sourceFile: componentFilename, modId: modBId, baseId: coolComponentBaseId, originalData: modBComponentData },
        { type: 'components', key: 'modC:unique_comp', sourceFile: diverseComponentFilename, modId: modCId, baseId: 'unique_comp', originalData: modCComponentData },
        { type: 'actions', key: 'modD:unique_action', sourceFile: diverseActionFilename, modId: modDId, baseId: 'unique_action', originalData: modDActionData },
      ];
