...
describe('Thematic Direction - Event Schema Validation Integration', () => {
  ...
  beforeEach(async () => {
    ...
    characterDatabase = {
      ...
      saveThematicDirections: jest.fn().mockResolvedValue([{
        ...
        description: 'A comprehensive description of a thematic direction that explores the character concept in meaningful depth',
        coreTension: 'The internal struggle between past failures and the drive for redemption',
        uniqueTwist: 'The character discovers their greatest failure was actually their most noble act',
        narrativePotential: 'Rich opportunities for character growth through challenging moral choices and personal sacrifice',
        createdAt: new Date().toISOString(),
        llmMetadata: {}
      }]),
    ...
    try {
      ...
      const thematicDirectionSchema = JSON.parse(
        fs.readFileSync(path.resolve('data/schemas/thematic-direction.schema.json'), 'utf8')
      );
      
      // Check if schema is already loaded to avoid duplicate errors
      if (!schemaValidator.isSchemaLoaded('thematic-direction')) {
        await schemaValidator.addSchema(thematicDirectionSchema, 'thematic-direction');
      }
    } catch (error) {
      mockLogger.warn('Could not load thematic direction schema for storage:', error.message);
    }
    ...
    const mockLlmJsonService = {
      ...
      parseAndRepair: jest.fn().mockImplementation(async (input) => JSON.parse(input))
    };
    ...
    const thematicDirectionGenerator = new ThematicDirectionGenerator({
      ...
      llmStrategyFactory: {
        getAIDecision: jest.fn().mockResolvedValue(JSON.stringify({
          thematicDirections: [
            {
              title: 'Test Direction 1',
              description: 'A comprehensive test description for the first thematic direction that explores the character concept in depth',
              coreTension: 'A fundamental conflict between duty and personal desires that drives the narrative forward',
              uniqueTwist: 'An unexpected revelation about the character past that changes everything',
              narrativePotential: 'Rich opportunities for character growth through challenging moral choices and personal sacrifice'
            },
            {
              title: 'Test Direction 2',
              description: 'An elaborate test description for the second thematic direction that provides meaningful narrative context',
              coreTension: 'The struggle between maintaining old traditions and embracing necessary change',
              uniqueTwist: 'A hidden connection to ancient powers that awakens at a critical moment',
              narrativePotential: 'Compelling storylines exploring themes of identity, legacy, and the price of power'
            },
            {
              title: 'Test Direction 3',
              description: 'A detailed test description for the third thematic direction with sufficient character development ideas',
              coreTension: 'Inner conflict between seeking revenge and finding redemption through forgiveness',
              uniqueTwist: 'The enemy they seek to destroy turns out to be their only hope for salvation',
              narrativePotential: 'Deep exploration of moral ambiguity, redemption arcs, and the transformative power of choice'
            }
          ]
        }))
      },
    ...
    const characterConceptSchema = JSON.parse(
      fs.readFileSync(path.resolve('data/schemas/character-concept.schema.json'), 'utf8')
    );
    const thematicDirectionSchema = JSON.parse(
      fs.readFileSync(path.resolve('data/schemas/thematic-direction.schema.json'), 'utf8')
    );
    ...
    if (!schemaValidator.isSchemaLoaded(thematicDirectionSchema.$id)) {
      await schemaValidator.addSchema(thematicDirectionSchema, thematicDirectionSchema.$id);
    }
    
    // Add character-concept schema which depends on thematic-direction
    if (!schemaValidator.isSchemaLoaded('character-concept')) {
      await schemaValidator.addSchema(characterConceptSchema, 'character-concept');
    }
    ...
    const directionsGeneratedSchemaId = 'thematic:thematic_directions_generated#payload';
  ...
  it('should dispatch CHARACTER_CONCEPT_CREATED event without schema validation warnings', async () => {
    ...
    const concept = await characterBuilderService.createCharacterConcept('A brave knight on a quest', true);
    ...
    const schemaWarnings = warnSpy.mock.calls.filter(call => 
      call[0] && call[0].includes('Payload schema') && 
  ...
  it('should dispatch THEMATIC_DIRECTIONS_GENERATED event without schema validation warnings', async () => {
    ...
    const concept = await characterBuilderService.createCharacterConcept('A mysterious sorceress with ancient powers', true);
    
    // Generate thematic directions
    const directions = await characterBuilderService.generateThematicDirections(concept.id, true);
  ...
  it('should have event schemas properly registered before dispatch', async () => {
    ...
    expect(schemaValidator.isSchemaLoaded(directionsGeneratedSchemaId)).toBe(true);
  });
  ...
  it('should reject invalid payloads', async () => {
    ...
    expect(conceptValidation.errors.some(err => err.params?.missingProperty === 'concept')).toBe(true);
  ...
  it('should use namespaced event names that match the schema IDs', () => {
    ...
    expect(directionsSchemaId).toBe('thematic:thematic_directions_generated#payload');
  });
});