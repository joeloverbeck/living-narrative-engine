...
describe('Scope DSL Phase 2: Enhanced Filtering Integration', () => {
    ...
    mockEntities = {
      ...
      'boots_003': {
        ...
        components: new Map([
          ...
          ['core:material', { type: 'leather', quality: 'high' }],
          ['clothing:armor', { rating: 5, protection: { physical: 5, magical: 0 } }]
        ]),
        getComponentData: function(componentId) {
          return this.components.get(componentId);
        }
      ...
      'steel_helmet_005': {
        ...
        components: new Map([
          ...
          ['clothing:armor', { rating: 10, protection: { physical: 10, magical: 1 } }]
        ]),
        getComponentData: function(componentId) {
          return this.components.get(componentId);
        }
    ...
    mockRuntimeContext = {
      entityManager: {
        getComponentData: jest.fn((entityId, componentId) => {
          if (entityId === 'player_character' && componentId === 'clothing:equipment') {
      ...
      jsonLogicEval: {
        evaluate: jest.fn(function(logic, context) {
          ...
          if (logic.and && Array.isArray(logic.and)) {
            return logic.and.every(condition => this.evaluate(condition, context));
          }
          if (logic.or && Array.isArray(logic.or)) {
            return logic.or.some(condition => this.evaluate(condition, context));
          }
          return false;
        }),
  ...
  describe('Enhanced Entity Context Creation', () => {
    ...
    it('should provide entities with components through getItemComponents gateway method', () => {
      ...
      const entity = mockRuntimeContext.entityManager.getEntity('leather_jacket_001');
      expect(entity).toBeDefined();
      expect(entity.components).toBeDefined();
      expect(entity.components.get('core:tags')).toEqual({ tags: ['waterproof', 'armor', 'outer'] });
    });
  ...
  describe('JSON Logic Property Filtering (Enhanced Parser)', () => {
    it('should create proper evaluation contexts for entities', () => {
      ...
      const { createEvaluationContext } = require('../../../src/scopeDsl/core/entityHelpers.js');
      const gateway = engine._createEntitiesGateway(mockRuntimeContext);
      
      const evalCtx = createEvaluationContext('leather_jacket_001', mockActorEntity, gateway, { getLocation: () => null });
      ...
      expect(evalCtx.entity.components['core:tags']).toEqual({ tags: ['waterproof', 'armor', 'outer'] });
    });

    it('should filter clothing items by tags using JSON Logic', () => {
      const ast = parser.parse('actor.all_clothing[][{"in": ["waterproof", {"var": "components.core:tags.tags"}]}]');
      const result = engine.resolve(ast, mockActorEntity, mockRuntimeContext);
    ...
    it('should filter clothing items by numeric armor rating', () => {
      const ast = parser.parse('actor.all_clothing[][{">": [{"var": "components.clothing:armor.rating"}, 5]}]');
      const result = engine.resolve(ast, mockActorEntity, mockRuntimeContext);
      expect(Array.from(result).sort()).toEqual(['leather_jacket_001', 'steel_helmet_005']);
    });

    it('should filter clothing items by material type', () => {
      const ast = parser.parse('actor.all_clothing[][{"==": [{"var": "components.core:material.type"}, "leather"]}]');
      const result = engine.resolve(ast, mockActorEntity, mockRuntimeContext);
      expect(Array.from(result).sort()).toEqual(['boots_003', 'leather_jacket_001']);
    });
  });
  ...
  describe('Infrastructure Validation', () => {
    it('should have working getItemComponents gateway method', () => {
      ...
      expect(components['core:tags']).toEqual({ tags: ['waterproof', 'armor', 'outer'] });
      expect(components['clothing:armor']).toEqual({ rating: 8, protection: { physical: 8, magical: 2 } });
    });
  ...
  describe('Backward Compatibility', () => {
    it('should maintain existing clothing query functionality', () => {
      ...
      const tests = [
        ...
        { query: 'actor.topmost_clothing.torso_upper', expected: ['leather_jacket_001'] },
