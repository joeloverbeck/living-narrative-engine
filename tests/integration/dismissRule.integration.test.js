/**
 * @file Integration test for the rule that executes the action "core:dismiss"
 * @see tests/integration/dismissRule.integration.test.js
 */

/**
 * @file Integration tests for follower-system rules: `core:handle_dismiss`.
 * @description This file contains comprehensive unit tests for the system rule logic defined in
 * `core:handle_dismiss.rule.json`. The tests verify
...
describe('core:handle_dismiss rule', () => {
  let h;
  ...
  test('Given a leader has a follower, When they dismiss the follower, Then component is removed from target, sync is called, and turn ends', async () => {
    // Arrange
    h = buildTestHarness();
    ...
    await h.eventBus.dispatch('core:attempt_action', {
      actorId: LEADER_ID, // The leader is the actor
      targetId: FOLLOWER_ID, // The follower is the target
      actionId: 'core:dismiss',
    });

    // Assert
    // 1. EntityManager's removeComponent was called on the TARGET
    expect(h.entityManager.removeComponent).toHaveBeenCalledWith(
      FOLLOWER_ID,
      'core:following'
    );

    // 2. LeaderListSyncService was called with the ACTOR's (leader's) ID
    expect(h.leaderSyncService.handleQuery).toHaveBeenCalledWith({
      action: 'rebuildFor',
      leaderIds: [LEADER_ID],
    });

    // 3. A turn_ended event was dispatched for the actor (the leader)
    expect(h.validatedDispatcher.dispatch).toHaveBeenCalledWith(
      'core:turn_ended',
      expect.objectContaining({ entityId: LEADER_ID })
    );
    expect(h.logger.error).not.toHaveBeenCalled();
  });

  test('Given leader and follower are in the SAME location, When dismiss is performed, Then a perceptible_event SHOULD be dispatched', async () => {
    // Arrange
    h = buildTestHarness();
    seedLeaderAndFollower(h.entityManager, {
      followerLocation: LOCATION_A,
  ...
  test('Given leader and follower are in DIFFERENT locations, When dismiss is performed, Then a perceptible_event SHOULD NOT be dispatched', async () => {
    // Arrange
    h = buildTestHarness();
    seedLeaderAndFollower(h.entityManager, {
      followerLocation: LOCATION_A,
