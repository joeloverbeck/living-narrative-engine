...
describe('ActionButtonsRenderer', () => {
    ...
    const createMockElement = (tagName = 'div', id = '', classes = [], textContent = '') => {
        ...
        if (tagName === 'button') {
            Object.defineProperty(element, 'disabled', {
                get: () => isDisabled, set: (value) => { isDisabled = !!value; }, configurable: true
            });
        }
    ...
    beforeEach(() => {
        ...
        jest.spyOn(mockDomElementFactoryInstance, 'p').mockImplementation((cls, text) => {
            const classes = cls ? (Array.isArray(cls) ? cls : cls.split(' ').filter(c => c)) : [];
        ...
        if (!jest.isMockFunction(mockVed.dispatchValidated)) mockVed.dispatchValidated = jest.fn();
    ...
    describe('render() functionality', () => { // Changed describe name slightly
        ...
        it('should render nothing and log debug if actions list is empty', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringMatching(/\[ActionButtonsRenderer\] Rendered 0 action buttons. Selected action: none./)
            );
            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringMatching(/\[ActionButtonsRenderer\] List data is empty or not an array. Displaying empty message./)
            );
            const emptyMessageP = actionButtonsContainer.querySelector('p.empty-list-message');
            expect(emptyMessageP).not.toBeNull();
            if(emptyMessageP) expect(emptyMessageP.textContent).toBe("No actions available.");
        ...
        it('should skip invalid actions (e.g., missing name/command/description) and log warning', async () => {
            ...
            const actionsToSet = [
                validAction1, invalidActionNoName, invalidActionNoCmd,
                {id: 'test:no_desc', name: 'Name No Desc', command: 'cmd_no_desc', description: ""},
                createValidTestAction('test:empty_cmd', 'Empty Command Test', ' ', 'Valid desc for empty cmd'),
                createValidTestAction('test:empty_name', ' ', 'empty_name_cmd', 'Description for empty name'),
                null, {id: null, name: 'Null Id', command: 'null_id_cmd', description: 'Null id desc'},
                {id: 'test:undef_cmd', name: 'Undef Cmd', command: undefined, description: 'Undef cmd desc'},
                {}, createValidTestAction('test:valid2', 'Valid Two', 'do two', 'Description two.')
            ];
            ...
            expect(mockDomElementFactoryInstance.button).toHaveBeenCalledTimes(expectedRenderedCount);
            expect(actionButtonsContainer.appendChild).toHaveBeenCalledTimes(expectedRenderedCount);
            expect(actionButtonsContainer.children.length).toBe(expectedRenderedCount);

            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object in _renderListItem (missing or empty id): `, {actionObject: null});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing name):`, {actionObject: invalidActionNoName});
            // ... (all 9 warning checks) ...
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing command):`, {actionObject: invalidActionNoCmd});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing description):`, {actionObject: actionsToSet[3]});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing command):`, {actionObject: actionsToSet[4]});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing name):`, {actionObject: actionsToSet[5]});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object in _renderListItem (missing or empty id): `, {actionObject: actionsToSet[7]});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object (missing command):`, {actionObject: actionsToSet[8]});
            expect(mockLogger.warn).toHaveBeenCalledWith(`${CLASS_PREFIX} Skipping invalid action object in _renderListItem (missing or empty id): `, {actionObject: {}});
        ...
        it('should treat non-array actions argument as empty list, not log error, and clear container', async () => {
            ...
            const testCases = [
                // ... (same test cases)
                { type: 'textUI:update_available_actions', payload: 'not an object payload' },
                { type: 'textUI:update_available_actions', payload: { actorId: 'testActor', actions: 'not an array' } },
                { type: 'textUI:update_available_actions', payload: { actorId: 'testActor', actions: null } },
                { type: 'textUI:update_available_actions', payload: { actorId: 'testActor', actions: undefined } },
                { type: 'textUI:update_available_actions', payload: { actorId: 'testActor', actions: { /* not an array */ } } },
                { type: 'textUI:update_available_actions', payload: { actorId: '', actions: [] } },
                { type: 'textUI:update_available_actions', payload: null },
                { type: 'textUI:update_available_actions', payload: { actorId: 'testActor' } },
            ...
            for (const eventInputCase of testCases) {
                ...
                const isValidPayloadForProcessing = eventInputCase?.payload &&
                ...
                if (!isValidPayloadForProcessing) {
                    expect(mockLogger.warn).toHaveBeenCalledWith(
                        expect.stringContaining(`${CLASS_PREFIX} Received invalid or incomplete event for '${eventInputCase.type || 'textUI:update_available_actions'}'. Clearing actions.`),
                ...
                const emptyMsgEl = actionButtonsContainer.querySelector('p.empty-list-message');
                expect(emptyMsgEl).not.toBeNull();
                if(emptyMsgEl) expect(emptyMsgEl.textContent).toBe("No actions available.");
        ...
        it('should log error and skip if factory fails to create a button', async () => {
            ...
            if (!jest.isMockFunction(mockDomElementFactoryInstance.p)) {
                jest.spyOn(mockDomElementFactoryInstance, 'p').mockImplementation((cls, text) => createMockElement('p', '', cls, text));
            }
            ...
            expect(mockDomElementFactoryInstance.button).toHaveBeenCalledTimes(actionsForRenderer.length);
