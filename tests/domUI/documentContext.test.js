...
/**
 * @file Unit tests for the DocumentContext class using Jest's JSDOM environment.
 */
...
describe('DocumentContext', () => {
  ...
  beforeEach(() => {
    ...
    document.body.innerHTML =
      '<div id="app-root"><span class="existing-span"></span></div>';
  ...
  describe('when initialized without a root element', () => {
    it('should use the global document for querying', () => {
      ...
      const contextDirectQueryResult =
        docContext.document?.querySelector('#app-root');
  ...
  describe('when initialized with a root element', () => {
    ...
    beforeEach(() => {
      ...
      if (!rootElement) {
        throw new Error(
          "Test setup failed: Could not find #app-root in Jest's global document."
        );
      }
    });
