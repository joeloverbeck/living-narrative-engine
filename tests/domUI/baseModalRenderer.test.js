...
jest.mock('../../src/domUI/boundDomRendererBase.js', () => {
    ...
    const MockCtor = jest.fn().mockImplementation(function (deps) {
        ...
        if (deps.elementsConfig && deps.documentContext && typeof deps.documentContext.query === 'function') {
            for (const key in deps.elementsConfig) {
                if (Object.prototype.hasOwnProperty.call(deps.elementsConfig, key)) {
                    ...
                    if (typeof configValue === 'string') {
                        ...
                        if (this.logger && this.logger.warn) this.logger.warn(`${this._logPrefix} Invalid config for element ${key}`);
                        continue;
                    }
                    try {
                        ...
                        if (this.logger && this.logger.error) this.logger.error(`${this._logPrefix} Error querying for ${key}`, e);
...
describe('BaseModalRenderer', () => {
    ...
    beforeEach(() => {
        ...
        mockModalElement = {
            ...
            classList: {
                add: jest.fn(),
                remove: jest.fn(),
                contains: jest.fn(cls => mockModalElement._classes && mockModalElement._classes.has(cls)),
                _classes: new Set(),
            },
        ...
        mockModalElement.classList.add.mockImplementation(cls => mockModalElement.classList._classes.add(cls));
        mockModalElement.classList.remove.mockImplementation(cls => mockModalElement.classList._classes.delete(cls));
        ...
        mockStatusElement = {
            textContent: '', classList: {
                add: jest.fn(),
                remove: jest.fn(),
                contains: jest.fn(cls => mockStatusElement.classList._classes && mockStatusElement.classList._classes.has(cls)),
                _classes: new Set(),
            }, style: {display: ''},
        };
        mockStatusElement.classList.add.mockImplementation(cls => mockStatusElement.classList._classes.add(cls));
        mockStatusElement.classList.remove.mockImplementation(cls => mockStatusElement.classList._classes.delete(cls));
        ...
        mockDocumentForModal = {
            activeElement: null, body: {
                contains: jest.fn().mockReturnValue(true), focus: jest.fn(),
        ...
        onShowSpy = jest.spyOn(baseModalInstance, '_onShow').mockImplementation(() => {
        });
        onHideSpy = jest.spyOn(baseModalInstance, '_onHide').mockImplementation(() => {
        });
        getInitialFocusElementSpy = jest.spyOn(baseModalInstance, '_getInitialFocusElement').mockReturnValue(null);
        getReturnFocusElementSpy = jest.spyOn(baseModalInstance, '_getReturnFocusElement').mockReturnValue(null);
    ...
    describe('Constructor & Initialization', () => {
        ...
        it('should call _addDomListener for close button, backdrop, and Escape key via _bindCoreModalEvents', () => {
            ...
            expect(mockAddDomListener).toHaveBeenCalledWith(mockCloseButton, 'click', expect.any(Function));
            expect(mockAddDomListener).toHaveBeenCalledWith(mockModalElement, 'click', expect.any(Function));
            expect(mockAddDomListener).toHaveBeenCalledWith(mockDocumentForModal, 'keydown', expect.any(Function));
        });

        it('should set initial aria-hidden attribute on modalElement if it exists', () => {
            expect(mockModalElement.setAttribute).toHaveBeenCalledWith('aria-hidden', 'true');
        });

        it('should log error if modalElement is not found during construction', () => {
            const tempDocContext = {
                ...mockDocumentContextService,
                query: jest.fn(selector => (selector === '#close' ? mockCloseButton : null))
            };
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(newInstance)} 'modalElement' is not defined in 'elementsConfig' or not found in the DOM.`));
        });

        it('should log error if closeButton specified in config is not found', () => {
            const tempDocContext = {
                ...mockDocumentContextService,
                query: jest.fn(selector => (selector === '#modal' ? mockModalElement : null))
            };
            const newInstance = new TestableModalRenderer({
                logger: mockLogger,
                documentContext: tempDocContext,
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(newInstance)} 'closeButton' was configured in 'elementsConfig' but not found in the DOM.`));
        });
        ...
        it('should log warning if closeButton is not specified in config', () => {
            ...
            const tempDocContextQuery = jest.fn(selector => {
                ...
                if (selector === '#status' && configWithoutClose.statusMessageElement) return null;
                return null;
            });
            const tempDocContext = {...mockDocumentContextService, query: tempDocContextQuery};
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(newInstance)} 'closeButton' was not specified in 'elementsConfig'.`));
            if (configWithoutClose.statusMessageElement) {
                expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(newInstance)} 'statusMessageElement' was configured in 'elementsConfig' but not found in the DOM.`));
            }
        });
    });
    ...
    describe('show()', () => {
        it('should make the modal visible, update styles, classes, and ARIA attributes', () => {
            ...
            expect(mockModalElement.setAttribute).toHaveBeenCalledWith('aria-hidden', 'false');
            expect(mockLogger.info).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Modal shown.`);
        });

        it('should call lifecycle hooks: _clearStatusMessage (if exists), _onShow, _getInitialFocusElement', () => {
            const clearStatusSpy = jest.spyOn(baseModalInstance, '_clearStatusMessage');
            baseModalInstance.show();
        ...
        it('should fallback to focus closeButton if _getInitialFocusElement returns null and closeButton is focusable', () => {
            ...
            if (getInitialFocusElementSpy.mockRestore) getInitialFocusElementSpy.mockRestore();
        ...
        it('should store document.activeElement as _previouslyFocusedElement', () => {
            const dummyActiveElement = {id: 'dummy-active', focus: jest.fn(), offsetParent: {}};
            mockDocumentForModal.activeElement = dummyActiveElement;
            baseModalInstance.show();
            expect(baseModalInstance._previouslyFocusedElement).toBe(dummyActiveElement);
        });
        ...
        it('should log warning and return if already visible', () => {
            ...
            baseModalInstance.show();
            expect(mockLogger.warn).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} show() called when modal is already visible.`);
            expect(mockModalElement.classList.add).not.toHaveBeenCalledWith('visible');
        });
        ...
        it('should log error and return if modalElement is not available (e.g. destroyed)', () => {
            ...
            baseModalInstance.show();
            expect(mockLogger.error).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Cannot show modal, 'modalElement' is not available.`);
            expect(baseModalInstance.isVisible).toBe(false);
        });
    });
    ...
    describe('hide()', () => {
        beforeEach(() => {
            ...
            baseModalInstance._previouslyFocusedElement = {focus: jest.fn(), offsetParent: {}};
        ...
        it('should make the modal hidden, update styles, classes, and ARIA attributes', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Modal hidden.`);
        });
        ...
        it('should return early if not visible', () => {
            ...
            onHideSpy.mockClear();
            baseModalInstance.hide();
            expect(mockModalElement.classList.remove).not.toHaveBeenCalledWith('visible');
        ...
        it('should log error if modalElement is not available', () => {
            baseModalInstance.elements.modalElement = null;
            baseModalInstance.hide();
            expect(mockLogger.error).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Cannot hide modal, 'modalElement' is not available.`);
            expect(baseModalInstance.isVisible).toBe(false);
        });
    });
    ...
    describe('Event Handling via _bindCoreModalEvents', () => {
        ...
        beforeEach(() => {
            ...
            const closeCall = mockAddDomListener.mock.calls.find(call => call[0] === baseModalInstance.elements.closeButton && call[1] === 'click');
            closeButtonClickHandler = closeCall ? closeCall[2] : null;

            const backdropCall = mockAddDomListener.mock.calls.find(call => call[0] === baseModalInstance.elements.modalElement && call[1] === 'click');
            backdropClickHandler = backdropCall ? backdropCall[2] : null;

            const escapeCall = mockAddDomListener.mock.calls.find(call => call[0] === mockDocumentForModal && call[1] === 'keydown');
            escapeKeyHandler = escapeCall ? escapeCall[2] : null;
        });

        afterEach(() => { // Added afterEach for this describe block
            if (hidePrototypeSpy) {
    ...
    describe('Status Message Methods', () => {
        ...
        it('_displayStatusMessage should set text and class', () => {
            ...
            expect(mockStatusElement.classList.add).toHaveBeenCalledWith('status-message-area');
            expect(mockStatusElement.classList.add).toHaveBeenCalledWith('status-message-success');
        ...
        it('_displayStatusMessage should remove previous type class', () => {
            ...
            expect(mockStatusElement.classList.remove).toHaveBeenCalledWith('status-message-info');
            expect(mockStatusElement.classList.add).toHaveBeenCalledWith('status-message-error');
            expect(baseModalInstance._currentStatusTypeClass).toBe('status-message-error');
        });
        ...
        it('_displayStatusMessage should log warning if statusElement is not available', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} '_displayStatusMessage' called, but 'statusMessageElement' is not available. Message: "No show" (type: info)`);
        });
        ...
        it('_clearStatusMessage should clear text and remove class', () => {
            ...
            expect(mockStatusElement.classList.remove).toHaveBeenCalledWith('status-message-success');
        ...
        it('_clearStatusMessage should do nothing if statusElement not available (no error, logs debug)', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} '_clearStatusMessage' called, but 'statusMessageElement' is not available. No message to clear.`);
        });
    });
    ...
    describe('_setOperationInProgress()', () => {
        beforeEach(() => {
            ...
            baseModalInstance._operationInProgressAffectedElements = ['confirmButton', 'cancelButton'];
        });
        ...
        it('should disable configured elements when inProgress is true', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Operation in progress state set to true. Affected elements: confirmButton, cancelButton.`);
        });
        ...
        it('should enable configured elements when inProgress is false', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Operation in progress state set to false. Affected elements: confirmButton, cancelButton.`);
        });
        ...
        it('should log debug if an element key is configured but element not found', () => {
            ...
            baseModalInstance._setOperationInProgress(true);
            expect(mockConfirmButton.disabled).toBe(true);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(baseModalInstance)} Element "cancelButton" not found for _setOperationInProgress (may be optional for this modal).`));
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(baseModalInstance)} Operation in progress state set to true.`));
        });

        it('should log warning if an element is found but does not support "disabled" property', () => {
            const mockNonDisableableElement = {id: 'nonDisableable', offsetParent: {}};
            baseModalInstance.elements.nonDisableableButton = mockNonDisableableElement;
            baseModalInstance._operationInProgressAffectedElements = ['nonDisableableButton'];
            baseModalInstance._setOperationInProgress(true);
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`${getExpectedLogPrefix(baseModalInstance)} Element "nonDisableableButton" found for '_setOperationInProgress' but does not support 'disabled' property.`));
        });
        ...
        it('should log debug and return if _operationInProgressAffectedElements is empty', () => {
            ...
            baseModalInstance._setOperationInProgress(true);
            expect(mockLogger.debug).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} No elements configured for '_setOperationInProgress'.`);
            expect(mockConfirmButton.disabled).toBe(false);
        });
    });
    ...
    describe('Default Lifecycle Hook Behaviors', () => {
        beforeEach(() => {
            if (getInitialFocusElementSpy && getInitialFocusElementSpy.mockRestore) getInitialFocusElementSpy.mockRestore();
            if (getReturnFocusElementSpy && getReturnFocusElementSpy.mockRestore) getReturnFocusElementSpy.mockRestore();
        ...
        it('_getInitialFocusElement should return closeButton by default if available and focusable', () => {
            ...
            expect(typeof baseModalInstance.elements.closeButton.focus).toBe('function');
        ...
        it('_getInitialFocusElement should return modalElement if closeButton is unavailable and modalElement has tabindex and is focusable', () => {
            ...
            baseModalInstance.elements.modalElement = mockModalElement;
            mockModalElement.hasAttribute.mockReturnValue(true);
            expect(typeof baseModalInstance.elements.modalElement.focus).toBe('function');
            const focusElement = baseModalInstance._getInitialFocusElement();
    ...
    describe('destroy()', () => {
        ...
        it('should log that it was destroyed', () => {
            baseModalInstance.destroy();
            expect(mockLogger.info).toHaveBeenCalledWith(`${getExpectedLogPrefix(baseModalInstance)} Destroyed.`);
        });
    });
});