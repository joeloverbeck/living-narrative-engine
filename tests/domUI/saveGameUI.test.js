...
describe('SaveGameUI', () => {
    ...
    let listContainerElement, saveNameInputEl, confirmSaveButtonEl, statusMessageElement;
    ...
    beforeEach(() => {
        ...
        global.InputEvent = mockWindow.InputEvent || function (type, options) {
        ...
        statusMessageElement = mockDocument.getElementById('save-game-status-message');

        mockDocumentContext = {
            query: jest.fn((selector, element) => (element || mockDocument).querySelector(selector)),
    ...
    async function awaitMockCall(mockFn, callIndex = 0) {
        ...
        if (mockFn.mock.results[callIndex] && mockFn.mock.results[callIndex].type === 'return') {
            if (mockFn.mock.results[callIndex].value && typeof mockFn.mock.results[callIndex].value.then === 'function') {
    ...
    describe('Save Operation and Slot Re-selection', () => {
        it('should correctly re-select the slot after a successful save to an empty slot', async () => {
            ...
            mockGameEngine.triggerManualSave.mockResolvedValue({
                success: true, message: 'Game saved successfully!', filePath: newSaveFilePath
            });

            saveGameUI.show();
            await awaitMockCall(mockSaveLoadService.listManualSaveSlots, 0);

            const firstSlotElement = listContainerElement.querySelector('.save-slot[data-slot-id="0"]');
            // --- JEST MATCHER FIX ---
            expect(firstSlotElement).not.toBeNull();
            // --- END FIX ---
            if (!firstSlotElement) {
                console.error('Test Error: firstSlotElement is null after initial population. HTML:', listContainerElement.innerHTML);
            ...
            saveNameInputEl.dispatchEvent(new global.InputEvent('input', {bubbles: true}));
            ...
            expect(mockGameEngine.triggerManualSave).toHaveBeenCalledWith(saveName, undefined);
            expect(mockSaveLoadService.listManualSaveSlots).toHaveBeenCalledTimes(2);
            expect(mockLogger.warn).not.toHaveBeenCalledWith(expect.stringContaining('Could not find metadata for newly saved slot'));
            expect(mockLogger.error).not.toHaveBeenCalledWith(expect.stringContaining('Save operation succeeded but did not return a valid filePath/identifier'));
            ...
            const selectedSlotInDOM = listContainerElement.querySelector('.save-slot.selected');
            ...
            expect(statusMessageElement.textContent).toBe(`Game saved as "${saveName}".`);
            expect(statusMessageElement.classList.contains('status-message-success')).toBe(true);
        });
        ...
        it('should log an error if save succeeds but filePath is missing in the result', async () => {
            ...
            mockSaveLoadService.listManualSaveSlots.mockResolvedValueOnce([]);
            mockGameEngine.triggerManualSave.mockResolvedValue({
                success: true, message: 'Saved but no path!', filePath: undefined
            });
            ...
            // --- END FIX ---
            if (!firstSlotElement) {
                console.error('Test Error: firstSlotElement is null in filePath missing test. HTML:', listContainerElement.innerHTML);
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                expect.stringContaining('Save operation succeeded but did not return a valid filePath/identifier'),
                expect.any(Object)
            );
            expect(mockLogger.warn).toHaveBeenCalledWith(
                expect.stringContaining(`Could not find metadata for newly saved slot named "${saveName}" to re-select. Searched with ID: undefined`)
            );
        });
    });
});
