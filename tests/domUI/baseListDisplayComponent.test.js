...
describe('BaseListDisplayComponent', () => {
    ...
    it('should throw an error if listContainerElement is not found in elementsConfig during construction', () => {
        ...
        expect(() => createInstance({listContainerElement: '#list-container-missing'}))
    ...
    it('should call abstract methods if they are not implemented by subclass (and throw)', () => {
        ...
        expect(() => instance._getListItemsData()).toThrow('Abstract method _getListItemsData() not implemented.');
        expect(() => instance._renderListItem({}, 0, [])).toThrow('Abstract method _renderListItem() not implemented.');
        expect(() => instance._getEmptyListMessage()).toThrow('Abstract method _getEmptyListMessage() not implemented.');
    });
    ...
    describe('renderList()', () => {
        ...
        it('should log an error and return if listContainerElement is not available', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Cannot render list: 'listContainerElement' is not available."));
        ...
        describe('Empty List Handling', () => {
            it('should call _getEmptyListMessage and append its string result as a paragraph', async () => {
                ...
                expect(mockDomElementFactory.p).toHaveBeenCalledWith('empty-list-message', emptyMsg);
                expect(listContainerElement.innerHTML).toBe('<p>The list is quite empty!</p>');
                expect(instance._onListRendered).toHaveBeenCalledWith(expect.arrayContaining([]), listContainerElement);
            });
            ...
            it('should append HTMLElement result from _getEmptyListMessage directly', async () => {
                ...
                expect(instance._onListRendered).toHaveBeenCalledWith(null, listContainerElement);
            });
            ...
            it('should handle if _getEmptyListMessage returns invalid type and use fallback', async () => {
                ...
                expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("_getEmptyListMessage() returned an invalid type"), expect.any(Object));
            ...
            it('should handle if domElementFactory is not available for string empty message', async () => {
                instance = new TestListDisplayComponent({ // Recreate instance without domElementFactory
                    logger: mockLogger,
                    documentContext: mockDocumentContext,
                    validatedEventDispatcher: mockValidatedEventDispatcher,
        ...
        describe('Populating List with Items', () => {
            ...
            it('should call _renderListItem for each item and append valid HTMLElement results', async () => {
                ...
                expect(instance._renderListItem).toHaveBeenCalledTimes(sampleData.length);
                expect(instance._renderListItem).toHaveBeenCalledWith(sampleData[0], 0, sampleData);
                expect(instance._renderListItem).toHaveBeenCalledWith(sampleData[1], 1, sampleData);
                ...
                expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Rendered ${sampleData.length} out of ${sampleData.length} items`));
                expect(instance._onListRendered).toHaveBeenCalledWith(sampleData, listContainerElement);
            });
            ...
            it('should handle _renderListItem returning null by skipping the item', async () => {
                ...
                expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`Rendered 1 out of ${sampleData.length} items`));
            });
            ...
            it('should log warning if _renderListItem returns non-HTMLElement and non-null', async () => {
                ...
                expect(mockLogger.warn).toHaveBeenCalledWith(
                    expect.stringContaining('_renderListItem for item at index 0 did not return an HTMLElement or null'),
                    expect.anything()
                );
            ...
            it('should log error and continue if _renderListItem throws an error', async () => {
                ...
                expect(mockLogger.error).toHaveBeenCalledWith(
                    expect.stringContaining('Error in _renderListItem for item at index 0:'),
                    error,
                    expect.anything()
                );
        ...
        it('should handle asynchronous _getListItemsData', async () => {
            ...
            instance._getListItemsData.mockReturnValue(new Promise(resolve => setTimeout(() => resolve(asyncData), 0))); // Use 0 for faster test
            ...
            expect(instance._onListRendered).toHaveBeenCalledWith(asyncData, listContainerElement);
        });
        ...
        it('should handle _getListItemsData throwing an error', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error fetching list items data'), error);
            expect(DomUtils.clearElement).toHaveBeenCalledWith(listContainerElement);
        ...
        it('should call _onListRendered with itemsData and container after populating', async () => {
            ...
            instance._renderListItem.mockReturnValue(dom.window.document.createElement('div'));

            await instance.renderList();
            expect(instance._onListRendered).toHaveBeenCalledWith(data, listContainerElement);
        });
        ...
        it('should call _onListRendered even if _getListItemsData returns empty', async () => {
            ...
            await instance.renderList();
            expect(instance._onListRendered).toHaveBeenCalledWith([], listContainerElement);
        });
        ...
        it('should gracefully handle errors in _onListRendered', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Error in _onListRendered hook:"), onRenderedError);
        });
    });
    ...
    describe('refreshList()', () => {
        it('should call renderList and return its promise', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("refreshList() called, invoking renderList()."));
        });
    });
