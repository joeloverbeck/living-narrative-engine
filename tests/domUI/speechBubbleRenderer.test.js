...
describe('SpeechBubbleRenderer', () => {
    let logger, docContext, eventDispatcher, entityManager, domFactory, entityDisplayDataProvider, renderer;
    ...
    describe('Constructor and Initialization', () => {
        it('should initialize and subscribe to DISPLAY_SPEECH_ID', () => {
            ...
            expect(logger.debug).toHaveBeenCalledWith(`${renderer._logPrefix} Initialized and subscribed to ${DISPLAY_SPEECH_ID}.`);
        ...
        it('should warn if #message-list is not found and use #outputDiv as fallback', () => {
            ...
            const localEntityDisplayDataProvider = createMockEntityDisplayDataProvider();
            ...
            expect(localLogger.warn).toHaveBeenCalledWith(`${tempRenderer._logPrefix} #message-list not found. Speech will be appended to #outputDiv.`);
        ...
        it('should error if #outputDiv is not found (and thus effectiveSpeechContainer is null)', () => {
            ...
            const localDocContext = createMockDocumentContext();
            const localEntityDisplayDataProvider = createMockEntityDisplayDataProvider();
            localDocContext.query = jest.fn(s => (s === '#outputDiv' || s === '#message-list') ? null : undefined);
            ...
            expect(localLogger.error).toHaveBeenCalledWith(expect.stringContaining("Element 'outputDivElement' with selector '#outputDiv' not found. (Required)"));
            expect(localLogger.error).toHaveBeenCalledWith(`${tempRenderer._logPrefix} Critical: Effective speech container (#message-list or #outputDiv) could not be determined as #outputDiv was also not found or bound.`);
    ...
    describe('#onDisplaySpeech event handler', () => {
        ...
        beforeEach(() => {
            ...
            const subscribeCall = eventDispatcher.subscribe.mock.calls.find(call => call[0] === DISPLAY_SPEECH_ID);
        ...
        it('should warn and not call renderSpeech if event payload is invalid (missing entityId)', () => {
            ...
            eventCallback({type: DISPLAY_SPEECH_ID, payload: p});
            expect(logger.warn).toHaveBeenCalledWith(`${renderer._logPrefix} Invalid payload for 'textUI:display_speech'.`, p);
        ...
        it('should warn and not call renderSpeech if event object or payload is null/undefined', () => {
            eventCallback(null);
            expect(logger.warn).toHaveBeenCalledWith(`${renderer._logPrefix} Received invalid 'textUI:display_speech' event object.`, null);

            const evNoPayload = {type: DISPLAY_SPEECH_ID, payload: null};
            eventCallback(evNoPayload);
            expect(logger.warn).toHaveBeenCalledWith(`${renderer._logPrefix} Received invalid 'textUI:display_speech' event object.`, evNoPayload);
            expect(renderSpeechSpy).not.toHaveBeenCalled();
        });
    });
    ...
    describe('renderSpeech method', () => {
        ...
        const getQuotedTextSpanFromDomFactory = () => {
            ...
            const quotedTextSpanCallIndex = spanCalls.findIndex(callArgs => callArgs[0] === 'speech-quoted-text');
        ...
        it('should render simple speech without portrait if entity not found', () => {
            entityDisplayDataProvider.getEntityName.mockReturnValue(DEFAULT_SPEAKER_NAME_IN_TEST);
            ...
            expect(entityDisplayDataProvider.getEntityName).toHaveBeenCalledWith(entityId, DEFAULT_SPEAKER_NAME_IN_TEST);
            expect(entityDisplayDataProvider.getEntityPortraitPath).toHaveBeenCalledWith(entityId);
        ...
        it('should render speech with name and portrait if entity and components found', () => {
            ...
            entityDisplayDataProvider.getEntityPortraitPath.mockReturnValue(portraitPath);
            ...
            expect(entityDisplayDataProvider.getEntityName).toHaveBeenCalledWith(entityId, DEFAULT_SPEAKER_NAME_IN_TEST);
            expect(entityDisplayDataProvider.getEntityPortraitPath).toHaveBeenCalledWith(entityId);
            expect(domFactory.img).toHaveBeenCalledWith(portraitPath, `Portrait of ${speakerName}`, 'speech-portrait');
        ...
        it('should handle entity without a name component (EDDP returns default)', () => {
            ...
            entityDisplayDataProvider.getEntityPortraitPath.mockReturnValue('/data/mods/myMod/portraits/bob.png');
        ...
        it('should handle entity without a portrait component (EDDP returns null)', () => {
            entityDisplayDataProvider.getEntityName.mockReturnValue('NoPortraitPerson');
            entityDisplayDataProvider.getEntityPortraitPath.mockReturnValue(null);
            ...
            const speechEntryDivCall = domFactory.create.mock.calls.find(call => call[0] === 'div' && call[1] && call[1].cls === 'speech-entry');
            expect(speechEntryDivCall).toBeDefined(); // Check if the create call happened
            const speechEntryDiv = domFactory.create.mock.results[domFactory.create.mock.calls.indexOf(speechEntryDivCall)].value;

            expect(speechEntryDiv).toBeDefined();
            expect(speechEntryDiv.classList.add).toHaveBeenCalledWith('no-portrait');
            expect(speechEntryDiv.classList.add).not.toHaveBeenCalledWith('has-portrait');
        });
        ...
        describe('Action Text Parsing', () => {
            ...
            it('should correctly parse and render one action text segment', () => {
                ...
                expect(children[2].classList.add).toHaveBeenCalledWith('speech-action-text');
            ...
            it('should correctly parse and render multiple action text segments', () => {
                ...
                expect(children[4].classList.add).toHaveBeenCalledWith('speech-action-text');
    ...
    describe('#scrollToBottom', () => {
        ...
        it('should warn if elements.outputDivElement is not available', () => {
            ...
            renderer.renderSpeech('id', 'test');
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('Could not scroll #outputDiv. Element not found in this.elements'));
        });
    });
    describe('dispose method', () => {
        it('should call super.dispose and nullify effectiveSpeechContainer', () => {
            const superDisposeSpy = jest.spyOn(BoundDomRendererBase.prototype, 'dispose');
            renderer.dispose();
            expect(superDisposeSpy).toHaveBeenCalled();
            expect(renderer.effectiveSpeechContainer).toBeNull();
            expect(logger.debug).toHaveBeenCalledWith(`${renderer._logPrefix} Disposing.`);
            expect(logger.info).toHaveBeenCalledWith(`${renderer._logPrefix} Disposed.`);
