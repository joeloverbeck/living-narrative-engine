...
const createMockElement = (tag = 'div', id = null) => {
    ...
    const classListMock = {
        ...
        add: jest.fn((...classes) => classes.forEach(c => {
            if (typeof c === 'string') c.trim().split(/\s+/).forEach(cn => {
                if (cn) _internalClassSet.add(cn);
            });
        })),
        remove: jest.fn((...classes) => classes.forEach(c => {
            if (typeof c === 'string') c.trim().split(/\s+/).forEach(cn => {
                if (cn) _internalClassSet.delete(cn);
            });
        })),
        contains: jest.fn(cls => typeof cls === 'string' && cls.trim() !== '' && _internalClassSet.has(cls.trim())),
        toggle: jest.fn((cls, force) => {
            ...
            if (shouldHave) _internalClassSet.add(trimmedClass); else _internalClassSet.delete(trimmedClass);
            return shouldHave;
        }),
    ...
    const element = {
        ...
        getAttribute: jest.fn(name => _attributes[name] === undefined ? null : _attributes[name]), // Mock getAttribute
        hasAttribute: jest.fn(name => _attributes.hasOwnProperty(name)), // Mock hasAttribute
        focus: jest.fn(),
        querySelectorAll: jest.fn(selector => {
            if (selector === 'li.llm-item') {
                return element.children.filter(c => c.tag === 'li' && c.classList && c.classList.contains('llm-item'));
            }
            return [];
        }),
        querySelector: jest.fn(selector => {
            if (selector === 'li.llm-item.selected[tabindex="0"]') {
                return element.children.find(c => c.tag === 'li' && c.classList && c.classList.contains('llm-item') && c.classList.contains('selected') && c.attributes['tabindex'] === '0') || null;
            }
            if (selector === 'li.llm-item[tabindex="0"]') {
                return element.children.find(c => c.tag === 'li' && c.classList && c.classList.contains('llm-item') && c.attributes['tabindex'] === '0') || null;
            }
            return null;
        }),
    };
...
describe('LlmSelectionModal', () => {
    ...
    beforeEach(() => {
        ...
        documentContextMock.document.body.contains = jest.fn().mockReturnValue(true);
        ...
        llmAdapterMock = {getAvailableLlmOptions: jest.fn(), getCurrentActiveLlmId: jest.fn(), setActiveLlm: jest.fn()};
        ...
        llmSelectionModal = new LlmSelectionModal({
            logger: loggerMock, documentContext: documentContextMock, domElementFactory: domElementFactoryMock,
            llmAdapter: llmAdapterMock, validatedEventDispatcher: validatedEventDispatcherMock,
        });
    });
    ...
    describe('show() method', () => {
        it('should log warnings and show 0 valid options if adapter returns an array of strings (simulating bug)', async () => {
            ...
            expect(loggerMock.debug).toHaveBeenCalledWith('[LlmSelectionModal] Fetching LLM list data...');
            expect(loggerMock.debug).toHaveBeenCalledWith(`[LlmSelectionModal] Fetched ${mockLlmIdsAsStrings.length} LLM options. Active ID: ${currentActiveId}`);
            ...
            expect(loggerMock.info).toHaveBeenCalledWith('[LlmSelectionModal] _onShow hook called. Rendering LLM list.');
            expect(loggerMock.info).toHaveBeenCalledWith('[LlmSelectionModal] Modal shown.');
            ...
            const emptyMessageCreationCall = domElementFactoryMock.create.mock.calls.find(
                call => call[0] === 'li' && call[1]?.text === 'No Language Models are currently configured.'
            );
        ...
        it('should display a "no options" message if adapter returns an empty list', async () => {
            ...
            expect(appendedArgs[0][0].textContent).toBe('No Language Models are currently configured.');
            expect(appendedArgs[0][0].classList.contains('llm-empty-message')).toBe(true);

            expect(loggerMock.warn).toHaveBeenCalledWith('[LlmSelectionModal] LLM list is empty or failed to load. Displaying empty/error message.');
            expect(loggerMock.info).toHaveBeenCalledWith(
        ...
        it('should display an error message if fetching LLM options fails', async () => {
            ...
            llmAdapterMock.getAvailableLlmOptions.mockRejectedValue(new Error(errorMessageContent));
            llmAdapterMock.getCurrentActiveLlmId.mockResolvedValue(null);
            ...
            const expectedUserErrorMessage = 'Failed to load Language Model list. Please try again later.';
            expect(domElementFactoryMock.create).toHaveBeenCalledWith('li', {
            ...
            expect(appendedArgs[0][0].classList.contains('llm-error-message')).toBe(true);
