...
jest.mock('../../src/domUI/rendererBase.js', () => {
    const RendererBaseMock = jest.fn().mockImplementation(function (params) {
        ...
        this._addSubscription = jest.fn(fn => this._managedValidatedEventSubscriptions.push(fn));
        this._addDomListener = jest.fn(listener => this._managedDomListeners.push(listener));
    });
...
describe('BoundDomRendererBase', () => {
    ...
    const MockedRendererBaseActual = require('../../src/domUI/rendererBase.js').RendererBase;
    ...
    it('should throw error if elementsConfig is missing or not an object in constructor', () => {
        ...
        expect(() => new ConcreteBoundRenderer({
            logger: mockLogger,
            documentContext: mockDocumentContext,
            validatedEventDispatcher: mockValidatedEventDispatcher,
        })).toThrow("'elementsConfig' must be provided as an object.");
        expect(mockLogger.error).toHaveBeenCalledWith(`${expectedPrefixForLogError} 'elementsConfig' must be provided as an object.`);
    ...
    it('should bind required elements successfully using shorthand and full config', () => {
        const mockTitleElement = {nodeType: 1, tagName: 'H1', constructor: {name: 'HTMLHeadingElement'}};
        const mockContainerElement = {nodeType: 1, tagName: 'DIV', constructor: {name: 'HTMLDivElement'}};
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith("[ConcreteBoundRenderer] Successfully bound element 'title' to selector '#title'.");
        expect(mockLogger.debug).toHaveBeenCalledWith("[ConcreteBoundRenderer] Successfully bound element 'mainContainer' to selector '.container'.");
    });
    ...
    it('should log an error if a required element (default) is not found', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith("[ConcreteBoundRenderer] Element 'missingRequired' with selector '#missing' not found. (Required)");
    });
    ...
    it('should log an error if a required element (explicit) is not found', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith("[ConcreteBoundRenderer] Element 'anotherMissing' with selector '#another' not found. (Required)");
    });
    ...
    it('should log a warning if an optional element is not found', () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith("[ConcreteBoundRenderer] Element 'missingOptional' with selector '#optional' not found. (Optional)");
        expect(mockLogger.error).not.toHaveBeenCalled();
    });
    ...
    it('should log a warning if element type does not match expectedType', () => {
        ...
        const elementsConfig = {
            typedElement: {selector: '#div-as-button', expectedType: HTMLButtonElement}
        };
        const renderer = new ConcreteBoundRenderer({
            logger: mockLogger,
            documentContext: mockDocumentContext,
    ...
    it('should correctly bind element if expectedType matches', () => {
        ...
        const elementsConfig = {
            correctlyTypedElement: {selector: '#real-button', expectedType: HTMLButtonElement}
        };
        const renderer = new ConcreteBoundRenderer({
            logger: mockLogger,
            documentContext: mockDocumentContext,
        ...
        const typeMismatchWarning = warnCalls.find(call => call[0].includes("not of expected type"));
    ...
    it('dispose method should clear elements object and call super.dispose', () => {
        ...
        const superDisposeSpy = jest.spyOn(MockedRendererBaseActual.prototype, 'dispose');

        renderer.dispose();

        expect(superDisposeSpy).toHaveBeenCalledTimes(1);
        expect(renderer.elements).toEqual({});
        expect(mockLogger.debug).toHaveBeenCalledWith('[ConcreteBoundRenderer] Bound DOM elements cleared.');
        expect(mockLogger.debug).toHaveBeenCalledWith('[ConcreteBoundRenderer] MockedRendererBase dispose called.');
