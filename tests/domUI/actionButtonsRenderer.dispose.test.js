...
describe('ActionButtonsRenderer', () => {
  ...
  const createMockElement = (
    ...
    const originalClassListRemove =
      actualClassList.remove.bind(actualClassList);
    const originalClassListContains =
      actualClassList.contains.bind(actualClassList);
  ...
  beforeEach(() => {
    ...
    methodsToSpy.forEach((methodName) => {
      ...
      if (
        typeof mockLogger[methodName] === 'function' &&
        jest.isMockFunction(mockLogger[methodName])
      ) {
        const originalMockImpl =
          mockLogger[methodName].getMockImplementation() || jest.fn();
        mockLogger[methodName].mockImplementation((...args) => {
          // Log to Jest console for visibility during test run
          console.log(
            `[JEST_TEST_LOGGER.${methodName.toUpperCase()}]:`,
            ...args.map((arg) =>
        ...
        mockLogger[methodName] = jest.fn((...args) => {
          console.log(
            `[JEST_TEST_LOGGER.${methodName.toUpperCase()} STUBBED]:`,
            ...args.map((arg) =>
              typeof arg === 'object' ? JSON.stringify(arg, null, 2) : arg
            )
          );
    ...
    if (!jest.isMockFunction(mockDomElementFactoryInstance.button)) {
      mockDomElementFactoryInstance.button = jest
        .fn()
    ...
    if (!jest.isMockFunction(mockDomElementFactoryInstance.create)) {
      mockDomElementFactoryInstance.create = jest
        .fn()
    ...
    if (
      mockSendButtonEl &&
      !jest.isMockFunction(mockSendButtonEl.removeEventListener)
    ) {
  ...
  describe('dispose()', () => {
    ...
    beforeEach(() => {
      ...
      if (
        rendererForDispose.elements.sendButtonElement &&
        !jest.isMockFunction(
          rendererForDispose.elements.sendButtonElement.removeEventListener
        )
      ) {
        jest.spyOn(
          rendererForDispose.elements.sendButtonElement,
    ...
    it('should unsubscribe from VED event and remove send button listener', () => {
      ...
      expect(boundSendButton.removeEventListener).toHaveBeenCalledWith(
        'click',
        expect.any(Function),
        undefined
      );
    ...
    it('should call base class dispose (logs message from base and specific dispose)', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `${CLASS_PREFIX} Starting disposal: Unsubscribing VED events and removing DOM listeners.`
        )
      );
    ...
    it('should clear action buttons container and reset internal state', () => {
      if (!actionButtonsContainerEl)
        throw new Error('actionButtonsContainerEl is null in test');
      ...
      rendererForDispose.selectedAction =
        rendererForDispose.availableActions[0];
