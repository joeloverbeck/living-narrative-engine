...
async function cleanupOrphanedTestDirectories() {
  ...
  try {
    ...
    if (testTempDirs.length > 0) {
      console.log(`[Test Setup] Found ${testTempDirs.length} orphaned test directories to clean up`);
      
      // Remove each test directory
      for (const dir of testTempDirs) {
        try {
          await fs.rm(dir, { recursive: true, force: true });
          console.log(`[Test Setup] Removed orphaned test directory: ${path.basename(dir)}`);
        } catch (error) {
    ...
    if (error.code !== 'ENOENT') {
      console.warn('[Test Setup] Error during test directory cleanup:', error.message);
    }
  }
}
