// src/tests/core/turnOrder/turnOrderService.startNewRound.errors.test.js
...
describe('TurnOrderService', () => {
    ...
    describe("startNewRound (Error Handling)", () => {
        ...
        it("Test Case 11.4.1: should throw error and log if entities array is null, undefined, or empty", () => {
            ...
            const expectedLogMsg = 'TurnOrderService.startNewRound: Failed - entities array must be a non-empty array.';
            ...
            inputs.forEach(invalidEntities => {
                ...
                expect(() => service.addEntity({id: 'test'})).toThrow('Cannot add entity: No round is active.');
                // removeEntity logs a warning but doesn't throw if no round active
                service.removeEntity('test');
                expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('when no round is active'));
        ...
        it("Test Case 11.4.2: should throw error and log if entities array contains invalid items", () => {
            ...
            const invalidEntities = [{id: 'a'}, {name: 'no id'}, {id: ''}, null];
            const expectedErrorMsg = 'Entities array contains invalid entities.';
            const expectedLogMsg = 'TurnOrderService.startNewRound: Failed - entities array contains invalid entities (missing or invalid id).';
        ...
        it("Test Case 11.4.3: should throw error and log if strategy is 'initiative' and initiativeData is missing", () => {
            ...
            const entities = [{id: 'a'}];
            const expectedErrorMsg = 'Valid initiativeData Map is required for the "initiative" strategy.';
            const expectedSpecificLogMsg = 'TurnOrderService.startNewRound (initiative): Failed - initiativeData Map is required and must not be empty.';
            ...
            inputs.forEach(invalidData => {
                ...
                expect(mockLogger.error).toHaveBeenCalledWith(expectedCatchLogMsg, expect.any(Error));
                ...
                expect(mockLogger.info).toHaveBeenCalledWith(`TurnOrderService: Starting new round with strategy "${strategy}".`);
                expect(mockLogger.info).toHaveBeenCalledWith('TurnOrderService: Current round state cleared.'); // Called twice
                expect(mockLogger.debug).not.toHaveBeenCalledWith('TurnOrderService: Cleared existing turn queue.'); // Still no *existing* queue cleared
                expect(service.isEmpty()).toBe(true);
                expect(service.peekNextEntity()).toBeNull();
                expect(service.getCurrentOrder()).toEqual([]);
        ...
        it("Test Case 11.4.5: should throw error and log if the strategy is unsupported", () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('TurnOrderService initialized.'); // Constructor log
            expect(mockLogger.info).toHaveBeenCalledWith(`TurnOrderService: Starting new round with strategy "${unsupportedStrategy}".`);
            expect(mockLogger.info).toHaveBeenCalledWith('TurnOrderService: Current round state cleared.'); // Called twice
            expect(mockLogger.debug).not.toHaveBeenCalledWith('TurnOrderService: Cleared existing turn queue.');
