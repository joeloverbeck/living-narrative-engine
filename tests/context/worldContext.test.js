...
describe('WorldContext (Stateless)', () => {
    ...
    beforeEach(() => {
        ...
        mockEntityManager.getPrimaryInstanceByDefinitionId.mockReturnValue(undefined);
    });
    ...
    describe('Constructor', () => {
        it('should throw if EntityManager is invalid or missing key methods', () => {
            ...
            expect(() => new WorldContext(incompleteEntityManager, mockLogger)).toThrow('getPrimaryInstanceByDefinitionId methods.');
            const anotherIncompleteEntityManager = {};
            expect(() => new WorldContext(anotherIncompleteEntityManager, mockLogger)).toThrow('WorldContext requires a valid EntityManager instance');
        });
    ...
    describe('getCurrentLocation', () => {
        ...
        it('should return the location entity when the current actor exists and has a valid position', () => {
            ...
            mockEntityManager.getEntityInstance.mockImplementation(id => id === LOCATION_1_ID ? location1Entity : undefined);
        ...
        it('should return null and log error if the current actor exists but position data lacks a valid locationId', () => {
            mockEntityManager.getComponentData.mockReturnValue({someOtherProp: 'value'}); // Position invalid
            const location = worldContext.getCurrentLocation();
            expect(location).toBeNull();
        ...
        it('should return null and log warning if the actor position points to a non-existent location entity', () => {
            ...
            const location = worldContext.getCurrentLocation();
            expect(location).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`Could not find location entity INSTANCE with ID 'nonexistent-location' referenced by actor '${PLAYER_ID}'`));
        });
    });
    ...
    describe('getLocationOfEntity', () => {
        it('should return the location entity for a given entityId with a valid position', () => {
            ...
            itemEntity.addComponent(POSITION_COMPONENT_ID, {locationId: LOCATION_2_ID});
        ...
        it('should return null and log warning if the entity for entityId is not found', () => {
            ...
            const location = worldContext.getLocationOfEntity(ITEM_ID);
            expect(location).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`Entity with ID '${ITEM_ID}' not found.`));
        });
        ...
        it('should return null if the entity does not have a POSITION component', () => {
            ...
            const location = worldContext.getLocationOfEntity(ITEM_ID);
            expect(location).toBeNull();
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining(`Entity '${ITEM_ID}' has no position component.`));
        });

        it('should return null and log warning if the entity position data lacks a valid locationId', () => {
            itemEntity.addComponent(POSITION_COMPONENT_ID, {someOtherProp: 'value'}); // Position lacks locationId
        ...
        it('should return null and log warning if the entity position points to a non-existent location entity', () => {
            itemEntity.addComponent(POSITION_COMPONENT_ID, {locationId: 'nonexistent-location'});
            mockEntityManager.getEntityInstance.mockImplementation(id => {
                if (id === ITEM_ID) return itemEntity;
            ...
            expect(location).toBeNull();
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining(`Could not find location entity INSTANCE with ID 'nonexistent-location' referenced by entity '${ITEM_ID}'`));
        });
    });
});
// --- FILE END ---