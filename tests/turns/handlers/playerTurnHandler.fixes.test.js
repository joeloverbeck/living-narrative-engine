...
describe('PlayerTurnHandler Constructor', () => {
    ...
    beforeEach(() => {
        ...
        setInitialStateSpy = jest.spyOn(BaseTurnHandler.prototype, '_setInitialState')
    ...
    it('should construct successfully with all valid dependencies', () => {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('PlayerTurnHandler initialised. Dependencies assigned. Initial state set.'));
        expect(mockTurnStateFactory.createInitialState).toHaveBeenCalledWith(handler);
    ...
    it('should throw an error if logger is not provided', () => {
        const deps = getValidDependencies();
        delete deps.logger;
        expect(() => new PlayerTurnHandler(deps)).toThrow('BaseTurnHandler: logger is required.');
    });

    it('should throw an error if turnStateFactory is not provided', () => {
        const deps = getValidDependencies();
        delete deps.turnStateFactory;
        expect(() => new PlayerTurnHandler(deps)).toThrow('BaseTurnHandler: turnStateFactory is required.');
    });

    it('should throw an error if commandProcessor is not provided', () => {
        const deps = getValidDependencies();
        delete deps.commandProcessor;
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: commandProcessor is required');
    });

    it('should throw an error if turnEndPort is not provided', () => {
        const deps = getValidDependencies();
        delete deps.turnEndPort;
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: turnEndPort is required');
    });

    it('should throw an error if playerPromptService is not provided', () => {
        const deps = getValidDependencies();
        delete deps.playerPromptService;
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: playerPromptService is required');
    });

    it('should throw an error if commandOutcomeInterpreter is not provided', () => {
        const deps = getValidDependencies();
        delete deps.commandOutcomeInterpreter;
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: commandOutcomeInterpreter is required');
    });

    it('should throw an error if safeEventDispatcher is not provided', () => {
        const deps = getValidDependencies();
        delete deps.safeEventDispatcher;
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: safeEventDispatcher is required');
    });
    ...
    it('should throw an error if subscriptionManager is not provided (undefined)', () => {
        ...
        expect(() => new PlayerTurnHandler(deps)).toThrow('PlayerTurnHandler: injected subscriptionManager is required');
    });
