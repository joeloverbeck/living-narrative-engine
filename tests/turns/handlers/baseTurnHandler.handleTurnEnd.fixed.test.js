...
describe('BaseTurnHandler._handleTurnEnd', () => {
    ...
    beforeEach(() => {
        ...
        handler = new TestTurnHandler({logger, turnStateFactory: mockTurnStateFactory});
    });
    ...
    it('returns quietly when called after the handler is destroyed', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(
            expect.stringContaining(`TestTurnHandler._handleTurnEnd ignored for actor actor1 – handler already destroyed.`)
        );
    ...
    it('still runs normally when handler is active and not in Idle/Ending state', async () => {
        ...
        expect(logger.warn).not.toHaveBeenCalledWith(
            expect.stringContaining('_handleTurnEnd ignored for actor actor1 – handler already destroyed.')
        );
        ...
        expect(mockTurnStateFactory.createEndingState).toHaveBeenCalledWith(handler, 'actor1', null);
        ...
        expect(logger.debug).toHaveBeenCalledWith(
            expect.stringContaining(`TestTurnHandler: State Transition: None (Initial) → TurnEndingState`)
        );
