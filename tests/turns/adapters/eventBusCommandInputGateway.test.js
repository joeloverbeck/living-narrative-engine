...
describe('EventBusCommandInputGateway', () => {
  ...
  beforeEach(() => {
    ...
    jest.resetAllMocks();
    gateway = new EventBusCommandInputGateway({
  ...
  it('should throw an error if VED dependency is missing or invalid', () => {
    expect(() => new EventBusCommandInputGateway({})).toThrow(
      /Invalid or missing validatedEventDispatcher/
    );
    expect(
      () => new EventBusCommandInputGateway({ validatedEventDispatcher: {} })
    ).toThrow(/Invalid or missing validatedEventDispatcher/);
    expect(
      () =>
        new EventBusCommandInputGateway({
  ...
  it('should throw an error if listener provided to onCommand is not a function', () => {
    expect(() => gateway.onCommand(null)).toThrow(
      /listener must be a function/
    );
    expect(() => gateway.onCommand('not a function')).toThrow(
      /listener must be a function/
    );
  ...
  it('should call the listener with the trimmed command when a valid event is received', () => {
    ...
    eventHandler({
      type: 'core:submit_command',
      payload: { command: ' another command ' },
    });
    expect(listenerCallback).toHaveBeenCalledTimes(1); // CORRECTED MATCHER
  ...
  it('should return an unsubscribe function that calls VED.unsubscribe with the correct listener', () => {
    ...
    expect(mockVed.unsubscribe).toHaveBeenCalledWith(
      'core:submit_command',
      subscribedHandler
    );
  });
