    ...
    describe('getAIDecision() Method', () => {
        ...
        const operationalConfigsStructure = {
            ...
            configs: {
                ...
                'test-llm-cloud-server-requires-key': createBaseModelConfig('test-llm-cloud-server-requires-key', {
                ...
                'test-llm-local-no-key-needed': createBaseModelConfig('test-llm-local-no-key-needed', {
        ...
        const getOperationalConfigs = () => JSON.parse(JSON.stringify(operationalConfigsStructure));
        ...
        beforeEach(async () => {
            ...
            mockLlmStrategyFactory.getStrategy.mockReset().mockReturnValue(mockLlmStrategy);
        ...
        describe('Initial State Checks', () => {
            ...
            it('should throw Error if called when not operational', async () => {
                ...
                mockLlmConfigLoader.loadConfigs.mockResolvedValue({
                    error: true, message: "Failed to load", stage: "test"
                });
            ...
            it('should throw ConfigurationError if no activeConfig is set', async () => {
                ...
                mockLlmConfigLoader.loadConfigs.mockResolvedValue(configsWithUnmatchableDefault);
                await adapterWithNoActiveLLM.init({llmConfigLoader: mockLlmConfigLoader});

                expect(adapterWithNoActiveLLM.isOperational()).toBe(true);
                expect(await adapterWithNoActiveLLM.getCurrentActiveLlmConfig()).toBeNull();

                mockLogger.error.mockClear();
                const expectedErrorMessage = "No active LLM configuration is set. Use setActiveLlm() or ensure a valid defaultConfigId is in the config file.";
                await expect(adapterWithNoActiveLLM.getAIDecision(gameSummary))
                    .rejects.toThrow(new ConfigurationError(expectedErrorMessage, {llmId: null}));
        ...
        describe('Active Configuration Validation (Ticket 21)', () => {
            const createInvalidTestSetup = async (invalidFieldOverride) => {
                ...
                const internalConfigId = invalidFieldOverride.hasOwnProperty('configId') ? invalidFieldOverride.configId : baseConfigId;

                const baseConfig = createBaseModelConfig(baseConfigId, {displayName: 'Config For Validation'});
                // Ensure internal configId is set by override or base correctly to what internalConfigId holds
                const invalidConfig = {...baseConfig, ...invalidFieldOverride, configId: internalConfigId};
                ...
                const tempConfigPayload = {
                    ...
                    configs: {
                        ...
                        'some-other-valid-config': createBaseModelConfig('some-other-valid-config')
                    }
                };
            ...
            it.each([
                ...
                const testAdapter = await createInvalidTestSetup(invalidFieldOverride);
                ...
                try {
                    ...
                    const actualConfigIdInError = invalidFieldOverride.hasOwnProperty('configId') ? invalidFieldOverride.configId : 'config-under-test';
                    const displayIdInErrorMessage = (actualConfigIdInError === null || String(actualConfigIdInError).trim() === '') ? 'unknown' : actualConfigIdInError;

                    expect(e.message).toContain(`Active LLM config '${displayIdInErrorMessage}' is invalid:`);
            ...
            it('should throw ConfigurationError if jsonOutputStrategy is not an object when provided', async () => {
                const testAdapter = await createInvalidTestSetup({jsonOutputStrategy: "not-an-object"});
                await expect(testAdapter.getAIDecision(gameSummary)).rejects.toThrow(ConfigurationError);
                try {
                    await testAdapter.getAIDecision(gameSummary);
                } catch (e) {
                    expect(e.message).toContain("jsonOutputStrategy: Is required and must be an object.");
                }
            });

            it('should throw ConfigurationError if jsonOutputStrategy.method is invalid when provided', async () => {
                const testAdapter = await createInvalidTestSetup({jsonOutputStrategy: {method: "  "}});
                await expect(testAdapter.getAIDecision(gameSummary)).rejects.toThrow(ConfigurationError);
                try {
                    await testAdapter.getAIDecision(gameSummary);
                } catch (e) {
                    expect(e.message).toContain("jsonOutputStrategy.method: Is required and must be a non-empty string.");
                }
            });
        });
        ...
        describe('API Key Retrieval', () => {
            it('should call IApiKeyProvider.getKey with correct activeConfig and environmentContext', async () => {
                ...
                const expectedActiveConfig = operationalConfigsStructure.configs['test-llm-operational'];
            ...
            it('should log info if API key is retrieved successfully', async () => {
                ...
                await adapter.getAIDecision(gameSummary);
                expect(mockLogger.info).toHaveBeenCalledWith(`API key retrieved for LLM '${llmId}'.`);
            });
        });
        ...
        describe('Strategy Factory Interaction', () => {
            ...
            it('should throw ConfigurationError if factory.getStrategy throws an error', async () => {
                ...
                await expect(adapter.getAIDecision(gameSummary))
                    .rejects.toThrow(new ConfigurationError(expectedWrappedErrorMessage, {
                        llmId: llmId,
            ...
            it('should throw ConfigurationError if factory.getStrategy returns null/undefined', async () => {
                ...
                await expect(adapter.getAIDecision(gameSummary))
                    .rejects.toThrow(new ConfigurationError(expectedErrorMessage, {llmId: llmId}));
            });
        });
        ...
        describe('Error Propagation', () => {
            ...
            beforeEach(async () => {
                await adapter.setActiveLlm(llmId);
                expect((await adapter.getCurrentActiveLlmConfig())?.configId).toBe(llmId);
                mockLogger.info.mockClear();
                mockLogger.error.mockClear();
            });
            ...
            it('should catch, log, and re-throw errors from IApiKeyProvider.getKey', async () => {
                ...
                await expect(adapter.getAIDecision(gameSummary)).rejects.toThrow(apiKeyError);
            ...
            it('should catch, log, and re-throw ConfigurationError when LLMStrategyFactory.getStrategy throws non-ConfigurationError', async () => {
                ...
                await expect(adapter.getAIDecision(gameSummary))
                    .rejects.toThrow(new ConfigurationError(expectedWrappedMessage, {
                        llmId,
            ...
            it('should catch, log, and re-throw errors from strategy.execute', async () => {
                ...
                await expect(adapter.getAIDecision(gameSummary)).rejects.toThrow(strategyExecuteError);
                expect(mockLogger.error).toHaveBeenCalledWith(
                    expect.stringContaining(`Error during getAIDecision for LLM '${llmId}': ${strategyExecuteError.message}`),
                    expect.objectContaining({llmId: llmId, errorName: strategyExecuteError.name})
                );
            });
            ...
            it('should re-throw ConfigurationError from validation steps directly, and log it once via main catch', async () => {
                ...
                const invalidConfig = createBaseModelConfig(configKey, {configId: null}); // configId property is null
                const tempConfigs = {
                ...
                await testAdapter.init({llmConfigLoader: mockLlmConfigLoader});
                expect(testAdapter.isOperational()).toBe(true);
                expect((await testAdapter.getCurrentActiveLlmConfig())?.configId).toBeNull();

                mockLogger.error.mockClear();

                const expectedValidationErrorMessage = `Active LLM config 'unknown' is invalid: configId: Missing or invalid`;
                await expect(testAdapter.getAIDecision(gameSummary))
                    .rejects.toThrow(new ConfigurationError(expectedValidationErrorMessage, {
                        llmId: null,
                        problematicFields: [{field: 'configId', reason: 'Missing or invalid'}]
                    }));
                ...
                expect(mockLogger.error).toHaveBeenCalledWith(
                    expect.stringContaining(`Error during getAIDecision for LLM '${configKey}': ${expectedValidationErrorMessage}`),
                    expect.objectContaining({llmId: configKey, errorName: 'ConfigurationError'})
                );
            });
        });
    });
