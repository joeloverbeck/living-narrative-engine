...
/**
 * @file Unit tests for TurnIdleState.
...
describe('TurnIdleState', () => {
  ...
  beforeEach(() => {
    ...
    originalAbstractHandleTurnEndedEvent =
      AbstractTurnState.prototype.handleTurnEndedEvent;
    originalAbstractHandleSubmittedCommand =
      AbstractTurnState.prototype.handleSubmittedCommand;
    originalAbstractProcessCommandResult =
      AbstractTurnState.prototype.processCommandResult;
    originalAbstractHandleDirective =
      AbstractTurnState.prototype.handleDirective;
  ...
  describe('enterState', () => {
    test('should call super.enterState and then handler._resetTurnStateAndResources', async () => {
      AbstractTurnState.prototype.enterState.mockImplementationOnce(
        async function () {
          /* Minimal mock */
        }
      );
      ...
      expect(AbstractTurnState.prototype.enterState).toHaveBeenCalledWith(
        mockHandler,
        null
      );
      expect(specificLogger.debug).toHaveBeenCalledWith(
        'TurnIdleState: Ensuring clean state by calling handler._resetTurnStateAndResources().'
      );
      expect(mockHandler._resetTurnStateAndResources).toHaveBeenCalledWith(
        'enterState-TurnIdleState'
      );
      expect(specificLogger.debug).toHaveBeenCalledWith(
        'TurnIdleState: Entry complete. Handler is now idle.'
      );
    });
  ...
  describe('exitState', () => {
    test('should call super.exitState', async () => {
      AbstractTurnState.prototype.exitState.mockImplementationOnce(
        async function () {
          /* Minimal mock */
        }
      );
      ...
      expect(AbstractTurnState.prototype.exitState).toHaveBeenCalledWith(
        mockHandler,
        mockNextState
      );
    });
  ...
  describe('startTurn', () => {
    ...
    beforeEach(() => {
      ...
      AbstractTurnState.prototype.enterState.mockImplementation(
        async function () {}
      );
      AbstractTurnState.prototype.exitState.mockImplementation(
        async function () {}
      );
    ...
    test('should throw error and re-transition to Idle if actorEntity is invalid', async () => {
      ...
      await expect(
        turnIdleState.startTurn(mockHandler, invalidActor)
      ).rejects.toThrow(
        'TurnIdleState: startTurn called with invalid actorEntity.'
      );
    ...
    test('should throw error and re-transition to Idle if ITurnContext is missing', async () => {
      ...
      await expect(
        turnIdleState.startTurn(mockHandler, testActor)
      ).rejects.toThrow(
        `TurnIdleState: ITurnContext is missing or invalid. Expected concrete handler to set it up. Actor: ${testActor.id}.`
      );
    ...
    test('should throw error and re-transition to Idle if ITurnContext actor mismatches', async () => {
      ...
      await expect(
        turnIdleState.startTurn(mockHandler, wrongActor)
      ).rejects.toThrow(
        `TurnIdleState: Actor in ITurnContext ('${testActor.id}') does not match actor provided to state's startTurn ('${wrongActor.id}').`
      );
  ...
  describe('Disallowed Operations in Idle State', () => {
    ...
    test('handleSubmittedCommand should log warning and call super (which throws)', async () => {
      ...
      await expect(
        turnIdleState.handleSubmittedCommand(
          mockHandler,
          commandString,
          testActor
        )
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `TurnIdleState: Command ('${commandString}') submitted by ${testActor.id} but no turn is active (handler is Idle).`
      );
    ...
    test('handleTurnEndedEvent should log warning and call super (which logs warn)', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `TurnIdleState: handleTurnEndedEvent called (for ${eventPayload.entityId}) but no turn is active (handler is Idle).`
      );
    ...
    test('processCommandResult should log warning and call super (which throws)', async () => {
      ...
      await expect(
        turnIdleState.processCommandResult(
          mockHandler,
          testActor,
          cmdProcResult,
          commandString
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `TurnIdleState: processCommandResult called (for ${testActor.id}) but no turn is active.`
      );
      // This assertion now checks the error log from AbstractTurnState
      expect(mockLogger.error).toHaveBeenCalledWith(expectedErrorMsg);
    });
    ...
    test('handleDirective should log warning and call super (which throws)', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `TurnIdleState: handleDirective called (for ${testActor.id}) but no turn is active.`
      );
      // This assertion now checks the error log from AbstractTurnState
      expect(mockLogger.error).toHaveBeenCalledWith(expectedErrorMsg);
    });
  ...
  describe('destroy', () => {
    test('should log information and call super.destroy', async () => {
      AbstractTurnState.prototype.destroy.mockImplementationOnce(
        async function () {
          /* Minimal mock for super call */
        }
      );
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'TurnIdleState: Destroy handling complete.'
      );
    });
  });
