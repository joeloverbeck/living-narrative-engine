...
/**
 * @file Unit tests for AwaitingExternalTurnEndState.
...
import { AbstractTurnState } from '../../../src/turns/states/abstractTurnState.js'; // For spying on super methods
...
describe('AwaitingExternalTurnEndState', () => {
  ...
  beforeEach(() => {
    ...
    awaitingExternalTurnEndState = new AwaitingExternalTurnEndState(
      mockHandler
    );
    ...
    superHandleSubmittedCommandSpy = jest.spyOn(
      AbstractTurnState.prototype,
  ...
  test('getStateName should return "AwaitingExternalTurnEndState"', () => {
    expect(awaitingExternalTurnEndState.getStateName()).toBe(
  ...
  describe('enterState', () => {
    test('should call super.enterState, set awaiting flag, and subscribe to TURN_ENDED_ID', async () => {
      ...
      expect(mockTestTurnContext.setAwaitingExternalEvent).toHaveBeenCalledWith(
        true,
        testActor.id
      );
      expect(mockTestTurnContext.getSubscriptionManager).toHaveBeenCalled();
      expect(
        mockSubscriptionManager.subscribeToTurnEnded
      ).toHaveBeenCalledTimes(1);
      // Check that the callback passed is indeed the state's handleTurnEndedEvent method
      const actualCallback =
        mockSubscriptionManager.subscribeToTurnEnded.mock.calls[0][0];
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Successfully subscribed – awaiting ${TURN_ENDED_ID} for actor ${testActor.id}.`
      );
    });
    ...
    test('should transition to Idle if ITurnContext is not available', async () => {
      ...
      expect(handlerLogger.error).toHaveBeenCalledWith(
        'AwaitingExternalTurnEndState: Critical - ITurnContext not available on entry. Transitioning to Idle.'
      );
      expect(mockHandler._resetTurnStateAndResources).toHaveBeenCalledWith(
        `critical-entry-no-context-${awaitingExternalTurnEndState.getStateName()}`
      );
      expect(mockHandler._transitionToState).toHaveBeenCalledWith(
        expect.any(TurnIdleState)
      );
      expect(
        mockTestTurnContext.setAwaitingExternalEvent
    ...
    test('should end turn if actor is not found in ITurnContext', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: No actor in ITurnContext. Ending turn.`
      );
      expect(mockTestTurnContext.endTurn).toHaveBeenCalledWith(
        expect.any(Error)
      );
      expect(mockTestTurnContext.endTurn.mock.calls[0][0].message).toBe(
        'AwaitingExternalTurnEndState: No actor in ITurnContext on entry.'
      );
      expect(
        mockTestTurnContext.setAwaitingExternalEvent
      ).not.toHaveBeenCalled();
    ...
    test('should end turn if turnCtx.setAwaitingExternalEvent throws', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Error during enterState setup for actor ${testActor.id}: ${flagError.message}`,
        flagError
      );
    ...
    test('should end turn and attempt to clear flag if subscription fails', async () => {
      ...
      mockTestTurnContext
        ...
        .subscribeToTurnEnded.mockImplementationOnce(() => {
          throw subError;
        });
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Error during enterState setup for actor ${testActor.id}: ${subError.message}`,
        subError
      );
    ...
    test('should log warning if subscribeToTurnEnded does not return a function', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: subscribeToTurnEnded did not return an unsubscribe function for ${testActor.id}. This is unexpected.`
      );
  ...
  describe('exitState', () => {
    ...
    test('should only unsubscribe if awaiting flag is not set (or cleared by event)', async () => {
      ...
      expect(
        mockTestTurnContext.setAwaitingExternalEvent
      ).not.toHaveBeenCalledWith(false, testActor.id);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'Awaiting external event flag was already false'
        )
      );
    });
    ...
    test('should handle error during unsubscription gracefully', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `Error unsubscribing from ${TURN_ENDED_ID} for actor ${testActor.id}: ${unSubError.message}`
        )
      );
      expect(mockTestTurnContext.setAwaitingExternalEvent).toHaveBeenCalledWith(
        false,
        testActor.id
      ); // Still tries to clear flag
    });
    ...
    test('should handle error clearing flag gracefully', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `Failed to clear awaiting external event flag for ${testActor.id} on exit: ${flagClearError.message}`
        )
      );
    });
    ...
    test('should proceed if context is null on exit, attempting unsubscription', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('ITurnContext not available on exit')
      );
      expect(
        mockTestTurnContext.setAwaitingExternalEvent
      ).not.toHaveBeenCalled(); // Cannot call on null context
    });
  });
  ...
  describe('handleTurnEndedEvent', () => {
    ...
    test('should end turn via ITurnContext if event is for current actor and isAwaitingExternalEvent is true', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Matched ${TURN_ENDED_ID} for actor ${testActor.id}. Ending turn via ITurnContext.`
      );
    ...
    test('should ignore event if it is not for the current actor', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: ${TURN_ENDED_ID} for ${eventPayloadForOtherActor.entityId} ignored; current context actor is ${testActor.id}.`
      );
    ...
    test('should ignore event and clean up subscription if isAwaitingExternalEvent is false', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `ITurnContext is no longer awaiting an external event`
        )
      );
      expect(mockTestTurnContext.endTurn).not.toHaveBeenCalled();
    ...
    test('should log warning and clean up if context is missing when event arrives', async () => {
      ...
      expect(handlerLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `AwaitingExternalTurnEndState: ${TURN_ENDED_ID} received, but no active ITurnContext.`
        )
      );
    ...
    test('should log warning and clean up if actor in context is missing when event arrives', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `but no actor in current ITurnContext. Cleaning up subscription and ignoring.`
        )
      );
      expect(mockUnsubscribeTurnEndedFn).toHaveBeenCalledTimes(1); // Cleanup
      expect(mockTestTurnContext.endTurn).not.toHaveBeenCalled();
  ...
  describe('handleSubmittedCommand', () => {
    test('should log an error and end the turn via ITurnContext', async () => {
      ...
      await awaitingExternalTurnEndState.handleSubmittedCommand(
        mockHandler,
        commandString,
        submitter
      );
    ...
    test('should use handler._handleTurnEnd if context is missing', async () => {
      ...
      expect(mockHandler._handleTurnEnd).toHaveBeenCalledWith(
        submitter.id,
        expect.any(Error)
      );
      expect(mockHandler._handleTurnEnd.mock.calls[0][1].message).toBe(
        expectedErrorMessage
      );
      expect(mockTestTurnContext.endTurn).not.toHaveBeenCalled();
    });
  ...
  describe('destroy', () => {
    test('should unsubscribe, clear flag, end turn, and call super.destroy if context and actor exist and awaiting', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Handler destroyed while awaiting external turn end for ${testActor.id}.`
      );
      expect(mockUnsubscribeTurnEndedFn).toHaveBeenCalledTimes(1);
      ...
      expect(mockTestTurnContext.endTurn.mock.calls[0][0].message).toBe(
        `Handler destroyed while ${testActor.id} was in AwaitingExternalTurnEndState.`
      );
    ...
    test('should handle context being null during destroy', async () => {
      ...
      expect(handlerLogger.warn).toHaveBeenCalledWith(
        `AwaitingExternalTurnEndState: Handler destroyed while awaiting external turn end for N/A_at_destroy.`
      );
      expect(mockUnsubscribeTurnEndedFn).toHaveBeenCalledTimes(1); // Attempt unsubscribe
      expect(handlerLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'ITurnContext not available for actor N/A_at_destroy'
        )
      );
      expect(
        mockTestTurnContext.setAwaitingExternalEvent
    ...
    test('should handle flag clear error during destroy', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `Failed to clear awaiting flag for ${testActor.id}: ${flagError.message}`
        )
      );
  ...
  test('No direct PlayerTurnHandler dependencies in source code', () => {
    ...
    const privateFlagRegex =
      /\.(_isAwaitingTurnEndEvent|_awaitingTurnEndForActorId)\b|\[\s*['"](_isAwaitingTurnEndEvent|_awaitingTurnEndForActorId)['"]\s*\]/;
