...
class MockTurnContext {
    ...
    resetAllMocks() {
        ...
        this.requestTransition.mockClear().mockImplementation(() => Promise.resolve()); // Reset impl to original
...
describe('RepromptStrategy', () => {
    ...
    test('should call turnContext.endTurn with an error if turnContext.getActor() returns null', async () => {
        ...
        expect(errorArg.message).toBe('RepromptStrategy: No actor found in ITurnContext. Cannot re-prompt.');
    });
    ...
    test('should call turnContext.endTurn with an error if turnContext.requestTransition fails', async () => {
        ...
        mockTurnContext.requestTransition.mockRejectedValue(new Error(transitionErrorMessage));
