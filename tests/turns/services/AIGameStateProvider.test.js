...
class MockEntity {
  constructor(
    ...
    this.hasComponent = jest.fn((compId) =>
      this._presentComponents.has(compId)
    );
  }
  ...
  describe('buildGameState', () => {
    describe('Input Validation', () => {
      ...
      test('should throw error and log if actor has no ID', async () => {
        ...
        const expectedErrorMsg =
          'AIGameStateProvider: Actor is invalid or missing ID. Cannot build game state.';
        await expect(
          provider.buildGameState(actorWithoutId, turnContext, logger)
        ).rejects.toThrow(expectedErrorMsg);
        expect(logger.error).toHaveBeenCalledWith(expectedErrorMsg, {
    ...
    describe('_getComponentText (implicitly and explicitly)', () => {
      test('should populate AIActorStateDTO with name and description if components exist', async () => {
        ...
        actor.setComponentData(DESCRIPTION_COMPONENT_ID, {
          text: 'A brave test actor.',
        });
      ...
      test('should use default name/description if component data exists but propertyPath is missing', async () => {
        ...
        actor.setComponentData(NAME_COMPONENT_ID, {
          wrongProperty: 'Some Name',
        });
        actor.setComponentData(DESCRIPTION_COMPONENT_ID, {
          otherProp: 'Some Desc',
        });
    ...
    describe('_buildLocationSummary (via buildGameState)', () => {
      ...
      beforeEach(() => {
        mockActor.setComponentData(POSITION_COMPONENT_ID, {
          locationId: 'loc1',
        });

        locationEntity = new MockEntity('loc1');
        locationEntity.setComponentData(NAME_COMPONENT_ID, {
          text: 'The Grand Hall',
        });
        locationEntity.setComponentData(DESCRIPTION_COMPONENT_ID, {
          text: 'A vast and echoing chamber.',
        });
        ...
        otherChar1.setComponentData(DESCRIPTION_COMPONENT_ID, {
          text: 'A sturdy guard.',
        });
        ...
        entityManager.getEntityInstance.mockImplementation(async (id) => {
          ...
          if (id === 'loc2')
            return new MockEntity('loc2', {
              [NAME_COMPONENT_ID]: { text: 'North Room' },
            });
          if (id === 'loc3')
            return new MockEntity('loc3', {
              [NAME_COMPONENT_ID]: { text: 'East Room' },
            });
          return null;
        });
        entityManager.getEntitiesInLocation.mockResolvedValue(
          new Set(['actor1', 'char1'])
        );
      });
      ...
      test('Full Data: should populate AILocationSummaryDTO with all details', async () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith(
          `AIGameStateProvider: Building location summary for actor ${mockActor.id}`
        );
        ...
        expect(logger.debug).toHaveBeenCalledWith(
          `AIGameStateProvider: Final generated location summary for actor ${mockActor.id} in location loc1. Exits: 2, Characters: 1`
        );
      });
      ...
      test('EntityManager Unavailable: turnContext.getEntityManager returns null', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(
          `AIGameStateProvider: EntityManager not available for actor ${mockActor.id}. Cannot fetch location details.`
        );
      });
      ...
      test('EntityManager Unavailable: turnContext.getEntityManager is not a function', async () => {
        ...
        expect(gameState.currentLocation).toBeNull();
        expect(logger.warn).toHaveBeenCalledWith(
          `AIGameStateProvider: turnContext.getEntityManager is not a function for actor ${mockActor.id}. EntityManager not available.`
        );
      });
      ...
      test('EntityManager Unavailable: turnContext.getEntityManager throws', async () => {
        ...
        expect(gameState.currentLocation).toBeNull();
        expect(logger.warn).toHaveBeenCalledWith(
          `AIGameStateProvider: Error accessing EntityManager for actor ${mockActor.id}: ${error.message}`
        );
      });
      ...
      test('Location Entity Not Found: entityManager.getEntityInstance(locationId) returns null', async () => {
        ...
        expect(gameState.currentLocation).toBeNull();
        expect(logger.warn).toHaveBeenCalledWith(
          `AIGameStateProvider: Location entity for ID 'loc1' not found for actor ${mockActor.id}.`
        );
      });
      ...
      describe('Exits Handling', () => {
        ...
        test('Malformed exit entries are filtered out (or handled with defaults)', async () => {
          ...
          const baseGetEntityInstanceImpl =
            entityManager.getEntityInstance.getMockImplementation();
      ...
      describe('Characters Handling', () => {
        ...
        test('entityManager.getEntityInstance for other char returns null', async () => {
          entityManager.getEntitiesInLocation.mockResolvedValue(
            new Set(['actor1', 'charNonExistent'])
          );
          ...
          expect(gameState.currentLocation.characters).toEqual([]);
          expect(logger.warn).toHaveBeenCalledWith(
            "AIGameStateProvider: Could not retrieve entity instance for ID 'charNonExistent' in location 'loc1'. This entity will NOT be listed."
          );
        });
      });
    ...
    describe('_getAvailableActions (via buildGameState)', () => {
      ...
      test('should populate actions with defaults for missing optional fields', async () => {
        ...
        expect(availableActions).toEqual([
          ...
          },
          {
            ...
            description: DEFAULT_ACTION_DESC,
          },
        ]);
      });
      ...
      test('should call ADS.getValidActions with correctly structured actionCtx', async () => {
        ...
        expect(actionDiscoverySystem.getValidActions).toHaveBeenCalledWith(
          mockActor,
          expect.objectContaining(expectedActionCtx)
        );
      });
    });
    ...
    describe('_getPerceptionLog (via buildGameState)', () => {
      beforeEach(() => {
        ...
        entityManager.getEntityInstance.mockImplementation(async (id) =>
          id === 'loc1'
            ? loc
            : baseGetEntityInstanceImpl
              ? await baseGetEntityInstanceImpl(id)
              : null
        );
        entityManager.getEntitiesInLocation.mockResolvedValue(new Set());
        actionDiscoverySystem.getValidActions.mockResolvedValue([]);
      ...
      test('should return empty perceptionLog if actor has no PERCEPTION_LOG_COMPONENT_ID', async () => {
        ...
        mockActor.hasComponent = jest.fn(
          (id) =>
            id !== PERCEPTION_LOG_COMPONENT_ID &&
            originalHasComponent.call(mockActor, id)
        );
    ...
    describe('Overall buildGameState Orchestration', () => {
      ...
      beforeEach(() => {
        ...
        mockLocationSummaryDTO = {
          ...
          exits: [
            {
              direction: 'mock',
              targetLocationId: 'mockT',
              targetLocationName: 'Mock Target',
            },
          ],
          characters: [],
        };
        ...
        jest
          .spyOn(provider, '_buildActorState')
          .mockReturnValue(mockActorStateDTO);
        jest
          .spyOn(provider, '_buildLocationSummary')
          .mockResolvedValue(mockLocationSummaryDTO);
        jest
          .spyOn(provider, '_getAvailableActions')
          .mockResolvedValue(mockAvailableActionsDTO);
        jest
          .spyOn(provider, '_getPerceptionLog')
          .mockResolvedValue(mockPerceptionLogDTO);
      });
      ...
      test('should correctly orchestrate calls and build a full AIGameStateDTO', async () => {
        ...
        expect(provider._buildActorState).toHaveBeenCalledWith(
          mockActor,
          logger
        );
        expect(provider._buildLocationSummary).toHaveBeenCalledWith(
          mockActor,
          turnContext,
          logger
        );
        expect(provider._getAvailableActions).toHaveBeenCalledWith(
          mockActor,
          turnContext,
          mockLocationSummaryDTO,
          logger
        );
        expect(provider._getPerceptionLog).toHaveBeenCalledWith(
          mockActor,
          logger
        );
      ...
      test('should populate other DTO parts if _buildLocationSummary returns null', async () => {
        ...
        expect(provider._getAvailableActions).toHaveBeenCalledWith(
          ...
          null,
          logger
        ); // Ensure it's called with null location
        expect(gameState).toEqual({
          actorState: mockActorStateDTO,
          actorPromptData: expectedActorPromptData,
