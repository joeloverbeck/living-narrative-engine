...
describe('TurnManager', () => {
  ...
  beforeEach(() => {
    ...
    mockTurnOrderService = {
      ...
      startNewRound: jest.fn().mockImplementation(async () => {
        ...
        mockTurnOrderService.getNextEntity.mockReturnValueOnce(
    ...
    mockActor1.hasComponent.mockImplementation(
  ...
  describe('Event Handling for core:turn_ended', () => {
    ...
    it('should not process core:turn_ended by advancing turn if manager is not running', async () => {
      ...
      const getNextEntityCallCountBeforeEvent =
        mockTurnOrderService.getNextEntity.mock.calls.length; // Should be 0
      ...
      expect(mockTurnOrderService.getNextEntity.mock.calls.length).toBe(
  ...
  describe('Constructor and Start/Stop', () => {
    ...
    it('stop should unsubscribe from TURN_ENDED_ID', async () => {
      ...
      const subscribeCall = mockDispatcher.subscribe.mock.calls.find(
        (call) => call[0] === TURN_ENDED_ID
      );
      expect(subscribeCall).toBeDefined();
      const unsubscribeFn =
        mockDispatcher.subscribe.mock.results[
          mockDispatcher.subscribe.mock.calls.indexOf(subscribeCall)
        ].value;
