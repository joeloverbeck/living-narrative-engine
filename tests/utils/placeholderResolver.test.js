...
describe('PlaceholderResolver', () => {
    ...
    describe('resolve', () => {
        ...
        it('should resolve placeholders from the first matching data source if multiple are provided', () => {
            ...
            expect(resolver.resolve(str, data1, data2, data3)).toBe("Name: Data1Name, City: Data2City.");
        });
        ...
        it('should skip non-object data sources gracefully', () => {
            ...
            // @ts-ignore
            expect(resolver.resolve(str, null, data1, undefined, "string_source", 123)).toBe("Value: actualValue");
        ...
        it('should replace placeholder with an empty string and log a warning if key is not found in any data source', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith('PlaceholderResolver: Placeholder "{city}" not found in provided data sources. Replacing with empty string.');
        });
        ...
        it('should replace with empty string for multiple missing keys and log warnings for each', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith('PlaceholderResolver: Placeholder "{val1}" not found in provided data sources. Replacing with empty string.');
            expect(mockLogger.warn).toHaveBeenCalledWith('PlaceholderResolver: Placeholder "{val3}" not found in provided data sources. Replacing with empty string.');
        });
        ...
        it('should convert boolean values to strings', () => {
            ...
            expect(resolver.resolve(str, data)).toBe("Is Active: true, Is Admin: false");
        });
        ...
        it('should handle complex strings with multiple types of placeholders and missing keys', () => {
            const str = "User: {user}, ID: {id}, Status: {status}, Role: {role}, Zip: {zip}";
            const data1 = {user: "johndoe", id: 123};
            const data2 = {status: true, role: null}; // zip is missing
            expect(resolver.resolve(str, data1, data2)).toBe("User: johndoe, ID: 123, Status: true, Role: , Zip: ");
            expect(mockLogger.warn).toHaveBeenCalledTimes(1);
            expect(mockLogger.warn).toHaveBeenCalledWith('PlaceholderResolver: Placeholder "{zip}" not found in provided data sources. Replacing with empty string.');
        });
        ...
        it('should not confuse object prototype properties with data source keys', () => {
            ...
            expect(resolver.resolve(str, data)).toBe("Value: ");
            expect(mockLogger.warn).toHaveBeenCalledWith('PlaceholderResolver: Placeholder "{toString}" not found in provided data sources. Replacing with empty string.');
        });
        ...
        it('should handle placeholders that look like regex special characters but are valid keys', () => {
            ...
            const data = {"key.with.dots": "dots_ok", "key-with-dashes": "dashes_ok"};
