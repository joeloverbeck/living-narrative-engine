...
describe('SafeEventDispatcher', () => {
    it('should throw an error if logger dependency is missing or invalid', () => {
        expect(() => new SafeEventDispatcher({validatedEventDispatcher: mockVed, logger: null}))
            .toThrow('SafeEventDispatcher: Invalid or missing logger dependency (requires error, debug, info methods).');
    ...
    it('AC1: should return true and not log error when VED.dispatchValidated resolves true', async () => {
        ...
        expect(mockVed.dispatchValidated).toHaveBeenCalledWith(testEventName, testPayload, expectedDefaultOptions);
        expect(mockLogger.error).not.toHaveBeenCalled();
        expect(mockLogger.debug).toHaveBeenCalledWith(`SafeEventDispatcher: Successfully dispatched event '${testEventName}'.`);
    });
    ...
    it('AC2: should return false and log error when VED.dispatchValidated resolves false', async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith( // Changed from error to warn
    ...
    it('AC3: should return false and log error when VED.dispatchValidated throws an exception', async () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(
            ...
            {payload: testPayload, error: testError, options: expectedDefaultOptions}
        );
    });
});
