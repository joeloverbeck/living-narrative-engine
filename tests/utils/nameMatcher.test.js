...
describe('NameMatcher Utility - matchNames', () => {
  ...
  describe('Exact Matches', () => {
    test('should find a unique exact match (case insensitive)', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Unique exact match found: ID "3", Name "Carrot Cake".'
      );
    });
    ...
    test('should find multiple exact matches (ambiguous)', () => {
      ...
      expect(result.error).toBe(
        'Which "apple pie" did you mean? For example: "Apple Pie", "apple pie".'
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Ambiguous exact matches found for "apple pie". Count: 2.'
      );
    });
  });
  ...
  describe('StartsWith Matches', () => {
    test('should find a unique startsWith match (no exact match present)', () => {
      ...
      expect(result.error).toBeUndefined();
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Unique startsWith match found: ID "2", Name "Banana Bread".'
      );
    });
    ...
    test('should find multiple startsWith matches (ambiguous, no exact match present)', () => {
      ...
      expect(result.error).toBe(
        'Which item starting with "Apple" did you mean? For example: "Apple Pie", "Apple Tart", "apple pie".'
      );
      // Corrected debug log count expectation
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Ambiguous startsWith matches found for "apple". Count: 3.'
      );
    });
    ...
    test('should find startsWith match when exact matches for *other* items also exist', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Unique startsWith match found: ID "6", Name "Blueberry Muffin".'
      );
    });
    ...
    test('should return ambiguous startsWith if multiple startsWith and no exact for phrase', () => {
      ...
      expect(result.error).toContain(
        'Which item starting with "Start" did you mean?'
      );
    });
  });
  ...
  describe('Substring Matches', () => {
    test('should find a unique substring match (no exact or startsWith match present)', () => {
      ...
      expect(result.error).toBeUndefined();
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Unique substring match found: ID "7", Name "Dragon Fruit Salad".'
      );
    });
    ...
    test('should find multiple substring matches (ambiguous, no exact or startsWith present)', () => {
      ...
      expect(resultMulti.candidates).toEqual(
        expect.arrayContaining([
          { id: 'f1', name: 'Sunflower Seed' },
          { id: 'f2', name: 'Wildflower Bouquet' },
          { id: 'f3', name: 'Lotus Flower' },
      ...
      expect(resultMulti.error).toBe(
        'Which item containing "flower" did you mean? For example: "Sunflower Seed", "Wildflower Bouquet", "Lotus Flower".'
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Ambiguous substring matches found for "flower". Count: 3.'
      );
    });
    ...
    test('should handle multiple ambiguous substring matches with truncation in error message', () => {
      ...
      expect(result.error).toBe(
        'Which item containing "Sub" did you mean? For example: "Item One Sub", "Item Two Sub", "Item Three Sub" or others....'
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Ambiguous substring matches found for "sub". Count: 4.'
      );
    });
  });
  ...
  describe('No Matches', () => {
    test('should return NOT_FOUND when phrase matches no candidates', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: No matches found for phrase "zucchini".'
      );
    });
  });
  ...
  describe('Input Variations', () => {
    test('should return NONE for an empty phrase', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Invalid or empty phrase provided.'
      );
    });
    ...
    test('should return NOT_FOUND for an empty candidates array', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'NameMatcher.matchNames: No candidates provided to match against phrase "Apple Pie".'
      );
    });
    ...
    test('should skip invalid candidates (missing name or id)', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: {"id":"invalid1"}'
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: {"name":"Invalid Item 2"}'
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: null'
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: {"id":"invalid3","name":null}'
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: {"id":"invalid4","name":"   "}'
      );

      const result2 = matchNames(
        invalidCandidates,
        'Another Valid',
        mockLogger
      );
    ...
    test('should handle candidates with empty string names correctly by skipping them', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'NameMatcher.matchNames: Skipping invalid candidate: {"id":"item1","name":""}'
      );
    });
    ...
    test('matching should trim whitespace for phrase and candidate names', () => {
      ...
      expect(result.target).toEqual({
        id: 'trim1',
        name: '  Spaced Out Item  ',
      }); // Original name returned
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Phrase: "spaced out item"')
      );
  ...
  describe('Logging Behavior', () => {
    test('should log initial call with phrase and candidate count', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        `NameMatcher.matchNames called with phrase: "Test Phrase", ${candidates.length} candidates.`
      );
    });
    ...
    test('should log initial call with 0 candidates if candidates array is null or empty', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        `NameMatcher.matchNames called with phrase: "Test Phrase", 0 candidates.`
      );
      mockLogger.debug.mockClear();
    ...
    test('should log match counts after processing candidates', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames - Phrase: "apple" - Exact: 0, StartsWith: 3, Substring: 0'
        )
      );
    });
    ...
    test('should log match counts accurately for exact matches', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames - Phrase: "carrot cake" - Exact: 1, StartsWith: 0, Substring: 0'
        )
      );
    });
    ...
    test('should log match counts accurately for substring matches', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          'NameMatcher.matchNames - Phrase: "fruit salad" - Exact: 0, StartsWith: 0, Substring: 1'
        )
      );
    });
