...
import { resolveEntityNameFallback, resolvePlaceholders } from '../../src/utils/contextUtils.js';
...
describe('contextUtils.js', () => {
  ...
  describe('resolveEntityNameFallback', () => {
    test('should return undefined if resolutionRoot is null or undefined', () => {
      expect(resolveEntityNameFallback('actor.name', null, mockLogger)).toBeUndefined();
      expect(resolveEntityNameFallback('actor.name', undefined, mockLogger)).toBeUndefined();
    });

    test('should return undefined for a placeholder path that is not a recognized shorthand', () => {
      const resolutionRoot = { actor: {} };
      expect(
        resolveEntityNameFallback('inventory.item.name', resolutionRoot, mockLogger)
      ).toBeUndefined();
    });
    ...
    test("should return undefined if 'actor.name' is requested but actor is not in the resolution root", () => {
      ...
      expect(resolveEntityNameFallback('actor.name', resolutionRoot, mockLogger)).toBeUndefined();
    });

    test("should return undefined if 'target.name' is requested but target is not in the resolution root", () => {
      const resolutionRoot = { actor: {} };
      expect(resolveEntityNameFallback('target.name', resolutionRoot, mockLogger)).toBeUndefined();
    });
    ...
    test("should correctly call getEntityDisplayName for 'actor.name'", () => {
      ...
      const result = resolveEntityNameFallback('actor.name', resolutionRoot, mockLogger);

      expect(result).toBe('Hero');
      expect(mockGetEntityDisplayName).toHaveBeenCalledWith(mockActor, undefined, mockLogger);
    });
    ...
    test("should correctly call getEntityDisplayName for 'target.name'", () => {
      ...
      const result = resolveEntityNameFallback('target.name', resolutionRoot, mockLogger);

      expect(result).toBe('Villain');
      expect(mockGetEntityDisplayName).toHaveBeenCalledWith(mockTarget, undefined, mockLogger);
    });
