...
import {parseAndRepairJson, JsonProcessingError, cleanLLMJsonOutput} from '../../src/utils/llmUtils.js'; // Adjust path as needed
...
describe('parseAndRepairJson', () => {
    ...
    describe('Invalid Input Handling', () => {
        test('should throw TypeError for null input', async () => {
            await expect(parseAndRepairJson(null, mockLogger))
                .rejects
                .toThrow(TypeError("Input 'jsonString' must be a string."));
            // Corrected assertion: expect a single argument matching the actual log
            expect(mockLogger.error).toHaveBeenCalledWith("parseAndRepairJson: Input 'jsonString' must be a string. Received type: object");
        });

        test('should throw TypeError for undefined input', async () => {
            await expect(parseAndRepairJson(undefined, mockLogger))
                .rejects
                .toThrow(TypeError("Input 'jsonString' must be a string."));
            // Corrected assertion: expect a single argument matching the actual log
            expect(mockLogger.error).toHaveBeenCalledWith("parseAndRepairJson: Input 'jsonString' must be a string. Received type: undefined");
        });

        test('should throw JsonProcessingError for empty string input after cleaning', async () => {
            await expect(parseAndRepairJson('', mockLogger))
                .rejects
                .toThrow(new JsonProcessingError("Cleaned JSON string is null or empty, cannot parse.", {
                    stage: 'initial_clean',
                    attemptedJsonString: ''
                }));
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Cleaned JSON string is null or empty"), {originalInput: ''});
        });
    ...
    describe('Valid JSON Input', () => {
        test('should parse a perfectly valid JSON string without repair', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Successfully parsed JSON on first attempt"), expect.any(Object));
    ...
    describe('Repairable JSON Input', () => {
        test('should repair and parse JSON with trailing commas', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Initial JSON.parse failed"), expect.any(Object));
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Successfully parsed JSON after repair"), expect.any(Object));
        });
        ...
        test('should repair and parse JSON with missing quotes on keys', async () => {
            ...
            const cleanedRepairableString = cleanLLMJsonOutput(repairableJsonString);
            const repairedJsonString = '{"key": "value", "anotherKey": "anotherValue"}';
    ...
    describe('Unrepairable JSON Input', () => {
        test('should throw JsonProcessingError if repair library returns still-invalid JSON', async () => {
            ...
            const cleanedUnrepairableString = cleanLLMJsonOutput(unrepairableJsonString);
            ...
            await expect(parseAndRepairJson(unrepairableJsonString, mockLogger))
                .rejects
                .toThrow(JsonProcessingError);

            expect(mockRepairJson).toHaveBeenCalledWith(cleanedUnrepairableString);
            expect(mockLogger.warn).toHaveBeenCalled();
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Failed to parse JSON even after repair attempt"), expect.any(Object));
        });
        ...
        test('should throw JsonProcessingError if repair library itself throws an error', async () => {
            ...
            const cleanedProblematicString = cleanLLMJsonOutput(problematicJsonString);
            ...
            await expect(parseAndRepairJson(problematicJsonString, mockLogger))
                .rejects
                .toThrow(JsonProcessingError);
    ...
    describe('Logger Interactions', () => {
        ...
        test('should call logger.warn when initial parse fails', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Initial JSON.parse failed'), expect.anything());
        });
        ...
        test('should call logger.info when repair and subsequent parse succeed', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Successfully parsed JSON after repair'), expect.anything());
        });
        ...
        test('should call logger.error when repair attempt fails to produce valid JSON', async () => {
            ...
            await expect(parseAndRepairJson(unrepairableJson, mockLogger)).rejects.toThrow(JsonProcessingError);
            expect(mockLogger.error).toHaveBeenCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to parse JSON even after repair attempt'), expect.anything());
        });
    });
    ...
    describe('Interaction with cleanLLMJsonOutput', () => {
        test('should process a string that requires cleaning before parsing', async () => {
            ...
            expect(result).toEqual(expectedObject);
            expect(mockRepairJson).not.toHaveBeenCalled();
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Successfully parsed JSON on first attempt"), expect.objectContaining({
