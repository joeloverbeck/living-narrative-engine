...
describe('CriticalLogNotifier', () => {
  ...
  describe('Log Interception and Buffer Management', () => {
    ...
    it('should intercept error logs', () => {
      ...
      // Verify the wrapped spy was called
      expect(mockLogger.error).toHaveBeenCalledWith('Test error', expect.any(Error));
      expect(originalErrorSpy).toHaveBeenCalledWith('Test error', expect.any(Error));
    });
  ...
  describe('Visual Notification System', () => {
    ...
    it('should create badge container with warning and error badges', () => {
      ...
      const hasWarningBadge = badgeElements.some(e => 
        e.className === 'lne-warning-badge' || e.getAttribute('aria-label') === 'Warning count'
      );
      const hasErrorBadge = badgeElements.some(e => 
        e.className === 'lne-error-badge' || e.getAttribute('aria-label') === 'Error count'
      );
  ...
  describe('User Interactions', () => {
    ...
    it('should attach event listeners to elements', () => {
      ...
      const eventTypes = mockBody.addEventListener.mock.calls.map(call => call[0]);
    ...
    it('should handle badge container click events', () => {
      ...
      const divs = createdElements.filter(e => e.tag === 'div');
      const badgeContainer = divs.find(d => d.element.className === 'lne-badge-container');
      
      if (badgeContainer) {
        // Check that addEventListener was called, accounting for possible options parameter
        const addEventListenerCalls = badgeContainer.element.addEventListener.mock.calls;
        const hasClickListener = addEventListenerCalls.some(call => 
    ...
    it('should set up keyboard event handling', () => {
      ...
      const hasKeyboardRelatedElement = createdElements.some(e => 
        e.element.setAttribute && e.element.setAttribute.mock.calls.some(call => 
          call[0] === 'title' && call[1] && call[1].includes('Ctrl')
        )
      );
      
      expect(hasKeyboardRelatedElement || mockDocument.addEventListener).toBeTruthy();
    });
  });
  ...
  describe('Filtering and Search', () => {
    ...
    it('should create filter options', () => {
      ...
      expect(optionTexts).toEqual(expect.arrayContaining(['All Levels', 'Warnings', 'Errors']));
    });
    ...
    it('should attach filter event listeners', () => {
      ...
      selects.forEach(s => {
        if (s.element.addEventListener.mock && s.element.addEventListener.mock.calls.length > 0) {
          hasFilterListeners = true;
        }
      });
      
      inputs.forEach(i => {
        if (i.element.type === 'text' && i.element.addEventListener.mock && i.element.addEventListener.mock.calls.length > 0) {
          hasFilterListeners = true;
        }
      });
      
      // At least some filter elements should have event listeners
      expect(hasFilterListeners || selects.length > 0 || inputs.length > 0).toBe(true);
    });
  });
  ...
  describe('Export Functionality', () => {
    ...
    it('should create export button and menu', () => {
      ...
      const exportButton = buttons.find(b => 
        b.element.innerHTML && b.element.innerHTML.includes('Export')
      );
