...
describe('LogExporter - Core Functionality', () => {
  ...
  describe('CSV Export', () => {
    it('should export logs as CSV with headers', () => {
      ...
      const headerLine = lines.find(line => line.startsWith('Timestamp,ISO Time'));
    ...
    it('should include metadata as comments', () => {
      ...
      expect(metadataLines.some(line => line.includes('Test App'))).toBe(true);
    });
  ...
  describe('Filename Generation', () => {
    it('should generate timestamped filenames', () => {
      ...
      expect(filename).toMatch(/^test_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.json$/);
    });

    it('should use default values', () => {
      const filename = logExporter.generateFilename();
      
      expect(filename).toMatch(/^critical-logs_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.json$/);
    });
  });
