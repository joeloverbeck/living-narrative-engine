  ...
  describe('Module-level functions', () => {
    ...
    describe('Event definitions', () => {
      it('should define correct event IDs', async () => {
        ...
        const directionUpdated = eventDefs.find(
          (e) => e.id === 'core:direction_updated'
        );
        expect(directionUpdated).toBeDefined();
        expect(directionUpdated.description).toBe(
          'Fired when a thematic direction is updated.'
        );
        expect(directionUpdated.payloadSchema.required).toEqual([
          'directionId',
          'field',
          'oldValue',
          'newValue',
        ...
        const directionDeleted = eventDefs.find(
          (e) => e.id === 'core:direction_deleted'
        );
        expect(directionDeleted).toBeDefined();
        expect(directionDeleted.description).toBe(
          'Fired when a thematic direction is deleted.'
        );
        expect(directionDeleted.payloadSchema.required).toEqual([
          'directionId',
        ]);

        // Verify ORPHANS_CLEANED event
        const orphansCleaned = eventDefs.find(
          (e) => e.id === 'core:orphans_cleaned'
        );
        expect(orphansCleaned).toBeDefined();
        expect(orphansCleaned.description).toBe(
          'Fired when orphaned directions are cleaned up.'
        );
