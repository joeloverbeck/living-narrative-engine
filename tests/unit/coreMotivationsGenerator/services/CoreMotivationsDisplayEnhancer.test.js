...
describe('CoreMotivationsDisplayEnhancer', () => {
  ...
  beforeEach(() => {
    ...
    displayEnhancer = new CoreMotivationsDisplayEnhancer({ logger: mockLogger });
    ...
    mockMotivation = new CoreMotivation({
      ...
      internalContradiction: 'Desires fame but fears the loss of privacy and authentic relationships',
      centralQuestion: 'Is lasting legacy worth sacrificing personal connections?',
  ...
  describe('constructor', () => {
    ...
    it('should throw error when logger lacks required methods', () => {
      ...
      expect(() => new CoreMotivationsDisplayEnhancer({ logger: invalidLogger })).toThrow();
    });
  });
  ...
  describe('createMotivationBlock', () => {
    it('should create a complete motivation block with correct structure', () => {
      ...
      expect(element.getAttribute('data-motivation-id')).toBe('test-motivation-123');
    });
    ...
    it('should include header with timestamp and actions', () => {
      ...
      expect(deleteBtn.getAttribute('data-motivation-id')).toBe('test-motivation-123');
    });
    ...
    it('should include all three content sections with correct classes', () => {
      ...
      const coreSection = content.querySelector('.motivation-section.core-motivation');
      ...
      expect(coreText.textContent).toBe('To achieve greatness and be remembered for generations');

      // Check contradiction section
      const contradictionSection = content.querySelector('.motivation-section.contradiction');
      ...
      expect(contradictionText.textContent).toBe('Desires fame but fears the loss of privacy and authentic relationships');

      // Check central question section
      const questionSection = content.querySelector('.motivation-section.central-question');
      ...
      expect(questionText.textContent).toBe('Is lasting legacy worth sacrificing personal connections?');
    });

    it('should throw error for invalid motivation object', () => {
      expect(() => displayEnhancer.createMotivationBlock(null)).toThrow('Motivation object is required');
      expect(() => displayEnhancer.createMotivationBlock(undefined)).toThrow('Motivation object is required');
      expect(() => displayEnhancer.createMotivationBlock('invalid')).toThrow('Motivation object is required');
      
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Invalid motivation provided to createMotivationBlock'));
    });
    ...
    it('should throw error for missing required fields', () => {
      ...
      expect(() => displayEnhancer.createMotivationBlock(invalidMotivation)).toThrow('Motivation missing required field: internalContradiction');
    });
    ...
    it('should throw error for non-string ID', () => {
      ...
      expect(() => displayEnhancer.createMotivationBlock(invalidMotivation)).toThrow('Motivation ID must be a string');
    });
    ...
    it('should sanitize HTML in text content', () => {
      const maliciousMotivation = new CoreMotivation({
        ...
        internalContradiction: 'Test <img src=x onerror=alert(1)> contradiction',
        centralQuestion: 'Test <b>bold</b> question?',
        createdAt: '2024-12-20T12:00:00.000Z',
      });

      const element = displayEnhancer.createMotivationBlock(maliciousMotivation);
      ...
      const contradictionText = element.querySelector('.contradiction p').textContent;
      ...
      const questionText = element.querySelector('.central-question p').textContent;
    ...
    it('should add ARIA labels to buttons', () => {
      ...
      expect(copyBtn.getAttribute('aria-label')).toBe('Copy motivation test-motivation-123');
      
      const deleteBtn = element.querySelector('.delete-btn');
      expect(deleteBtn.getAttribute('aria-label')).toBe('Delete motivation test-motivation-123');
    });

    it('should log debug message when creating block', () => {
      displayEnhancer.createMotivationBlock(mockMotivation);
      expect(mockLogger.debug).toHaveBeenCalledWith('Created motivation block for ID: test-motivation-123');
    });
  });

  describe('formatTimestamp', () => {
    it('should format valid ISO string correctly', () => {
      const result = displayEnhancer.formatTimestamp('2024-12-20T15:45:00.000Z');
    ...
    it('should format different times correctly', () => {
      const morning = displayEnhancer.formatTimestamp('2024-01-15T09:30:00.000Z');
      expect(morning).toContain('Jan');
      expect(morning).toContain('15');
      expect(morning).toContain('2024');
      
      const evening = displayEnhancer.formatTimestamp('2024-06-30T20:15:00.000Z');
  ...
  describe('handleCopy', () => {
    it('should handle missing Clipboard API gracefully', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('Clipboard API not available');
  ...
  describe('handleCopy with Clipboard API', () => {
    ...
    it('should copy motivation text to clipboard', async () => {
      ...
      expect(clipboardWriteTextSpy).toHaveBeenCalledWith(expect.stringContaining('Core Motivation: To achieve greatness'));
      expect(clipboardWriteTextSpy).toHaveBeenCalledWith(expect.stringContaining('Internal Contradiction: Desires fame'));
      expect(clipboardWriteTextSpy).toHaveBeenCalledWith(expect.stringContaining('Central Question: Is lasting legacy'));
    });
    ...
    it('should log info message on successful copy', async () => {
      await displayEnhancer.handleCopy(mockMotivation);
      expect(mockLogger.info).toHaveBeenCalledWith('Copied motivation test-motivation-123 to clipboard');
    });
    ...
    it('should handle clipboard API failure', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to copy motivation: Clipboard access denied');
      expect(dispatchEventSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          type: 'motivationCopyFailed',
        })
  ...
  describe('handleDelete', () => {
    ...
    it('should handle non-string motivation ID', () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Invalid motivation ID provided for deletion');
    ...
    it('should log info message when triggering deletion', () => {
      displayEnhancer.handleDelete('test-motivation-123');
      expect(mockLogger.info).toHaveBeenCalledWith('Triggering deletion for motivation test-motivation-123');
    });
  ...
  describe('formatMotivationsForExport', () => {
    it('should format single motivation for export', () => {
      const result = displayEnhancer.formatMotivationsForExport([mockMotivation]);
    ...
    it('should format multiple motivations for export', () => {
      ...
      const result = displayEnhancer.formatMotivationsForExport([mockMotivation, motivation2]);
    ...
    it('should include direction title when provided', () => {
      ...
      const result = displayEnhancer.formatMotivationsForExport([mockMotivation], direction);
  ...
  describe('attachEventHandlers', () => {
    ...
    it('should log debug message when handlers attached', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('Event handlers attached for motivation test-motivation-123');
    });
    ...
    it('should handle missing block element', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('No block element provided for event attachment');
  ...
  describe('Event button interactions', () => {
    it('should have clickable copy button that triggers handleCopy', async () => {
      ...
      expect(handleCopySpy).toHaveBeenCalledWith(expect.objectContaining({
        id: 'test-motivation-123',
      }));
    });
  ...
  describe('DOM structure validation', () => {
    ...
    it('should handle minimal content gracefully', () => {
      ...
      const coreSection = element.querySelector('.motivation-section.core-motivation p');
      expect(coreSection.textContent).toBe('Brief desire');
      
      const contradictionSection = element.querySelector('.motivation-section.contradiction p');
      expect(contradictionSection.textContent).toBe('Brief contradiction');
      
      const questionSection = element.querySelector('.motivation-section.central-question p');
  ...
  describe('formatSingleMotivation', () => {
    ...
    it('should throw error for invalid motivation', () => {
      expect(() => displayEnhancer.formatSingleMotivation(null)).toThrow('Valid motivation object is required');
      expect(() => displayEnhancer.formatSingleMotivation('invalid')).toThrow('Valid motivation object is required');
      expect(() => displayEnhancer.formatSingleMotivation(undefined)).toThrow('Valid motivation object is required');
      
      expect(mockLogger.error).toHaveBeenCalledWith('Invalid motivation provided to formatSingleMotivation');
    });
  });
  ...
  describe('cleanupEventListeners', () => {
    ...
    it('should handle missing block element', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('No block element provided for cleanup');
    });
    ...
    it('should handle block without copy button', () => {
      ...
      expect(() => displayEnhancer.cleanupEventListeners(emptyBlock)).not.toThrow();
    });
