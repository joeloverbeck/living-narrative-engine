...
describe('ScopeEngine - Additional Coverage Tests', () => {
  ...
  describe('Error scenarios and edge cases', () => {
    test('should handle unknown AST node type', () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Unknown AST node type: UnknownNodeType');
    });

    test('should handle unknown source kind', () => {
      const unknownSourceAst = { type: 'Source', kind: 'unknownSource' };
      const result = engine.resolve(unknownSourceAst, actorEntity, mockRuntimeCtx);
      
      expect(result).toEqual(new Set());
      expect(mockLogger.error).toHaveBeenCalledWith('Unknown source kind: unknownSource');
    });
    ...
    test('should handle location source without location in runtime context', () => {
      ...
      const result = engine.resolve(ast, actorEntity, runtimeCtxWithoutLocation);
    ...
    test('should handle location source with location missing id', () => {
      ...
      const result = engine.resolve(ast, actorEntity, runtimeCtxWithBadLocation);
  ...
  describe('Negative component queries edge cases', () => {
    test('should handle negative component query using getEntities fallback', () => {
      ...
      mockEntityManager.hasComponent.mockImplementation((entityId, componentName) => {
        ...
        return entity && entity.components && entity.components[componentName];
      });
  ...
  describe('Tracing and instrumentation', () => {
    ...
    test('should add trace logs for source resolution', () => {
      ...
      const result = engine.resolve(ast, actorEntity, runtimeCtx, mockTraceContext);
  ...
  describe('Field access edge cases', () => {
    ...
    test('should handle object field access with missing property', () => {
      ...
      mockEntityManager.getComponentData.mockImplementation((entityId, fieldName) => {
    ...
    test('should handle non-object, non-string parent values', () => {
      ...
      engine.resolveNode = jest.fn().mockImplementation((node, actorEntity, runtimeCtx, depth, path, trace) => {
        if (node.type === 'Source') {
  ...
  describe('Filter resolution edge cases', () => {
    test('should handle filter with empty parent result', () => {
      const ast = parseDslExpression('entities(core:missing)[{"==": [{"var": "type"}, "test"]}]');
    ...
    test('should handle filter evaluation that throws exception', () => {
      ...
      mockEntityManager.getEntitiesWithComponent.mockReturnValue(entitiesWithComponent);
      mockEntityManager.getEntityInstance.mockReturnValue({ id: 'entity1', components: {} });
      ...
      expect(() => engine.resolve(ast, actorEntity, mockRuntimeCtx)).toThrow('JSON Logic evaluation failed');
    });
  ...
  describe('Union resolution edge cases', () => {
    ...
    test('should handle union with both sides empty', () => {
      const ast = parseDslExpression('entities(core:missing1) + entities(core:missing2)');
      mockEntityManager.getEntitiesWithComponent.mockReturnValue([]);
  ...
  describe('Depth and cycle detection edge cases', () => {
    test('should handle exact depth limit', () => {
      ...
      expect(() => engine.resolve(ast, actorEntity, mockRuntimeCtx)).not.toThrow();
    });
    ...
    test('should handle cycle detection with complex node keys', () => {
      ...
      engine.resolveNode = jest.fn().mockImplementation((node, actorEntity, runtimeCtx, depth, path, trace) => {
        ...
        if (callCount === 1) {
          ...
          return engine.resolveNode(node, actorEntity, runtimeCtx, depth, path, trace);
        }
        return new Set(['test']);
      });
      ...
      expect(() => engine.resolve(cyclicAst, actorEntity, mockRuntimeCtx)).not.toThrow();
