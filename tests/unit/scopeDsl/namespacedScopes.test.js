...
describe('Namespaced Scope Requirements', () => {
    ...
    describe('getScope validation', () => {
      beforeEach(() => {
        scopeRegistry.initialize({
          'core:followers': { expr: 'actor.core:leading.followers[]', modId: 'core' },
          'intimacy:close_actors': { expr: 'actor.components.intimacy:closeness.partners[]', modId: 'intimacy' },
        });
      });
      ...
      it('should throw error for unknown scope names regardless of format', () => {
        ...
        expect(() => scopeRegistry.getScope('core:unknown_scope')).not.toThrow();
        expect(scopeRegistry.getScope('core:unknown_scope')).toBeNull();
      });

      it('should throw error for multiple non-namespaced scope names', () => {
        const nonNamespacedScopes = ['potential_leaders', 'environment', 'close_actors', 'actors_in_location'];
  ...
  describe('Scope Definition Parser', () => {
    it('should require namespace in scope definitions', () => {
      ...
      expect(() => parseScopeDefinitions(validContent, 'test.scope')).not.toThrow();
    ...
    it('should reject non-namespaced scope definitions', () => {
      ...
      expect(() => parseScopeDefinitions(invalidContent, 'test.scope')).toThrow();
    });
  ...
  describe('Scope Loader Validation', () => {
    ...
    let scopeLoader;
         ...
         beforeEach(() => {
       mockPathResolver = { resolve: jest.fn(), resolveModContentPath: jest.fn() };
       mockDataFetcher = { fetch: jest.fn() };
       mockSchemaValidator = { validate: jest.fn(), getValidator: jest.fn(), isSchemaLoaded: jest.fn() };
       mockDataRegistry = { set: jest.fn(), has: jest.fn().mockReturnValue(false), store: jest.fn(), get: jest.fn().mockReturnValue(undefined) };
    ...
    describe('transformContent validation', () => {
      it('should reject scope definitions without namespace', () => {
        ...
        expect(() => scopeLoader.transformContent(parsedContent, 'core')).toThrow(
    ...
    it('should enforce namespaced scopes throughout the entire pipeline', () => {
      ...
      const parsedScopes = parseScopeDefinitions(validContent, 'test.scope');
       ...
       const mockLoader = new ScopeLoader(
         ...
         { fetch: jest.fn() },
         { validate: jest.fn(), getValidator: jest.fn(), isSchemaLoaded: jest.fn() },
         { set: jest.fn(), has: jest.fn().mockReturnValue(false), store: jest.fn(), get: jest.fn().mockReturnValue(undefined) },
         { error: jest.fn(), warn: jest.fn(), info: jest.fn(), debug: jest.fn() }
       );
      
      const transformedScopes = mockLoader.transformContent(parsedScopes, 'core');
