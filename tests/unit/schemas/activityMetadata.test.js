...
describe('Activity Metadata Schema Validation', () => {
  const componentsToTest = [
    { mod: 'companionship', name: 'following', targetRole: 'leaderId', priority: 60 },
    { mod: 'companionship', name: 'leading', targetRole: 'followers', priority: 58 },
    { mod: 'hand-holding', name: 'hand_held', targetRole: 'holding_entity_id', priority: 65 },
    { mod: 'hand-holding', name: 'holding_hand', targetRole: 'held_entity_id', priority: 67 },
    { mod: 'kissing', name: 'kissing', targetRole: 'partner', priority: 72 },
    { mod: 'positioning', name: 'being_bitten_in_neck', targetRole: 'biting_entity_id', priority: 70 },
    { mod: 'positioning', name: 'being_hugged', targetRole: 'hugging_entity_id', priority: 63 },
    { mod: 'positioning', name: 'being_restrained', targetRole: 'restraining_entity_id', priority: 64 },
    { mod: 'positioning', name: 'bending_over', targetRole: 'surface_id', priority: 68 },
    { mod: 'positioning', name: 'biting_neck', targetRole: 'bitten_entity_id', priority: 73 },
    { mod: 'positioning', name: 'giving_blowjob', targetRole: 'receiving_entity_id', priority: 80 },
    { mod: 'positioning', name: 'hugging', targetRole: 'embraced_entity_id', priority: 66 },
    { mod: 'positioning', name: 'restraining', targetRole: 'restrained_entity_id', priority: 67 },
    { mod: 'positioning', name: 'lying_down', targetRole: 'furniture_id', priority: 64 },
    { mod: 'positioning', name: 'receiving_blowjob', targetRole: 'giving_entity_id', priority: 78 },
    { mod: 'positioning', name: 'sitting_on', targetRole: 'furniture_id', priority: 62 },
    { mod: 'positioning', name: 'straddling_waist', targetRole: 'target_id', priority: 82 },
  ];
  ...
  componentsToTest.forEach(({ mod, name, targetRole, priority }) => {
    ...
    describe(`${componentId}`, () => {
      ...
      it('should have activityMetadata property in dataSchema', () => {
        expect(componentSchema.dataSchema.properties).toHaveProperty('activityMetadata');
      });
      ...
      it('should have correct shouldDescribeInActivity configuration', () => {
        const field = componentSchema.dataSchema.properties.activityMetadata.properties.shouldDescribeInActivity;
        expect(field.type).toBe('boolean');
        expect(field.default).toBe(true);
      });

      it('should have template with {actor} placeholder', () => {
        const field = componentSchema.dataSchema.properties.activityMetadata.properties.template;
        expect(field.type).toBe('string');
        expect(field.default).toContain('{actor}');
      });

      it('should have correct targetRole mapping', () => {
        const field = componentSchema.dataSchema.properties.activityMetadata.properties.targetRole;
        expect(field.type).toBe('string');
        expect(field.default).toBe(targetRole);
      });

      it('should have priority in valid range (0-100)', () => {
        const field = componentSchema.dataSchema.properties.activityMetadata.properties.priority;
