...
describe('PartGroupingStrategies', () => {
  ...
  beforeEach(() => {
    mockTextFormatter = {
      ...
      getPartLabel: jest.fn().mockImplementation((type, count, pluralizer, paired) => {
      ...
      formatIndexedItem: jest.fn((type, index, desc) => `${mockTextFormatter.capitalize(type)} ${index}: ${desc}`),
      formatSidedItem: jest.fn((side, type, desc) => `${side} ${type}: ${desc}`),
  ...
  describe('PartGroupingStrategy base class', () => {
    it('should throw error when canHandle not implemented', () => {
      ...
      expect(() => strategy.canHandle()).toThrow('canHandle must be implemented by subclass');
    });

    it('should throw error when format not implemented', () => {
      const strategy = new PartGroupingStrategy();
      expect(() => strategy.format()).toThrow('format must be implemented by subclass');
    });
  });
  ...
  describe('SinglePartStrategy', () => {
    ...
    describe('canHandle', () => {
      it('should return true for single description', () => {
        const result = strategy.canHandle('arm', [mockPart], ['strong arm'], mockConfig);
        expect(result).toBe(true);
      });

      it('should return false for multiple descriptions', () => {
        const result = strategy.canHandle('arm', [mockPart, mockPart], ['arm1', 'arm2'], mockConfig);
  ...
  describe('PairedPartsStrategy', () => {
    ...
    describe('canHandle', () => {
      it('should return true for paired part with 2 descriptions', () => {
        const result = strategy.canHandle('eye', [mockPart, mockPart], ['blue', 'green'], mockConfig);
        expect(result).toBe(true);
      });

      it('should return false for non-paired part', () => {
        mockConfig.getPairedParts.mockReturnValue(new Set(['eye']));
        const result = strategy.canHandle('torso', [mockPart], ['muscular'], mockConfig);
        expect(result).toBe(false);
      });

      it('should return false for wrong number of descriptions', () => {
        const result = strategy.canHandle('eye', [mockPart], ['blue'], mockConfig);
        expect(result).toBe(false);
      });
    });
  ...
  describe('MultiplePartsStrategy', () => {
    ...
    describe('canHandle', () => {
      it('should return true for any non-empty descriptions', () => {
        expect(strategy.canHandle('tentacle', [mockPart], ['slimy'], mockConfig)).toBe(true);
        expect(strategy.canHandle('wing', [mockPart, mockPart, mockPart], ['a', 'b', 'c'], mockConfig)).toBe(true);
      });
  ...
  describe('PartGroupingStrategyFactory', () => {
    ...
    it('should return SinglePartStrategy for single part', () => {
      const strategy = factory.getStrategy('arm', [mockPart], ['strong arm'], mockConfig);
    ...
    it('should return PairedPartsStrategy for paired parts with 2 descriptions', () => {
      const strategy = factory.getStrategy('eye', [mockPart, mockPart], ['blue', 'green'], mockConfig);
      expect(strategy).toBeInstanceOf(PairedPartsStrategy);
    });

    it('should return MultiplePartsStrategy for multiple parts', () => {
      const strategy = factory.getStrategy('tentacle', [mockPart, mockPart, mockPart], ['a', 'b', 'c'], mockConfig);
    ...
    it('should return MultiplePartsStrategy for non-paired parts with 2 descriptions', () => {
      ...
      const strategy = factory.getStrategy('tentacle', [mockPart, mockPart], ['a', 'b'], mockConfig);
      expect(strategy).toBeInstanceOf(MultiplePartsStrategy);
    });
