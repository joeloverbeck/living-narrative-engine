...
describe('TextFormatter', () => {
  ...
  describe('getPartLabel', () => {
    ...
    it('should return capitalized singular for single part', () => {
      const result = formatter.getPartLabel('arm', 1, mockPluralizerFn, pairedPartsSet);
    ...
    it('should return capitalized plural for multiple paired parts', () => {
      const result = formatter.getPartLabel('arm', 2, mockPluralizerFn, pairedPartsSet);
    ...
    it('should return singular for multiple non-paired parts', () => {
      ...
      const result = formatter.getPartLabel('tail', 3, mockPluralizerFn, nonPairedSet);
      expect(result).toBe('Tail');
      expect(mockPluralizerFn).not.toHaveBeenCalled();
    });

    it('should handle irregular plurals', () => {
      const result = formatter.getPartLabel('foot', 2, mockPluralizerFn, pairedPartsSet);
      expect(result).toBe('Feet');
      expect(mockPluralizerFn).toHaveBeenCalledWith('foot');
    });

    it('should handle empty part type', () => {
      const result = formatter.getPartLabel('', 2, mockPluralizerFn, pairedPartsSet);
      expect(result).toBe('');
    });
  });
  ...
  describe('joinDescriptors', () => {
    it('should join multiple descriptors with commas', () => {
      expect(formatter.joinDescriptors(['red', 'large', 'round'])).toBe('red, large, round');
    });
    ...
    it('should filter out empty values', () => {
      expect(formatter.joinDescriptors(['red', '', 'blue', null, 'green'])).toBe('red, blue, green');
    });
  ...
  describe('formatLabelValue', () => {
    it('should format label and value with colon separator', () => {
      expect(formatter.formatLabelValue('Hair', 'long black hair')).toBe('Hair: long black hair');
      expect(formatter.formatLabelValue('Eyes', 'blue eyes')).toBe('Eyes: blue eyes');
    });
  ...
  describe('formatIndexedItem', () => {
    it('should format indexed items correctly', () => {
      expect(formatter.formatIndexedItem('arm', 1, 'strong arm')).toBe('Arm 1: strong arm');
      expect(formatter.formatIndexedItem('wing', 2, 'feathered wing')).toBe('Wing 2: feathered wing');
    });

    it('should capitalize the type', () => {
      expect(formatter.formatIndexedItem('tentacle', 3, 'slimy tentacle'))
        .toBe('Tentacle 3: slimy tentacle');
    });
  ...
  describe('formatSidedItem', () => {
    it('should format left/right items correctly', () => {
      expect(formatter.formatSidedItem('Left', 'eye', 'blue eye')).toBe('Left eye: blue eye');
      expect(formatter.formatSidedItem('Right', 'arm', 'strong arm')).toBe('Right arm: strong arm');
    });
