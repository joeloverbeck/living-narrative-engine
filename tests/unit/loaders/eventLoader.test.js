...
/** @typedef {import('../../../src/loaders/baseManifestItemLoader.js').LoadItemsResult} LoadItemsResult */
...
beforeEach(() => {
  ...
  if (EventLoader.prototype._processFetchedItem) {
    eventLoader._processFetchedItem =
      EventLoader.prototype._processFetchedItem.bind(eventLoader);
  }
});
...
describe('EventLoader', () => {
  ...
  describe('loadItemsForMod', () => {
    ...
    it('should return 0 result object and log error if modId is missing or invalid', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          `Invalid 'modId' provided for loading ${EVENT_TYPE_NAME}`
        )
      );
    ...
    it('should return 0 result object and log error if modManifest is missing or invalid', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          `Invalid 'modManifest' provided for loading ${EVENT_TYPE_NAME}`
        )
      );
      expect(loadItemsInternalSpy).not.toHaveBeenCalled();
    });
    ...
    it('should throw TypeError and log error if contentKey is invalid', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(`Invalid 'contentKey' provided`)
      );
      expect(loadItemsInternalSpy).not.toHaveBeenCalled();
    });
    ...
    it('should throw TypeError and log error if contentTypeDir is invalid', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(`Invalid 'contentTypeDir' provided`)
      );
      expect(loadItemsInternalSpy).not.toHaveBeenCalled();
    });
    ...
    it('should throw TypeError and log error if typeName is invalid', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(`Invalid 'typeName' provided`)
      );
      expect(loadItemsInternalSpy).not.toHaveBeenCalled();
    });
    ...
    it('should return the result object from _loadItemsInternal and log summary', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        `EventLoader [${TEST_MOD_ID}]: Finished loading for type '${EVENT_TYPE_NAME}'. Result: C:${expectedLoadResult.count}, O:${expectedLoadResult.overrides}, E:${expectedLoadResult.errors}`
      );
    });
  ...
  describe('_processFetchedItem', () => {
    ...
    it('Success Path (with payload): should extract ID, register payload schema, store, log, and return result object', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Extracted full event ID '${fullEventIdFromFile}' and base event ID '${baseEventIdExtracted}'`
        )
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(`Found valid payloadSchema in ${filename}`)
      );
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Successfully registered payload schema '${payloadSchemaId}'`
        )
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Delegating storage for event (base ID: '${baseEventIdExtracted}')`
        )
      );
      expect(storeItemInRegistrySpy).toHaveBeenCalledTimes(1);
      expect(storeItemInRegistrySpy).toHaveBeenCalledWith(
        EVENT_TYPE_NAME,
        TEST_MOD_ID,
        baseEventIdExtracted,
        fetchedData,
        filename
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Successfully processed event definition from ${filename}. Returning final registry key: ${finalRegistryKey}, Overwrite: false`
        )
      );
    ...
    it('Success Path (no payload): should extract ID, skip payload schema, store, log, and return result object', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Extracted full event ID '${simpleFullId}' and base event ID '${simpleBaseId}'`
        )
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(`No valid payloadSchema found`)
      );
    ...
    it('Failure: Primary schema validation fails (simulated via spy) - *Test checks ID failure due to direct call*', async () => {
      ...
      }); // Mock ineffective for direct call
    ...
    it('Failure: Payload schema registration fails', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Extracted full event ID '${fullEventIdFromFile}'`
        )
      );
      expect(mockValidator.isSchemaLoaded).toHaveBeenCalledWith(
        payloadSchemaId
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          `CRITICAL - Failed to register payload schema '${payloadSchemaId}' for event '${fullEventIdFromFile}'.`
        ), // Check substring
        expect.objectContaining({
    ...
    it('Failure: Storage fails (error from base _storeItemInRegistry)', async () => {
      ...
      storeItemInRegistrySpy.mockImplementation(() => {
        mockLogger.error(
          `Simulated base storage failure log for ${finalRegistryKey}`
        );
        throw storeError;
      });
      mockValidator.isSchemaLoaded.mockReturnValueOnce(false);
      ...
      expect(mockLogger.debug).not.toHaveBeenCalledWith(
        expect.stringContaining('Successfully processed event definition')
      );
    });
    ...
    it('Edge Case: Event ID without namespace', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Extracted full event ID '${baseId}' and base event ID '${baseId}'`
        )
      );
      expect(storeItemInRegistrySpy).toHaveBeenCalledWith(
        EVENT_TYPE_NAME,
    ...
    it('Edge Case: Event ID with multiple colons', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Extracted full event ID '${fullId}' and base event ID '${baseId}'`
        )
      );
      expect(storeItemInRegistrySpy).toHaveBeenCalledWith(
        EVENT_TYPE_NAME,
