    ...
    it('should resolve valid rule filenames and process them successfully', async () => {
      ...
      expect(mockRegistry.store).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          id: `${modId}:${ruleFile1Name}`, // QUALIFIED ID (prefixed)
      ...
      expect(mockRegistry.store).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          id: `${modId}:${ruleFile2Name}`, // QUALIFIED ID (prefixed)
  ...
  describe('Empty/Invalid Entries in rules Array', () => {
    ...
    it('should filter out invalid entries, log warnings, and process only valid ones', async () => {
      ...
      const dataForValidFile = {
        ...
        actions: [
          { type: 'LOG', parameters: { message: 'Test valid action' } },
        ],
      };
      ...
      expect(mockRegistry.store).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...dataForValidFile,
          id: `${modId}:${validFileName}`, // QUALIFIED ID (prefixed)
