...
describe('SpeechPatternsGeneratorController', () => {
  ...
  beforeEach(() => {
    ...
    // Mock ControllerLifecycleOrchestrator
    ...
    // Mock DOMElementManager
    mockDOMElementManager = {
      cacheElementsFromMap: jest.fn(),
      cacheElement: jest.fn(),
      getElement: jest.fn((name) => mockElements[name] || null),
      hasElement: jest.fn(() => true),
    };

    // Mock AsyncUtilitiesToolkit
  ...
  /**
   * Helper function to add base class dependencies
   * @param {object} dependencies - Controller-specific dependencies
   * @returns {object} Dependencies with base class requirements
   */
  function addBaseClassDependencies(dependencies) {
    return {
  ...
  async function createInitializedController(dependencies) {
    // Merge with base class dependencies using helper
    const fullDependencies = addBaseClassDependencies(dependencies);

    const controller = new SpeechPatternsGeneratorController(fullDependencies);

    // Mock eventRegistry from base class (needed for event listener migration)
    Object.defineProperty(controller, 'eventRegistry', {
      get: () => mockEventRegistry,
      configurable: true,
    });
  ...
  describe('Constructor Tests', () => {
    it('should create instance with all required dependencies', () => {
      const dependencies = addBaseClassDependencies({
        logger: mockLogger,
  ...
  describe('Event Listeners Setup Tests', () => {
    ...
    it('should create debounced validation function', () => {
      // Find the input handler from EventListenerRegistry calls
      const inputCall = mockEventRegistry.addEventListener.mock.calls.find(
        (call) => call[1] === 'input' && call[0] === mockElements.characterDefinition
      );
      expect(inputCall).toBeDefined();

      const inputHandler = inputCall[2];
    ...
    it('should register 8 event listeners via EventListenerRegistry', () => {
      expect(mockEventRegistry.addEventListener).toHaveBeenCalledTimes(8);
    });

    it('should register characterDefinition input listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const inputListenerCall = calls.find(
        (call) => call[1] === 'input'
      );
      expect(inputListenerCall).toBeDefined();
      expect(inputListenerCall[2]).toBeInstanceOf(Function);
    });

    it('should register characterDefinition blur listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const blurListenerCall = calls.find(
        (call) => call[1] === 'blur'
      );
      expect(blurListenerCall).toBeDefined();
      expect(blurListenerCall[2]).toBeInstanceOf(Function);
    });

    it('should register generate button listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const generateListenerCall = calls.find(
        (call) => call[0] === mockElements.generateBtn && call[1] === 'click'
      );
      expect(generateListenerCall).toBeDefined();
    });

    it('should register export button listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const exportListenerCall = calls.find(
        (call) => call[0] === mockElements.exportBtn && call[1] === 'click'
      );
      expect(exportListenerCall).toBeDefined();
    });

    it('should register export format selector listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const formatListenerCall = calls.find(
        (call) => call[0] === mockElements.exportFormat && call[1] === 'change'
      );
      expect(formatListenerCall).toBeDefined();
    });

    it('should register clear button listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const clearListenerCall = calls.find(
        (call) => call[0] === mockElements.clearBtn && call[1] === 'click'
      );
      expect(clearListenerCall).toBeDefined();
    });

    it('should register back button listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const backListenerCall = calls.find(
        (call) => call[0] === mockElements.backBtn && call[1] === 'click'
      );
      expect(backListenerCall).toBeDefined();
    });

    it('should register retry button listener', () => {
      const calls = mockEventRegistry.addEventListener.mock.calls;
      const retryListenerCall = calls.find(
        (call) => call[0] === mockElements.retryBtn && call[1] === 'click'
      );
      expect(retryListenerCall).toBeDefined();
    });
  });
