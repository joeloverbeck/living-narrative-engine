...
describe('SpeechPatternsResponseProcessor - Schema Validation Fix', () => {
  ...
  describe('Enhanced Validation Issues', () => {
    it('should accept examples with quoted speech', async () => {
      ...
      const validResponse = JSON.stringify({
        characterName: 'Test Character',
        speechPatterns: [
          { pattern: 'Direct communication style', example: '"I need this done by tomorrow."' },
          { pattern: 'Uses rhetorical questions', example: '"Don\'t you think we should consider other options?"' },
          { pattern: 'Expresses uncertainty with qualifiers', example: '"I think maybe we could try a different approach."' },
          { pattern: 'Apologetic tendencies', example: '"Sorry, I didn\'t mean to interrupt."' },
        ],
      });
    ...
    it('should reject examples without quoted speech in enhanced validation', async () => {
      ...
      const invalidResponse = JSON.stringify({
        characterName: 'Test Character',
        speechPatterns: [
          { pattern: 'Direct communication style', example: 'Uses direct statements without quotes' }, // Invalid - no quotes
          { pattern: 'Uses rhetorical questions', example: 'Asks questions indirectly' }, // Invalid - no quotes
          { pattern: 'Expresses uncertainty', example: 'Shows hesitation in speech' }, // Invalid - no quotes
        ],
      });

      await expect(processor.processResponse(invalidResponse, {
        characterName: 'Test Character',
    ...
    it('should require specific pattern descriptions not generic terms', async () => {
      ...
      mockSchemaValidator.validate.mockReturnValue({
        isValid: false,
        errors: ['Pattern 1: pattern description should be more specific than generic terms']
      });
  ...
  describe('Minimum Pattern Requirements', () => {
    it('should reject responses with insufficient patterns', async () => {
      ...
      await expect(processor.processResponse(insufficientResponse, {
        characterName: 'Test Character',
    ...
    it('should accept responses with sufficient patterns', async () => {
      const sufficientResponse = JSON.stringify({
        ...
        speechPatterns: [
          ...
          { pattern: 'Expresses uncertainty with qualifiers', example: '"I think maybe we could try a different approach."' },
          { pattern: 'Apologetic tendencies', example: '"Sorry, I didn\'t mean to interrupt."' },
          { pattern: 'Confident assertions', example: '"I\'m absolutely certain this will work."' },
        ],
      });
  ...
  describe('JSON Parsing Error Handling', () => {
    it('should handle malformed JSON responses gracefully', async () => {
      ...
      await expect(processor.processResponse(malformedResponse, {
        characterName: 'Test Character',
      })).rejects.toThrow();
    ...
    it('should clean and repair JSON before parsing', async () => {
      ...
      const messyResponse = ' { "characterName" : "Test Character" , speechPatterns : [ } '; // Messy JSON
