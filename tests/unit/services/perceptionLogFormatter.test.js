...
describe('PerceptionLogFormatter', () => {
  ...
  describe('Constructor', () => {
    it('should initialize with a logger', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'PerceptionLogFormatter initialized.'
      );
    });

    it('should throw an error if logger is not provided', () => {
      expect(() => new PerceptionLogFormatter({})).toThrow(
        'PerceptionLogFormatter: Logger dependency is required.'
      );
      expect(() => new PerceptionLogFormatter({ logger: null })).toThrow(
        'PerceptionLogFormatter: Logger dependency is required.'
      );
    });
  });
  ...
  describe('format', () => {
    ...
    it('should return an empty array for an empty input array and log debug message', () => {
      ...
      expect(result).toEqual([]);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'PerceptionLogFormatter.format attempting to process 0 entries.'
      );
    });
    ...
    it('should correctly format valid log entries with all fields', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'PerceptionLogFormatter.format attempting to process 2 entries.'
      );
    ...
    it('should warn if timestamp is missing and map it as undefined', () => {
      const rawEntries = [
        {
          eventId: 'ev-no-ts',
          descriptionText: 'No timestamp here',
          perceptionType: 'info',
        },
      ];
      const result = formatter.format(rawEntries);
    ...
    it('should use eventId "N/A" in warnings if eventId itself is missing when other fields are also missing', () => {
      const rawEntries = [
        { descriptionText: 'No timestamp, no eventId', perceptionType: 'info' },
      ]; // No eventId, no timestamp
      formatter.format(rawEntries);
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          "PerceptionLogFormatter.format: Perception log entry (event ID: N/A) missing 'timestamp'."
        )
      );
      ...
      const rawEntries2 = [
        { descriptionText: 'No type, no eventId', timestamp: 'ts-no-type' },
      ]; // No eventId, no perceptionType
      formatter.format(rawEntries2);
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          "PerceptionLogFormatter.format: Perception log entry (event ID: N/A) missing 'perceptionType' (for 'type')."
        )
      );
    });
    ...
    it('should warn if perceptionType is missing and map it as undefined', () => {
      const rawEntries = [
        {
          eventId: 'ev-no-type',
          descriptionText: 'No type here',
          timestamp: 'ts-no-type',
        },
      ];
      const result = formatter.format(rawEntries);
    ...
    it('should filter out null entries and log a warning', () => {
      const rawEntries = [
        ...
        {
          eventId: 'evValid1',
          descriptionText: 'Valid Entry 1',
          timestamp: 'ts1',
          perceptionType: 'type1',
        },
        null,
        {
          eventId: 'evValid2',
          descriptionText: 'Valid Entry 2',
          timestamp: 'ts2',
          perceptionType: 'type2',
        },
      ];
      const result = formatter.format(rawEntries);
    ...
    it('should filter out non-object entries and log a warning', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `PerceptionLogFormatter.format: Invalid raw perception log entry skipped: "not_an_object_string"`
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `PerceptionLogFormatter.format: Invalid raw perception log entry skipped: 12345`
      );
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `PerceptionLogFormatter.format: Invalid raw perception log entry skipped: true`
      );
    });
    ...
    it('should handle a mix of valid, invalid, and missing-field entries correctly', () => {
      const rawEntries = [
        ...
        {
          eventId: 'evNoTs',
          descriptionText: 'No Timestamp',
          perceptionType: 'debug',
        }, // Valid, 1 warn (missing timestamp)
        {
          eventId: 'evNoType',
          descriptionText: 'No Type',
          timestamp: 'tsNoType',
        }, // Valid, 1 warn (missing perceptionType)
        {
          eventId: 'evValid2',
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'PerceptionLogFormatter.format attempting to process 7 entries.'
      );
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          `Perception log entry (event ID: evNoDesc) resulted in empty 'content'`
        )
      );
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `Perception log entry (event ID: evNoTs) missing 'timestamp'`
        )
      );
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          `Perception log entry (event ID: evNoType) missing 'perceptionType'`
        )
      );
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `PerceptionLogFormatter.format: Invalid raw perception log entry skipped: "invalid_string_entry"`
      );
