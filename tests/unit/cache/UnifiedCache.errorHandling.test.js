...
describe('UnifiedCache - error handling coverage', () => {
  ...
  it('should wrap strategy set errors in CacheError', async () => {
    ...
    await jest.isolateModulesAsync(async () => {
      const [strategyModule, { UnifiedCache }, { CacheError: CacheErrorClass }] = await Promise.all([
      ...
      try {
        const cache = new UnifiedCache({ logger: mockLogger }, { evictionPolicy: 'lru' });
        cache.set('key', 'value');
      } catch (error) {
        thrownError = error;
      } finally {
  ...
  it('should throw InvalidArgumentError when unsupported strategies reach creation', async () => {
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      const valuesSpy = jest.spyOn(Object, 'values').mockImplementation((target) => {
      ...
      try {
        const [{ UnifiedCache, EvictionPolicy }, { InvalidArgumentError: InvalidArgumentErrorClass }] =
          await Promise.all([
            import('../../../src/cache/UnifiedCache.js'),
        ...
        expect(() => {
          new UnifiedCache({ logger: mockLogger }, { evictionPolicy: 'unsupported' });
