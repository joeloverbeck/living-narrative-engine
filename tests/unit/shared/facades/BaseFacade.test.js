...
class TestFacade extends BaseFacade {
  ...
  async testExecuteWithResilience(operationName, operation, fallback, options) {
    return await this.executeWithResilience(operationName, operation, fallback, options);
  }
...
describe('BaseFacade', () => {
  ...
  beforeEach(() => {
    ...
    mockCache = testBed.createMock('cache', ['get', 'set', 'invalidate', 'invalidateByPattern']);
    ...
    mockCircuitBreaker.execute.mockImplementation(async (name, op) => await op());
  ...
  describe('executeWithResilience', () => {
    it('should execute operation successfully', async () => {
      ...
      const result = await facade.testExecuteWithResilience('testOp', operation);
    ...
    it('should use circuit breaker when available', async () => {
      ...
      expect(result).toBe('success');
      expect(mockCircuitBreaker.execute).toHaveBeenCalledWith('testOp', expect.any(Function));
    });
    ...
    it('should execute directly when circuit breaker is not available', async () => {
      ...
      const result = await facadeWithoutCB.testExecuteWithResilience('testOp', operation);
    ...
    it('should use fallback when operation fails', async () => {
      ...
      const result = await facade.testExecuteWithResilience('testOp', operation, fallback);
    ...
    it('should throw error when operation fails and no fallback', async () => {
      ...
      await expect(facade.testExecuteWithResilience('testOp', operation))
    ...
    it('should handle operation timeout', async () => {
      ...
      await expect(facade.testExecuteWithResilience('testOp', operation, null, { timeout: 100 }))
    ...
    it('should retry failed operations', async () => {
      ...
      const result = await facade.testExecuteWithResilience('testOp', operation, null, { retries: 2 });
    ...
    it('should validate operation parameter', async () => {
      await expect(facade.testExecuteWithResilience('testOp', null))
        .rejects.toThrow(InvalidArgumentError);
      
      await expect(facade.testExecuteWithResilience('testOp', 'not a function'))
        .rejects.toThrow(InvalidArgumentError);
    });
    ...
    it('should validate operation name', async () => {
      ...
      await expect(facade.testExecuteWithResilience('', operation))
        .rejects.toThrow();
      
      await expect(facade.testExecuteWithResilience(null, operation))
        .rejects.toThrow();
    });
  });
  ...
  describe('cacheableOperation', () => {
    ...
    it('should execute operation when cache miss', async () => {
      ...
      expect(operation).toHaveBeenCalledTimes(1);
      expect(mockCache.set).toHaveBeenCalledWith('test-key', 'fresh result', {});
      expect(mockLogger.debug).toHaveBeenCalledWith(
    ...
    it('should force refresh when forceRefresh option is true', async () => {
      ...
      const result = await facade.testCacheableOperation('test-key', operation, { forceRefresh: true });
    ...
    it('should use custom TTL when provided', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith('test-key', 'result', { ttl: 3600 });
    });

    it('should validate cache key parameter', async () => {
      const operation = jest.fn().mockResolvedValue('result');
      
      await expect(facade.testCacheableOperation('', operation))
        .rejects.toThrow();
      
      await expect(facade.testCacheableOperation(null, operation))
        .rejects.toThrow();
    });

    it('should validate operation parameter', async () => {
      await expect(facade.testCacheableOperation('test-key', null))
        .rejects.toThrow(InvalidArgumentError);
      
      await expect(facade.testCacheableOperation('test-key', 'not a function'))
        .rejects.toThrow(InvalidArgumentError);
    });
    ...
    it('should handle cache errors gracefully', async () => {
      ...
      await expect(facade.testCacheableOperation('test-key', operation))
        .rejects.toThrow();
  ...
  describe('dispatchEvent', () => {
    ...
    it('should validate event type parameter', () => {
      expect(() => facade.testDispatchEvent('', { data: 'test' }))
        .not.toThrow(); // Should just not dispatch
      
      expect(() => facade.testDispatchEvent(null, { data: 'test' }))
        .not.toThrow(); // Should just not dispatch
    });
    ...
    it('should handle event bus errors gracefully', () => {
      ...
      expect(() => facade.testDispatchEvent('TEST_EVENT', { data: 'test' }))
        .not.toThrow();
  ...
  describe('logOperation', () => {
    ...
    it('should handle logger failures gracefully', () => {
      ...
      expect(() => facade.testLogOperation('info', 'Test message'))
        .not.toThrow();
    });
  ...
  describe('invalidateCache', () => {
    ...
    it('should handle pattern invalidation when not supported', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Cache does not support pattern invalidation: test-*'),
        expect.any(Object)
      );
    });
  ...
  describe('Integration', () => {
    it('should work together in complex scenarios', async () => {
      ...
      const result = await facade.testCacheableOperation('complex-key', async () => {
        return await facade.testExecuteWithResilience('complexOp', operation, null, { retries: 1 });
      });
