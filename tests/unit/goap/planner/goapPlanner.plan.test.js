...
describe('GoapPlanner - plan() Method (A* Search)', () => {
      ...
      mockRepository.get.mockReturnValue({
        core: {
          'core:eat_apple': {
            ...
            cost: 1,
            planningEffects: [
              { op: 'set', path: 'actor:hunger', value: false },
            ],
          },
        },
      });
      ...
      mockRepository.get.mockReturnValue({
        core: {
          'core:acquire_apple': {
            ...
            cost: 1,
            planningEffects: [
              { op: 'set', path: 'actor:has_apple', value: true },
            ],
          },
          'core:eat_apple': {
            id: 'core:eat_apple',
      ...
      mockRepository.get.mockReturnValue({
        core: {
          'core:walk': {
            ...
            cost: 1,
            planningEffects: [
              { op: 'set', path: 'actor:position', value: 'new' },
            ],
          },
        },
      });
      ...
      mockRepository.get.mockReturnValue({
        core: {
          'core:cheap_to_x': {
            ...
            cost: 2,
            planningEffects: [
              { op: 'set', path: 'actor:state_x', value: true },
            ],
          },
          'core:expensive_to_x': {
          ...
          'core:x_to_goal': {
            ...
            cost: 1,
            planningEffects: [
              { op: 'set', path: 'actor:at_goal', value: true },
            ],
          },
        },
      });
      ...
      mockRepository.get.mockReturnValue({
        core: {
          'core:use_item': {
            ...
            cost: 1,
            planningEffects: [
              { op: 'set', path: 'actor:success', value: true },
            ],
          },
          'core:direct_action': {
  ...
  describe('13. Depth semantics and telemetry instrumentation', () => {
     ...
     */
    function setupHighCostHealingScenario({
      stepValue = 10,
      targetHp = 100,
    } = {}) {
      mockRepository.get.mockReturnValue({
    ...
    it('should allow short high-cost plans when depth limit is satisfied', () => {
      ...
      expect(plan.tasks.every((task) => task.taskId === 'core:heal_10')).toBe(
        true
      );
    });
  ...
  describe('Goal path validation', () => {
    it('records diagnostics when goal uses actor.* without components', () => {
      ...
      expect(failure).toEqual(
        expect.objectContaining({
          code: GOAP_PLANNER_FAILURES.INVALID_GOAL_PATH,
        })
      );
