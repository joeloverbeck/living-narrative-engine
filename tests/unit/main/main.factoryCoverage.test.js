...
describe('main.js factory-driven bootstrap coverage', () => {
  ...
  it('executes stage-provided factories for UI, container, and engine setup', async () => {
    ...
    mockSetupDI.mockImplementation(
      async (elements, _configure, { createAppContainer }) => {
    ...
    mockInitEngine.mockImplementation(
      async (container, loggerArg, { createGameEngine }) => {
        expect(container.marker).toBe('app-container');
    ...
    mockDisplayFatalStartupError.mockImplementation(() => {
      throw new Error(
        'displayFatalStartupError should not be invoked during successful bootstrap'
      );
    });
