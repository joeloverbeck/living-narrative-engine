...
describe('AnatomyGraphRenderer - Radial Layout', () => {
  ...
  describe('_polarToCartesian', () => {
    it('should convert polar coordinates correctly', () => {
      ...
      result = renderer._polarToCartesian(center.x, center.y, radius, Math.PI / 2);
      ...
      result = renderer._polarToCartesian(center.x, center.y, radius, 3 * Math.PI / 2);
      expect(result.x).toBeCloseTo(100);
  ...
  describe('Radial positioning', () => {
    ...
    it('should handle linear chain correctly', async () => {
      ...
      const distB = Math.sqrt(Math.pow(nodeB.x - nodeA.x, 2) + Math.pow(nodeB.y - nodeA.y, 2));
      expect(distB).toBeCloseTo(150, 1);
      
      // C should be at appropriate radius from B (depth-based)
      const distC = Math.sqrt(Math.pow(nodeC.x - nodeB.x, 2) + Math.pow(nodeC.y - nodeB.y, 2));
      expect(distC).toBeGreaterThan(100); // Minimum distance
      
      // D should be at appropriate radius from C (depth-based)
      const distD = Math.sqrt(Math.pow(nodeD.x - nodeC.x, 2) + Math.pow(nodeD.y - nodeC.y, 2));
      expect(distD).toBeGreaterThan(100); // Minimum distance
      
      // All nodes should be at increasing distances from root
      const distBFromRoot = Math.sqrt(Math.pow(nodeB.x - nodeA.x, 2) + Math.pow(nodeB.y - nodeA.y, 2));
      const distCFromRoot = Math.sqrt(Math.pow(nodeC.x - nodeA.x, 2) + Math.pow(nodeC.y - nodeA.y, 2));
      const distDFromRoot = Math.sqrt(Math.pow(nodeD.x - nodeA.x, 2) + Math.pow(nodeD.y - nodeA.y, 2));
