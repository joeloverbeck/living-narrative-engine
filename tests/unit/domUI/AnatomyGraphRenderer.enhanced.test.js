...
describe('AnatomyGraphRenderer - Enhanced Features', () => {
    ...
    it('should render all body parts in the graph', async () => {
      ...
      const createMockEntity = (id, name, type, parentId = null) => ({
        getComponentData: jest.fn((component) => {
          ...
          if (component === 'core:description') return { text: `${name} description` };
      ...
      mockEntityManager.getEntityInstance.mockImplementation((id) => {
        const entities = {
          ...
          'left-arm-id': createMockEntity('left-arm-id', 'Left Arm', 'arm', 'torso-id'),
          'right-arm-id': createMockEntity('right-arm-id', 'Right Arm', 'arm', 'torso-id'),
          'left-hand-id': createMockEntity('left-hand-id', 'Left Hand', 'hand', 'left-arm-id'),
          'right-hand-id': createMockEntity('right-hand-id', 'Right Hand', 'hand', 'right-arm-id'),
          'left-leg-id': createMockEntity('left-leg-id', 'Left Leg', 'leg', 'torso-id'),
          'right-leg-id': createMockEntity('right-leg-id', 'Right Leg', 'leg', 'torso-id'),
          'left-foot-id': createMockEntity('left-foot-id', 'Left Foot', 'foot', 'left-leg-id'),
          'right-foot-id': createMockEntity('right-foot-id', 'Right Foot', 'foot', 'right-leg-id')
        };
  ...
  describe('Curved Edge Rendering', () => {
    it('should render curved paths instead of straight lines', async () => {
      ...
      expect(pathElement.setAttribute).toHaveBeenCalledWith('stroke-width', '2');
      expect(pathElement.setAttribute).toHaveBeenCalledWith('fill', 'none');
      expect(pathElement.setAttribute).toHaveBeenCalledWith('stroke-opacity', '0.6');
    });
  });
  ...
  describe('Pan and Zoom Functionality', () => {
    ...
    it('should prevent panning when interacting with nodes', async () => {
      ...
      const mousedownCall = node.addEventListener.mock.calls
  ...
  describe('Dynamic ViewBox Adjustment', () => {
    it('should adjust viewBox to fit all content', async () => {
      ...
      expect(renderer._viewBox.x + renderer._viewBox.width).toBeGreaterThan(maxX);
      expect(renderer._viewBox.y + renderer._viewBox.height).toBeGreaterThan(maxY);
    });
  });
          ...
          return Promise.resolve({
            getComponentData: jest.fn((component) => {
              ...
              if (component === 'anatomy:joint' && index > 0) {
                ...
                return { parentId: `part-${parentIndex}-id`, socketId: 'socket' };
              }
              return null;
            })
          });
