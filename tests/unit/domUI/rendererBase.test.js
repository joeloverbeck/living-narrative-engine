...
/**
 * @file Unit tests for the RendererBase abstract class.
...
import { RendererBase } from '../../../src/domUI'; // Assumes index exports RendererBase
...
const createRealDocContext = () => {
  if (typeof document === 'undefined') {
    throw new Error(
      "JSDOM 'document' is not available. Ensure environment is 'jsdom'."
    );
  }
...
describe('RendererBase', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should throw an error if logger is missing or invalid', () => {
      ...
      expect(
        () => new ConcreteRenderer({ ...validDeps, logger: null })
      ).toThrow('ConcreteRenderer: Logger dependency is missing or invalid.');
      expect(
        () => new ConcreteRenderer({ ...validDeps, logger: undefined })
      ).toThrow('ConcreteRenderer: Logger dependency is missing or invalid.');
      // Test case for invalid type (missing debug function)
      expect(
        () =>
          new ConcreteRenderer({ ...validDeps, logger: { info: jest.fn() } })
      ).toThrow('ConcreteRenderer: Logger dependency is missing or invalid.');
    });
    ...
    it('should throw an error if ValidatedEventDispatcher is missing or invalid', () => {
      ...
      expect(
        () =>
          new ConcreteRenderer({ ...validDeps, validatedEventDispatcher: null })
      ).toThrow(
      ...
      expect(
        () =>
          new ConcreteRenderer({
            ...validDeps,
            validatedEventDispatcher: { someOtherFunc: jest.fn() },
          })
      ).toThrow(
        'ConcreteRenderer: ValidatedEventDispatcher dependency is missing or invalid.'
      );
    ...
    it('should throw an error if DocumentContext is missing or invalid', () => {
      ...
      // --- FIX: Test should now correctly isolate this failure ---
      expect(
        () => new ConcreteRenderer({ ...validDeps, documentContext: null })
      ).toThrow(
        'ConcreteRenderer: DocumentContext dependency is missing or invalid.'
      );
      expect(
        () => new ConcreteRenderer({ ...validDeps, documentContext: undefined })
      ).toThrow(
        'ConcreteRenderer: DocumentContext dependency is missing or invalid.'
      );
      ...
      expect(
        () =>
          new ConcreteRenderer({
            ...validDeps,
            documentContext: { create: jest.fn() },
          })
      ).toThrow(
        'ConcreteRenderer: DocumentContext dependency is missing or invalid.'
      );
    ...
    it("should throw an error if 'RendererBase' is instantiated directly", () => {
      ...
      expect(() => new RendererBase(validDeps)).toThrow(
  ...
  describe('Protected Property Access', () => {
    // Renamed from Getters as they are direct properties now
  ...
  describe('_logPrefix', () => {
    it('should generate the correct log prefix based on the concrete class name', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        '[ConcreteRenderer] Rendering something...'
      );
