...
describe('ActionCategorizationService', () => {
  ...
  describe('Performance Monitoring', () => {
    it('should verify performance monitoring wrapper exists', () => {
      // The performance monitoring wrapper is a private method that can't be directly tested
      // However, we can verify that the service has methods that would use it

      // Create a service with a large number of actions to test performance
      ...
      // Note: In a real slow operation scenario (>5ms), the private #withPerformanceMonitoring
      // method would log a warning. Since we can't directly test private methods,
      // we've verified the service structure and that operations complete successfully.
    });
  });
});
