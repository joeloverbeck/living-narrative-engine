...
describe('GracefulDegradation', () => {
  ...
  it('uses cached data when available and skips files otherwise', () => {
    ...
    const keyedResult = instance.applyDegradation(
      new Error('ACCESS: cache key'),
      {
        cacheKey: 'manual-key',
        modId: 'mod-a',
    ...
    const compositeResult = instance.applyDegradation(
      new Error('ACCESS: composite cache'),
      {
        modId: 'mod-a',
        componentId: 'component-a',
        hasCache: true,
    ...
    const skipResult = instance.applyDegradation(
  ...
  it('supports partial extraction and basic parsing for corruption errors', () => {
    const partial = instance.applyDegradation(
      new Error('corruption detected'),
      {
    ...
    const minimal = instance.applyDegradation(
      new Error('corruption minimal data'),
      {
    ...
    const empty = instance.applyDegradation(
      new Error('corruption without data'),
      {
  ...
  it('performs reduced validation on timeout and blocks security violations', () => {
    const timeoutResult = instance.applyDegradation(
    ...
    const securityResult = instance.applyDegradation(
  ...
  it('honors contextual hints and default fallbacks', () => {
    const allowSkipResult = instance.applyDegradation(
    ...
    const defaultComponent = instance.applyDegradation(
      new Error('mysterious issue'),
      {
    ...
    const unknownDefault = instance.applyDegradation(
  ...
  it('respects contexts that explicitly forbid degradation attempts', () => {
    const result = instance.applyDegradation(
      new Error('no fallback permitted'),
      {
  ...
  it('provides meaningful defaults for different contexts', () => {
    expect(instance.getDefaultValue('mod', { id: 'special' })).toEqual({
      sentinel: 'mod-default',
    });
    ...
    expect(
      instance.getDefaultValue('component', { componentId: 'arm' })
    ).toEqual({
      id: 'arm',
      data: {},
      partial: true,
  ...
  it('derives error types from codes and message content', () => {
    const timeout = instance.applyDegradation(
      { message: 'no code', code: 'TIMEOUT' },
      {
    ...
    const messageDerived = instance.applyDegradation(
      new Error('File corruption occurred'),
      {
    ...
    expect(messageDerived.strategy).toBe(
  ...
  it('executes built-in fallback strategies when custom handlers are absent', () => {
    ...
    const accessSkipResult = bareInstance.applyDegradation(
      new Error('ENOENT: file missing'),
      {
    ...
    const accessCacheResult = bareInstance.applyDegradation(
      new Error('ENOENT: cached file missing'),
      {
    ...
    expect(bareLogger.debug).toHaveBeenCalledWith(
      'Cache hit for mods/bare-cache.json'
    );

    const partialResult = bareInstance.applyDegradation(
      new Error('corruption detected'),
      {
    ...
    const basicResult = bareInstance.applyDegradation(
      new Error('corruption again'),
      {
    ...
    const reducedResult = bareInstance.applyDegradation(
  ...
  it('falls back to default degradation when no hints or handlers are available', () => {
    ...
    const result = bareInstance.applyDegradation(
      new Error('mysterious failure'),
      {
        filePath: 'mods/unknown.json',
        modId: 'mod-bare',
        type: 'validation',
