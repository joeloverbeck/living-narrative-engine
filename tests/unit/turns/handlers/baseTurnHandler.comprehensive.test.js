...
describe('BaseTurnHandler Comprehensive Coverage Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockTurnStateFactory.createProcessingCommandState.mockImplementation((h, cmd, action, resolver) => {
      const state = {
        enterState: jest.fn().mockResolvedValue(undefined),
        exitState: jest.fn().mockResolvedValue(undefined),
    ...
    mockTurnStateFactory.createAwaitingExternalTurnEndState.mockImplementation((h) => {
      const state = {
        enterState: jest.fn().mockResolvedValue(undefined),
        exitState: jest.fn().mockResolvedValue(undefined),
  ...
  describe('getCurrentActor() method', () => {
    ...
    it('should fallback to _currentActor when TurnContext throws error', () => {
      ...
      const context = { 
        getActor: jest.fn().mockImplementation(() => { throw new Error('Context error'); })
      };
  ...
  describe('_setCurrentActorInternal() method', () => {
    it('should warn when setting different actor while TurnContext exists', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining("Handler's actor set to 'new-actor' while an active TurnContext exists for 'context-actor'")
      );
    });
  ...
  describe('_setCurrentTurnContextInternal() method', () => {
    it('should align _currentActor with TurnContext actor when different', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining("Aligning _currentActor ('old-actor') with new TurnContext actor ('context-actor')")
      );
    });
  ...
  describe('_transitionToState() method', () => {
    it('should throw error for invalid state', async () => {
      ...
      await expect(handler._exposeTransitionToState(invalidState)).rejects.toThrow(
    ...
    it('should handle exitState error gracefully', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Error during OldState.exitState or onExitState hook'),
        expect.any(Error)
      );
    ...
    it('should handle critical failure when forced idle transition fails', async () => {
      ...
      const failingIdleState = {
        ...mockState,
        enterState: jest.fn().mockRejectedValue(new Error('Idle transition failed')),
  ...
  describe('_handleTurnEnd() method', () => {
    it('should warn when endedActorId differs from contextActorId', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining("called for actor 'different-actor', but TurnContext is for 'context-actor'")
      );
    });
    ...
    it('should warn when endedActorId differs from handlerActorId when no context', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining("called for actor 'different-actor', no active TurnContext, but handler's _currentActor is 'handler-actor'")
      );
    });
    ...
    it('should warn when called with error while already in ending state', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining("called for test-actor with error 'Test error', but already in TurnEndingState")
      );
    });
  ...
  describe('_resetTurnStateAndResources() method', () => {
    it('should handle cancelActivePrompt error gracefully', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Error during cancelActivePrompt: Cancel error'),
        expect.any(Error)
      );
    });
  ...
  describe('destroy() method', () => {
    it('should handle cancelActivePrompt error during destroy', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Error during cancelActivePrompt: Cancel error during destroy'),
        expect.any(Error)
      );
    });
    ...
    it('should handle state destroy error gracefully', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Error during TestState.destroy(): State destroy error'),
        expect.any(Error)
      );
    });
  ...
  describe('Public transition request methods', () => {
    ...
    it('should request awaiting input state transition', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Received request to transition to AwaitingInput state')
      );
    ...
    it('should request processing command state transition', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Received request to transition to ProcessingCommand state')
      );
    ...
    it('should request awaiting external turn end state transition', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Received request to transition to AwaitingExternalTurnEnd state')
      );
  ...
  describe('Abstract method implementations', () => {
    it('should throw error for unimplemented getTurnEndPort', () => {
      ...
      expect(() => baseHandler.getTurnEndPort()).toThrow('Method not implemented');
      expect(mockLogger.error).toHaveBeenCalledWith(
  ...
  describe('Hook methods', () => {
    it('should call onEnterState hook', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('onEnterState hook: Entering CurrentState from MockState')
      );
    });
    ...
    it('should call onExitState hook', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('onExitState hook: Exiting CurrentState to NextState')
      );
    });
    ...
    it('should handle null previous state in onEnterState', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('onEnterState hook: Entering CurrentState from None')
      );
    });
    ...
    it('should handle null next state in onExitState', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('onExitState hook: Exiting CurrentState to None')
      );
    });
  });
