// src/tests/turns/order/turnOrderService.addEntity.roundRobin.test.js
...
describe('TurnOrderService', () => {
  ...
  describe("addEntity ('round-robin' active)", () => {
    ...
    it('Test Case 11.9.1: should delegate adding an entity to SimpleRoundRobinQueue when a round-robin round is active', () => {
      ...
      expect(mockSimpleQueueInstance.add).toHaveBeenCalledTimes(
        initialEntities.length
      );
      expect(mockSimpleQueueInstance.add).toHaveBeenNthCalledWith(
        1,
        initialEntities[0]
      );
      expect(mockSimpleQueueInstance.add).toHaveBeenNthCalledWith(
      ...
      expect(mockSimpleQueueInstance.add).not.toHaveBeenCalledWith(
        expect.anything(),
        expect.anything()
      );
