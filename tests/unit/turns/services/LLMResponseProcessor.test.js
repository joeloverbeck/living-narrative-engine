...
/**
 * Creates a minimal valid response with all required fields for V5 schema.
 * Note: V5 schema does NOT include moodUpdate/sexualUpdate - those are
 * handled by MoodResponseProcessor in Phase 1.
 *
...
function createValidPayload(overrides = {}) {
  return {
    ...
    cognitive_ledger: {
      settled_conclusions: ['Test conclusion'],
  ...
  describe('processResponse', () => {
    ...
    describe('Valid Inputs', () => {
      ...
      test('extracts cognitive_ledger from valid response', async () => {
        ...
        expect(result.extractedData.cognitiveLedger).toEqual({
          settled_conclusions: ['Test conclusion'],
          open_questions: ['Test question?'],
        });
      });

      test('logs INFO for cognitive_ledger extraction when present', async () => {
        const payload = createValidPayload();
        const llmResponse = JSON.stringify(payload);
        schemaValidatorMock.validate.mockReturnValue({
          isValid: true,
        ...
        expect(logger.info).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            settled_conclusions_count: 1,
            open_questions_count: 1,
          })
        );
      });

      test('logs INFO when cognitive_ledger is missing', async () => {
        const payload = {
          chosenIndex: 1,
          speech: 'Hello',
