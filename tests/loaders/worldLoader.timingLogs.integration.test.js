...
describe('WorldLoader Integration Test Suite - Performance Timing Logs (Sub-Ticket 9)', () => {
    ...
    beforeEach(() => {
        ...
        mockRegistry = {
            ...
            getEntityDefinition: jest.fn(), getItemDefinition: jest.fn(), getLocationDefinition: jest.fn(),
            getConnectionDefinition: jest.fn(), getBlockerDefinition: jest.fn(), getActionDefinition: jest.fn(),
            getEventDefinition: jest.fn(), getComponentDefinition: jest.fn(), getAllEntityDefinitions: jest.fn(() => []),
            getAllItemDefinitions: jest.fn(() => []), getAllLocationDefinitions: jest.fn(() => []),
            getAllConnectionDefinitions: jest.fn(() => []), getAllBlockerDefinitions: jest.fn(() => []),
            getAllActionDefinitions: jest.fn(() => []), getAllEventDefinitions: jest.fn(() => []),
            getAllComponentDefinitions: jest.fn(() => []), getStartingPlayerId: jest.fn(() => null),
        ...
        mockEntityLoader = { loadItemsForMod: jest.fn().mockResolvedValue(mockLoadResult) };
    ...
    it('should log per-mod performance timing information at INFO level', async () => {
        ...
        for (const modId of finalOrder) {
            ...
            const expectedLogRegex = new RegExp(`^Mod '${modId}' loaded in (\\d+\\.\\d{2})ms:`); // Regex to capture duration

            // Find the specific log message for this mod
            const timingLogCall = infoCalls.find(call => expectedLogRegex.test(call[0]));
            ...
            if (timingLogCall) {
                ...
                if (match && match[1]) {
                    ...
                    throw new Error(`Could not extract duration from log message for mod '${modId}': ${timingLogCall[0]}`);
                }
            }
        ...
        expect(mockEntityLoader.loadItemsForMod).toHaveBeenCalledWith(fooModId, mockFooManifest, 'items', 'items', 'items');

        // Ensure the final summary block is also logged
        const summaryLogExists = infoCalls.some(call => call[0].includes(`WorldLoader Load Summary (World: '${worldName}')`));
