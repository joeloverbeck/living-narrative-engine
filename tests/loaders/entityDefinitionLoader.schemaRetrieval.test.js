...
describe('EntityDefinitionLoader Schema Retrieval', () => {
  ...
  test('should correctly retrieve and set _primarySchemaId when config provides it for "entityDefinitions"', () => {
    ...
    expect(mockConfig.getContentTypeSchemaId).toHaveBeenCalledWith(EXPECTED_CONTENT_TYPE_KEY);
    expect(mockLogger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining('Primary schema ID for content type \\\'entityDefinitions\\\' not found')
    );
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining('Primary schema ID for content type \'' + EXPECTED_CONTENT_TYPE_KEY + '\' found: \'' + ENTITY_DEFINITION_SCHEMA_ID + '\'')
    );
  });
  ...
  test('should set _primarySchemaId to null and log a warning if config does not provide it for "entityDefinitions"', () => {
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(
      "EntityDefinitionLoader: Primary schema ID for content type '" + EXPECTED_CONTENT_TYPE_KEY + "' not found in configuration. Primary validation might be skipped."
    );
  });
