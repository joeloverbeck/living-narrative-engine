  ...
  it('should correctly load mods with partial or empty content definitions', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /WorldLoader \[modA\]: Skipping content type 'events' \(key: 'events'\)/
      )
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /WorldLoader \[modB\]: Skipping content type 'events' \(key: 'events'\)/
      )
    );
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /WorldLoader \[core\]: Skipping content type 'events' \(key: 'events'\)/
      )
    );
    ...
    expect(summaryLines.some((line) => /entities\s+: C:/.test(line))).toBe(
      false
    );
    expect(summaryLines.some((line) => /blockers\s+: C:/.test(line))).toBe(
      false
    );
