...
describe('ModManifestLoader Error Handling', () => {
  ...
  beforeEach(() => {
    ...
    mockPathResolver = {
      resolveModManifestPath: jest.fn((modId) => `path/to/${modId}/mod.manifest.json`),
    };
    mockDataFetcher = {
      fetch: jest.fn(),
    };
  ...
  test('should throw if manifest ID does not match expected mod ID', async () => {
    ...
    const manifestWithWrongId = { id: 'modY', name: 'Mod X with wrong ID', version: '1.0.0' };
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
        ...
        expect.stringContaining("manifest ID 'modY' does not match expected mod ID 'modX'"),
  ...
  test('should throw if manifest is missing an id field', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
        ...
        expect.stringContaining("manifest 'path/to/modZ/mod.manifest.json' is missing an 'id' field."),
