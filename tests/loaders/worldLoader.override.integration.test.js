...
describe('WorldLoader Integration Test Suite - Overrides (TEST-LOADER-7.2)', () => {
  ...
  beforeEach(() => {
    ...
    mockActionLoader.loadItemsForMod.mockImplementation(
      async (
        ...
        if (modIdArg === CORE_MOD_ID) {
          ...
          mockLogger.debug(
            `Mock ActionLoader: Stored override for ${overrideItemId} by ${modIdArg}`,
            overrideItemData
          );
          ...
          mockLogger.debug(
            `Mock ActionLoader: Stored ${newItemId} for ${modIdArg}`,
            newItemData
          );
    ...
    mockComponentLoader.loadItemsForMod.mockImplementation(
      async (
        ...
        if (modIdArg === overrideModId) {
          ...
          mockLogger.debug(
            `Mock ComponentLoader: Stored ${itemId} for ${modIdArg}`,
            itemData
          );
    ...
    mockRuleLoader.loadItemsForMod.mockResolvedValue({
      count: 0,
      overrides: 0,
      errors: 0,
    });
  ...
  it('should load core and override mod, applying overrides correctly', async () => {
    ...
    const newComponent = mockRegistry.get(
      'components',
    ...
    // Actions: C:3, O:1, E:0
    expect(
      summaryLines.some((line) => /actions\s+: C:3, O:1, E:0/.test(line))
    ).toBe(true);
    // Components: C:1, O:0, E:0
    expect(
      summaryLines.some((line) => /components\s+: C:1, O:0, E:0/.test(line))
    ).toBe(true);

    // Grand totals: C:4, O:1, E:0 - Use regex for flexibility with padding
    expect(
      summaryLines.some((line) => /TOTAL\s+: C:4, O:1, E:0/.test(line))
    ).toBe(true);

    // Ensure types NOT processed aren't in summary counts > 0 (regex allows for C:0)
    expect(summaryLines.some((line) => /events\s+: C:[1-9]/.test(line))).toBe(
      false
    );
    expect(summaryLines.some((line) => /rules\s+: C:[1-9]/.test(line))).toBe(
      false
    );
    expect(summaryLines.some((line) => /items\s+: C:[1-9]/.test(line))).toBe(
      false
    );
