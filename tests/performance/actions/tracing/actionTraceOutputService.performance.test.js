...
describe('ActionTraceOutputService Performance', () => {
  ...
  // Circuit Breaker Performance Tests - COMMENTED OUT
  // These tests are for unimplemented circuit breaker functionality
  // The production ActionTraceOutputService has basic error handling but no circuit breaker pattern
  // TODO: Implement circuit breaker feature or remove these tests entirely
  /*
  describe('Circuit Breaker Performance', () => {
    it('should quickly detect and respond to failures', async () => {
      service = new ActionTraceOutputService({
        storageAdapter: mockStorageAdapter,
        logger: mockLogger,
        queueConfig: {
      ...
      const circuitBreakerTime = Date.now() - startTime;

      // Circuit breaker should activate quickly
      expect(circuitBreakerTime).toBeLessThan(500);

      // Should stop attempting writes after threshold
      expect(mockStorageAdapter.setItem).toHaveBeenCalledTimes(10);
    });

    it('should recover efficiently after circuit breaker opens', async () => {
      service = new ActionTraceOutputService({
        storageAdapter: mockStorageAdapter,
        logger: mockLogger,
        queueConfig: {
      ...
      const failingTraces = Array.from({ length: 5 }, (_, i) =>
        createMockTrace(`core:fail${i}`)
      );
      ...
      const successTrace = createMockTrace('core:success');
      await service.writeTrace(successTrace);

      // Wait for processing
      await new Promise((resolve) => setTimeout(resolve, 100));

      // Should recover and process new traces
