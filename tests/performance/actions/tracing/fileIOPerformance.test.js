...
describe('File I/O Performance Tests', () => {
  ...
  describe('Single File Write Performance', () => {
    it('should write dual-format traces within acceptable time limits', async () => {
      ...
      console.log(`File I/O performance: avg=${avgTime.toFixed(2)}ms, p95=${p95Time.toFixed(2)}ms`);
    ...
    it('should show consistent file write performance across multiple runs', async () => {
      ...
      for (let run = 0; run < runs; run++) {
        ...
        for (let i = 0; i < iterationsPerRun; i++) {
          ...
          // Write both formats
          await fileHandler.writeTrace(
            JSON.stringify(traceData, null, 2),
            { ...trace, actionId: `consistency_${run}_${i}_json` }
          );
          
          await fileHandler.writeTrace(
            JSON.stringify(traceData, null, 2),
            { ...trace, actionId: `consistency_${run}_${i}_text`, _outputFormat: 'text' }
          );
  ...
  describe('Batch Operations Performance', () => {
    it('should benefit from batch operations when available', async () => {
      ...
      if (typeof fileHandler.writeBatch !== 'function') {
        console.log('Batch operations not supported by FileTraceOutputHandler, skipping batch tests');
        return;
      }
      ...
      if (!serverAvailable) {
        console.log('LLM proxy server not available - batch operations will use fallback mode');
      } else {
        console.log('LLM proxy server available - testing full batch functionality');
      }
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const traceBatch = [
          ...
          { 
            content: JSON.stringify(traceData, null, 2), 
            originalTrace: { ...trace, actionId: `batch_${i}_text`, _outputFormat: 'text' } 
          },
        ];
      ...
      console.log(`Individual operations average: ${avgIndividualTime.toFixed(2)}ms`);
      ...
      if (batchTimes.length > 0) {
        ...
        console.log('Batch operations failed, testing individual performance only');
    ...
    it('should handle large batch operations efficiently', async () => {
      if (typeof fileHandler.writeBatch !== 'function') {
        console.log('Batch operations not supported, skipping large batch test');
        return;
      }
      ...
      try {
        ...
        console.log(`Server availability check failed for large batch test: ${error.message}`);
        serverAvailable = false;
      }

      console.log(`Starting large batch test with server ${serverAvailable ? 'available' : 'unavailable'}`);
      ...
      for (const batchSize of batchSizes) {
        ...
        const traceBatch = traces.flatMap((trace, i) => {
          ...
          return [
            { 
              content: JSON.stringify(traceData, null, 2), 
              originalTrace: { ...trace, actionId: `large_${batchSize}_${i}_json` } 
            },
            { 
              content: JSON.stringify(traceData, null, 2), 
              originalTrace: { ...trace, actionId: `large_${batchSize}_${i}_text`, _outputFormat: 'text' } 
            },
          ];
        });
        ...
        try {
          ...
          if (error.message.includes('timeout')) {
            timedOut = true;
            console.log(`Batch size ${traceBatch.length} timed out after 30s - may indicate server overload`);
            result = false;
          } else {
            console.log(`Batch size ${traceBatch.length} failed: ${error.message}`);
            result = false;
          }
        }
        ...
        console.log(`Batch size ${traceBatch.length}: ${batchTime.toFixed(2)}ms total, ${timePerItem.toFixed(2)}ms per item${timedOut ? ' (TIMED OUT)' : ''}${result ? ' ✓' : ' ✗'}`);
      }
      ...
      console.log('Batch scaling efficiency:', scalingEfficiency.map(e => e.toFixed(2)).join(', '));
      ...
      if (batchResults.length > 0 && batchResults.some(r => r.success)) {
        ...
        console.log(`Batch test summary: ${successfulBatches.length} successful, ${failedBatches.length} failed, ${timedOutBatches.length} timed out`);
        ...
        if (serverAvailable) {
          ...
          expect(successfulBatches.length + timedOutBatches.length).toBeGreaterThan(0);
        }
        
        // Successful batches should be reasonably fast - increased tolerance for I/O variability
        if (successfulBatches.length > 0) {
          const avgTimePerItem = successfulBatches.reduce((sum, r) => sum + r.timePerItem, 0) / successfulBatches.length;
          console.log(`Average time per item in successful batches: ${avgTimePerItem.toFixed(2)}ms`);
        ...
        console.log('No successful batch results available - method may not be implemented or all operations failed');
      }
      ...
      if (scalingEfficiency.length > 0) {
        ...
        if (filteredEfficiency.length > 0) {
          const avgEfficiency = filteredEfficiency.reduce((sum, e) => sum + e, 0) / filteredEfficiency.length;
          console.log(`Scaling efficiency analysis: avg=${avgEfficiency.toFixed(3)}, count=${filteredEfficiency.length}/${scalingEfficiency.length}`);
          
          // Use average efficiency for more stable testing, allow for I/O variability
          expect(avgEfficiency).toBeGreaterThan(0.2);
        } else {
          console.log('All scaling efficiency measurements were extreme outliers - may indicate system load issues');
  ...
  describe('Concurrent File I/O Performance', () => {
    it('should handle concurrent file writes without blocking', async () => {
      ...
      console.log(`  Successful: ${successfulWrites} (${successRate.toFixed(1)}%)`);
      console.log(`  Failed: ${failedWrites}`);
      console.log(`  Total time: ${totalTime.toFixed(2)}ms`);
      console.log(`  Average time per operation: ${(totalTime / writePromises.length).toFixed(2)}ms`);
      ...
      if (successRate > 85) {
        ...
        console.log('Lower success rate detected - may indicate server or system load issues');
    ...
    it('should maintain performance under I/O pressure', async () => {
      ...
      for (let round = 0; round < rounds; round++) {
        ...
        for (let i = 0; i < traces.length; i++) {
          ...
          writePromises.push(
            fileHandler.writeTrace(
              JSON.stringify(traceData, null, 2),
              { ...trace, actionId: `pressure_${round}_${i}_json` }
            ),
            fileHandler.writeTrace(
              JSON.stringify(traceData, null, 2),
              { ...trace, actionId: `pressure_${round}_${i}_text`, _outputFormat: 'text' }
            )
          );
        }
        ...
        console.log(`Round ${round + 1}: ${roundTime.toFixed(2)}ms, ${successRate.toFixed(1)}% success`);
      ...
      const avgTime = roundResults.reduce((sum, r) => sum + r.time, 0) / roundResults.length;
      const avgSuccessRate = roundResults.reduce((sum, r) => sum + r.successRate, 0) / roundResults.length;
      const timeVariance = Math.sqrt(roundResults.reduce((acc, r) => acc + Math.pow(r.time - avgTime, 2), 0) / roundResults.length);
      ...
      if (avgSuccessRate > 80) {
        ...
        console.log('Degraded performance detected under I/O pressure - adjusting expectations');
