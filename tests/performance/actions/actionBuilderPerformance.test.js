...
describe('ActionDefinitionBuilder Performance', () => {
  describe('creation performance', () => {
    it('should create definitions quickly (target: <0.1ms per action)', () => {
      ...
      console.log(`Total time for ${iterations} actions: ${(endTime - startTime).toFixed(2)}ms`);
    });
    ...
    it('should handle bulk creation efficiently (target: <100ms for 1000 actions)', () => {
      ...
      console.log(`Bulk creation time: ${duration.toFixed(2)}ms for 1000 actions`);
    });
    ...
    it('should scale linearly with complexity', () => {
      ...
      testCases.forEach(({ size, name }) => {
        ...
        console.log(`${name}: ${duration.toFixed(2)}ms total, ${avgTime.toFixed(4)}ms per action`);
      });
    ...
    it('should handle complex configurations efficiently', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        new ActionDefinitionBuilder(`test:complex${i}`)
          ...
          .asTargetedAction('test:targets', `perform complex operation ${i} on {target}`)
          .requiresComponents([
            'test:comp1', 'test:comp2', 'test:comp3', 'test:comp4', 'test:comp5'
          ])
          .withPrerequisites([
            'test:cond1',
      ...
      console.log(`Complex action average time: ${avgTime.toFixed(4)}ms per action`);
    });

    it('should handle large component arrays efficiently', () => {
      const componentCount = 100;
      const components = Array.from({ length: componentCount }, (_, i) => `test:comp${i}`);
      ...
      console.log(`Large components average time: ${avgTime.toFixed(4)}ms per action`);
    });

    it('should handle large prerequisite arrays efficiently', () => {
      const prerequisiteCount = 50;
      const prerequisites = Array.from({ length: prerequisiteCount }, (_, i) => `test:cond${i}`);
      ...
      console.log(`Large prerequisites average time: ${avgTime.toFixed(4)}ms per action`);
    });
  });
  ...
  describe('memory usage', () => {
    it('should have minimal memory overhead (target: <2KB per action)', () => {
      if (typeof process !== 'undefined' && process.memoryUsage) {
        ...
        console.log(`Memory usage: ${memoryPerAction.toFixed(0)} bytes per action`);
        console.log(`Total memory increase: ${((finalMemory - initialMemory) / 1024).toFixed(2)} KB`);
      } else {
    ...
    it('should not leak memory during repeated operations', () => {
      if (typeof process !== 'undefined' && process.memoryUsage) {
        ...
        console.log('Memory snapshots:', memorySnapshots.map(m => `${(m/1024).toFixed(2)}KB`));
  ...
  describe('validation performance', () => {
    it('should validate efficiently', () => {
      ...
      console.log(`Validation average time: ${avgTime.toFixed(6)}ms per validation`);
    });
    ...
    it('should build efficiently after validation', () => {
      ...
      console.log(`Validate + build average time: ${avgTime.toFixed(4)}ms per operation`);
    });
  });
  ...
  describe('fromDefinition performance', () => {
    it('should recreate builders efficiently', () => {
      ...
      console.log(`fromDefinition average time: ${avgTime.toFixed(4)}ms per recreation`);
    });
  });
  ...
  describe('regression testing', () => {
    it('should maintain performance baselines', () => {
      ...
      console.log(`Simple creation: ${simpleTime.toFixed(4)}ms (limit: ${performanceBaselines.simpleCreation}ms)`);
      console.log(`Complex creation: ${complexTime.toFixed(4)}ms (limit: ${performanceBaselines.complexCreation}ms)`);
    });
  });
});