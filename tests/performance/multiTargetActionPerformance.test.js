...
describe('Multi-Target Action Performance Tests', () => {
  ...
  describe('Schema Validation Performance', () => {
    it('should validate legacy events within performance targets', () => {
      ...
      const benchmark = performanceTracker.startBenchmark('Legacy Event Validation', { trackMemory: true });
      ...
      console.log(`Legacy validation performance: ${result.totalTime.toFixed(2)}ms for 1000 iterations`);
      if (result.memoryUsage) {
        console.log(`Memory usage: ${(result.memoryUsage.peak / 1024).toFixed(2)}KB peak`);
      }
    });
    ...
    it('should validate multi-target events within performance targets', () => {
      ...
      const benchmark = performanceTracker.startBenchmark('Multi-Target Event Validation', { trackMemory: true });
      ...
      console.log(`Multi-target validation performance: ${result.totalTime.toFixed(2)}ms for 1000 iterations`);
    });
    ...
    it('should handle complex multi-target events efficiently', () => {
      ...
      const benchmark = performanceTracker.startBenchmark('Complex Multi-Target Event Validation', { trackMemory: true });
      ...
      console.log(`Complex multi-target validation: ${averageTime.toFixed(2)}ms average (${iterations} iterations)`);
    });
  });
  ...
  describe('Business Rule Validation Performance', () => {
    it('should perform business rule validation within targets', () => {
      ...
      const benchmark = performanceTracker.startBenchmark('Business Rule Validation', { trackMemory: true });
      ...
      console.log(`Business rule validation performance: ${result.totalTime.toFixed(2)}ms for 1000 iterations`);
    });
    ...
    it('should handle complex business rules efficiently', () => {
      ...
      const timerId = performanceMonitor.startTimer('complex-business-validation');
      ...
      console.log(`Complex business rule validation: ${averageTime.toFixed(2)}ms average`);
    });
  });
  ...
  describe('Memory Usage Tests', () => {
    ...
    it('should handle large event batches efficiently', () => {
      ...
      const benchmark = performanceTracker.startBenchmark('Batch Event Validation (100 events)', { trackMemory: true });
      ...
      const memoryPerBatch = (memoryUsage.peak - memoryUsage.initial) / iterations;
      console.log(`Memory per batch: ${(Math.abs(memoryPerBatch) / 1024).toFixed(2)}KB`);
      
      // Basic memory sanity check
      expect(typeof memoryUsage).toBe('object');

      console.log(`Batch validation: ${averageTime.toFixed(2)}ms average per 100-event batch`);
    });
  });
  ...
  describe('Regression Tests', () => {
    it('should maintain performance parity with legacy validation', () => {
      ...
      const legacyBenchmark = performanceTracker.startBenchmark('Legacy Performance Test');
      ...
      const enhancedBenchmark = performanceTracker.startBenchmark('Enhanced Schema Performance Test');
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const targetCount = hasTargets ? Object.keys(legacyEvent.targets).length : 1;
      ...
      console.log(`Legacy mean: ${legacyMeanTime.toFixed(2)}ms`);
      console.log(`Enhanced mean: ${enhancedMeanTime.toFixed(2)}ms`);
    });
    ...
    it('should show reasonable performance scaling with target count', () => {
      ...
      for (const targetCount of targetCounts) {
        const event = targetCount === 1 
          ? createValidLegacyEvent()
          : createComplexMultiTargetEvent(targetCount);

        mockValidator.validate.mockClear();
        const benchmark = performanceTracker.startBenchmark(`${targetCount} targets`);
  ...
  describe('Stress Tests', () => {
    it('should handle sustained load without degradation', async () => {
      ...
      console.log(`Average operation time: ${(timings.reduce((a, b) => a + b, 0) / timings.length).toFixed(2)}ms`);
    ...
    it('should handle concurrent validation requests', async () => {
      ...
      console.log(`Concurrent validation: ${concurrentRequests} requests in ${totalTime.toFixed(2)}ms`);
  ...
  describe('Performance Monitoring Integration', () => {
    it('should integrate with PerformanceMonitor for continuous tracking', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const timerId = performanceMonitor.startTimer('multi-target-validation', `operation_${i}`);
      ...
      console.log(`PerformanceMonitor integration: ${metrics.averageOperationTime.toFixed(2)}ms average`);
    });
  });
});