...
describe('Debug Scope Resolution - Performance Test', () => {
  ...
  it('should demonstrate entity creation and component access patterns', async () => {
    ...
    for (const entity of furnitureEntities) {
      ...
      console.log(`    All components: [${entityManager.getAllComponentTypesForEntity(entity.id).join(', ')}]`);
    }
    console.log();

    // Debug: Check actor
    console.log('👤 Checking actor components...');
    const actorComponents = entityManager.getAllComponentTypesForEntity(actorId);
    console.log(`Actor ${actorId} has components: [${actorComponents.join(', ')}]`);
    ...
    const actorLocation = entityManager.getComponentData(actorId, POSITION_COMPONENT_ID)?.locationId;
    console.log(`Actor location: ${actorLocation}`);
    
    const filteredFurniture = furnitureEntities.filter(entity => {
      const furnitureLocation = entityManager.getComponentData(entity.id, POSITION_COMPONENT_ID)?.locationId;
      const allowsSitting = entityManager.getComponentData(entity.id, 'positioning:allows_sitting');
      ...
      const hasAvailableSpots = allowsSitting.spots.some(spot => spot === null);
    ...
    if (filteredFurniture.length > 0) {
      console.log(`Filtered furniture IDs: [${filteredFurniture.map(e => e.id).join(', ')}]`);
    }
  ...
  it('should demonstrate component state tracing differences', async () => {
    ...
    if (benchEntity) {
      console.log(`  Direct access to components:`, Object.keys(benchEntity.components));
      console.log(`  Component type IDs:`, benchEntity.componentTypeIds);
      console.log(`  getAllComponents():`, Object.keys(benchEntity.getAllComponents()));
      
      // Check each component individually
      const allComponentTypes = entityManager.getAllComponentTypesForEntity(parkBenchId);
      for (const componentType of allComponentTypes) {
        const dataViaManager = entityManager.getComponentData(parkBenchId, componentType);
        const dataViaEntity = benchEntity.getComponentData(componentType);
        const hasViaManager = entityManager.hasComponent(parkBenchId, componentType);
        ...
        console.log(`    Data match: ${JSON.stringify(dataViaManager) === JSON.stringify(dataViaEntity)}`);
      }
    }
    ...
    if (actorEntity) {
      console.log(`  Direct access to components:`, Object.keys(actorEntity.components));
