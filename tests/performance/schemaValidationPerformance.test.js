...
describe('Schema Validation Performance Tests', () => {
  ...
  beforeAll(async () => {
    ...
    isCI = !!(process.env.CI || process.env.GITHUB_ACTIONS || process.env.JENKINS_URL);
    ...
    const warmupData = { $schema: 'schema://living-narrative-engine/action.schema.json', id: 'test:warmup', description: 'warmup' };
    for (let i = 0; i < 5; i++) {
      validate(warmupData);
    }
  ...
  describe('Action Schema Validation Performance', () => {
    test('should validate actions within performance threshold', async () => {
      ...
      for (const filePath of testFiles) {
        try {
          ...
          expect(medianTime).toBeLessThan(performanceThresholds.maxValidationTime);
        } catch (error) {
      ...
      if (performanceResults.length === 0) {
        throw new Error('No files were successfully processed for performance testing');
      }
      ...
      console.log(`  Files tested: ${performanceResults.length}/${testFiles.length}`);
      ...
      console.log(`  Per-file threshold: ${performanceThresholds.maxValidationTime}ms`);
      console.log(`  Target average: ${performanceThresholds.targetAverageTime}ms`);
      ...
      expect(p95Time).toBeLessThan(performanceThresholds.maxValidationTime * 1.5); // Use P95 instead of max

      // Ensure reasonable distribution
      expect(minTime).toBeGreaterThan(0);
      // Use more lenient variation ratio based on environment
      expect(maxTime / minTime).toBeLessThan(performanceThresholds.maxVariationRatio);
    ...
    test('should handle batch validation efficiently', async () => {
      ...
      if (actionDataBatch.length < batchSize * 0.8) {
        throw new Error(`Too many file load failures: ${loadFailures.length}/${testFiles.length}. Cannot perform meaningful batch test.`);
      }
      ...
      for (let iteration = 0; iteration < iterations; iteration++) {
        ...
        for (const { filePath, actionData } of actionDataBatch) {
          try {
            ...
            console.warn(`Validation error for ${filePath}:`, validationError.message);
        ...
        if (iteration > 0) { // Skip first iteration (warmup)
          ...
          if (iteration === 1) { // Use results from first non-warmup iteration
      ...
      console.log(`  Environment: ${isCI ? 'CI' : 'Local'}`);
      console.log(`  Batch size: ${actionDataBatch.length} actions (${loadFailures.length} load failures)`);
      console.log(`  Average total time: ${avgTotalTime.toFixed(2)}ms (${iterations - 1} iterations)`);
      console.log(`  Average per validation: ${averageTimePerValidation.toFixed(3)}ms`);
      console.log(`  Load success rate: ${((actionDataBatch.length / testFiles.length) * 100).toFixed(1)}%`);
      ...
      expect(averageTimePerValidation).toBeLessThan(performanceThresholds.batchAverageTime);
      ...
      console.log(`  Validation success rate: ${(validationSuccessRate * 100).toFixed(1)}%`);
      
      // Expect high validation success rate, but allow for some invalid files
      expect(validationSuccessRate).toBeGreaterThan(0.90); // At least 90% of loaded files should validate
      
      // Ensure we have a reasonable number of successful validations
      expect(validCount).toBeGreaterThan(Math.min(40, actionDataBatch.length * 0.8));
    });
    ...
    test('should maintain performance under repeated validation', async () => {
      ...
      console.log(`  Performance variation (P99/P50): ${(p99 / p50).toFixed(1)}x`);
