...
describe('BaseCompletionLLMStrategy', () => {
    describe('_constructPromptPayload', () => {
        ...
        test('should log debug messages for ignored promptFrame and prompt preview', () => {
            ...
            const llmConfig = {apiType: currentApiType, promptFrame: "System Frame."}; // promptFrame will be ignored
            const currentgameSummary = "Test game summary for logging.";
            const expectedTrimmedSummary = currentgameSummary.trim();

            strategy._constructPromptPayload(currentgameSummary, llmConfig.promptFrame, llmConfig);

            expect(mockLogger.debug).toHaveBeenCalledWith(
                expect.stringContaining(`BaseCompletionLLMStrategy._constructPromptPayload for apiType '${currentApiType}'. gameSummary (finalPromptString) length: ${currentgameSummary.length}. llmConfig.promptFrame will be ignored.`)
            );
            expect(mockLogger.debug).toHaveBeenCalledWith(
                expect.stringContaining(`BaseCompletionLLMStrategy._constructPromptPayload: Using finalPromptString as the 'prompt'. Preview: ${expectedTrimmedSummary.substring(0, 100)}`)
            );
        });
