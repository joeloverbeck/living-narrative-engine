...
describe('ClientApiKeyProvider', () => {
    ...
    describe('Constructor', () => {
        test('should store injected logger and log creation', () => {
            ...
            expect(logger.debug).toHaveBeenCalledWith('ClientApiKeyProvider: Instance created.');
        });

        test('should throw error if logger is invalid or missing', () => {
            expect(() => new ClientApiKeyProvider({logger: null}))
                .toThrow('ClientApiKeyProvider: Constructor requires a valid logger instance.');
            expect(() => new ClientApiKeyProvider({logger: {}}))
                .toThrow('ClientApiKeyProvider: Constructor requires a valid logger instance.');
        });
    });
    ...
    describe('getKey Method', () => {
        ...
        test('should return null and log error if environmentContext is invalid', async () => {
            ...
            expect(key).toBeNull();
            expect(logger.error).toHaveBeenCalledWith("ClientApiKeyProvider.getKey (test-llm): Invalid environmentContext provided.");
        });
        ...
        test('should return null and log error if environmentContext.isClient is not a function', async () => {
            ...
            const key = await provider.getKey(llmConfig, /** @type {any} */ (invalidEc));
            expect(key).toBeNull();
            expect(logger.error).toHaveBeenCalledWith("ClientApiKeyProvider.getKey (test-llm): Invalid environmentContext provided.");
        });
        ...
        test('should return null and log error if llmConfig is null', async () => {
            ...
            expect(key).toBeNull();
            expect(logger.error).toHaveBeenCalledWith("ClientApiKeyProvider.getKey (UnknownLLM): llmConfig is null or undefined.");
        });
