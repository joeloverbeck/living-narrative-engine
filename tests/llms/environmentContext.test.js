...
describe('EnvironmentContext', () => {
    ...
    describe('Constructor and Validations', () => {
        describe('Logger Validation', () => {
            test('should throw error if logger is null', () => {
                expect(() => new EnvironmentContext({logger: null, executionEnvironment: 'client'}))
                    .toThrow('EnvironmentContext: Constructor requires a valid logger instance with info, warn, error, and debug methods.');
            });

            test('should throw error if logger is an empty object', () => {
                expect(() => new EnvironmentContext({logger: {}, executionEnvironment: 'client'}))
                    .toThrow('EnvironmentContext: Constructor requires a valid logger instance with info, warn, error, and debug methods.');
            });
            ...
            ['info', 'warn', 'error', 'debug'].forEach(method => {
                test(`should throw error if logger is missing ${method} method`, () => {
                    ...
                    expect(() => new EnvironmentContext({logger: incompleteLogger, executionEnvironment: 'client'}))
                        .toThrow('EnvironmentContext: Constructor requires a valid logger instance with info, warn, error, and debug methods.');
                });
            });
            ...
            test('should use console.error if logger.error is missing during critical logger validation failure', () => {
                ...
                try {
                    new EnvironmentContext({logger: invalidLogger, executionEnvironment: 'client'});
                } catch (e) {
                    // Expected to throw
                }
                expect(consoleErrorSpy).toHaveBeenCalledWith('EnvironmentContext: Constructor requires a valid logger instance with info, warn, error, and debug methods.');
        ...
        describe('Execution Environment Validation', () => {
            test('should correctly set executionEnvironment for "client"', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'client'});
            ...
            test('should correctly set executionEnvironment for "unknown"', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'unknown'});
            ...
            test('should default to "unknown" and log warning for invalid executionEnvironment string', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'invalid_env'});
                expect(context.getExecutionEnvironment()).toBe('unknown');
                expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid executionEnvironment provided: 'invalid_env'. Defaulting to 'unknown'."));
            });

            test('should default to "unknown" and log warning if executionEnvironment is null', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: null});
                expect(context.getExecutionEnvironment()).toBe('unknown');
                expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid executionEnvironment provided: 'null'. Defaulting to 'unknown'."));
            });
        ...
        describe('Project Root Path Validation', () => {
            ...
            test('server env: should correctly set projectRootPath if valid', () => {
                ...
                expect(logger.debug).toHaveBeenCalledWith("EnvironmentContext: Server-side projectRootPath set to: '/my/project/root/'");
            });

            test('server env: should throw error if projectRootPath is null', () => {
                expect(() => new EnvironmentContext({logger, executionEnvironment: 'server', projectRootPath: null}))
                    .toThrow("EnvironmentContext: Constructor requires 'projectRootPath' (non-empty string) when executionEnvironment is 'server'.");
                expect(logger.error).toHaveBeenCalledWith("EnvironmentContext: Constructor requires 'projectRootPath' (non-empty string) when executionEnvironment is 'server'.");
            });

            test('server env: should throw error if projectRootPath is empty string', () => {
                expect(() => new EnvironmentContext({logger, executionEnvironment: 'server', projectRootPath: '  '}))
                    .toThrow("EnvironmentContext: Constructor requires 'projectRootPath' (non-empty string) when executionEnvironment is 'server'.");
                expect(logger.error).toHaveBeenCalledWith("EnvironmentContext: Constructor requires 'projectRootPath' (non-empty string) when executionEnvironment is 'server'.");
            });
            ...
            test('client env: projectRootPath should be null and log warning if provided', () => {
                ...
                expect(logger.warn).toHaveBeenCalledWith("EnvironmentContext: 'projectRootPath' (\"/my/project/root/\") was provided, but executionEnvironment is 'client', not 'server'. It will be ignored.");
            });
            test('client env: projectRootPath should be null if not provided', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'client'});
                expect(context.getProjectRootPath()).toBeNull();
                expect(logger.warn).not.toHaveBeenCalledWith(expect.stringContaining("'projectRootPath'"));
            });
            ...
            test('unknown env: projectRootPath should be null and log warning if provided', () => {
                ...
                expect(context.getProjectRootPath()).toBeNull();
                expect(logger.warn).toHaveBeenCalledWith("EnvironmentContext: 'projectRootPath' (\"/my/project/root/\") was provided, but executionEnvironment is 'unknown', not 'server'. It will be ignored.");
            });
        });
        ...
        describe('Proxy Server URL Validation', () => {
            ...
            test('client env: should correctly set proxyServerUrl if valid', () => {
                ...
                expect(logger.info).toHaveBeenCalledWith(`EnvironmentContext: Client-side proxy URL configured to: '${url}'.`);
            });

            test('client env: should use default proxyServerUrl and log info if null', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'client', proxyServerUrl: null});
                expect(context.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
                expect(logger.info).toHaveBeenCalledWith(`EnvironmentContext: Client-side proxyServerUrl not provided. Using default: '${DEFAULT_PROXY_SERVER_URL}'.`);
            });

            test('client env: should use default proxyServerUrl and log warning if empty string', () => {
                const context = new EnvironmentContext({logger, executionEnvironment: 'client', proxyServerUrl: '   '});
                expect(context.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
                expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining(`Client-side proxyServerUrl provided but was empty or invalid ('   '). Using default: '${DEFAULT_PROXY_SERVER_URL}'.`));
            });
            ...
            test('client env: should use default proxyServerUrl and log warning if invalid URL', () => {
                ...
                expect(context.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
                expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining(`Provided proxyServerUrl '${invalidUrl}' for client environment is not a valid URL. Falling back to default: '${DEFAULT_PROXY_SERVER_URL}'.`));
            });
            ...
            test('server env: should use provided valid proxyServerUrl and log debug', () => {
                ...
                expect(context.getProxyServerUrl()).toBe(url);
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(`proxyServerUrl ('${url}') was provided for non-client environment ('server'). It might not be used.`));
            });
            ...
            test('server env: should use default proxyServerUrl and log debug if null', () => {
                ...
                expect(context.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(`proxyServerUrl not provided for non-client environment ('server'). Defaulting to '${DEFAULT_PROXY_SERVER_URL}', though it might not be used.`));
            });
            ...
            test('server env: should use default proxyServerUrl and log debug if invalid URL provided', () => {
                ...
                expect(context.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(`Provided proxyServerUrl '${invalidUrl}' for non-client environment is not a valid URL. Setting to default ('${DEFAULT_PROXY_SERVER_URL}'), but it might not be used.`));
            });
            ...
            test('unknown env: should use provided valid proxyServerUrl and log debug', () => {
                ...
                const context = new EnvironmentContext({logger, executionEnvironment: 'unknown', proxyServerUrl: url});
                expect(context.getProxyServerUrl()).toBe(url);
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(`proxyServerUrl ('${url}') was provided for non-client environment ('unknown'). It might not be used.`));
            });
        });
    });
    ...
    describe('Getter Methods', () => {
        test('getExecutionEnvironment should return correct environment', () => {
            const clientContext = new EnvironmentContext({logger, executionEnvironment: 'client'});
        ...
        test('getProxyServerUrl should return correct URL', () => {
            ...
            const clientDefaultContext = new EnvironmentContext({logger, executionEnvironment: 'client'});
            expect(clientDefaultContext.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
            ...
            expect(serverDefaultContext.getProxyServerUrl()).toBe(DEFAULT_PROXY_SERVER_URL);
        });
        ...
        test('isClient should return true only for client environment', () => {
            ...
            const unknownContext = new EnvironmentContext({logger, executionEnvironment: 'unknown'});
    ...
    test('should log final info message on successful instantiation', () => {
        ...
        expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("EnvironmentContext: Instance created. Execution environment: client."));
    });
});
