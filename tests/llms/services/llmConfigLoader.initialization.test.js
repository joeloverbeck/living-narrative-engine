...
jest.mock('../../../src/utils/apiUtils.js', () => ({ // Adjust path if needed
...
const LLM_CONFIG_SCHEMA_ID = 'http://example.com/schemas/llm-configs.schema.json';
...
describe('LlmConfigLoader - Initialization and Schema Handling', () => {
    ...
    beforeEach(() => {
        ...
        Workspace_retry.mockResolvedValue(JSON.parse(JSON.stringify(MOCK_RAW_LLM_CONFIG_DATA)));
        configurationMock.getContentTypeSchemaId.mockReturnValue(LLM_CONFIG_SCHEMA_ID);
    ...
    test('should return an error result with stage "validation" if schema is not found by validator', async () => {
        ...
        // Assert
        expect(Workspace_retry).toHaveBeenCalledWith(MOCK_LLM_CONFIG_PATH, expect.any(Object), expect.any(Number), expect.any(Number), expect.any(Number));
        expect(configurationMock.getContentTypeSchemaId).toHaveBeenCalledWith('llm-configs');
        expect(schemaValidatorMock.validate).toHaveBeenCalledWith(LLM_CONFIG_SCHEMA_ID, MOCK_RAW_LLM_CONFIG_DATA);
        ...
        expect(standardizedError.details).toEqual(expect.objectContaining(schemaNotFoundErrorDetail));
