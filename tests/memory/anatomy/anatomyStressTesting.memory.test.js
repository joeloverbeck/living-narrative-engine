...
describe('Anatomy Memory Stress Testing', () => {
  ...
  const createMemoryMonitor = () => ({
    ...
    compareSnapshots: function (startLabel, endLabel) {
      ...
      return {
        ...
        heapGrowthPercent:
          ((end.heapUsed - start.heapUsed) / start.heapUsed) * 100,
    ...
    detectMemoryLeak: function (
      iterations,
      threshold = MEMORY_THRESHOLDS.LEAK_DETECTION_GROWTH
    ) {
      if (this.snapshots.length < 2) return false;

      // For batched execution, use first and last batch snapshots
      const batchSnapshots = this.snapshots.filter((s) =>
        s.label.startsWith('batch_')
      );
      ...
      if (batchSnapshots.length >= 2) {
        ...
        const growth =
          (lastBatch.heapUsed - firstBatch.heapUsed) / firstBatch.heapUsed;
      ...
      const lateAvg =
        this.snapshots
          .slice(lastQuarter)
          .reduce((sum, s) => sum + s.heapUsed, 0) /
        (this.snapshots.length - lastQuarter);
  ...
  describe('Memory Leak Detection', () => {
    it('should not leak memory during repeated anatomy generation and cleanup', async () => {
      ...
      const checkInterval = Math.max(
        batchSize,
        Math.floor(totalIterations / 6)
      ); // Check memory a handful of times per run
      ...
      for (let batch = 0; batch < batches; batch++) {
        ...
        for (let i = 0; i < batchIterations; i++) {
          ...
          if (i % 400 === 0 && i > 0) {
            // Slightly more frequent clears to keep batches light
            if (testBed.logger && testBed.logger.info) {
              testBed.logger.info.mockClear();
              testBed.logger.debug.mockClear();
            }
          ...
          if (
            completedIterations % checkInterval === 0 &&
            ...
            console.log(
              `  Iteration ${completedIterations}/${totalIterations}: Heap ${heapMB}MB`
            );
          }
        }
        ...
        const heapMB = (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(
          2
        );
        console.log(
          `  Batch ${batch + 1}/${batches} complete. Heap: ${heapMB}MB`
        );
      ...
      console.log(
        `  Heap growth: ${(comparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  Growth percent: ${comparison.heapGrowthPercent.toFixed(2)}%`
      );
    ...
    it('should not leak memory during cache operations', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (i % 700 === 0 && i > 0) {
          // Snapshot cadence tuned to reduced iterations
      ...
      console.log(
        `Cache Operations Memory Growth: ${comparison.heapGrowthPercent.toFixed(2)}%`
      );
    ...
    it('should not leak memory during description generation', async () => {
      ...
      console.log(
        `Description Generation Memory Growth: ${comparison.heapGrowthPercent.toFixed(2)}%`
      );
  ...
  describe('Large Dataset Memory Usage', () => {
    ...
    it('should efficiently manage memory for cache structures', async () => {
      ...
      const fullComparison = memoryMonitor.compareSnapshots(
        'cache_struct_start',
        'all_cached'
      );
      const invalidComparison = memoryMonitor.compareSnapshots(
        'all_cached',
        'half_invalidated'
      );
      const rebuildComparison = memoryMonitor.compareSnapshots(
        'half_invalidated',
        'rebuilt'
      );
      ...
      console.log(
        `  Full cache build: ${(fullComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  After invalidation: ${(invalidComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  After rebuild: ${(rebuildComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
    ...
    it('should analyze component memory overhead', async () => {
      ...
      for (const componentType of componentTypes) {
        ...
        const entities = [];
        for (let i = 0; i < 150; i++) {
          // Balanced for speed and validation quality
      ...
      results.forEach((r) => {
        console.log(
          `  ${r.componentType}: ${(r.memoryPerComponent / 1024).toFixed(2)}KB per component`
        );
      });
    });
  });
  ...
  describe('Garbage Collection Patterns', () => {
    ...
    it('should handle memory reclamation efficiently', async () => {
      ...
      for (const phase of phases) {
        ...
        if (phase.name === 'growth') {
          ...
          for (let i = 0; i < phase.count; i++) {
            ...
            bodyGraphService.buildAdjacencyCache(anatomy.rootId);
            // Get entity first for description generation
            const entity = testBed.entityManager.getEntityInstance(
              anatomy.rootId
            );
            await descriptionService.generateAllDescriptions(entity);
          }
      ...
      const growthComparison = memoryMonitor.compareSnapshots(
        'growth_start',
        'growth_end'
      );
      const stableComparison = memoryMonitor.compareSnapshots(
        'stable_start',
        'stable_end'
      );
      const cleanupComparison = memoryMonitor.compareSnapshots(
        'cleanup_start',
        'cleanup_end'
      );
      ...
      console.log(
        `  Growth phase: ${(growthComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  Stable phase: ${(stableComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  Cleanup phase: ${(cleanupComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
    ...
    it('should analyze reference retention patterns', async () => {
      ...
      const gcComparison = memoryMonitor.compareSnapshots(
        'before_gc',
        'after_gc'
      );
      const finalComparison = memoryMonitor.compareSnapshots(
        'ref_start',
        'ref_end'
      );
      ...
      console.log(
        `  After GC: ${(gcComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `  Final: ${(finalComparison.heapGrowth / 1024 / 1024).toFixed(2)}MB`
      );
