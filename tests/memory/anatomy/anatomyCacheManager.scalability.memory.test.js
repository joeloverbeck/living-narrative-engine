...
describe('AnatomyCacheManager - Scalability Memory Tests', () => {
  ...
  it('should not leak memory across multiple character generation batches', async () => {
    ...
    memorySnapshots.forEach((snap) => {
      console.log(
        `  ${snap.label}: ${(snap.heapUsed / 1024 / 1024).toFixed(2)}MB`
      );
    });
    ...
    const growthRatio =
      Math.abs(laterGrowth) / Math.max(Math.abs(earlierGrowth), 1024 * 1024);
    ...
    console.log(
      `Total memory increase: ${totalIncreaseMB.toFixed(2)}MB for ${batchCount * charactersPerBatch} characters`
    );
