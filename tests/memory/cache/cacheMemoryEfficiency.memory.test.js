...
describe('Cache Memory Efficiency Tests', () => {
  ...
  describe('Memory Usage Under Load', () => {
    it('should maintain efficient memory usage under load', () => {
      ...
      console.log(`Memory Efficiency: ${metrics.size} items cached from ${itemCount} total`);
    ...
    it('should handle memory pressure gracefully', () => {
      ...
      console.log(`Memory Pressure Test: ${successfulSets}/${totalAttempts} items set`);
      console.log(`Final: ${finalMetrics.size} items, ${finalMemoryUsage.utilizationPercent.toFixed(1)}% memory utilization`);
      console.log(`Memory usage: ${finalMemoryUsage.currentMB.toFixed(2)} MB of ${finalMemoryUsage.maxMB} MB limit`);
    });
  });
  ...
  describe('Memory-Based Eviction', () => {
    it('should evict items based on memory constraints', () => {
      ...
      console.log(`Eviction Test: ${metrics.size}/${itemsAdded} items retained`);
      console.log(`Memory: ${memoryUsage.currentBytes} bytes (${memoryUsage.utilizationPercent.toFixed(1)}% of ${memoryLimit} byte limit)`);
    });
    ...
    it('should evict items when memory limit is reached', () => {
      ...
      console.log(`Memory: ${memoryUsage.currentBytes} bytes (${memoryUsage.utilizationPercent.toFixed(1)}% of limit)`);
    });
  });
  ...
  describe('Memory Pressure Scenarios', () => {
    it('should handle extreme memory constraints', () => {
      ...
      console.log(`Extreme Constraints: ${totalOperations} operations, ${metrics.size} items retained, ${errorCount} errors`);
      console.log(`Memory: ${memoryUsage.currentBytes} bytes (${memoryUsage.utilizationPercent?.toFixed(1) || 'N/A'}%)`);
    });
    ...
    it('should recover from memory pressure when load decreases', () => {
      ...
      console.log(`Pressure Phase: ${pressureMetrics.size} items, ${pressureMemory.utilizationPercent.toFixed(1)}% utilization`);
      console.log(`Recovery Phase: ${recoveryMetrics.size} items, ${recoveryMemory.utilizationPercent.toFixed(1)}% utilization`);
    });
  });
  ...
  describe('Memory Monitoring', () => {
    it('should accurately report memory usage statistics', () => {
      ...
      expect(memoryUsage.currentBytes / (1024 * 1024)).toBeCloseTo(memoryUsage.currentMB, 5);

      console.log(`Memory Monitoring: ${itemCount} items`);
      console.log(`  Current: ${memoryUsage.currentBytes} bytes (${memoryUsage.currentMB.toFixed(3)} MB)`);
      console.log(`  Maximum: ${memoryUsage.maxBytes} bytes (${memoryUsage.maxMB} MB)`);
      console.log(`  Utilization: ${memoryUsage.utilizationPercent.toFixed(2)}%`);
    });
    ...
    it('should track memory changes during cache operations', () => {
      ...
      for (let i = 1; i < snapshots.length; i++) {
        expect(snapshots[i].currentBytes).toBeGreaterThanOrEqual(snapshots[i - 1].currentBytes);
      }
      ...
      expect(afterDelete.currentBytes).toBeLessThan(snapshots[snapshots.length - 1].currentBytes);

      console.log(`Memory Tracking: Initial ${initialMemory.currentBytes}B → Peak ${snapshots[snapshots.length - 1].currentBytes}B → After Delete ${afterDelete.currentBytes}B`);
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle memory calculation for various data types', () => {
      ...
      console.log(`Edge Cases: ${metrics.size} different data types, ${memoryUsage.currentBytes} bytes total`);
    });
    ...
    it('should handle memory usage with undefined and non-serializable values', () => {
      ...
      console.log(`Problematic Values: ${cache.getMetrics().size} items, ${memoryUsage.currentBytes} bytes`);
    });
  });
});
