...
import { TestConfigurationValidator } from './validation/testConfigurationValidator.js';
...
export class TestConfigurationFactory {
   ...
   * @returns {Promise<object>} A configured test module builder.
   * @example
  ...
  static async createTestModule(moduleType = 'turnExecution', mockFn = null) {
    ...
    switch (moduleType) {
      ...
      case 'actionProcessing':
        return await TestModuleBuilder.forActionProcessing();
      case 'entityManagement':
        return await TestModuleBuilder.forEntityManagement();
      case 'llmTesting':
        return await TestModuleBuilder.forLLMTesting();
      default:
  ...
  static createScenario(scenario) {
    try {
      const preset = TestModuleRegistry.getPreset(scenario);
      return preset();
    } catch (error) {
      throw new Error(
        `Unknown scenario: ${scenario}. Available: ${TestModuleRegistry.getPresetNames().join(', ')}`
      );
    }
  }
   ...
   * @returns {Promise<object>} A test module configured with the legacy settings.
   */
  static async migrateToTestModule(legacyConfig) {
    // Use dynamic import to avoid circular dependency
    const { TestModuleBuilder } = await import('./testModuleBuilder.js');
    const module = await TestModuleBuilder.forTurnExecution();
