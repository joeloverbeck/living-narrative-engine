...
export class ClothingTestAssertions {
   ...
   * @param {string} [description] - Optional description for the assertion
   */
  static async assertAsyncPerformanceWithin(asyncFn, maxTimeMs, description = '') {
    const start = performance.now();
    await asyncFn();
    const duration = performance.now() - start;
   ...
   */
  static assertAccessibilityResult(result, expectedAccessible, expectedReason = null, expectedBlockingItems = null) {
  ...
  static assertLaylaAgirreScenario(result) {
    ...
    expect(resultIds).toContain('clothing:dark_olive_high_rise_double_pleat_trousers');
    expect(resultIds).not.toContain('clothing:power_mesh_boxer_brief');
    
    // In topmost mode, should only be one item
    if (result.length === 1) {
      expect(resultIds[0]).toBe('clothing:dark_olive_high_rise_double_pleat_trousers');
    }
  }
