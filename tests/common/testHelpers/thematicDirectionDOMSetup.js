...
export function createMockElement(tag = 'div') {
  ...
  const element = {
    ...
    get outerHTML() {
      ...
      if (this.value !== undefined && this.value !== '') attrs.push(`value="${this.value}"`);
...
export function setupThematicDirectionDOM(mockElements) {
  ...
  if (mockElements.previousConceptsSelect) {
    mockElements.previousConceptsSelect.innerHTML = '<option value="">-- Select a saved concept --</option>';
  ...
  const mockDocument = {
    ...
    querySelector: jest.fn((selector) => {
      ...
      console.warn(`querySelector called with unsupported selector: ${selector}`);
      return null;
    }),
