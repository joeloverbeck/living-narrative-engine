/**
 * @file E2E tests for NEEDS_SEPARATION classification
 * Tests full pipeline from bootstrap to classification generation.
 * These tests validate complete system behavior with Monte Carlo sampling.
 *
...
/**
 * Setup helper for creating test environment.
 *
 * @param {object} prototypeData - Prototype data to register
 * @returns {Promise<{dom: JSDOM, container: object, analyzer: object}>}
 */
async function setupEnvironment(prototypeData) {
  const dom = new JSDOM(
    `
    <!DOCTYPE html>
    <html>
  ...
  const analyzer = result.container.resolve(
    diagnosticsTokens.IPrototypeOverlapAnalyzer
  );

  return { dom, container: result.container, analyzer };
}
 ...
 */
function cleanupEnvironment(dom, container) {
...
describe('NEEDS_SEPARATION Classification E2E', () => {
  let dom;
  let container;
  let analyzer;
  let analysisResult;
  ...
  beforeAll(async () => {
    const env = await setupEnvironment(NEEDS_SEPARATION_PROTOTYPES);
    dom = env.dom;
    container = env.container;
    analyzer = env.analyzer;

    // Run analysis ONCE and share result across all tests in this suite
  ...
  afterAll(() => {
    cleanupEnvironment(dom, container);
  });

  describe('joy_high_arousal ↔ excitement style pair (needs_separation candidate)', () => {
    it('should track needs_separation in classificationBreakdown', () => {
      expect(analysisResult.metadata).toBeDefined();
      expect(analysisResult.metadata.classificationBreakdown).toBeDefined();
      expect(
        typeof analysisResult.metadata.classificationBreakdown.needsSeparation
      ).toBe('number');
    });

    it('should generate recommendations for needs_separation pairs', () => {
      // needs_separation pairs should generate 'prototype_needs_separation' recommendations
      const separationRecs = analysisResult.recommendations.filter(
      ...
      expect(separationRecs.length).toBe(
        analysisResult.metadata.classificationBreakdown.needsSeparation
      );
    });

    it('should include complete v2 evidence fields', () => {
      const separationRecs = analysisResult.recommendations.filter(
        (r) => r.type === 'prototype_needs_separation'
      );
    ...
    it('should include appropriate actions for needs_separation', () => {
      const separationRecs = analysisResult.recommendations.filter(
        (r) => r.type === 'prototype_needs_separation'
      );
    ...
    it('should have valid severity and confidence values', () => {
      const separationRecs = analysisResult.recommendations.filter(
        (r) => r.type === 'prototype_needs_separation'
      );
    ...
    it('should produce deterministic classification breakdown', async () => {
      ...
      expect(
        analysisResult.metadata.classificationBreakdown.needsSeparation
      ).toBe(result2.metadata.classificationBreakdown.needsSeparation);

      // Overall classification counts should be identical
      expect(analysisResult.metadata.classificationBreakdown).toEqual(
        result2.metadata.classificationBreakdown
      );
    });

    it('should include gate banding suggestions for needs_separation', () => {
      const separationRecs = analysisResult.recommendations.filter(
        (r) => r.type === 'prototype_needs_separation'
      );
  ...
  describe('Metadata structure validation', () => {
    it('should have complete classificationBreakdown structure', () => {
    ...
    it('should track all pairs evaluated correctly', () => {
      ...
      expect(totalClassified).toBe(
        analysisResult.metadata.candidatePairsEvaluated
      );
    });
  });
});

describe('Distinct Prototypes E2E (should NOT trigger needs_separation)', () => {
  let dom;
  let container;
  let analyzer;
  let analysisResult;
  ...
  beforeAll(async () => {
    const env = await setupEnvironment(DISTINCT_PROTOTYPES);
    dom = env.dom;
    container = env.container;
    analyzer = env.analyzer;
  ...
  it('should NOT classify as needs_separation when prototypes have non-overlapping gates', () => {
