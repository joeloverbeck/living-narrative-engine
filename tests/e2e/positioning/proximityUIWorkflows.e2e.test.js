...
describe('Proximity UI Workflows E2E', () => {
  ...
  describe('Action Discovery UI Updates', () => {
    it('should update available actions UI when actors sit together', async () => {
      ...
      expect(document.getElementById('action-positioning:sit_down')).not.toBeNull();
      expect(document.getElementById('action-positioning:sit_down').disabled).toBe(false);
      ...
      const intimateActions = [
        ...
        { id: 'intimacy:lean_close', name: 'Lean close to Bob', available: true },
        { id: 'positioning:move_closer', name: 'Move closer to Bob', available: true }
      ];
      ...
      expect(document.getElementById('action-intimacy:lean_close')).not.toBeNull();
      expect(document.getElementById('action-positioning:move_closer')).not.toBeNull();
      ...
      expect(document.getElementById('action-positioning:move_closer')).toBeNull();
    });
    ...
    it('should update UI aria-live regions for proximity changes', async () => {
      ...
      gameLog.textContent = 'Bob sits next to Alice on the park bench. Alice and Bob are now close.';
      ...
      expect(proximityIndicator.textContent).toBe('Alice is sitting close to Bob.');
      ...
      gameLog.textContent = 'Alice stands up. Alice and Bob are no longer close.';
      proximityIndicator.textContent = 'Alice is standing. Bob is sitting alone.';
  ...
  describe('Character Status UI Updates', () => {
    it('should update character status display during proximity workflows', async () => {
      ...
      const aliceStatus = statusElement.querySelector(`[data-actor="${aliceId}"]`);
      const bobStatus = statusElement.querySelector(`[data-actor="${bobId}"]`);

      expect(aliceStatus.querySelector('.position').textContent).toBe('Standing');
      ...
      aliceStatus.querySelector('.position').textContent = 'Sitting on Dining Chairs';

      expect(aliceStatus.querySelector('.position').textContent).toBe('Sitting on Dining Chairs');
      expect(aliceStatus.querySelector('.closeness').textContent).toBe('Alone');
      ...
      bobStatus.querySelector('.position').textContent = 'Sitting on Dining Chairs';
      aliceStatus.querySelector('.closeness').textContent = 'Close to Bob';
      bobStatus.querySelector('.closeness').textContent = 'Close to Alice';

      expect(bobStatus.querySelector('.position').textContent).toBe('Sitting on Dining Chairs');
      expect(aliceStatus.querySelector('.closeness').textContent).toBe('Close to Bob');
      expect(bobStatus.querySelector('.closeness').textContent).toBe('Close to Alice');
    ...
    it('should handle dynamic action button state changes', async () => {
      ...
      expect(whisperButton.classList.contains('proximity-dependent')).toBe(true);
  ...
  describe('Accessibility and Keyboard Navigation', () => {
    it('should maintain proper tab order during UI state changes', async () => {
      ...
      const focusableElements = actionListElement.querySelectorAll('button:not([disabled]):not([tabindex="-1"])');
    ...
    it('should announce proximity changes to screen readers properly', async () => {
      ...
      expect(proximityIndicator.textContent).toBe('Alice is sitting alone on the bench');
      ...
      announce('Bob sits next to Alice. Alice and Bob are now close together', 'assertive');
      proximityIndicator.textContent = 'Alice is sitting close to Bob on the bench';

      expect(gameLog.textContent).toBe('Bob sits next to Alice. Alice and Bob are now close together');
      expect(proximityIndicator.textContent).toBe('Alice is sitting close to Bob on the bench');
      ...
      announce('Alice stands up. Alice and Bob are no longer close', 'assertive');
      proximityIndicator.textContent = 'Alice is standing. Bob is sitting alone on the bench';

      expect(gameLog.textContent).toBe('Alice stands up. Alice and Bob are no longer close');
      expect(proximityIndicator.textContent).toBe('Alice is standing. Bob is sitting alone on the bench');
    });
  });
});