...
describe('Traits Generator E2E Tests', () => {
  ...
  describe('Page Loading and Structure', () => {
    ...
    it('should have all required CSS files linked', () => {
      ...
      expect(cssFiles.some(href => href.includes('components.css'))).toBe(true);
      expect(cssFiles.some(href => href.includes('traits-generator.css'))).toBe(true);
    });
  ...
  describe('UI Elements and Initial State', () => {
    ...
    it('should have accessibility features properly configured', () => {
      ...
      expect(resultsContainer.getAttribute('aria-label')).toBe('Generated character traits');
  ...
  describe('Form Validation and Input Handling', () => {
    ...
    it('should handle form field interactions correctly', () => {
      ...
      coreMotivationInput.value = 'A character driven by the need to prove themselves worthy of love and acceptance.';
      expect(coreMotivationInput.value).toBe('A character driven by the need to prove themselves worthy of love and acceptance.');

      internalContradictionInput.value = 'They push people away when they get too close, fearing vulnerability.';
      expect(internalContradictionInput.value).toBe('They push people away when they get too close, fearing vulnerability.');

      centralQuestionInput.value = 'Can they learn to accept love without losing their identity?';
      expect(centralQuestionInput.value).toBe('Can they learn to accept love without losing their identity?');
    ...
    it('should have proper placeholder text and help text', () => {
      ...
      const coreMotivationHelp = document.getElementById('core-motivation-help');
  ...
  describe('Button and Control Interactions', () => {
    it('should have properly configured generate button', () => {
      ...
      expect(generateBtn.getAttribute('aria-label')).toBe('Generate character traits');
  ...
  describe('Empty State Display', () => {
    it('should show appropriate empty state content', () => {
      ...
      const emptyStateSubtext = emptyState.querySelector('.empty-state-subtext');
      ...
      expect(emptyStateSubtext.textContent).toContain('specific character inputs');
    });
  });
        ...
        json: () => Promise.resolve([
          {
            ...
            centralQuestion: 'Can peace exist without the threat of violence?'
          }
        ])
      });
    }
    ...
    if (url.includes('generate-traits')) {
      return Promise.resolve({
        ...
        json: () => Promise.resolve({
          ...
          physicalDescription: 'A tall, weathered figure with calculating eyes and battle scars',
          personality: [
            { trait: 'Cautious', explanation: 'Carefully considers all options before acting' },
            { trait: 'Protective', explanation: 'Instinctively shields others from danger' }
          ],
          ...
          profile: 'A complex character torn between desire for peace and necessity of war',
