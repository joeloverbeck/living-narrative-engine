...
describe('Clothing Resolution Workflows E2E', () => {
  ...
  function createGameContext() {
    return {
      ...
      entityManager,
      allEntities: [clothingTestActor.id, ...Object.values(testActors).map(a => a.id)],
  ...
  async function createClothingTestActor() {
    ...
    container.resolve(tokens.IDataRegistry).store('entityDefinitions', actorId, actorDefinition);
  ...
  async function createClothingItemEntities() {
    const clothingItems = [
      { id: 'jacket_001', name: 'Leather Jacket', type: 'jacket', layer: 'outer', slot: 'torso_upper' },
      { id: 'shirt_001', name: 'Cotton Shirt', type: 'shirt', layer: 'base', slot: 'torso_upper' },
      { id: 'undershirt_001', name: 'Undershirt', type: 'undershirt', layer: 'underwear', slot: 'torso_upper' },
      { id: 'pants_001', name: 'Jeans', type: 'pants', layer: 'outer', slot: 'torso_lower' },
      { id: 'underwear_001', name: 'Underwear', type: 'underwear', layer: 'underwear', slot: 'torso_lower' },
      { id: 'shoes_001', name: 'Running Shoes', type: 'shoes', layer: 'outer', slot: 'feet' },
      { id: 'socks_001', name: 'Cotton Socks', type: 'socks', layer: 'base', slot: 'feet' },
      { id: 'gloves_001', name: 'Winter Gloves', type: 'gloves', layer: 'base', slot: 'hands' },
    ];
  ...
  describe('Basic Clothing Access', () => {
    test('should resolve topmost_clothing items', async () => {
      ...
      expect(resultArray).toEqual(
        expect.arrayContaining(['jacket_001', 'pants_001', 'shoes_001', 'gloves_001'])
      );
    });
    ...
    test('should handle missing equipment gracefully', async () => {
      ...
      // Add the definition to the registry
      container.resolve(tokens.IDataRegistry).store('entityDefinitions', emptyActorId, emptyActorDefinition);
  ...
  describe('Complex Clothing Scenarios', () => {
    test('should handle multiple layers correctly', async () => {
      ...
      expect(allArray).toEqual(
        expect.arrayContaining([
          'jacket_001', 'shirt_001', 'undershirt_001', // torso_upper layers
          'pants_001', 'underwear_001', // torso_lower layers
          'shoes_001', 'socks_001', // feet layers
    ...
    test('should handle missing equipment gracefully', async () => {
      ...
      // Add the definition to the registry
      container.resolve(tokens.IDataRegistry).store('entityDefinitions', partialActorId, partialActorDefinition);
  ...
  describe('Action Integration', () => {
    ...
    test('should update after clothing changes', async () => {
      ...
      // Add the definition to the registry
      const registry = container.resolve(tokens.IDataRegistry);
      registry.store('entityDefinitions', modifiedActorId, modifiedActorDefinition);
