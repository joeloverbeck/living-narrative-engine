...
describe('Entity Factory E2E Workflow', () => {
  ...
  describe('Factory Service Initialization', () => {
    it('should initialize factory services with proper dependency resolution', async () => {
      ...
      const container = testBed.container;
      
      // Import factory classes for validation
      const EntityFactory = (await import('../../../src/entities/factories/entityFactory.js')).default;
      const EntityValidationFactory = (await import('../../../src/entities/factories/EntityValidationFactory.js')).default;
      const EntityConstructionFactory = (await import('../../../src/entities/factories/EntityConstructionFactory.js')).default;
      const EntityDefinitionLookupFactory = (await import('../../../src/entities/factories/EntityDefinitionLookupFactory.js')).default;
  ...
  describe('Definition Lookup and Caching', () => {
    ...
    it('should optimize lookup performance for large definition sets', async () => {
      ...
      expect(avgWarmLookup).toBeLessThan(avgColdLookup); // Warm faster than cold
      expect(avgWarmLookup).toBeLessThan(5); // Warm lookups < 5ms
      expect(speedupFactor).toBeGreaterThan(1.0); // At least some speedup from caching (very relaxed)
