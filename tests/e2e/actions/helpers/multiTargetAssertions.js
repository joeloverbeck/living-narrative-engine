...
export const multiTargetAssertions = {
  ...
  expectTargetsProcessed(result, expectedTargets) {
    ...
    for (const [role, expectedId] of Object.entries(expectedTargets)) {
      ...
      expect(resolvedTargets[role].id || resolvedTargets[role]).toBe(
        expectedId
      );
      ...
      if (Object.keys(processedTargets).length > 0) {
        expect(
          processedTargets[role] || processedTargets[expectedId]
        ).toBeTruthy();
      }
    }
  },
    ...
    for (const [entityId, expectedComponents] of Object.entries(
      ...
      for (const [componentId, expectedValues] of Object.entries(
        ...
        if (actualChange.after) {
          ...
          expect(stateChanges[entityId][componentId]).toMatchObject(
            expectedValues
          );
        }
      }
  ...
  toFormValidPattern() {
    return {
      toFormValidPattern(received, patternType, centerPosition) {
        const pass = validateFormationPattern(
          received,
          patternType,
          centerPosition
        );
