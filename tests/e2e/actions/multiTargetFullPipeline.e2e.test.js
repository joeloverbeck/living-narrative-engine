...
describe('Multi-Target Action Full Pipeline E2E', () => {
  ...
  describe('Optional Target Support', () => {
    it('should handle optional targets correctly', async () => {
      // Load the optional targets action definition
      const actionPath = join(
        process.cwd(),
      ...
      const appleEntity = await entityTestBed.createEntity('basic', {
      ...
      const noteEntity = await entityTestBed.createEntity('basic', {
        instanceId: 'note_001',
        overrides: {
          'core:item': { name: 'Thank You Note' },
        },
      });

      const mockDiscoveryWithNote = [
        {
          actionId: actionDefinition.id,
          targets: {
            primary: { id: 'apple_001', displayName: 'Red Apple' },
          ...
          command: 'give Red Apple to Merchant with Thank You Note',
          available: true,
        },
      ];
