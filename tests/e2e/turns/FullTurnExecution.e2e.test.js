...
describe('E2E: Complete AI Turn Execution', () => {
  ...
  test('should execute full AI turn from decision to action', async () => {
    ...
    const expectedDecision = {
      ...
      speech: "I think I'll explore the market and see what stories await there!",
      thoughts: "The market sounds bustling and full of potential for new tales.",
      notes: [
        {
    ...
    console.log('turnResult.action.chosenIndex:', turnResult.action.chosenIndex);
    ...
    expect(turnResult.action.chosenIndex).toBeLessThanOrEqual(availableActions.length);
  ...
  test('should handle LLM configuration switching between strategies', async () => {
    ...
    const toolCallingResponse = testBed.createToolCallingResponse(toolCallingDecision);
    ...
    expect(turnResult1.action.chosenIndex).toBe(toolCallingDecision.chosenIndex); // Should match exactly
    ...
    const jsonSchemaResponse = testBed.createToolCallingResponse(jsonSchemaDecision);
    ...
    expect(turnResult1.action.chosenIndex).not.toBe(turnResult2.action.chosenIndex);
  });
  ...
  test('should handle LLM errors gracefully during turn execution', async () => {
    // Test Network Error
    const networkError = testBed.createErrorResponse(500, 'Internal Server Error');
    testBed.setDefaultLLMResponse(networkError);

    await expect(
      testBed.executeFullAiTurn('test-ai-actor')
    ).rejects.toThrow();
  ...
  test('should properly integrate all AI subsystems', async () => {
    ...
    const comprehensiveDecision = {
      ...
      thoughts: "A performance will lift everyone's spirits and might draw some interesting characters forward.",
      notes: [
        {
    ...
    expect(comprehensiveDecision.chosenIndex).toBeLessThanOrEqual(actionComposites.length);
    const chosenAction = actionComposites[comprehensiveDecision.chosenIndex - 1]; // Convert to 0-based for array access
  ...
  test('should generate valid prompts and estimate tokens correctly', async () => {
    ...
    const turnResult = await testBed.executeFullAiTurn('test-ai-actor', turnContext, availableActions);
