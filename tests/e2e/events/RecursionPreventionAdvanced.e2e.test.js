...
class RecursionTestBed extends IntegrationTestBed {
  ...
  async initialize() {
    ...
    console.warn = jest.fn((...args) => {
      this.capturedConsoleOutput.push({ level: 'warn', message: args.join(' '), timestamp: Date.now() });
      this.originalConsole.warn(...args);
    });
    
    console.error = jest.fn((...args) => {
      this.capturedConsoleOutput.push({ level: 'error', message: args.join(' '), timestamp: Date.now() });
  ...
  createRecursiveHandler(eventName, maxAttempts, options = {}) {
    ...
    return jest.fn(async () => {
      ...
      if (shouldThrowError && attemptCount === 2) {
        throw new Error(`Intentional error in recursive handler - attempt ${attemptCount}`);
      }
  ...
  getConsoleMessagesByLevel(level) {
    return this.capturedConsoleOutput.filter(output => output.level === level);
  }
...
describe('Advanced Recursion Prevention E2E', () => {
  ...
  describe('Standard Event Recursion Limits', () => {
    it('should enforce 10-depth limit for standard events', async () => {
      ...
      const recursiveHandler = testBed.createRecursiveHandler('test:standard_event', 15);
    ...
    it('should allow normal sequential dispatch after recursion limit hit', async () => {
      ...
      const recursiveHandler = testBed.createRecursiveHandler('test:recovery_event', 15);
      testBed.eventBus.subscribe('test:recovery_event', recursiveHandler);
      
      await testBed.eventBus.dispatch('test:recovery_event', { phase: 'recursion' });
      ...
      await testBed.eventBus.dispatch('test:recovery_event', { phase: 'recovery' });
  ...
  describe('Workflow Event Elevated Limits', () => {
    it('should allow higher recursion depth for workflow events', async () => {
      ...
      const workflowHandler = testBed.createRecursiveHandler(TURN_STARTED_ID, 25);
  ...
  describe('Component Lifecycle Extreme Limits', () => {
    it('should allow deep recursion for component lifecycle events', async () => {
      ...
      const componentHandler = testBed.createRecursiveHandler(COMPONENT_ADDED_ID, 50);
    ...
    it('should handle complex component cascades during entity creation', async () => {
      ...
      await testBed.eventBus.dispatch(ENTITY_CREATED_ID, { entityId: 'complex_entity' });
  ...
  describe('Batch Mode Custom Recursion Limits', () => {
    it('should respect custom recursion limits during batch mode', async () => {
      ...
      const batchHandler = testBed.createRecursiveHandler('test:batch_event', 20);
  ...
  describe('Progressive Warning System', () => {
    it('should issue warnings at 50%, 75%, and 90% of recursion limits', async () => {
      ...
      const warningHandler = testBed.createRecursiveHandler('test:warning_event', 12);
      testBed.eventBus.subscribe('test:warning_event', warningHandler);
      
      await testBed.eventBus.dispatch('test:warning_event', { warningTest: true });
      ...
      const hasPercentageWarnings = recursionWarnings.some(msg => 
  ...
  describe('Recovery Behavior After Limits Hit', () => {
    it('should continue normal operation after recursion limit exceeded', async () => {
      ...
      const recursiveHandler = testBed.createRecursiveHandler('test:recovery_test', 20);
      testBed.eventBus.subscribe('test:recovery_test', recursiveHandler);
      
      await testBed.eventBus.dispatch('test:recovery_test', { phase: 'recursion' });
      ...
      await testBed.eventBus.dispatch('test:normal_operation', { phase: 'recovery' });
    ...
    it('should handle errors in recursive handlers gracefully', async () => {
      ...
      const errorRecursiveHandler = testBed.createRecursiveHandler('test:error_recursive', 10, {
        shouldThrowError: true
      });
      
      testBed.eventBus.subscribe('test:error_recursive', errorRecursiveHandler);
      
      await testBed.eventBus.dispatch('test:error_recursive', { errorTest: true });
      ...
      await testBed.eventBus.dispatch('test:normal_after_error', { test: true });
  ...
  describe('Multi-Level Cascade Scenarios', () => {
    it('should handle complex multi-event type cascades correctly', async () => {
      ...
      // Start the cascade
      await testBed.eventBus.dispatch(ENTITY_CREATED_ID, { entityId: 'cascade_entity' });
