...
import { createEventBatch, createPerformanceTestEvents } from '../../common/eventTestHelpers.js';
...
class EventSystemTestBed extends IntegrationTestBed {
  ...
  verifyEventSequence(expectedEntityCount, expectedComponentsPerEntity = 3.5) {
    ...
    const componentAddedEvents = this.capturedEvents.filter(e => e.type === COMPONENT_ADDED_ID);
    
    expect(entityCreatedEvents).toHaveLength(expectedEntityCount);
    expect(componentAddedEvents.length).toBeGreaterThan(expectedEntityCount * 2); // At least 2 components per entity
  ...
  verifyNoRecursionWarnings() {
    const errorEvents = this.capturedEvents.filter(e => e.type === SYSTEM_ERROR_OCCURRED_ID);
    const recursionWarnings = errorEvents.filter(e => 
      e.payload && e.payload.message && e.payload.message.includes('recursion')
    );
...
describe('Batch Mode Game Loading E2E Test', () => {
  ...
  describe('Scenario 1: Complex Game State Loading', () => {
    it('should load complex game state with 100+ entities in batch mode without recursion warnings', async () => {
      ...
      testBed.logger.info(`Batch loading completed: ${executionTime}ms for ${eventStats.totalEvents} events`);
    });
  ...
  describe('Scenario 4: Error Handling During Batch Loading', () => {
    it('should handle errors gracefully while maintaining batch mode state', async () => {
      ...
      const newEntityEvents = newEvents.filter(e => e.type === ENTITY_CREATED_ID);
  ...
  describe('Scenario 5: Recursion Prevention', () => {
    ...
    it('should still enforce recursion limits appropriately in batch mode', async () => {
      ...
      const recursiveListener = jest.fn((event) => {
        ...
        if (events.length < 50) { // Attempt way more than the limit
  ...
  describe('Performance and Integration', () => {
    ...
    it('should integrate properly with SafeEventDispatcher', async () => {
      ...
      const entityEvents = testBed.capturedEvents.filter(e => e.type === ENTITY_CREATED_ID);
