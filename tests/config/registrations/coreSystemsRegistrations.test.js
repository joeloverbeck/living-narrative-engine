  ...
  const container = {
    ...
    resolve: jest.fn((token) => {
      ...
      if (!registration) {
        ...
        if (token === tokens.EventBus)
          return { on: jest.fn(), off: jest.fn(), emit: jest.fn() };
        if (token === tokens.EntityManager)
          return {
            /* basic mock */
          };
        if (token === tokens.BlockerSystem)
          return {
            /* basic mock */
          };
        if (token === tokens.MovementSystem)
          return {
            /* basic mock */
          };
        if (token === tokens.GameDataRepository)
          return {
            /* basic mock */
          };
        if (token === tokens.IWorldContext)
          return {
            /* basic mock */
          };
        if (token === tokens.IActionExecutor)
          return {
            /* basic mock */
          };
        if (token === tokens.ConditionEvaluationService)
          return {
            /* basic mock */
          };
        if (token === tokens.ItemTargetResolverService)
          return {
            /* basic mock */
          };
        ...
        if (token === tokens.ActionValidationService)
          return {
            /* basic mock */
          };
        if (token === tokens.ITurnOrderService)
          return {
            /* basic mock */
        ...
        if (token === tokens.ICommandProcessor)
          return {
            /* basic mock */
          }; // Added for PlayerTurnHandler/AITurnHandler
        // --- Added missing mocks potentially needed by factories ---
        if (token === tokens.PlayerTurnHandler)
          return {
            /* basic mock */
          };
        if (token === tokens.AITurnHandler)
          return {
            /* basic mock */
          };
        if (token === tokens.TurnHandlerResolver)
          return {
            /* basic mock */
          };
        ...
        console.warn(
          `Mock Resolve Warning: Token not explicitly registered or mocked, returning basic object: ${String(token)}`
        );
      ...
      if (
        typeof registration.factoryOrValue === 'function' &&
    ...
    resolveByTag: jest.fn((tag) => {
      // Renamed to match AppContainer method
      const found = [];
...
describe('registerCoreSystems', () => {
  ...
  const initializableSystemTokens = [
    tokens.GameRuleSystem,
    tokens.EquipmentEffectSystem,
    tokens.EquipmentSlotSystem,
    tokens.InventorySystem,
    tokens.CombatSystem,
    tokens.DeathSystem,
    tokens.WorldPresenceSystem,
    tokens.BlockerSystem,
    tokens.MovementSystem,
    tokens.MoveCoordinatorSystem,
    tokens.PerceptionSystem,
    tokens.NotificationUISystem,
