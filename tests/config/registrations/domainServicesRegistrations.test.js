...
jest.mock('../../../src/actions/validation/actionValidationService.js', () => ({
  ...
  ActionValidationService: jest
    .fn()
    .mockImplementation(() => mockActionValidationServiceInstance),
}));
...
jest.mock('../../../src/turns/order/turnOrderService.js', () => ({
  __esModule: true,
  TurnOrderService: jest.fn(),
}));
...
const createMockContainer = () => {
  ...
  const registerSpy = jest.fn((token, factoryOrValueOrClass, options = {}) => {
    ...
    if (!token) {
      console.error('[Mock Register ERROR] Falsy token received!', {
        token,
  ...
  const resolveSpy = jest.fn((token) => {
    const tokenString = String(token);
    const lifecycleFromRegistration =
      registrations.get(token)?.options?.lifecycle;
    const isSingleton =
      lifecycleFromRegistration === 'singleton' ||
      lifecycleFromRegistration === 'singletonFactory';
    ...
    if (!registrationData) {
      ...
      if (token === tokens.IValidatedEventDispatcher)
      ...
      if (token === tokens.IPlayerPromptService)
      ...
      const registeredTokens = Array.from(registrations.keys())
        ...
        .join(', ');
      console.error(
        `[Mock Resolve Error] Token not registered or explicitly mocked via fallback: ${tokenString}. Registered: [${registeredTokens}]`
      );
      throw new Error(
        `Mock Resolve Error: Token not registered or explicitly mocked: ${tokenString}. Registered: [${registeredTokens}]`
      );
    }
    ...
    if (isClassRegistration) {
      ...
      deps.forEach((depTokenOrConfig) => {
        ...
        try {
          ...
          console.error(
            `[Mock Resolve - Class Dep Error] Failed to resolve dependency "${String(depToken)}" (for prop "${propName}") needed by "${targetClassName}" (${tokenString})`,
            e
          );
          throw new Error(
            `Failed dependency resolution for "${targetClassName}": ${e.message}`,
            { cause: e }
          );
        }
      });
      try {
        if (
          jest.isMockFunction(ClassConstructor) &&
          !ClassConstructor.prototype?.constructor
        ) {
        ...
        console.error(
          `[Mock Resolve - Class Instantiation Error] Constructor error for mocked "${targetClassName}" (${tokenString})`,
          constructorError
        );
        throw new Error(
          `Error constructing mock for ${targetClassName}: ${constructorError.message}`,
      ...
      try {
        ...
        console.error(
          `[Mock Resolve - Factory Execution Error] Error executing factory for ${tokenString}:`,
          factoryError
        );
        throw factoryError;
      }
...
describe('registerDomainServices', () => {
  ...
  beforeEach(() => {
    ...
    mockContainer.register(tokens.ICommandParser, mockCommandParserInstance, {
      lifecycle: 'singleton',
    ...
    mockContainer.register(
      ...
      mockPromptOutputPortInstance,
      { lifecycle: 'singleton' }
    ); // For PlayerPromptService factory
    mockContainer.register(tokens.IWorldContext, mockWorldContextInstance, {
      lifecycle: 'singleton',
    }); // For factories
    mockContainer.register(
      tokens.TargetResolutionService,
      mockTargetResolutionServiceInstance,
      { lifecycle: 'singleton' }
    ); // For CommandProcessor factory
    ...
    Object.values(mockJsonLogicServiceInstance).forEach((fn) =>
      fn.mockClear?.()
    );
    ...
    Object.values(mockTargetResolutionServiceInstance).forEach((fn) =>
      fn.mockClear?.()
    );
    Object.values(mockActionValidationServiceInstance).forEach((fn) =>
      fn.mockClear?.()
    );
    ...
    Object.values(mockPlayerPromptServiceInstance).forEach((fn) =>
      fn.mockClear?.()
    );
  });
  ...
  it('resolving TurnOrderService does not throw and returns instance', () => {
    ...
    expect(TurnOrderService).toHaveBeenCalledWith(
      expect.objectContaining({
        logger: mockLogger,
      })
    );
