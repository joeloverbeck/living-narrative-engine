  ...
  const container = {
    ...
    resolve: jest.fn((token) => {
      ...
      if (options?.lifecycle === 'singletonFactory') {
        ...
        if (typeof factoryOrValue === 'function') {
          // Assume ALL singletonFactories are functions needing the container
          try {
            ...
            console.error(
              `Mock container: Error executing singletonFactory for ${String(token)}: ${e.message}\n${e.stack}`
            );
            throw e;
          }
        } else {
          ...
          console.error(
            `Mock container: Invalid registration for singletonFactory ${String(token)}. Expected a function.`
          );
          throw new Error(
            `Invalid singletonFactory registration for ${String(token)}.`
          );
        }
      ...
      // Handles classes, simple factories (no args), or direct values registered as singletons.
        ...
        if (
          ...
          jest.isMockFunction(factoryOrValue) &&
          factoryOrValue.prototype &&
          factoryOrValue.prototype.constructor === factoryOrValue
        ) {
          try {
            instance = new factoryOrValue(); // Instantiate mock class
          } catch (e) {
            console.error(
              `Mock container: Error invoking Jest mocked constructor for ${String(token)}: ${e.message}\n${e.stack}`
            );
            throw e;
          }
        }
        // Check if it's a non-mock CLASS constructor (e.g., register(token, RealClass))
          ...
          try {
            ...
            console.warn(
              `Mock container: Attempting to instantiate non-mock singleton class ${String(token)}. Ensure dependencies are globally mocked or it has a no-arg constructor.`
            );
            instance = new factoryOrValue();
          } catch (e) {
            console.error(
              `Mock container: Error instantiating non-mock class for singleton ${String(token)}: ${e.message}\n${e.stack}`
            );
            throw e;
          }
        }
        // Check if it's a SIMPLE factory (no args)
          ...
          try {
            ...
            console.error(
              `Mock container: Error executing simple factory for singleton ${String(token)}: ${e.message}\n${e.stack}`
            );
            throw e;
          }
        }
      ...
      // Simplified: Create new instance for functions, return value otherwise
        ...
        if (
          ...
          try {
            ...
            console.error(
              `Mock container: Error executing transient factory for ${String(token)}: ${e.message}\n${e.stack}`
            );
            throw e;
          }
        }
    ...
    resolveAll: jest.fn((tag) => {
      ...
      registrations.forEach((reg, token) => {
        if (
          reg.options &&
          Array.isArray(reg.options.tags) &&
          reg.options.tags.includes(tag)
        ) {
          try {
            resolved.push(container.resolve(token));
          } catch (e) {
            console.warn(
              `Mock resolveAll: Failed to resolve token ${String(token)} with tag ${tag}: ${e.message}`
            );
          }
        }
      });
...
describe('registerRuntime', () => {
  ...
  beforeEach(() => {
    ...
    mockContainer.register(tokens.GameDataRepository, mockGameDataRepository, {
      lifecycle: 'singleton',
    });
    ...
    mockContainer.register(tokens.IWorldContext, GameStateManagerMock, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.ICommandParser, CommandParserMock, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.ICommandProcessor, CommandProcessorMock, {
      lifecycle: 'singleton',
    });
    ...
    mockContainer.register(
      ...
      TurnHandlerResolverMock,
      { lifecycle: 'singleton' }
    );
    mockContainer.register(tokens.PlayerTurnHandler, PlayerTurnHandler, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.ITurnManager, TurnManager, {
      lifecycle: 'singleton',
    });

    // UI Mocks
    mockContainer.register(tokens.IInputHandler, mockInputHandler, {
      lifecycle: 'singleton',
    });

    // Simple object mocks for remaining dependencies
    mockContainer.register(
      tokens.ActionValidationService,
    ...
    Object.values(mockActionValidationService).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockTargetResolutionService).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockPayloadValueResolverService).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockPrerequisiteEvaluationService).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockDomainContextCompatibilityChecker).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockJsonLogicEvaluationService).forEach((fn) =>
      fn?.mockClear?.()
    );
    Object.values(mockActionValidationContextBuilder).forEach((fn) =>
      fn?.mockClear?.()
    );
  ...
  it('should register runtime services without throwing errors', () => {
    ...
    const inputSetupCall = registerCalls.find(
      (call) => call[0] === tokens.InputSetupService
    );
  ...
  it('resolving InputSetupService does not throw', () => {
    ...
    expect(InputSetupService).toHaveBeenCalledWith(
    ...
    expect(mockContainer.resolve).not.toHaveBeenCalledWith(
      tokens.ICommandProcessor
    );
    expect(mockContainer.resolve).not.toHaveBeenCalledWith(
      tokens.IInputHandler
    ); // InputHandler is resolved later by InputSetupService.configureInputHandler
  });
});
