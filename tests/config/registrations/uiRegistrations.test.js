...
const setupDomMocks = () => {
  ...
  mockLocationInfoContainer.append(
    mockNameDisplay,
    ...
    .mockImplementation(function (selector) {
      ...
      switch (selector) {
        ...
        case '#location-name-display':
          return mockLocationInfoContainer.querySelector(
        ...
        case '#location-exits-display':
          return mockLocationInfoContainer.querySelector(
            '#location-exits-display'
          );
        case '#location-characters-display':
          return mockLocationInfoContainer.querySelector(
            '#location-characters-display'
          );
  ...
  const resolveSpy = jest.fn((token) => {
    ...
    if (registration) {
      ...
      const isFactoryFunction =
        typeof factoryOrValue === 'function' && !isClassForSingle;
      ...
      if (isClassForSingle) {
        ...
        const resolvedDeps = options.dependencies.map((depToken) => {
          try {
            ...
            console.error(
              `[Mock Resolve - isClassForSingle] Failed to resolve dependency '${String(depToken)}' for '${ClassConstructor.name}'`
            );
            throw resolveError;
          }
        });
        try {
          ...
          console.error(
            `[Mock Resolve - isClassForSingle] Error constructing '${ClassConstructor.name}' with deps object:`,
            constructorError
          );
        ...
        try {
          ...
          console.error(
            `[Mock Resolve - Factory] Error executing factory for token ${tokenString}:`,
            factoryError
          );
          throw factoryError;
        }
      ...
      if (
        (lifecycle === 'singleton' ||
          ...
          lifecycle === 'single' ||
          (options.isInstance && lifecycle === 'singleton')) &&
        instance !== undefined
      ) {
    ...
    throw new Error(
      `Mock Resolve Error: Token not registered or explicitly mocked: ${tokenString}`
    );
  });
...
describe('registerUI (with Mock Pure JS DI Container and Mocked Dependencies)', () => {
  ...
  beforeEach(() => {
    ...
    mockContainer.register(tokens.EventBus, mockEventBus, {
      lifecycle: 'singleton',
    }); // For ISafeEventDispatcher
    mockContainer.register(tokens.ISafeEventDispatcher, mockEventBus, {
      lifecycle: 'singleton',
    }); // Explicitly for EngineUIManager
    mockContainer.register(tokens.IEntityManager, mockEntityManagerService, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.IDataRegistry, mockDataRegistryService, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.ISaveLoadService, mockSaveLoadService, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.ILLMAdapter, mockLlmAdapter, {
      lifecycle: 'singleton',
    });
    // +++ PRE-REGISTER EntityDisplayDataProvider +++
    mockContainer.register(
      tokens.EntityDisplayDataProvider,
      mockEntityDisplayDataProviderService,
      { lifecycle: 'singleton' }
    );
  ...
  it('should register IDocumentContext via singletonFactory resolving to DocumentContext', () => {
    registerUI(mockContainer, mockUiArgs);
    const iDocContextReg = mockContainer._registrations.get(
  ...
  it('should register UiMessageRenderer using single()', () => {
    ...
    const regCall = mockContainer.register.mock.calls.find(
      (call) => call[0] === tokens.UiMessageRenderer
    );
  ...
  it('should register TitleRenderer via singletonFactory injecting titleElement', () => {
    ...
    const factoryFn = mockContainer._registrations.get(
  ...
  it('should register LocationRenderer via singletonFactory querying for its container', () => {
    ...
    expect(mockLogger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining(
        "Could not find '#location-info-container' element for LocationRenderer"
      )
    );
  ...
  it('should register ActionButtonsRenderer via singletonFactory querying for container', () => {
    ...
    expect(mockLogger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining(
        "Could not find '#action-buttons' element for ActionButtonsRenderer"
      )
    );
  });
  ...
  it('should register DomUiFacade under its own token using single()', () => {
    registerUI(mockContainer, mockUiArgs);
    const regCall = mockContainer.register.mock.calls.find(
      (call) => call[0] === tokens.DomUiFacade
    );
