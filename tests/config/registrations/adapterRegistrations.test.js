    ...
    resolve: jest.fn((token) => {
      ...
      if (options?.lifecycle === 'singletonFactory') {
        ...
        if (typeof factoryOrValue !== 'function') {
          throw new Error(
            `Mock Resolve Error: singletonFactory for ${String(token)} is not a function.`
          );
        }
        ...
        if (
          ...
          factoryOrValue.name === factoryOrValue.prototype.constructor.name
        ) {
...
describe('Adapter Registrations - registerAdapters', () => {
  ...
  beforeEach(() => {
    ...
    mockContainer.register(tokens.ISchemaValidator, mockSchemaValidator, {
      lifecycle: 'singleton',
    });
    mockContainer.register(tokens.IConfiguration, mockConfiguration, {
      lifecycle: 'singleton',
    });
  });
  ...
  it('should register EventBusTurnEndAdapter with ISafeEventDispatcher if available, and logger', () => {
    ...
    expect(constructorArgs.validatedEventDispatcher).toBe(
      mockValidatedEventDispatcher
    );
  ...
  it('should correctly register EventBusCommandInputGateway', () => {
    ...
    expect(EventBusCommandInputGateway.mock.calls[0][0]).toEqual({
  ...
  it('should pass the resolved logger to ConfigurableLLMAdapter and its direct dependencies during its registration', () => {
    ...
    expect(mockClientApiKeyProviderConstructor).toHaveBeenCalledWith(
    ...
    expect(mockRetryHttpClientConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ logger: mockLogger })
    );
    ...
    expect(mockLLMStrategyFactoryConstructor).toHaveBeenCalledWith(
    ...
    const adapterConstructorArgs =
      mockConfigurableLLMAdapterConstructor.mock.calls[0][0];
    ...
    expect(adapterConstructorArgs.llmStrategyFactory).toEqual(
      expect.any(Object)
    ); // Instance from mockLLMStrategyFactoryConstructor
    ...
    expect(llmAdapterInstance.isOperational).toBe(
      mockConfigurableLLMAdapterIsOperational
    );
  });
